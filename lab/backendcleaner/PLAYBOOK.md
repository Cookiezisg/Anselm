...
## 3. 开工先做依赖 + 复用扫描（backend-new 语境）

- **复用先于造轮子**：动工前盘点 `pkg/*`(idgen/orm/pagination/reqctx/tokencount/…) 与 `infra/*` 既有能力——能直接复用就复用；若业务层将要手搓的样板本应由地基提供（如 orm 漏翻译 UNIQUE 冲突 → 补在 orm），**强化地基**而非在模块内重抄。错误抽象 / 重复样板比多写一行更糟。
