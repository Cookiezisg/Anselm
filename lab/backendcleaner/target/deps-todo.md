...
| **工具适配器全景** | 各波次 | ✅ 已建：filesystem/search/web/toolset（2.3）· function/handler/trigger/agent/skill/mcp（波次 3 各实体域）· **memory/document/shell（M3.7 R0044）**。⏸ 待后续波次：todo（chat M5.2，SystemReminder 耦合）· subagent（波次 5）· workflow（M4.5）· ask（波次 6）。**shell 进 `Toolset.Resident`**（同 filesystem/search，chat M5.2 host 装）；其余实体工具进 `Toolset.Lazy` |
