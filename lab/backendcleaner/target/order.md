...
| ⚠️ R0051 | `attachment`（M5.2 前置子模块，用户加） | sandbox（R0053 提取）· llm（R0052 注入）| 多模态附件：CAS blob 存储 + 元数据 + 4 端点。抄 LibreChat 最全流水线 + 3 升级（**CAS 内容寻址** / **sandbox 当本地提取引擎** / **多 provider 中立注入**）；**无 RAG**。**R0051 存储核心 ✅**；主线 图/文本/PDF/Office 扎实做、音频/视频/OCR 经可插 Extractor 端口留插槽延后；**R0052 多 provider 注入 ⬜ + R0053 sandbox 提取 ⬜** 续 |
