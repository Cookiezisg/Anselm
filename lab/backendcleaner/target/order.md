...
| M5.2 | `chat`（runner 庞大，重点审，**拆 3 子轮**） | document, loop, tool, **messages(R0054)**, attachment, mention, memory | hooks/permissionsgate 已解散（M1.9 R0027），危险控制下放工具。**子轮**：**R0054 messages 持久化地基 ✅**（domain `Message` + `Repository` 5 法两段式 + `infra/store/messages` 两表 + seq 单调；Block 补 workspace_id；归 domain/messages）→ **R0055 chat Service+host+runner**（chatHost 实 loop.Host〔LoadHistory 拼历史〔LoadThread + conv.Summary + attachment ToContentParts + mention 渲染〕/ Tools〔toolset + AutoActivator + ReminderProvider todo〕/ WriteFinalize〔FinalizeMessage + message_stop〕〕 + convQueue〔per-conv 容量 5 + idle GC + STREAM_IN_PROGRESS〕 + System Prompt〔Section + locale〕 + message_start 发射 + reqctx 埋 msgID/convID/Bridge + Detached 终态 + model resolve〔conv.ModelOverride→workspace 默认〕）→ **R0056 chat HTTP + auto-title + 跨域接线**（Send 202/List/Cancel 204/Export/llm-trace/system-prompt-preview + mention 注册表 + compaction 钩子〔contextmgr M5.3〕 + conversation tokensUsed 富化〔R0050 deferred〕） |
