...
| ✅ R0053 | `attachment`（M5.2 前置子模块，用户加，**3 轮收官**） | sandbox（R0053 提取）· llm（R0052 注入）| 多模态附件全链路：CAS blob 存储 + 元数据 + 4 端点。抄 LibreChat 最全流水线 + 3 升级（**CAS 内容寻址** / **sandbox 当本地提取引擎** / **多 provider 中立注入**）；**无 RAG**。**R0051 存储核心 ✅**（domain/blob CAS/orm/app/4 端点）。**R0052 多 provider 注入 ✅**（11 家逐一对官方文档渲染：原生内联 PDF 仅 anthropic/openai/gemini、7 家 image_url data-URL + PDF 降级、ollama images[]、moonshot content 升 RawMessage）。**R0053 sandbox 提取 ✅**（可插 `Extractor` 端口 + `SandboxExtractor` 跑 python pdfplumber/python-docx/openpyxl/python-pptx；sandbox 加 attachment owner kind；`ToContentParts(att_ids, Capabilities{Vision,NativeDocs})` document = native?file:抽文本截断；音频/视频/OCR 留独立 extractor 延后；无新 REST/码）。**子模块完整收官 🎉** → **M5.2 接线**（chat 拼文本 part + 调 ToContentParts 传 Capabilities、model 目录补 vision/nativeDocs flag、消息持久化抽取结果） |
| M5.2 | `chat`（runner 庞大，重点审，**拆 3 子轮**） | document, loop, tool, **messages(R0054)**, attachment, mention, memory | hooks/permissionsgate 已解散（M1.9 R0027），危险控制下放工具。**子轮**：**R0054 messages 持久化地基 ✅**（domain `Message` + `Repository` 5 法两段式 + `infra/store/messages` 两表 + seq 单调；Block 补 workspace_id；归 domain/messages）→ **R0055 chat 引擎核心 ✅**（chatHost 实 loop.Host〔LoadHistory〔LoadThread + conv.Summary + attachment ToContentParts + assistant 投影〕/ Tools〔resident + search_tools + discovered lazy〕/ WriteFinalize〔FinalizeMessage + message_stop，Detached〕 + AutoActivator + ReminderProvider〕 + convQueue〔容量 5 + idle GC + STREAM_IN_PROGRESS + cancel 存储〕 + Send〔202〕 + System Prompt〔10 Section，静态段重写〕 + SSE message 节点 + model resolve〔ResolveChat〕 + DIP Deps；**mention 渲染移到 R0056**；fake LLM 端到端 5 测）→ **R0056 chat 可用面 + mention ✅**（HTTP handler〔Send 202 / List Paged N4 / Cancel 204〕 + `Service.Cancel`〔queue.cancel + drain + finalizeCancelled〕 + mention 整套〔注册表 + freeze-on-send + `<mentions>` 渲染 + 补 workflow/agent resolver，5 类全在〕 + ListMessages 透传；chat 端到端可用；+4 测试）→ **R0057 chat 收尾 ✅·收官**（auto-title〔首回合 detached utility Generate + SetAutoTitle〔不覆盖用户标题〕 + notify〕 + `GET /usage`〔tokensUsed，messages.SumTokens，专用端点解耦〕 + `GET /system-prompt-preview`；**砍 export/llm-trace**〔投机〕；model 目录 vision/nativeDocs flag 属 model 域延后）。**chat 模块完整收官 🎉**（R0054+R0055+R0056+R0057） |
| ✅ R0058 | `subagent` + `tool/subagent` | chat, loop, tool, messages | **递归子对话 ✅**：app/subagent（registry 3 类型 Explore/Plan/general-purpose + `Spawn` 实现 SubagentRunner + **混血 host**〔agentHost prompt/白名单 + chatHost 落盘/message_stop〕 + emit）+ `Subagent` 工具（防递归 Execute）。**持久化 B**：Message += SubagentID（sub-message 落父对话 + Attrs.parentBlockId；chat LoadHistory 排除、ListMessages 返回供 reload 树）。**E3**：loop ToolCallID 种子 + subagent message 节点 parentID=tool_call。model 承袭=自有 Resolver（workspace dialogue 默认；override 承袭延后）。防递归=SubagentID 存在性 + filterTools 剔 Subagent（深度 1）。无 catalog/relation/REST/新码；DOC-123 整篇重写（砍 cv_xxx/深度2/沙箱/4 虚构码）。9 测试绿。M7 装配：skill 注入 runner + 工具入 Toolset + Deps 注真 |
