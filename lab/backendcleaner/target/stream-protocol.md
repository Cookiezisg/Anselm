...
> ⚠️ **词表暂定**：协议骨架（信封 / 四动词 Frame / Node interface）已定稿稳定；下面具体词表（有哪些 node、各带什么字段）是依设计推演的**初版，非契约**。到各 producer 模块接线那一轮（order.md 各自波次）依实际需求重新确定。代码里 `nodes.go` 顶部同标 PROVISIONAL。
