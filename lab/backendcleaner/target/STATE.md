...
- **波次0 地基**：M0.1 pkg 🔧（**reqctx/idgen/pagination ✅** R0001；余 tokencount·pathguard·userpath·wikilink·jsonrepair·limits + `modelcaps` 残留判定）· M0.2 db ⬜ · M0.3 logger/crypto ⬜ · M0.4 errors/eventlog/notif(domain) ⬜ · M0.5 infra eventlog/notif/chat ⬜ · M0.6 llm ⬜ · M0.7 transport 框架 ⬜
...
- **M0.1 续**：重写其余 pkg 纯工具（tokencount/pathguard/userpath/wikilink/jsonrepair/limits），逐个判定 `modelcaps`（modelcatalog 取代→大概率删）等残留。然后 M0.2 `infra/db`。
