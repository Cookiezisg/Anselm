...
- **波次0 地基**：M0.1 pkg ✅（**reqctx/idgen/pagination ✅** R0001；**tokencount ✅** R0002；**pathguard ✅** R0003；**userpath ⏭️删** R0004；**wikilink ✅** R0005；**jsonrepair ✅** R0006；**limits ✅** R0007；modelcaps/modelcatalog 移交 M1.3）· M0.2 数据库层：**pkg/orm ✅ R0008** · db 网关 ⬜ · M0.3 logger/crypto ⬜ · M0.4 errors/eventlog/notif(domain) ⬜ · M0.5 infra eventlog/notif/chat ⬜ · M0.6 llm ⬜ · M0.7 transport 框架 ⬜
...
- **M0.2 续（db 网关）**：`infra/db` 用 `database/sql` + `glebarez/go-sqlite` 重写（连接/WAL/pragma/单连接 + 手写 schema DDL 对齐 `database.md`，取代 GORM AutoMigrate）。`modelcaps`/`modelcatalog` 移交 M1.3。
