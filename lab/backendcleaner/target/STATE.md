...
- **波次0 地基**：M0.1 pkg ✅（**reqctx/idgen/pagination ✅** R0001；**tokencount ✅** R0002；**pathguard ✅** R0003；**userpath ⏭️删** R0004；**wikilink ✅** R0005；**jsonrepair ✅** R0006；**limits ✅** R0007；modelcaps/modelcatalog 移交 M1.3）· M0.2 数据库层 ✅（**pkg/orm R0008 · db 网关 R0009**；业务表 DDL 分散各模块）· M0.3 ✅（**logger R0010 · crypto R0011**）· M0.4 ✅：**errors R0012** · **stream 统一协议 R0013**（单一 domain/stream：信封+四动词Frame+通用 Node{Type,Content}+Bridge/ListReader；词表下放业务）· M0.5 ✅ infra **stream bus（单一 Bus）R0014**（实例化三次=三流；frame 分级；D2 全量推；infra/chat extractor 移交 M5.2）· M0.6 llm **框架+openai R0015 · anthropic R0016** 🔧（剩 9 provider：gemini + 8 OpenAI-compat）· M0.7 transport 框架 ⬜
...
- **M0.6 续（R0016）**：`infra/llm` 其余 10 provider，**每家完整自包含 wire（不共享基座）**——anthropic/gemini 原生方言 → deepseek/qwen/zhipu/moonshot/doubao/openrouter/ollama/custom 8 家 OpenAI-compat 各写各的 + golden，逐家加 registry。`modelcaps`/`modelcatalog` 移交 M1.3。
