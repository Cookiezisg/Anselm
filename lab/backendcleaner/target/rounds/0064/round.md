...
| 期 | 内容 | 状态 |
|---|---|---|
| **H0** | `humanloop` 包：Broker + Request/Resolve/IsAllowed/Allow + ctx + 类型 + 测 | ✅ `80bb6bc4` |
| **H1 danger** | loop runOneTool 门控（broker 在 ctx 时，fail-safe）+ chat seed broker/surface + resolve 端点 + always-allow + 测 | ✅ `a37156e7` |
| **H2 ask** | `ask_user` 工具（Execute 调 Request、普通工具无 loop 改动）+ 测 | ✅ `6d40274a` |
| **H3 列表/重连** | `GET /conversations/{id}/interactions`（列 pending）| ✅ 并入 H1（PendingInteractions + 端点） |
| 嵌套证明 | agent 包测试：broker 经 ctx 流进子 agent loop、撞同一门、resolve 解阻、零冒泡 | ✅ `9d4ad6d8` |
| 文档 | events.md §1（interaction signal + resolve）+ api.md（2 端点）+ domains/agent + contract #48 + lab。**DB 无变更**（内存、不动 schema）。**CLAUDE.md S18 无需改**（已写「危险靠 LLM 自报 + 逐次确认」，H1 正是那个逐次确认） | ✅ 本提交 |

**R0064 全完成**（内存阻塞版）：H0 broker → H1 danger → H2 ask → 嵌套自动就对。durable park 版（D0–D4）做过、用户质疑后整体回退（`5137cd46`）。
