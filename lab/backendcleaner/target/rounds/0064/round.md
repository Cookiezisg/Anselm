...
- **messages 流**（非 notifications）：park = `message_stop` 带 `status="parked"`/`stop_reason="parked"` + 被门调用挂 `tool_result` 占位块（`status="pending"`、`attrs.park=danger|ask|agent`）。park 的 tool_call 已带 danger+summary（S18，前端显示）。resolve 时填那条 pending（`block_stop` 关它）+ 驱动续跑回合（新 message 经 messages 流）。嵌套子 agent 的 leaf 经 B5 嵌在 invoke_agent tool_call 下。
- **收件箱 = 查 `parked` 的 message / execution 行**（无投影表，同 approval）。**未建 notifications-inbox**（events.md §2 早先投机的 ask 行已删）——v1 内联呈现足够；跨对话待办收件箱留后续。
...
| 期 | 内容 | 状态 |
|---|---|---|
| **D0** | loop park 原语：`InteractiveTool` 接口 + `runTools` park 判定/返回 + Parked Result + pending tool_result 块 status；message CHECK 加 `parked`；domain status 常量 + 测 | ✅ `a8a602ac` |
| **D1 danger（chat）** | danger 门控 park；resolve endpoint（approve=execute-at-resolve / deny=反馈）；续跑驱动 + 测 | ✅ `5897af65` |
| **D2 ask（chat）** | `ask_user` 工具（InteractiveTool, message+options）；三态 resolve（accept/decline/cancel）；续跑 + 测 | ✅ `f983d9c8` |
| **D3a agent run 可 park/恢复** | `agent_executions` status=parked + `parkableAgentHost`（仅交互 chat/manual）+ `ResumeExecution`（填 leaf + replay 重驱）+ UpdateExecution + 独立 REST 端点 | ✅ `f405316e` |
| **D3b 嵌套冒泡** | `ParkSignal`（工具 Execute 发起 park）+ runTools 末尾扫统一收 parks + invoke_agent 传播 + chat 级联 resolve（resolveAgentInteraction）+ AgentResumer 端口/适配器 | ✅ `ad7efb71` |
| **D4 always-allow** | approve_always 会话级工具白名单（sync.Map，deny-first）；chatHost.AllowsTool 读它 | ✅ `95f6f9a1` |
| 文档 | events.md §1（park/resolve as-built）+ §2 去投机 ask 行 + api.md（2 interactions 端点）+ database（messages/agent_executions parked）+ domains/agent + contract #48 + lab。**S18 无需改**——它已写「危险靠 LLM 自报 + 逐次确认」，D1 正是实现那个「逐次确认」 | ✅ 本提交 |

**R0064 全完成**：D0 park 原语 → D1 danger（chat）→ D2 ask（chat）→ D3a/D3b agent invoke（含嵌套冒泡）→ D4 always-allow。研究确认全程业界标准 durable HITL。

### D3 子阶段（嵌套范围，各≈D1 量级）

- **D3a 独立 agent run 可 park/恢复**：`agent_executions` status=parked；agent runLoop 在交互调起（TriggeredBy=manual）时实现 `loop.ParkHandler`；recordExecution 写 parked transcript；resume 路径从 parked transcript 续跑 runLoop；agent-execution resolve 端点。自动语境（workflow/sensor/subagent）不实现 ParkHandler → 不 park、危险照跑。
- **D3b 嵌套冒泡**：新 `Execute→park` 信号——invoke_agent 工具的 Execute 发现内层 agent park 时返回 park 信号；`runTools` 在执行后识别它、转成 chat 回合的 park（pending tool_result 引用 {agentExecutionID, leafToolCallID}）。chat resolve 级联：先恢复子 agent（填 leaf + 续跑 runLoop 到完成或再 park）→ 子 agent 终果填 chat 的 invoke_agent pending → 续跑 chat。双层 pending 数据模型。
- **踩坑提醒**：danger 是安全敏感；execute-at-resolve 的 at-least-once 窗口（D1 §6）在 D3 同样适用；嵌套 resolve 的级联恢复要保证 exactly-once（子 agent 已完成步记忆化在 transcript，续跑不重跑——同 D1）。
