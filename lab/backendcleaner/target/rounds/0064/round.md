...
| **D3 agent invoke**（决定 2026-06-10：**嵌套也做**）| 见下 D3a/D3b | ⏳ 待做 |
| **D4** | always-allow 会话白名单（deny-first 优先级；chatHost.allowsToolForConversation 现 stub）| ⏳ 待做（v1） |
| 文档 | events.md §1/§2（ask/danger as-built）+ api.md（interactions 端点）+ database（parked status）+ domains（chat/agent ask-danger）+ contract-changes。**S18 无需改**——它已写「危险靠 LLM 自报 + 逐次确认」，D1 正是实现那个「逐次确认」 | ⏳ 待做（PLAYBOOK ④）|
