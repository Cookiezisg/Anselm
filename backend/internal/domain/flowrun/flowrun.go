...
const (
	...
	StatusPaused         = "paused" // legacy (old scheduler); replaced by awaiting_signal in M4
...
type FlowRun struct {
	...
	// Durable-execution columns (M1; 17 §1). PinnedCallables = transitive callable→version
	// closure snapshot (A-5/ADR-020); Generation = replay-reset epoch (ADR-019).
