...
// flowrun's events in seq order — the input to deterministic replay. EventsForNode returns events
// for a specific (nodeId, iterationKey, generation) tuple, used by the durable timer gate.
//
// JournalRepository 是 append-only journal port;撞 record-once 键 = no-op 返既有事件(first-wins)。
// EventsForNode 按 (nodeId, iterationKey, generation) 过滤，供 durable timer gate 使用。
type JournalRepository interface {
	...
	EventsForNode(ctx context.Context, flowrunID, nodeID string, iter, generation int) ([]FlowRunEvent, error)
}
