...
// schedulerapp.StartInput; KillWorkflow / CountRunning / CountOutstanding pass straight through. This is the D1
...
// 调度器。StartRun 把 (workflowID, payload) 映射成 schedulerapp.StartInput；KillWorkflow / CountRunning /
// CountOutstanding 直通。
...
func (a runnerAdapter) CountOutstanding(ctx context.Context, workflowID string) (int, error) {
	return a.sched.CountOutstanding(ctx, workflowID)
}
