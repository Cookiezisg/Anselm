...
func (h *HandlerHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	items, next, err := h.svc.List(r.Context(), handlerdomain.ListFilter{Cursor: p.Cursor, Limit: p.Limit, Search: r.URL.Query().Get("search")})
	if err != nil {
		responsehttpapi.FromDomainError(w, h.log, err)
		return
	}
...
func (h *HandlerHandler) GetVersion(w http.ResponseWriter, r *http.Request) {
	...
	v, err := h.svc.GetVersionForHandler(r.Context(), r.PathValue("id"), versionStr)
	if err != nil {
		responsehttpapi.FromDomainError(w, h.log, err)
		return
	}
