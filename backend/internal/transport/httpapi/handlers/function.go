...
// free-moving active pointer — no pending/accept endpoints. The :iterate verb opens an AI
...
// :iterate 动词经 aispawn 开一个 AI 对话来编辑本 function。
...
func (h *FunctionHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	items, next, err := h.svc.List(r.Context(), functiondomain.ListFilter{Cursor: p.Cursor, Limit: p.Limit, Search: r.URL.Query().Get("search")})
	if err != nil {
		responsehttpapi.FromDomainError(w, h.log, err)
		return
	}
...
// postOnFunction dispatches POST /functions/{id}:<action> (:run / :revert / :edit / :iterate).
//
// postOnFunction 派发 POST /functions/{id}:<action>（:run / :revert / :edit / :iterate）。
...
func (h *FunctionHandler) GetVersion(w http.ResponseWriter, r *http.Request) {
	...
	v, err := h.svc.GetVersionForFunction(r.Context(), r.PathValue("id"), versionStr)
	if err != nil {
		responsehttpapi.FromDomainError(w, h.log, err)
		return
	}
