...
func iterateEntity(w http.ResponseWriter, r *http.Request, log *zap.Logger, svc *aispawnapp.Service, mentionType mentiondomain.MentionType, id string) {
	...
	responsehttpapi.Success(w, http.StatusAccepted, map[string]string{"id": convID}) // 异步动作返新资源 id 统一 {id}
}

// TriageHandler serves the universal `:triage` verb: diagnose ANY execution record, not
...
//
// TriageHandler 提供通用 `:triage` 动词：诊断**任意**执行记录、不只工作流运行。统一的
