...
func (r *fakeAttachmentRenderer) ToolResultContentParts(ctx context.Context, toolCallID string, ids []string, caps ContentCapabilities) ([]llminfra.ContentPart, error) {
...
func TestBuildSystemPrompt_Sections(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Disambiguate skill intent by the user's verb",
		"when the user asks to create, author, write, or make a new skill, call create_skill directly",
		"activate_skill is only for an existing skill",
		"Opaque machine values",
		"A redaction placeholder is not a value",
		"If the user asks for a verbatim quote of tool output",
		"Do not put a redaction placeholder inside the quote",
		"Never claim that all values were copied verbatim",
		"If a skill has completed its explicit instruction and the user has not supplied another task",
		"omit that field",
		...
		"Honor explicit bounds and filters in the first tool call",
		"the assistant message carrying that call must contain no user-facing answer text",
		"wait for the tool result and answer once",
	} {
