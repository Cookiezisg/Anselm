...
// mode, or provisioning still pending) — so there is no install token to read a quota for. The
...
// ——故无 install token 可读配额。设置页据此隐藏免费档仪表，而非渲染误导性的清零。
...
// ResolveCredentialsByID to decrypt its gwk_ install token. A subset of *apikeyapp.Service.
//
// QuotaKeys 是配额读取所需的 apikey 端口：List 定位受管 anselm 行、ResolveCredentialsByID 解密其 gwk_
// install token。*apikeyapp.Service 的子集。
...
// QuotaFetcher reads live quota from the gateway by bearer token; *llm.QuotaClient satisfies it.
//
// QuotaFetcher 按 bearer token 从网关读 live 配额；*llm.QuotaClient 满足它。
type QuotaFetcher interface {
	Fetch(ctx context.Context, baseURL, token string) (llminfra.QuotaResult, error)
}
...
// token surfaces as the gateway's auth error, not a flipped local row). It exists because the install
// token is AES-GCM encrypted at rest and never leaves the backend — the client cannot read /quota
// directly, so the backend that holds the plaintext proxies it.
//
// QuotaReader 解析 workspace 的受管 anselm 凭证、把 live 配额读代理给网关。只读、每请求一次：不持状态、
// 绝不改凭证（失效 token 现为网关鉴权错误、而非本地翻行）。存在的理由：install token 静态 AES-GCM 加密、
// 永不出后端——客户端无法直读 /quota，故持明文的后端代理之。
...
// Read resolves the managed anselm key for the ctx workspace, decrypts its install token, and proxies
...
//
// Read 解析 ctx workspace 的受管 anselm key、解密其 install token、代理一次 live 配额读。无受管行时返
