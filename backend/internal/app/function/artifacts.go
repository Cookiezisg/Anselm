...
const (
	...
	// artifactSource marks the receipt's producer — the fifth production site.
	artifactSource = "function_artifact"
)
...
func collectArtifacts(ctx context.Context, up ArtifactUploader, outDir string, v any) (any, []string) {
	if up == nil || outDir == "" || v == nil {
		return v, nil
	}
	c := &artifactCollector{ctx: ctx, up: up, outDir: outDir}
