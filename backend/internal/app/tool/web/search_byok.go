...
// doSearchHTTP is shared HTTP send + status check; 2xx returns body, otherwise a
// sentinel-wrapped (ErrAuthFailed / ErrRateLimited / ErrUpstreamHTTP) error.
//
// doSearchHTTP 共享 HTTP 发送 + 状态检查；2xx 返 body，否则返 sentinel 包裹
// （ErrAuthFailed / ErrRateLimited / ErrUpstreamHTTP）的错误。
