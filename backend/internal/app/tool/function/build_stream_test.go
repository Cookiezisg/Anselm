...
// TestBuildSink_AccumulatesAndStreams: the env-fix sink both folds attempts into the create/edit
...
//
// TestBuildSink_AccumulatesAndStreams：env-fix sink 既把尝试折进 create/edit 结果（给 LLM），又把
// 装环境/修复叙事实时流成 tool_call 下的 progress 块（给用户）——同一 sink 两用，正如包注释所诺。
func TestBuildSink_AccumulatesAndStreams(t *testing.T) {
	b := &capBridge{}
