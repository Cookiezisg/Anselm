...
const configProps = `
		...
		"tools": {"type": "array", "description": "Callables the agent may use: each {ref}. ref is fn_… / hd_…method / mcp:server/tool — NEVER ag_ (an agent cannot call another agent; to chain agents, build a workflow with an agent node for each instead). The agent always sees each tool under the referenced entity's CURRENT name (a rename auto-updates), so you cannot alias it.", "items": {"type": "object", "required": ["ref"], "properties": {"ref": {"type": "string"}, "name": {"type": "string", "description": "Optional and IGNORED at runtime — the entity's live name is always used. Pass only ref; do not supply a custom alias expecting it to take effect."}}}},
