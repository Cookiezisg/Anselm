...
var killSchema = json.RawMessage(`{
	...
	"properties": {
		"shell_id": {
			"type": "string",
			"description": "ID of the background shell process to terminate (the bash_id returned by Bash with run_in_background:true)."
		}
	}
}`)
...
// ValidateInput rejects empty shell_id.
//
// ValidateInput 拒绝空 shell_id。
func (t *KillShell) ValidateInput(args json.RawMessage) error {
	...
	if strings.TrimSpace(a.ShellID) == "" {
		return errors.New("shell_id is required")
	}
	return nil
}
...
func (t *KillShell) Execute(_ context.Context, argsJSON string) (string, error) {
	...
	proc, err := t.mgr.Get(args.ShellID)
	if err != nil {
		return fmt.Sprintf("Background shell process not found: %s", args.ShellID), nil
	}
	...
	t.mgr.Remove(args.ShellID)

	if wasRunning {
		return fmt.Sprintf("Killed background shell %s.", args.ShellID), nil
	}
	return fmt.Sprintf("Background shell %s already finished; removed from registry.", args.ShellID), nil
}
