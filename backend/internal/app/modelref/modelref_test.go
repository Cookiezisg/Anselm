...
func (f fakeOptions) ValidateOptions(_ context.Context, _ modeldomain.ModelRef) error { return f.err }
...
func TestValidate(t *testing.T) {
	...
	if err := Validate(ctx, nil, errStruct, checker, nil); err != nil {
		t.Fatalf("nil ref must pass: %v", err)
	}
	// (b) set-but-incomplete → the caller's structural error (NOT API_KEY_NOT_FOUND).
	if err := Validate(ctx, &modeldomain.ModelRef{ModelID: "m"}, errStruct, checker, nil); !errors.Is(err, errStruct) {
		t.Fatalf("missing apiKeyId must be structErr, got %v", err)
	}
	if err := Validate(ctx, &modeldomain.ModelRef{APIKeyID: "aki_real"}, errStruct, checker, nil); !errors.Is(err, errStruct) {
	...
	if err := Validate(ctx, &modeldomain.ModelRef{APIKeyID: "aki_x", ModelID: "m"}, errStruct, nil, nil); err != nil {
	...
	if err := Validate(ctx, &modeldomain.ModelRef{APIKeyID: "aki_deadbeef", ModelID: "m"}, errStruct, checker, nil); !errors.Is(err, apikeydomain.ErrNotFound) {
	...
	if err := Validate(ctx, &modeldomain.ModelRef{APIKeyID: "aki_real", ModelID: "deepseek-v9-ultra"}, errStruct, checker, nil); err != nil {
		t.Fatalf("a typo'd modelId with a REAL key must still pass write-time validation (fail-loud at invoke by design), got %v", err)
	}
	// (f) options take a stricter path: once non-empty, they need a published contract. An
	// unavailable catalog must never turn into an implicit pass-through.
	unsupported := errors.New("unsupported option")
	if err := Validate(ctx, &modeldomain.ModelRef{APIKeyID: "aki_real", ModelID: "m", Options: map[string]string{"x": "y"}}, errStruct, checker, fakeOptions{err: unsupported}); !errors.Is(err, unsupported) {
		t.Fatalf("native options must be delegated to their contract validator, got %v", err)
	}
}
