...
func TestNew_NonAbsoluteRuleAppliedBySegmentMatch(t *testing.T) {
	// Relative rules apply via segment / basename matching (used by
	// DefaultWriteOnlyExtras for .git/, .env, etc): "relative/path/" is
	// detected as a path segment anywhere in the cleaned path; uncorrelated
	// paths fall through to allow.
	//
	// 相对规则按段 / basename 匹配（DefaultWriteOnlyExtras 用 .git/、.env 等）：
	// "relative/path/" 当路径段在 cleaned 路径中任意检测；不相关路径放行。
