...
// TestNew_FileSink: with a logDir the logger tees into <dir>/anselm.log — the desktop
// support story ("send me the log file").
//
// TestNew_FileSink：给 logDir 时 logger tee 进 <dir>/anselm.log——桌面报障故事（「把日志文件发我」）。
func TestNew_FileSink(t *testing.T) {
	...
	_ = log.Sync()
	b, err := os.ReadFile(filepath.Join(dir, "anselm.log"))
	if err != nil {
