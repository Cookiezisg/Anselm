...
// JSON to stderr) TEE'd with a rotating JSON file at <logDir>/foryx.log — the desktop
...
// <logDir>/foryx.log 的轮转 JSON 文件——桌面 app 的报障故事：「把日志文件发我」必须永远
...
func New(dev bool, logDir string) (*zap.Logger, error) {
	...
	if logDir != "" {
		...
		fileSink := zapcore.AddSync(&lumberjack.Logger{
			Filename:   filepath.Join(logDir, "foryx.log"),
