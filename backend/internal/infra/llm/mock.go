...
// It backs the zero-token T6 fake_llm test mode (and a dev script endpoint).
//
// MockClient FIFO 队列化 MockScript，通过 Stream 回放；并发安全。它支撑零 token 的 T6
// fake_llm 测试模式（及 dev script 端点）。
