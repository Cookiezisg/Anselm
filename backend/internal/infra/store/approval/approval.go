...
func (s *Store) CreateWithVersion(ctx context.Context, e *approvaldomain.ApprovalForm, v *approvaldomain.Version) error {
	return s.db.Transaction(ctx, func(tx *ormpkg.DB) error {
		if err := ormpkg.For[approvaldomain.ApprovalForm](tx, "approval_forms").Save(ctx, e); err != nil {
			if errors.Is(err, ormpkg.ErrConflict) {
				return approvaldomain.ErrDuplicateName
			}
			return fmt.Errorf("approvalstore.CreateWithVersion: entity: %w", err)
		}
		if err := ormpkg.For[approvaldomain.Version](tx, "approval_form_versions").Save(ctx, v); err != nil {
			return fmt.Errorf("approvalstore.CreateWithVersion: version: %w", err)
		}
		return nil
	})
}
...
func (s *Store) SaveVersionAndActivate(ctx context.Context, v *approvaldomain.Version, entityID string) error {
	return s.db.Transaction(ctx, func(tx *ormpkg.DB) error {
		...
		if err != nil {
			return fmt.Errorf("approvalstore.SaveVersionAndActivate: pointer: %w", err)
		}
		if n == 0 {
			return approvaldomain.ErrNotFound
		}
