...
func (s *Store) CreateWithVersion(ctx context.Context, e *controldomain.ControlLogic, v *controldomain.Version) error {
	return s.db.Transaction(ctx, func(tx *ormpkg.DB) error {
		if err := ormpkg.For[controldomain.ControlLogic](tx, "control_logics").Save(ctx, e); err != nil {
			if errors.Is(err, ormpkg.ErrConflict) {
				return controldomain.ErrDuplicateName
			}
			return fmt.Errorf("controlstore.CreateWithVersion: entity: %w", err)
		}
		if err := ormpkg.For[controldomain.Version](tx, "control_logic_versions").Save(ctx, v); err != nil {
			return fmt.Errorf("controlstore.CreateWithVersion: version: %w", err)
		}
		return nil
	})
}
...
func (s *Store) SaveVersionAndActivate(ctx context.Context, v *controldomain.Version, entityID string) error {
	return s.db.Transaction(ctx, func(tx *ormpkg.DB) error {
		...
		if err != nil {
			return fmt.Errorf("controlstore.SaveVersionAndActivate: pointer: %w", err)
		}
		if n == 0 {
			return controldomain.ErrNotFound
		}
