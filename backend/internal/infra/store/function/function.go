...
// bootstrap to collect and apply via db.Migrate. functions has a partial-UNIQUE name
...
//
// Schema 是 function 三表 DDL，按序幂等导出，由 bootstrap 汇总经 db.Migrate 应用。functions
...
// always sparing the function's active version (which may be old after a revert), and
// returns the deleted versions' EnvIDs so the caller can reclaim their orphaned venvs
...
// （revert 后它可能很老），返回被删版本的 EnvID 列表，供调用方回收其孤儿 venv（每个版本
// 独占一份 per-version env——不回收则泄漏到盘上直到手动 sandbox:gc）。
func (s *Store) TrimOldestVersions(ctx context.Context, functionID string, keep int) ([]string, error) {
	if keep <= 0 {
	...
	if err := s.vers.WhereEq("function_id", functionID).Order("version DESC").Pluck(ctx, "version", &nums); err != nil {
		return nil, fmt.Errorf("functionstore.TrimOldestVersions: %w", err)
	}
	if len(nums) <= keep {
		return nil, nil
	}
	...
	if err != nil {
		return nil, fmt.Errorf("functionstore.TrimOldestVersions: load active: %w", err)
	}
	// Capture the doomed versions' EnvIDs BEFORE the delete (the rows vanish after) so the
	// caller can destroy their venvs. Same predicate as the delete — single source of truth.
	// 删除前先抓住将死版本的 EnvID（删后行就没了），让调用方销毁其 venv。与删除同谓词——单一真相源。
	var envIDs []string
	if err := s.vers.
		...
		return nil, fmt.Errorf("functionstore.TrimOldestVersions: pluck env ids: %w", err)
	}
	if _, err := s.vers.
		WhereEq("function_id", functionID).
		Where("version < ?", cutoff).
...
// ——中途失败不留无版本实体行。
func (s *Store) CreateWithVersion(ctx context.Context, e *functiondomain.Function, v *functiondomain.Version) error {
	return s.db.Transaction(ctx, func(tx *ormpkg.DB) error {
		if err := ormpkg.For[functiondomain.Function](tx, "functions").Save(ctx, e); err != nil {
			if errors.Is(err, ormpkg.ErrConflict) {
				return functiondomain.ErrDuplicateName
			}
			return fmt.Errorf("functionstore.CreateWithVersion: entity: %w", err)
		}
		if err := ormpkg.For[functiondomain.Version](tx, "function_versions").Save(ctx, v); err != nil {
			return fmt.Errorf("functionstore.CreateWithVersion: version: %w", err)
		}
		return nil
	})
}
...
func (s *Store) SaveVersionAndActivate(ctx context.Context, v *functiondomain.Version, f *functiondomain.Function) error {
	return s.db.Transaction(ctx, func(tx *ormpkg.DB) error {
		...
		// edit must land too, else the function keeps its old name/description while a new version
		// is active — the edit silently loses the rename. f was just read by the caller, so Save
		// upserts the existing row.
		//
		// 同事务持久化整行（active 指针 + meta）：edit 带的 set_meta 也必须落，否则函数保留旧名/旧述
		// 而新版本已 active——edit 静默丢了改名。f 是调用方刚读出的，Save 对已存在行做 upsert。
		f.ActiveVersionID = v.ID
		if err := ormpkg.For[functiondomain.Function](tx, "functions").Save(ctx, f); err != nil {
			return fmt.Errorf("functionstore.SaveVersionAndActivate: function: %w", err)
		}
		return nil
	})
}
