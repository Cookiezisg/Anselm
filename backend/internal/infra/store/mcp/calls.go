...
func (s *Store) ListCalls(ctx context.Context, filter mcpdomain.CallFilter) ([]*mcpdomain.Call, string, error) {
	...
	if filter.Status != "" && !slices.Contains(mcpdomain.CallStatuses, filter.Status) {
		return nil, "", mcpdomain.ErrInvalidCallStatus.WithDetails(map[string]any{"allowed": mcpdomain.CallStatuses, "got": filter.Status})
	}
