...
- **S15 ID 生成统一**：业务 ID 一律 `<prefix>_<16hex>` 格式（前缀按 domain 取，如 `aki_` apikey / `mc_` model config / `cv_` conversation / `msg_` message / `att_` attachment / `blk_` block / `f_` forge / `fv_` forge version / `tc_` test case / `fe_` forge execution（Phase 5 起，统一替代旧 `frh_`/`fth_` 两表）/ `b_` forge test 批跑 batch / `td_` todo（对话级 TODO，2026-05-05 由原 `tk_` task 改名）/ `bsh_` Bash 后台 shell 进程 / `sar_` subagent run / `smm_` subagent message / `fnenv_` function venv / `hdenv_` handler venv（每个 sandbox 消费者用各自前缀给 sandbox 当不透明 string，per D-redo-8）/ `rel_` relation（V1.2 §16 跨实体边）/ `mch_` mcp health snapshot（V1.2 §17 mcp 健康历史） )；8 字节从 `crypto/rand` 取，**`rand.Read` 失败必须 panic**——熵源损坏继续会生成碰撞 ID。所有 `newID()` 函数遵守此格式（实现统一在 `pkg/idgen.New(prefix)`）
