...
- **当前状态**（快照——本条只留当前物理事实,逐 STEP 过程 / workflow ID / 历史 bug 修复从 git + working 文档取,见 #7）：
  ...
  - **Phase 4.2 Chat 纯聊天骨干已落**（V0–V2+V7半 模块 + 组装切片①–⑧，真后端免费模型端到端亲测）：rail（信号点 / 改名 / 置顶 / 归档 / 删除 / 搜索 / 排序 / 无限翻 / 空标题回落）+ 中心海洋（landing 首发懒建 → transcript **CustomScrollView+center 锚**［prepend 零位移；超屏贴 max / 未满屏钉 min 让浮层头］+ 三层合并模型［settled/live/pending FIFO 回声］+ composer send↔stop + 浮层头改名·模型菜单·自动命名活着双落 + 诚实终态横幅）+ demo 脚本流式。**基础聊天完整体已打磨**（landing 静态问候［业界定稿］+ 两态模型选择器［粘性+首发盖章］+ 自动命名假流式［head/rail 打字机同播］+ **@ 提及 picker**［combobox 标准+伪药丸+core MentionSource DIP］+ **附件三入口**［📎/粘贴/拖放 → chip → attachmentIds + 泡内元数据解析］）。**tool 卡 V3a 底盘已落**（`ChatToolCard` 全生命周期裸行+通用展开体+注册表缝+transcript 接线，规范 WRK-053 [`tool-cards.md`](docs/working/frontend/tool-cards.md)）；余族皮肤 V3b–V3e / 人在环（V6）/ 特殊块（V5）/ 右岛（V8，后端 touchpoint 台账已落）待建，见 [`chat.md`](docs/working/frontend/chat.md)。
  ...
  - **门禁**：`make verify`（后端）+ `make fe-verify`（前端）+ `make docs`（文档）各自全绿；前端当前 **1243 测绿**。
...
- **N4 分页**：**无界集合** List 接口必须支持 `?cursor=...&limit=...`（function/handler/agent/workflow/flowrun/trigger/control/approval/mcp/conversation/relation/notification/search/touchpoint 及各版本·执行·调用日志）。**有界可枚举资源**（单用户少量或系统级固定集：workspaces / skills / memories / documents 树 / sandbox runtimes·envs / todos / model-capabilities）豁免——返全集不分页、无 `nextCursor`，分页参数按标准 HTTP 忽略而非报错。
