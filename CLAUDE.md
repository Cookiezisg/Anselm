...
- **当前状态**（快照——本条只留当前物理事实,逐 STEP 过程 / workflow ID / 历史 bug 修复从 git + working 文档取,见 #7）：
  - **后端 `backend/`**：全 Quadrinity 实体 + durable 引擎，编译/装配/启动/服务全通；loopback 加固（默认绑 `127.0.0.1` + `RequireBearerToken`[`ANSELM_AUTH_TOKEN`，空=关] + `RequireLoopbackHost` 防 DNS rebinding）。契约成体系，见 `references/backend/`。
  ...
  - **门禁**：`make verify`（后端）+ `make fe-verify`（前端）+ `make docs`（文档）各自全绿；前端当前 **1209 测绿**。
