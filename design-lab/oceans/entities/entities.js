/* Foryx design-lab — 实体海洋编排（产品级重做 · 终极优化：同源 documents 美学）。
...
(function () {
  ...
  const D = {
    process_invoice: { kind: 'function', version: 5, status: 'done', runs: 1284, desc: '解析上传的发票 PDF / 图片，抽取结构化字段（供应商、金额、税号、行项目），校验后返回 JSON。',
      ...
      rel: [{ title: 'Built in', rows: [{ kind: 'chat', name: '前端设计 (fork)', meta: 'cv_3f' }] }, { title: 'Used by', rows: [{ kind: 'workflow', name: 'invoice_flow', meta: 'extract' }, { kind: 'agent', name: 'summarizer', meta: 'tool' }] }] },
