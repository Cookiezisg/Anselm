...
| 7 | install.go:141-144 | `st, err := s.GetServer(ctx, name); if err != nil { return nil, err }` | A.4 | EDGE | bare return — same style inconsistency as site #1; sentinel preserved | LOW | same as site #1 | wrap: `return nil, fmt.Errorf("mcpapp.InstallFromRegistry %s: %w", name, err)` | FOUND |
