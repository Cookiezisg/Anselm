...
| 21 | mcp.go:392-403 | `tools, err := client.ListTools(ctx); if err != nil { ... _ = client.Close(); s.publishStatus(ctx, name); return err }` | A.1/A.4 | EDGE | same dual issues as #20 (bare-return + silent Close) | LOW × 2 | same | same | FOUND |
...
| 27 | mcp.go:510-512 | `func (s *Service) ListRegistry(ctx) ([]..., error) { return s.source.List(ctx) }` | A.4 | EDGE | bare passthrough — source.List returns sentinel from CuratedRegistrySource (which currently never errors per infra/mcp/curated_registry.go); no wrap. If source ever switches to a network-backed implementation, ErrMarketplaceUnavailable would surface unwrapped (sentinel preserved through bare return, but no call-site loc). | LOW | identical UX (sentinel preserved); harder to grep | optional wrap: `if err != nil { return nil, fmt.Errorf("mcpapp.ListRegistry: %w", err) }` | FOUND |
