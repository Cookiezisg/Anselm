...
- 缺 header 或 id → 走默认 `local-user`

### 前端契约
- `store/settings.js` 加 `activeUserId` (持久化) + `onboarded` 标志
- `api/client.js` 的 `apiFetch` 自动从 settings 取 `activeUserId` 注入 header
- `sse/shared.js` 的 `createSSE` 自动给 URL 追加 `?userID=`
- 切换 user → `settings.set({ activeUserId })` + `queryClient.invalidateQueries()` 全量失效

### UI
- **SettingsPopover** 顶部账号区（详 `boilerplate/src/app.jsx` SettingsPopover）：avatar / "切换" / 用户列表 / 添加输入框
- **Onboarding 5 步向导**（`boilerplate/Forgify Onboarding.html`）：
  1. intro — 项目介绍
  2. account — username + avatar 颜色
  3. look — accent 选择
  4. provider — provider + API key（可跳过）
  5. done — 总结 + "进入应用"

### 首次启动检测（App.jsx）
```
showOnboarding = !settings.onboarded
              && (users.length === 0 || users[0].username === "default")
```
即"后端只有自动建的 default user 且本地从未完成 onboarding"。完成后 `settings.onboarded = true` 永不再现。
...
*最后更新：2026-05-20*
*状态：Phase 0-11 + 多账号/Onboarding (§19) 已交付*
