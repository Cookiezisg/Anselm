...
主键 `fe_<16hex>`；无软删；表行无主动 eviction（**TODO**：300 条/forge 上限策略待实现，原文档曾设此目标但 store 层无 TrimExecutions / 触发器；当前由用户手动 GC / 调试需要时清理）。
字段：
