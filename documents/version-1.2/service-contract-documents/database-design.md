...
- **环境运行时状态**：`env_status` TEXT default `'pending'`（5 值：`pending`/`syncing`/`ready`/`failed`/`evicted`,白名单 service 层校验）/ `env_error` TEXT / `env_synced_at` DATETIME / `env_sync_stage` TEXT / `env_sync_detail` TEXT

每版本独立的 env 状态——pending 自带自己的 sync 历史,跟 active 不串。EnvID 相同的多版本共用 venv,但 env_status 各自独立。accepted 版本上限 `functiondomain.AcceptedVersionCap = 50`,超限硬删最旧。
...
class code parts:`imports` / `init_body` / `shutdown_body` / `methods` JSON / `init_args_schema` JSON。Sandbox: `dependencies` JSON / `python_version` / `env_id` 索引 / `env_status`(5 值 service-level 校验)/ `env_error` / `env_synced_at` / `env_sync_stage` / `env_sync_detail`。
AcceptedVersionCap = 50/handler。
