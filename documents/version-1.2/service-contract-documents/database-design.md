...
主键 `cv_<16hex>`；软删（`deleted_at`）；`user_id` 索引。新增字段：`system_prompt TEXT`（对话级自定义系统提示词，可为空）/ `auto_titled BOOLEAN`（标记标题是 AI 自动生成的还是用户手动改的）/ **`summary TEXT default ''`**（V1.2 §1 final-sweep；compaction running summary，anchored-append。空 = 尚未发生压缩）/ **`summary_covers_up_to_seq INT64 default 0`**（V1.2 §1；该 summary 覆盖到哪个 block seq；下次压缩从此 seq+1 起算）。Title 允许空字符串，首轮完成后 auto-titling goroutine 回写。
