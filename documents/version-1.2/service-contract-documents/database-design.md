...
- **`env_id`** TEXT 索引（**= version_id**,D-redo-8 每版本独立 venv,不再做 deps+python 哈希共享。详 [`../adhoc-topic-documents/forge_redesign/discussions/2026-05-12-env-and-sse-rework.md`](../adhoc-topic-documents/forge_redesign/discussions/2026-05-12-env-and-sse-rework.md) §D）
...
class code parts:`imports` / `init_body` / `shutdown_body` / `methods` JSON / `init_args_schema` JSON。Sandbox: `dependencies` JSON / `python_version` / `env_id` 索引(**= version_id**,D-redo-8 每版本独立 venv)/ `env_status`(5 值 service-level 校验)/ `env_error` / `env_synced_at` / `env_sync_stage`(含 `fixing` 状态)/ `env_sync_detail`。
