...
### apikey RESTRICT — 3 store 实现 RefScanner + main.go 装配（2026-05-28）

| 日期 | 内容 |
|---|---|
| 2026-05-28 | **[feat]** Task 0b：apikey RESTRICT 落地 store 层。3 store 实现 `AnyReferencesApiKey`：model（`api_key_id =`）/ conv（`json_extract(model_override,'$.apiKeyId') =`）/ workflow（LIKE `%"apiKeyId":"<id>"%` over `workflow_versions.graph` JSON，JOIN workflows.user_id 限定用户 —— workflow_versions 表本身无 user_id 列）。main.go 抽出 `modelStore/convStore/workflowStore` 3 句柄,统一所有权,装配 3 setter。9 新单测（True/False/CrossUserIsolated × 3）全绿;build + staticcheck + apikey app tests 全绿。 |
| 2026-05-28 | **[feat]** Task 9：`workflow/apply.go` 第 10 个 op `set_node_model_override` + F1 校验 + `Service.SetKeyProvider`。omitted/null `modelOverride` 字段 = clear；缺 `apiKeyId`/`modelId` → `ErrInvalidNodeModelOverride`（400 `INVALID_NODE_MODEL_OVERRIDE`）；未知 apiKeyId → `apikey.ErrNotFound`（404 `API_KEY_NOT_FOUND`，跨用户隔离）。`ApplyOps` 加 `keyProvider` 参数,顶层包检测 F1 sentinel 跳过 `ErrOpInvalid` 包装让 errmap 命中正确码。`main.go` 装配 `workflowService.SetKeyProvider(apikeyService)`。errmap 加 `ErrInvalidNodeModelOverride` 行。4 新单测全绿;build + staticcheck + workflow/conv/model 测试全绿。 |
| 2026-05-28 | **[test]** Task 13：3 stale pipeline 测试迁移到 3-scenario + APIKeyID shape + errcodes sweep 加新案例。`chat_pipeline_test.go` `ScenarioChat→ScenarioDialogue` + `{provider→apiKeyId}` + 直接 DB 删绕过 RefScanner；`model_pipeline_test.go` 5 测试全用 `SeedDeepSeek` 返回的 apiKeyID + 新增 `Upsert_UnknownAPIKeyID_404`；`sweep_pipeline_test.go` `PROVIDER_REQUIRED→API_KEY_ID_REQUIRED` 列项 + 新增 `TestErrcodes_APIKeyInUse`（422）+ `TestErrcodes_InvalidNodeModelOverride`（400）。harness 镜像 main.go 装配 3 ref scanner + `workflowService.SetKeyProvider`；`SeedDeepSeek` 返 apiKeyID（兼容旧调用）。`make matrix` 自动更新 README：errcode 覆盖 23→25,新增 INVALID_NODE_MODEL_OVERRIDE / API_KEY_IN_USE / API_KEY_ID_REQUIRED 行。3 文件 vet/build 全绿、新测全过、`make audit` clean。 |
| 2026-05-28 | **[feat]** Task 20：workflow editor InspectorBody 接入 `KeyModelPicker`,仅 agent/llm 节点渲染。`NodeSpec` 加 `modelOverride?:ModelRef\|null`,经 `entities/conversation/@x/workflow.ts` 跨 slice 暴露(FSD `@x`)。`useWorkflowEdit.diffToOps` 监测现存节点 modelOverride 翻转,发专属 `set_node_model_override` op(后端 F1 校验 apiKeyId 归属);新节点 modelOverride 走 `add_node.node` 内联避免双发。`nodeToSpec` round-trip + `parseGraph` 回填。i18n 新增 `workflow.json`(zh/en) nodeModelOverride 文案。8 新单测 + 全 952 测试绿,typecheck/build/steiger clean。|
| 2026-05-28 | **[bug-fix]** `diffToOps` 的 `update_node` op 形状不对：前端发 `{op,node:NodeSpec}`,后端 `applyUpdateNode` 期望 `{op,nodeId,patch}`(RFC 7396 Merge Patch),任何节点编辑会在后端 apply 阶段 fail。改 emit `{op:"update_node", nodeId, patch: nodeToPatch(n)}`;新增 `nodeToPatch` 导出(排除 `id` 与 `modelOverride`,后者走专属 op)。同步 `feature-workflow-edit.md` op 联合类型 + 纯函数导出表;测试改成断言 `{nodeId, patch}` 形状。typecheck + 全 952 测试绿。|
