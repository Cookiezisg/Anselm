...
| 日期 | 内容 |
|---|---|
| 2026-05-02 | **[doc] 沙箱迭代 1 设计文档落地**：新增 `adhoc-topic-documents/sandbox-iteration-documents/01-uv-bundled-python-per-forge-venv.md`。盘清当前 sandbox 现状（`infra/sandbox/python.go` 147 行 + `app/forge/ast.go` 211 行，两处重复用 hardcode `python3` subprocess；只能跑 stdlib；30s timeout 常量在 forge.md 写了但代码没接）。定方向：**捆绑 python-build-standalone + uv 二进制 + 每 forge 独立 .venv**——拒绝纯系统 Python（桌面体验差）和纯 uv 在线下 Python（首启联网抖动），落定 C 方案（资源进 .app embed.FS，仅装 PyPI 包时联网）。**核心架构**：每 forge 一个 `<dataDir>/forges/<id>/` 目录含 pyproject.toml + uv.lock + .venv；sync 时机在 forge 创建/依赖变更，run 时 `uv run --no-sync` 永远热路径；uv 全局 wheel cache 硬链接自动去重。**数据模型扩展**：`Forge` 加 5 字段（`Dependencies` JSON / `PythonVersion` / `EnvStatus` 4 值状态机 / `EnvError` / `EnvSyncedAt`）；`ForgeVersion` 加 2 字段（依赖快照随版本走）；`Sandbox` 接口扩成 Sync/Run/Destroy 三方法。**Run 不设 timeout**：彻底删 30s 限制——工具可能正常需跑很久，死循环是 LLM/用户问题，sandbox 不越权拦；唯一停止信号是上游 ctx-cancel；删 `ErrSandboxTimeout` sentinel。**Forge 元属性不落 entity**：destructive 维持 §S18 既有 per-call AI 自报模式（同 tool 不同 args 可能不同 destructive 性，LLM 现场判精准胜于静态），不加 `Destructive`/`IsReadOnly`/`IsConcurrencySafe` 字段——曾考虑过但讨论后否决。**异步 sync worker**：in-memory channel 队列 + per-forge mutex + detached context 写终态；推 4 个新 SSE 事件 `sandbox.sync_*`。**4 个新 sentinel**：`ErrEnvNotReady` / `ErrEnvFailed` / `ErrSandboxUnavailable` / `ErrDependencyResolution`，errmap 4 行。**老数据迁移**：AutoMigrate 加列 EnvStatus 默认 "pending"，第一次 Run 触发懒同步（无依赖 forge 几乎瞬间）。**ast.go 也走捆绑 Python**——抽 `ASTParser` 接收路径，跟 sandbox 共用一个 Python 概念。**Phase 划分** ~4-5 天独立交付不阻塞 Phase 4：A sandbox 内部 / B domain+service 扩展 / C tool 层+HTTP / D 装配 / E 文档同步 / F testend UI。**明确不做**：安全隔离 / 多 Python 并存 / 自动 venv GC / sync 任务持久化队列 / run 超时——本地单用户单作者场景下都是过度工程。 |
