...
| 2026-05-31 | **[design]** workflow-revamp **B5 定:forge SSE = quadrinity 4 kind**(commits `aa44c79`/`88722e3`;`11`/`12`)：交叉审挑出三方不一致——`09` quadrinity(4)、`12` 写"扩到 6(加 agent/document/skill)"、CLAUDE.md E1 trinity(3)。核代码定论 **4**:forge SSE 事件带 `versionId`/`envStatus`/accept,这套**锻造生命周期**只 function/handler/workflow 有、agent 同样有(无 env,同 workflow)→ quadrinity;**document 无版本**(已核 `app/document` 无 version/accept)、**skill 是 marketplace 装的非锻造**(09:216 mcp/skill 不算 forge)→ 二者**不进** forge SSE,其 create/edit 是 chat 工具动作走 eventlog SSE、实体变更走 notifications SSE。`12` 写 6 是把 **lazy 工具分组**(含 document/skill 工具)与 **forge 实体集**搞混。CLAUDE.md E1 维持 trinity-3(描述已建代码;revamp 实现时按 E2 先改 `event-log-protocol.md` 再 bump 4)。|
