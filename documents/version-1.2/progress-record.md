...
| 日期 | 内容 |
|---|---|
| 2026-05-03 | **[doc] 测试流水线迭代 1 设计文档完整撰写**：`adhoc-topic-documents/test-pipeline-iteration-documents/01-foundation-and-coverage.md` 新建。先做完整代码摸排：3 个并行 Explore agent 通读 5 个 domain（apikey/model/conv/chat/forge）+ tool framework + sandbox lifecycle，自查 `app/tool/tool.go` + `pkg/llmclient` + `infra/llm/factory.go`。**触发原因**：发现 `backend/test/harness.go:146` 仍用 `sandboxinfra.New("python3")` 旧签名（沙箱迭代 1 已改为 `New(Config{...})`），`make test-pipeline` 编译失败；progress-record 自己已记 "harness.go 待 sandbox iteration A 收尾"——迭代落地后 main.go 修了但 harness 漏。同时盘点出更大的覆盖缺口：6 个 chat 场景之外，**forge 22 端点零 handler 测试 / 5 个 forge LLM tool 零测试 / 沙箱 lifecycle 仅 fake-sandbox 单测覆盖 / chat × forge 交集零覆盖 / 32 个 sentinel 无端到端 sweep**。**核心设计决策**：(1) **fake LLM via httptest server**——不改 production 接口，让真 `*llminfra.Factory` 指向 fake server（注入点是 apikey.BaseURL 字段，天然存在），优势：复用真 openAIClient SSE 解析、wire 协议变化能被抓到、零侵入；(2) **fake LLM vs 真 LLM 分两层**——`make test-pipeline` 默认 fake（~30s，CI 友好、离线可跑），`make test-pipeline-live` opt-in 真 DeepSeek（reasoner/wire 协议捕获），共享同一 harness 仅注入点不同；(3) **真 sandbox in pipeline**——pipeline 层不引入 fake sandbox，`$FORGIFY_DEV_RESOURCES` 设了就跑真 uv，没设 t.Skip，跟 `infra/sandbox/integration_test.go` 同一约定；单测层保留 fake sandbox 不动；(4) **文件夹按场景簇切 14 文件**：基础设施 5（harness/seed/sse/helpers/fake_llm + scripts）+ 3 轻 domain（apikey/model/conv）+ 5 chat 场景（basic/react/attachment/queue/autotitle）+ 2 forge（http/lifecycle）+ 1 chat×forge + 1 errcodes sweep + 1 isolation；目标 ~80 测试用例。**Coverage matrix**：每个 domain 列已有 unit/store/handler 现状对照本迭代 pipeline 目标，明示 ✅/⚠️/❌/🎯 四种状态，便于实施时勾选。**Phase 划分 7 段共 ~13h**：A 基础设施修 drift+fake LLM（2h）→ B 3 轻 domain HTTP（1h）→ C chat 全场景（3h）→ D forge HTTP+lifecycle 真 sandbox（3h）→ E chat × forge 交集（2h）→ F errcodes+isolation sweep（1h）→ G 文档+Makefile+doctor（1h）。MVP 截止线 = A+B+C+F = ~7h，完成后 fake-LLM 套覆盖 chat 全场景 + 3 轻 domain + 错误码扫描，已是巨大提升。**明确不做**：Anthropic wire 协议 fake / 真 OpenAI 真 Anthropic 测试 / 多用户真 auth / Bootstrap 失败诊断（已在 sandbox unit）/ HTML/CSS testend 测试 / coverage 报告 / fuzz / benchmark / Phase 4-5 domain。**风险与缓解**：fake LLM 协议漂移靠 1 个 Live reasoner 兜底；真 sandbox flakiness 靠 t.Skip + 180s timeout；SSE 时序敏感靠 entity-state 模型本身的最终态导向；测试间状态污染靠 t.TempDir + harness.New per-test。**预估测试代码量** ~3000-4500 行 + 600 行 helpers/fake_llm。等用户拍板后开工。 |
