...
#### Tool 系统大重构（2026-05-02 起，Phase 0-6 计划，进行中）

对照 Claude Code 内部机制调研（`adhoc-topic-documents/claude-code-research-documents/`）后认定当前 13 个 tool 的实现"基础设施过于薄"——Tool 接口仅 4 方法、无元字段、无并发分批、Edit 工具缺失、forge 5 件 system tool 与 8 件通用 system tool 混在一个文件、`agent` 包名与 Tool 内涵不匹配、`app/tool/` 这个名字其实是"用户造的 tool 的服务"应该叫 forge。决定按 7 阶段重构：Phase 0 清理 → 1 大重命名 tool→forge → 2 包嵌套子包 → 3 新接口+forge tool 重写 → 4 文档同步 #1 → 5 重建 system tools → 6 文档同步 #2。
