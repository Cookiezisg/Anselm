...
> **调研依据**:`documents/version-1.2/adhoc-topic-documents/llm-providers/{01-audit, 02-build-vs-adopt, 03-implementation-reference, 04-capability-catalog}.md`(体检 / 选型 / 施工图 / 能力目录)。
...
3. **用户覆盖**:目录陈旧时用户手动标"这模型支持 thinking / 窗口=N"(存哪见 §7 DB,挂在 api_key 或 model-config 上的可选 override)。
...
| 用户能力覆盖 | 无 | 可选:api_key 上加 JSON 列存 per-model override(或新小表);本期可先不做,留 §1 非目标边界 |
...
- **新 `GET /model-capabilities`**:返 modelcaps 目录(per provider/model-pattern 的 thinking 形状 + 窗口 + contextMode),供前端 capability-aware 渲染。**不扩 `/providers`**(粒度不同:providers 是 provider 级,caps 是 model 级)。
...
- **新端点 hook**:`entities/model-config/api/model-config.ts` 加 `useModelCapabilities()` + `qk.modelCapabilities()`;`entities/model-config/model/` 加 `capabilityFor(provider, modelId)` selector(三处消费者统一解析)。
...
- **P4 能力端点 + 前端**:`/model-capabilities`+hook;ModelDefaults capability-aware 控件;override 编辑器;onboarding 保持极简。
...
涉及:`service-design-documents/{model,chat,subagent,conversation,workflow}.md` + 新建 `modelcaps` 相关;`api-design.md`(新端点 + widened PUT)+ `database-design.md`(model_configs thinking 列)+ `error-codes.md`(新 sentinel);前端 `entity-types.md`(ModelRef/ModelConfig + capability 类型)+ `cross-cutting.md`(新 queryKey/hook)+ 对应 `frontend-design-documents/*`;`progress-record.md` 全程;新规范进 `CLAUDE.md`。`04` 的窗口数据取代 `01` 零散提及。
...
- **用户能力覆盖**:本期是否做?倾向 P4 之后看实际陈旧痛感再定(YAGNI)。
