...
本域**自身**只 2 表(handlers / handler_versions);call 记录(`handler_calls` 表)走**共享 schema 模板**,含 method / instance_id / owner_kind / owner_id 等 kind-specific 字段,详见 [`08-executions.md`](./08-executions.md) §4.2。Service.Call 终态写入。LLM 通过 `query_executions({kind:"handler", entityId:hd_xxx})` 看历史。
