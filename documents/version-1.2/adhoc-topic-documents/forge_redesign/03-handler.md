...
- **sandbox venv 引用** — **per HandlerVersion**(D-redo-8 `env_id = version_id`,每版本独立 venv;销 instance 不影响 venv,销 version 才 destroy env)
...
| env_id | TEXT 索引 | **= version_id**(D-redo-8 每版本独立 venv;不再做 deps+python 哈希) |
...
- **env**:`env_id = version_id`(D-redo-8)— **每 HandlerVersion 独立 venv**,跨 instance 共享同 Version 的 venv,但不跨 Version 共享。销 instance 不影响 venv,销 Version 才 destroy env
...
2. `EnsureEnv(owner=<versionID>, deps)` 走 PythonEnvManager — env 维度 owner = versionID(每 Version 一份)
