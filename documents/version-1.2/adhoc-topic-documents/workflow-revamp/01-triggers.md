...
5 种内部实现不同,对外统一:**都 emit 消息进 workflow 入口 queue** → 每条消息起一个独立 flowrun → trigger 节点 emit 该消息进下游 queue。(术语:msg-queue 模型下"event/out 端口"= "消息/emit 进下游 queue",见 00)

| Kind | 一次几条消息 | 消息 payload |
|---|---|---|
| cron | 1 | `{firedAt}` |
...
- 填完触发 → workflow 从这个 trigger 节点开始跑
...
trigger 永远是**分发任务**:**1 条消息 = 1 个 flowrun**,每次给下游 workflow 一个任务。

- polling 返 `[e1, e2, e3]` → trigger Service **起 3 个独立 flowrun**(每个一条入口消息)
- 每个 flowrun 的 trigger 节点 emit 单条消息进下游 queue
- workflow 内部永远是"单消息单流",不存在 list 在节点间流动
