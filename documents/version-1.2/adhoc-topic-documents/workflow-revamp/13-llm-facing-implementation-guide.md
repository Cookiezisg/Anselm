...
- **叠加预期(未实测投影,非实测值)**:gold 示例 + 架构守则进 prompt,复杂建时自一致性采样 + 自审 → 复杂建**有望** 85%+(把若干 paired lift 线性叠加到 42-52% 实测基线的投影;自一致性 / 自审需 **N× token**,非免费);剩下靠 §4 的 `:iterate` 对话兜(实测改一次正确 67%、96% 不破坏其余)。**实测地板**:复杂 create_workflow 首发 42-52%(±15pt 判官方差)、一次性端到端 0/24 —— 复杂建是**人在环迭代**,不是一发入魂。
