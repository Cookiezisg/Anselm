...
| 22 | 优雅 drain 生命周期(refcount) | deactivate/accept 即时 `DestroyOwner`(抽在途) | `workflows.lifecycle_state`(active/draining/inactive)+ handler 实例注册表 refcount,走停新→排空(归 0 才销毁)→销毁状态机,零停机(详 §D1/D5) | 🔴 强 |
...
if workflow.Active {
    ...
    //     共享 handler 实例按 refcount,归 0 才销毁(不在此处即时 Destroy)
...
func dispatch(firing) {
    ...
    fr := scheduler.StartRun(firing.workflowID, firing.nodeID, firing.payload, isFromListener=true)
...
deactivate / accept **不即时 `DestroyOwner`**(会抽在途 flowrun)。走三段状态机(`workflows.lifecycle_state` + handler 实例 refcount,A6 加的):

```
(1) 停新  —— 撤 listener + 派发器不再起新 flowrun;workflow 进 draining
(2) 排空  —— 在途 flowrun(durable、靠 journal 活着)在老版本/老实例跑完;
             共享 handler 实例按 refcount,归 0 才销毁
(3) 销毁  —— in-flight=0 后销毁老实例 +(accept)挂新版本 listener → inactive / 新 active
```

- **deactivate**:走 (1)(2)(3) → 终态 `inactive`。
- **accept(改 version)**:走 (1)(2)(3) → (3) 末挂新版本 listener → 终态 `active`(D1 调的就是这条)。
- `internal/infra/handler/registry.go` 加 refcount(`Acquire` +1 / flowrun 结束 `Release` −1,归 0 触发 drain 回调);`internal/app/scheduler/` 加 drainCoordinator 跟在途 flowrun 计数。
...
**G1. forge SSE 第 4 kind = agent(CANON-X1:扩共享 scope 枚举)**

`internal/infra/forge/bridge.go` + `infra/forge/protocol.go`:加 `agent` 到 kind 枚举。

> **CANON-X1**:给 forge SSE 加 agent kind,本质是**扩 eventlog 的共享 scope-kind 闭枚举**——`forge.IsValidScopeKind` 复用 `eventlogdomain.Kind*`,两条 SSE 共用同一套 scope 词表。这是项目重构,**扩这个共享枚举是接受的、不另立 forge 私有枚举**;按 **E2**(闭枚举先改协议文档再加 code)**先更新 [`event-log-protocol.md`](../eventlog-redesign/event-log-protocol.md) 再加 kind**。(后续若给 forge 再加 document/skill kind 同理:都是扩这套共享 scope 枚举。)

**G2. eventlog SSE 事件本体不动,但共享 scope 枚举要扩(澄清旧说)**

消息流本体 = 主对话 block,新 domain agent 锻造对齐 chat 既有 5 events × 7 block types,**事件/block 类型不动**。但 G1 已点明:eventlog 与 forge **共用同一套 scope-kind 闭枚举**,加 agent(及后续 document/skill)kind 时这套**共享 scope 枚举要扩**(走 E2)。旧表述里"eventlog SSE 不动"应理解为"eventlog 的 5×7 事件本体不动",**不等于共享 scope 枚举不动**。
