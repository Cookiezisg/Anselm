...
- **不嵌套结构化循环(V1,C6)** — 一个循环体内不再含另一个循环头,`iteration_key` 因此保持**一维**(节点 + 第几轮)。要嵌套(外层遍历 N 项、内层每项重试)就把内层做成 **sub-workflow 或 forge 工具**(对位"map over N → 工具":动态扇出 / 逐项迭代塞进锻造的能力内部)。accept 校验拒嵌套回边。
