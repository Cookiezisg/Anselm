...
`approval.prompt` 用 `{{ CEL }}` 模板(`{{ }}` 内是 CEL,求值后字符串化插入)——跟 agent prompt 同一种用法。注意 tool.args 现在是**裸 CEL**(产出类型化值),三者底层都是同一套 CEL 表达式语言,只是文本文档字段(prompt)走 `{{ }}` 插值、求值字段(args)直接裸写。`{{ }}` 不是第二种语言,只是 CEL 的插值定界符;心智一致,无新概念。详 [`04-case-node.md`](./04-case-node.md) 的表达式语言段。插值在执行器记"等信号"事件时一次性渲染完(读的是已记账的 payload / ctx),渲染结果连同 prompt 落进 approvals 记录;之后即便重放也直接读这条记录,不重算。
