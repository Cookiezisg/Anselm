...
func SearchTools(pathGuard pathguardpkg.PathGuard, log *zap.Logger) []toolapp.Tool {
    return []toolapp.Tool{
        newGrep(pathGuard, log),  // 构造时锁定 rgPath（PATH 上 LookPath("rg")）；log 给 rg→stdlib fallback warn 路径
        newGlob(pathGuard),       // Glob 不需 log（无 fallback 警示）
    }
}
```

`log` 可为 nil（test 友好）；Grep 内部用 nil-tolerant `warnf` helper 调 log，不会 NPE。
...
Grep.Execute(ctx, args)
  ...
  ├─ rgPath != "" → execRg
  │   └─ 失败（rg exit ≠ 0,1）→ warn log + fallback execStdlib（防止 rg 偶发故障让搜索全跪）
...
**fallback 必须打 warn log**——defect-class lesson: silent backend swap 会让用户/运维"以为还在用 rg"实际跑的是 stdlib（PCRE-only pattern 行为会偷偷不同）。Grep struct 持 `log *zap.Logger`，rg 失败时打 `Grep: rg backend failed; falling through to stdlib (results may differ for PCRE-only patterns)` 让运维能在日志里看到这次降级。
