...
**不新增节点类型**。给现有 `llm` 节点 NodeConfig 加字段：

```go
type LLMNodeConfig struct {
    Prompt              string   `json:"prompt"`
    Model               string   `json:"model,omitempty"`
    AttachedDocumentIDs []string `json:"attachedDocumentIds,omitempty"` // ← 新
    // ... 现有字段
}
```

`dispatch_llm.go` 跑前拼 documents 段：

```go
func (d *LLMDispatcher) Dispatch(ctx context.Context, node *flowrun.Node, input map[string]any) (map[string]any, error) {
    cfg := node.ParseConfig[LLMNodeConfig]()

    docPrefix := ""
    if len(cfg.AttachedDocumentIDs) > 0 {
        docs, err := d.documentSvc.GetBatch(ctx, cfg.AttachedDocumentIDs)
        if err != nil {
            return nil, fmt.Errorf("dispatch_llm: load attached documents: %w", err)
        }
        var sb strings.Builder
        sb.WriteString("<documents>\n")
        for _, doc := range docs {
            sb.WriteString(fmt.Sprintf("<document path=%q>\n", doc.Path))
...
`workflow.validate.go` 加 capability check：当节点 config 有 `AttachedDocumentIDs` 时，validate 阶段调 `documentSvc.GetBatch` 验证全部存在；任一不存在 → 422 `WORKFLOW_DOCUMENT_NOT_FOUND`（接 `ErrDocumentNotFound` 的跨域翻译）。

---

## 10. Conversation 接入

Conversation entity 加字段：

```go
type Conversation struct {
    // ... 现有字段
    AttachedDocumentIDs []string `gorm:"serializer:json" json:"attachedDocumentIds,omitempty"`
}
```

PATCH `/api/v1/conversations/{id}` 可改 `attachedDocumentIds`；验证 doc 存在（任一不在 → 422 `CONVERSATION_DOCUMENT_NOT_FOUND`）。

`chat/runner.buildSystemPrompt` 看到 `conv.AttachedDocumentIDs` 非空就 prepend `<documents>` 段（跟 memory pinned 同一 cache-friendly 静态层）：
...
| **§14.5 Workflow + Conversation 接入 + testend UI** | 1 天 | `LLMNodeConfig.AttachedDocumentIDs` + dispatch_llm 拼 `<documents>` + workflow validate capability check + Conversation.AttachedDocumentIDs + runner.buildSystemPrompt prepend + testend 双 UI（侧边栏树 + 对话挂载下拉）+ 3 pipeline test |
...
| Conversation.AttachedDocumentIDs 全引用存在 | PATCH 时 GetBatch 校验；任一不存在 → 422 |
| Workflow LLM 节点 AttachedDocumentIDs 全引用存在 | workflow.validate.go capability check；任一不存在 → 422 |
