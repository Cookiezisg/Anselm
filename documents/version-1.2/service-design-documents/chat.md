...
|---|---|---|---|
| `create_forge` | 3 | 创建新工具（名称/描述/代码）| forge + tool |
| `edit_forge` | 3 | 修改已有工具的代码或描述 | tool |
| `run_forge` | 3 | 通用执行器，按 id 运行任意工具 | tool sandbox |
| `search_forges` | 3 | 语义搜索工具库，返回相关工具列表 | tool（FTS5）|
...
type Tool interface {
    ...
    Name() string
    Description() string
    Parameters() json.RawMessage   // JSON Schema；禁止含 "summary" / "destructive"

    // 静态元数据（3 个固有属性）
    IsReadOnly() bool              // 决定 runTools 并发分批默认
    NeedsReadFirst() bool          // Phase 5 Edit/Write 用
    RequiresWorkspace() bool       // Phase 5 用
    ...
    // 主入口（args 已剥除 summary / destructive）
    Execute(ctx context.Context, argsJSON string) (string, error)
}
```

### 4.2 标准字段注入机制（summary + destructive）

```
ToLLMDef(tool)
  → injectStandardFields(tool.Parameters())
    → properties 加 "summary"（必填 string）和 "destructive"（可选 bool 默认 false）
    → required 把 "summary" 插到第一位
  → 返回发给 LLM 的 ToolDef（含两个标准字段）
...
runOneTool(ctx, t, tc)
  ...
  → t.Execute(ctx, argsJSON) — 此时 args 已是剥除 summary/destructive 的 JSON

parseToolArgs(rawArgs)
  → toolapp.StripStandardFields(rawArgs)
    → (summary, destructive, stripped)
  → 填进 ToolCallData.Summary / .Destructive；剩余 args 作为 Arguments map
```
...
5 个 forge tool 在 `app/tool/forge/` 子包（每文件一 tool，Phase 3 后优化轮重组）。Phase 0 删了 8 个旧通用 system tool；Phase 5 重建新一代 Read/Write/Edit/Bash/Glob/Grep/LS。
