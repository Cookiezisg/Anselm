...
| Bug | 修复 |
|---|---|
| Gemini base-url：`providers.go` 中 google 的 `DefaultBaseURL` 缺少 `/v1beta/openai` 后缀，chat 调用返 404 | `providers.go` 已修：`https://generativelanguage.googleapis.com/v1beta/openai`；Tester 侧 `testGoogleListModels` 在探测前剥去 `/openai` 后缀（走 `?key=` 认证的 list-models 端点不含该段）|
...
## 9.5 CapabilityService（`internal/app/apikey/capability.go`，2026-05-30）

`CapabilityService` 是 **apikeyapp 包内的独立 service 结构**（非 `apikey.Service` 方法），消费 `pkg/modelcaps`（静态规则）+ `ModelCapOverrideRepository`（用户 override store），提供以下方法：

- `ResolveCapabilities(ctx, provider, modelID) ModelCapability` — 合并 override（优先）+ 静态规则（兜底）
- `SetOverride(ctx, provider, modelID, in CapOverrideInput) error` — 写 / 更新 override（部分 UNIQUE per user+provider+model）
- `ClearOverride(ctx, provider, modelID) error` — 软删 override
- `ListOverrides(ctx) ([]*ModelCapOverride, error)` — 列出当前用户所有 override
