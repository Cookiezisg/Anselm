...
- [`../service-contract-documents/database-design.md`](../service-contract-documents/database-design.md) — 无独立 subagent 表；sub-run = `messages` 行（attrs.kind=subagent_run），sub blocks = `message_blocks`
- [`../service-contract-documents/error-codes.md`](../service-contract-documents/error-codes.md) — subagent sentinel
- [`../event-log-protocol.md`](../event-log-protocol.md) — sub-run 通过 message_start/block_start 嵌套写入父对话事件流（无独立 SSE 事件）
- 关联设计文档：[`skill.md`](./skill.md)（`context: fork` 复用本服务）/ [`chat.md`](./chat.md)

> **⚠️ 2026-05 schema 统一**：本文档内 §3 的 `SubagentRun` / `SubagentMessage` Go struct 定义 + `subagent_runs` / `subagent_messages` 表设计已**作废**——递归事件日志统一后，sub-run 是统一 `messages` 表的一行（role=assistant，attrs.kind=subagent_run + type/runId/maxTurns，parent_block_id 指向触发它的 message-block 占位），sub-run transcript 是该 message 在 `message_blocks` 表的 blocks（经 eventlog Emitter 实时写）。无独立表 / 无 `sar_` / `smm_` 前缀。具体落库形状见 `internal/app/subagent/host.go::WriteFinalize` 与 `spawn.go::Spawn`。
...
> **⚠️ 已作废段（保留作历史参考）**：以下 SubagentRun / SubagentMessage struct 是 2026-05 schema 统一前的设计；现行实现走统一 `messages` + `message_blocks` 表（`attrs.kind=subagent_run`），无独立类型 / 无独立表。详见文件头警告。
