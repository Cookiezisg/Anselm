  ...
  → transport 层：无（system tool 不走 HTTP）
    → app 层：app/tool/subagent.SubagentTool.Execute
        ...
        → subagentapp.Service.Spawn(ctx, type, prompt, opts)
            ...
            → 构建 subRunOpts：
                · 独立 messages slice（system prompt = type.SystemPrompt）
                · 过滤后 tool registry（**物理排除 SubagentTool 自身**；只保留 type.AllowedTools）
                · maxTurns = opts.MaxTurns ?? type.DefaultMaxTurns
                · model = opts.Model ?? type.DefaultModel ?? PickForChat
                · 创建 SubagentRun 入库（status=running）
                · ctx 注入 SubagentRunID + SubagentDepth+1
            → chatapp.RunSub(ctx, subRunOpts)（主 runner 的子入口）
                → 复用 agentRun 循环：stream + tool dispatch + writeBlocks
                → 每 step 末更新 SubagentRun 的 token 累计
                → 终止条件：finish_reason = stop / max_turns / ctx.cancel
            → 取 last assistant message 文本
            → 写 SubagentRun（status=completed/cancelled，含 token 总计）
            → bridge.Publish entity-state `subagent` SSE
        → 返回 last message string 给主 LLM 作为 tool_result
  → 主 LLM 收到 tool_result，继续主 loop
```

**端到端跨 domain 依赖**：
- `app/chat`：`RunSub` 是新增导出方法（不破坏现有 `Send`）；复用 `runner.agentRun`
...
type Service struct {
    ...
    chatRunner SubRunner   // 注入 chatapp 暴露的 RunSub 入口（防循环依赖：chat 也注入 subagent 是单向的）
...
### `SubRunner` 接口（解耦 chat ↔ subagent）

```go
// internal/domain/subagent/subagent.go
type SubRunner interface {
    RunSub(ctx context.Context, opts SubRunOpts) (lastMessage string, tokensIn, tokensOut, steps int, status string, err error)
}

type SubRunOpts struct {
    SystemPrompt string
    UserPrompt   string
    Tools        []toolapp.Tool       // 已经过滤
    Model        string                // 空 = 用 PickForChat
    MaxTurns     int
    OnTokens     func(in, out int)     // 每 step 末回调（用于实时累计）
    RunID        string                // 写日志/事件用
}
```

**实现位置**：`internal/app/chat/subrunner.go` —— 复用 `runner.agentRun` 的内部循环，参数化 messages 和 tools。**关键**：复用而非复制；agentRun 内部本就支持参数化的 tool registry。
...
| 关系 | 说明 |
|---|---|
| **chat** | chatapp 暴露 `SubRunner` 接口 / `RunSub` 方法；subagent 服务持有此接口（防循环 import）|
| **events** | 同标准 entity-state 模式 |
...
```
domain/subagent/  → 定义 SubRunner 接口
                ↑ 被 implements
app/chat/         → 实现 SubRunner（subrunner.go）
                ↑ 被 wired
app/subagent/     → 持有 SubRunner 字段，构造时注入
                ↑
cmd/server/main.go → 装配：chatapp.New() → subagentapp.New(chatapp.SubRunner())
```

**chat 可以 import subagent（用于 Skill fork 时 chat runner 也调 subagent）**——但只通过 Service 接口，不依赖具体实现。这是 §S13 标准 port 模式。
