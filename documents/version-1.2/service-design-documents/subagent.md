  ...
  → transport 层：无（system tool 不走 HTTP）
    → app 层：app/tool/subagent.SubagentTool.Execute
        ...
        → subagentapp.Service.Spawn(parentCtx, type, prompt, opts)
            ...
            → AgentState.AddSubagentTokens(subMsgID, typ.Name, in, out)  per-conversation token 账
...
- `pkg/reqctx`：`SubagentRunID` / `SubagentDepth` / `ParentBlockID` ctx key
- `pkg/agentstate`：`AddSubagentTokens(runID, typeName, in, out)` per-conversation 累计
...
type SubagentType struct {
    ...
    DefaultModel    string   `json:"defaultModel"`    // V1 未消费（resolve 走主对话同 picker）
...
镜像 `chatdomain.Role*`——保留供 type system prompt 引用。事实源在 chatdomain。
...
{
    Name:            "Explore",
    Description:     "Fast read-only search agent for locating code/files. ...",
...
type Service struct {
    ...
    activeRuns   map[string]context.CancelFunc  // 在 flight sub-run 的 cancel 注册
}
...
type SpawnOpts struct {
    ...
    Model    string // V1 未消费（resolve 走主对话同 picker）
}
...
**Service 公开 API 仅这 2 个 + ctor + SetTools 后置注入**——没有 `Get` / `ListTypes` / `ListByConversation`。Type 列表通过 `Registry.List()` 暴露（HTTP API 不暴露——见 §11）。

**defaultRunTimeout = 5 分钟**（spawn.go 顶常量）——单次 Spawn 总超时；防 stuck tool 让 sub-runner 永挂。
...
- `s.activeRuns[subMsgID] = cancel` 让外部 `Service.Cancel(_, runID)` 能抢占
...
- agentstate.SubagentTokenLog 按 `runID = subMsgID` 分桶；每个 sibling 独立计数
...
```go
// pkg/agentstate/agentstate.go（参考实现）
func (s *AgentState) AddSubagentTokens(runID, typeName string, in, out int)
func (s *AgentState) SubagentTokenLog() []SubagentTokenEntry  // 返 copy 非别名
```
...
**4 参签名**：`AddSubagentTokens(runID, typeName, in, out)`——`runID` 为第一参（= subMsgID），让多个 sibling subagent 不撞车。
...
| **reqctx** | `SubagentRunID` / `SubagentDepth` / `ParentBlockID` ctx key |
| **agentstate** | `AddSubagentTokens(runID, typeName, in, out)` per-conversation 累计（4 参签名） |
...
- **Cancel HTTP 端点**：v1 仅内部 `Service.Cancel(_, runID)` 暴露——主对话 cancel 通过 ctx 级联自然 cancel 所有 sub。未来如有"局部 cancel 某个 sibling"需求再加 HTTP
