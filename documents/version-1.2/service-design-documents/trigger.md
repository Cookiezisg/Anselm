...
| `webhook` | `net/http` (挂主 ServeMux 子路径) | `path / method / secret?` | POST → onFire |
...
- **webhook**:`http.ServeMux` 子路径 `/api/v1/webhooks/{wfId}/{path}` + path 冲突拒 + body 10MB cap + JSON auto-parse fallback bodyRaw + secret 校验
