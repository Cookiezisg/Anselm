...
(function () {
  ...
  function mount(host, entity, opts) {
      ...
      + `<div class="fg-ec-ptree">` + (grps || []).map(g =>
          `<div class="fg-ec-pgrp">${esc(g.group)}</div>` + (g.types || []).map(t =>
            ...
            + `<div class="fg-ec-pents">` + (t.keys || []).map(key =>
                `<button class="fg-ec-popt fg-ec-pent" data-key="${esc(key)}" data-src="ws"><span class="fg-ec-pdot"></span><span class="fg-ec-pnm">${esc(key)}</span></button>`).join('')
            + `</div></div>`).join('')
        ).join('') + `</div></div>`;
    const pmenu = o.picker
      ? `<div class="fg-ec-pmenu">` + (o.picker.items || []).map(it =>
          `<button class="fg-ec-popt${it.key === o.picker.current ? ' on' : ''}" data-key="${esc(it.key)}" data-src="${esc(it.src || '')}"><span class="fg-ec-pico">${ICO(kindMeta(it.kind).icon, 15)}</span><span class="fg-ec-pnm">${esc(it.name)}</span><span class="fg-ec-pkd">${esc(kindMeta(it.kind).label)}</span></button>`).join('')
    ...
    if (o.picker) {
      ...
      menu.querySelectorAll('.fg-ec-popt[data-key]').forEach(op => op.onclick = e => { e.stopPropagation(); menu.classList.remove('open'); o.picker.onPick && o.picker.onPick(op.dataset.key, op.dataset.src); });
