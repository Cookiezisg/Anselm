/* Forgify demo — 组件 code-editor（单一事实源；收掉 entities codeEditor()/hl()/TOK/KW + documents highlight() 各处副本）。
   契约：工厂挂载 → handle；自载同名 .css；只读令牌（Atom One --cd-*）；fg- 前缀；不碰别的海洋。
   形态：白底块 = 透明 textarea 叠在高亮 pre 上 + 左侧行号槽 + 右上角语言标签；可改（活体重绘 + onDirty 回执）或只读。
...
(function () {
  ...
  function mount(host, opts = {}) {
    ...
    const cornerLabel = corner || lang || '';
    const w = tag('div.fg-ce');
    w.innerHTML = `<div class="fg-ce-gut"></div><div class="fg-ce-area"><pre><code></code></pre><textarea spellcheck="false" ${readOnly ? 'readonly' : ''}></textarea></div>${cornerLabel ? `<span class="fg-ce-lang">${esc(cornerLabel)}</span>` : ''}`;

    const ta = w.querySelector('textarea'), codeEl = w.querySelector('code'), gut = w.querySelector('.fg-ce-gut'), pre = w.querySelector('pre');

    // 重绘：高亮叠层 + 行号槽随行数生成。
    const paint = () => {
      codeEl.innerHTML = highlight(ta.value, lang);
      const n = ta.value.split('\n').length;
      gut.innerHTML = Array.from({ length: n }, (_, i) => `<i>${i + 1}</i>`).join('');
    };
    ta.value = code;
    paint();
