...
(function () {
  ...
  window.CHAT_CONVOS_LIST = [
    ...
    { group: "今天", open: true, rows: [
      { id: "cv_iterate", label: "AI 编辑 · sync_inventory 加重试", dot: "wait", meta: "10 分钟前" },
      { id: "cv_triage", label: "诊断 · flowrun frn_8a1c 失败", dot: "err", meta: "1 时前" },
    ]},
    { group: "昨天", open: true, rows: [
      { id: "cv_invoice", label: "发票处理 v3 迭代", dot: "done", meta: "昨天 18:20" },
    ]},
  ...
  window.CHAT_CONVOS = {
    // ── 旗舰：脚本化 live 回合（reasoning → tool_call → 危险确认 → tool_result → subagent 子树 → 回答）──
    cv_daily: {
      ...
      turn: [
        ...
        { ms: 750, push: { type: "tool_call", open: true, items: [
          { verb: "run_shell", name: "shell", danger: "dangerous", gate: true,
        ...
        { gate: {
          onApprove: { ms: 700, patch: { type: "tool_call", open: true, summary: "已清理过期快照（释放 3.4 GB）", items: [
            { verb: "run_shell", name: "shell", danger: "dangerous",
    ...
    // ── :iterate 对话：AI 编辑实体 → 右岛实时填充 pending 草稿（entities build 流镜像）──
    cv_iterate: {
      id: "cv_iterate", title: "AI 编辑 · sync_inventory 加重试", crumb: "Chat · iterate", kind: "iterate", meta: "claude-opus · 10 分钟前",
      island: { entity: "sync_inventory", kind: "function", lang: "python", version: "草稿 v2" },
      blocks: [
    ...
    cv_triage: {
      ...
      blocks: [
        ...
        { type: "reasoning", label: "推理", text: "triage 上下文已注入失败 flowrun 的全节点行。先看哪一步 failed、错误是什么。" },
        { type: "tool_call", open: true, summary: "读取 flowrun 节点记忆化结果", items: [
          { verb: "read_flowrun", name: "frn_8a1c…", danger: "safe",
            result: { json: { failedNode: "fetch", iteration: 0, error: "HTTP 429 Too Many Requests", retry: "none" } } },
        ] },
        { type: "text", text: "根因找到了：节点 **`fetch`** 在第 0 轮因 **HTTP 429（限流）** 失败，且该节点**没配重试**。\n\n两个修法：\n1. 给 `fetch` 节点加 durable 重试（指数退避，避开瞬时限流）；\n2. 给上游 trigger 加并发上限（serial）。\n\n我可以直接改图加重试——要我提一个草稿吗？" },
        { type: "compaction" },
      ],
    },
    ...
    cv_invoice: {
      ...
      blocks: [
        ...
        { type: "tool_call", open: false, summary: "改 validate_invoice：金额校验放宽 ±0.01", items: [
          { verb: "edit_function", name: "validate_invoice", danger: "cautious", result: { json: { version: "v3", pending: false } } },
        ] },
        { type: "text", text: "已更新并采用 **v3**：金额校验现允许 ±0.01 的舍入误差，其余规则不变。" },
