# Batch 63 unified gate · EDGE-151..160

- 批次：六十三
- 范围：`EDGE-151` 至 `EDGE-160`，50 个单格已逐格登记
- formal journal：`3286 = 2300 baseline + 986 live judgments`
- COVERAGE：`848 rows / 657 carried judgments / 0 tombstones`
...
make -C backend testend
  ok github.com/sunweilin/anselm/testend/scenarios 277.328s

python3 -m unittest discover -s testend/rig -p 'test_*.py' -v
  Ran 51 tests in 3.069s
  OK
...
python3 testend/rig/gen_coverage.py --check
  gen_coverage: clean (848 rows, 657 carried judgments, 0 tombstones)
...
RIG_HOME=/private/tmp/anselm-rig-formal-20260801-3 python3 testend/rig/alarms.py check
  alarms: clean (986 live judgments; 2300 baseline judgments excluded from drift curves)
...
Batch 63 is green and eligible for submission. EDGE-151..160 all stayed within their evidence
