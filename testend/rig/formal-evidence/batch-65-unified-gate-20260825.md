# Batch 65 unified gate · EDGE-171..180

- 批次：六十五
- 范围：`EDGE-171` 至 `EDGE-180`，50 个单格已逐格登记
- formal journal：`3386 = 2300 baseline + 1086 live judgments`
- COVERAGE：`848 rows / 677 carried judgments / 0 tombstones`
...
make -C backend testend
  ok github.com/sunweilin/anselm/testend/scenarios 300.441s

python3 -m unittest discover -s testend/rig -p 'test_*.py' -v
  Ran 51 tests in 3.161s
  OK
...
python3 testend/rig/gen_coverage.py --check
  gen_coverage: clean (848 rows, 677 carried judgments, 0 tombstones)
...
RIG_HOME=/private/tmp/anselm-rig-formal-20260801-3 python3 testend/rig/alarms.py check
  alarms: clean (1086 live judgments; 2300 baseline judgments excluded from drift curves)
...
Batch 65 is green and eligible for submission. EDGE-171..180 stayed within their evidence boundaries:
