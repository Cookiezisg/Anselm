...
//
// ErrSeqTooOld 在 fromSeq 已被 replay buffer 淘汰时返回；客户端须全量重取
// （messages 走 DB 历史；entities/notifications 重订阅）。
var ErrSeqTooOld = errors.New("stream: requested seq too old (evicted from replay buffer)")
