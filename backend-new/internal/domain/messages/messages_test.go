...
func TestIsValidBlockType(t *testing.T) {
	for _, v := range []string{BlockTypeText, BlockTypeReasoning, BlockTypeToolCall, BlockTypeToolResult, BlockTypeCompaction} {
		if !IsValidBlockType(v) {
			t.Errorf("IsValidBlockType(%q) = false, want true", v)
		}
	}
	// progress / message were dropped from the old eventlog vocabulary — nesting is now
	// expressed via stream Open.ParentID, so they must NOT validate.
	for _, bad := range []string{"", "progress", "message", "unknown"} {
