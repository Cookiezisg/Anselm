...
func registerHandlers(mux *http.ServeMux, s *services, bus buses, log *zap.Logger) {
	...
	regs := []interface {
		...
		handlershttpapi.NewNotificationHandler(s.notification, bus.notifications, log),
		handlershttpapi.NewMemoryHandler(s.memory, log),
		handlershttpapi.NewSandboxHandler(s.sandbox, log),
		handlershttpapi.NewDocumentHandler(s.document, log),
		handlershttpapi.NewTodoHandler(s.todo, log),
		handlershttpapi.NewAttachmentHandler(s.attachment, log),
		handlershttpapi.NewFunctionHandler(s.function, log),
		handlershttpapi.NewHandlerHandler(s.handler, log),
		handlershttpapi.NewAgentHandler(s.agent, log),
		handlershttpapi.NewTriggerHandler(s.trigger, log),
		handlershttpapi.NewMCPHandler(s.mcp, log),
		handlershttpapi.NewSkillHandler(s.skill, log),
		handlershttpapi.NewControlHandler(s.control, log),
		handlershttpapi.NewApprovalHandler(s.approval, log),
		handlershttpapi.NewWorkflowHandler(s.workflow, log),
