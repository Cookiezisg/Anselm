...
// VersionDraft is the accumulated mutable class snapshot during ops apply.
//
// VersionDraft 是 ops 应用过程中累积的可变类快照。
...
// ApplyOps applies ops to a base draft, validating after each. Pure (no streaming) —
// progress narration is the tool layer's job.
//
// ApplyOps 把 ops 套到 base 草稿，每步后校验。纯函数（不推流）——进度叙述是 tool 层的事。
...
func ParseOps(raw json.RawMessage) ([]Op, error) {
	...
	if err := json.Unmarshal(raw, &arr); err != nil {
		return nil, fmt.Errorf("handlerapp.ParseOps: ops array unmarshal: %w", err)
	}
	...
	for i, r := range arr {
		...
		if err := json.Unmarshal(r, &disc); err != nil {
			return nil, fmt.Errorf("handlerapp.ParseOps: ops[%d]: %w", i, err)
		}
		if disc.Op == "" {
			return nil, fmt.Errorf("handlerapp.ParseOps: ops[%d]: missing 'op' discriminator", i)
		}
...
func applyOne(state *VersionDraft, op Op) error {
	...
	case "add_method":
		...
		if findMethodIdx(state.Methods, p.Method.Name) >= 0 {
...
func mergeMethodPatch(target handlerdomain.MethodSpec, patch json.RawMessage) (handlerdomain.MethodSpec, error) {
	...
	rawMerged, err := json.Marshal(mergePatch(targetMap, patchMap))
	if err != nil {
...
// mergePatch implements RFC 7396 recursively; a nil patch value deletes the target key.
//
