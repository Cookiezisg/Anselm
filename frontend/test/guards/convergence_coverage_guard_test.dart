...
void main() {
  test(
    'convergence coverage: every lib file is registered with a review status (WRK-066 §3.3b)',
    () {
      final tree = _tree();
      ...
      expect(
        file.existsSync(),
        isTrue,
        reason: 'coverage ledger missing: $_coveragePath',
      );
      ...
      if (Platform.environment['UPDATE_COVERAGE'] == '1') {
        ...
        // ignore: avoid_print
        print(
          'coverage ledger reconciled: ${next.length} files, $pending pending.',
        );
        return;
      }

      final unregistered = tree.difference(ledger.keys.toSet()).toList()
        ..sort();
      ...
      expect(
        ...
        isEmpty,
        reason:
            'NEW files must enter the coverage denominator (as pending) — run '
      ...
      expect(
        ...
        isEmpty,
        reason:
            'Deleted files must leave the ledger — run '
            'UPDATE_COVERAGE=1 flutter test test/guards/:\n${stale.join('\n')}',
      );
      expect(
        badStatus,
        isEmpty,
        reason:
            'Unknown status (allowed: ${_statuses.join('/')}):\n${badStatus.join('\n')}',
      );
    },
  );
}
