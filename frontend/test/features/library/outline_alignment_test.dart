...
void main() {
  ...
  testWidgets(
    'extractDocOutline and headingNodeIds agree on count AND order over tricky markdown',
    (tester) async {
      await tester.pumpWidget(_host(_tricky));
      await tester.pumpAndSettle();

      final outline = extractDocOutline(_tricky);
      final editorIds = tester
          .state<AnEditorState>(find.byType(AnEditor))
          .headingNodeIds;

      // 甲/乙/丙/丁四级/戊五级/己 — six headings; the fenced + quoted # are excluded by BOTH sides. 六标题。
      expect(outline.map((e) => e.text).toList(), [
        '甲',
        '乙',
        '丙',
        '丁四级',
      ...
      expect(
        ...
        outline.length,
        reason:
            'the index IS the jump key — a count mismatch misaligns every later jump 计数不齐=后续全错位',
      );
  ...
  testWidgets(
    'the order maps 1:1 — the editor heading at index k renders the outline text at k',
    (tester) async {
      await tester.pumpWidget(_host(_tricky));
      await tester.pumpAndSettle();
      ...
      for (var k = 0; k < outline.length; k += 1) {
        ...
        expect(
          (node as dynamic).text.toPlainText(),
          outline[k].text,
