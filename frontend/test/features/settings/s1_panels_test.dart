...
Widget _host(SettingsPrefs prefs, Widget child, {SettingsRepository? repo}) =>
    ProviderScope(
      overrides: [
...
void main() {
  ...
  test('persisted Chinese UI locale restores synchronously at cold start', () {
    final container = ProviderContainer(
      overrides: [
        settingsPrefsProvider.overrideWithValue(
          SettingsPrefs.inMemory({SettingsKeys.locale.key: 'zh-CN'}),
        ),
      ],
    );
    addTearDown(container.dispose);

    expect(container.read(localePreferenceProvider), 'zh-CN');
    expect(LocaleSettings.currentLocale, AppLocale.zhCn);
  });

  group('GeneralPanel 通用', () {
    testWidgets(
      'theme segmented: all three notches apply + persist (dark live since S1b)',
      (tester) async {
        final prefs = SettingsPrefs.inMemory();
        await tester.pumpWidget(
          _host(prefs, const GeneralPanel(), repo: FixtureSettingsRepository()),
        );
        await tester.pumpAndSettle();

        await tester.tap(
          find.text(t.settings.themeSystem).first,
        ); // theme segment(语言下拉同词)
        await tester.pumpAndSettle();
        ...
        await tester.pumpAndSettle();
        expect(
          prefs.getString(SettingsKeys.theme),
          'dark',
          reason: 'dark 已随 S1b 点亮,可选可存',
        );
      },
    );

    testWidgets('language double-writes: UI locale + workspace.language', (
      tester,
    ) async {
      final prefs = SettingsPrefs.inMemory();
      ...
      await tester.tap(
        find.text(t.settings.langSystem).last,
      ); // open the dropdown 开下拉
      await tester.pumpAndSettle();
      await tester.tap(find.text(t.settings.langEn).last);
      await tester.pumpAndSettle();

      expect(
        prefs.getString(SettingsKeys.locale),
        'en',
        reason: 'UI half persisted',
      );
      expect(
        repo.workspace.language,
        'en',
        reason: 'workspace half PATCHed (双写)',
      );
      // Restore for the rest of the suite (setLocaleSync is process-global). 还原全局 locale。
      LocaleSettings.setLocaleRaw('zh-CN');
    });

    testWidgets('window/update switches round-trip their declared keys', (
      tester,
    ) async {
      final prefs = SettingsPrefs.inMemory();
      await tester.pumpWidget(
      ...
      expect(
        switches,
        findsNothing,
        reason: 'NO Material Switch — AnSwitch only 禁 Material',
      );
      ...
      final sw = find.descendant(
        of: find.ancestor(
          ...
          matching: find.byWidgetPredicate(
            (w) => w.runtimeType.toString() == 'AnSettingRow',
          ),
        ),
        matching: find.byWidgetPredicate(
          (w) => w.runtimeType.toString() == 'AnSwitch',
        ),
      );
      await tester.tap(sw);
      await tester.pumpAndSettle();
  ...
  group('NotificationsPanel 通知', () {
    testWidgets(
      'level notches persist; switching to silent stages the one-shot hint',
      (tester) async {
        final prefs = SettingsPrefs.inMemory();
        await tester.pumpWidget(_host(prefs, const NotificationsPanel()));
        final container = ProviderScope.containerOf(
          tester.element(find.byType(NotificationsPanel)),
          listen: false,
        );
        await tester.pumpAndSettle();
  ...
  group('ChatPanel 对话', () {
    testWidgets(
      'auto-stage reads/writes the SAME followModeProvider as the sidestage',
      (tester) async {
        final prefs = SettingsPrefs.inMemory({'an.stage.follow': 'never'});
        late ProviderContainer container;
        await tester.pumpWidget(
          _host(prefs, const ChatPanel(), repo: FixtureSettingsRepository()),
        );
        ...
        await tester.pumpAndSettle();
        expect(
          container.read(followModeProvider),
          FollowMode.never,
          reason: '同一 provider 恢复',
        );
    ...
    testWidgets(
      'webFetch PATCHes the workspace; failure rolls back + stages feedback',
      (tester) async {
        final prefs = SettingsPrefs.inMemory();
        final repo = FixtureSettingsRepository();
  ...
  group('WindowBounds.clampToDisplays 多显示器 clamp 纪律', () {
    ...
    test('a rect on a live display restores as-is', () {
      final r = WindowBounds.clampToDisplays(
        const Rect.fromLTWH(100, 84, 1280, 792),
        [main],
      );
    ...
    test(
      'a rect from an unplugged display is stranded → null (fallback centered)',
      () {
        final r = WindowBounds.clampToDisplays(
          const Rect.fromLTWH(2000, 100, 1280, 792),
          [main],
        );
    ...
    test(
      'a body-visible-but-bar-offscreen rect does NOT restore (grab test on the bar)',
      () {
        // The bar sits above the screen top; only the body pokes in. 标题栏在屏外上方。
        final r = WindowBounds.clampToDisplays(
          const Rect.fromLTWH(100, -100, 1280, 792),
          [main],
        );
    ...
    test('degenerate stored sizes are rejected', () {
      expect(
        WindowBounds.clampToDisplays(const Rect.fromLTWH(0, 0, 10, 10), [main]),
        isNull,
      );
    });
  });
