...
void main() {
  ...
  testWidgets(
    'manual add (stdio) lands; an unreachable server shows the honest failed face',
    (tester) async {
  ...
  testWidgets(
    'marketplace: client-side search + installed mark; :plan form stars required',
    (tester) async {
      final repo = FixtureSettingsRepository()
        ...
        ..mcpServers.add(
          const McpServerStatus(id: 'mcp_1', name: 'context7', status: 'ready'),
        )
        ..mcpPlan = const McpRegistryPlan(
          transport: 'stdio',
          runtime: 'node',
          envVars: [
      ...
      expect(
        find.text(t.settings.mcp.installed),
        findsOneWidget,
        reason: '短名比对已装标',
      );
      ...
      container
          ...
          .push('mcpInstall', id: 'io.github.x/weather');
      await tester.pumpAndSettle();
      expect(find.text('API_KEY'), findsOneWidget);
      expect(
        find.textContaining(t.settings.mcp.requiredMark),
        findsOneWidget,
        reason: '必填星标',
      );
      ...
      await tester.pumpAndSettle();
      expect(
        repo.mcpServers.any((s) => s.name == 'weather'),
        isTrue,
        reason: '安装落短名行',
      );
    },
  ...
  testWidgets('import skips same names when overwrite is off', (tester) async {
    final repo = FixtureSettingsRepository()
      ..mcpServers.add(
        const McpServerStatus(id: 'mcp_1', name: 'dup', status: 'ready'),
      );
    await tester.pumpWidget(_host(repo));
    await tester.pumpAndSettle();
    final panelEl = tester.element(find.byType(McpPanel));
      ...
      final repo = FixtureSettingsRepository()
        ..mcpServers.add(
          const McpServerStatus(
            ...
            tools: [
              McpToolDef(name: 'do-thing', description: 'does the thing'),
            ],
          ),
        );
      await tester.pumpWidget(_host(repo));
  ...
  testWidgets(
    'zero MCP: the marketplace takes over the body — quiet lead + market, no installed group',
    (tester) async {
      final repo = FixtureSettingsRepository()
        ..mcpRegistry.addAll(const [
          McpRegistryEntry(
      ...
      expect(
        find.text(t.settings.mcp.empty),
        findsNothing,
        reason: '墓碑句不再是空态主体',
      );
    },
  ...
  testWidgets(
    'market card: hover reveals the install CTA; a click installs in place → «已安装»',
    (tester) async {
      final repo = FixtureSettingsRepository()
        ..mcpRegistry.add(
      ...
      double revealOpacity() => tester
          .widget<Opacity>(
            find.ancestor(of: installBtn, matching: find.byType(Opacity)).first,
          )
          .opacity;
      ...
      expect(
        find.text(t.settings.mcp.browse),
        findsOneWidget,
        reason: '装了第一个→已装区长出+浏览钮再现',
      );
    },
  );

  testWidgets(
    'market card: a thrown install shows the honest red line in place; nothing lands',
    (tester) async {
      final repo = FixtureSettingsRepository()
