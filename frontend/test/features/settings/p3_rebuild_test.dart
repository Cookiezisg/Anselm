  ...
  ModelCapability(
      ...
      knobs: [
        ModelKnob(
            ...
            defaultValue: 'medium'),
        ModelKnob(key: 'logprobs', label: 'Logprobs', type: 'bool', defaultValue: 'false'),
      ]),
  ModelCapability(
      apiKeyId: 'aki_2',
      keyName: 'Second key',
...
void main() {
  ...
  group('ModelPickerPanel 三段面板', () {
    testWidgets('credential → model (specs in meta) → knobs prefilled → apply payload', (tester) async {
      String? gotKey;
      String? gotModel;
      Map<String, String>? gotOptions;
      await tester.pumpWidget(_host(ModelPickerPanel(
        caps: _caps,
        onApply: (k, m, o) {
          gotKey = k;
          gotModel = m;
      ...
      expect(find.textContaining(t.settings.keys.visionBadge), findsOneWidget, reason: '视觉徽');
      ...
      expect(gotOptions, {'reasoning_effort': 'high'}, reason: 'default 值不进 options');
    });

    testWidgets('switching credential resets the model stage', (tester) async {
      await tester.pumpWidget(_host(ModelPickerPanel(caps: _caps, onApply: (_, _, _) {})));
      await tester.pumpAndSettle();
      await tester.tap(find.text('Main key'));
      await tester.pumpAndSettle();
      ...
      expect(
          tester
              .widget<Text>(find.text(t.settings.keys.pickerApply))
              .style,
          isNotNull);
    });

    testWidgets('zero capabilities → guidance + the add-key jump (no dead dropdown)', (tester) async {
      var jumped = false;
      await tester.pumpWidget(_host(ModelPickerPanel(
        caps: const [],
        onApply: (_, _, _) {},
      ...
      await tester.pumpAndSettle();
      expect(find.text(t.settings.keys.noCapsGuide), findsOneWidget, reason: '零可用引导');
  ...
  group('KeyForm 添加流 baseUrl 门', () {
    ...
    testWidgets('ollama: save stays disarmed until the required base URL is filled', (tester) async {
      ...
      await tester.pumpAndSettle();
      expect(find.text(t.settings.keys.baseUrlRequiredHint), findsOneWidget, reason: '必填注记');
  ...
  group('brand_registry 品牌词元映射', () {
    test('io.github.* namespaces never read as the GitHub brand', () {
      ...
      expect(mcpBrandFor('io.github.hashicorp/terraform-mcp-server'), 'terraform');
    });
    ...
    test('every registered slug has a vendored asset name', () {
      for (final slug in kBrandAssets) {
        expect(slug, matches(RegExp(r'^[a-z0-9]+$')), reason: 'slug=资产文件名,$slug');
      }
    });
  });
  ...
  group('MCP 双列卡', () {
    ...
    testWidgets('installed roster renders brand cards; zero tool/call counts stay silent',
        (tester) async {
      final repo = FixtureSettingsRepository()
        ..mcpServers.addAll([
          McpServerStatus(id: 'mcp_1', name: 'github', status: 'ready', totalCalls: 42, tools: const [
            McpToolDef(serverName: 'github', name: 'search', description: ''),
          ]),
          const McpServerStatus(
              id: 'mcp_2', name: 'flaky', status: 'failed', lastError: 'connect refused'),
        ]);
      await tester.pumpWidget(host(repo, const McpPanel()));
      await tester.pumpAndSettle();
      expect(find.byType(AnCard), findsNWidgets(2), reason: '双列卡,一台一卡');
      expect(find.text('github'), findsOneWidget);
      expect(find.text('connect refused'), findsOneWidget, reason: '失败卡诚实错误句');
    ...
    testWidgets('marketplace lists the WHOLE registry by default; search only filters',
        (tester) async {
      final repo = FixtureSettingsRepository()
        ..mcpRegistry.addAll(const [
          McpRegistryEntry(name: 'io.github.getsentry/sentry-mcp', description: 'Errors'),
          McpRegistryEntry(
              name: 'com.supabase/mcp', description: 'Postgres platform', prerequisite: 'account'),
          McpRegistryEntry(name: 'oraios/serena', description: 'Coding agent toolkit'),
        ]);
      ...
      expect(find.text(t.settings.mcp.prerequisite), findsOneWidget, reason: '前置徽在卡上');
