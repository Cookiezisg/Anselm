...
void main() {
  ...
  test(
    'D-034 memories: a pinned row + user + AI notes (each source/pin state)',
    () async {
      ...
      expect(
        (await repo.listMemories(pinned: true)).every((m) => m.pinned),
        isTrue,
      );
    },
  );
  ...
  test(
    'D-032 MCP: a ready server (with tools) + a failed server + registry entries',
    () async {
      final servers = await repo.listMcpServers();
      expect(
        servers.any((s) => s.status == 'ready' && s.tools.isNotEmpty),
        isTrue,
        reason: '就绪+工具',
      );
      expect(
        servers.any(
          (s) => s.status == 'failed' && (s.lastError ?? '').isNotEmpty,
        ),
        isTrue,
        reason: '失败+诚实错误',
      );
  ...
  test(
    'D-033 sandbox: installed runtimes + a ready env under a function owner',
    () async {
