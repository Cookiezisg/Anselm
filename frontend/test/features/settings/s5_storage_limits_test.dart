...
void main() {
  ...
  testWidgets(
    'storage: backend-resolved data dir renders; factory zone stays locked until typed',
    (tester) async {
      ...
      await tester.tap(
        find.text(t.settings.storage.factoryConfirm),
        warnIfMissed: false,
      );
      await tester.pumpAndSettle();
  ...
  testWidgets(
    'retention: hydrates from the WIRE (never a hardcoded default) and commits on pick',
    (tester) async {
      final repo = FixtureSettingsRepository()
        ..fixtureDataDir = '/Users/x/.anselm'
      ...
      expect(
        find.text(t.settings.storage.retention30),
        findsOneWidget,
      ...
      await tester.pumpAndSettle();
      expect(
        repo.fixtureRetention.runRetentionDays,
        180,
        reason: '选中即提交,无 Save 钮',
      );
      final container = ProviderScope.containerOf(
        tester.element(find.byType(StoragePanel)),
        listen: false,
      );
  ...
  testWidgets(
    'retention: «forever» commits 0 — the sweeper must be switchable OFF',
    (tester) async {
  ...
  // ── 数据库 (磁盘回收, WRK-070 T4) ──
  testWidgets(
    'database: footprint shows the DB size AND the reclaimable dead space (honest)',
    (tester) async {
      final repo = FixtureSettingsRepository()
      ...
      expect(
        find.text(t.settings.storage.database),
        findsWidgets,
        reason: '数据库节存在',
      );
      ...
      expect(
        find.text(
          t.settings.storage.dbFootprint(
            size: formatBytes(120 * 1024 * 1024),
            dead: formatBytes(48 * 1024 * 1024),
          ),
        ),
        findsOneWidget,
        reason: '诚实展示库大小 + 死空间',
  ...
  testWidgets(
    'database: compact shows a BUSY state while locked, then reclaims and refetches',
    (tester) async {
      final repo = FixtureSettingsRepository()
        ..fixtureDataDir = '/tmp/x'
        ..fixtureDbBytes = 120 * 1024 * 1024
      ...
      await tester
          .pump(); // let setState(_busy=true) land, but the gate keeps compact in flight

      // Busy: label flips to «Compacting…» + a spinner, and the button is disabled (no re-tap).
      // 忙态:标签翻成「压缩中…」+ 转圈,按钮禁用(不可重复点)。
      expect(
        find.text(t.settings.storage.compacting),
        findsWidgets,
        reason: '锁库期间显示忙态',
      );
  ...
  testWidgets(
    'database: a compact failure (disk full) stages honest error feedback',
    (tester) async {
      final repo = FixtureSettingsRepository()
        ..fixtureDataDir = '/tmp/x'
      ...
      expect(
        find.text(t.settings.storage.compact),
        findsOneWidget,
        reason: '失败后按钮回可用',
      );
    },
