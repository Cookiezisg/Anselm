...
void main() {
  ...
  Widget host(SettingsPrefs prefs) => ProviderScope(
    ...
    child: TranslationProvider(
      child: MaterialApp(
        ...
        theme: AnTheme.light(),
        home: const Scaffold(
          body: SingleChildScrollView(child: ShortcutsPanel()),
        ),
      ),
    ),
  );
  ...
  testWidgets('capturing a fresh chord rebinds the command live', (
    tester,
  ) async {
    ...
    await tester.tap(
      find.text(commandLabel(t, ShortcutCommand.toggleLeftIsland)),
    );
    await tester.pump();
  ...
  testWidgets(
    'after a capture completes the row stops intercepting (stray chords do not rebind)',
    (tester) async {
      final prefs = SettingsPrefs.inMemory();
      await tester.pumpWidget(host(prefs));
      await tester.pumpAndSettle();
      ...
      expect(
        container
            .read(shortcutBindingsProvider)[ShortcutCommand.toggleLeftIsland]!
            .key,
