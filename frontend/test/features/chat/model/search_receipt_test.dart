...
ToolReceipt? r(String out, {String key = 'functions'}) => searchReceipt(
  out,
  listKey: key,
  hits: _hits,
  hitsOfTotal: _hitsTotal,
  empty: _empty,
);
...
void main() {
  group('parseSearchHits — shape probing by key existence', () {
    test('engine path: has total → truncation known', () {
      final h = parseSearchHits(
        '{"count":20,"total":47,"functions":[{"id":"fn_1","name":"a"}]}',
        'functions',
      );
      expect(h!.count, 20);
      expect(h.total, 47);
    ...
    test('fallback path: no total', () {
      final h = parseSearchHits(
        '{"count":3,"functions":[{"id":"fn_1"},{"id":"fn_2"},{"id":"fn_3"}]}',
        'functions',
      );
      expect(h!.count, 3);
    ...
    test(
      'nil-slice defense: count 0 with the list key entirely absent is a valid empty',
      () {
    ...
    test(
      'broken shape: count>0 but list missing → null (never a phantom count)',
      () {
        expect(parseSearchHits('{"count":5}', 'functions'), isNull);
        expect(
          parseSearchHits('{"count":5,"functions":null}', 'functions'),
          isNull,
        );
        expect(
          parseSearchHits('{"count":5,"functions":[]}', 'functions'),
          isNull,
        );
      },
    );
    ...
    test('no JSON / no int count → null', () {
      ...
      expect(
        parseSearchHits('{"functions":[]}', 'functions'),
        isNull,
      ); // no count
  ...
  group('searchReceipt — N / N·共M / empty', () {
    test('plain count', () {
      ...
      expect(
        r('{"count":12,"functions":[{"id":"a"}]}')!.tone,
    ...
    test('server-truncated → hitsOfTotal', () {
      expect(
        r('{"count":20,"total":47,"functions":[{"id":"a"}]}')!.text,
        '20 of 47',
      );
    });
    ...
    test('total == count is NOT truncation (exact page)', () {
      expect(
        r('{"count":5,"total":5,"functions":[{"id":"a"}]}')!.text,
        '5 hits',
      );
    });

    test(
      'count 0 → honest empty (receipt IS the card, never falls through)',
      () {
    ...
    test('search_blocks soft-empty string → empty', () {
      expect(
        r(
          'No blocks matched "http retry". Try different keywords.',
          key: 'blocks',
        )!.text,
        'empty',
      );
    ...
    test('unparseable → null (never guess)', () {
      ...
      expect(
        r('{"count":9}'),
        isNull,
      ); // count claims hits, list missing → broken
    });
  });
}
