...
void main() {
    ...
    final r = BlockTreeReducer()
      ...
      ..apply(
        const StreamEnvelope(
          ...
          scope: scope,
          id: 'tc_1',
          frame: FrameDelta(chunk: '{"command":"ls'),
        ),
      );
  ...
  test(
    'closed without tool_result → running; awaitingConfirm overlay wins',
    () {
      final node = _call();
      expect(ToolCardState.of(node).phase, ToolCardPhase.running);
      expect(
        ToolCardState.of(node, awaitingConfirm: true).phase,
  ...
  test('terminals split on the tool_result child', () {
    ...
    final failed = _call()
      ..children.add(_result('FN_NOT_FOUND: no such function', error: true));
  ...
  test(
    'cancelled tool_call (args-phase kill) → cancelled regardless of children',
    () {
  ...
  test('awaitingConfirm never overrides a landed result', () {
    final ok = _call()..children.add(_result('{"ok":true}'));
    expect(
      ToolCardState.of(ok, awaitingConfirm: true).phase,
      ToolCardPhase.succeeded,
    );
  });

  test(
    'progress text reads the wire `text` key (snapshot) and falls back to deltas (live)',
    () {
      final settled = _call()
        ..children.add(
  ...
  test(
    'nested subagent MESSAGE wrapper is flattened into the trajectory (real backend shape)',
    () {
      ...
      final msg = BlockNode(id: 'sub_msg', kind: BlockKind.message)
        ..status = 'completed';
      msg.children.addAll([
        BlockNode(id: 'n1', kind: BlockKind.reasoning)
          ..content = {'content': '先定位注册表'},
        BlockNode(id: 'n2', kind: BlockKind.toolCall)
          ..content = {'name': 'Grep'},
      ...
      expect(
        ...
        3,
        reason:
            'the message wrapper must be flattened to its 3 trajectory blocks',
      );
      expect(s.nested.map((n) => n.kind), [
        BlockKind.reasoning,
        BlockKind.toolCall,
        BlockKind.text,
      ]);
  ...
  test(
    'raw E3 blocks directly under the call still nest (fixture / reload path)',
    () {
  ...
  test(
    'memoization: same (revision, awaitingConfirm) returns the SAME instance; a frame re-derives',
    () {
      const scope = StreamScope(kind: 'conversation', id: 'cv_1');
      final r = BlockTreeReducer()
        ..apply(
