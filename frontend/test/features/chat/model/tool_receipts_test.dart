...
void main() {
  ToolReceipt? bashR(String out) => bashReceipt(
    out,
    exitLabel: exitLabel,
    timedOutLabel: '超时',
    blockedLabel: '已拦截',
  ...
  group('bashReceipt', () {
    ...
    test(
      'note priority: blocked > timeout > cancelled (all close exit -1, note must win)',
      () {
    ...
    test('background spawn → «bsh_… · 后台» (muted), NOT null', () {
      expect(
        bashR(
          'Started background command (bash_id=bsh_1a2b3c): npm run dev\nUse BashOutput…',
        ),
    ...
    test(
      'double-cap: the general tool-result cap ate the footer → exit unknown (warn)',
      () {
        expect(
          bashR('huge output …[tool result truncated: 4096 bytes]')!.text,
          'exit 未知',
        );
      },
    );
  ...
  group('statusReceipt (BashOutput)', () {
    ToolReceipt? st(String out) => statusReceipt(
      out,
      running: '运行中',
      exited: (c) => '退出 $c',
      killed: '已终止',
    ...
    test(
      'four states + not-found; exited/errored danger, running/killed neutral',
      () {
        expect(st('log\n\n[status: running]'), (
          text: '运行中',
          tone: ToolReceiptTone.none,
        ...
        expect(st('Background shell process not found: bsh_9'), (
          text: '会话不存在',
          tone: ToolReceiptTone.danger,
        ));
  ...
  group('readReceipt — four quadrants (F × truncated)', () {
    ToolReceipt? r(String out) => readReceipt(
      out,
      lines: (l) => '$l 行',
      range: (f, l) => '行 $f–$l',
      linesFloor: (n) => '$n+ 行',
  ...
  group('fsErrorKind — classify every fs error prefix', () {
    test('the nine kinds', () {
      ...
      expect(
        fsErrorKind('path is denied by safety guard: /a')!.kind,
        FsErrorKind.denied,
      );
      expect(
        fsErrorKind(
          'File must be read first before editing: /a. Use the Read tool first.',
        )!.kind,
        FsErrorKind.readFirst,
      );
      expect(
        fsErrorKind('old_string not found in the file. Verify …')!.kind,
        FsErrorKind.noMatch,
      );
      final amb = fsErrorKind(
        'Found 3 matches of old_string in /a, but replace_all is false. …',
      )!;
      expect(amb.kind, FsErrorKind.ambiguous);
      expect(amb.n, 3);
      expect(
        fsErrorKind(
          'File has been modified since last read (current size 10, expected 8): /a',
        )!.kind,
        FsErrorKind.modified,
      );
      expect(
        fsErrorKind('Parent directory does not exist: /a. Use Bash …')!.kind,
        FsErrorKind.parentMissing,
      );
      expect(fsErrorKind('path must be absolute')!.kind, FsErrorKind.badPath);
      expect(
        fsErrorKind('Write failed (writing temp): disk full')!.kind,
        FsErrorKind.failed,
      );
    });
  ...
  group('countReceipt', () {
    ...
    test('honest empties and errors', () {
      ...
      expect(
        countReceipt(
          'Cannot access /nope: permission denied',
          countLabel: count,
          noneLabel: '无匹配',
        ),
        isNull,
  ...
  group('argString — streaming-tolerant extraction', () {
    test('complete args', () {
      expect(
        argString('{"file_path":"/a/b.py","x":1}', 'file_path'),
        '/a/b.py',
      );
    });
    ...
    test('PARTIAL fragment mid-stream still yields a closed earlier field', () {
      expect(
        argString('{"command":"npm test","summary":"Run the te', 'command'),
        'npm test',
      );
      expect(
        argString('{"command":"npm te', 'command'),
        isNull,
      ); // value not closed yet 未闭合
    });

    test('escapes unescaped', () {
      expect(
        argString(r'{"command":"echo \"hi\"\nls"}', 'command'),
        'echo "hi"\nls',
      );
    });
  });
