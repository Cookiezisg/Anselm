...
void main() {
  test('entrance debounce: stages only if still open after 500ms; a short op NEVER stages', () {
  ...
  test('the stage-worthy closed set: get/read/search/delete/gate verbs never stage', () {
  ...
  test('followMode never: nothing auto-stages; firstPerConversation: only the first', () {
    ...
    expect(first.state.stageOpen, isFalse); // second act needs the user 第二场要用户开
  });
  ...
  test('抢镜 VETO: pinned means NOTHING auto-switches — pill + unread only', () {
    ...
    expect(d.state.phase, StagePhase.pinned);
    expect(d.state.followPillTarget!.blockId, 'b2'); // the pill offers, never takes 药丸只提示
  ...
  test('switch arbitration: same-priority newcomer waits for idle 800ms + dwell 2400ms', () {
    final d = StageDirector();
    d.onToolOpen('b1', 'create_function', _t(0));
    d.advance(_t(500)); // staged at 500 登台
    d.onToolOpen('b2', 'create_document', _t(600));
    d.advance(_t(1100)); // b2 entrance due — but b1 active + dwell unmet b1 仍活跃
  ...
  test('priority ladder: a BUILD preempts a subagent broadcast without waiting', () {
    final d = StageDirector();
    ...
    expect(d.state.subject!.blockId, 'b2');
    expect(d.state.channels.single.blockId, 'b1'); // subagent keeps a tab 子代理进频道条
  });
  ...
  test('curtain: close + breath → idle; live work preempts the curtain', () {
    ...
    d2.advance(_t(3300)); // b2's entrance (2800+500) lands during the breath 防抖到点在停拍窗内
    d2.advance(_t(4800));
  ...
  test('failed close → failed-hold; new work displaces it into a red-dot tab (scene kept)', () {
    final d = StageDirector();
    d.onToolOpen('b1', 'create_function', _t(0));
    d.advance(_t(500));
    ...
    d.advance(_t(30000));
    expect(d.state.subject!.blockId, 'b1'); // holds until displaced/dismissed 驻留
  ...
  test('dismiss: still-live activities re-earn the stage through a fresh debounce', () {
    final d = StageDirector();
    d.onToolOpen('b1', 'create_function', _t(0));
    d.advance(_t(500));
    d.onToolOpen('b2', 'create_document', _t(600));
  ...
  test('gate flag rides the state; unread resets when an activity takes the stage', () {
    final d = StageDirector();
