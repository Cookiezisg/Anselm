...
void main() {
  test('C-018 arg reads a top-level string value (matches argStringPartial)', () {
    final s = _state('{"file_path":"/a/b.py","command":"go test ./..."}');
    expect(s.arg('file_path'), '/a/b.py');
    expect(s.arg('command'), 'go test ./...');
    expect(argStringPartial(s.argsText, 'file_path'), '/a/b.py'); // the old helper agrees 旧 helper 一致
  });
  ...
  test('C-018 \\n / \\t decode the same as the old scan (common case)', () {
    ...
    expect(argStringPartial(s.argsText, 'msg'), 'line1\nline2\ttab', reason: r'\n \t 两者一致');
  });

  test('C-018 \\r decodes CORRECTLY via the session (the fix over the old char-scan)', () {
    ...
    expect(s.arg('x'), isNot(argStringPartial(s.argsText, 'x')), reason: '迁移=同或更好');
  });

  test('C-018 arg reads a NESTED key at any depth (depth-agnostic, like argStringPartial)', () {
