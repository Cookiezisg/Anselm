...
void main() {
  test('C-005 revertReceipt reads the version correctly (behaviour preserved)', () {
    final r = revertReceipt('{"id":"fn_1","version":7}', rewind: (v) => 'rolled back to v$v');
  ...
  test('C-005 budget: re-parsing the SAME large result is O(1) after the first (cache hit)', () {
    // A large result JSON — without the cache, 5000 re-renders re-parse it every time. 大结果 JSON。
    final big = jsonEncode({'version': 4, 'log': List.filled(4000, 'a line of output').join('\n')});
