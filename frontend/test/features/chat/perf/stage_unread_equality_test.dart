...
StageActivityView _view({bool live = true, bool failed = false, int unread = 0, String? itemId}) =>
    StageActivityView(
        blockId: 'b1', toolName: 'run_function', kind: 'function',
        live: live, failed: failed, unread: unread, itemId: itemId);
...
void main() {
  group('C-003 StageActivityView equality ignores unread', () {
    ...
    test('a real change (live / failed / itemId) still breaks equality', () {
      expect(_view(live: true), isNot(_view(live: false)), reason: 'close 广播不吞');
      expect(_view(failed: false), isNot(_view(failed: true)));
      expect(_view(itemId: null), isNot(_view(itemId: 'fn_1')), reason: 'Cast 脉冲仍到');
    });
  });
  ...
  group('C-003 StageState skips no-op broadcasts on channel unread churn', () {
    StageState state(int chUnread) => StageState(
          ...
          channels: [
            StageActivityView(
                blockId: 'b2', toolName: 'call_handler', kind: 'handler',
                live: true, failed: false, unread: chUnread, itemId: 'other'),
          ],
        );
