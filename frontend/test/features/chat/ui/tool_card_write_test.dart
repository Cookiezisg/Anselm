...
void main() {
  ...
  testWidgets('LIVE: collapsed by default; TAP opens the growth show (WRK-065)', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      _host(
        ChatToolCard(
          node: _streaming(
            '{"file_path":"/ws/a.py","content":"line1\\nline2\\nline3\\ndef f(): pass',
          ),
        ),
      ),
    );
    await tester.pump();
    expect(
      find.textContaining('def f(): pass'),
      findsNothing,
    ); // default collapsed 默认收起
    ...
    expect(
      find.textContaining('def f(): pass'),
      findsOneWidget,
    ); // the streamed-so-far tail
  });

  testWidgets('SETTLED: highlighted code window + path chip + copy full content', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(
  ...
  testWidgets(
    'SETTLED: a long file stays in the SAME bounded viewport tier (批2 零跳变,折叠退役)',
    (tester) async {
      ...
      await tester.pumpWidget(
        _host(
          ChatToolCard(
            node: _settled(
              '{"file_path":"/ws/big.py","content":"$big"}',
              'Wrote /ws/big.py',
            ),
          ),
        ),
  ...
  testWidgets('empty content → no body (receipt says 空文件)', (tester) async {
    await tester.pumpWidget(
      _host(
        ChatToolCard(
          node: _settled(
            '{"file_path":"/ws/empty.txt","content":""}',
            'Wrote /ws/empty.txt',
          ),
        ),
      ),
    ...
    await tester.pump();
    expect(
      find.textContaining(t.chat.tool.emptyFile),
      findsOneWidget,
    ); // receipt
