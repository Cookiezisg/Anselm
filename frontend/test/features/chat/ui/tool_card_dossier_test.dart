...
BlockNode _get(String name, String args, Map<String, dynamic> record) =>
    BlockNode(id: 'tc_g', kind: BlockKind.toolCall)
      ..status = 'completed'
      ..content = {'name': name, 'arguments': args}
      ..children.add(
        BlockNode(id: 'tr_g', kind: BlockKind.toolResult)
          ..status = 'completed'
...
void main() {
  ...
  group('receipts', () {
    test('statusElapsedReceipt: ok→grey, failed/timeout→danger', () {
      expect(
        statusElapsedReceipt(t, 'ok', 942)!.tone,
        isNot(ToolReceiptTone.danger),
      );
      expect(
        statusElapsedReceipt(t, 'failed', 30021)!.tone,
        ToolReceiptTone.danger,
      );
      expect(
        statusElapsedReceipt(t, 'timeout', 60000)!.tone,
        ToolReceiptTone.danger,
      );
      expect(statusElapsedReceipt(t, '', null), isNull);
    });
    test(
      'activationFireReceipt: fired→已fire·扇出N; not fired→grey; error→danger',
      () {
        final f = activationFireReceipt(
          t,
          jsonEncode({'fired': true, 'firingCount': 2}),
        )!;
        expect(f.text, contains(t.chat.tool.fireYes));
        expect(f.text, contains('2'));
        expect(f.tone, isNot(ToolReceiptTone.danger));
        expect(
          activationFireReceipt(
            t,
            jsonEncode({'fired': false, 'firingCount': 0}),
          )!.text,
          t.chat.tool.fireNo,
        );
        expect(
          activationFireReceipt(
            t,
            jsonEncode({'fired': false, 'error': 'probe crashed'}),
          )!.tone,
          ToolReceiptTone.danger,
        );
      },
  ...
  testWidgets(
    'fn execution ok: dossier head + input/output + log drawer + provenance',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
            node: _get('get_function_execution', '{"executionId":"fnexec_1"}', {
              'id': 'fnexec_1',
              'status': 'ok',
              'triggeredBy': 'chat',
              'input': {'url': 'https://x'},
      ...
      await tester.pump();
      await tester.tap(
        find.textContaining(t.chat.tool.gotFnExec),
        warnIfMissed: false,
      );
      ...
      expect(
        find.textContaining(t.run.provConversation),
        findsOneWidget,
  ...
  testWidgets(
    'fn execution failed auto-expands, shows errorMessage; flowrunId = mono badge',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
            node: _get(
              ...
              '{"executionId":"fnexec_bad"}',
              {
                'id': 'fnexec_bad',
                'status': 'failed',
                'triggeredBy': 'workflow',
                'input': {'url': 'https://down'},
                'errorMessage': 'ConnectionError: max retries exceeded',
                'elapsedMs': 30021,
                'startedAt': '2026-07-05T14:03:00Z',
      ...
      await tester.pumpAndSettle();
      expect(
        find.textContaining('ConnectionError'),
        findsOneWidget,
      ); // auto-expanded
      expect(
        find.textContaining(t.run.provFlowrun),
        findsOneWidget,
      ); // flowrun mono badge
    },
  );
  ...
  testWidgets(
    'mcp call: the stderr tail splits into its own danger segment on the fixed separator',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
      ...
      await tester.tap(
        find.textContaining('日志'),
      ); // open the log drawer (LogDrawer 计行标签)
      await tester.pumpAndSettle();
      expect(
        find.text(t.chat.tool.dossierStderr),
        findsOneWidget,
      ); // the stderr segment header (danger)
      expect(find.textContaining('Traceback: TimeoutError'), findsOneWidget);
      expect(
        find.textContaining('main log line'),
        findsOneWidget,
      ); // main segment
  ...
  testWidgets(
    'get_agent_execution: modelId micro + hydrated TranscriptPeek (thought/tool/reply)',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
            node: _get('get_agent_execution', '{"executionId":"agexec_1"}', {
              'id': 'agexec_1',
              'agentId': 'ag_1',
              ...
              'transcript': [
                ...
                },
                {
                  ...
                  'type': 'tool_call',
                  'content': '{}',
                  'attrs': {'tool': 'run_function', 'summary': 'fetch'},
                  'status': 'completed',
                },
                {
                  'type': 'text',
      ...
      await tester.pump();
      await tester.tap(
        find.textContaining(t.chat.tool.gotAgentExec),
        warnIfMissed: false,
      );
      await tester.pumpAndSettle();
      expect(find.byType(TranscriptPeek), findsOneWidget);
      expect(
        find.text('claude-sonnet-5'),
        findsOneWidget,
      ); // modelId micro badge
      expect(
        find.textContaining(t.chat.tool.transcriptSteps(n: '3')),
        findsOneWidget,
      ); // 3 blocks
      expect(
        find.textContaining('PLANNING_THE_WORK', findRichText: true),
        findsOneWidget,
      ); // reasoning line
      expect(
        find.textContaining('run_function'),
        findsWidgets,
      ); // tool_call row
      expect(
        find.textContaining('FINAL_REPLY_TEXT', findRichText: true),
        findsOneWidget,
      ); // text reply line
    },
  );
  ...
  testWidgets(
    'activation fired: fire conclusion + returnValue + payload + trigger provenance',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
      ...
      await tester.pump();
      await tester.tap(
        find.textContaining(t.chat.tool.gotActivation),
        warnIfMissed: false,
      );
      await tester.pumpAndSettle();
