...
void main() {
  ...
  test('approvalTemplateToMarkdown turns {{ input.x }} into an inline-code chip', () {
    expect(approvalTemplateToMarkdown('金额 {{ input.amount }} 元'), '金额 `amount` 元');
  ...
  testWidgets('control body: the decision ladder (ordered rows + catch-all + emit)', (tester) async {
    await tester.pumpWidget(_host(ChatToolCard(
        node: _node('create_control', _ctlArgs,
  ...
  testWidgets('approval body: the form preview (rendered template + rules + mock decision)', (tester) async {
    await tester.pumpWidget(_host(ChatToolCard(
        node: _node('create_approval', _apfArgs,
  ...
  testWidgets('approval with no timeout says never-times-out', (tester) async {
    await tester.pumpWidget(_host(ChatToolCard(
        node: _node('create_approval', '{"name":"a","template":"# x"}',
            '{"id":"apf_1","activeVersionId":"apfv_1","version":1}'))));
    await tester.pump();
    await tester.tap(find.textContaining('已创建审批'), warnIfMissed: false);
    await tester.pumpAndSettle();
