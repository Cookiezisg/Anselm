...
void main() {
  ...
  test('argString survives an MB-scale UNCLOSED value (null, no stack overflow)', () {
    final frag = '{"name":"war-and-peace.md","content":"${'x' * 1024 * 1024}';
    expect(argString(frag, 'content'), isNull); // unclosed → partial's business 未闭合归 partial
    expect(argString(frag, 'name'), 'war-and-peace.md');
    expect(argStringPartial(frag, 'content')!.length, 1024 * 1024);
  });

  test('argString survives an MB-scale CLOSED value (full decode, no stack overflow)', () {
    final v = '${'line\\n' * 200000}end'; // ~1.2MB with escapes 带转义
    final frag = '{"content":"$v"}';
    final out = argString(frag, 'content')!;
    expect(out.length, 200000 * 5 + 3); // \n decoded to 1 char per line 每行转义解码为 1 字符
    expect(out.endsWith('end'), isTrue);
    expect(out.startsWith('line\n'), isTrue);
  });

  test('argString skips a non-value "key" occurrence and honest whitespace', () {
    expect(argString('{"note":"the \\"content\\" word","content" : "real"}', 'content'), 'real');
    expect(argString('{"content":42}', 'content'), isNull); // non-string value 非字符串
  });

  testWidgets('a create_document card streaming ~800KB renders live without exceptions', (tester) async {
    ...
    for (var i = 0; i < 200; i++) {
      r.apply(StreamEnvelope(seq: 0, scope: _scope, id: 'tc', frame: FrameDelta(chunk: chunk)));
      await tester.pumpWidget(host());
    }
