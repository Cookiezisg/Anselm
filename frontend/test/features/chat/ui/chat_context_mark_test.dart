...
Widget _host(String marker, {double width = 680}) => TranslationProvider(
    child: MaterialApp(theme: AnTheme.light(), home: Scaffold(body: SizedBox(width: width, child: ChatContextMark(marker: marker)))));

void main() {
  setUpAll(() => LocaleSettings.setLocaleRaw('zh-CN'));

  testWidgets('parses the count off the marker → localized sentence', (tester) async {
    await tester.pumpWidget(_host('Context compacted — 42 earlier blocks folded into the running summary.'));
    expect(find.textContaining('42'), findsOneWidget);
    expect(find.textContaining('上下文已压缩'), findsOneWidget); // localized, not the English marker
    expect(find.textContaining('Context compacted', findRichText: true), findsNothing);
  });
  ...
  testWidgets('injection + overlong marker: inert, no overflow', (tester) async {
    await tester.pumpWidget(_host('Context compacted — 7 <script>alert(1)</script> ${'x' * 200}', width: 300));
    await tester.pump();
    expect(tester.takeException(), isNull);
