...
import 'package:anselm/features/chat/ui/tool_card_skins.dart'
    show WindowCopyButton, rawMonoWindow;
...
BlockNode _search(
  ...
  ..children.add(
    BlockNode(id: 'tr_s', kind: BlockKind.toolResult)
      ..status = 'completed'
...
void main() {
  ...
  group('aggregatesReceipt', () {
    test(
      'rollup ok✓·failed✗, always grey; empty → 无记录; unparseable → null',
      () {
        final r = aggregatesReceipt(
          t,
          _page('executions', [], ok: 41, failed: 4),
        )!;
        expect(r.text, contains('41'));
        expect(r.text, contains('4'));
        expect(
          r.tone,
          ToolReceiptTone.none,
        ); // NEVER danger — archive failures aren't this call failing
        expect(
          aggregatesReceipt(t, _page('executions', [], ok: 0, failed: 0))!.text,
          t.chat.tool.logNoRecords,
        );
    ...
    test('hasBody: records → true, none → false (receipt IS the card)', () {
      expect(
        aggregatesHasBody(
          _page('executions', [_exec('e1', 'ok')], ok: 1, failed: 0),
        ),
        isTrue,
      );
      expect(
        aggregatesHasBody(_page('executions', [], ok: 0, failed: 0)),
        isFalse,
      );
    });
  });
  ...
  testWidgets(
    'fn executions: bead strip + slim ledger; NO input/output/logs render',
    (tester) async {
              ...
              _page(
                ...
                [
                  _exec(
                    ...
                    'ok',
                    extra:
                        ',"output":{"secret":"SHOULD_NOT_RENDER"},"logs":"LOGLINE_SHOULD_NOT_RENDER"',
                  ),
      ...
      await tester.pump();
      await tester.tap(
        find.textContaining(t.chat.tool.searchedFnExec),
        warnIfMissed: false,
      );
      ...
      expect(
        find.textContaining(t.chat.tool.aggNote),
        findsOneWidget,
      ); // ✗ incl. cancelled/timeout note
    },
  );

  testWidgets(
    'fn executions header shows the resolved function NAME, not the id (B4)',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
  ...
  testWidgets('handler calls: method() chip renders', (tester) async {
            ...
            _page(
              ...
              [
                _exec(
                  'hdcall_01',
                  'ok',
                  extra: ',"method":"charge","instanceId":"inst_1"',
                ),
              ],
              ok: 12,
              failed: 1,
    ...
    await tester.pump();
    await tester.tap(
      find.textContaining(t.chat.tool.searchedHdCalls),
      warnIfMissed: false,
    );
    await tester.pumpAndSettle();
  ...
  testWidgets(
    'agent executions: the fat transcript on every row is NEVER rendered',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
              ...
              _page(
                ...
                [
                  _exec(
                    'agexec_01',
                    'timeout',
                    extra: ',"transcript":"TRANSCRIPT_30KB_SHOULD_NOT_RENDER"',
                  ),
                ],
                ok: 7,
                failed: 2,
      ...
      await tester.pump();
      await tester.tap(
        find.textContaining(t.chat.tool.searchedAgentExec),
        warnIfMissed: false,
      );
      await tester.pumpAndSettle();
      expect(find.text('agexec_01'), findsOneWidget);
      expect(
        find.textContaining('TRANSCRIPT_30KB_SHOULD_NOT_RENDER'),
        findsNothing,
      );
    },
  );
  ...
  testWidgets(
    'empty (no records): the receipt IS the card — no chevron, no body',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
  ...
  group('count families (flowruns/firings/activations)', () {
    test(
      'countListReceipt: N 条 / N+ 条 (hasMore); no fabricated ✓✗; empty → 无记录',
      () {
        final r = countListReceipt(
          t,
          jsonEncode({
            'count': 3,
            'firings': [1, 2, 3],
        ...
        expect(
          r.text,
          isNot(contains('✓')),
        ); // count families NEVER fabricate an ok/failed split
        expect(
          countListReceipt(
            t,
            jsonEncode({
    ...
    testWidgets(
      'flowruns: pageScoped bead strip + replay badge + failed subtext + park caption',
      (tester) async {
        await tester.pumpWidget(
          _host(
            ChatToolCard(
        ...
        await tester.pump();
        await tester.tap(
          find.textContaining(t.chat.tool.searchedFlowruns),
          warnIfMissed: false,
        );
        await tester.pumpAndSettle();
    ...
    testWidgets(
      'firings: disposition badges (started/skipped/pending); started carries a flowrunId',
      (tester) async {
        await tester.pumpWidget(
          _host(
            ChatToolCard(
        ...
        await tester.pump();
        await tester.tap(
          find.textContaining(t.chat.tool.searchedFirings),
          warnIfMissed: false,
        );
        await tester.pumpAndSettle();
        ...
        expect(
          find.textContaining('fr_abc123def'),
          findsOneWidget,
        ); // started → flowrunId chip (truncated to 12)
      },
    );

    testWidgets('activations: fired mark + returnValue expands inline on tap', (
      tester,
    ) async {
      await tester.pumpWidget(
        _host(
      ...
      await tester.pump();
      await tester.tap(
        find.textContaining(t.chat.tool.searchedActivations),
        warnIfMissed: false,
      );
      await tester.pumpAndSettle();
      expect(find.text('act_01'), findsOneWidget);
      expect(
        find.textContaining(t.chat.tool.actFanout(n: '2')),
        findsOneWidget,
      );
      ...
      await tester.pumpAndSettle();
      expect(
        find.textContaining('temp'),
        findsOneWidget,
      ); // now the returnValue tree is inline
  ...
  group('ToolIOSection bare 缝 (批4 复审 HIGH 的修法)', () {
    testWidgets('bare: a LIST renders the JSON tree with NO window of its own', (
      tester,
    ) async {
      ...
      await tester.pumpWidget(
        _host(const ToolIOSection(label: 'v', value: [1, 2, 'x'], bare: true)),
      );
      await tester.pumpAndSettle();
      expect(find.byType(AnWindow), findsNothing);
    });
    testWidgets(
      'not bare: the same list wears its own window (the inverse edge)',
      (tester) async {
        await tester.pumpWidget(
          _host(const ToolIOSection(label: 'v', value: [1, 2, 'x'])),
        );
        await tester.pumpAndSettle();
        expect(find.byType(AnWindow), findsOneWidget);
      },
    ...
    testWidgets(
      'value-shape split: an all-short-scalar map rides the family AnKv, bools on the flag row (批6 A-077)',
      (tester) async {
        await tester.pumpWidget(
          _host(
            const ToolIOSection(
              label: 'v',
              value: {'status': 200, 'ok': true, 'path': '/tmp/a'},
              bare: true,
            ),
          ),
        );
        ...
        expect(
          find.byType(AnKv),
          findsOneWidget,
        ); // the KV branch, not per-key sections KV 分支
        expect(
          find.text('✓'),
          findsOneWidget,
        ); // the bool routed to the flag row bool 走 flag 行
        expect(
          find.text('true'),
          findsNothing,
        ); // never the raw literal 绝不裸渲字面量
      },
    );
    testWidgets(
      'value-shape split: a map holding a long value keeps per-key label-above recursion',
      (tester) async {
        ...
        expect(
          find.byType(AnKv),
          findsNothing,
        ); // a long value never squeezes into the KV face 长值不进 KV 脸
        expect(
          find.text('summary'),
          findsOneWidget,
        ); // its own per-key section head 逐键节头
        expect(find.text('n'), findsOneWidget);
      },
    );
    testWidgets(
      'bare + renderAsProse inside a host window: typeset bare, no nested window (leaf law)',
      (tester) async {
        ...
        expect(
          ...
          findsOneWidget,
        ); // only the host — ProseWindow's shell withheld 仅宿主窗
      },
    );
    testWidgets(
      'bare truncated mono keeps the full-payload copy escape (显示可截,copy 永不截)',
      (tester) async {
        final big = 'x' * 7000;
        await tester.pumpWidget(
          _host(
            AnWindow(
              child: ToolIOSection(label: 'v', value: big, bare: true),
            ),
          ),
        );
        await tester.pumpAndSettle();
  ...
  group('rawMonoWindow 共享 helper (A-003)', () {
    testWidgets(
      'wraps mono text in the family window; maxLines tier caps the render',
      (tester) async {
        await tester.pumpWidget(
          _host(
            Builder(
              builder: (c) =>
                  rawMonoWindow(c, 'k=v', maxLines: AnCap.monoErrorLines),
            ),
          ),
        );
        await tester.pumpAndSettle();
    ...
    testWidgets('null maxLines = unbounded (clip, no ellipsis)', (
      tester,
    ) async {
      await tester.pumpWidget(
        _host(Builder(builder: (c) => rawMonoWindow(c, 'x'))),
      );
      await tester.pumpAndSettle();
