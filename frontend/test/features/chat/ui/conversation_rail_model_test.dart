...
void main() {
  ...
  group('buildConversationRailModel', () {
    test(
      'two icon section heads (Pinned + Recents), counts, server order within each',
      () {
        final rows = [
          ...
          _cAt(
            'cv_b',
            DateTime(2026, 6, 1, 9),
          ), // old, but no time bucketing → still Recents
        ];
        final types = buildConversationRailModel(
          rows,
          now: _now,
          labels: _labels,
        ).groups.single.types;
        ...
        expect(
          types.every((t) => t.icon != null),
          isTrue,
        ); // both sections are icon'd (entities head style)
        expect(types.first.rows.single.id, 'cv_pin');
        expect(types[1].rows.map((r) => r.id), [
          'cv_a',
          'cv_b',
        ]); // recents keep server order
    ...
    test(
      'zero conversations → both Pinned + Recents heads render (empty), no counts',
      () {
        final types = buildConversationRailModel(
          const [],
          now: _now,
        ...
        expect(types.map((t) => t.label), [
          'PINNED',
          'RECENTS',
        ]); // both heads, unlike the "no pinned" case
        expect(types.every((t) => t.rows.isEmpty), isTrue); // empty bodies
        expect(
          types.every((t) => t.count == null),
          isTrue,
        ); // no "0" on an empty head
      },
    );
    ...
    test(
      ...
      () {
        ...
        expect(
          types.every((t) => t.count == null),
          isTrue,
        ); // no count badge on the heads
        expect(
          types.expand((t) => t.rows).every((r) => r.meta == null),
          isTrue,
        ); // no relative time on rows
      },
    );
  });
}
