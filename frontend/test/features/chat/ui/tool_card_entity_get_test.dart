...
void main() {
  ...
  testWidgets(
    'AnKvRow row-level mono: a mono row renders monospace, a prose row does not',
    (tester) async {
  ...
  testWidgets(
    'ToolEntityHeader: navigable kind → tappable pill; no-panel kind → inert',
    (tester) async {
      await tester.pumpWidget(
        _host(
  ...
  testWidgets(
    'EntityCodeWindow: over the char cap → a truncation note; under → none',
    (tester) async {
      await tester.pumpWidget(
        _host(
          EntityCodeWindow(
            code: 'x' * (kEntityContentCap + 10),
            lang: 'python',
          ),
        ),
      );
      ...
      await tester.pumpWidget(
        _host(const EntityCodeWindow(code: 'print(1)', lang: 'python')),
      );
      await tester.pumpAndSettle();
  ...
  testWidgets('RawResultDisclosure: toggles the unfiltered JSON tree', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(
        const RawResultDisclosure(
          rawJson: '{"id":"fn_1","secret":"visible-in-raw"}',
        ),
      ),
    );
    await tester.pumpAndSettle();
