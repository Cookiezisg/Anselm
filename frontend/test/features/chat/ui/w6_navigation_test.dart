...
void main() {
  ...
  testWidgets('deep jump re-anchors on the target + shows the back-to-present pill; '
    ...
    await tester.pumpWidget(
      _host(repo, ChatTranscriptView(conversationId: _conv)),
    );
    await tester.pump();
    ...
    final pos = tester
        ...
        .position;
    expect(
      (pos.maxScrollExtent - pos.pixels).abs() < 2,
      isTrue,
      reason:
          'rejoin docks to the present 归队即贴底 (pixels=${pos.pixels}, max=${pos.maxScrollExtent})',
    );
    expect(
      find.textContaining('第 59 回内容', findRichText: true),
      findsOneWidget,
      reason: 'the newest turn is on screen 最新回合在屏',
    );
  });

  testWidgets('场次条: the drawer groups anchors and a tap fires the jump', (
    tester,
  ) async {
    ...
    expect(
      find.text(t.chat.toc.toolCluster(n: 2)),
      findsOneWidget,
    ); // ⚙ 2 folded 折叠簇
    expect(
      find.text('delete_function · fn_old'),
      findsOneWidget,
    ); // danger surfaces 危险露出
    ...
    final sub = container.listen(
      transcriptJumpProvider(_conv),
      (_, next) => received = next,
    );
  ...
  testWidgets(
    'Cast 点行登台: an entity row pins the identity-face exhibit; attachment gets the 展品座',
    (tester) async {
      ...
      repo.touchpoints[_conv]![1] = repo.touchpoints[_conv]![1].copyWith(
        itemId: att.id,
      );
      ...
      await tester.pumpWidget(
        _host(
          repo,
          SizedBox(width: 340, child: StagePanel(conversationId: _conv)),
        ),
      );
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 100));
      ...
      expect(
        find.text(t.feedback.cast.verb.edited),
        findsWidgets,
      ); // verb history 动词史
