...
void main() {
  ...
  test('docSentenceReceipt: success → path tail; soft-fail sentence → warn 未生效', () {
    final ok = docSentenceReceipt(t, _state('create_document', '{}', 'Created document "口径" (id=doc_1, path=/specs/口径).'));
    expect(ok!.text, '口径');
    expect(ok.tone, ToolReceiptTone.none);
    final bad = docSentenceReceipt(t, _state('create_document', '{}', 'Parent folder not found. Call list_documents.'));
  ...
  testWidgets('document body: typeset prose window + auto-rename note', (tester) async {
    await tester.pumpWidget(_host(ChatToolCard(node: _node('create_document',
    ...
    expect(find.textContaining('季度汇总口径'), findsOneWidget); // the rendered heading
    expect(find.textContaining('自动改名'), findsOneWidget); // the auto-rename note
  });

  testWidgets('document soft failure reframes the English sentence as an amber note (no prose window)', (tester) async {
    await tester.pumpWidget(_host(ChatToolCard(node: _node('edit_document',
    ...
    await tester.pumpAndSettle();
    expect(find.textContaining('A sibling document already has'), findsOneWidget);
    expect(find.byType(ProseWindow), findsNothing);
  });

  testWidgets('skill body: prose + allowedTools WARN chips + pre-auth note', (tester) async {
    await tester.pumpWidget(_host(ChatToolCard(node: _node('create_skill',
  ...
  testWidgets('edit_skill shows the no-revert small print', (tester) async {
    await tester.pumpWidget(_host(ChatToolCard(node: _node('edit_skill',
