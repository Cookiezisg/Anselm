...
BlockNode _node(
  ...
  if (progress != null) {
    call.children.add(
      BlockNode(id: 'pg_x', kind: BlockKind.progress)
        ..status = 'completed'
...
void main() {
  ...
  group('execReceipt', () {
    test('ok → elapsed, no danger', () {
      final r = execReceipt(
        t,
        '{"ok":true,"output":1,"errorMsg":"","elapsedMs":942}',
      )!;
      expect(r.text, '942ms');
    ...
    test('ok:false → red 运行失败 · elapsed', () {
      final r = execReceipt(
        t,
        '{"ok":false,"output":null,"errorMsg":"boom","elapsedMs":30021}',
      )!;
    ...
    test('unparseable → null (a non-exec string never fakes a receipt)', () {
      ...
      expect(
        execReceipt(t, '{"result":5}'),
        isNull,
      ); // call_handler shape, not exec
    });
  });
  ...
  group('execResultFailed', () {
    test('ok:false → true (green but broken); ok:true / non-exec → false', () {
      expect(
        execResultFailed(
          '{"ok":false,"output":null,"errorMsg":"x","elapsedMs":1}',
        ),
        isTrue,
      );
      expect(
        execResultFailed('{"ok":true,"output":1,"errorMsg":"","elapsedMs":1}'),
        isFalse,
      );
  ...
  testWidgets('run_function ok: input keys + output + exec bar (运行成功 · elapsed)', (
    tester,
  ) async {
    ...
    await tester.tap(
      find.textContaining(t.chat.tool.ranFn),
      warnIfMissed: false,
    );
  ...
  testWidgets(
    'run_function target chip shows the resolved entity NAME, not the id (B3/B4)',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
  ...
  testWidgets(
    'run_function falls back to the truncated id when no name is resolved',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
      ...
      await tester.pump();
      expect(
        find.textContaining('fn_1a2b3c4d'),
        findsOneWidget,
      ); // truncated id chip (12 + …)
    },
  );

  testWidgets('run_function ok:false auto-expands, shows red errorMsg + 失败 bar', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(
    ...
    expect(
      find.textContaining('ConnectionError: max retries exceeded'),
      findsOneWidget,
    );
    expect(
      find.text(t.chat.tool.execFailed),
      findsWidgets,
  ...
  testWidgets('run_function logs drawer folds print() output', (tester) async {
    ...
    await tester.pumpAndSettle();
    expect(
      find.textContaining(t.chat.tool.execLogs(n: '3')),
      findsOneWidget,
    ); // 日志 · 3 行
  });

  testWidgets(
    'call_handler: method() label, scalar result preview receipt, no fabricated elapsed',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
      ...
      await tester.tap(
        find.textContaining(t.chat.tool.calledMethod),
        warnIfMissed: false,
      );
      await tester.pumpAndSettle();
  ...
  group('invoke_agent', () {
    ...
    test(
      'receipt: ok→steps·elapsed (none); failed/timeout→danger; cancelled→grey',
      () {
        expect(
          invokeReceipt(t, res('ok', output: '"done"', steps: 9))!.text,
          contains(t.chat.tool.agentSteps(n: '9')),
        );
        expect(
          invokeReceipt(t, res('ok', output: '"done"'))!.tone,
        ...
        expect(
          invokeReceipt(t, res('cancelled'))!.tone,
          isNot(ToolReceiptTone.danger),
        ); // user stop, not red
    ...
    test(
      'resultFailed: failed/timeout true; ok/cancelled false (cancelled never auto-expands)',
      () {
    ...
    testWidgets(
      'ok string output → prose; stat bar shows steps/tokens + agent pill + executionId',
      (tester) async {
        await tester.pumpWidget(
          _host(
            ChatToolCard(
        ...
        await tester.pump();
        await tester.tap(
          find.textContaining(t.chat.tool.invokedAgent),
          warnIfMissed: false,
        );
        await tester.pumpAndSettle();
        expect(
          find.textContaining('季度已归类'),
          findsOneWidget,
        ); // the prose answer
        expect(
          find.textContaining(t.chat.tool.agentSteps(n: '9')),
          findsWidgets,
        ); // steps in bar
        expect(find.textContaining('↑8420 ↓1203'), findsOneWidget); // tokens
        expect(
          find.textContaining('ag_9f8e7d6c5b4a3f2e'),
          findsWidgets,
        ); // navigable agent pill
        expect(
          find.textContaining('agexec_1a2b3c4d5e6f7a8b'),
          findsWidgets,
        ); // executionId copy
      },
    );

    testWidgets('failed auto-expands and shows the red errorMsg', (
      tester,
    ) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
            node: _node(
              ...
              res(
                'failed',
                errorMsg:
                    'AGENT_OUTPUT_NOT_STRUCTURED: prose but schema needs int',
              ),
            ),
          ),
        ),
      ...
      await tester.pumpAndSettle();
      expect(
        find.textContaining('AGENT_OUTPUT_NOT_STRUCTURED'),
        findsOneWidget,
      ); // auto-expanded
    });
  });
  ...
  group('fire_trigger', () {
    test('fireReceipt → activationId truncated, never danger', () {
      final r = fireReceipt(
        t,
        '{"fired":true,"triggerId":"trg_1","activationId":"act_1f2e3d4c5b6a7980"}',
      )!;
      expect(r.text, 'act_1f2e3d4c…'); // 12 + …
      expect(
        r.tone,
        isNot(ToolReceiptTone.danger),
      ); // firing is success — never red
      expect(fireReceipt(t, 'not json'), isNull);
    });

    testWidgets(
      'body: navigable trigger pill + activation copy chip + payload note',
      (tester) async {
        await tester.pumpWidget(
          _host(
            ChatToolCard(
        ...
        await tester.tap(
          find.textContaining(t.chat.tool.firedTrigger),
          warnIfMissed: false,
        );
        await tester.pumpAndSettle();
        ...
        expect(
          ...
          findsOneWidget,
        ); // the activation copy chip 活化复制芯片
        expect(
          find.textContaining('act_1f2e3d4c5b6a7980'),
          findsWidgets,
        ); // full id in the copy chip
  ...
  testWidgets(
    'invoke TIMEOUT keeps the danger tone (批7 B-037 突变闸 — 钉 fromRaw 的 timeout 别名)',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
