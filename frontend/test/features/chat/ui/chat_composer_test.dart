...
class _EmptyConnectionLostSpeech extends SpeechInputController {
  @override
  Future<void> start() async {
    state = const SpeechInputState(
...
class _RetryableConnectionLostSpeech extends SpeechInputController {
  ...
  Future<void> retry() async {
    ...
    state = const SpeechInputState(committed: 'hello world');
  }
...
Finder _buttonWithLabel(String label) => find.byWidgetPredicate(
  (w) => w is AnButton && w.label == label,
  description: 'AnButton("$label")',
);
...
void main() {
  ...
  testWidgets('voice disconnect with no text explains the retry recording', (
    tester,
  ) async {
    final repo = FixtureChatRepository(
      conversations: [_conv('cv_1')],
    ...
    await tester.pumpWidget(
      _host(
        ...
        overrides: [
          ...
          speechInputProvider.overrideWith(_EmptyConnectionLostSpeech.new),
        ],
      ),
    );
    await _settle(tester);
    ...
    await tester.pumpWidget(
      _host(
        ...
        overrides: [
          ...
          speechInputProvider.overrideWith(_RetryableConnectionLostSpeech.new),
        ],
      ),
    );
    await _settle(tester);
  ...
  testWidgets('voice disconnect discard deletes the voice draft', (
    tester,
  ) async {
    final repo = FixtureChatRepository(
      conversations: [_conv('cv_1')],
    ...
    tester
        .widget<AnButton>(_buttonWithLabel('Delete voice draft'))
        .onPressed!();
    await _settle(tester);
