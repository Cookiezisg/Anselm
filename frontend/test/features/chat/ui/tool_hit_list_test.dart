...
ToolHitRow _hit(String kind, String id, String name, {String? snippet}) =>
    ToolHitRow(
...
void main() {
  ...
  testWidgets(
    'a row is tappable iff its kind has a panel — fires (kind,id); no-panel kind is inert',
    (tester) async {
          ...
          ToolHitList(
            rows: [
              ...
              _hit(
                ...
                'mem_1',
                'note',
              ), // no panel → inert, no chevron, no callback
            ],
            cap: 20,
  ...
  testWidgets(
    'LOCAL over-cap: escape-hatch footer → taps to the full bounded JSON tree',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ToolHitList(
            ...
            cap: 2, // 3 rows > cap
            rawJson:
                '{"count":3,"functions":[{"id":"fn_1"},{"id":"fn_2"},{"id":"ag_1"}]}',
            onRowTap: (_, _) {},
          ),
        ),
      );
      ...
      await tester.pumpAndSettle();
      expect(
        find.byType(AnJsonTree),
        findsOneWidget,
      ); // swapped to the full result
    },
  );

  testWidgets(
    'SERVER truncated: a display-only note (no escape hatch, not tappable)',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ToolHitList(
      ...
      await tester.pumpAndSettle();
      expect(
        find.text(t.chat.tool.serverTruncatedNote(n: '3', total: '47')),
        findsOneWidget,
      );
      // NOT the local escape hatch (they must never be conflated).
      expect(
        find.text(t.chat.tool.cappedFooter(n: '3', total: '47')),
        findsNothing,
      );
    },
  );
  ...
  testWidgets(
    'animate:false → rows land at full opacity instantly (no cascade)',
    (tester) async {
      await tester.pumpWidget(_host(ToolHitList(rows: _rows, cap: 20)));
      ...
      expect(
        op.every((o) => o == 1.0),
        isTrue,
        reason: 'no cascade when not animating',
      );
    },
  );
}
