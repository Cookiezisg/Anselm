...
void main() {
  ...
  testWidgets(
    'WORKFLOW create: closed ops build the canvas + counts + the discriminant drawer',
    (tester) async {
      final repo = _repo();
      ...
      expect(
        find.textContaining('节点 2'),
        findsOneWidget,
      ); // only CLOSED ops count 只数闭合
      ...
      expect(
        find.text('pull.result'),
        findsOneWidget,
      ); // the reference capsule 引用药囊
    },
  );

  testWidgets(
    'CONTROL: rungs from closed branches — when CEL, emit grid, 透传 ghost, 否则 catch-all',
    (tester) async {
      final repo = _repo();
      ...
      expect(
        find.textContaining('改之前的梯'),
        findsOneWidget,
      ); // 40% old ladder 旧梯垫底
      expect(find.text('big'), findsOneWidget);
      expect(find.text('input.total'), findsWidgets); // reference capsules 引用药囊
      expect(
        find.textContaining('透传'),
        findsNWidgets(2),
      ); // both empty emits ghost 两级空 emit 皆幽灵
      expect(
        find.text('否则'),
        findsOneWidget,
      ); // when:"true" badge, not code 兜底徽非代码
      expect(find.text('true'), findsNothing);
    },
  );

  testWidgets(
    'APPROVAL: {{ }} amber capsules + timeout prose + the unsent seal; settle = the preview',
    (tester) async {
      final repo = _repo();
      await tester.pumpWidget(_host(repo));
      await tester.pump();
    ...
    (tester) async {
      ...
      repo.emitFrame(
        ...
        StreamEnvelope(
          ...
          frame: FrameOpen(
            parentId: 'inner',
            node: StreamNode(type: 'progress', content: {}),
          ),
        ),
      );
      repo.emitFrame(
        ...
        StreamEnvelope(
          ...
          scope: _scope,
          id: 'prog',
          frame: FrameDelta(chunk: 'compiling module_1\n'),
        ),
      );
      await _stageAndSettleFrames(tester);
  ...
  testWidgets(
    'TRIGGER: radar while kind is unknown → the cron face; R-16 settle facts come from GET',
    (tester) async {
      final repo = _repo();
      ...
      repo.emitFrame(
        _conv,
        _delta('tc', '"kind":"cron","config":{"expression":"0 9 * * *"},'),
      );
      await tester.pump(const Duration(milliseconds: 60));
      await tester.pump(const Duration(milliseconds: 60));
      expect(
        find.textContaining('0 9 * * *'),
        findsOneWidget,
      ); // the cron face 钟面
          ...
          frame: const FrameClose(
            ...
            result: StreamNode(
              ...
              content: {
                'name': 'create_trigger',
                'arguments':
                    '{"name":"daily","kind":"cron","config":{"expression":"0 9 * * *"}}',
              },
            ),
          ),
        ),
      ...
      repo.emitFrame(
        ...
        const StreamEnvelope(
          ...
          frame: FrameOpen(
            ...
            node: StreamNode(
              type: 'tool_result',
              content: {'content': '{"id":"trg_1","listening":false}'},
            ),
          ),
        ),
      );
