...
void main() {
  ...
  testWidgets(
    'streaming body = the bare prose tail: AnLiveTail present, NO AnWindow',
    (tester) async {
      ...
      ); // reveal tween (bounded pumps — shimmer never settles)
      expect(find.byType(AnLiveTail), findsOneWidget);
      expect(
        find.byType(AnWindow),
        findsNothing,
      ); // inline prose, not a machine window 内联散文非机器窗
  ...
  testWidgets(
    'settled + expanded body is the plain full prose (no tail machinery)',
    (tester) async {
      await tester.pumpWidget(
        _host(
          const ChatThinking(
