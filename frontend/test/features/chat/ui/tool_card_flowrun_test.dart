...
BlockNode _replay(String result) =>
    BlockNode(id: 'tc_r', kind: BlockKind.toolCall)
      ..status = 'completed'
      ..content = {
        'name': 'replay_flowrun',
        'arguments': '{"flowrunId":"fr_9a8b7c6d5e4f3a2b"}',
      }
      ..children.add(
        BlockNode(id: 'tr_r', kind: BlockKind.toolResult)
          ..status = 'completed'
...
void main() {
  ...
  group('replayReceipt', () {
    test('completed → 完成 · N 节点 (N from nodeSummary, never nodes.length)', () {
      final r = replayReceipt(
        t,
        _run(
          'completed',
          [_node('a', 'action', 'completed')],
          nodeSummary:
              '{"totalNodes":213,"shownNodes":80,"byStatus":{"completed":213},"note":"x"}',
        ),
      )!;
    ...
    test('failed → red 仍失败 + auto-expand signal', () {
      final r = replayReceipt(
        t,
        _run('failed', [_node('a', 'action', 'failed', error: 'boom')]),
      )!;
      expect(r.text, t.run.runStillFailed);
      expect(r.tone, ToolReceiptTone.danger);
      expect(
        replayResultFailed(_run('failed', [_node('a', 'action', 'failed')])),
        isTrue,
      );
    });
    test(
      'running with a parked node → 等待审批 (grey, not danger — park is a node state)',
      () {
        final r = replayReceipt(
          t,
          _run('running', [_node('a', 'approval', 'parked')]),
        )!;
    ...
    test('running with no parked node → no receipt; unparseable → null', () {
      expect(
        replayReceipt(t, _run('running', [_node('a', 'action', 'completed')])),
        isNull,
      );
    ...
    test('cancelled → 已取消, not failed', () {
      final r = replayReceipt(
        t,
        _run('cancelled', [_node('a', 'action', 'completed')]),
      )!;
  ...
  testWidgets(
    'completed run: node ledger rows + workflow pill + flowrunId copy',
    (tester) async {
      ...
      await tester.pump();
      await tester.tap(
        find.textContaining(t.chat.tool.replayedRun),
        warnIfMissed: false,
      );
      ...
      expect(
        find.textContaining(t.run.replayPinNote),
        findsOneWidget,
      ); // honesty note
      expect(
        find.textContaining('wf_1a2b3c4d5e6f7a8b'),
        findsWidgets,
      ); // navigable workflow pill
      expect(
        find.textContaining('fr_9a8b7c6d5e4f3a2b'),
        findsWidgets,
      ); // flowrunId copy chip
    },
  );

  testWidgets('failed run auto-expands and shows the failed node error inline', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(
          ...
          node: _replay(
            _run('failed', [
              ...
              _node(
                ...
                'action',
                'failed',
                error: 'HANDLER_RPC_TIMEOUT: charge() exceeded 30s',
              ),
  ...
  testWidgets(
    ...
    (tester) async {
          ...
          ChatToolCard(
            node: _replay(
              _run(
                ...
                nodeSummary:
                    '{"totalNodes":213,"shownNodes":80,"byStatus":{"completed":209,"failed":3,"parked":1},"note":"x"}',
              ),
            ),
          ),
        ),
      ...
      await tester.pumpAndSettle();
      expect(
        find.textContaining('80/213'),
        findsOneWidget,
      ); // shown/total honest bar
      expect(
        find.textContaining('#42'),
        findsOneWidget,
      ); // loop turn index disambiguator
    },
  );

  group('trigger_workflow', () {
    BlockNode trig(String args, String result) =>
        BlockNode(id: 'tc_t', kind: BlockKind.toolCall)
          ..status = 'completed'
          ..content = {'name': 'trigger_workflow', 'arguments': args}
          ..children.add(
            BlockNode(id: 'tr_t', kind: BlockKind.toolResult)
              ..status = 'completed'
              ..content = {'content': result},
          );

    test('receipt → flowrunId truncated, never danger; unparseable → null', () {
      final r = triggerWorkflowReceipt(
        t,
        '{"flowrunId":"fr_9a8b7c6d5e4f3a2b","workflowId":"wf_1"}',
      )!;
    ...
    testWidgets(
      'body: payload input + navigable workflow pill + flowrunId copy + get_flowrun note',
      (tester) async {
        await tester.pumpWidget(
          _host(
            ChatToolCard(
        ...
        await tester.pump();
        await tester.tap(
          find.textContaining(t.chat.tool.triggeredWf),
          warnIfMissed: false,
        );
        await tester.pumpAndSettle();
        expect(
    ...
    testWidgets('empty payload is stated, never dressed as an empty tree', (
      tester,
    ) async {
      await tester.pumpWidget(
        _host(
  ...
  group('get_flowrun', () {
    ...
    test('receipt: status·nodes; failed→danger; capped→shown/total', () {
      expect(
        getFlowrunReceipt(
          t,
          _run('completed', [_node('a', 'action', 'completed')]),
        )!.tone,
      ...
      expect(
        getFlowrunReceipt(
          t,
          _run('failed', [_node('a', 'action', 'failed')]),
        )!.tone,
        ToolReceiptTone.danger,
      );
      final capped = getFlowrunReceipt(
        t,
        _run(
          'completed',
          [_node('a', 'action', 'completed')],
      ...
      expect(
        capped.text,
        contains('80/213'),
      ); // capped → shown/total, from summary
      expect(getFlowrunFailed(_run('failed', [])), isTrue);
    });

    testWidgets('completed: run header + node ledger + trigger provenance', (
      tester,
    ) async {
      await tester.pumpWidget(
        _host(
      ...
      await tester.pump();
      await tester.tap(
        find.textContaining(t.chat.tool.gotFlowrun),
        warnIfMissed: false,
      );
      await tester.pumpAndSettle();
      expect(find.byType(FlowrunNodeList), findsOneWidget);
