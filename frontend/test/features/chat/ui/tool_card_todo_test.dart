...
BlockNode _todoWrite(List<Map<String, String>> items) =>
...
BlockNode _todoRead(String rendered) =>
...
void main() {
  ...
  test('todoReceipt: N 项 · M 完成; cleared', () {
    ...
    expect(
      todoReceipt(t, argsJson: jsonEncode({'items': []}))!.text,
      t.chat.tool.todoCleared,
    );
  });

  testWidgets('todo_write body: a checklist with a ✓ / ▶ / ☐ row per task', (
    tester,
  ) async {
    ...
    await tester.pump();
    await tester.tap(
      find.textContaining(t.chat.tool.todoWrote),
      warnIfMissed: false,
    );
    ...
    expect(
      find.text('DOING_TASK'),
      findsOneWidget,
    ); // in_progress → activeForm, not content
  ...
  testWidgets('todo_read body: parses the rendered checklist', (tester) async {
    await tester.pumpWidget(
      _host(ChatToolCard(node: _todoRead('- [x] READ_DONE\n- [ ] READ_TODO'))),
    );
    await tester.pump();
    await tester.tap(
      find.textContaining(t.chat.tool.todoRead),
      warnIfMissed: false,
    );
    await tester.pumpAndSettle();
