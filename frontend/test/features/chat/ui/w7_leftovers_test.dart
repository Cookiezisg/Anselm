...
void main() {
  ...
  testWidgets(
    'a delegate\'s streaming tool progress rolls an inline terminal tail',
    (tester) async {
      final repo = _repo();
      await tester.pumpWidget(_host(repo, StagePanel(conversationId: _conv)));
      await tester.pump();
      repo.emitFrame(
        ...
        const StreamEnvelope(
          ...
          id: 'sa',
          frame: FrameOpen(
            node: StreamNode(type: 'tool_call', content: {'name': 'Subagent'}),
          ),
        ),
      );
      repo.emitFrame(
      ...
      expect(
        find.textContaining('1204 shelf.csv', findRichText: true),
        findsWidgets,
        reason: 'the inner terminal tail is live 内层终端活窗在场',
      );
      expect(
        find.textContaining(r'$ wc -l shelf.csv', findRichText: true),
        findsWidgets,
      );
    },
  );
