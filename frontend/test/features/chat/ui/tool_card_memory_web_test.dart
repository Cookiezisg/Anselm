...
void main() {
  ...
  group('parseMemoryTemplate 记忆模板反解', () {
    ...
    test(
      'description absent + template mismatch → null (honest degradation)',
      () {
  ...
  group('memory receipts 三分支正向门控', () {
    test('write: Saved → N 行 off ARGS; Cannot save → danger; drift → none', () {
      final saved = memoryWriteReceipt(
        t,
        _settled(
          args: '{"name":"a","content":"一\\n二\\n三"}',
          result: 'Saved memory "a" (3 lines).',
        ),
      );
      expect(saved!.text, t.chat.tool.lines(n: '3'));

      final rejected = memoryWriteReceipt(
        t,
        _settled(result: 'Cannot save memory: name is invalid.'),
      );
      expect(rejected!.text, t.chat.tool.memNotSaved);

      expect(
        memoryWriteReceipt(t, _settled(result: 'Totally new template')),
        isNull,
    ...
    test('read: template hit → N 行; not found → grey miss', () {
      final hit = memoryReadReceipt(
        t,
        _settled(result: '### a (source: user)\n---\nl1\nl2'),
      );
      expect(hit!.text, t.chat.tool.lines(n: '2'));
      final miss = memoryReadReceipt(
        t,
        _settled(result: 'Memory "a" not found.'),
      );
  ...
  group('web outcome classifiers 结局分类器', () {
    test('WebSearch: five degraded anchors + hits/empty', () {
      expect(
        webSearchOutcome('No search backend configured. …'),
        WebSearchOutcome.noBackend,
      );
      expect(
        webSearchOutcome('The configured default search key is provider "x" …'),
        WebSearchOutcome.misconfig,
      );
      expect(
        webSearchOutcome('Search provider "x" has no base URL configured.'),
        WebSearchOutcome.misconfig,
      );
      expect(
        webSearchOutcome('Search via brave failed: 429'),
        WebSearchOutcome.providerFail,
      );
    ...
    test(
      'WebFetch: failure/empty/raw/js-shell anchors classify red facts off a green status',
      () {
        expect(webFetchOutcome('Invalid URL "x"'), WebFetchOutcome.fail);
        expect(
          webFetchOutcome('Refusing to fetch a private address.'),
          WebFetchOutcome.fail,
        );
        expect(
          webFetchOutcome('Failed to fetch: timeout'),
          WebFetchOutcome.fail,
        );
        expect(
          webFetchOutcome('Fetched https://a.b but body was empty.'),
          WebFetchOutcome.empty,
        );
        expect(
          webFetchOutcome(
            'Summarisation unavailable (boom). Raw content (first 4 KB):\n<html>',
          ),
          WebFetchOutcome.raw,
        );
        expect(
          webFetchOutcome(
            'The page at https://a.b rendered almost no readable text (12 chars). '
            'It is likely a JavaScript application.',
          ),
          WebFetchOutcome.jsShell,
        );
        expect(
          webFetchOutcome('# A fine summary\n正文'),
          WebFetchOutcome.summary,
        );
      },
    );
  });
  ...
  testWidgets('MemoryNoteCard renders name + source badge + typeset body', (
    tester,
  ) async {
    ...
    expect(
      find.textContaining('退避', findRichText: true),
      findsOneWidget,
    ); // markdown 渲染态
  });

  testWidgets('webSearchBody renders clickable hit rows (title/snippet/host)', (
    tester,
  ) async {
      ...
      TranslationProvider(
        child: MaterialApp(
          ...
          home: Scaffold(
            body: SizedBox(
              width: 420,
              child: Builder(builder: (c) => webSearchBody(c, state)),
            ),
          ),
        ),
      ),
  ...
  testWidgets(
    'an unparseable hit URL never crushes the title in a 280px host (批6 复审 — 尾格契约刚性)',
    (tester) async {
      ...
      final state = _settled(
        result:
            '{"source":"brave","results":[{"title":"A readable page title","url":"$rawUrl","snippet":"s"}]}',
      );
      await tester.pumpWidget(
        TranslationProvider(
          child: MaterialApp(
      ...
      expect(
        tester.getSize(find.text('A readable page title')).width,
        greaterThan(0),
      );
      expect(
        find.textContaining(rawUrl),
        findsNothing,
      ); // the raw string never renders 原始串绝不上屏
    },
  );

  testWidgets(
    'searchToolsBody renders the compact loaded_tools hit card without raw JSON',
    (tester) async {
      final state = _settled(
        result:
            '{"loaded_tools":[{"name":"search_function","purpose":"按关键词查找函数"}],'
        ...
        TranslationProvider(
          child: MaterialApp(
            ...
            home: Scaffold(
              body: SizedBox(
                width: 420,
                child: Builder(builder: (c) => searchToolsBody(c, state)),
              ),
            ),
          ),
        ),
      ...
      await tester.pump();
      expect(find.text('search_function'), findsOneWidget);
      expect(find.text('按关键词查找函数'), findsOneWidget);
      expect(
        find.text(
          'Full parameter schemas are active in the next model request.',
        ),
        findsOneWidget,
      );
      expect(find.textContaining('loaded_tools'), findsNothing);
  ...
  group('MemoryNoteCard 壳 (WRK-066 批4 族一)', () {
    ...
    testWidgets('the shell is the ONE window — the grey well is retired', (
      tester,
    ) async {
      await tester.pumpWidget(
        host(
          const MemoryNoteCard(
            note: (
              name: 'note',
              source: 'user',
              description: 'd',
    ...
    testWidgets(
      'a long body collapses with the standard expand affordance; markdown fences never nest a window',
      (tester) async {
        final long =
            "${'line\n' * 30}```dart\nvoid main() {}\n```\n${'tail\n' * 200}";
        await tester.pumpWidget(
