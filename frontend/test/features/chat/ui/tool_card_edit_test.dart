...
void main() {
  ...
  testWidgets(
    'LIVE two-act behind the chevron: collapsed by default, TAP opens − old then + new (WRK-065)',
    (tester) async {
      ...
      await tester.pumpWidget(
        _host(
          ChatToolCard(
            node: _streaming(
              '{"file_path":"/ws/a.py","old_string":"return x","new_string":"return y + z',
            ),
          ),
        ),
      );
      ...
      expect(
        find.textContaining('return x'),
        findsOneWidget,
      ); // the removed row 删行
      expect(
        find.textContaining('return y + z'),
        findsOneWidget,
      ); // the added row (streaming) 加行
  ...
  testWidgets('SETTLED: a unified AnVersionDiff (before=old, after=new)', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(
  ...
  testWidgets('SETTLED replace_all: an «N 处全部替换» note', (tester) async {
    ...
    await tester.pumpAndSettle();
    expect(
      find.textContaining(t.chat.tool.replaceAllNote(n: '4')),
      findsOneWidget,
    );
  });
}
