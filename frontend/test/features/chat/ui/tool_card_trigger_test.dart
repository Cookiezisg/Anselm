...
void main() {
  ...
  test('triggerReceipt: create → 未监听 (none); edit on live → 热更新 (warn)', () {
    final create = triggerReceipt(t, _state('create_trigger', '{}', '{"id":"trg_1","listening":false}'));
    expect(create!.text, t.chat.tool.trgNotListening);
    expect(create.tone, ToolReceiptTone.none);
    final edit = triggerReceipt(t, _state('edit_trigger', '{}', '{"id":"trg_1","listening":true}'));
  ...
  testWidgets('cron face: expression prominent + create-not-listening note', (tester) async {
    await tester.pumpWidget(_host(ChatToolCard(node: _node('create_trigger',
  ...
  testWidgets('webhook face: copyable URL + secret lock chip (value never shown)', (tester) async {
    await tester.pumpWidget(_host(ChatToolCard(node: _node('create_trigger',
  ...
  testWidgets('sensor face: target ref pill + interval + CEL condition/output', (tester) async {
    await tester.pumpWidget(_host(ChatToolCard(node: _node('edit_trigger',
    ...
    expect(find.textContaining('result.total > 10000'), findsOneWidget); // CEL condition
    expect(find.textContaining(t.chat.tool.trgEvery(n: '60')), findsOneWidget);
  });

  testWidgets('unknown kind degrades to a raw JSON window (never throws)', (tester) async {
    await tester.pumpWidget(_host(ChatToolCard(node: _node('create_trigger',
