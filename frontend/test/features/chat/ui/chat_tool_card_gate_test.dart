...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.text('等待确认'),
        findsOneWidget,
      ); // chat.tool.awaitingConfirm (row verb)
      expect(find.byType(ToolInteractionGate), findsOneWidget);
      expect(
        find.text('remove the obsolete triager'),
        findsOneWidget,
      ); // summary in the gate
  ...
  testWidgets(
    'resolving the gate flows through onResolve with the exact action',
    (tester) async {
  ...
  testWidgets('decided approve → a provenance章 sits atop the expanded body', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(
  ...
  BlockNode askNode({
    ...
    final res = declined
        ...
        : (result ?? 'USD');
    n.children.add(
      BlockNode(id: 'tr_ask', kind: BlockKind.toolResult)
        ..status = 'completed'
        ..content = {'content': res},
    );
    return n;
  ...
  testWidgets(
    'ask_user settled verbs: answered / skipped / empty by result prose',
    (tester) async {
      await tester.pumpWidget(
        _host(ChatToolCard(node: askNode(result: 'USD'))),
      );
      await tester.pump();
      expect(find.text('已回答'), findsOneWidget);

      await tester.pumpWidget(
        _host(ChatToolCard(node: askNode(declined: true))),
      );
      await tester.pump();
      expect(
        find.text('已跳过'),
        findsOneWidget,
      ); // decline prose → skipped, NOT the danger 已拒绝执行
  ...
  testWidgets(
    'ask_user receipt is the answer first line (only for a real answer)',
    (tester) async {
      await tester.pumpWidget(
        _host(ChatToolCard(node: askNode(result: 'euros please'))),
      );
      await tester.pump();
      expect(
        find.textContaining('euros please'),
        findsWidgets,
      ); // in the collapsed receipt
    },
  );

  testWidgets(
    'ask_user expanded → frozen Q/A via the gate resolved mode (chosen option章)',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
  ...
  testWidgets(
    'ask_user awaiting → the ask gate (options + free text) via the interaction record',
    (tester) async {
      ...
      final awaitingNode =
          BlockNode(id: 'tc_ask_user', kind: BlockKind.toolCall)
            ..status = 'completed'
            ..content = {
              'name': 'ask_user',
              'arguments':
                  '{"message":"which currency?","options":["CNY","USD"]}',
            };
      await tester.pumpWidget(
        _host(ChatToolCard(node: awaitingNode, interaction: rec)),
      );
      await tester.pump();
  ...
  BlockNode decideNode({
    ...
    n.children.add(
      BlockNode(id: 'tr_d', kind: BlockKind.toolResult)
  ...
  testWidgets('decide verbs: yes → 已批准, no → 已否决; failed → neutral 已裁决', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(
        ChatToolCard(
          node: decideNode(
            decision: 'yes',
            result: '{"flowrun":{"status":"running"},"nodes":[]}',
          ),
        ),
      ),
    );
    ...
    await tester.pumpWidget(
      _host(
        ChatToolCard(
          node: decideNode(
            decision: 'no',
            result: '{"flowrun":{"status":"failed"},"nodes":[]}',
          ),
        ),
      ),
    );
    ...
    await tester.pumpWidget(
      _host(
        ChatToolCard(
          node: decideNode(
            decision: 'yes',
            errorProse: 'approval node is not awaiting a decision',
          ),
        ),
      ),
    );
  ...
  testWidgets('decide approved body: verdict章 + reason + consequence counts', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(
    ...
    expect(
      find.text('completed 2'),
      findsOneWidget,
    ); // consequence count off nodes[]
    expect(find.text('running 1'), findsOneWidget);
  });

  testWidgets(
    'decide NOT_PARKED: friendly note, no raw crash prose (ownsError)',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
      ...
      expect(
        find.textContaining('本次裁决未生效'),
        findsOneWidget,
      ); // the reframed note
      expect(
        find.textContaining('approval node is not awaiting'),
        findsNothing,
      ); // raw prose suppressed
    },
  );
  ...
  BlockNode inboxNode(String result) =>
      BlockNode(id: 'tc_list_approval_inbox', kind: BlockKind.toolCall)
        ...
        ..children.add(
          BlockNode(id: 'tr_i', kind: BlockKind.toolResult)
            ..status = 'completed'
            ..content = {'content': result},
        );

  testWidgets('inbox receipt: N awaiting / none awaiting by count', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(
        ChatToolCard(
          node: inboxNode(
            '{"count":2,"parked":[{"flowrunId":"flr_1","ref":"a","parkedAt":"2026-07-06T00:00:00Z"},{"flowrunId":"flr_2","ref":"b","parkedAt":"2026-07-06T00:00:00Z"}]}',
          ),
        ),
      ),
    );
    ...
    await tester.pumpWidget(
      _host(ChatToolCard(node: inboxNode('{"count":0,"parked":[]}'))),
    );
    await tester.pump();
  ...
  testWidgets(
    'inbox body: a thin table of parked approvals (flattened summary + ref + run)',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
      ...
      expect(
        find.text('# 采购审批'),
        findsOneWidget,
      ); // rendered FIRST LINE only (flattened)
      expect(
        find.textContaining('金额 ¥12,400'),
        findsNothing,
      ); // the 2nd line is NOT rendered in-cell
      expect(
        find.text('flr_abcdefgh…'),
        findsOneWidget,
      ); // run id truncated at the family id tier (12, 批5 A-044) 族档截断
    },
  );

  testWidgets('inbox empty → an honest muted empty state, no table', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(ChatToolCard(node: inboxNode('{"count":0,"parked":[]}'))),
