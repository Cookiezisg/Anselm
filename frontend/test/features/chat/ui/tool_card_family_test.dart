...
void main() {
  ...
  testWidgets(
    'Read: receipt-only card — verb+basename+lines, no chevron, tap inert',
    (tester) async {
      final read = _call(
        'Read',
        extra: {'arguments': '{"file_path":"/ws/rollup.py"}'},
      ...
      await tester.pumpAndSettle();
      expect(
        tester.widget<AnExpandReveal>(find.byType(AnExpandReveal).last).open,
        isFalse,
      );
    },
  );
  ...
  testWidgets(
    'Bash exit≠0: danger receipt on the line AND auto-expanded terminal window',
    (tester) async {
      ...
      expect(
        find.textContaining('\$ npm test'),
        findsOneWidget,
      ); // command echo header 命令回显头
  ...
  testWidgets(
    'Bash running: collapsed by default; TAP opens the live terminal (WRK-065)',
    (tester) async {
      final running = _call(
        'Bash',
        extra: {'arguments': '{"command":"npm test"}'},
      )..children.add(_progress('line 1\nline 2\nline 3\nline 4', live: true));
      await tester.pumpWidget(
        _host(ChatToolCard(node: running, key: const ValueKey('run'))),
      );
      await tester.pumpAndSettle();
      ...
      expect(
        find.textContaining('\$ npm test'),
        findsOneWidget,
      ); // in-flight command echo 在途命令回显
      expect(find.textContaining('line 4'), findsOneWidget);

      final settled =
          _call('Bash', extra: {'arguments': '{"command":"npm test"}'})
            ..children.add(_progress('line 1\nline 2'))
            ..children.add(_result('done\n[exit code: 0]'));
      await tester.pumpWidget(
        _host(ChatToolCard(node: settled, key: const ValueKey('settled'))),
      );
      await tester.pumpAndSettle();
      expect(
        tester.widget<AnExpandReveal>(find.byType(AnExpandReveal).first).open,
        isFalse,
      );
  ...
  testWidgets('Edit renders the diff window; Write renders the code window', (
    tester,
  ) async {
    final edit = _call(
      'Edit',
    ...
    await tester.pumpWidget(
      _host(ChatToolCard(node: edit, key: const ValueKey('edit'))),
    );
    await tester.pumpAndSettle();
    ...
    await tester.pumpWidget(
      _host(ChatToolCard(node: write, key: const ValueKey('write'))),
    );
    await tester.pumpAndSettle();
    expect(
      find.textContaining('2 行'),
      findsOneWidget,
    ); // receipt from args content 回执自 args
  ...
  testWidgets(
    'mid-stream: family verb + target appear while args still streaming',
    (tester) async {
      ...
      final r = BlockTreeReducer()
        ...
        ..apply(
          const StreamEnvelope(
            ...
            id: 'tc_s',
            frame: FrameDelta(
              chunk: '{"command":"npm test","summary":"Run the',
            ),
          ),
        );
      ...
      expect(
        find.text('npm test'),
        findsOneWidget,
      ); // closed field extracted mid-stream 流中提取
    },
  );

  testWidgets(
    'call_handler RUNNING opened: live yields directly visible, NO fake «无返回值» (WRK-065)',
    (tester) async {
      ...
      await tester.pumpAndSettle();
      expect(
        find.textContaining('yield 2'),
        findsOneWidget,
      ); // live tail direct, not behind a drawer 直显
      expect(
        find.text(t.chat.tool.noReturn),
        findsNothing,
      ); // never lie mid-run 在飞绝不渲「无返回值」
    },
  );

  testWidgets(
    'mcp mount RUNNING opened: the progress tail shows, no empty result shell (WRK-065)',
    (tester) async {
      final running = _call(
      ...
      await tester.pumpAndSettle();
      expect(
        find.textContaining('contacting server'),
        findsOneWidget,
      ); // live progress 活进度
  ...
  testWidgets(
    'live mono tail: whitespace-only progress renders NO empty machine window',
    (tester) async {
      await tester.pumpWidget(
        _host(const AnLiveTail('\n', style: AnLiveTailStyle.mono)),
      );
      await tester.pumpAndSettle();
