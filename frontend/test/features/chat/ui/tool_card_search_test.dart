...
BlockNode _node(String name, String result) => BlockNode(id: 'tc_$name', kind: BlockKind.toolCall)
  ..status = 'completed'
...
void main() {
  ...
  group('hasBodyOf — empty is «receipt IS the card»', () {
    test('non-empty search has a body; empty search does not', () {
      ...
      expect(spec.hasBodyOf!(_state('search_function', '{"count":2,"functions":[{"id":"a"},{"id":"b"}]}')), isTrue);
      expect(spec.hasBodyOf!(_state('search_function', '{"count":0,"functions":null}')), isFalse);
    });

    test('search_blocks soft-empty string → no body', () {
      final spec = toolCardSpecFor('search_blocks');
      expect(spec.hasBodyOf!(_state('search_blocks', 'No blocks matched "x".')), isFalse);
    });
  });

  testWidgets('empty search renders NO body (receipt is the whole card)', (tester) async {
    await tester.pumpWidget(_host(ChatToolCard(node: _node('search_function', '{"count":0,"functions":null}'))));
    await tester.pump();
  ...
  testWidgets('non-empty search expands to a ToolHitList of the hits', (tester) async {
    await tester.pumpWidget(_host(ChatToolCard(node: _node('search_function',
  ...
  group('row projections', () {
    test('workflowHitRow: fallback lifecycleState/active → badge; engine path (no ls) → none', () {
      final fb = workflowHitRow(t, {'id': 'wf_1', 'name': 'sync', 'lifecycleState': 'active', 'active': true});
      expect(fb.kind, 'workflow');
      expect(fb.trailing, isNotNull); // active badge
      final engine = workflowHitRow(t, {'id': 'wf_1', 'name': 'sync', 'description': 'snippet'});
    ...
    test('triggerHitRow: fallback kind/refCount/listening badges', () {
      final r = triggerHitRow(t, {'id': 'trg_1', 'name': 'cron', 'kind': 'cron', 'refCount': 2, 'listening': true});
      expect(r.kind, 'trigger');
      expect(r.trailing, isNotNull);
    });

    test('blockHitRow: per-hit kind + entityId is the nav target', () {
      final r = blockHitRow({'ref': 'hd_9.charge', 'kind': 'handler', 'entityId': 'hd_9', 'name': 'charge'});
    ...
    test('attachmentListRow: inert (no panel) + human size', () {
      final r = attachmentListRow({'id': 'att_1', 'filename': 'q3.pdf', 'mime': 'application/pdf', 'sizeBytes': 48210});
