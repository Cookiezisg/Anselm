...
void main() {
  ...
  group('manage_conversation verb dispatch', () {
    ToolCardState st(String action, {String? title}) => _s('manage_conversation',
    ...
    test('settled verb reads output.action (5 pairs)', () {
      expect(manageConversationVerb(t, st('archive'), live: false), t.chat.tool.cvArchived);
      expect(manageConversationVerb(t, st('pin'), live: false), t.chat.tool.cvPinned);
      expect(manageConversationVerb(t, st('rename', title: 'x'), live: false), t.chat.tool.cvRenamed);
    });

    test('soft-fail (no conversation in ctx) → NEUTRAL 已调用, never a lying 已置顶对话', () {
      final soft = _s('manage_conversation',
          'manage_conversation is only available inside a conversation (no conversationId in context).',
          args: '{"action":"pin"}');
      expect(manageConversationVerb(t, soft, live: false), t.chat.tool.called);
    });
  });

  group('receipts', () {
    test('list: N 条 / N+ 条 (nextCursor) / 无对话', () {
      expect(listConversationsReceipt(t, '{"count":3,"conversations":[{}]}')!.text, t.chat.tool.cvCount(n: '3'));
      expect(listConversationsReceipt(t, '{"count":3,"conversations":[{}],"nextCursor":"x"}')!.text, t.chat.tool.cvCountMore(n: '3'));
      expect(listConversationsReceipt(t, '{"count":0,"conversations":[]}')!.text, t.chat.tool.cvEmpty);
    });

    test('search: N 命中 / 无匹配 (uses total, not count)', () {
      expect(searchConversationsReceipt(t, '{"total":9,"hits":[{}]}')!.text, t.chat.tool.cvHits(n: '9'));
      expect(searchConversationsReceipt(t, '{"total":0,"hits":[]}')!.text, t.chat.tool.cvNoMatch);
    });
  });

  testWidgets('list_conversations → a mini-rail of tappable conversation rows', (tester) async {
    await tester.pumpWidget(_host(ChatToolCard(node: _node('list_conversations', '{}',
    ...
    expect(find.text(t.chat.tool.cvArchivedBadge), findsOneWidget); // beta is archived
  });

  testWidgets('search_conversations → hits with snippet + truncation note (total > shown)', (tester) async {
    await tester.pumpWidget(_host(ChatToolCard(node: _node('search_conversations', '{"query":"x"}',
    ...
    expect(find.text(t.chat.tool.cvShownOfTotal(n: '1', total: '9')), findsOneWidget);
  });

  testWidgets('empty search/list → «receipt IS the card» (no body)', (tester) async {
    await tester.pumpWidget(_host(ChatToolCard(node: _node('search_conversations', '{"query":"x"}', '{"total":0,"hits":[]}'))));
    await tester.pump();
    await tester.tap(find.textContaining('已搜索对话'), warnIfMissed: false);
    await tester.pumpAndSettle();
