...
Widget _host(FixtureChatRepository repo) => ProviderScope(
      ...
      child: TranslationProvider(
        child: MaterialApp(
          theme: AnTheme.light(),
          home: Scaffold(
            body: SizedBox(width: 380, height: 720, child: StagePanel(conversationId: _conv)),
          ),
        ),
      ),
    );
...
StreamEnvelope _close(String id, String args, {String? entityName}) => StreamEnvelope(
    seq: 2, scope: _scope, id: id,
    frame: FrameClose(status: 'completed', result: StreamNode(type: 'tool_call', content: {
      'name': '', 'arguments': args, 'entityName': ?entityName,
    })));
...
void main() {
  ...
  testWidgets(
    ...
    await tester.pump(const Duration(milliseconds: 400)); // truth fetch + reveal 真相+揭示
    ...
    const finalArgs =
        '{"functionId":"fn_1","ops":[{"op":"set_meta","description":"retry"},{"op":"set_code","code":"def sync():\\n    for a in range(3):\\n        run(a)\\n    return 3\\n"}]}';
    repo.emitFrame(_conv, _close('tc', finalArgs, entityName: 'sync_inventory'));
  ...
  testWidgets(
    ...
    repo.emitFrame(_conv, _delta('tc', '{"id":"doc_1","content":"# Runbook\\n\\nstep'));
    ...
    expect(find.textContaining('已快进'), findsOneWidget); // prefixKept label 前缀标

    repo.emitFrame(
        _conv,
        _close('tc', '{"id":"doc_1","content":"# Runbook\\n\\nstep ONE (revised)\\nstep two\\n"}',
            entityName: 'runbook'));
    await tester.pump(const Duration(milliseconds: 200));
    await tester.pump(const Duration(milliseconds: 200));

    expect(find.textContaining('全量替换'), findsOneWidget); // the honest badge 诚实徽
  });

  testWidgets('DOCUMENT R-9: an edit whose args never open `content` never fakes a prose curtain',
      (tester) async {
    final repo = _repo();
    repo.documents['doc_1'] = DocumentNode(
      id: 'doc_1',
    ...
    repo.emitFrame(_conv, _close('tc', '{"id":"doc_1","name":"runbook-v2"}', entityName: 'runbook-v2'));
    await tester.pump(const Duration(milliseconds: 300));

    expect(find.text(t.chat.stage.proseUntouched), findsOneWidget); // metadata-only 元数据小卡
    expect(find.textContaining('全量替换'), findsNothing); // no fake replace badge 不伪造替换徽
  });
}
