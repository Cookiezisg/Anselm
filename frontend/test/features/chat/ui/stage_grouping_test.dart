...
ProviderContainer _container(WidgetTester tester) => ProviderScope.containerOf(
  tester.element(find.byType(StagePanel)),
  listen: false,
);
...
void main() {
  ...
  group('sidestageTierKey — three-tier boundaries', () {
    ...
    test('within the 10-min window → just (boundary inclusive)', () {
      ...
      expect(
        sidestageTierKey(now.subtract(const Duration(minutes: 5)), now),
        'just',
      );
      expect(
        sidestageTierKey(now.subtract(const Duration(minutes: 10)), now),
        'just',
      ); // exactly 10 = just
    });
    test('earlier the same calendar day → today', () {
      expect(
        sidestageTierKey(now.subtract(const Duration(minutes: 11)), now),
        'today',
      );
    ...
    test('a past calendar day → earlier', () {
      ...
      expect(
        sidestageTierKey(now.subtract(const Duration(days: 2)), now),
        'earlier',
      );
    });
    test(
      'the 10-min window wins even across midnight → just (the current burst)',
      () {
        final justAfterMidnight = DateTime(2026, 7, 19, 0, 5);
        expect(
          sidestageTierKey(DateTime(2026, 7, 18, 23, 58), justAfterMidnight),
          'just',
        );
      },
    );
  ...
  group('sidestage time-tier grouping (widget)', () {
    ...
    testWidgets(
      'multiple tiers → heads (label + count); an EMPTY tier draws NO head',
      (tester) async {
        final repo = _repo();
    ...
    testWidgets('a SINGLE tier draws NO head — bare rows (anti-目录病)', (
      tester,
    ) async {
      final repo = _repo();
      repo.touchpoints[_conv] = [
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        ); // past AnMotion.mid (240) → settled 收合完成

        expect(find.text('sync_inventory'), findsNothing);
        expect(find.text('reconcile'), findsNothing);
        expect(
          find.text(t.chat.stage.groupJustNow),
          findsOneWidget,
        ); // head stays 头留存
        expect(
          find.text('nightly_rollup'),
          findsOneWidget,
        ); // the 更早 tier untouched 更早档不动
      },
    );
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.tap(find.text(t.chat.stage.groupJustNow)); // fold 刚刚
        await tester
            .pump(); // one frame — reduced sets the reveal to 0 instantly 一帧即收
        expect(
          find.text('sync_inventory'),
          findsNothing,
          reason: 'reduced motion collapses instantly',
        );
      },
    );

    testWidgets(
      'a collapsed tier FORCE-OPENS when a row inside it is (auto-)expanded (深跳/auto-expand lock)',
      (tester) async {
        final repo = _repo();
        repo.touchpoints[_conv] = [
          _tp(
        ...
        container
            .read(stageGroupCollapseProvider(_conv).notifier)
            .toggle('just'); // fold 刚刚
        ...
        container
            .read(stageExpansionProvider(_conv).notifier)
            .open('function:fn_a');
        ...
        expect(
          find.text('sync_inventory'),
          findsOneWidget,
          reason: 'the collapsed tier force-opened',
        );
      },
    );

    testWidgets(
      'the glance strip shows touched · executed · needs-you, signal-only',
      (tester) async {
        final repo = _repo();
        repo.touchpoints[_conv] = [
          _tp(
    ...
    testWidgets(
      'the glance strip drops K when nothing awaits, and vanishes when all-zero',
      (tester) async {
        final repo = _repo();
        repo.touchpoints[_conv] = [
          _tp(
        ...
        expect(
          find.text(t.chat.stage.island),
          findsOneWidget,
        ); // the head title still renders 头标题仍在
      },
    );
  });
}
