...
BlockNode _blk(String type, String content, {String? tool}) {
  final n = BlockNode(
    id: '${type}_${content.hashCode}',
    kind: blockKindFromWire(type),
  )..status = 'completed';
  n.content = type == 'tool_call'
      ? {'name': tool, 'arguments': content}
      : {'content': content};
  return n;
}
...
void main() {
  ...
  testWidgets(
    'Subagent settled: task + nested trajectory (NestedRunPane) + answer prose',
    (tester) async {
        ...
        _host(
          ChatToolCard(
            node: _subagent(
              ...
              nested: [
                _blk('reasoning', 'LOCATING_FILE'),
                _blk('tool_call', '{}', tool: 'Grep'),
                _blk('text', 'FOUND_IT'),
              ],
      ...
      await tester.pump();
      await tester.tap(
        find.textContaining(t.chat.tool.spawnedSubagent),
        warnIfMissed: false,
      );
      await tester.pumpAndSettle();
      expect(find.byType(NestedRunPane), findsOneWidget);
      expect(
        find.textContaining('FIND_THE_REGISTRY'),
        findsOneWidget,
      ); // the task prompt
      expect(
        find.textContaining('LOCATING_FILE', findRichText: true),
        findsOneWidget,
      ); // nested reasoning
      expect(find.textContaining('Grep'), findsWidgets); // nested tool_call row
      expect(
        find.textContaining('The registry is in catalog.dart'),
        findsWidgets,
      ); // answer prose
    },
  );
  ...
  testWidgets(
    'Subagent with no nested subtree (reload): falls back to the get_subagent_trace note',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
            node: _subagent(
              '{"subagent_type":"Plan","prompt":"x"}',
              result: 'answer',
            ),
          ),
        ),
  ...
  group('get_subagent_trace', () {
    ...
    testWidgets(
      'list form: a run ledger with status dots + finalText subtext + block count',
      (tester) async {
        await tester.pumpWidget(
          _host(
            ChatToolCard(
        ...
        await tester.pump();
        await tester.tap(
          find.textContaining(t.chat.tool.gotSubTrace),
          warnIfMissed: false,
        );
        await tester.pumpAndSettle();
        expect(find.text('subagt_01'), findsOneWidget);
        expect(
          find.textContaining('FOUND_THE_REGISTRY'),
          findsOneWidget,
        ); // finalText subtext
      },
    );

    testWidgets('detail form: blocks hydrate into a TranscriptPeek', (
      tester,
    ) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
            node: _trace('{"subagentRunId":"subagt_01"}', {
              ...
              'blocks': [
                {
                  'type': 'reasoning',
                  'content': 'HYDRATED_THOUGHT',
                  'status': 'completed',
                },
                {
                  'type': 'text',
                  'content': 'HYDRATED_REPLY',
                  'status': 'completed',
                },
              ],
            }),
      ...
      expect(
        find.textContaining('HYDRATED_THOUGHT', findRichText: true),
        findsOneWidget,
      );
      expect(
        find.textContaining('HYDRATED_REPLY', findRichText: true),
        findsOneWidget,
      );
    });
  });
