...
void main() {
  test(
    'function → a live:false scene whose session yields the code (create semantics, no edit target)',
    () {
      final scene = sceneFromTruth(
        kind: 'function',
        truth: _fn(code: 'def main():\n  return 1'),
        id: 'fn_1',
        conversationId: 'cv',
        rowId: 'r',
      );
      ...
      expect(
        scene.editTargetId,
        isNull,
      ); // create_ semantics: no GET / no diff / no old stratum
  ...
  test(
    'function with no active version → null (caller degrades to the summary)',
    () {
      expect(
        sceneFromTruth(
          kind: 'function',
          truth: _fn(noVersion: true),
          id: 'fn_1',
          conversationId: 'cv',
          rowId: 'r',
        ),
  ...
  test(
    'hasTruthStage gates the truth-stage kinds; the summary/pedestal/unreachable kinds stay out',
    () {
      for (final k in [
        'function',
        'workflow',
  ...
  test(
    'workflow → the final graph replayed as add_node / add_edge ops (the settled canvas)',
    () {
      ...
      final addNodes = ops
          .whereType<Map>()
          .where((o) => o['op'] == 'add_node')
          .toList();
      expect(addNodes.length, 2);
      expect(
        (addNodes.first['node'] as Map)['kind'],
        'action',
      ); // NodeKind → its .name string
    },
  );
  ...
  test(
    'agent → prompt + belt tools + knowledge in the session (modelOverride null projects to null)',
    () {
      final ag = AgentEntity(
        ...
        activeVersion: AgentVersion(
          ...
          tools: const [
            ToolRef(ref: 'read', name: 'read'),
            ToolRef(ref: 'grep', name: 'grep'),
          ],
          createdAt: _now,
          updatedAt: _now,
        ),
      ...
      expect(scene, isNotNull);
      expect(
        scene!.session.liveStringNamed('prompt'),
  ...
  test(
    'document → an edit_document scene (editTargetId resolves) whose session yields the content',
    () {
      ...
      expect(
        scene!.editTargetId,
        'doc_1',
      ); // edit_document → 'id' key → the byte badge / path light up
      expect(scene.session.closedStringAt(['content']), '# 模板\n第一行');
    },
  );

  test(
    'handler → ops only for present bodies (empty init/shutdown fabricate no rail segment)',
    () {
      final hd = HandlerEntity(
        ...
        activeVersion: HandlerVersion(
          ...
          version: 1,
          initBody:
              '', // no init → no set_init op (else a phantom lit rail + empty editor)
          shutdownBody: '', // no shutdown
          methods: const [
            MethodSpec(name: 'run', streaming: false, body: 'return ok'),
          ],
          createdAt: _now,
          updatedAt: _now,
        ),
      ...
      expect(scene, isNotNull);
      final ops = scene!.session
          .arrayItemsAt(['ops'])
          .whereType<Map>()
          .toList();
      expect(ops.map((o) => o['op']), [
        'add_method',
      ]); // no set_init / set_shutdown fabricated 无捏造轨段
    },
  );

  test(
    'handler with no init/shutdown/methods/schema → null (degrades to the summary)',
    () {
      final hd = HandlerEntity(
        id: 'hd_1',
        name: 'x',
  ...
  test(
    'content-less versions degrade to the summary (empty graph / empty doc → null, not a blank stage)',
    () {
      final wf = WorkflowEntity(
        id: 'wf_1',
        name: 'x',
  ...
  test(
    'workflow add_node carries the node input CEL map (the discriminant drawer reads it)',
    () {
      final wf = WorkflowEntity(
        id: 'wf_1',
        name: 'x',
      ...
      final node =
          (scene!.session.arrayItemsAt(['ops']).first as Map)['node'] as Map;
      expect((node['input'] as Map)['rows'], 'pull.result');
    },
  );

  test(
    'skill → a create_skill scene carrying the frontmatter tools + full body',
    () {
      final sk = Skill(
        name: 'commit-helper',
        context: 'inline',
      ...
      expect(
        scene.session.liveStringNamed('body'),
        '# Commit Helper\nWrite good messages.',
      );
  ...
  test(
    'content-less skill degrades to the summary despite the backend-defaulted context',
    () {
  ...
  test(
    'mcp → a create_mcp scene carrying the tool shelf names; no tools → null',
    () {
      final srv = McpServerStatus(
        id: 'mcp_1',
        name: 'github',
        status: 'ready',
        tools: const [
      ...
      expect(scene, isNotNull);
      expect(
        scene!.session.arrayItemsAt(['tools']),
        containsAll(<String>['create_issue', 'list_repos']),
      );
  ...
  test('trigger → an edit_trigger scene carrying kind + config', () {
    ...
    expect(
      scene.session.closedStringAt(['kind']),
      'webhook',
    ); // TriggerSource → .name
  });
  ...
  group('C-026 memoization by truth instance', () {
    test(
      'the SAME truth + rowId returns the SAME scene instance (identity)',
      () {
        final truth = _fn(code: 'x=1');
        final a = sceneFromTruth(
          kind: 'function',
          truth: truth,
          id: 'fn_1',
          conversationId: 'cv',
