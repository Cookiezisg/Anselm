...
void main() {
  ...
  testWidgets('phases speak the registry verbs', (tester) async {
    ...
    final denied = _call('delete_agent')
      ..children.add(
        _result('The user denied running this tool. Do not retry it.'),
      );
    ...
    await tester.pumpWidget(
      _host(ChatToolCard(node: cancelled), reduced: true),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets(
    'failure auto-expands once and shows the error body; user can re-collapse',
    (tester) async {
      final failed = _call(
        'edit_gizmo',
        extra: {'arguments': '{"gizmoId":"gz_1"}'},
      ...
      await tester.pumpAndSettle();
      expect(
        find.textContaining('GRAPH_CYCLE'),
        findsOneWidget,
      ); // auto-expanded 自动展开
      ...
      final reveal = tester.widget<AnExpandReveal>(
        find.byType(AnExpandReveal).first,
      );
  ...
  testWidgets(
    'big JSON result gets the virtualized tree inside a bounded viewport',
    (tester) async {
  ...
  testWidgets(
    'elapsed counter: hidden before 3s, ticking after; absent under reduced motion',
    (tester) async {
      ...
      await tester.pumpWidget(
        _host(ChatToolCard(node: running2), reduced: true),
      );
      await tester.pump(const Duration(seconds: 5));
      expect(
        find.textContaining('秒'),
        findsNothing,
      ); // reduced: no ticker, battery-safe 降级无表
      await tester.pumpAndSettle(); // must settle instantly 必须立即安定
    },
  );

  testWidgets(
    'oversized prose result is excerpted with the honest truncation note',
    (tester) async {
  ...
  testWidgets(
    'an absurd mcp__ name + unparseable args routes to the MCP skin and still renders',
    (tester) async {
      // B4 F01.5: an mcp__ name now routes to the MCP skin (not the generic fallback). mcp__ 路由 MCP 皮。
      final weird = _call(
        'mcp__x__y____z!!',
        extra: {'arguments': '{"broken": tru'},
      )..children.add(_result('not json at all'));
      await tester.pumpWidget(_host(ChatToolCard(node: weird), reduced: true));
      await tester.pumpAndSettle();
      expect(
        find.textContaining('MCP'),
        findsOneWidget,
      ); // the MCP verb / chip, not a crash
      await tester.tap(find.byType(AnInteractive).first);
      await tester.pumpAndSettle();
      expect(
        find.textContaining('not json at all'),
        findsOneWidget,
      ); // raw mono body 原始等宽体
    },
  );
  ...
  testWidgets(
    'a genuinely un-cataloged name (no mount pattern) falls to the generic body',
    (tester) async {
      final weird = _call(
        'frobnicate!!',
        extra: {'arguments': '{"broken": tru'},
      ...
      await tester.pumpAndSettle();
      expect(
        find.text('frobnicate!!'),
        findsOneWidget,
      ); // generic target = the tool name
      await tester.tap(find.byType(AnInteractive).first);
      await tester.pumpAndSettle();
