...
void main() {
  ...
  group('danger gate', () {
    ...
    testWidgets(
      'fail-safe order: the negative (Deny) sits LEFT of the positive (Allow)',
      (tester) async {
        await pump(tester, danger());
        final deny = tester.getCenter(find.widgetWithText(AnButton, 'Deny')).dx;
        final allow = tester
            .getCenter(find.widgetWithText(AnButton, 'Allow'))
            .dx;
  ...
  group('ask gate', () {
    testWidgets(
      'options render as numbered buttons; selecting one accepts with its label',
      (tester) async {
    ...
    testWidgets("Don't answer declines", (tester) async {
      ...
      await pump(
        tester,
        ask(options: ['CNY'], onResolve: (a, {answer}) => fired.add(a)),
      );
      await tester.tap(find.widgetWithText(AnButton, "Don't answer"));
      expect(fired.single, InteractionAction.decline);
    });

    testWidgets('free text: typing + Send accepts with the trimmed answer', (
      tester,
    ) async {
      final fired = <(InteractionAction, String?)>[];
      await pump(
        tester,
        ask(freeText: true, onResolve: (a, {answer}) => fired.add((a, answer))),
      );
    ...
    testWidgets('frozen accept (option): the chosen option pins, others fade', (
      tester,
    ) async {
      await pump(
        tester,
    ...
    testWidgets(
      'frozen accept (free text): the answer renders as a quotation',
      (tester) async {
        await pump(
          tester,
          ask(
    ...
    testWidgets(
      'frozen decline shows EXACTLY ONE Skipped章 (no body+footer double)',
      (tester) async {
        await pump(
          tester,
          ask(options: ['CNY'], decided: InteractionAction.decline),
        );
    ...
    testWidgets(
      'number key 1–9 quick-selects an option while the gate holds focus',
      (tester) async {
        final fired = <(InteractionAction, String?)>[];
        await pump(
          tester,
