...
Widget _host(FixtureChatRepository repo, {SettingsPrefs? prefs}) =>
    ProviderScope(
      overrides: [
...
void main() {
  ...
  testWidgets(
    'R-10 retires end-to-end: the held 202 stage settles when ITS flowrun terminal '
    'arrives (wrong flowrunId ignored)',
    (tester) async {
      final repo = _repo();
      ...
      // The model first closes its arguments; the real execution receipt then opens and closes.
      // 模型先收参数流；真实执行回执随后 open→close。
      repo.emitFrame(
        _conv,
        const StreamEnvelope(
          seq: 2,
      ...
      repo.emitFrame(
        ...
        const StreamEnvelope(
          ...
          frame: FrameClose(
            ...
            result: StreamNode(
              type: 'tool_result',
              content: {'content': '{"flowrunId":"fr_1","status":"running"}'},
            ),
          ),
        ),
      );
      ...
      expect(
        container.read(stageDirectorProvider(_conv)).stageOpen,
        isTrue,
      ...
      repo.emitWorkflowFrame(
        ...
        const StreamEnvelope(
          seq: 10,
          scope: StreamScope(kind: 'workflow', id: 'wf_9'),
          id: 's1',
          frame: FrameSignal(
            node: StreamNode(
              type: 'run_terminal',
              content: {'flowrunId': 'fr_other', 'status': 'completed'},
            ),
          ),
        ),
      );
      ...
      expect(
        find.text('→ pass'),
        findsOneWidget,
      ); // the taken branch, live 选中分支实时可见
      ...
      repo.emitWorkflowFrame(
        ...
        const StreamEnvelope(
          ...
          frame: FrameSignal(
            node: StreamNode(
              type: 'run_terminal',
              content: {'flowrunId': 'fr_1', 'status': 'completed'},
            ),
          ),
        ),
      );
      await tester.pump(const Duration(milliseconds: 100));
      expect(
        find.text(t.chat.stage.run.done),
        findsOneWidget,
      ); // the honest closing line 收卷行
  ...
  testWidgets('the follow three-notch persists (restore + write-through)', (
    tester,
  ) async {
    ...
    container
        ...
        .set(FollowMode.firstPerConversation);
    await tester.pump(const Duration(milliseconds: 50));
    expect(prefs.getString(SettingsKeys.chatAutoStage), 'firstPerConversation');
  });

  testWidgets(
    'parallel calls to one function collapse into one target row with a live count',
    (tester) async {
      final repo = _repo();
      await tester.pumpWidget(_host(repo));
      await tester.pump();

      for (final id in ['tc1', 'tc2']) {
        repo.emitFrame(
          _conv,
          StreamEnvelope(
            seq: 1,
        ...
        repo.emitFrame(
          ...
          StreamEnvelope(
            ...
            scope: _scope,
            id: id,
            frame: const FrameDelta(chunk: '{"functionId":"fn_9"}'),
          ),
        );
      }
      await tester.pump(const Duration(milliseconds: 600));
        ...
        repo.emitFrame(
          ...
          StreamEnvelope(
            ...
            frame: const FrameClose(
              ...
              result: StreamNode(
                type: 'tool_call',
                content: {'arguments': '{"functionId":"fn_9"}'},
              ),
            ),
          ),
        );
      ...
      expect(
        find.text(t.chat.stage.parallelRunning(n: 2)),
        findsOneWidget,
        reason:
            'parallel runs of one target must not become duplicate island cards',
      );
    },
  );

  testWidgets(
    'a clean settle keeps the touchpoint row in place (no curtain removal, WRK-064)',
    (tester) async {
      final repo = _repo();
      ...
      ); // the director dismisses the subject 导演器谢幕
      await tester.pump(const Duration(milliseconds: 50));
