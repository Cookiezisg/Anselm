...
Widget _host(Widget child) => TranslationProvider(
  child: MaterialApp(
    theme: AnTheme.light(),
    home: Scaffold(
      body: SingleChildScrollView(child: SizedBox(width: 680, child: child)),
    ),
  ),
);
...
void main() {
  ...
  group('_entityGet verb + chip settle', () {
    test('verb: viewing → viewed with the kind noun', () {
      ...
      expect(
        ...
        t.chat.tool.viewingKind(kind: t.chat.tool.kind.function),
      );
      expect(
        spec.verb(t, live: false),
        t.chat.tool.viewedKind(kind: t.chat.tool.kind.function),
      );
    });
    ...
    test('chip settles from the args id to the output name', () {
      ...
      expect(
        spec.target!(
          _state('get_function', '', args: '{"functionId":"fn_1a2b"}'),
        ),
        'fn_1a2b',
      );
    ...
    test(
      'receipt: the version, never danger (a bad env is body info, not a row hijack)',
      () {
        final spec = toolCardSpecFor('get_function');
  ...
  testWidgets('get_function → the four-part exhibit', (tester) async {
    await tester.pumpWidget(
      _host(ChatToolCard(node: _node('get_function', _fnJson))),
    );
    await tester.pump();
  ...
  testWidgets('get_agent → deep-linkable capability pills', (tester) async {
    ...
    expect(
      find.widgetWithText(AnRefPill, 'doc_9'),
      findsOneWidget,
    ); // knowledge pill
  });

  testWidgets('get_skill → allowedTools WARN chips + pre-auth note', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(
  ...
  group('read_document / read_attachment templates', () {
    testWidgets(
      'read_document: parses the template → header + path + rendered prose',
      (tester) async {
        await tester.pumpWidget(
          _host(
            ChatToolCard(
    ...
    testWidgets(
      'read_document: not-found soft-fail → an amber note (no exhibit)',
      (tester) async {
        await tester.pumpWidget(
          _host(
            ChatToolCard(
              node: _node(
                'read_document',
                'Document "x" not found. Call list_documents.',
                args: '{"id":"x"}',
              ),
            ),
          ),
    ...
    testWidgets(
      'read_attachment: media descriptor → info note; not-found → warn',
      (tester) async {
        await tester.pumpWidget(
          _host(
            ChatToolCard(
