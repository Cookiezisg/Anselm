...
void main() {
  Future<TextSpan> build(WidgetTester tester, MentionTextEditingController ctl,
      {bool withComposing = true}) async {
    late TextSpan span;
    await tester.pumpWidget(MaterialApp(
      theme: AnTheme.light(),
      home: Builder(builder: (context) {
        span = ctl.buildTextSpan(context: context, style: const TextStyle(), withComposing: withComposing);
  ...
  List<TextSpan> flat(TextSpan root) => [
        root,
        ...?root.children?.expand((c) => c is TextSpan ? flat(c) : const <TextSpan>[]),
      ];

  test('token boundaries: only whitespace-delimited @name tints; substrings stay plain', () {
  ...
  testWidgets('pills tint; a coincidental substring does not', (tester) async {
    ...
    final tinted = spans.where((s) => s.text == '@bot' && s.style?.fontWeight == FontWeight.w400);
    expect(tinted, hasLength(1)); // only the leading token 只有开头的 token
  });

  testWidgets('an open composition keeps surrounding pills tinted and underlines only itself',
      (tester) async {
    final ctl = MentionTextEditingController(text: '@bot 你好');
    ctl.pillNames.add('bot');
    addTearDown(ctl.dispose);
    // composing over the trailing CJK 合成区间在尾部中文上
    ctl.value = ctl.value.copyWith(composing: const TextRange(start: 5, end: 7));
    final spans = flat(await build(tester, ctl));
    final pill = spans.where((s) => s.text == '@bot' && s.style?.fontWeight == FontWeight.w400);
    expect(pill, hasLength(1)); // tint SURVIVES the IME (the bug) 合成期不掉色
    final underlined =
        spans.where((s) => s.text == '你好' && s.style?.decoration == TextDecoration.underline);
