...
void main() {
  ...
  group('receipt parsers', () {
    test('revertReceipt: ↩ vN', () {
      expect(
        revertReceipt('{"id":"fn_1","version":3}', rewind: (v) => 'r$v')!.text,
        'r3',
      );
    ...
    test('deleteReceipt: dependents → danger; none → neutral', () {
      final d = deleteReceipt(
        '{"id":"fn_1","deleted":true,"dependentCount":3,"dependents":[{"kind":"agent","id":"ag_1"}]}',
        deleted: 'del',
        affected: (n) => 'aff$n',
      );
      expect(d!.text, 'aff3');
      expect(d.tone, ToolReceiptTone.danger);
      final none = deleteReceipt(
        '{"id":"fn_1","deleted":true}',
        deleted: 'del',
        affected: (n) => 'aff$n',
      );
    ...
    test('killReceipt: N>0 danger, N==0 neutral', () {
      expect(
        killReceipt('{"killed":3}', killedN: (n) => 'k$n', none: 'z')!.tone,
        ToolReceiptTone.danger,
      );
      expect(
        killReceipt('{"killed":0}', killedN: (n) => 'k$n', none: 'z')!.text,
        'z',
      );
    });

    test(
      'restartReceipt: error key → danger + surfaces the actual error; runtimeState words',
      () {
        // The real error must surface (not a generic label) — the «green but broken» reason was invisible.
        final e = restartReceipt(
          '{"error":"boom"}',
          label: (s) => s,
          errored: (err) => 'ERR: $err',
        )!;
    ...
    test('movedReceipt / deletedDocReceipt templates', () {
      ...
      expect(
        deletedDocReceipt(
          'Deleted document doc_1 along with 4 descendant(s).',
          deleted: 'd',
    ...
    () {
      ...
      expect(
        spec.resultFailed!(
          _s(
            'restart_handler',
            '{"id":"hd_1","runtimeState":"crashed","error":"boom"}',
          ),
        ),
        isTrue,
      );
      expect(
        spec.resultFailed!(
          _s('restart_handler', '{"id":"hd_1","runtimeState":"running"}'),
        ),
        isFalse,
      );
    },
  ...
  test(
    'update_meta dynamic verb: rename when ONLY name; update-info otherwise',
    () {
      final spec = toolCardSpecFor('update_function_meta');
      final rename = _s(
        'update_function_meta',
        '{"id":"fn_1","name":"x"}',
        args: '{"functionId":"fn_1","name":"x"}',
      );
  ...
  testWidgets(
    'delete audit: N refs affected (danger) + jump-to-fix pills, auto-expanded',
    (tester) async {
      await tester.pumpWidget(
        _host(
          ChatToolCard(
      ...
      expect(
        find.byType(ToolDependentsBlock),
        findsOneWidget,
      ); // auto-expanded (danger)
