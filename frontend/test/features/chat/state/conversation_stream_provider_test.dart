...
(ProviderContainer, FixtureChatRepository) _setup({
  ...
  final c = ProviderContainer(
    overrides: [
      ...
      selectedConversationProvider.overrideWith(
        () => _FakeSelected(selected ? const ConversationRef('cv_1') : null),
      ),
    ],
  );
...
void main() {
  ...
  test(
    'hydrates: phase ready, turns chronological, opening marks seen (selected)',
    () async {
      final (c, repo) = _setup();
      final sub = c.listen(conversationStreamProvider('cv_1'), (_, _) {});
      await pumpEventQueue();
      expect(
        c.read(conversationStreamProvider('cv_1')).phase,
  ...
  test(
    'frames landing DURING hydration buffer and drain (nothing lost in the fetch window)',
    () async {
      ...
      repo.emitFrame(
        'cv_1',
        _open('msg_x', 'message', content: {'role': 'assistant'}),
      );
      await pumpEventQueue();
      final ctl = c.read(conversationStreamProvider('cv_1').notifier);
  ...
  test(
    'send: optimistic bubble → POST recorded → durable echo reconciles it away',
    () async {
      final (c, repo) = _setup(messages: {'cv_1': []});
      c.listen(conversationStreamProvider('cv_1'), (_, _) {});
      await pumpEventQueue();
      ...
      repo.emitFrame(
        'cv_1',
        _open('msg_echo', 'message', content: {'role': 'user'}),
      );
      await pumpEventQueue();
  ...
  test(
    'cancelTurn passes through; assistant terminal while selected marks seen again',
    () async {
      final (c, repo) = _setup(messages: {'cv_1': []});
      c.listen(conversationStreamProvider('cv_1'), (_, _) {});
      await pumpEventQueue();
  ...
  test(
    '410 resync refetches the head: a turn completed during the gap lands settled',
    () async {
      final (c, repo) = _setup(messages: {'cv_1': []});
      c.listen(conversationStreamProvider('cv_1'), (_, _) {});
      await pumpEventQueue();
