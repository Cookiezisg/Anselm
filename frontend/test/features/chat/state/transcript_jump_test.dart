...
ChatMessage _turn(int i) => ChatMessage(
  ...
  status: 'completed',
  blocks: [
    ChatBlock(id: 'b_$i', type: 'text', content: '第 $i 回', status: 'completed'),
  ],
...
void main() {
  ...
  group('ConversationTranscript window primitives 模型窗口原语', () {
    test(
      'setWindow re-anchors on the target; setHistory resets windowMode',
      () {
        ...
        expect(
          t.settled[t.olderCount].id,
          'msg_5',
        ); // the target IS the center row 目标即中心行
    ...
    test('appendNewer extends the window downward', () {
      ...
      expect(t.settled.map((n) => n.id).toList(), [
        'msg_3',
        'msg_4',
        'msg_5',
        'msg_6',
    ...
    test(
      'messageIdOf walks a nested live block to its turn (the R-14 anchor)',
      () {
        final t = ConversationTranscript('cv_1');
        ...
        t.applyFrame(
          const StreamEnvelope(
            ...
            frame: FrameOpen(
              parentId: 'tc1',
              node: StreamNode(type: 'reasoning', content: {}),
            ),
          ),
        );
  ...
  group('controller jump pipeline 控制器跳转管道', () {
    ...
    test(
      'deep jump opens the window; both directions page; back-to-live rejoins the head',
      () async {
        final (c, _) = _setup();
        c.listen(conversationStreamProvider('cv_1'), (_, _) {});
        await pumpEventQueue();
    ...
    test(
      'a deep jump to an unknown target fails honestly (identity anchoring)',
      () async {
        final (c, _) = _setup();
        c.listen(conversationStreamProvider('cv_1'), (_, _) {});
        await pumpEventQueue();
    ...
    test(
      'M4: a resync re-hydrate consumes the pending bubble whose send already landed',
      () async {
        ...
        expect(
          ctl.transcript.value.pending,
          isEmpty,
          reason:
              'the landed send\'s bubble is consumed by the re-hydrate 落盘泡被重拉消费',
        );
        expect(
          ctl.transcript.value.turns
              .where(
                (n) =>
                    ConversationTranscript.turnRole(n) == 'user' &&
                    ConversationTranscript.turnText(n).contains('缺口里的发送'),
              )
              .length,
          1,
        ...
        await pumpEventQueue();
        expect(
          ctl.transcript.value.pending.length,
          1,
    ...
    test(
      'a send exits the window first (a send speaks to the present)',
      () async {
        final (c, _) = _setup();
        c.listen(conversationStreamProvider('cv_1'), (_, _) {});
        await pumpEventQueue();
