...
void main() {
  ...
  test('removing a READY chip deletes server-side; clear is local-only', () async {
    final (c, repo) = setup();
    final n = c.read(pendingAttachmentsProvider('k').notifier);
    ...
    expect(repo.deletedAttachments, hasLength(1)); // clear never deletes server-side 不删服务端
  });
