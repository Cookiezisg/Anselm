...
void main() {
  ...
  test('hydrates the first page; an empty ledger is EMPTY, not loading', () async {
  ...
  test('loadMore pages forward and dedupes by row id (newer lastAt wins)', () async {
    final (c, repo) = harness();
    repo.touchpoints[_conv] = [
      for (var i = 0; i < 120; i++) _tp('tp_$i', at: _t0.add(Duration(minutes: i))),
    ];
    c.listen(touchpointLedgerProvider(_conv), (_, _) {});
    await pumpEventQueue();
    expect(c.read(touchpointLedgerProvider(_conv)).rows.length, 50); // default page 默认页宽
  ...
  test('§5-9: deep-paged window + signals for a LOADED and an UNLOADED row both upsert cleanly',
      () async {
    final (c, repo) = harness();
    repo.touchpoints[_conv] = [
      for (var i = 0; i < 120; i++) _tp('tp_$i', at: _t0.add(Duration(minutes: i))),
    ...
    c.listen(touchpointLedgerProvider(_conv), (_, _) {});
    await pumpEventQueue();
    await c.read(touchpointLedgerProvider(_conv).notifier).loadMore(); // 100 rows loaded 已载百行
    ...
    await pumpEventQueue();
    final s = c.read(touchpointLedgerProvider(_conv));
    expect(s.rows.length, 101); // 100 + the unloaded arrival (loaded one deduped) 已载去重、未载新增
    expect(s.rows['tp_119']!.count, 2);
    expect(s.entities.first.primary.id, 'tp_3'); // freshest tops the cast 最新登顶
  ...
  test('only DURABLE signals patch; an ephemeral echo is ignored', () async {
    ...
    repo.emitFrame(
      ...
      StreamEnvelope(
        ...
        id: 'tp_x',
        frame: FrameSignal(node: StreamNode(type: 'touchpoint', content: _tp('tp_x', at: _t0).toJson())),
      ),
    );
    await pumpEventQueue();
  ...
  test('R-2: verbs of one item aggregate to ONE entity row, freshest verb fronting', () {
    final rows = [
      _tp('tp_1', itemId: 'fn_x', name: 'rollup', verb: TouchpointVerb.created, at: _t0),
      _tp('tp_2', itemId: 'fn_x', verb: TouchpointVerb.edited, at: _t0.add(const Duration(minutes: 5))),
      _tp('tp_3', itemId: 'fn_x', verb: TouchpointVerb.executed, at: _t0.add(const Duration(minutes: 2))),
      _tp('tp_4', itemId: 'fn_y', name: 'other', verb: TouchpointVerb.viewed, at: _t0),
    ];
    ...
    expect(x.secondary.map((r) => r.verb), [TouchpointVerb.executed, TouchpointVerb.created]);
  ...
  test('R-2: mcp rows normalize to the NAME key (three id paths converge)', () {
    final rows = [
      _tp('tp_1', kind: 'mcp', itemId: 'mcp_abc123', name: 'github', verb: TouchpointVerb.created, at: _t0),
      _tp('tp_2', kind: 'mcp', itemId: 'github', name: 'github', verb: TouchpointVerb.executed,
  ...
  test('R-2: any deleted row tombstones the whole entity', () {
    final rows = [
      _tp('tp_1', itemId: 'fn_x', name: 'rollup', verb: TouchpointVerb.created, at: _t0),
      _tp('tp_2', itemId: 'fn_x', verb: TouchpointVerb.deleted, at: _t0.add(const Duration(minutes: 9))),
    ];
  ...
  test('410 resync refetches page one and MERGES (gap rows have fresh lastAt → they land in page 1)',
      () async {
    final (c, repo) = harness();
    repo.touchpoints[_conv] = [
      for (var i = 0; i < 60; i++) _tp('tp_$i', at: _t0.add(Duration(minutes: i))),
    ];
    c.listen(touchpointLedgerProvider(_conv), (_, _) {});
    await pumpEventQueue();
    ...
    repo.touchpoints[_conv]!.add(_tp('tp_gap', at: _t0.add(const Duration(hours: 2))));
    repo.emitResync();
    await pumpEventQueue();
    final s = c.read(touchpointLedgerProvider(_conv));
    expect(s.rows.containsKey('tp_gap'), isTrue); // page-one merge covers the gap 首页并入覆盖缺口
    expect(s.rows.length, greaterThanOrEqualTo(51)); // loaded window survives 已载窗口存续
...
class _FailOnceRepository extends FixtureChatRepository {
  ...
  Future<Page<Touchpoint>> listTouchpoints(
    ...
    return super.listTouchpoints(conversationId, cursor: cursor, limit: limit, kind: kind, verb: verb);
  }
}
