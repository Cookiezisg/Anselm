...
void main() {
  ...
  test(
    'auto-title: the lifecycle echo re-reads the row — the head title fills in live',
    () async {
      final (c, repo) = setup();
      ...
      repo.upsert(
        _conv('cv_1', title: '季度对账问题'),
      ); // the backend titled it 后端已命名
      repo.emitSignal(
  ...
  test(
    'an ephemeral signal or another thread\'s signal never patches',
    () async {
      final (c, repo) = setup();
      await c.read(conversationHeaderProvider('cv_1').future);
  ...
  test(
    'rename + setModel patch from the authoritative response (tristate clear works)',
    () async {
      final (c, repo) = setup();
      await c.read(conversationHeaderProvider('cv_1').future);
      ...
      expect(
        c
            .read(conversationHeaderProvider('cv_1'))
            .value!
            .modelOverride
            ?.modelId,
        'deepseek-chat',
      ...
      expect(
        c.read(conversationHeaderProvider('cv_1')).value!.modelOverride,
        isNull,
      );
    },
  );
  ...
  test(
    'model capability options come through the CORE seam (S-15 moved off the chat repo)',
    () async {
      final c = ProviderContainer(
        overrides: [
