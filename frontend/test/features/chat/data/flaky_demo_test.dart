...
void main() {
  test('D-012 Cast ledger: cv_flaky fails its first touchpoint fetch, then a retry succeeds', () async {
    final repo = demoChatRepository();
    await expectLater(repo.listTouchpoints('cv_flaky'), throwsA(isA<StateError>()), reason: '首拉失败');
    ...
    expect((await demoChatRepository().listTouchpoints('cv_sync')).items, isNotEmpty);
  });

  test('D-021 send failure: cv_flaky fails its first send (→ the retry/discard bubble)', () async {
    final repo = demoChatRepository();
    // The throw happens BEFORE the reply playback, so no timers are scheduled. 抛在回放前,不排 timer。
    await expectLater(
        repo.sendMessage('cv_flaky', content: '测试一下发送'), throwsA(isA<StateError>()),
        reason: '首发失败→乐观泡失败态');
  });

  test('D-029 entity detail: fn_broken is listed in the rail but its detail GET throws', () async {
    final repo = demoEntityRepository();
    final rows = await repo.listEntities(EntityKind.function);
    expect(rows.items.any((r) => r.id == 'fn_broken'), isTrue, reason: 'rail 列出坏行');
    await expectLater(repo.getFunction('fn_broken'), throwsA(isA<StateError>()), reason: '详情 GET 抛→错误面');
