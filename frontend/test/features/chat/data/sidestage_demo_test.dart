...
void main() {
  ...
  test(
    'D-008/010 control + trigger stages: the wf_night graph refs resolve to real snapshots',
    () async {
      final rows = await repo.listTouchpoints('cv_sync');
      expect(
        rows.items.any(
          (r) => r.itemKind == 'control' && r.itemId == 'amount_gate',
        ),
        isTrue,
      );
      expect(
        rows.items.any(
          (r) => r.itemKind == 'trigger' && r.itemId == 'cron_nightly',
        ),
        isTrue,
      );
      // The stage GET (R-16: trigger trusts only this GET) must succeed. 舞台 GET 必成。
      expect(
        (await repo.getControlSnapshot('amount_gate')).activeVersion!.branches,
        isNotEmpty,
      );
  ...
  test('D-006/007/009 agent + approval + handler stages open', () async {
    ...
    expect(
      rows.items.any(
        (r) => r.itemKind == 'agent' && r.itemId == 'ag_reconcile',
      ),
      isTrue,
    );
    expect(
      rows.items.any(
        (r) => r.itemKind == 'approval' && r.itemId == 'apf_refund',
      ),
      isTrue,
    );
    expect(
      rows.items.any((r) => r.itemKind == 'handler' && r.itemId == 'hd_ledger'),
      isTrue,
    );
    expect(
      (await repo.getAgentSnapshot('ag_reconcile')).activeVersion!.prompt,
      isNotEmpty,
    );
    expect(
      (await repo.getApprovalSnapshot('apf_refund')).activeVersion!.template,
      contains('{{ input.amount }}'),
    );
    expect(
      (await repo.getHandlerSnapshot('hd_ledger')).activeVersion!.methods,
      isNotEmpty,
    );
  });

  test(
    'D-013 tombstone: a deleted-verb Cast row (its GET is banned, never seeded)',
    () async {
      final rows = await repo.listTouchpoints(
        'cv_sync',
      ...
      expect(
        () => repo.getFunctionSnapshot('fn_legacy_sync'),
