...
void main() {
  final term = showcaseConversations().firstWhere(
    (c) => c.conv.id == 'cv_show_term',
  );
  final bots = term.messages.where((m) => m.role == 'assistant').toList();

  test(
    'D-019/018/022 turn-end banners: max_steps / LLM_RESOLVE_ERROR / generic error',
    () {
      expect(
        bots.any((m) => m.stopReason == 'max_steps'),
        isTrue,
        reason: '琥珀 max_steps',
      );
      expect(
        bots.any((m) => m.errorCode == 'LLM_RESOLVE_ERROR'),
        isTrue,
        reason: '重选模型 CTA',
      );
      expect(
        bots.any(
          (m) =>
              m.stopReason == 'error' &&
              m.errorCode.isNotEmpty &&
              m.errorCode != 'LLM_RESOLVE_ERROR',
        ),
        isTrue,
        reason: '通用红 error 横幅',
      );
  ...
  test(
    'D-016 WebFetch soft-fail: status=completed but the sentence classifies RED',
    () {
      final blocks = bots.expand((m) => m.blocks).toList();
      final wf = blocks.firstWhere(
        (b) => b.type == 'tool_result' && b.parentBlockId == 'tm_wf',
      );
      expect(wf.status, 'completed', reason: '绿 status(软失败非硬错)');
      expect(
        webFetchOutcome(wf.content),
  ...
  test(
    'D-020 hard tool_result error: error:true → status=error / ownsError',
    () {
      final blocks = bots.expand((m) => m.blocks).toList();
      final hard = blocks.firstWhere(
        (b) => b.type == 'tool_result' && b.parentBlockId == 'tm_hard',
      );
  ...
  test(
    'D-015 failedHold: a failed subagent (cv_show_nested) opens its sidestage to failedHold',
    () {
      final nested = showcaseConversations().firstWhere(
        (c) => c.conv.id == 'cv_show_nested',
      );
      final t = ConversationTranscript('cv_show_nested')
        ..setHistory(nested.messages.reversed.toList());
