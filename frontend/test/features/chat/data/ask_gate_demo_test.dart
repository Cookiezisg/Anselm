...
void main() {
  ...
  test('cv_ask seeds an unresolved ask interaction with a prompt + options', () async {
  ...
  test('the ask gate rides a streaming turn (closed tool_call, no tool_result yet)', () async {
    final msgs = (await repo.listMessages('cv_ask')).items;
    final streaming = msgs.firstWhere((m) => m.role == 'assistant' && m.status == 'streaming');
    final gate = streaming.blocks.firstWhere((b) => b.attrs?['tool'] == 'ask_user');
    expect(gate.type, 'tool_call');
    // No tool_result block nested under it → still awaiting the human. 无 result 子块→仍等人答。
    expect(msgs.expand((m) => m.blocks).any((b) => b.parentBlockId == gate.id), isFalse);
  });
}
