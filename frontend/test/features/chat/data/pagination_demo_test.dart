...
void main() {
  ...
  test('D-005 rail pagination: >30 active conversations → a full first page + a cursor', () async {
  ...
  test('D-011 Cast ledger pagination: cv_p20 touched >50 things → a full page + a cursor', () async {
    final p1 = await repo.listTouchpoints('cv_p20', limit: 50);
    expect(p1.items.length, 50, reason: '满页');
    expect(p1.hasMore, isTrue);
    expect(p1.nextCursor, isNotNull);
    final p2 = await repo.listTouchpoints('cv_p20', limit: 50, cursor: p1.nextCursor);
