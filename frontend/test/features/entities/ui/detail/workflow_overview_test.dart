...
void main() {
  group('workflow overview (W2)', () {
    testWidgets('graph hero renders FIRST as a framed canvas with node cards', (
      tester,
    ) async {
      ...
      expect(
        ...
        lessThan(tester.getTopLeft(find.byType(AnKv).first).dy),
      );
      ...
      expect(
        find.text(
          TranslationProvider.of(
            tester.element(canvas),
          ).translations.entities.detail.graph.openEditor,
        ),
        findsOneWidget,
      );
    });
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.pump();
        expect(
          (await repo.getWorkflow('wf_1')).description,
          'A better description',
        );
        expect(
    ...
    testWidgets('meta: tags row edits via ➕/✕ and PATCHes tags', (
      tester,
    ) async {
      final repo = FixtureEntityRepository(
        workflows: [
          _wf(tags: const ['daily', 'ops']),
        ],
      );
      await tester.pumpWidget(
        _host(WorkflowOverview(wf: _wf(tags: const ['daily', 'ops'])), repo),
      );
      await tester.pump();
    ...
    testWidgets(
      'hero lights up from the SAME run state the right island drives (W3)',
      (tester) async {
        final repo = FixtureEntityRepository(
          workflows: [_wf()],
        ...
        expect(
          tester.widget<AnGraphCanvas>(find.byType(AnGraphCanvas)).run,
          isNull,
        );

        final container = ProviderScope.containerOf(
          tester.element(find.byType(WorkflowOverview)),
        );
        ...
        ); // reconcile lands truth rows
        final overlay = tester
            .widget<AnGraphCanvas>(find.byType(AnGraphCanvas))
            .run;
    ...
    testWidgets('unparseable graph blob → honest inset, not a blank hero', (
      tester,
    ) async {
  ...
  group('workflowVersionSummary (pure)', () {
    ...
    test('identical graphs → no chips; unparseable side → no chips', () {
      ...
      expect(
        workflowVersionSummary(_v(2, 'broken{'), _v(1, _graphV1)),
        isEmpty,
      );
    });

    test(
      'prettyJsonSource pretty-prints valid JSON, passes garbage through',
      () {
  ...
  group('fixture write plane', () {
    test(
      'patchWorkflowMeta mutates seeds + emits a durable lifecycle signal',
      () async {
        final repo = FixtureEntityRepository(workflows: [_wf()]);
        final signals = repo
            .lifecycleSignals(EntityKind.workflow)
            .take(1)
            .toList();
        final next = await repo.patchWorkflowMeta('wf_1', {
          'name': 'renamed',
          'tags': <String>['x'],
        });
