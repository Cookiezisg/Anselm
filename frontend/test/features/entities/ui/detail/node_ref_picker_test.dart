...
void main() {
  ...
  Future<String Function()> pumpPicker(WidgetTester tester,
      {required NodeKind kind, required String initial}) async {
    var ref = initial;
  ...
  testWidgets('action node: family dropdown + target dropdown render', (tester) async {
  ...
  testWidgets('switch family → target clears; pick handler → member appears; pick method → hd.method',
      (tester) async {
    final refOf = await pumpPicker(tester, kind: NodeKind.action, initial: 'fn_test');
    ...
    tester.widget<AnDropdown<RefFamily>>(find.byType(AnDropdown<RefFamily>)).onChanged!(RefFamily.handler);
    await tester.pump(const Duration(milliseconds: 30));
    expect(refOf(), 'hd_new');
    expect(tester.widget<AnDropdown<RefFamily>>(find.byType(AnDropdown<RefFamily>)).value, RefFamily.handler);
    ...
    tester.widget<AnDropdown<String>>(find.byType(AnDropdown<String>)).onChanged!('hd_db');
    await tester.pump(const Duration(milliseconds: 30));
    expect(refOf(), 'hd_db');
    expect(find.byType(AnDropdown<String>), findsNWidgets(2)); // target + member

    // 3) pick a method (the 2nd String dropdown = member) → ref = hd_db.query. 选方法 → hd_db.query。
    tester.widget<AnDropdown<String>>(find.byType(AnDropdown<String>).at(1)).onChanged!('query');
    await tester.pump(const Duration(milliseconds: 30));
    expect(refOf(), 'hd_db.query');
  });

  testWidgets('mcp: pick server then tool → mcp:server/tool (second level drills in)', (tester) async {
    final refOf = await pumpPicker(tester, kind: NodeKind.action, initial: 'mcp:');
    ...
    tester.widget<AnDropdown<String>>(find.byType(AnDropdown<String>)).onChanged!('github');
    await tester.pump(const Duration(milliseconds: 30));
    ...
    tester.widget<AnDropdown<String>>(find.byType(AnDropdown<String>).at(1)).onChanged!('create_issue');
    await tester.pump(const Duration(milliseconds: 30));
  ...
  testWidgets('reach mcp via the family dropdown (from a function ref) → mcp:server', (tester) async {
    final refOf = await pumpPicker(tester, kind: NodeKind.action, initial: 'fn_test');
    tester.widget<AnDropdown<RefFamily>>(find.byType(AnDropdown<RefFamily>)).onChanged!(RefFamily.mcp);
    await tester.pump(const Duration(milliseconds: 30));
    expect(refOf(), 'mcp:');
    expect(tester.widget<AnDropdown<RefFamily>>(find.byType(AnDropdown<RefFamily>)).value, RefFamily.mcp);
  ...
  testWidgets('agent node: a single target dropdown, no family chooser', (tester) async {
    await pumpPicker(tester, kind: NodeKind.agent, initial: 'ag_new');
    expect(find.byType(AnDropdown<RefFamily>), findsNothing); // single family → no chooser
    expect(find.byType(AnDropdown<String>), findsOneWidget);
  });

  testWidgets('trigger node: single target dropdown lists triggers', (tester) async {
    final refOf = await pumpPicker(tester, kind: NodeKind.trigger, initial: 'trg_new');
    expect(find.byType(AnDropdown<RefFamily>), findsNothing);
    tester.widget<AnDropdown<String>>(find.byType(AnDropdown<String>)).onChanged!('trg_cron');
    await tester.pump(const Duration(milliseconds: 30));
