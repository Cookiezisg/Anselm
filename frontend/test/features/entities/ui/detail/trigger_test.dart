...
Widget _wrap(Widget child) => TranslationProvider(
      child: MaterialApp(
        theme: AnTheme.light(),
        home: Scaffold(body: SizedBox(width: 720, child: SingleChildScrollView(child: child))),
      ),
    );

Widget _hostProvider(FixtureEntityRepository repo, Widget child) => ProviderScope(
...
void main() {
  group('TriggerOverview', () {
    testWidgets('cron: expression headline (only) + runtime + fire payload', (tester) async {
      await tester.pumpWidget(_wrap(TriggerOverview(
        trigger: _trigger(
          kind: TriggerSource.cron,
          config: const {'expression': '0 9 * * *'}, // cron reads nothing else (config.go:56)
      ...
      await tester.pump();
      expect(find.text('0 9 * * *'), findsOneWidget); // the copyable headline spec
    ...
    testWidgets('webhook: renders the mounted URL as the headline (copyable)', (tester) async {
      await tester.pumpWidget(_wrap(TriggerOverview(
        trigger: _trigger(kind: TriggerSource.webhook, config: const {'path': 'gh/push'}),
      )));
      await tester.pump();
      expect(find.text('/api/v1/webhooks/trg_1/gh/push'), findsOneWidget);
    });

    testWidgets('sensor: condition headline + target/interval; no next-fire (non-cron)', (tester) async {
      await tester.pumpWidget(_wrap(TriggerOverview(
        trigger: _trigger(
          kind: TriggerSource.sensor,
          config: const {'targetKind': 'handler', 'targetId': 'hd_q', 'method': 'depth', 'intervalSec': 30, 'condition': 'output.depth > 100'},
        ),
      )));
      await tester.pump();
      expect(find.text('output.depth > 100'), findsOneWidget);
      expect(find.text('30s'), findsOneWidget);
      expect(find.text(t.entities.detail.trigger.nextFire), findsNothing); // cron-only
    });
  });
  ...
  group('EntityOceanHeader (trigger)', () {
    EntityDetail detail() => EntityDetail(
          ...
          trigger: _trigger(kind: TriggerSource.cron, config: const {'expression': '* * * * *'}),
        );
    ...
    testWidgets('tapping Fire invokes onFire', (tester) async {
      var fired = false;
      await tester.pumpWidget(_wrap(EntityOceanHeader(detail: detail(), onFire: () => fired = true)));
      await tester.pump();
  ...
  group('observability tabs', () {
    FixtureEntityRepository repo() => FixtureEntityRepository(
          activations: {
            'trg_1': [
              Activation(id: 'tra_2', triggerId: 'trg_1', kind: TriggerSource.sensor, fired: true, firingCount: 1, createdAt: _t),
              Activation(id: 'tra_1', triggerId: 'trg_1', kind: TriggerSource.sensor, fired: false, detail: 'condition false', createdAt: _t),
            ],
          },
          firings: {
            'trg_1': [
              Firing(id: 'trf_1', triggerId: 'trg_1', workflowId: 'wf_x', activationId: 'tra_2', status: FiringStatus.started, flowrunId: 'flr_1', createdAt: _t, updatedAt: _t),
            ],
          },
        );

    testWidgets('activity tab lists activations; fired-only filter narrows', (tester) async {
      await tester.pumpWidget(_hostProvider(repo(), const TriggerActivityTab('trg_1')));
      await tester.pump();
      await tester.pump();
      final r = t.entities.detail.trigger;
      expect(find.textContaining(r.fired), findsWidgets);
      expect(find.textContaining(r.notFired), findsOneWidget); // the non-fired probe row
    ...
    testWidgets('dispatch tab lists firings', (tester) async {
      await tester.pumpWidget(_hostProvider(repo(), const TriggerDispatchTab('trg_1')));
      await tester.pump();
      await tester.pump();
    ...
    testWidgets('empty activity shows the inset empty state', (tester) async {
      await tester.pumpWidget(_hostProvider(FixtureEntityRepository(), const TriggerActivityTab('trg_none')));
      await tester.pump();
      await tester.pump();
  ...
  group('railDot', () {
    EntityRow row({required bool listening}) => EntityRow(
        kind: EntityKind.trigger, id: 'trg_1', listening: listening, createdAt: _t, updatedAt: _t);
