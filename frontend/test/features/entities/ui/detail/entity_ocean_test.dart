...
void main() {
  ...
  testWidgets('no selection → the Overview home (retires the tombstone)', (
    tester,
  ) async {
  ...
  testWidgets('function → header + tabs + overview content', (tester) async {
    await tester.pumpWidget(
      _host(sel: const EntityRef(EntityKind.function, 'fn_normalize')),
    );
    await tester.pump(const Duration(milliseconds: 50));
    expect(
      find.text('normalize-input'),
      findsWidgets,
    ); // ocean header title (+ the name KV row)
    ...
    expect(
      find.text('Coerce + trim raw fields'),
      findsOneWidget,
  ...
  testWidgets(
    'workflow → governance + alerts (graph deferred to the editor phase)',
    (tester) async {
      await tester.pumpWidget(
        _host(sel: const EntityRef(EntityKind.workflow, 'wf_digest')),
      );
      await tester.pump(const Duration(milliseconds: 50));
      expect(find.text('daily-digest'), findsWidgets);
      expect(find.text(d.sec.governance), findsOneWidget); // 运行治理
      expect(
        find.text(d.card.concurrency),
        findsWidgets,
      ); // 并发策略 (card title + kv label)
      expect(find.text(d.sec.graph), findsNothing); // graph viz deferred
    },
  );

  testWidgets('handler → init args shown but sensitive default MASKED', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(sel: const EntityRef(EntityKind.handler, 'hd_slack')),
    );
    await tester.pump(const Duration(milliseconds: 50));
    expect(find.text('token'), findsWidgets); // the sensitive arg name is shown
    expect(
      find.textContaining('xoxb'),
      findsNothing,
    ); // its secret default is NEVER rendered
  });
