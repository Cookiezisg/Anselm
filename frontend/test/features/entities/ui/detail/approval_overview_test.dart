...
ApprovalForm _apf({String timeout = '2d', String behavior = 'reject', bool allowReason = true}) => ApprovalForm(
      id: 'apf_1',
      name: 'deploy-gate',
...
void main() {
  testWidgets('renders meta + inputs + markdown template + decision rules', (tester) async {
    await tester.pumpWidget(_host(ApprovalOverview(approval: _apf())));
    await tester.pump();
    final d = TranslationProvider.of(tester.element(find.byType(ApprovalOverview))).translations.entities.detail;

    expect(find.text('Approve a production deploy'), findsOneWidget); // description
  ...
  testWidgets('empty timeout → "never", behavior row dropped', (tester) async {
    await tester.pumpWidget(_host(ApprovalOverview(approval: _apf(timeout: ''))));
    await tester.pump();
    final d = TranslationProvider.of(tester.element(find.byType(ApprovalOverview))).translations.entities.detail;
    expect(find.text(d.val.never), findsOneWidget); // no timeout
    expect(find.text('reject'), findsNothing); // behavior meaningless without a timeout → dropped
  });

  testWidgets('no active version → honest empty inset', (tester) async {
    await tester.pumpWidget(_host(ApprovalOverview(approval: _apf().copyWith(activeVersion: null))));
    await tester.pump();
