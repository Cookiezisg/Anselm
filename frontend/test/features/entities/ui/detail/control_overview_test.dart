...
ControlLogic _ctl({ControlVersion? version}) => ControlLogic(
      ...
      activeVersion: version ??
          ControlVersion(
            ...
            branches: const [
              Branch(port: 'pass', when: 'input.score > 0.8', emit: {'grade': '"A"'}),
...
void main() {
  testWidgets('renders meta + declared inputs + routing branches (port / when / emit / catch-all)',
      (tester) async {
    await tester.pumpWidget(_host(ControlOverview(control: _ctl())));
    await tester.pump();
    final d = TranslationProvider.of(tester.element(find.byType(ControlOverview))).translations.entities.detail;
    ...
    expect(find.text('${d.editor.branchEmit}: grade'), findsOneWidget); // pass reshapes payload
    expect(find.text('retry'), findsWidgets);
    expect(find.text(d.editor.branchDefault), findsOneWidget); // retry is the catch-all
    expect(find.text(d.val.passthrough), findsOneWidget); // retry has no emit → passthrough
  });

  testWidgets('no active version → honest empty inset', (tester) async {
    await tester.pumpWidget(_host(ControlOverview(control: _ctl().copyWith(activeVersion: null))));
    await tester.pump();
    expect(find.byType(AnState), findsOneWidget); // insetEmpty, not a blank page
  });
}
