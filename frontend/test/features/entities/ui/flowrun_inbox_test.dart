...
FlowrunComposite _parked({bool allowReason = true}) => FlowrunComposite(
  ...
  nodes: [
    FlowrunNode(
      ...
      status: 'parked',
      result: {
        'rendered': 'Approve production deploy of v4?',
...
Widget _host(FixtureEntityRepository repo) => ProviderScope(
  ...
  child: TranslationProvider(
    child: MaterialApp(
      theme: AnTheme.light(),
      home: Scaffold(
        body: SizedBox(width: 340, height: 640, child: const FlowrunInbox()),
      ),
    ),
  ),
);
          ...
          home: const Scaffold(
            body: SizedBox(
              ...
              child: SingleChildScrollView(
                child: FlowrunInbox(sectioned: true),
              ),
            ),
          ),
        );
...
void main() {
  testWidgets(
    'parked approvals render as decision cards (prompt + reason + approve/reject)',
    (tester) async {
      await tester.pumpWidget(_host(_repo(comp: _parked())));
      await tester.pump(); // provider resolves
      await tester.pump();
      final r = t
          .run; // gate copy lives in the core run namespace (S2b 上收) 门文案在 core run 命名空间
      expect(
        find.text('Approve production deploy of v4?'),
        findsOneWidget,
      ); // rendered prompt
      expect(
        find.text(t.run.addReason),
        findsOneWidget,
      ); // «+ 理由» pill (allowReason = true, B13 按需长出)
  ...
  testWidgets('deciding sends :decide with the reason', (tester) async {
    ...
    await tester.pumpAndSettle();
    final node = (await repo.getFlowrun(
      'flr_1',
    )).nodes.firstWhere((n) => n.nodeId == 'deploy_gate');
  ...
  testWidgets(
    'sectioned → collapsible «Needs you» head + cards; head collapse hides the cards',
    (tester) async {
      final repo = FixtureEntityRepository(
        flowrunDetail: {
          'flr_a': _parkedNamed('flr_a', 'gate_a'),
          'flr_b': _parkedNamed('flr_b', 'gate_b'),
        },
      );
      ...
      await tester.pump();
      expect(
        find.text(t.notifications.needsYou),
        findsOneWidget,
      ); // the group head
      ...
      final headRow = find.ancestor(
        of: find.text(t.notifications.needsYou),
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final dialog = find
          .ancestor(
            of: find.text(t.run.batchApproveTitle(n: '2')),
      ...
      await tester.tap(
        find.descendant(
          of: dialog,
          matching: find.widgetWithText(AnButton, t.run.approve),
        ),
      );
      await tester.pumpAndSettle();
      // both parked nodes were decided 'yes'
      final a = (await repo.getFlowrun(
        'flr_a',
      )).nodes.firstWhere((n) => n.nodeId == 'gate_a');
      final b = (await repo.getFlowrun(
        'flr_b',
      )).nodes.firstWhere((n) => n.nodeId == 'gate_b');
