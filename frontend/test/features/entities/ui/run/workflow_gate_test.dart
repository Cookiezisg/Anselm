...
void main() {
  ...
  testWidgets('parked run shows the approval gate; Approve resumes to ok', (
    tester,
  ) async {
    await tester.pumpWidget(
      routedHost(
        const Scaffold(
          body: SizedBox(width: 340, height: 800, child: RunTerminal()),
        ),
    ...
    ); // debounced reconcile lands truth
  ...
  testWidgets(
    'while walking, the terminal is honestly running (no premature ok)',
    (tester) async {
      await tester.pumpWidget(
        routedHost(
          const Scaffold(
