...
AnCodeEditor _editor(WidgetTester tester) =>
    tester.widget<AnCodeEditor>(find.byType(AnCodeEditor).first);
    ...
    HandlerEntity(
      ...
      activeVersion: HandlerVersion(
        ...
        methods: const [
          MethodSpec(
            name: 'send',
            inputs: [Field(name: 'to', type: 'string')],
          ),
          MethodSpec(
            name: 'archive',
            inputs: [Field(name: 'days', type: 'number')],
          ),
        ],
        createdAt: _t0,
        updatedAt: _t0,
...
void main() {
  ...
  testWidgets(
    'fn: ONE JSON editor prefilled with the example skeleton (哪里填哪个消失)',
    (tester) async {
      ...
      expect(ed.lang, 'json');
      expect(
        ed.editable && ed.seamless,
        isTrue,
      ); // seamless in-place JSON editor 同款嵌入代码块
      final seed = _json(ed.code);
      expect(
        seed.keys,
        containsAll(<String>['text', 'limit', 'strict', 'options']),
      );
  ...
  testWidgets(
    'hd: the method chip re-seeds the example; drafts remembered PER METHOD',
    (tester) async {
      ...
      expect(
        store.textFor(runDraftKey(hdRef, 'archive')),
        isNotNull,
      ); // its own seeded bucket
    },
  );

  testWidgets(
    'wf: the source chip re-seeds the per-source FIRE-PAYLOAD template',
    (tester) async {
      ...
      expect(
        _json(_editor(tester).code),
        <String, dynamic>{},
      ); // manual (default) → a free {}
      ...
      ); // trigger detail loads (kind resolves)
      await tester.pump();
      expect(_json(_editor(tester).code).keys, [
        'firedAt',
      ]); // cron → {firedAt} ONLY (real payload)

      ctrl.setSource('tr_fs');
      await tester.pump(const Duration(milliseconds: 50));
      await tester.pump();
      expect(
        _json(_editor(tester).code).keys,
        containsAll(<String>['firedAt', 'path', 'eventKind']),
      );
  ...
  testWidgets(
    'lint: invalid JSON disables the verb + shows a red line; valid re-enables',
    (tester) async {
      await tester.pumpWidget(_host(_fix(), fnRef));
      await tester.pump(const Duration(milliseconds: 50));
      ...
      expect(
        tester.widget<AnButton>(find.widgetWithText(AnButton, verb)).onPressed,
        isNull,
      ); // disabled
    },
  );

  testWidgets('recent strip: top-5 rows, human origin word, expand to IO', (
    tester,
  ) async {
    await tester.pumpWidget(_host(_fix(), fnRef));
    await tester.pump(const Duration(milliseconds: 50));
    ...
    expect(
      ...
      findsNWidgets(5),
    ); // seeded 7 → strip caps at 5
    expect(
      find.text(r.origin.manual),
      findsWidgets,
    ); // «manual» spoken as human 手动
    await tester.tap(find.byType(AnLedgerRow).at(1)); // an ok row 展开一行
    await tester.pumpAndSettle();
    expect(find.text(r.inputHeading), findsOneWidget);
    expect(
      find.textContaining('run-1'),
      findsWidgets,
    ); // that execution's raw input
  });

  testWidgets(
    '用这份输入: fills the editor from a past run (method restored for hd)',
    (tester) async {
      await tester.pumpWidget(_host(_fix(), hdRef));
      await tester.pump(const Duration(milliseconds: 50));
      await tester.pump();
      ...
      await tester.pump();
      expect(
        c.read(runTerminalProvider(hdRef)).method,
        'archive',
      ); // method followed 方法跟随
      final store = c.read(runDraftStoreProvider);
      expect(_json(store.textFor(runDraftKey(hdRef, 'archive'))!), {
        'days': 30,
  ...
  testWidgets(
    'wf recent row expands to «在运行页打开 →» (right island never hosts a long run)',
    (tester) async {
      await tester.pumpWidget(_host(_fix(), wfRef));
      await tester.pump(const Duration(milliseconds: 50));
      ...
      await tester.pumpAndSettle();
      expect(
        find.text(r.openRunPage),
        findsOneWidget,
      ); // the deep link, not inline IO 深链而非内联
    },
  );

  testWidgets(
    '⌘↵ submits from inside the editor (0719 用户裁定: the toolbar keycap glyph is gone, the '
    'chord itself stays wired)',
    (tester) async {
      await tester.pumpWidget(_host(_fix(), fnRef));
      await tester.pump(const Duration(milliseconds: 50));
      await tester.tap(
        find.byType(TextField).first,
      ); // focus the JSON editor 聚焦编辑器
      await tester.pump();
  ...
  testWidgets(
    'recent strip stays SILENT when there is no ledger (no tombstone, no section)',
    (tester) async {
      ...
      await tester.pumpWidget(
        _host(repo, const EntityRef(EntityKind.function, 'fn_2')),
      );
      await tester.pump(const Duration(milliseconds: 50));
      await tester.pump(const Duration(milliseconds: 50));
  ...
  group('head 三段式文法 (§1+§2)', () {
    ...
    testWidgets(
      'head = AnPanelHead (name title, NO ⋯ since the debugger has no panel action)',
      (tester) async {
        await tester.pumpWidget(
    ...
    testWidgets(
      'glance omits absent segments — a versioned entity with no ledger shows only v{N}',
      (tester) async {
        await tester.pumpWidget(
          _host(glanceFix(), const EntityRef(EntityKind.function, 'fn_v')),
        );
        await tester.pump(const Duration(milliseconds: 50));
        await tester.pump(const Duration(milliseconds: 50));
        expect(
