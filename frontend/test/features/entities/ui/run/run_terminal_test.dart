...
Widget _host(
  ...
  EntityRef? sel,
  Widget child = const RunTerminal(),
}) => routedHost(
...
void main() {
  ...
  testWidgets(
    'function idle → JSON editor card prefilled, ZERO tombstone (零墓碑)',
    (tester) async {
      await tester.pumpWidget(
        _host(_fix(), sel: const EntityRef(EntityKind.function, 'fn_1')),
      );
      await tester.pump(const Duration(milliseconds: 50)); // detail load
      expect(
        find.byType(RunEditorCard),
        findsOneWidget,
      ); // the JSON-first input card
      expect(
        find.byType(AnCodeEditor),
        findsOneWidget,
      ); // one prefilled JSON editor 预填 JSON 编辑器
      expect(
        find.byType(AnState),
        findsNothing,
      ); // never-ran = air, not a tombstone 没跑过=空气
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.pump(const Duration(milliseconds: 250));
      expect(
        find.byType(AnTermViewport),
        findsOneWidget,
      ); // scroll-back survives mid-run 运行中可回看
      ...
      // 运行信号=动词钮翻成「取消」。
      expect(
        find.widgetWithText(AnButton, t.entities.run.cancel),
        findsOneWidget,
      );
      ...
      expect(
        find.byType(AnTermViewport),
        findsOneWidget,
      ); // settled = SAME component, no material flip 落定同件零换脸
  ...
  testWidgets('agent invoke → ReAct trace with the tool name', (tester) async {
    await tester.pumpWidget(
      _host(_fix(), sel: const EntityRef(EntityKind.agent, 'ag_1')),
    );
    await tester.pump(const Duration(milliseconds: 50));
    await tester.tap(
      find.widgetWithText(AnButton, t.entities.detail.verb.invoke),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets(
    'block tree: reasoning collapsed by default, danger badge on a dangerous tool_call',
    (tester) async {
      ...
      final reducer = BlockTreeReducer()
        ..apply(
          const StreamEnvelope(
            ...
            scope: scope,
            id: 'b1',
            frame: FrameOpen(node: StreamNode(type: 'reasoning')),
          ),
        )
        ..apply(
          StreamEnvelope(
            ...
            frame: FrameClose(
              ...
              result: const StreamNode(
                type: 'reasoning',
                content: {'content': 'secret thought'},
              ),
            ),
          ),
        )
        ..apply(
          const StreamEnvelope(
            ...
            id: 'b2',
            frame: FrameOpen(
              node: StreamNode(type: 'tool_call', content: {'name': 'rm'}),
            ),
          ),
        )
        ..apply(
      ...
      await tester.pumpWidget(
        _host(
          ...
          child: SingleChildScrollView(
            child: BlockTreeView(roots: reducer.roots),
          ),
        ),
      );
      await tester.pump();
      ...
      expect(
        find.text(r.danger.dangerous),
        findsOneWidget,
      ); // danger badge (header, always visible)
      expect(
        find.text('secret thought'),
        findsNothing,
      ); // reasoning collapsed by default

      await tester.tap(
        find.text(r.reasoning),
      ); // expand the reasoning disclosure
      await tester.pumpAndSettle();
