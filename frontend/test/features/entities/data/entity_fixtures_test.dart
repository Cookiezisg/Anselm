...
FunctionExecution _exec(String id, String status) => FunctionExecution(
    id: id, functionId: 'fn_1', status: status, createdAt: _t);
...
void main() {
  ...
  test('keyset paging: limit slices + nextCursor + isLastPage (loadMore path)', () async {
    final repo = _repo();
    ...
    final p2 = await repo.listEntities(EntityKind.function, cursor: p1.nextCursor, limit: 2);
    expect(p2.items.map((r) => r.id), ['fn_3']);
    expect(p2.isLastPage, isTrue);
  });

  test('rail rows carry kind-specific badges (derived from the seed)', () async {
  ...
  test('listFunctionExecutions returns PageWithAggregate (ok/failed tally)', () async {
  ...
  test('scripted lifecycle signal flows to the kind stream', () async {
    ...
    const signal = EntitySignal(
        kind: EntityKind.function, id: 'fn_9', action: EntityAction.created, durable: true);
    final received = expectLater(repo.lifecycleSignals(EntityKind.function), emits(signal));
  ...
  test('scripted panel frame flows to the scope stream', () async {
    ...
    final env = StreamEnvelope(
        seq: 0, scope: scope, id: 'n1', frame: const FrameDelta(chunk: 'tick'));
