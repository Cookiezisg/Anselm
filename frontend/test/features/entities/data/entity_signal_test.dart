...
void main() {
  test(
    'projects a matching lifecycle frame (kind + id + action + durable)',
    () {
  ...
  test('ephemeral frame (seq 0) → durable false (list must NOT patch)', () {
    final s = EntitySignal.fromEnvelope(
      EntityKind.agent,
      _notif('agent.updated', {'agentId': 'ag_1'}, seq: 0),
    );
  ...
  test('action vocab collapses correctly', () {
    ...
    expect(
      act(EntityKind.function, 'function.created', 'functionId', 'fn_1'),
      EntityAction.created,
    );
    expect(
      act(EntityKind.function, 'function.deleted', 'functionId', 'fn_1'),
      EntityAction.deleted,
    );
    expect(
      act(EntityKind.function, 'function.edited', 'functionId', 'fn_1'),
      EntityAction.edited,
    );
    expect(
      act(EntityKind.function, 'function.reverted', 'functionId', 'fn_1'),
      EntityAction.edited,
    );
    expect(
      act(EntityKind.function, 'function.env_rebuilt', 'functionId', 'fn_1'),
      EntityAction.updated,
    );
    expect(
      act(EntityKind.handler, 'handler.crashed', 'handlerId', 'hd_1'),
      EntityAction.updated,
    );
    expect(
      act(EntityKind.handler, 'handler.config_updated', 'handlerId', 'hd_1'),
      EntityAction.updated,
    );
    expect(
      act(
        EntityKind.workflow,
        'workflow.lifecycle_changed',
        'workflowId',
        'wf_1',
      ),
      EntityAction.updated,
    ...
    expect(
      act(EntityKind.workflow, 'workflow.surprise', 'workflowId', 'wf_1'),
      EntityAction.unknown,
    );
  });
}
