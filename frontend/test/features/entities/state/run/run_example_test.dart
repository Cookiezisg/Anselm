...
void main() {
  group('exampleValue precedence', () {
    ...
    test('enum-first wins when no example/default', () {
      expect(
        exampleValue(
          const ExampleNode(type: 'string', enumValues: ['red', 'green']),
        ),
        'red',
      );
    });
    ...
    test('empty enum falls through to the type skeleton', () {
      expect(
        exampleValue(const ExampleNode(type: 'string', enumValues: [])),
        '',
      );
    });
  });
  ...
  group('type skeletons', () {
    test(
      'string → empty string',
      () => expect(exampleValue(const ExampleNode(type: 'string')), ''),
    );
    test(
      'number → 0',
      () => expect(exampleValue(const ExampleNode(type: 'number')), 0),
    );
    test(
      'boolean → false',
      () => expect(exampleValue(const ExampleNode(type: 'boolean')), false),
    );
    test(
      'object (no props) → {}',
      () => expect(
        exampleValue(const ExampleNode(type: 'object')),
        <String, Object?>{},
      ),
    );
    test(
      'array (no items) → []',
      () => expect(exampleValue(const ExampleNode(type: 'array')), <Object?>[]),
    );
    test(
      'unknown type → null (honest, not a fake scalar)',
      ...
      final v = exampleValue(
        const ExampleNode(
          ...
          properties: {
            ...
            'flags': ExampleNode(
              type: 'object',
              properties: {'on': ExampleNode(type: 'boolean')},
            ),
          },
        ),
      );
    ...
    test('array yields a single-element sample of its item node', () {
      expect(
        exampleValue(
          const ExampleNode(
            type: 'array',
            items: ExampleNode(type: 'string', example: 'x'),
          ),
        ),
        ['x'],
      );
  ...
  group(
    'fn/hd/ag Field adapter (type skeleton — the contract carries no example/default/enum)',
    () {
      ...
      test(
        'empty fields → empty object; the JSON seed is valid + runnable',
        () {
          ...
          final decoded = jsonDecode(
            exampleJsonForFields(const [Field(name: 'a', type: 'string')]),
          );
  ...
  group('workflow per-source templates (verbatim backend fire payloads)', () {
    ...
    test(
      'cron → {firedAt} only (NOT {firedAt, schedule} — the real cron payload)',
      () {
