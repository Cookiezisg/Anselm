...
void main() {
  ...
  test('runDraftKey: fn/ag collapse the dimension, hd/wf carry it', () {
    ...
    expect(
      runDraftKey(hd, 'recv'),
      isNot(runDraftKey(hd, 'send')),
    ); // per-method buckets 按方法分桶
  });

  test(
    'textFor: null until seeded/edited; setText writes, isolated per dimension',
    () {
      ...
      expect(
        store.textFor(runDraftKey(hd, 'recv')),
        isNull,
  ...
  test(
    'setText is QUIET (no notify — typing must not rebuild the lifecycle)',
    () {
      final store = RunDraftStore();
      var notified = 0;
  ...
  test(
    'fill: overwrites the bucket, bumps revision, notifies (an open editor re-seeds)',
    () {
      final store = RunDraftStore();
