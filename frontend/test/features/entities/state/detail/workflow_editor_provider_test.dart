...
void main() {
  ...
  test('approval fan-in: yes and no may target the SAME node (only exact dup rejected)', () async {
    final repo = FixtureEntityRepository(runDelay: Duration.zero, workflows: [
      WorkflowEntity(
        id: 'wf_1',
        name: 'a',
    ...
    expect(ctl.connect('gate', 'target'), isNull); // no → SAME target (different port) — allowed
    final st = c.read(workflowEditorProvider(_ref)).value!;
    final ports = st.working.edges.where((e) => e.to == 'target').map((e) => e.fromPort).toSet();
    expect(ports, {'yes', 'no'});
  ...
  test('setNodeKind resets the ref to the new kind placeholder (no cross-family leftover)', () async {
    final (c, ctl) = _harness(_repo());
    await c.read(workflowEditorProvider(_ref).future);
    // 'work' is an action with ref 'fn_1'. 'work' 是 action、ref fn_1。
    ctl.setNodeKind('work', NodeKind.agent);
    final n = c.read(workflowEditorProvider(_ref)).value!.working.nodes.firstWhere((n) => n.id == 'work');
    expect(n.kind, NodeKind.agent);
    expect(n.ref, 'ag_new'); // reset to the agent placeholder, NOT the stale 'fn_1'
    // A no-op kind set (same kind) leaves the ref untouched. 同 kind 不动 ref。
    ctl.setNodeKind('work', NodeKind.agent);
    expect(c.read(workflowEditorProvider(_ref)).value!.working.nodes.firstWhere((n) => n.id == 'work').ref,
        'ag_new');
  });
  ...
  test('an edit made during save survives as a re-saveable diff (no silent loss)', () async {
    final repo = _repo();
    final (c, ctl) = _harness(repo);
    await c.read(workflowEditorProvider(_ref).future);
    ctl.setNodeRef('work', 'fn_first');
    final saving = ctl.save(); // starts; parks on `await editWorkflow`
    ctl.setNodeRef('gate', 'ctl_midflight'); // edit while the save is in flight
    expect(await saving, isTrue);
    final st = c.read(workflowEditorProvider(_ref)).value!;
    // The mid-flight edit is still present AND still pending (not falsely clean). 在途编辑仍在且仍待存。
    expect(st.working.nodes.firstWhere((n) => n.id == 'gate').ref, 'ctl_midflight');
    expect(st.dirty, isTrue);
  ...
  test('reopening the editor and connecting again yields a distinct edge id (no collision)', () async {
    final repo = _repo();
    final (c, ctl) = _harness(repo);
    await c.read(workflowEditorProvider(_ref).future);
    expect(ctl.connect('gate', 'work'), isNull); // control back-edge, valid
    final firstId = c.read(workflowEditorProvider(_ref)).value!.working.edges.last.id;
    expect(await ctl.save(), isTrue);
    ...
    final ids = c.read(workflowEditorProvider(_ref)).value!.working.edges.map((e) => e.id).toList();
    expect(ids.toSet().length, ids.length); // all ids unique 全唯一
    expect(ids.where((id) => id == firstId), hasLength(1)); // the saved edge appears exactly once
  });
  ...
  test('back-edge validation matches the backend DFS (a cycle can make ANOTHER edge the back edge)',
      () async {
    final repo = FixtureEntityRepository(runDelay: Duration.zero, workflows: [
      WorkflowEntity(
        id: 'wf_1',
