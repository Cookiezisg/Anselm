...
List<String> _ids(ProviderContainer c) => c
    .read(entityListProvider(EntityKind.function))
    .value!
    .rows
    .map((r) => r.id)
    .toList();
...
void main() {
  test('first page loads → hasValue with seeded rows', () async {
    final c = _container(
      FixtureEntityRepository(functions: [_fn('fn_1', 'a'), _fn('fn_2', 'b')]),
    );
  ...
  test(
    'loadMore appends the next keyset page, staying hasValue throughout',
    () async {
      final seed = List.generate(
        25,
        (i) => _fn('fn_${i.toString().padLeft(2, '0')}', 'f$i'),
      );
  ...
  test('durable created → fetch new row + prepend', () async {
    final fixture = FixtureEntityRepository(
      functions: [_fn('fn_1', 'a'), _fn('fn_2', 'b')],
    );
    final c = _container(fixture);
    await c.read(entityListProvider(EntityKind.function).future);

    fixture.upsertFunction(
      _fn('fn_9', 'new'),
    ); // server-side create, now fetchable
  ...
  test(
    'durable edited → refetch that row + replace in place (length + order preserved)',
    () async {
      final fixture = FixtureEntityRepository(
  ...
  test(
    'ephemeral signal (seq=0 / durable false) never touches the list',
    () async {
      final fixture = FixtureEntityRepository(
        functions: [_fn('fn_1', 'a'), _fn('fn_2', 'b')],
      );
  ...
  test(
    'railModelProvider fans the 4 kinds into ordered groups with counts',
    () async {
      final c = _container(
  ...
  test(
    'setting search re-pages the kind list, filtered by name (case-insensitive)',
    () async {
      final c = _container(
        FixtureEntityRepository(
          functions: [
            _fn('fn_1', 'quarterly_report'),
            _fn('fn_2', 'random_helper'),
          ],
        ),
      );
      await c.read(entityListProvider(EntityKind.function).future);
