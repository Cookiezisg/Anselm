...
SchedulerSelection? parse(String location) =>
    SelectedScheduler.parseForTest(Uri.parse(location));
...
void main() {
  group('SelectedScheduler URL parse table (WRK-069 §11)', () {
    ...
    test(
      '/scheduler/w/:id → operations home; ?run= selects the linked pane',
      () {
        expect(parse('/scheduler/w/wf_1'), const SchedulerWorkflow('wf_1'));
        expect(
          parse('/scheduler/w/wf_1?run=fr_9'),
          const SchedulerWorkflow('wf_1', linkedRunId: 'fr_9'),
        );
      },
    );

    test(
      '/scheduler/w/:id/runs/:frId → run flagship; ?node=&iter= select a node',
      () {
        expect(
          parse('/scheduler/w/wf_1/runs/fr_9'),
          const SchedulerRun('wf_1', 'fr_9'),
        );
        expect(
          parse('/scheduler/w/wf_1/runs/fr_9?node=analyze&iter=2'),
    ...
    test(
      '/scheduler/runs/:frId → the id-only relay (fr_ paste / panel_registry)',
      () {
  ...
  group('AnStatus firing-disposition aliases (WRK-069 状态学「未执行」桶)', () {
    ...
    test(
      'skipped / superseded / shed / missed → idle (neutral, never red)',
      () {
        ...
        expect(
          AnStatus.fromRaw('missed').tone,
          AnTone.none,
          reason: '中性桶:一个饱和像素都不给',
        );
      },
    );
  });
  ...
  group('AnCountdown', () {
    ...
    testWidgets('pending deadline renders the remaining word (amber default)', (
      tester,
    ) async {
      await tester.pumpWidget(
        host(
          AnCountdown(
            deadline: DateTime.now().add(const Duration(hours: 2, minutes: 1)),
          ),
        ),
      );
    ...
    testWidgets('overdue deadline renders the overdue word in danger ink', (
      tester,
    ) async {
      await tester.pumpWidget(
        host(
    ...
    testWidgets(
      'many countdowns share ONE timer (C-track: never a per-row ticker)',
      (tester) async {
        await tester.pumpWidget(
          host(
            Column(
              children: [
                for (var i = 1; i <= 20; i++)
                  AnCountdown(deadline: DateTime.now().add(Duration(hours: i))),
              ],
            ),
          ),
        );
