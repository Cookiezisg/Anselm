...
void main() {
  ...
  test(
    'workflowTriggerEdges sends kind=equip ONLY — a kind-only from/to pair is a backend 400',
    () async {
      final b = build(
        (o) => _json([
          ...
          edge(
            ...
            'agent',
            'function',
          ), // must be narrowed away client-side. 客户端须滤掉。
  ...
  test(
    'runMatrix sends ONE flowrunIds csv — never recentN/workflowId (0717 主页重建线缆形)',
    () async {
      ...
      expect(
        q.containsKey('workflowId'),
        isFalse,
        reason: '端点不再有 workflow 轴——内容就是 ids',
      );
    },
  );

  test(
    'listFlowruns serializes the time window as RFC3339 UTC (picker absolute range wire form)',
    () async {
      ...
      expect(
        q['startedAfter'],
        endsWith('Z'),
        reason: 'RFC3339 UTC,绝不发无时区的本地串',
      );
      expect(q['startedBefore'], endsWith('Z'));
      expect(
        DateTime.parse(q['startedAfter']!),
        DateTime(2026, 6, 1, 9, 0).toUtc(),
      );
    },
  );
}
