...
void main() {
  group(
    'runListFilter — the ONE wire grammar (page-level AnTimeRange, 0717 拍板)',
    () {
      test('all: no status, no time bound when «全部»', () {
        final f = runListFilter(
          filter: RunStatusFilter.all,
          range: const AnPresetRange(AnTimePreset.all),
          now: _now,
        );
      ...
      test('waiting asks for RUNNING — never ?status=parked (封闭集无此值,422)', () {
        final f = runListFilter(
          filter: RunStatusFilter.waiting,
          range: const AnPresetRange(AnTimePreset.d7),
          now: _now,
        );
      ...
      test(
        'presets subtract their exact span from now (live expressions, no startedBefore)',
        () {
          expect(
            runListFilter(
              filter: RunStatusFilter.all,
      ...
      test(
        'an absolute pair sends BOTH bounds — inclusive minute end pushed one past (半开)',
        () {
          final f = runListFilter(
            filter: RunStatusFilter.all,
          ...
          expect(
            ...
            reason:
                '闭分钟端 → API 半开 [from, to+1min):23:59:30 的 run 含在「…– 23:59」里',
          );
        },
      );
  ...
  group('runSourceOf — the row IS its source phrase', () {
    test('manual / chat (chat carries its conversation coordinate)', () {
      ...
      final chat = runSourceOf(
        _run(origin: 'chat', conversationId: 'cv_1'),
        const {},
      );
    ...
    test('cron detail = THIS run\'s local fire time-of-day', () {
      final started = DateTime(
        ...
        9,
        5,
      ); // local — the phrase is a wall-clock read. 本地。
    ...
    test('webhook detail = its path (config beats the name)', () {
      ...
      expect(
        runSourceOf(_run(origin: 'webhook', triggerId: 'tr_h'), {
          'tr_h': t,
        }).detail,
        '/invoice',
      );
    ...
    test('fsnotify / sensor detail = the trigger name', () {
      ...
      expect(
        runSourceOf(_run(origin: 'fsnotify', triggerId: 'tr_f'), {
          'tr_f': t,
        }).detail,
        '监听 inbox/',
      );
      expect(
        runSourceOf(_run(origin: 'sensor', triggerId: 'tr_f'), {
          'tr_f': t,
        }).detail,
        '监听 inbox/',
      );
    ...
    test(
      'a pre-provenance row (origin null) is honestly UNKNOWN — never a zero-value lie',
      () {
    ...
    test(
      'an unknown future origin word degrades to unknown, never crashes',
      () {
    ...
    test(
      'a dangling triggerId (deleted trigger) leaves the detail absent, not a crash',
      () {
        expect(
          runSourceOf(
            _run(origin: 'webhook', triggerId: 'tr_gone'),
            const {},
          ).detail,
          isNull,
        );
  ...
  group('waitingRunIds — inbox-derived membership', () {
    test('this workflow only, de-duped, order preserved', () {
      final rows = [
        ...
        stubInboxRow(
          ...
          'gate_b',
          wfId: 'wf_b',
        ), // another workflow. 别的 workflow。
        stubInboxRow(
          'fr_1',
          'gate_c',
          wfId: 'wf_a',
        ), // same run, second gate. 同 run 两闸。
    ...
    test(
      'empty inbox → empty set',
      () => expect(waitingRunIds(const [], 'wf_a'), isEmpty),
    );
  });
  ...
  group('replayCounts — the memoization promise\'s real numbers', () {
    ...
    test(
      'failed rows re-run, completed rows are reused, parked counts as neither',
      () {
  ...
  group('statsWindowOf — the flowrun-stats wire words (需求②)', () {
    ...
    test(
      'duration presets ride the duration grammar, until absent (now-anchored)',
      () {
    ...
    test('«today» = local midnight as RFC3339 UTC, until absent', () {
      ...
      expect(
        DateTime.tryParse(w.since),
        isNotNull,
        reason: '后端 parseSince 只认 RFC3339/时长',
      );
    });

    test(
      '«all» = the epoch (stats has no unbounded spelling; absent-since defaults to 7d)',
      () {
