...
void main() {
  ...
  test(
    'stats honours the requested-id filter; totals stay workspace-wide',
    () async {
  ...
  test(
    'run rows: every running workflow has a running run; the streak workflow has a failed run '
    'with a wire error (the Overview zones all render real data)',
    () async {
      final stats = await repo.stats(const []);
      for (final s in stats.byWorkflow.where((s) => s.running > 0)) {
        ...
        expect(
          page.items.every((r) => r.status == 'running' && r.startedAt != null),
          isTrue,
          reason: '活耗时需要 startedAt',
        );
      }
      final streak = stats.byWorkflow.firstWhere(
        (s) => s.consecutiveFailures > 0,
      );
      final failed = await repo.listFlowruns(
        workflowId: streak.workflowId,
        status: 'failed',
        limit: 1,
      );
  ...
  test(
    'failed totals are DERIVED and window-monotonic: 24h < 48h ≤ 7d, delta positive (worsening)',
    () async {
      ...
      final list24 = await repo.listFailedSince(
      ...
      expect(
        f24 - (f48 - f24),
        greaterThan(0),
        reason: 'worsening: 坏的一夜就在最近 24h',
      );
    },
  );

  test(
    'a listening cron trigger fires WITHIN the next 24h (the upcoming zone has a row)',
    () async {
      final triggers = await repo.listTriggers();
      final edges = await repo.workflowTriggerEdges();
      final now = DateTime.now();
      final soon = triggers.where(
        (t) =>
            t.listening &&
            t.nextFireAt != null &&
  ...
  test(
    'decide is stateful: the row leaves the inbox; a second decide loses first-wins (422)',
    () async {
      final fresh = demoSchedulerRepository();
      final before = await fresh.listInbox();
      final target = before.first;
      await fresh.decideApproval(
        target.node.flowrunId,
        target.node.nodeId,
        decision: 'yes',
      );
      ...
      expect(
        after.where((r) => r.node.flowrunId == target.node.flowrunId),
        isEmpty,
      );

      await expectLater(
        fresh.decideApproval(
          target.node.flowrunId,
          target.node.nodeId,
          decision: 'no',
        ),
  ...
  test(
    'cancel is stateful: the running row leaves; its parked inbox row is withdrawn; '
    'a second cancel (and a non-running cancel) earn 422',
    () async {
      final fresh = demoSchedulerRepository();
      ...
      final running = await fresh.listFlowruns(
      ...
      expect(
        inbox.where((r) => r.node.flowrunId == frId),
        isEmpty,
        reason: '收件箱不留死项',
      );
      ...
      await expectLater(
        fresh.cancelRun('fr_c3d4e5f607182930'),
        throwsA(
          isA<ApiException>().having((e) => e.httpStatus, 'httpStatus', 422),
        ),
  ...
  test(
    'cron triggers carry a FUTURE nextFireAt and an equip edge to their workflow',
    () async {
      final triggers = await repo.listTriggers();
      final edges = await repo.workflowTriggerEdges();
      expect(
        triggers.where(
          (t) => t.nextFireAt != null && t.nextFireAt!.isAfter(DateTime.now()),
        ),
        isNotEmpty,
        reason: '⏱ meta 需要未来 fire',
      );
      for (final e in edges) {
  ...
  test(
    'the home\'s run history pages (25+ rows over every origin, failures carry wire errors)',
    () async {
      ...
      final second = await repo.listFlowruns(
        workflowId: 'wf_clean',
      ...
      final ids = {
        for (final r in [...first.items, ...second.items]) r.id,
      };
      expect(
        ids.length,
      ...
      final all = (await repo.listFlowruns(
        workflowId: 'wf_clean',
        limit: 200,
      )).items;
      ...
      expect(
        origins,
        containsAll(<String?>['cron', 'chat', 'webhook', 'manual']),
        reason: '全来源种齐',
      );
      ...
      expect(
        failed.every((r) => (r.error ?? '').isNotEmpty),
        isTrue,
        reason: '失败行带错误(渲 danger 副行)',
      );
      expect(
        failed.any((r) => r.error!.contains('\n')),
        isTrue,
        reason: '多行错误 → 行只取首句',
      );
    },
  ...
  test(
    'the wire filters really narrow: status / origin / startedAfter (工单⑥)',
    () async {
      final failed = await repo.listFlowruns(
        workflowId: 'wf_clean',
        status: 'failed',
      ...
      final cron = await repo.listFlowruns(
        workflowId: 'wf_clean',
        origin: 'cron',
        limit: 50,
      );
      ...
      final win = await repo.listFlowruns(
        workflowId: 'wf_clean',
        startedAfter: since,
        limit: 50,
      );
  ...
  test(
    'run nodes seed the linked pane: failures stop at analyze (notify 未及), completions walk all',
    () async {
      ...
      expect(
        failed.nodes.map((n) => n.nodeId),
        isNot(contains('notify')),
        reason: 'notify 未及(灰弱)',
      );

      final done = await repo.getRunFull('fr_chat00000000a1');
      expect(done.nodes.map((n) => n.nodeId), [
        'fetch',
        'gate',
        'analyze',
        'notify',
      ...
      expect(
        done.nodes.every((n) => n.readyAt != null && n.startedAt != null),
        isTrue,
        reason: '⑫ 排队戳:甘特三段条的数据源',
      );
      expect(
        done.nodes.every((n) => !n.startedAt!.isBefore(n.readyAt!)),
        isTrue,
        reason: '因果序 readyAt ≤ startedAt(与后端 timing_test 同一不变式)',
      );
      expect(
        done.nodes.any(
          (n) => n.completedAt!.difference(n.startedAt!).inMilliseconds > 0,
        ),
        isTrue,
      );
      // createdAt IS the terminal write — it must NOT be usable as a start stamp. createdAt=终态写入时刻。
      expect(
        done.nodes.every((n) => n.completedAt == n.createdAt),
        isTrue,
  ...
  test(
    'wf_clean carries the active-version graph the pane\'s graph face renders',
    () async {
      ...
      expect(
        ...
        containsAll(comp.nodes.map((n) => n.nodeId)),
      );
    },
  );

  test(
    'a PAUSED trigger is seeded and reads the wire truth: no nextFireAt, not listening (工单⑦)',
    () async {
      final triggers = await repo.listTriggers();
      final paused = triggers.where((t) => t.paused);
      expect(paused, isNotEmpty, reason: '暂停态卡种子');
      expect(
        paused.every((t) => t.nextFireAt == null),
        isTrue,
        reason: '暂停时 nextFireAt 缺席',
      );
      expect(paused.every((t) => !t.listening), isTrue, reason: '暂停时监听器冷');
      expect(
        triggers.where((t) => t.kind == TriggerSource.webhook),
        isNotEmpty,
  ...
  test(
    'pause / resume are stateful + idempotent, and flip the whole wire trio',
    () async {
      final fresh = demoSchedulerRepository();
      await fresh.pauseTrigger('tr_cron_clean');
      var t = (await fresh.listTriggers()).firstWhere(
        (t) => t.id == 'tr_cron_clean',
      );
      ...
      t = (await fresh.listTriggers()).firstWhere(
        (t) => t.id == 'tr_cron_clean',
      );
  ...
  test(
    'replay is stateful: a failed run flips running; only failed replays (422 otherwise)',
    () async {
      final fresh = demoSchedulerRepository();
      ...
      expect(
        (await fresh.getRun(frId)).status,
        'running',
        reason: 'replay 后翻 running',
      );
      ...
      await expectLater(
        fresh.replayRun('fr_chat00000000a1'),
        throwsA(
          isA<ApiException>().having((e) => e.httpStatus, 'httpStatus', 422),
        ),
  ...
  test(
    'Run now births a running manual run; :kill flips inactive and cancels the in-flight',
    () async {
      final fresh = demoSchedulerRepository();
      ...
      expect(
        (await fresh.listWorkflows())
            .firstWhere((w) => w.id == 'wf_clean')
            .lifecycleState,
        'inactive',
        reason: 'rail 行随之停用(左岛沉底段)',
      );
    },
  );
  ...
  test(
    'S4 · the COLD-OPEN run is live with NO rows — the page must not blank (§5.5)',
    () async {
  ...
  test(
    'S4 · activity (⑤) is derived from the SAME rows, so gantt and ledger cannot disagree',
    () async {
      ...
      final analyze = [
        for (final r in rows)
          if (r.nodeId == 'analyze') r,
      ];
      expect(analyze, hasLength(3), reason: '逐迭代各一行审计');
      expect(
        analyze.every((r) => r.execId.startsWith('agx_')),
        isTrue,
        reason: 'agent 族审计 id',
      );
  ...
  test(
    'S4 · the legacy run carries NO queue stamps — the two-part degradation is on screen too',
    () async {
      final comp = await repo.getRunFull('fr_legacy000000c3');
      expect(
        comp.nodes.every((n) => n.readyAt == null && n.startedAt == null),
        isTrue,
  ...
  test(
    'S4 · :triage hands back a conversation id the caller can deep-link into chat',
    () async {
  ...
  test(
    'S5 · ⑧ the forward schedule seeds a DENSE lane (bucket folding is provable on screen) '
    'and a paused trigger contributes NOTHING — exactly as the real endpoint behaves',
    () async {
      final sched = await repo.triggerSchedule();
      expect(sched.points, isNotEmpty);
      expect(
        sched.points.map((p) => p.at).toList(),
        orderedEquals(sched.points.map((p) => p.at)),
        reason: 'at 升序:端点契约',
      );
      ...
      expect(
        dense.length,
        greaterThan(50),
        reason: '密集泳道:不折叠就是亚像素纸屑,故必须种到能逼出折叠',
      );
      ...
      expect(
        sched.points.where((p) => p.triggerId == 'tr_cron_archive'),
        isEmpty,
        reason: '端点只为监听中且未暂停的 cron 发刻度——暂停的一个都不发',
      );
      final paused = (await repo.listTriggers()).firstWhere(
        (t) => t.id == 'tr_cron_archive',
      );
  ...
  test(
    'S5 · ⑩ unknown ids are NOT an error — three empty lists, silently absent (wire law)',
    () async {
  ...
  test(
    'S5 · ⑩ column order is canonical regardless of request order (行轴不许被乱序左右)',
    () async {
  ...
  test(
    '调度轨 · the past grid has data: listRunsSince returns in-window runs, multi-status, multi-source',
    () async {
      final since = DateTime.now().subtract(const Duration(hours: 24));
      final runs = await repo.listRunsSince(since);
      expect(runs, isNotEmpty, reason: '过去健康格需要窗内 run');
      expect(
        runs.every((r) => r.startedAt != null && !r.startedAt!.isBefore(since)),
        isTrue,
        reason: '全部在窗内开始(按 started_at 开窗)',
      );
      expect(
        runs.map((r) => r.status).toSet().length,
        greaterThan(1),
        reason: '多状态:健康格既有绿(completed)又有红(failed)',
      );
      expect(
        runs.any((r) => r.status == 'failed'),
        isTrue,
        reason: '至少一个失败 → 一个红格',
      );
      expect(
        runs.map((r) => r.origin).toSet().length,
        greaterThan(1),
        reason: '裁决③:格统计**所有来源**——窗内不止 cron',
      );
      // The dense workflow (wf_clean) has many in-window runs → the high-frequency lane. 高频泳道种子。
      expect(
        runs.where((r) => r.workflowId == 'wf_clean').length,
        greaterThan(5),
  ...
  test(
    'S6 · ⑭ every missed row obeys the backend\'s rules: the tick IS the timestamp, no run, no '
    'activation, cron only',
    () async {
      ...
      for (final f in missed) {
        ...
        expect(
          f.activationId,
          isEmpty,
          reason: '记账不是一次动作 —— sweep 不为它记 activation',
        );
        expect(
          f.createdAt.isBefore(now),
          isTrue,
          reason: 'createdAt 是**错过的刻度**,刻度只能在过去',
        );
        ...
        expect(
          t!.kind,
          TriggerSource.cron,
          reason: 'sweep 只看 cron —— 只有 cron 有「本该发生」的刻度',
        );
      }
    },
  );
  ...
  test(
    'S6 · ⑭ a started firing carries its run; the live run\'s row and its firing agree',
    () async {
      final started = (await repo.listFirings(
        status: FiringStatus.started,
        limit: 500,
      )).items;
      final runIds = {
        for (final r in (await repo.listFlowruns(workflowId: 'wf_clean')).items)
          r.id,
      };
      for (final f in started) {
        expect(
          f.flowrunId,
          isNotEmpty,
          reason: 'started = 刻度**成了** run,故必有 run id',
        );
      }
      ...
      final live = (await repo.listFlowruns(workflowId: 'wf_clean')).items
          .firstWhere(
            (r) => (r.firingId ?? '').isNotEmpty,
            orElse: () => fail('没有任何 run 引用 firing —— 出处链无从验起'),
          );
      final cited = started.firstWhere(
        (f) => f.id == live.firingId,
  ...
  test(
    'S6 · ⑭ the window is HALF-OPEN [after, before) and filters compose with AND',
    () async {
      final all = (await repo.listFirings(limit: 500)).items;
      expect(
        all.map((f) => f.createdAt).toList(),
        orderedEquals(
          ([...all]..sort((a, b) => b.createdAt.compareTo(a.createdAt))).map(
            (f) => f.createdAt,
          ),
        ),
        reason: '新→旧:端点契约',
  ...
  test(
    'S6 · ⑭ the page CAPS and says so — a truncated ledger is the newest slice, not the whole one',
    () async {
      ...
      expect(
        capped.items.map((f) => f.id),
        whole.items.take(2).map((f) => f.id),
        reason: '截断的是**最新**那片',
      );
    },
  );
