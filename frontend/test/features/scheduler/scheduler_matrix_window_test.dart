...
void main() {
  ...
  StubSchedulerRepo repo(int n) {
    final runs = [
      for (var i = 0; i < n; i++)
        Flowrun(
          ...
          completedAt: now.subtract(Duration(minutes: i + 1)).add(const Duration(seconds: 5)),
          updatedAt: now,
        ),
    ];
    final grid = FlowrunMatrix(
      cols: [
        for (final r in runs)
          MatrixCol(flowrunId: r.id, startedAt: r.startedAt!, status: 'completed', elapsedMs: 5000),
      ],
      rows: const [
      ...
      cells: [
        for (final r in runs) MatrixCell(flowrunId: r.id, nodeId: 'n1', status: 'completed'),
    ...
    return StubSchedulerRepo(
      workflows: [
        SchedulerWorkflowRow(id: 'wf_m', name: 'M', lifecycleState: 'active', updatedAt: now),
      ],
      runs: runs,
      matrixGrid: grid,
    );
  ...
  test('page 1: the window pages the runs and batch-fetches EXACTLY that page of ids', () async {
    final h = harness(60);
    final s = await h.container.read(schedulerMatrixWindowProvider('wf_m').future);
    expect(s.matrix.cols.length, SchedulerWindows.matrixPageSize, reason: '首页=页尺 50');
    expect(s.matrix.cols.first.flowrunId, 'fr_000', reason: '正典新→旧');
    expect(s.hasMore, isTrue);
    expect(h.stub.matrixAsks.single.length, SchedulerWindows.matrixPageSize,
    ...
    expect(s.matrix.rows.map((r) => r.nodeId), ['n1'],
        reason: '行轴=已加载列的并集,不替未加载的历史发明行');
  });

  test('loadOlder merges: cols concat, rows first-seen union (older-only node APPENDS), cells pile',
      () async {
    final h = harness(60);
    await h.container.read(schedulerMatrixWindowProvider('wf_m').future);
    await h.container.read(schedulerMatrixWindowProvider('wf_m').notifier).loadOlder();
    ...
    expect(s.matrix.rows.map((r) => r.nodeId), ['n1', 'n2'],
  ...
  test('a failed older page keeps the window STANDING (busy flag dropped, nothing merged)',
      () async {
    final h = harness(60);
    await h.container.read(schedulerMatrixWindowProvider('wf_m').future);
    ...
    final s = h.container.read(schedulerMatrixWindowProvider('wf_m')).value!;
    expect(s.matrix.cols.length, SchedulerWindows.matrixPageSize, reason: '窗照旧站着');
    expect(s.loadingOlder, isFalse, reason: 'busy 旗收回');
    expect(s.hasMore, isTrue, reason: '旧页是可选历史,下次还能再试');
  });

  test('mid-flight range change DROPS the stale older page — two ranges never mix (复审 [4])',
      () async {
    final h = harness(60);
    await h.container.read(schedulerMatrixWindowProvider('wf_m').future);
    ...
    final older = h.container.read(schedulerMatrixWindowProvider('wf_m').notifier).loadOlder();
    await Future<void>.delayed(const Duration(milliseconds: 10));
    h.container.read(schedulerTimeRangeProvider.notifier).set(const AnPresetRange(AnTimePreset.h24));
    await h.container.read(schedulerMatrixWindowProvider('wf_m').future);
    h.stub.matrixLatency = Duration.zero;
    await older;

    final s = h.container.read(schedulerMatrixWindowProvider('wf_m')).value!;
    expect(s.matrix.cols.length, SchedulerWindows.matrixPageSize,
