...
FlowrunActivityRow _act(String nodeId, {int iteration = 0, int elapsedMs = 5000, int startSec = 1}) =>
    FlowrunActivityRow(
      nodeId: nodeId,
      iteration: iteration,
      kind: 'agent',
...
FlowrunComposite _comp(List<FlowrunNode> nodes, {String status = 'completed'}) => FlowrunComposite(
      flowrun: Flowrun(id: 'fr_1', workflowId: 'wf_1', status: status, startedAt: _t, updatedAt: _t),
      nodes: nodes,
    );
...
void main() {
  ...
  group('nodeTiming — the queue/exec split (§5.3 判决⑤)', () {
    test('⑫ stamps → the grey queue leg; ⑤ audit row → the exec leg is the AUDIT\'s own elapsed', () {
      final n = _node('analyze', 'completed', kind: 'agent', readySec: 0, startSec: 2, endSec: 9);
      final t = nodeTiming(n, activity: _act('analyze', elapsedMs: 5000, startSec: 2));
      expect(t.queue, const Duration(seconds: 2)); // readyAt→startedAt
      expect(t.exec, const Duration(milliseconds: 5000)); // the audit row's self-report 审计自报
      expect(t.hasSplit, isTrue);
    });

    test('no audit row (control evaluates inline) → exec falls back to the row\'s own stamps', () {
      ...
      final n = _node('gate', 'completed', kind: 'control', readySec: 0, startSec: 1, endSec: 4);
    ...
    test('an approval\'s exec ends at the PARK write; the wait is the amber parked leg', () {
      ...
      final n = FlowrunNode(
        ...
        nodeId: 'approve',
        kind: 'approval',
        status: 'completed', // decided — the status flipped off 'parked' 已决,状态已翻
        readyAt: _t,
        startedAt: _t.add(const Duration(seconds: 1)),
        createdAt: _t.add(const Duration(seconds: 2)), // parked here 停在这
        completedAt: _t.add(const Duration(minutes: 18)), // a human answered 18m later 人 18 分钟后答
        updatedAt: _t,
      );
      final t = nodeTiming(n);
      expect(t.exec, const Duration(seconds: 1), reason: '执行段=渲染 prompt 到停车,不含人等');
      // The wait starts at the PARK WRITE (+2s), not at the node's start — 18m − 2s. 人等自停车写入起算。
      expect(t.parked, const Duration(minutes: 17, seconds: 58), reason: '人等是琥珀段,自停车时刻起算');
    });

    test('a negative span (replay\'s stale audit row / clock skew) clamps to zero, never renders < 0',
        () {
      final n = FlowrunNode(
        id: 'frn_x',
        flowrunId: 'fr_1',
        nodeId: 'x',
        status: 'completed',
        readyAt: _t.add(const Duration(seconds: 5)), // AFTER startedAt — api.md ⑤ warns of this 反序
        startedAt: _t,
        createdAt: _t,
        completedAt: _t,
        updatedAt: _t,
  ...
  group('runTiming — the head IS the ledger\'s total (§5.3 双数同源)', () {
    test('sums its rows\' splits, so head and ledger can never disagree', () {
      final nodes = [
        ...
        _node('analyze', 'completed', kind: 'agent', readySec: 3, startSec: 5, endSec: 10),
      ];
    ...
    test('rows with no stamps contribute nothing rather than zeroing the total', () {
  ...
  group('foldNodeLedger — ×N fold + diagnostic ordering (§5.4)', () {
    test('a loop folds to ONE line carrying every turn, newest as its face', () {
      final nodes = [
        for (var i = 0; i < 3; i++)
          _node('analyze', 'completed', kind: 'agent', iteration: i, startSec: i, endSec: i + 1),
      ];
      final out = foldNodeLedger(_graph, nodes);
      expect(out, hasLength(1));
      expect(out.first.iterations, 3);
      expect(out.first.rows.map((r) => r.iteration), [0, 1, 2], reason: '逐轮升序,供迭代切换器');
      expect(out.first.latest!.iteration, 2);
    });

    test('failed → parked → the live front → the rest; graph order holds WITHIN a rank', () {
      final nodes = [
        _node('fetch', 'completed', startSec: 0, endSec: 1),
        _node('gate', 'parked', kind: 'control', startSec: 1),
        _node('analyze', 'failed', kind: 'agent', startSec: 2, endSec: 3, error: 'boom'),
      ];
      final out = foldNodeLedger(_graph, nodes, inferredRunning: {'notify'});
      expect(out.map((e) => e.nodeId), ['analyze', 'gate', 'notify', 'fetch']);
      expect(out.first.failed, isTrue);
      expect(out[2].inferred, isTrue, reason: '活前沿在诊断行之后、其余之前');
    });

    test('a graph node that never ran is NOT a ledger row (the ledger records what HAPPENED)', () {
      final out = foldNodeLedger(_graph, [_node('fetch', 'completed', startSec: 0, endSec: 1)]);
      expect(out.map((e) => e.nodeId), ['fetch']);
    });

    test('an orphan row (renamed/removed node) keeps its place instead of vanishing', () {
  ...
  group('inferredRunningNodes — the speculative front (§5.5 冷打开推测态)', () {
    test('a live run with settled predecessors infers the front from the PINNED graph', () {
      final comp = _comp([
        _node('fetch', 'completed', startSec: 0, endSec: 1),
        _node('gate', 'completed', kind: 'control', startSec: 1, endSec: 2,
    ...
    test('a SETTLED run infers nothing — history has no speculation', () {
      final comp = _comp([_node('fetch', 'completed', startSec: 0, endSec: 1)], status: 'completed');
      expect(inferredRunningNodes(_graph, comp), isEmpty);
    });

    test('a live run with NO rows at all infers nothing (and must not crash) — the cold-open floor',
        () {
      // The page then renders the graph's stubs + an honest empty ledger: not blank, not authority.
      // 页面此时渲图占位 + 诚实空台账:不空白,也不装权威。
      expect(inferredRunningNodes(_graph, _comp(const [], status: 'running')), isEmpty);
    });
  });

  group('graphOfVersion', () {
    test('graphParsed wins; a bad blob is null so the caller says «no graph» honestly', () {
      final now = DateTime.utc(2026);
      final bad = WorkflowVersion(
          id: 'wfv_1', workflowId: 'wf_1', version: 1, graph: '{not json', createdAt: now, updatedAt: now);
      expect(graphOfVersion(bad), isNull);
      expect(graphOfVersion(null), isNull);
      final parsed = WorkflowVersion(
          id: 'wfv_1', workflowId: 'wf_1', version: 1, createdAt: now, updatedAt: now,
          graphParsed: _graph);
  ...
  group('flowrunChart — three-part bars follow DATA AVAILABILITY (§5.3)', () {
    test('⑫ stamps + ⑤ audit → queue + exec parts, positioned on a real time axis', () {
      final chart = flowrunChart(
        _graph,
        _comp([_node('analyze', 'completed', kind: 'agent', readySec: 0, startSec: 2, endSec: 9)]),
      ...
      expect(seg.at + seg.queueW + seg.w + seg.parkedW, lessThanOrEqualTo(1.0001));
    });

    test('a PRE-⑫ row degrades to a single-part bar — no invented queue leg', () {
      final chart = flowrunChart(_graph, _comp([_node('fetch', 'completed')]));
      final row = chart.rows.firstWhere((r) => r.nodeId == 'fetch');
      expect(row.segments.single.queueW, 0);
    });

    test('a still-parked node grows its amber leg toward NOW on a live surface', () {
      ...
      expect(gate.segments.single.parkedW, greaterThan(0.5),
          reason: '18m 的人等应占满轴的绝大部分');
      expect(chart.nowAt, isNotNull, reason: '活面有 now 线');
    });

    test('a SETTLED run gets no now-line (its axis is history, there is no «now» on it)', () {
      final chart = flowrunChart(
          _graph, _comp([_node('fetch', 'completed', readySec: 0, startSec: 0, endSec: 1)]),
          now: _t.add(const Duration(hours: 5)));
      expect(chart.nowAt, isNull);
    });

    test('a collapsed span falls back to equal slots AND says so via timeMode=false', () {
      ...
      final chart = flowrunChart(_graph, _comp([
    ...
    test('the speculative front claims a POSITION, never a measured duration', () {
      final now = _t.add(const Duration(seconds: 30));
      final chart = flowrunChart(
        _graph,
        _comp([
          _node('fetch', 'completed', readySec: 0, startSec: 0, endSec: 1),
          _node('gate', 'completed', kind: 'control', readySec: 1, startSec: 1, endSec: 2,
              result: const {'__port': 'high'}),
        ], status: 'running'),
        now: now,
