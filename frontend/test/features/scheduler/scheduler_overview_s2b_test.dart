...
Future<void> _pumpBoard(WidgetTester tester, StubSchedulerRepo repo) async {
  ...
  ); // count-ups settle; breath keeps looping
}
...
void _expectNotice(WidgetTester tester, String text, AnTone tone) {
  final container = ProviderScope.containerOf(
    tester.element(find.byType(SchedulerOverviewView)),
    listen: false,
  );
...
void main() {
  ...
  group('等你处理 waiting zone', () {
    testWidgets(
      'full inbox: head count, amber rows, countdown ONLY where a deadline exists, '
      ...
      (tester) async {
        ...
        for (final name in ['周报生成', '发布上线', 'wf_ghost']) {
          expect(
            find.ancestor(of: find.text(name), matching: find.byType(AnCard)),
            findsWidgets,
            reason: '审批卡带边框($name)',
          );
        }
        expect(
          find.textContaining('fr_park'),
          findsNothing,
          reason: '裸 id 清除(B1)',
        );
    ...
    testWidgets(
      'a single decision settles: verdict + trimmed reason ride :decide, the row slides '
      'out and the refetch removes it',
      (tester) async {
        final repo = _repo();
        await _pumpBoard(tester, repo);
    ...
    testWidgets(
      'losing first-wins (422) earns honest feedback and the row reconciles away',
      (tester) async {
        final repo = _repo();
        await _pumpBoard(tester, repo);
    ...
    testWidgets(
      'batch approve: hover checkbox selection → bar at ≥2 → SEQUENTIAL dispatch with '
      'per-row pending → summary feedback → rows settle',
      (tester) async {
        ...
        expect(repo.decideOrder, [
          'fr_park1/approve_send:yes',
          'fr_park2/approve_deploy:yes',
        ], reason: '按行序逐发');
    ...
    testWidgets(
      'batch approve over a half-lost race: «1 approved · 1 already handled»',
      (tester) async {
        final repo = _repo();
        await _pumpBoard(tester, repo);
        ...
        _expectNotice(
          tester,
          '${ov.sumApproved(n: '1')} · ${ov.sumLost(n: '1')}',
          AnTone.warn,
        );
        await g.removePointer();
      },
    ...
    testWidgets(
      'batch reject: the shared-reason strip; the reason rides ONLY allowReason rows',
      (tester) async {
        final repo = _repo();
        await _pumpBoard(tester, repo);
  ...
  group('正在跑 running zone (B10 大表行文法)', () {
    testWidgets(
      'the ⏹ Stop verb is HOVER-revealed, opens the danger confirm; cancel settles the row '
      'away (0718 宁静化: 动词安静待命)',
      (tester) async {
        final repo = _repo();
        await _pumpBoard(tester, repo);
        ...
        expect(
          _rowVerb('数据清洗流水线', t.scheduler.home.rowCancel),
          findsOneWidget,
          reason: 'hover → ⏹',
        );
    ...
    testWidgets(
      'batch cancel: ≥2 selected → danger dialog with the victim LIST → sequential '
      'cancel → summary feedback',
      (tester) async {
        final repo = _repo();
        await _pumpBoard(tester, repo);
        ...
        expect(
          find.textContaining('数据清洗流水线 · fr_live1'),
          findsOneWidget,
          reason: '弹窗带行清单',
        );
  ...
  group('24h 失败 failed zone (B10 补齐)', () {
    testWidgets(
      'the ↻ Retry verb is hover-revealed; a single replay confirms with REAL numbers and '
      'settles the row away (0718 宁静化)',
      (tester) async {
        ...
        // Resting: no verb (动词安静待命). 静息:零动词。
        expect(
          find.text(t.scheduler.home.rowRetry),
          findsNothing,
          reason: '静息行无 ↻',
        );
        ...
        await tester.pump(const Duration(milliseconds: 300)); // reveal
        expect(
          _rowVerb('数据清洗流水线', t.scheduler.home.rowRetry),
          findsOneWidget,
          reason: 'hover → ↻',
        );
    ...
    testWidgets(
      'multi-select batch replay: hover checkbox → bar at ≥2 → SEQUENTIAL dispatch → '
      'summary feedback',
      (tester) async {
        final repo = _failedRepo();
        await _pumpBoard(tester, repo);
        ...
        await tester.pump(
          const Duration(milliseconds: 300),
        ); // getRunFull ×2 + confirm
        expect(
          find.text(t.scheduler.home.batchReplayTitle(n: '2')),
          findsOneWidget,
        );
    ...
    testWidgets(
      'front-end pager: 12 failed runs → 10 on page 1, 2 on page 2 (drained list sliced '
