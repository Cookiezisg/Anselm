...
Graph get _graph => const Graph(
  ...
  edges: [
    Edge(id: 'e1', from: 'fetch', to: 'analyze'),
    Edge(id: 'e2', from: 'analyze', to: 'notify'),
  ],
);
...
void _expectNotice(WidgetTester tester, String text, AnTone tone) {
  final container = ProviderScope.containerOf(
    tester.element(find.byType(SchedulerHomeView)),
    listen: false,
  );
...
void main() {
  ...
  group('① 健康头', () {
    testWidgets(
      'the documentary head: crumb + big title + range-scoped stats sentence + capsule '
      '+ Run now — and NO bead strip (需求②③ 0717-晚)',
      (tester) async {
        await _pump(tester, _repo());
        expect(
          find.byType(AnOceanHeader),
          findsOneWidget,
          reason: '文档化页头(entities 同文法)',
        );
        expect(find.text('数据清洗流水线'), findsOneWidget);
        expect(find.text(t.scheduler.status.active), findsOneWidget);
        expect(
          find.byType(RunBeadStrip),
          findsNothing,
          reason: '珠串已删——矩阵列头就是同一排珠子',
        );
        // The sentence's window word IS the capsule's word (需求②:成功率跟随选择器,默认近 7 天).
        // 句子窗口词=胶囊之词。
        expect(
          find.text(h.statsLine(rate: '80%', avg: '42.0s')),
          findsOneWidget,
        );
        expect(
          find.byType(AnTimeRangePicker),
          findsOneWidget,
          reason: '胶囊坐在页头 meta 行',
        );
    ...
    testWidgets(
      'breadcrumb law (用户 0719): grey crumb = «Scheduler» (parent only, never the name), '
      'floating head = ONLY the workflow name (zero path)',
      (tester) async {
        await _pump(tester, _repo());
        ...
        final crumbs = tester
            .widget<AnOceanHeader>(find.byType(AnOceanHeader))
            .crumbs;
    ...
    testWidgets('null stats read «—», NEVER 0% (缺席≠零)', (tester) async {
      ...
      expect(find.text(h.statsLine(rate: '—', avg: '—')), findsOneWidget);
      expect(find.textContaining('0%'), findsNothing);
    });

    testWidgets(
      'the stats sentence FOLLOWS the capsule: picking 24h re-asks stats with since=24h '
      '(需求②:一颗胶囊、句子同窗)',
      (tester) async {
        final repo = _repo();
        await _pump(tester, repo);
        ...
        expect(
          repo.statsWindows.any((w) => w.since == '24h' && w.until == null),
          isTrue,
          reason: '句子的统计真按新窗重取(预设走时长文法)',
        );
        expect(
          find.textContaining(t.scheduler.range.h24),
          findsWidgets,
    ...
    testWidgets(
      'an ABSOLUTE range sends `until` on the stats wire — the app\'s ONE non-null-until '
      'emitter reaches the repo (需求②/后端 089060f2 的整个前端理由)',
      (tester) async {
        final repo = _repo();
        await _pump(tester, repo);
        repo.statsWindows.clear();
        final container = ProviderScope.containerOf(
          tester.element(find.byType(AnOceanHeader)),
        );
    ...
    testWidgets(
      'a range switch renders «—» while the new window\'s numbers are IN FLIGHT — the '
      ...
      (tester) async {
        ...
        expect(
          find.text(h.statsLine(rate: '—', avg: '—')),
          findsOneWidget,
          reason: '范围章不符=只render词不render数',
        );
        expect(
          find.text(h.statsLine(rate: '80%', avg: '42.0s')),
          findsNothing,
    ...
    testWidgets('Run now hits :trigger and reports the new run', (
      tester,
    ) async {
      final repo = _repo();
      await _pump(tester, repo);
    ...
    testWidgets(
      ':kill lives behind ⋯ + AnTypeToConfirm with the REAL blast radius',
      (tester) async {
        final repo = _repo();
        await _pump(tester, repo);
        ...
        expect(
          find.text(h.killWarning(n: '1')),
          findsOneWidget,
          reason: '影响面=真在途 run 数',
        );
  ...
  group('② run 大表', () {
    testWidgets(
      'rows speak «source · start instant»; NO bare id anywhere in a row (需求⑤⑦)',
      (tester) async {
        await _pump(tester, _repo());
        ...
        expect(
          find.textContaining(
            h.srcWithName(kind: h.srcWebhookBare, name: '/invoice'),
          ),
          findsOneWidget,
    ...
    testWidgets(
      'failed rows are single-line — the error moved off the row into the peek card '
      '(0718 宁静化: 错误句撤出行 → 速览卡)',
      (tester) async {
        ...
        await _pump(tester, _repo());
        expect(
          find.textContaining('HTTP 502 Bad Gateway'),
          findsNothing,
        ...
        expect(
          ...
          findsOneWidget,
          reason: '速览卡内 run.error 全文(不再只取首句)',
        );
      },
    );

    testWidgets(
      'the count strip carries TRUE numbers and each click IS the wire filter',
      (tester) async {
        final repo = _repo();
        await _pump(tester, repo);
        expect(
          find.text(h.filterRunning(n: '1')),
          findsOneWidget,
          reason: '在跑数来自 stats',
        );
        expect(
          find.text(h.filterFailed(n: '2')),
          findsOneWidget,
          reason: '失败数来自同文法探针',
        );
        expect(
          find.text(h.filterWaiting(n: '1')),
          findsOneWidget,
          reason: '等人数来自 inbox 派生',
        );
        ...
        expect(
          find.textContaining(h.srcChat),
          findsNothing,
          reason: '过滤后 completed 行退场',
        );
      },
    );

    testWidgets('«等人» intersects the inbox and NEVER sends ?status=parked', (
      tester,
    ) async {
      final repo = _repo();
      await _pump(tester, repo);
      ...
      await _settle(tester);
      expect(
        repo.listFilters.every((f) => f.status != 'parked'),
        isTrue,
    ...
    testWidgets(
      '«等人» filter is UNPAGED: the pager never renders even with >10 matches '
      '(复审 [1]/[5] 死钮修)',
      (tester) async {
        ...
        final repo = StubSchedulerRepo(
          ...
          byWorkflow: [
            WorkflowRunStats(workflowId: 'wf_a', running: 12, lastRunAt: _now),
          ],
          runs: runs,
          inbox: [
            for (var i = 0; i < 12; i++)
              stubInboxRow(
                'fr_wait${i.toString().padLeft(2, '0')}',
                'gate',
                now: _now,
              ),
          ],
    ...
    testWidgets(
      'the page-level time-range capsule governs the TABLE too (0717 拍板:一颗胶囊治两区)',
      (tester) async {
        final repo = _repo();
        await _pump(tester, repo);
        repo.listFilters.clear();
        ...
        await tester.pump();
        await _settle(tester);
        final asked = repo.listFilters
            .where((f) => f.startedAfter != null)
            .toList();
    ...
    testWidgets(
      'a stale in-flight page fetch NEVER clobbers a newer filter pick (复审 [2] 请求代号)',
      (tester) async {
        final runs = [
          for (var i = 0; i < 24; i++)
            Flowrun(
        ...
        final ctrl = ProviderScope.containerOf(
          tester.element(find.byType(AnPager)),
        ...
        await tester.pump();
        unawaited(
          ctrl.refetchTop(filter: RunStatusFilter.failed),
        ); // newer request B (failed·page1) 更新请求 B
        await tester.pump();
        ...
        final s = ProviderScope.containerOf(
          tester.element(find.byType(SchedulerHomeView)),
        ).read(schedulerRunTableProvider('wf_a')).value!;
    ...
    testWidgets(
      'page-number pager: 10/page, ‹/›/number/jump navigate, single page hides '
      '(WRK-070 B4)',
      (tester) async {
        final runs = [
          for (var i = 0; i < 24; i++)
        ...
        await tester.tap(
          find.descendant(of: find.byType(AnPager), matching: find.text('3')),
        );
        await tester.pump();
        await _settle(tester);
    ...
    testWidgets(
      'zone seams stack FLUSH — each AnSection carries its own 24 bottom, no doubled '
      'wrapper (0718 全模块对齐审计:段缝曾 48)',
      (tester) async {
        await _pump(tester, _repo());
        Rect zoneOf(String label) => tester.getRect(
          find
              .ancestor(of: find.text(label), matching: find.byType(AnSection))
              .first,
        );
    ...
    testWidgets(
      'the verb is HOVER-revealed, inline after the phrase (0718 宁静化: 动词安静待命)',
      (tester) async {
        await _pump(tester, _repo());
        ...
        expect(
          find.descendant(
            ...
            matching: find.text(h.rowCancel),
          ),
          findsOneWidget,
          reason: 'hover 正在跑行 → ⏹ 从原位滑出(行内,紧随短语)',
        );
    ...
    testWidgets('single replay: the confirm carries the REAL memoization numbers', (
      tester,
    ) async {
      final repo = _repo();
      await _pump(tester, repo);
      ...
      final failRow = find.ancestor(
        of: find.textContaining('/invoice'),
      ...
      expect(
        find.text(h.replayBody(failed: '1', completed: '1')),
        findsOneWidget,
      );
    ...
    testWidgets(
      'replay when the node history is unavailable → the numberless honest sentence',
      (tester) async {
    ...
    testWidgets(
      'batch replay: ≥2 selected → merged real numbers → SEQUENTIAL dispatch',
      (tester) async {
        final repo = _repo();
        ...
        expect(
          find.text(h.batchReplayBody(failed: '2', completed: '2')),
          findsOneWidget,
        );
    ...
    testWidgets(
      'batch cancel lives in the RUNNING face and lists its victims',
      (tester) async {
        final repo = _repo();
        await _pump(tester, repo);
        await tester.tap(find.text(h.filterFailed(n: '2')));
        ...
        await _settle(tester);
        expect(
          find.text(t.scheduler.overview.batchCancel),
          findsNothing,
          reason: '失败态没有取消语义',
        );
      },
  ...
  group('③ 新 run 不插行(活性军规)', () {
    ...
    ({ProviderContainer container, SchedulerRunTableController ctl}) live(
      StubSchedulerRepo repo,
    ) {
      ...
      return (
        container: container,
        ctl: container.read(schedulerRunTableProvider('wf_a').notifier),
      );
    }

    StreamEnvelope frame({
      required int seq,
      String wf = 'wf_a',
      ...
      frame: FrameSignal(
        node: StreamNode(
          type: type,
          content: const {'flowrunId': 'fr_new', 'origin': 'cron'},
        ),
      ),
    );
    ...
    test('an ephemeral tick (seq=0) never touches the table', () async {
      ...
      expect(
        container.read(schedulerRunTableProvider('wf_a')).value!.newRuns,
        0,
    ...
    testWidgets(
      'the pill renders its count and the tap pulls the new runs in',
      (tester) async {
        final repo = _repo();
        await _pump(tester, repo);
  ...
  group('④ 行内速览卡 (0717 拍板:?run= 即展开行)', () {
    testWidgets(
      'the card is an ordinary 720 section — no face may widen the page (用户 0717 判决)',
      (tester) async {
        await _pump(tester, _repo(), runId: 'fr_fail1');
        ...
        Future<void> expectWithinColumn(Finder face, String who) async {
          ...
          expect(
            tester.getSize(face).width,
            lessThanOrEqualTo(reading + 0.5),
    ...
    testWidgets(
      'collapsed without ?run=; expanded IN PLACE with it (gantt default, graph on '
      'toggle, the flagship door on board)',
      (tester) async {
        await _pump(tester, _repo());
        expect(
          find.byType(AnNodeGantt),
          findsNothing,
          reason: '无 ?run= 即无展开行——收起的行绝不建卡(C-006)',
        );
    ...
    testWidgets('a row tap TOGGLES ?run= in the URL — expand, and tap again to collapse', (
      ...
      final router = GoRouter(
        initialLocation: '/scheduler/w/wf_a?run=fr_fail1',
        routes: [
          GoRoute(
            path: '/scheduler/w/:id',
            builder: (_, st) {
              ...
              return Scaffold(
                body: SchedulerHomeView(
                  workflowId: 'wf_a',
                  linkedRunId: st.uri.queryParameters['run'],
                ),
              );
            },
          ),
      ...
      Future<void> tapChatRow() async {
        ...
        await tester.pump();
        await tester.tap(
          find.textContaining(h.srcChat).first,
          warnIfMissed: false,
        );
    ...
    testWidgets(
      'no active-version graph → the honest sentence, never an empty frame',
      (tester) async {
        final repo = _repo();
  ...
  group('⑤ triggers 陈列', () {
    testWidgets(
      'cron mono + schedule sentence + the paused card wears its chip',
      (tester) async {
        await _pump(tester, _repo());
        ...
        expect(
          find.textContaining('Entities'),
          findsNothing,
          reason: '编辑归属提示已删(B3)',
        );
        expect(
          find.byType(AnAutoGrid),
          findsOneWidget,
          reason: 'trigger 卡双列网格(B9)',
        );
      },
    );

    testWidgets(
      'pause: the dialog states the exact semantics, then the switch flips',
      (tester) async {
        final repo = _repo();
        await _pump(tester, repo);
        ...
        expect(
          find.text(h.pauseBody),
          findsOneWidget,
          reason: '「不再产生新 firing,在途 run 不受影响」',
        );
    ...
    testWidgets(
      'resume is idempotent + dialog-free (harmless) and un-greys the card',
      (tester) async {
        final repo = _repo();
        await _pump(tester, repo);
  ...
  group('⑥ 边角', () {
    testWidgets(
      'a deleted / unknown workflow deep link is an honest not-found',
      (tester) async {
        final repo = StubSchedulerRepo(
          workflows: [
            SchedulerWorkflowRow(
  ...
  group('⑤ 页顶矩阵区', () {
    ...
    testWidgets(
      'the matrix is RESIDENT: opening the page pages the runs and batch-fetches the grid '
      'by EXPLICIT ids (惰性律反转,0717 拍板)',
      (tester) async {
        ...
        expect(
          repo.matrixAsks.first,
          containsAll(['fr_live1', 'fr_fail1']),
    ...
    testWidgets(
      'the grid renders: ×N in the cell, «未及» for a sparse one, CHRONOLOGICAL display '
      '(旧在左)',
      (tester) async {
        final repo = _repo()..matrixGrid = grid();
        await _pump(tester, repo);
        ...
        expect(
          find.text(h.matrixView),
          findsOneWidget,
          reason: '段题=矩阵视图(B3 方案 A)',
        );
        expect(
          find.text(h.matrixTitle.toUpperCase()),
          findsNothing,
          reason: '旧 NODE × RUN 段标题仍删(需求②)',
        );
        expect(find.text('3'), findsOneWidget, reason: 'iterations=3 → ×N 在格里');
        expect(
          find.byTooltip(h.matrixNotReached),
          findsOneWidget,
        ...
        final older = tester.getTopLeft(
          find
              .descendant(
                of: grid_,
                matching: find.bySemanticsLabel(RegExp('failed')),
              )
              .first,
        );
    ...
    testWidgets(
      'the grid takes its width from ITSELF, never from the page (用户 0717 判决)',
      (tester) async {
        final repo = _repo()..matrixGrid = grid();
        await _pump(tester, repo);

        expect(
          tester.getSize(find.byType(AnRunMatrix)).width,
    ...
    testWidgets(
      'an empty window says so; a failed read offers retry — neither blanks the zone',
      (tester) async {
        // The default stub grid is EMPTY → the zone answers with the honest sentence, never a bare
        // frame. 默认剧本格阵空 → 空窗句,绝不空框。
        await _pump(tester, _repo());
        expect(
          find.text(h.matrixEmpty),
          findsOneWidget,
          reason: '空窗是答案(这段时间没有运行)',
        );
        ...
        await _settle(tester);
        expect(
          find.text(h.paneError),
          findsWidgets,
          reason: '取数失败诚实报错 + 重试,绝不空白',
        );
      },
    );
      ...
      final router = GoRouter(
        routes: [
          GoRoute(
            path: '/',
            builder: (_, _) =>
                Scaffold(body: SchedulerHomeView(workflowId: 'wf_a')),
          ),
          GoRoute(
  ...
  group('⑥ 墓碑句已删(WRK-070 B3)', () {
    testWidgets(
      'the end of the history carries NO retention tombstone sentence',
      (tester) async {
        final repo = _repo();
        await _pump(tester, repo);
        await tester.tap(find.text(h.filterFailed(n: '2')));
