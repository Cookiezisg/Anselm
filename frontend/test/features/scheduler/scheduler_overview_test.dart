...
EntityRelation _edge(String wf, String trigger, {String wfName = ''}) =>
    EntityRelation(
      id: 'rel_$wf$trigger',
      kind: 'equip',
      fromKind: 'workflow',
...
StubSchedulerRepo _fullRepo({
  Map<String, int> failed = const {'24h': 4, '48h': 6},
}) => StubSchedulerRepo(
  workflows: [
  ...
  triggers: [
    _trigger(
      'tr_1',
      '每日 09:00',
      nextFireAt: _now.add(const Duration(minutes: 3, seconds: 30)),
    ),
  ],
  ...
  runs: [
    ...
    for (var i = 1; i <= 4; i++)
      Flowrun(
        ...
        status: 'failed',
        error:
            'HTTP 502 Bad Gateway: upstream did not respond\nretried 3 times',
...
Future<void> _pumpBoard(WidgetTester tester, Widget host) async {
  ...
  ); // count-ups settle (≤900ms); breath keeps looping
}
...
void main() {
  group('pure derivations 纯派生', () {
    ...
    test(
      'earliestNextFire: earliest FUTURE wins, past ignored, none → null',
      () {
        ...
        expect(
          earliestNextFire([now.subtract(const Duration(days: 1))], now),
          isNull,
        );
    ...
    test(
      'scheduleLanes: the lane set comes from the TRIGGER LIST — a PAUSED trigger keeps its lane '
      ...
      () {
        ...
        expect(
          [for (final l in lanes) l.triggerId],
          ['tr_now', 'tr_soon', 'tr_soon', 'tr_off'],
          reason: '最近的在前;没有将至之事的泳道(暂停)沉底——**沉底不是消失**',
        );
        expect(
          lanes.where((l) => l.triggerId == 'tr_hook'),
          isEmpty,
          reason: 'webhook 下次 fire 不可知,如实缺席而非在场且空',
        );
        expect(
          lanes.where((l) => l.triggerId == 'tr_orphan'),
          isEmpty,
          reason: '无边=不产 run,无泳道',
        );
        ...
        expect(lanes[1].futureAt, [
          now.add(const Duration(hours: 2)),
        ], reason: '窗外的点被丢弃,窗内的留下');
        expect(lanes.first.workflowName, '清洗');
        expect(
          lanes.firstWhere((l) => l.workflowId == 'wf_2').workflowName,
          'edge-name',
    ...
    test(
      'scheduleLanes: a point never lights a workflow the listener table did not resolve',
      () {
        final now = DateTime(2026, 7, 16, 9);
        final lanes = scheduleLanes(
        ...
        expect(
          lanes.firstWhere((l) => l.workflowId == 'wf_1').futureAt,
          hasLength(1),
        );
        expect(
          lanes.firstWhere((l) => l.workflowId == 'wf_2').futureAt,
          isEmpty,
    ...
    test('topFailing: streak-DESC, zero excluded, capped at 5', () {
      final stats = [
        for (var i = 0; i < 8; i++)
          WorkflowRunStats(workflowId: 'wf_$i', consecutiveFailures: i),
      ];
  ...
  group('widget battery 电池', () {
    testWidgets(
      'resting rhythm: plain head→first row = 12 (塌缩批量条不吃额外 gap), last row→zone '
      'bottom = 24 (单页空翻页器不占席) — 0718 全模块对齐审计 + 宁静化 plain 段题',
      (tester) async {
        ...
        final section = tester.getRect(
          find.ancestor(of: capFinder, matching: find.byType(AnSection)).first,
        );
        ...
        expect(
          row.top - caption.bottom,
          moreOrLessEquals(12, epsilon: 0.6),
          reason: '题→首行=12(plain headBody):塌缩批量条合进体列,不再被 AnSection 双夹',
        );
        expect(
          section.bottom - row.bottom,
          moreOrLessEquals(24, epsilon: 0.6),
          reason: '末行→区底=24:单页 SizedBox.shrink 翻页器不再占一个带距子席(曾 36)',
        );
      },
    );

    testWidgets('full board: KPI strip + running row + upcoming row + failure row', (
      tester,
    ) async {
      await _pumpBoard(tester, _host(_fullRepo()));
      final ov = t.scheduler.overview;
      ...
      expect(
        find.text(ov.failuresSegmentHead),
        findsOneWidget,
        reason: '失败段大字段题',
      );
      expect(
        find.text(ov.failed24hHead),
        findsOneWidget,
        reason: '24h 小节(工单⑮):牌点开的四行,quiet 小标',
      );
      expect(
        find.text(ov.failuresHead),
        findsOneWidget,
        reason: '7d 连败小节 quiet 小标',
      );
      ...
      expect(
        find.text(t.scheduler.home.rowCancel),
        findsNothing,
      ...
      expect(
        find.textContaining('数据清洗流水线 · '),
        findsOneWidget,
        reason: '正在跑行=workflow · 来源短语(B10)',
      );
      expect(
        find.text(ov.fireIn(d: '3m')),
        findsOneWidget,
        reason: 'KPI 下次调度牌',
      );
      ...
      expect(
        find.bySemanticsLabel(ov.kpiNextFireA11y(d: '3m')),
        findsOneWidget,
      ...
      expect(
        find.text('HTTP 502 Bad Gateway: upstream did not respond'),
        findsNothing,
        reason: '错误句撤出行,进速览卡(收起态不建);行单行化,红点=唯一警报',
      );
      expect(
        find.text(ov.openWorkflow),
        findsOneWidget,
        reason: '连败行 CTA=「打开 workflow →」',
      );
    ...
    testWidgets(
      '段序 (0718 宁静化 — 用户逐条拍板): schedule 打头 → 等你处理 → 正在跑 → 失败段, '
      'four plain segment heads top-down; the KPI strip stays above them all (牌不是段)',
      (tester) async {
        await _pumpBoard(tester, _host(_fullRepo()));
        final ov = t.scheduler.overview;
        ...
        expect(
          topOf(ov.kpiRunning),
          lessThan(topOf(ov.scheduleHead)),
          reason: '牌不是段,恒在段之上',
        );
        // The four plain segments, in the拍板 order. 四段 plain 顺序。
        expect(
          topOf(ov.scheduleHead),
          lessThan(topOf(ov.waitingHead)),
          reason: '调度时间轴打头',
        );
        expect(
          topOf(ov.waitingHead),
          lessThan(topOf(ov.runningHead)),
          reason: '等你处理在正在跑之上',
        );
        expect(
          topOf(ov.runningHead),
          lessThan(topOf(ov.failuresSegmentHead)),
          reason: '失败段殿后',
        );
        // Inside the failure segment: 24h subsection ABOVE the 7d subsection (同段两小节,24h 在上).
        expect(
          topOf(ov.failuresSegmentHead),
          lessThan(topOf(ov.failed24hHead)),
          reason: '段题在小节之上',
        );
        expect(
          topOf(ov.failed24hHead),
          lessThan(topOf(ov.failuresHead)),
          reason: '24h 小节在 7d 连败小节之上',
        );
      },
    );

    testWidgets('quiet workspace: three honest empty sentences, KPI dashes', (
      tester,
    ) async {
      ...
      expect(
        find.text(ov.kpiNone),
        findsOneWidget,
        reason: '无未来 fire → 下次调度牌 —',
      );
    ...
    testWidgets(
      'KPI delta both ways: improving reads ▼ green; flat hides the arrow',
      (tester) async {
        await _pumpBoard(
          tester,
          _host(_fullRepo(failed: const {'24h': 1, '48h': 4})),
        );
        final ov = t.scheduler.overview;
        ...
        await _pumpBoard(
          tester,
          _host(_fullRepo(failed: const {'24h': 3, '48h': 6})),
        );
        expect(find.textContaining('▲'), findsNothing);
        expect(find.textContaining('▼'), findsNothing);
      },
    ...
    testWidgets(
      'zero data: the whole page is ONE education card with both deep links',
      (tester) async {
        ...
        expect(
          find.byType(AnSection),
          findsNothing,
          reason: '教育卡替代全部区块,不渲空框废墟',
        );
    ...
    testWidgets(
      'the documentary head: grey crumb over the big title (WRK-070 B11 标准文法)',
      (tester) async {
        await _pumpBoard(tester, _host(_fullRepo()));
        expect(find.byType(AnOceanHeader), findsOneWidget);
        expect(
          find.text(t.scheduler.home.crumbRoot),
          findsOneWidget,
          reason: '灰 crumb「调度」',
        );
        expect(
          find.text(t.scheduler.overviewTitle),
          findsOneWidget,
          reason: '大标题 Overview',
        );
      },
    ...
    testWidgets(
      'a running row double-taps straight into the run flagship (B10:单击展开、双击进子页)',
      (tester) async {
  ...
  group('S5 · 调度时间轴', () {
    testWidgets(
      'the track replaces the row list: a lane per (workflow × cron), points from ⑧',
      (tester) async {
        final repo = _fullRepo();
        await _pumpBoard(tester, _host(repo));
        expect(find.byType(AnScheduleTrack), findsOneWidget);
        expect(
          repo.scheduleWithins,
          ['24h'],
    ...
    testWidgets('a PAUSED trigger keeps a greyed lane with «已暂停» — 判决①', (
      tester,
    ) async {
      final repo = StubSchedulerRepo(
        workflows: [
      ...
      expect(
        find.text(t.scheduler.home.paused),
        findsOneWidget,
        reason: '灰显必须配词:色不独行',
      );
    });
    ...
    testWidgets('an uncapped window says nothing about truncation', (
      tester,
    ) async {
      await _pumpBoard(tester, _host(_fullRepo()));
    ...
    testWidgets(
      'truncated says so out loud — a capped window must not read as the whole truth',
      (tester) async {
        ...
        expect(
          find.text(t.scheduler.overview.trackTruncated),
          findsOneWidget,
    ...
    testWidgets(
      'no cron lanes at all → the honest empty sentence, not an empty axis',
      (tester) async {
        final repo = StubSchedulerRepo(
          workflows: [
            SchedulerWorkflowRow(
        ...
        expect(
          find.byType(AnScheduleTrack),
          findsNothing,
          reason: '零泳道不画一条指着空无的轴',
        );
      },
    );
  });
  ...
  group('B5 · 发射台', () {
    ...
    DateTime hourCentre(int hoursAgo) {
      ...
      return DateTime(
        ...
        l.day,
        l.hour,
      ).subtract(Duration(hours: hoursAgo)).add(const Duration(minutes: 30));
    }
    ...
    Future<String> launch(
      ...
      final router = GoRouter(
        ...
        routes: [
          ...
          GoRoute(
            ...
            builder: (_, s) {
              landed =
                  '/scheduler/w/${s.pathParameters['id']}/runs/${s.pathParameters['frId']}';
              return const Scaffold(body: SizedBox());
            },
          ),
        ],
      ...
      ); // → the cursor (the NEWEST bin, 24) 光标=最新格
      await tester.pump();
      ...
      final track = ScheduleTrackData(
        lanes: [
          ScheduleLane(
            ...
            workflowName: 'A',
            paused: false,
            runs: [run('fr_solo', hoursAgo: 6)],
          ), // → bin 18 (25 whole-hour bins 整点 25 格)
        ],
      );
      expect(
        await launch(tester, track, bin: 18),
        '/scheduler/w/wf_a/runs/fr_solo',
        reason: '一格一 run → 直进该 run 旗舰',
      );
    });

    testWidgets(
      'a MANY-run hour opens the workflow operations home (no single flagship to pick)',
      (tester) async {
        final track = ScheduleTrackData(
          lanes: [
            ScheduleLane(
  ...
  group('判决⑥ 过去的点与错过的 ✕', () {
    ...
    group('scheduleLanes 纯派生', () {
      test(
        'past firings hang on their (trigger × workflow) lane; missed keeps its own identity',
        () {
          final lanes = scheduleLanes(
          ...
          expect(lanes.single.firings.map((e) => e.id), [
            'trf_2',
            'trf_1',
          ], reason: '旧→新:轴从左往右读');
      ...
      test(
        'a firing older than the past window is off the axis and dropped',
        () {
          final lanes = scheduleLanes(
          ...
          expect(
            lanes.single.firings,
            isEmpty,
            reason: '26h 前 > 25h 取数窗(整点 25 格):轴上无处可站',
          );
        },
      );
      ...
      test(
        'an ORPHANED missed tick still gets a lane — the card counts it, so it must be shown',
        () {
          final lanes = scheduleLanes(
            triggers: const [],
            edges: const [],
      ...
      test(
        'an orphaned NON-missed firing is dropped — nothing counts it, so it is only context',
        () {
          final lanes = scheduleLanes(
            triggers: const [],
            edges: const [],
      ...
      test(
        'a PAUSED lane still carries the fires it made before it was paused (判决①)',
        () {
          final lanes = scheduleLanes(
      ...
      test(
        'the missed window floor is the KPI window — equal by construction, not by coincidence',
        () {
          // 25 whole-hour bins ⊇ any rolling 24h KPI window (v2 整点分箱下 ✕ 完整性不变式的新形态)。
          expect(SchedulerWindows.trackBinCount, 25);
          expect(
            SchedulerWindows.trackFetchWindow,
    ...
    group('错过 KPI 牌', () {
      ...
      testWidgets(
        '「错过 0」 is NOT a tile — the absence of the card IS the good news',
        (tester) async {
          await _pumpBoard(tester, _host(missedRepo(missedCount: 0)));
          expect(
            find.text(t.scheduler.overview.kpiMissed),
            findsNothing,
      ...
      testWidgets(
        'missed > 0 → the fifth tile appears carrying the backend count',
        (tester) async {
      ...
      testWidgets(
        'the tile OPENS its evidence — the schedule track washes on tap',
        (tester) async {
          await _pumpBoard(tester, _host(missedRepo()));
          expect(
            find.byType(AnWashHighlight),
            findsNothing,
            reason: '未点击前不洗:注意力只随用户动作而动',
          );
          ...
          expect(
            find.descendant(
              of: find.byType(AnWashHighlight),
              matching: find.byType(AnScheduleTrack),
            ),
            findsOneWidget,
          );
        },
      ...
      testWidgets(
        'SAME PREDICATE: the missed page asks the byte-identical instant the card counted from',
        (tester) async {
          ...
          final absolutes = repo.statsSinces
              .where((s) => DateTime.tryParse(s) != null)
              .toList();
          expect(
            absolutes,
            hasLength(2),
            reason: 'Overview 发两次绝对窗:24h(牌)+ 48h(delta 探针)',
          );
      ...
      testWidgets(
        'the marks land on the track: a ✕ per missed tick, solid dots for real fires',
        (tester) async {
          await _pumpBoard(tester, _host(missedRepo()));
          expect(
            find.descendant(
              of: find.byType(AnScheduleTrack),
      ...
      testWidgets(
        'a capped firing page SAYS the older end is unknown — never a silent hole',
        (tester) async {
            ...
            firings: [
              for (var i = 0; i < 400; i++)
                f(
                  ...
                  'wf_a',
                  FiringStatus.started,
                  Duration(minutes: i * 3),
                ),
            ],
          );
          await _pumpBoard(tester, _host(repo));
          expect(
            find.textContaining(RegExp('未显示|not shown')),
            findsOneWidget,
      ...
      testWidgets('a firing read that FAILS never becomes a reassuring zero', (
        tester,
      ) async {
        final base = missedRepo();
        final repo = StubSchedulerRepo(
        ...
        await _pumpBoard(tester, _host(repo));
        expect(
          find.text(t.scheduler.overview.errorTitle),
          findsOneWidget,
  ...
  group('A · 海洋默认页', () {
    Widget oceanHost(SchedulerRepository repo, SchedulerSelection? selection) =>
        ProviderScope(
          overrides: [
            sseGatewayProvider.overrideWithValue(null),
            schedulerRepositoryProvider.overrideWithValue(repo),
            selectedSchedulerProvider.overrideWith(
              () => _PinnedScheduler(selection),
            ),
          ],
          child: TranslationProvider(
            child: MaterialApp(
              theme: AnTheme.light(),
              home: const Scaffold(body: SchedulerOcean()),
            ),
          ),
        );

    for (final (label, sel) in [
      ('/scheduler (overview)', const SchedulerOverview()),
      ('null', null),
    ]) {
      testWidgets('$label → the Overview board (full workspace)', (
        tester,
      ) async {
        ...
        await tester.pump(
          const Duration(seconds: 1),
        ); // count-ups settle; breath keeps looping 数字落定
        expect(
          find.byType(SchedulerOverviewView),
          findsOneWidget,
    ...
    testWidgets(
      'null selection with ZERO workflows still shows the Overview (its first-use card), not blank',
      (tester) async {
        ...
        expect(
          find.byType(SchedulerOverviewView),
          findsOneWidget,
          reason: '零 workflow 也进 Overview、非空墓碑',
        );
        expect(
          find.text(t.scheduler.overview.firstUseTitle),
          findsOneWidget,
          reason: '零 workflow=教育卡(仍是 Overview 的一部分)',
        );
      },
    );
  });
