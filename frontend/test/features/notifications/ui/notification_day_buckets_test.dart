...
void main() {
  ...
  test('bucketOf classifies a LOCAL createdAt into today / yesterday / earlier', () {
    expect(days.bucketOf(DateTime(2026, 7, 20, 9)), 0); // today
    expect(days.bucketOf(DateTime(2026, 7, 20, 0, 0)), 0); // today midnight (inclusive floor)
    expect(days.bucketOf(DateTime(2026, 7, 19, 10)), 1); // yesterday
    expect(days.bucketOf(DateTime(2026, 7, 19, 0, 0)), 1); // yesterday midnight (inclusive floor)
    expect(days.bucketOf(DateTime(2026, 7, 19, 23, 59)), 1); // last moment of yesterday
    expect(days.bucketOf(DateTime(2026, 7, 18, 23, 59)), 2); // earlier
  });

  test('windowOf maps each bucket to its half-open [after, before) UTC window', () {
  ...
  test('INVARIANT: bucketOf(d) agrees with windowOf — d lands in exactly its own bucket window', () {
    ...
    final cases = <DateTime>[
      ...
      DateTime(2026, 7, 19, 23, 59, 59), // yesterday, just under today's floor
      DateTime(2026, 7, 19, 12), // yesterday
      DateTime(2026, 7, 19, 0, 0), // yesterday floor (inclusive)
      DateTime(2026, 7, 18, 23, 59, 59), // earlier, just under yesterday's floor
    ...
    for (final d in cases) {
      ...
      for (final other in const [0, 1, 2]) {
        expect(days.windowOf(other).contains(d), other == b,
            reason: 'instant $d is bucket $b; windowOf($other).contains must be ${other == b}');
      }
    }
  });

  test('the today floor is INCLUSIVE while the yesterday ceiling is EXCLUSIVE (half-open, no gap/overlap)', () {
    final todayMidnight = DateTime(2026, 7, 20); // exactly today's floor / yesterday's ceiling
    expect(days.bucketOf(todayMidnight), 0); // classified as today
    expect(days.windowOf(0).contains(todayMidnight), isTrue); // today window includes its floor
    expect(days.windowOf(1).contains(todayMidnight), isFalse); // yesterday window excludes its ceiling
  });
}
