...
void main() {
  ...
  testWidgets('unread row renders the composed message + relative time', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(
        NotificationRow(
          item: _n('function.created', payload: {'name': 'fetch_orders'}),
          now: _now,
        ),
      ),
    );
      ...
      await tester.pumpWidget(
        _host(
          NotificationRow(
            item: _n(
              'function.created',
              payload: {'name': '${'x_' * 60}<script>alert(1)</script>'},
            ),
            now: _now,
          ),
        ),
  ...
  testWidgets(
    '已读律: no leading unread dot — the kind icon is the sole lead (未读点退役)',
    (tester) async {
      ...
      await tester.pumpWidget(
        _host(
          NotificationRow(
            item: _n('function.created', payload: {'name': 'fetch'}),
            now: _now,
          ),
        ),
      );
      ...
      expect(
        find.byType(Icon),
        findsWidgets,
      ); // the kind/tone icon is present as the lead
    },
  );

  testWidgets('nameless row renders without a dangling object', (tester) async {
    await tester.pumpWidget(
      _host(NotificationRow(item: _n('agent.created'), now: _now)),
    );
