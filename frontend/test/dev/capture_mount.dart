...
// Expands each get card to show its four-part exhibit → out/mount.png. F06 get 卡截图夹具(非门禁)。
import 'dart:io';
...
import 'package:anselm/dev/gallery/tool_card_mount_specimens.dart';
...
void main() {
  ...
  testWidgets('capture bash', (tester) async {
          ...
          home: Scaffold(
            body: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  for (final s in toolCardMountGalleryItem.specimens) ...[
                    Builder(
    ...
    for (final f in [find.text('linear/create_issue'), find.text('billing_webhook.charge()')]) {
    ...
    File('${dir.path}/mount.png').writeAsBytesSync(bytes);
  });
}
