...
// Expands each get card to show its four-part exhibit → out/conversation.png. F06 get 卡截图夹具(非门禁)。
import 'dart:io';
...
import 'package:anselm/dev/gallery/tool_card_conversation_specimens.dart';
...
void main() {
  ...
  testWidgets('capture conversation', (tester) async {
          ...
          home: Scaffold(
            body: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  for (final s in toolCardConversationGalleryItem.specimens) ...[
                    Builder(
    ...
    for (final v in ['已重命名对话', '已归档对话', '已列出对话', '已搜索对话']) {
      await tester.tap(find.text(v), warnIfMissed: false);
    ...
    File('${dir.path}/conversation.png').writeAsBytesSync(bytes);
  });
}
