// Dev screenshot harness for ToolHitList (B3.2) — NOT part of the gate. Run:
//   flutter test test/dev/capture_hit_list.dart
// Renders the hit-list states → out/hit_list.png. Pumped past the cascade so rows are settled.
// ToolHitList 截图夹具(非门禁)。
import 'dart:io';
...
import 'package:anselm/dev/gallery/tool_hit_list_specimens.dart';
...
void main() {
  ...
  testWidgets('capture hit-list states', (tester) async {
          ...
          home: Scaffold(
            body: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  for (final s in toolHitListGalleryItem.specimens) ...[
                    Builder(
    ...
    // Pump past the cascade so the animated rows settle to full opacity. 播完级联到终态。
    ...
    File('${dir.path}/hit_list.png').writeAsBytesSync(bytes);
  });
}
