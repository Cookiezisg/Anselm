...
// Expands each get card to show its four-part exhibit → out/lifecycle.png. F06 get 卡截图夹具(非门禁)。
import 'dart:io';
...
import 'package:anselm/dev/gallery/tool_card_lifecycle_specimens.dart';
...
void main() {
  ...
  testWidgets('capture lifecycle', (tester) async {
          ...
          home: Scaffold(
            body: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  for (final s in toolCardLifecycleGalleryItem.specimens) ...[
                    Builder(
    ...
    for (final v in ['已回退函数', '已删除函数', '已删除智能体', '已改名', '已上线']) {
    ...
    File('${dir.path}/lifecycle.png').writeAsBytesSync(bytes);
  });
}
