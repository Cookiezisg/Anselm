...
void main() {
  ...
  test('fresh install mints a 256-bit key, stores it, read-back verifies', () async {
    ...
    expect(key!.length, greaterThanOrEqualTo(43), reason: 'base64url(32B) ≥ 43 chars');
    // Second resolve returns the SAME key (stable across relaunches). 二次解析同钥。
    expect(await mk.resolve(), key);
  });

  test('legacy install (db exists, no entry) keeps the fingerprint path — never mints', () async {
    var wrote = false;
    final mk = MasterKey(
      read: (_) async => null,
      write: (_, _) async => wrote = true,
