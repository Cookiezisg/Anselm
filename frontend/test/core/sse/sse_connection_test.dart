...
class _SseAdapter implements HttpClientAdapter {
  ...
  final List<ResponseBody Function()>
...
ResponseBody _hang() =>
    ResponseBody(StreamController<Uint8List>().stream, 200, headers: _hdr);
...
void main() {
  test(
    'durable frame emits, advances cursor, reconnect carries Last-Event-ID + bearer',
    () async {
  ...
  test(
    '410 → resync fires, cursor drops, next connect has NO Last-Event-ID',
    () async {
  ...
  test(
    'backoff: full-jitter base doubles 500→10s cap (max-jitter reveals the base)',
    () {
      final adapter = _SseAdapter([_hang]);
