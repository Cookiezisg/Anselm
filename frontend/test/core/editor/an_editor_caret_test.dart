...
void main() {
  ...
  group(
    'anCaretBand — THE one caret-height rule (shared by the document layer and AnFieldCaret)',
    () {
      test('sits on the MEASURED glyph box whenever there is one', () {
        ...
        expect(
          ...
          103.2,
          reason:
              'the glyph box IS the caret band — never re-centred by a formula',
        );
        expect(band.height, closeTo(18.1, 0.001));
      });

      test(
        'with nothing to measure (empty text) falls back to the house formula, centred in the line box',
        () {
          final band = anCaretBand(
            glyph: null,
            lineTop: 100,
          ...
          expect(
            band.height,
            AnText.reading.fontSize! + AnSize.caretRise,
            reason: '15+3',
          );
      ...
      test(
        'the fallback never exceeds the line box, and scales with the style',
        () {
          expect(
            anCaretBand(
              glyph: null,
  ...
  testWidgets(
    'the code block caret is INK, [AnSize.caret] wide, and derived from the code style',
    (tester) async {
      ...
      try {
        ...
        expect(
          ...
          AnColors.light.ink,
          reason:
              'a caret is INK — the accent caret turned the document blue mid-prose',
        );
        expect(field.cursorWidth, AnSize.caret);
        expect(
          field.cursorHeight,
          AnText.codeReading.fontSize! + AnSize.caretRise,
          reason:
              'derived from the code style (16), NOT the platform line box (macOS gave 22.8)',
        );
      } finally {
  ...
  testWidgets('a table cell hides the package caret and paints the An one', (
    tester,
  ) async {
    debugDefaultTargetPlatformOverride = TargetPlatform.macOS;
    try {
      ...
      for (final field in tester.widgetList<SuperTextField>(
        find.byType(SuperTextField),
      )) {
        expect(
          field.caretStyle?.width,
          0,
          reason:
              'hidden by zero width — a transparent colour would be resurrected by the blink alpha',
        );
      }
      expect(
        find.byType(AnFieldCaret),
        findsNWidgets(4),
        reason: 'one An caret per cell',
      );
      final caret = tester.widget<AnFieldCaret>(
        find.byType(AnFieldCaret).first,
      );
  ...
  testWidgets(
    'the caret BESIDE an atomic block spans the WHOLE block (upstream behaviour, kept)',
    (tester) async {
      ...
      try {
        await tester.pumpWidget(
          _host('prose paragraph\n\n| A | B |\n| --- | --- |\n| 1 | 2 |'),
        );
        await tester.pumpAndSettle();

        await tester.tapAt(
          tester.getTopLeft(find.byType(AnEditor)) + const Offset(30, 10),
        );
        await tester.pumpAndSettle();
        await tester
            .pressDownArrow(); // prose → the table block's edge 正文 → 表格块边
        await tester.pumpAndSettle();

        final selection = SuperEditorInspector.findDocumentSelection();
        expect(
          selection?.extent.nodePosition,
          isA<UpstreamDownstreamNodePosition>(),
          reason: 'the arrow landed on the atomic block, as upstream intends',
        );
        final table = find.byType(AnEditableTable);
        expect(
          tester.getSize(find.byKey(DocumentKeys.caret)).height,
  ...
  testWidgets('the cell paints ONE caret and it is the An band — not the package bar', (
    tester,
  ) async {
    ...
    try {
      ...
      tester.widget<SuperTextField>(cell).textController!.selection =
      ...
      await tester.runAsync(() async {
        ...
        expect(
          ...
          lessThan(21),
          reason:
              'the painted caret is the glyph band (~18), not the package line box (24)',
        );
        expect(
          painted,
          greaterThan(15),
          reason: 'and it is really there, not a sliver',
        );
      });
    } finally {
      debugDefaultTargetPlatformOverride = null;
  ...
  group('ONE caret at a time', () {
    testWidgets('clicking into the code block drops the document caret', (
      tester,
    ) async {
      debugDefaultTargetPlatformOverride = TargetPlatform.macOS;
      try {
        ...
        expect(
          ...
          isNull,
          reason:
              'the document must drop its selection when an embedded field owns the keyboard',
        );
        expect(
          find.byKey(DocumentKeys.caret),
          findsNothing,
          reason: 'no second caret blinking',
        );
      } finally {
        debugDefaultTargetPlatformOverride = null;
      }
    ...
    testWidgets('clicking into a table cell drops the document caret', (
      tester,
    ) async {
      debugDefaultTargetPlatformOverride = TargetPlatform.macOS;
      try {
        ...
        expect(
          ...
          findsNothing,
          reason:
              'the block-edge caret (as tall as the whole table — 72px measured) must not linger',
        );
      } finally {
        debugDefaultTargetPlatformOverride = null;
      }
