...
const _nbsp =
    ' '; // U+00A0 — the inline-code padding spacer the codec injects on load
...
List<String> _codeRuns(TextNode n) {
  ...
  final spans = text.getAttributionSpans({codeAttribution}).toList()
    ..sort((a, b) => a.start.compareTo(b.start));
  return [
    for (final s in spans) text.copyText(s.start, s.end + 1).toPlainText(),
  ];
}
...
MutableDocument _docWithMention() => MutableDocument(
  nodes: [
    ParagraphNode(
      id: 'p1',
...
void main() {
  group('E9a mention round-trip', () {
    test('a mention pill serializes to the [[id]] wikilink', () {
      ...
      expect(
        md.contains('[[$_id]]'),
        isTrue,
        reason: 'pill → [[id]] wire form',
      );
      expect(
        md.contains('每日销量对账'),
        isFalse,
        reason: 'the display name is NOT persisted (only the id)',
      );
      expect(
        md.contains('￼'),
        isFalse,
        reason: 'no raw placeholder character leaks into the markdown',
      );
    });

    test(
      'a [[id]] wikilink deserializes to a mention pill (name resolved, kind from prefix)',
      () {
        ...
        final mentions = node.text.placeholders.values
            .whereType<MentionPlaceholder>()
            .toList();
    ...
    test('an UNKNOWN id falls back to the bare id as the display name', () {
      ...
      final m = (doc.getNodeAt(0)! as ParagraphNode).text.placeholders.values
          .whereType<MentionPlaceholder>()
          .first;
      expect(m.name, _id);
    });
    ...
    test('full round-trip preserves the [[id]] verbatim and is idempotent', () {
      ...
      expect(
        md2.contains('[[$_id]]'),
        isTrue,
        reason: 'the id survives the whole trip verbatim',
      );
    });
  ...
  group('E9a block + inline types', () {
    ...
    test('a fenced code block KEEPS its language tag (```dart)', () {
      ...
      expect(
        md.contains('```dart'),
        isTrue,
        reason: 'the language tag is part of the wire form',
      );
      expect(md.contains('final x = 1;'), isTrue);
    });

    test(
      'a fenced code block LOADS as an embedded CodeBlockNode (not a paragraph), code + lang stamped',
      () {
        const source = '```dart\nfinal x = 1;\nfinal y = 2;\n```';
        final doc = documentFromMarkdown(source);
        ...
        expect(
          ...
          hasLength(1),
          reason:
              'code is an atomic block node, the substrate that gives a gutter',
        );
        expect(
          blocks.single.code,
          'final x = 1;\nfinal y = 2;',
          reason: 'multiline code verbatim, no trailing \\n',
        );
        expect(
          blocks.single.language,
          'dart',
          reason: 'the fence language is stamped onto the node',
        );
        // No stray code ParagraphNode left behind. 没遗留 code 段落。
        expect(
          doc.toList().whereType<ParagraphNode>().where(
            (n) => n.getMetadataValue('blockType') == codeAttribution,
          ),
          isEmpty,
        );
      },
    ...
    test(
      'a [[id]]-looking run INSIDE code stays literal (code is atomic, never inflated to a pill)',
      () {
        const source = '```\nsee [[$_id]] here\n```';
        final doc = documentFromMarkdown(source);
        ...
        expect(
          block.code,
          'see [[$_id]] here',
          reason: 'code content is not mention-inflated',
        );
        final md = markdownFromDocument(doc);
        expect(
          md.contains('[[$_id]]'),
          isTrue,
          reason: 'still literal in the serialized code fence',
        );
      },
    );
    ...
    test(
      ...
      () {
        ...
        expect(_rawCodeRuns(node), [
          '${_nbsp}fetch_weather$_nbsp',
        ], reason: 'padded with a spacer NBSP each side');
        expect(
          node.text.toPlainText().contains(_nbsp),
          isTrue,
          reason: 'the spacer really is in the document text',
        );
        // On save, the spacers are stripped by attribution — the markdown is `code`, never ` code `. 存盘剥离。
        final md = markdownFromDocument(doc);
        expect(
          md.contains('`fetch_weather`'),
          isTrue,
          reason: 'clean backtick code, no injected padding',
        );
        expect(
          md.contains(_nbsp),
          isFalse,
    ...
    test(
      'inline code round-trips: text → backtick → text, second pass stable',
      () {
    ...
    test(
      'a [[id]]-looking substring INSIDE inline code stays literal (not inflated to a mention)',
      () {
        const source = '用 `see [[$_id]] here` 引用。';
        final doc = documentFromMarkdown(source);
        final node = doc.first as ParagraphNode;
        expect(
          _codeRuns(node).single,
          'see [[$_id]] here',
          reason: 'code content is verbatim',
        );
        expect(
          node.text.placeholders.values.whereType<MentionPlaceholder>(),
          isEmpty,
          reason: 'no pill created',
        );
        final md = markdownFromDocument(doc);
        expect(
          md.contains('`see [[$_id]] here`'),
          isTrue,
          reason: 'still literal backtick code on save',
        );
      },
    );
    ...
    test('inline code and a real mention coexist and both round-trip', () {
      ...
      final node = doc.first as ParagraphNode;
      expect(
        node.text.placeholders.values.whereType<MentionPlaceholder>().length,
        1,
      );
      expect(_codeRuns(node), [
        'helper()',
      ], reason: 'code is a codeAttribution run');
      final md = markdownFromDocument(doc);
    ...
    test(
      'inline code inside a LIST item round-trips (not just paragraphs)',
      () {
        const source = '- 跑 `make test`\n- 再 `make docs`';
        final doc = documentFromMarkdown(source);
        final runs = [
          for (final n in doc.toList().whereType<ListItemNode>())
            ..._codeRuns(n),
        ];
    ...
    test(
      'a table round-trips: cells survive and the second pass is stable',
      () {
        const source = '| A | B |\n| --- | --- |\n| 1 | 2 |';
        final md1 = markdownFromDocument(documentFromMarkdown(source));
        final md2 = markdownFromDocument(documentFromMarkdown(md1));
    ...
    test(
      'table header follows the COLUMN alignment (GFM std; AnTableComponentBuilder un-centres it)',
      () {
        ...
        final vm =
            builder.createViewModel(doc, table) as MarkdownTableViewModel;
        const wantAligns = [TextAlign.left, TextAlign.center, TextAlign.right];
        expect(
          vm.cells.first.map((c) => c.textAlign).toList(),
          wantAligns,
          reason: 'header follows the column, not super_editor centre',
        );
        expect(
          vm.cells[1].map((c) => c.textAlign).toList(),
          wantAligns,
    ...
    test('a NESTED list keeps its indent levels', () {
      ...
      expect(
        [for (final n in items) n.indent],
        [0, 1, 2],
        reason: 'indent parsed',
      );
      final md1 = markdownFromDocument(doc);
      final md2 = markdownFromDocument(documentFromMarkdown(md1));
      expect(md1, md2, reason: 'nesting is stable across passes');
      final items2 = documentFromMarkdown(
        md1,
      ).toList().whereType<ListItemNode>().toList();
      expect(
        [for (final n in items2) n.indent],
        [0, 1, 2],
        reason: 'indent survives the trip',
      );
    });
  ...
  group('task round-trip (degeneration cascade)', () {
    test(
      'an EMPTY task survives the round-trip as a task (trailing space exempt from trim)',
      () {
        ...
        expect(
          md.contains('- [ ] \n'),
          isTrue,
          reason: 'the empty task marker keeps its load-bearing trailing space',
        );
        final doc2 = documentFromMarkdown(md);
    ...
    test('an empty COMPLETE task round-trips too', () {
      final doc1 = MutableDocument(
        nodes: [TaskNode(id: 't1', text: AttributedText(''), isComplete: true)],
      );
    ...
    test(
      'HEAL: a degraded bullet with literal "[ ]" text loads as a task again (old corrupted docs)',
      () {
        ...
        expect(
          (doc.getNodeAt(2)! as TaskNode).isComplete,
          isTrue,
          reason: '[x] heals complete',
        );
      },
    );

    test(
      'HEAL: a bullet whose text is "[ ] label" recovers the label as task text',
      () {
    ...
    test(
      'HEAL: a lazy-continuation-merged task un-merges losslessly (no leading-\\n task survives)',
      () {
        ...
        final allPlain = [
          for (final n in healed)
            if (n is TextNode) n.text.toPlainText(),
        ].join('\n');
        expect(
          allPlain,
          contains('plain: 前 plain 后'),
          reason: 'the swallowed content is not lost',
        );
        for (final n in healed) {
          if (n is TextNode) {
            expect(
              n.text.toPlainText().startsWith('\n'),
              isFalse,
              reason: 'no node leads with a newline',
            );
          }
        }
      },
    ...
    test(
      'the full cascade md1→doc2→md2→doc3 stays task-shaped for TWO rounds',
      () {
        var md = '- [ ] a\n- [ ] \n- [ ] b';
        for (var round = 0; round < 2; round++) {
          final doc = documentFromMarkdown(md);
          expect(
            [for (final n in doc) n is TaskNode].every((x) => x),
            isTrue,
            reason: 'round ${round + 1}: every node is still a TaskNode',
          );
          expect(
            [for (final n in doc) (n as TaskNode).text.toPlainText()],
            ['a', '', 'b'],
            reason:
                'round ${round + 1}: no content swallowed, no leading newlines',
          );
    ...
    test('a REAL bullet list is untouched by the heal', () {
      ...
      expect(
        [for (final n in doc) n.runtimeType.toString()],
        ['ListItemNode', 'ListItemNode', 'ListItemNode'],
        reason:
            'only a LEADING literal checkbox marker heals — prose bullets stay bullets',
      );
    });
  });
}
