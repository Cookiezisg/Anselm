...
DocumentNode _node(Editor e) =>
    e.context.find<MutableDocument>(Editor.documentKey).first;
...
void main() {
  test(
    'typing `[] ` converts the paragraph to an unchecked to-do (Notion gesture, not `- [ ]`)',
    () {
      final editor = _build();
      _type(editor, '[] ');
      ...
      expect(
        ...
        isEmpty,
        reason: 'the `[] ` trigger text is dropped',
      );
    },
  );

  test(
    'typing ```` ``` ```` + space converts the paragraph to the embedded CodeBlockNode',
    () {
      final editor = _build();
      _type(editor, '``` ');
      expect(
        _node(editor),
        isA<CodeBlockNode>(),
        reason: 'the opening fence alone triggers a code block',
      );
    },
  );

  test(
    'typing `+ ` converts the paragraph to an unordered list item (Notion bullet alias)',
    () {
      final editor = _build();
      _type(editor, '+ ');
      final node = _node(editor);
  ...
  test(
    'a `[]` WITHOUT a trailing space does not convert (still a paragraph)',
    () {
      final editor = _build();
      _type(editor, '[]');
      expect(
        _node(editor),
        isA<ParagraphNode>(),
        reason: 'no trailing space → no conversion',
      );
    },
  );
}
