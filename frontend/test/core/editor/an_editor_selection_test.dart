...
void main() {
  ...
  group('mergeBoxesByLine', () {
    ...
    test('adjacent wrapped lines stay separate even when their boxes overlap by a sub-pixel', () {
      // The measured real-font shape: line1 bottom 24.0, line2 top 23.7 — 0.3px overlap. 实测真字体形状。
      final merged = mergeBoxesByLine([box(99.7, -0.3, 688.5, 24.0), box(0.0, 23.7, 534.1, 48.0)]);
      expect(merged, hasLength(2), reason: 'two visual lines → two rects, never one union');
    });

    test('same-line split boxes (script-run boundaries) still merge into one rect', () {
      // CJK-in-code splits one line into several boxes sharing the line box (same top/bottom). CJK 同行断盒。
      final merged = mergeBoxesByLine([box(10, 0, 40, 24), box(40, 0, 90, 24), box(90, 0, 120, 24)]);
      expect(merged, hasLength(1));
      expect(merged.single, const Rect.fromLTRB(10, 0, 120, 24));
    });
  });

  testWidgets('a mid-run wrapped inline code keeps ONE pill rect PER line (real fonts)', (tester) async {
    ...
    text.addAttribution(codeAttribution, SpanRange(start, start + code.length - 1));
    ...
    final state = tester.state<AnTextComponentState>(find.byType(AnTextComponent));
    ...
    expect(boxes.length, greaterThanOrEqualTo(2), reason: 'the run must actually wrap for this test to bite');
    final merged = mergeBoxesByLine(boxes);
    expect(merged, hasLength(boxes.length),
        reason: 'each wrapped line keeps its own rect — the union collapse painted only the bottom line');
    // No merged rect spans more than one line box. 无 rect 高过一行盒。
    for (final r in merged) {
      expect(r.height, lessThan(30), reason: 'a per-line rect, not a two-line union (~48)');
    }
