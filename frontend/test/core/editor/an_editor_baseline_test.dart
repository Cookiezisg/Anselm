...
class _BaselineProbe extends SingleChildRenderObjectWidget {
  ...
  void updateRenderObject(
    ...
    _RenderBaselineProbe renderObject,
...
void main() {
  testWidgets(
    'outer box forwards the SuperText paragraph baseline; a raw SuperText reports none',
    (tester) async {
            ...
            body: Builder(
              builder: (context) {
                final span = AttributedText(
                  'Hg',
                ).computeInlineSpan(context, sb, const []);
                return Column(
      ...
      expect(
        rawSuperTextBaseline,
        isNull,
        reason: 'raw SuperText drops the baseline — the defect the proxy fixes',
      );
      ...
      expect(
        ...
        isNotNull,
        reason:
            'AnTextComponent (via _AnBaselineProxy) reports a real baseline',
      );
      expect(
        anBaseline,
        closeTo(paragraphBaseline!, 0.01),
        reason: 'and it equals the paragraph first-line baseline',
      );
    },
  );

  testWidgets('AnTextComponent still reports a baseline when the text is empty', (
    tester,
  ) async {
  ...
  testWidgets(
    'a baseline Row seats a short marker below a taller AnTextComponent (baseline engaged)',
    (tester) async {
