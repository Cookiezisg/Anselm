...
void main() {
  ...
  testWidgets(
    'framed wraps in a BORDERED AnCard (B13 有边卡壳); framed:false renders bare',
    (tester) async {
      await tester.pumpWidget(
        _host(ApprovalGate(parked: _node(), onDecide: (_, _) {})),
      );
      await tester.pump();
      ...
      await tester.pumpWidget(
        _host(
          ApprovalGate(parked: _node(), framed: false, onDecide: (_, _) {}),
        ),
      );
      await tester.pump();
  ...
  testWidgets(
    'star bug: the rendered prompt parses markdown — **strong** is bold, not literal asterisks (0719)',
    (tester) async {
      ...
      await tester.pumpWidget(
        _host(ApprovalGate(parked: node, onDecide: (_, _) {})),
      );
      await tester.pump();
  ...
  testWidgets('reason input shows ONLY with collectReason + an allowReason node', (
    tester,
  ) async {
    // allowReason node but collectReason off (terminal/cockpit path — their decide carries no reason).
    await tester.pumpWidget(
      _host(
        ApprovalGate(parked: _node(allowReason: true), onDecide: (_, _) {}),
      ),
    );
    await tester.pump();
    ...
    await tester.pumpWidget(
      _host(
        ApprovalGate(
          parked: _node(allowReason: false),
          collectReason: true,
          onDecide: (_, _) {},
        ),
      ),
  ...
  testWidgets(
    'Approve forwards the typed reason; Reject forwards the verdict',
    (tester) async {
      String? gotVerdict;
      String? gotReason;
      await tester.pumpWidget(
      ...
      await tester.tap(
        find.text(
          Translations.of(
            tester.element(find.byType(ApprovalGate)),
          ).run.approve,
        ),
      );
      await tester.pump();
  ...
  testWidgets('busy disables both buttons', (tester) async {
    ...
    await tester.pumpWidget(
      _host(
        ApprovalGate(parked: _node(), busy: true, onDecide: (_, _) => fired++),
      ),
    );
    await tester.pump();
