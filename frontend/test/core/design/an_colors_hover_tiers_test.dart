...
void main() {
  for (final (name, c) in [('light', AnColors.light), ('dark', AnColors.dark)]) {
    test('$name: row-hover / inline-button-hover / selection are three distinct fills', () {
      final tiers = <Color>{c.surfaceHover, c.surfaceHoverStrong, c.surfaceActive};
      expect(tiers.length, 3,
          reason: '$name: surfaceHover, surfaceHoverStrong, surfaceActive must be mutually distinct');
    });

    test('$name: inline-button hover is a CLEAR notch off the row hover (not a hair)', () {
      ...
      int lum(Color x) => ((x.r * 255).round() + (x.g * 255).round() + (x.b * 255).round()) ~/ 3;
      expect((lum(c.surfaceHover) - lum(c.surfaceHoverStrong)).abs(), greaterThanOrEqualTo(8),
          reason: '$name: surfaceHoverStrong must be a visible notch off surfaceHover');
    });
  }
}
