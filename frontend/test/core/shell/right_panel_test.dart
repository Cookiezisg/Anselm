...
void main() {
  ...
  test('default: open in every ocean EXCEPT chat (collapsed — on-demand existence)', () {
  ...
  test('chat: once the user opens it, the open sticks across switches (persistence 照旧)', () {
    final c = ProviderContainer();
    addTearDown(c.dispose);
  ...
  test('toggle flips only the current ocean\'s bucket', () {
    ...
    expect(c.read(rightPanelCollapsedProvider), isTrue);
    c.read(selectedOceanProvider.notifier).select(OceanKind.documents);
    expect(c.read(rightPanelCollapsedProvider), isFalse); // untouched — its own default OPEN
    c.read(rightPanelCollapsedProvider.notifier).toggle();
    c.read(rightPanelCollapsedProvider.notifier).toggle();
