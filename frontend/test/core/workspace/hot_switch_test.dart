...
class _FakeStartup extends BackendStartup {
  @override
  BackendState build() =>
      const BackendState(BackendPhase.ready, baseUrl: 'http://127.0.0.1:1', authToken: 'tk');
}
...
void main() {
  ...
  test('the hot-switch pulse: a workspace change rebuilds the ApiClient', () {
    ...
    expect(identical(before, c.read(apiClientProvider)), isFalse,
        reason: '客户端重建=全部 Live repo 级联重取的脉搏');
  });

  test('a workspace change tears down and rebuilds the SSE gateway (reconnect all three)', () {
    final c = _container();
    final before = c.read(sseGatewayProvider);
    expect(before, isNotNull);
    c.read(activeWorkspaceProvider.notifier).set('ws_2');
    final after = c.read(sseGatewayProvider);
    expect(identical(before, after), isFalse, reason: 'SSE 按连接时 workspace 定域,必须重连');
  });

  test('bootstrap PRODUCES the workspace and stays out of the loop — a switch is never yanked back',
      () async {
    final adapter = _WorkspacesAdapter();
    final c = _container(
        dio: Dio(BaseOptions(baseUrl: 'http://127.0.0.1:1'))..httpClientAdapter = adapter);
  ...
  test('chat sticky state self-heals: landing model + title reveals reset on switch', () {
    final c = _container();
    c.read(landingModelProvider.notifier).set((apiKeyId: 'ak_1', modelId: 'm'));
    ...
    c.read(activeWorkspaceProvider.notifier).set('ws_2');
    expect(c.read(landingModelProvider), isNull, reason: '键对属旧 workspace 的 key 集');
    expect(c.read(titleRevealsProvider), isEmpty, reason: '打字机队列是旧 workspace 的线程');
  });

  testWidgets('the switch action: leave the deep link first, then set the axis', (tester) async {
    ...
    c.read(workspaceSwitchProvider).switchTo(id: 'ws_2', name: 'Two');
    await tester.pumpAndSettle();
    expect(router.routerDelegate.currentConfiguration.uri.path, '/documents/doc_new');
  });
}
