...
void main() {
  ...
  test('repeated identical lines → keeps the LCS, removes the surplus copy', () {
    final d = lineDiff('x\nx\nx', 'x\nx');
  ...
  test('empty before is NOT special-cased here (caller renders all-context) — split yields one empty line', () {
    final d = lineDiff('', 'a\nb');
  ...
  test('degrade gate: beyond the cell cap → whole-segment replace (no LCS)', () {
    final before = List.generate(3000, (i) => 'b$i').join('\n'); // 3000 lines
    final after = List.generate(3000, (i) => 'a$i').join('\n'); // (3001)² ≈ 9M cells > 4M
  ...
  test('degrade gate: BALANCED diff trips the cell cap a total-line cap would miss (G5.0 review HIGH)', () {
    ...
    final d = lineDiff(before, after);
    expect(count(d, DiffOp.context), 0, reason: 'the cell cap degrades the balanced case');
    expect(count(d, DiffOp.del), 2500);
    expect(count(d, DiffOp.add), 2500);
  });

  test('under the cell cap → real LCS runs (large but identical → all context, not over-degraded)', () {
    ...
    expect(count(d, DiffOp.context), 1000, reason: 'below the cap LCS runs and finds the full match');
    expect(count(d, DiffOp.del), 0);
    expect(count(d, DiffOp.add), 0);
  });
}
