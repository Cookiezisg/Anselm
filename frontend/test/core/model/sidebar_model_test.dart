...
void main() {
              ...
              SidebarRow(
                ...
                children: [
                  SidebarRow(
                    ...
                    children: [SidebarRow(id: 'doc3', label: 'deep-leaf')],
                  ),
                ],
              ),
            ],
  ...
  test(
    'no match → empty visible set',
    () => expect(sidebarVisibleIds(tree, 'zzzznotfound'), isEmpty),
  );

  // ── 5-battery ──
  test(
    'empty model → empty (no crash)',
    () => expect(sidebarVisibleIds(const SidebarModel(), 'x'), isEmpty),
  );
    ...
    const m = SidebarModel(
      groups: [
        SidebarGroup(
          types: [
            SidebarType(
              rows: [SidebarRow(id: 'c', label: '工作流编排器')],
            ),
          ],
        ),
      ],
  ...
  test(
    'regex / markup metacharacters are inert literal substrings (injection-safe)',
    () {
      const m = SidebarModel(
        groups: [
          SidebarGroup(
            types: [
              SidebarType(
                rows: [SidebarRow(id: 'x', label: 'a.*b <script>')],
              ),
            ],
          ),
        ],
      ...
      expect(
        sidebarVisibleIds(m, '.*'),
        contains('x'),
      ); // literal, not a regex wildcard
      expect(
        sidebarVisibleIds(m, '<script>'),
        contains('x'),
      ); // literal, not markup
  ...
  test('massive flat list filters in one pass', () {
    final rows = [
      for (var i = 0; i < 2000; i++) SidebarRow(id: 'r$i', label: 'item $i'),
    ];
    final m = SidebarModel(
      groups: [
        SidebarGroup(types: [SidebarType(rows: rows)]),
      ],
    );
