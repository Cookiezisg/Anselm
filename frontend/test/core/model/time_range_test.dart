...
void main() {
  ...
  group('resolveTimeRange', () {
    ...
    test('h24/d7/d30 are true look-back durations', () {
      expect(
        resolveTimeRange(const AnPresetRange(AnTimePreset.h24), now).from,
        now.subtract(const Duration(hours: 24)),
      );
      expect(
        resolveTimeRange(const AnPresetRange(AnTimePreset.d7), now).from,
        now.subtract(const Duration(days: 7)),
      );
      expect(
        resolveTimeRange(const AnPresetRange(AnTimePreset.d30), now).from,
    ...
    test(
      'absolute: inclusive minute-grain end becomes an exclusive bound ONE minute later — '
    ...
    test(
      'presets re-resolve against the now they are handed — a live expression, never frozen',
      () {
        final later = now.add(const Duration(hours: 3));
        final a = resolveTimeRange(
          const AnPresetRange(AnTimePreset.h24),
          now,
        ).from!;
        final b = resolveTimeRange(
          const AnPresetRange(AnTimePreset.h24),
          later,
        ).from!;
  ...
  group('range equality (preset highlight relies on it)', () {
    test('presets compare by preset, absolutes by both endpoints', () {
      expect(
        const AnPresetRange(AnTimePreset.d7),
        const AnPresetRange(AnTimePreset.d7),
      );
      expect(
        const AnPresetRange(AnTimePreset.d7),
        isNot(const AnPresetRange(AnTimePreset.d30)),
      );
      expect(
      ...
      expect(
        AnAbsoluteRange(from: DateTime(2026, 1, 1), to: DateTime(2026, 1, 2)),
        isNot(
          AnAbsoluteRange(from: DateTime(2026, 1, 1), to: DateTime(2026, 1, 3)),
        ),
      );
    });
  });
  ...
  group('month grid', () {
    test('always 42 cells, Monday first, month fully contained', () {
      final days = monthGridDays(
        DateTime(2026, 7, 1),
      ); // Jul 2026: the 1st is a Wednesday 周三
    ...
    test(
      'a month starting on Monday still renders 6 rows (fixed weeks — zero height jump)',
      () {
        final days = monthGridDays(
          DateTime(2026, 6, 1),
        ); // Jun 2026: the 1st IS a Monday 周一起手
        expect(days.length, 42);
        expect(days.first, DateTime(2026, 6, 1));
        expect(
          days.last,
          DateTime(2026, 7, 12),
        ); // trailing July fills row 6 尾行补七月
      },
    );
  ...
  group('day helpers', () {
    test('isSameDay ignores time; dateOnly strips to local midnight', () {
      expect(
        isSameDay(DateTime(2026, 7, 17, 23, 59), DateTime(2026, 7, 17, 0, 1)),
        isTrue,
      );
