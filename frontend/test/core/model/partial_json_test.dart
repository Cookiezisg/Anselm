...
void main() {
  ...
  group('PartialJsonSession', () {
      ...
      for (final s in corpus) {
        ...
        for (final chunk in [1, 2, 3, 7, 16]) {
          ...
          for (var i = 0; i < s.length; i += chunk) {
            sess.append(s.substring(i, i + chunk > s.length ? s.length : i + chunk));
          }
          expect(sess.events.length, oneShot.length, reason: 'len mismatch for "$s" chunk=$chunk');
          for (var k = 0; k < oneShot.length; k++) {
            expect(sess.events[k].path, oneShot[k].path, reason: 'path[$k] for "$s" chunk=$chunk');
            expect(sess.events[k].value, oneShot[k].value, reason: 'value[$k] for "$s" chunk=$chunk');
          }
        }
      }
    });

    test('NO RE-EMISSION: a consumer cursor sees each completion exactly once', () {
      ...
      expect(seen, ['ops/0/op', 'ops/0', 'ops/1/op', 'ops/1', 'ops/2/op', 'ops/2', 'ops', '']);
    });

    test('in-flight channel: the open string value surfaces with its full path', () {
    ...
    test('in-flight channel: same-key values are told apart by path (handler multi-method body)', () {
      final sess = PartialJsonSession()
        ..append('{"ops":[{"method":{"name":"a","body":"AAA"}},{"method":{"name":"b","body":"BB');
      expect(sess.inFlightString!.path, ['ops', 1, 'method', 'body']);
      expect(sess.inFlightString!.text, 'BB');
      expect(sess.inFlightStringAt(['ops', 0, 'method', 'body']), isNull); // that one CLOSED
    ...
    test('in-flight text is decoded, and a pending escape is withheld until resolved', () {
    ...
    test('boundary straddles: \\uXXXX, surrogate pair, number, keyword split across appends', () {
      final u = PartialJsonSession()..append(r'{"c":"\u00')..append('41"}');
      expect(u.events.first.value, 'A');

      final emoji = PartialJsonSession()..append(r'{"c":"\ud83d')..append(r'\ude00"}');
      expect(emoji.events.first.value, '😀');

      final n = PartialJsonSession()..append('{"n":12')..append('3}');
      expect(n.events.first.value, 123);

      final k = PartialJsonSession()..append('{"k":tr')..append('ue}');
      expect(k.events.first.value, true);
    });

    test('malformed freezes the session but keeps completions; further input is ignored', () {
    ...
    test('done: root close ends the session; trailing input is ignored (facade semantics)', () {
    ...
    test('PERF: 1MB value streamed in 4KB deltas with per-delta in-flight reads stays cheap', () {
      final chunk = 'x' * 4096;
      ...
      expect(sw.elapsedMilliseconds, lessThan(1000), reason: 'incremental feed must be O(delta)');
    });
  });
}
