        ...
        SidebarType(
          ...
          rows: [
            SidebarRow(
              id: 'd1',
              label: 'docs',
              children: const [SidebarRow(id: 'd2', label: 'guide')],
            ),
          ],
        ),
      ],
...
void main() {
  test(
    'flattens groups→types→rows with section heads + a pagination footer',
    () {
  ...
  test('depth: label-null group → head + rows at 0, branch child at +1', () {
    final flat = flattenSidebar(_model());
    expect(
      flat.firstWhere((n) => n.type?.label == 'Docs').depth,
      0,
    ); // section head
    expect(
      flat.firstWhere((n) => n.row?.id == 'd1').depth,
      0,
    ); // row sits at the head's level
    expect(
      flat.firstWhere((n) => n.row?.id == 'd2').depth,
      1,
    ); // branch child nests +1
  });
  ...
  test('collapsed section contributes its head only — no rows, no footer', () {
    ...
    expect(
      flat.any((n) => n.kind == SidebarNodeKind.footer),
      isFalse,
    ); // footer gone under a collapsed section
  });
  ...
  test('ancestor chain: a deep row carries its section head + branch head', () {
    ...
    expect(guide.ancestors.map((a) => a.type?.label ?? a.row?.id), [
      'Docs',
      'd1',
    ]);
  });
  ...
  test(
    'query keeps only matching rows + force-expands ancestors, drops the footer',
    () {
      ...
      expect(
        flat.any((n) => n.type?.label == 'Functions'),
        isFalse,
      ); // section with no visible row dropped
      expect(
        flat.any((n) => n.kind == SidebarNodeKind.footer),
        isFalse,
      ); // search paginates via re-fetch, not the tail
    },
  );

  test(
    'semantic fold keys — folding one section never fuses with a sibling',
    () {
      final flat = flattenSidebar(_model(), collapsed: {'t:function'});
      expect(
        flat.any((n) => n.row?.id == 'd1'),
        isTrue,
      ); // Docs unaffected by Functions being folded
    },
  );
  ...
  test(
    'battery injection/overlong: markup + long labels are inert plain data (no throw)',
    () {
        ...
        groups: [
          SidebarGroup(
            types: [
              SidebarType(
                label: '<b>x</b> & 很长' * 20,
                rows: const [SidebarRow(id: 'x', label: '注入 <i>y</i>')],
              ),
            ],
          ),
        ],
