...
({ApiClient client, _FakeAdapter adapter}) _build(
  ...
  final client = ApiClient(dio: dio, workspaceId: () => ws, authToken: () => token);
  return (client: client, adapter: adapter);
}

void main() {
  test('getEntity unwraps {data:<obj>}', () async {
    final b = _build((_) => _json({'data': {'id': 'fn_1', 'name': 'norm'}}));
    final got = await b.client.getEntity('/functions/fn_1', (m) => m['name'] as String);
    expect(got, 'norm');
  });
  ...
  test('postForId returns data.id (202 async action)', () async {
    final b = _build((_) => _json({'data': {'id': 'run_9'}}, 202));
  ...
  test('postBare returns the bare (unwrapped) result', () async {
    ...
    final r = await b.client.postBare('/functions/fn_1:run') as Map<String, dynamic>;
    expect(r['out'], 42);
  });
  ...
  test('transport failure (no response) → ApiException.transport (status 0)', () async {
  ...
  test('interceptor attaches X-Anselm-Workspace-ID + Authorization: Bearer', () async {
    final b = _build((_) => _json({'data': {'status': 'ok'}}));
  ...
  test('no workspace / no token → headers omitted (not empty strings)', () async {
    final b = _build((_) => _json({'data': {'status': 'ok'}}), ws: null, token: '');
    await b.client.getData('/health');
    expect(b.adapter.last!.headers.containsKey('X-Anselm-Workspace-ID'), isFalse);
