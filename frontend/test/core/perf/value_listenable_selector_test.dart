...
void main() {
  ...
  testWidgets(
    'rebuilds ONLY when the selected slice changes, not on every notification',
    (tester) async {
      final vn = ValueNotifier<int>(0);
      var builds = 0;
      await tester.pumpWidget(
        host(
          ValueListenableSelector<int, int>(
            listenable: vn,
            selector: (v) =>
                v ~/ 10, // the slice = the tens place; unchanged for 0..9 切片=十位
            builder: (context, v) {
              builds++;
              return Text('$v');
            },
      ...
      expect(
        builds,
        1,
      ); // slice still 0 — NO rebuild (the whole point) 切片不变→不重建
  ...
  testWidgets(
    'didUpdateWidget re-derives the baseline when the selector changes (no stale-skip)',
    (tester) async {
      final vn = ValueNotifier<int>(100);
      var builds = 0;
      var watchTens =
          false; // false: watch the hundreds digit; true: the tens digit 看百位/十位
