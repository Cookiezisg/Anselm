...
void main() {
  ...
  testWidgets('notifies at the pulse cadence (~30fps), never at vsync rate (T1 guard)', (tester) async {
    final clock = PulseClock(period: const Duration(seconds: 1), idleAfter: const Duration(seconds: 5));
    var notified = 0;
    clock.addListener(() => notified++);
    clock.poke();
  ...
  testWidgets('losing the last listener stops the clock (nobody watching = no work)', (tester) async {
    final clock = PulseClock(period: const Duration(milliseconds: 100), idleAfter: const Duration(milliseconds: 500));
    void listener() {}
