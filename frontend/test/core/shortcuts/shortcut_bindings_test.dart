...
void main() {
  ...
  test('the GATE: every catalog command has a default and an effective binding', () {
    final c = container(SettingsPrefs.inMemory());
    final bindings = c.read(shortcutBindingsProvider);
    expect(bindings.keys.toSet(), ShortcutCommand.values.toSet(), reason: 'catalog == bound');
  ...
  test('chord serialize/parse round-trips (stable key-id encoding)', () {
    const chord = ShortcutChord(LogicalKeyboardKey.keyK, cmd: true, shift: true);
    final back = ShortcutChord.parse(chord.serialize());
    expect(back, chord);
    expect(ShortcutChord.parse('garbage'), isNull);
  });

  test('rebind persists ONLY overrides; a reset clears its key from storage', () async {
    ...
    await c
        ...
        .rebind(ShortcutCommand.openSettings, const ShortcutChord(LogicalKeyboardKey.keyP, cmd: true));

    final raw = prefs.getString(SettingsKeys.shortcuts);
    final map = jsonDecode(raw) as Map<String, dynamic>;
    expect(map.keys, ['openSettings'], reason: '只存被改的那一条');
    expect(c.read(shortcutBindingsProvider)[ShortcutCommand.openSettings]!.key,
        LogicalKeyboardKey.keyP);

    await c.read(shortcutBindingsProvider.notifier).reset(ShortcutCommand.openSettings);
    expect(prefs.getString(SettingsKeys.shortcuts), '', reason: '回默认后无覆写残留');
    expect(c.read(shortcutBindingsProvider)[ShortcutCommand.openSettings],
  ...
  test('conflictFor detects a chord already bound to another command', () {
    ...
    expect(notifier.conflictFor(settingsChord, self: ShortcutCommand.zoomIn),
        ShortcutCommand.openSettings);
    expect(notifier.conflictFor(settingsChord, self: ShortcutCommand.openSettings), isNull);
    expect(
        notifier.conflictFor(const ShortcutChord(LogicalKeyboardKey.keyJ, cmd: true, shift: true)),
        isNull);
  });
  ...
  test('malformed persisted json degrades to defaults', () {
    ...
    final c = container(prefs);
    final bindings = c.read(shortcutBindingsProvider);
    expect(bindings[ShortcutCommand.toggleLeftIsland],
