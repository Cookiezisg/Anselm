...
void main() {
  ...
  group('layoutGraph pinned positions', () {
    test('all-pos graph uses authored coords normalized to the pad', () {
      ...
      expect(
        l.nodeRects['a']!.topLeft,
        const Offset(GraphGeometry.pad, GraphGeometry.pad),
      );
      expect(
  ...
  group('edge routing', () {
    ...
    test('back edge routes through the below-bounds channel (LR)', () {
      ...
      final maxBottom = l.nodeRects.values
          .map((r) => r.bottom)
          .reduce((a, b) => a > b ? a : b);
    ...
    test('huge fan-out stays finite and collision-free on the main axis', () {
      ...
      final tops = [for (var i = 0; i < 40; i++) l.nodeRects['x$i']!.top]
        ..sort();
      for (var i = 1; i < tops.length; i++) {
        expect(
          tops[i] - tops[i - 1],
          greaterThanOrEqualTo(GraphGeometry.nodeH),
        );
      }
    });
  });
