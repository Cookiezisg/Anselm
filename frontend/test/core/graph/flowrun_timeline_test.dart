...
FlowrunComposite comp(List<FlowrunNode> nodes, {String status = 'completed'}) =>
    FlowrunComposite(
      flowrun: Flowrun(
        id: 'flr_1',
        workflowId: 'wf_1',
        ...
        completedAt: status == 'completed'
            ? _t.add(const Duration(seconds: 10))
            : null,
        updatedAt: _t,
      ),
      nodes: nodes,
...
void main() {
  group('flowrunTimeline', () {
    ...
    test(
      'time-mode positions bars by createdAt/completedAt over the run span',
      () {
    ...
    test(
      'zero-span run falls back to sequential slots (still reads left→right)',
      () {
        ...
        final positions = [
          for (final id in ['t', 'work', 'gate'])
            rows.firstWhere((r) => r.nodeId == id).segments.first.at,
        ];
    ...
    test(
      'orphan row keeps the backend-written kind (a renamed/removed node)',
      () {
        ...
        final ghost = FlowrunNode(
          ...
          status: 'completed',
          createdAt: _t,
          completedAt: _t.add(const Duration(seconds: 1)),
          updatedAt: _t,
        );
