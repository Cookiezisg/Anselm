...
void main() {
  group('deriveRunState', () {
    test('empty rows → empty overlay', () {
      expect(
        deriveRunState(g, rows: const [], runStatus: 'running'),
        same(GraphRunState.empty),
      );
    });

    test(
      'linear walk: completed nodes, taken edges, running successor synthesized',
      () {
        final s = deriveRunState(
          g,
          rows: [row('t', 'completed'), row('work', 'completed')],
          runStatus: 'running',
        );
    ...
    test(
      'control port gates its out-edges: only the recorded __port is taken',
      () {
        final s = deriveRunState(
          g,
          rows: [
    ...
    test(
      'retry loop: back edge targets iteration+1; ×N counts; running re-run lights',
      () {
        final s = deriveRunState(
          g,
          rows: [
        ...
        expect(
          s2.nodes['gate'],
          GraphNodeRun.running,
        ); // gate due at iter 1 now gate 应跑 iter1
      },
    );

    test(
      'parked blocks its OWN downstream only; parallel branches keep lighting (backend walk)',
      () {
        final ga = Graph(
          nodes: [
            n('t', NodeKind.trigger),
        ...
        expect(s.nodes.containsKey('post'), isFalse); // parked 自身下游不亮
        // The sibling branch is REALLY advancing on the backend — it must light. 并行分支真在推进,必须亮。
        expect(s.nodes['c'], GraphNodeRun.running);
        expect(s.liveEdges, {'e4'});
      },
    );

    test(
      'AND-join: a partially-satisfied merge does NOT light (backend predecessorsSatisfied)',
      () {
        final gd = Graph(
          nodes: [
            n('t', NodeKind.trigger),
        ...
        final s = deriveRunState(
          gd,
          rows: [row('t', 'completed'), row('a', 'completed')],
          runStatus: 'running',
        );
        // b is the one running; c waits for the join. b 在跑,c 等汇聚。
        expect(s.nodes['b'], GraphNodeRun.running);
        expect(s.nodes.containsKey('c'), isFalse);
        expect(s.liveEdges, {'e2'});
        // Once b completes, c lights. b 完成后 c 亮。
        final s2 = deriveRunState(
          gd,
          rows: [
            row('t', 'completed'),
        ...
        expect(s2.nodes['c'], GraphNodeRun.running);
        expect(s2.liveEdges, {'e3', 'e4'});
      },
    );

    test(
      '×N counts EXECUTIONS (rows), not max-iteration+1 — loop-exit successor stays ×1',
      () {
        final s = deriveRunState(
          g,
          rows: [
            ...
            row('gate', 'completed', iteration: 0, result: {'__port': 'retry'}),
            row('work', 'completed', iteration: 1),
            row('gate', 'completed', iteration: 1, result: {'__port': 'pass'}),
            // post first (and only) runs at the loop's final iteration — forward edges KEEP i.
            // post 首次也是唯一一次执行落在循环末迭代(前向边保持 i)。
            row('post', 'completed', iteration: 1),
          ],
          runStatus: 'completed',
        );
        expect(s.iters['post'], 1); // ran once → no ×N badge 只跑一次,无叠卡
        expect(s.iters['work'], 2);
        expect(s.iters['gate'], 2);
      },
    );
    ...
    test('failed run: no synthesis, failed node red', () {
      final s = deriveRunState(
        g,
        rows: [row('t', 'completed'), row('work', 'failed')],
        runStatus: 'failed',
      );
    ...
    test(
      'tick rows (no result) leave port edges honestly unlit until reconcile',
      () {
        final s = deriveRunState(
          g,
          rows: [
    ...
    test('rows referencing unknown node ids are harmless', () {
      final s = deriveRunState(
        g,
        rows: [row('ghost', 'completed'), row('t', 'completed')],
        runStatus: 'running',
      );
