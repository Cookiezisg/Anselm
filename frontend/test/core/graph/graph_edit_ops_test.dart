...
Node n(String id, NodeKind k, {String? ref, Map<String, String> input = const {}, NodePosition? pos}) =>
  ...
  group('workflowEditOps (diff)', () {
    ...
    test('add node + edge', () {
      final w = base.copyWith(
        ...
        edges: [...base.edges, e('e3', 'gate', 'post', port: 'pass')],
      );
      final ops = workflowEditOps(base, w);
      expect(ops, contains(predicate((o) => (o as Map)['op'] == 'add_node' && (o['node'] as Map)['id'] == 'post')));
      expect(ops, contains(predicate((o) => (o as Map)['op'] == 'add_edge' && (o['edge'] as Map)['fromPort'] == 'pass')));
    });

    test('delete node → delete_node, and its edges are NOT separately deleted (backend cascade)', () {
      final w = base.copyWith(
      ...
      expect(ops.where((o) => o['op'] == 'delete_edge'), isEmpty); // e2 touches gate → cascaded
    });

    test('change ref → update_node with a ref-only patch', () {
      final w = base.copyWith(nodes: [
        for (final node in base.nodes) node.id == 'work' ? node.copyWith(ref: 'fn_new') : node,
      ]);
      final ops = workflowEditOps(base, w);
    ...
    test('move node → update_node with a pos-only patch', () {
      final w = base.copyWith(nodes: [
        for (final node in base.nodes)
          node.id == 'work' ? node.copyWith(pos: const NodePosition(x: 300, y: 40)) : node,
      ]);
      final ops = workflowEditOps(base, w);
    ...
    test('change input → whole-map replace', () {
      final w = base.copyWith(nodes: [
        for (final node in base.nodes)
          node.id == 'work' ? node.copyWith(input: {'x': 'trigger.payload'}) : node,
      ]);
      final ops = workflowEditOps(base, w);
    ...
    test('change edge port → update_edge', () {
      final w = base.copyWith(edges: [
        for (final edge in base.edges) edge.id == 'e2' ? edge.copyWith(fromPort: 'retry') : edge,
      ]);
      final ops = workflowEditOps(base, w);
      expect(ops.first, {'op': 'update_edge', 'id': 'e2', 'patch': {'fromPort': 'retry'}});
    });
  });
  ...
  group('applyEditOps (inverse) + round-trip', () {
    test('applying the diff of A→B reproduces B', () {
      final b = base.copyWith(
        nodes: [
          for (final node in base.nodes) node.id == 'work' ? node.copyWith(ref: 'fn_x', pos: const NodePosition(x: 5, y: 9)) : node,
          n('post', NodeKind.approval),
        ],
        edges: [
          for (final edge in base.edges) edge.id == 'e2' ? edge.copyWith(fromPort: 'go') : edge,
      ...
      expect({for (final x in result.nodes) x.id: x}, {for (final x in b.nodes) x.id: x});
      expect({for (final x in result.edges) x.id: x}, {for (final x in b.edges) x.id: x});
    });
