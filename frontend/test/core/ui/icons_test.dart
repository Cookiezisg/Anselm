...
void main() {
  ...
  test(
    'entityKindGlyph: the shared named lookup (case-insensitive, degrades)',
    () {
      expect(AnIcons.entityKindGlyph('function'), AnIcons.function);
      expect(
        AnIcons.entityKindGlyph('Workflow'),
        AnIcons.workflow,
      ); // case-insensitive
      expect(
        AnIcons.entityKindGlyph('document'),
        AnIcons.doc,
      ); // contract wire vocab, not demo alias
  ...
  test(
    'every registry tool lands an intentional glyph (none falls to the wrench default)',
    () {
      expect(
        registry.length,
        116,
        reason: '13 resident + 103 lazy = 116 (census §6)',
      );
      final generic = registry
          .where((t) => AnIcons.toolIcon(t) == AnIcons.tool)
          .toList();
      expect(
        generic,
        isEmpty,
        reason:
            'these tools have no intentional glyph — add a binding (exact table or rule): $generic',
      );
    },
  );
  ...
  test('dynamic MCP tools + genuinely unknown names', () {
    ...
    expect(
      AnIcons.toolIcon('frobnicate_the_gizmo'),
      AnIcons.tool,
    ); // real unknown → wrench
  });

  test(
    'toolIcon: builds/get show the ENTITY, delete/revert/search show the ACTION',
    () {
  ...
  test(
    'toolIcon: run-log archives read as history (not captured by the entity suffix)',
    () {
  ...
  test('toolIcon: irregular families get their bespoke glyph', () {
    ...
    expect(
      AnIcons.toolIcon('call_handler'),
      AnIcons.handler,
    ); // exec verb, not history
