...
void main() {
  Widget host(Widget child) => TranslationProvider(
    child: MaterialApp(
      ...
      theme: AnTheme.light(),
      home: Scaffold(
        body: Center(child: SizedBox(width: 460, height: 360, child: child)),
      ),
    ),
  );
  ...
  Future<void> expectNoLeak(
    ...
    expect(
      ...
      greaterThan(0),
      reason:
          'guard must not be vacuous — rebuilds must re-run the measure site',
    );
    expect(
      ledger.disposed,
      ledger.created,
      reason:
          'T6: measure-site TextPainters all disposed — no growth across 20 rebuilds (notDisposed == 0)',
    );
  }

  test(
    'measureText disposes its TextPainter — normal AND throwing read paths',
    () {
      final ledger = _TextPainterLedger()..attach();
      final w = measureText(
        const TextSpan(text: 'hello', style: TextStyle(fontSize: 14)),
        read: (tp) => tp.width,
      );
      ...
      expect(
        ...
        2,
        reason:
            'both painters disposed — the normal path and the throwing path',
      );
    },
  );

  testWidgets(
    'AnOceanSwitcher (labelWidth measure) leaves no undisposed TextPainter',
    (tester) async {
      await expectNoLeak(
        tester,
        () => AnOceanSwitcher(
  ...
  testWidgets(
    'AnVersionDiff (gutter measure) leaves no undisposed TextPainter',
    (tester) async {
      await expectNoLeak(
        tester,
        () => AnVersionDiff(before: 'alpha\nbeta', after: 'alpha\ngamma'),
      );
    },
  );

  testWidgets('AnComposer (line-count measure) leaves no undisposed TextPainter', (
    tester,
  ) async {
    ...
    final controller = TextEditingController(
      text: 'a composer line long enough to actually measure',
    );
