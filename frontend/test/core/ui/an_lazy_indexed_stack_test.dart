...
void main() {
  ...
  Widget host({required int index, int count = 3}) => MaterialApp(
    home: Scaffold(
      body: AnLazyIndexedStack(
        ...
        count: count,
        builder: (context, i) =>
            _Slot(i, onInit: () => inits.add(i), key: ValueKey('slot$i')),
      ),
    ),
  );
  ...
  testWidgets('lazy: only the shown slot is mounted; unshown slots are not', (
    tester,
  ) async {
    await tester.pumpWidget(host(index: 0));
    expect(inits, [0]); // only slot 0 mounted 仅槽 0 挂载
    expect(
      find.byKey(const ValueKey('slot1')),
      findsNothing,
    ); // slot 1 is a zero-cost box 槽 1=零成本盒
    expect(find.byKey(const ValueKey('slot2')), findsNothing);
  });

  testWidgets(
    'switching mounts the new slot and keeps the old one alive (no re-init)',
    (tester) async {
      await tester.pumpWidget(host(index: 0));
      ...
      expect(inits, [
        0,
        1,
      ]); // slot 1 mounted; slot 0 NOT re-inited (kept alive) 槽 1 挂、槽 0 未重挂
      ...
      expect(
        find.byKey(const ValueKey('slot0'), skipOffstage: false),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('slot1'), skipOffstage: false),
        findsOneWidget,
      );
      ...
      expect(
        inits,
        [0, 1],
      ); // returning to slot 0 does NOT re-init it — instant, state-preserving 回槽 0 不重挂
    },
  );

  testWidgets(
    'a never-visited slot is never mounted; visiting it mounts once',
    (tester) async {
      await tester.pumpWidget(host(index: 0));
      await tester.pumpWidget(host(index: 1));
      expect(
        inits.contains(2),
        isFalse,
      ); // slot 2 never shown → never mounted 槽 2 从未显→从未挂
  ...
  testWidgets(
    'index selects the visible slot; out-of-range clamps without tearing down',
    (tester) async {
      await tester.pumpWidget(host(index: 1));
      ...
      expect(inits, [
        1,
      ]); // still only slot 1 ever mounted; no crash, no teardown 仍仅槽 1 挂过
    },
  );

  testWidgets(
    'keep-alive preserves a slot\'s mutable State across a switch away and back',
    (tester) async {
      await tester.pumpWidget(host(index: 0));
      ...
      expect(
        find.text('count:1'),
        findsOneWidget,
      ); // preserved, not reset to 0 保留、未重置
    },
  );
}
