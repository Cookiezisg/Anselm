...
void main() {
  ...
  testWidgets('header cells are header semantics', (tester) async {
    ...
    await tester.pumpWidget(host(const AnThinTable(columns: cols, rows: rows)));
    expect(tester.getSemantics(find.bySemanticsLabel('Name')).flagsCollection.isHeader, isTrue);
    handle.dispose();
  });

  testWidgets('selectable: tapping a row → onRowTap with that row map', (tester) async {
    Map<String, String>? tapped;
    await tester.pumpWidget(host(AnThinTable(columns: cols, rows: rows, selectable: true, onRowTap: (r) => tapped = r)));
  ...
  testWidgets('selectable row first cell carries a button + row summary', (tester) async {
    final handle = tester.ensureSemantics();
    await tester.pumpWidget(host(AnThinTable(columns: cols, rows: rows, selectable: true, onRowTap: (_) {})));
    expect(find.bySemanticsLabel('Name: deploy, Status: running'), findsOneWidget);
    handle.dispose();
  });
  ...
  testWidgets('long value in a NON-first column shrinks within the table (no overflow)', (tester) async {
    ...
    const longStatus = 'an-extremely-long-status-value-that-must-not-overflow-the-table-right-edge';
    await tester.pumpWidget(host(
      const AnThinTable(columns: cols, rows: [
    ...
    expect(cellRight, lessThanOrEqualTo(tableRight + 0.5),
        reason: 'a long non-first value must ellipsis within the table, not paint past its right edge');
  });
}
