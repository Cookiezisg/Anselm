...
Widget _host(Widget child) => TranslationProvider(
    child: MaterialApp(theme: AnTheme.light(), home: Scaffold(body: Center(child: child))));

void main() {
  testWidgets('renders the count, fires actions and clear', (tester) async {
    var ran = 0;
    var cleared = 0;
    await tester.pumpWidget(_host(AnBatchBar(
      count: 3,
      actions: [
        BatchAction(label: '批量批准', tone: AnTone.accent, onRun: () => ran++),
        BatchAction(label: '批量拒绝', tone: AnTone.danger, onRun: () => ran += 10),
      ],
      onClear: () => cleared++,
    )));
    await tester.pump();
  ...
  testWidgets('busy freezes every action AND the clear', (tester) async {
    ...
    await tester.tap(find.bySemanticsLabel(tr.feedback.batch.clear), warnIfMissed: false);
    expect(fired, 0);
  });

  testWidgets('count ≤ 0 renders nothing (no selection, no bar)', (tester) async {
    await tester.pumpWidget(_host(AnBatchBar(count: 0, actions: const [], onClear: () {})));
    await tester.pump();
    expect(find.byType(DecoratedBox), findsNothing);
  });

  testWidgets('overlong labels ellipsize inside a narrow host instead of overflowing',
      (tester) async {
    await tester.pumpWidget(_host(SizedBox(
      width: 320,
      child: AnBatchBar(count: 99999, actions: [
        BatchAction(label: '把选中的全部重新排队并等待下一个调度窗口再统一执行' * 3, onRun: () {}),
  ...
  testWidgets('AnBatchCheck toggles and carries checked semantics', (tester) async {
    final handle = tester.ensureSemantics();
    bool? got;
    await tester.pumpWidget(_host(AnBatchCheck(
      checked: false,
