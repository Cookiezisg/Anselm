...
void main() {
  ...
  testWidgets('numbered: done shows a check, current/upcoming show numbers', (
    tester,
  ) async {
  ...
  testWidgets('a11y value is "Step N of M" (clamped) and is FINDABLE', (
    tester,
  ) async {
    final handle = tester.ensureSemantics();
    await tester.pumpWidget(
      host(const AnStepper(count: 4, current: 3, semanticLabel: 'Setup')),
    );
  ...
  testWidgets(
    'arriving says NOTHING; ADVANCING announces — the furniture/news asymmetry',
    (tester) async {
      final handle = tester.ensureSemantics();
      final said = probeAnnouncements(tester);
      await tester.pumpWidget(
        host(const AnStepper(count: 4, current: 1, semanticLabel: 'Setup')),
      );
      ...
      await tester.pumpWidget(
        host(const AnStepper(count: 4, current: 2, semanticLabel: 'Setup')),
      );
      await tester.pumpAndSettle();
  ...
  testWidgets(
    'a rebuild that does not advance stays silent (no announce storm on repaint)',
    (tester) async {
      final handle = tester.ensureSemantics();
      await tester.pumpWidget(
        host(const AnStepper(count: 4, current: 2, semanticLabel: 'Setup')),
  ...
  testWidgets(
    'onStepTap: only COMPLETED steps are tappable and report the right 1-based index',
    (tester) async {
      int? tapped;
      await tester.pumpWidget(
        host(
