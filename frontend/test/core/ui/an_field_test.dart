...
void main() {
  ...
  testWidgets('read-only value: no pencil; merged "label: value" semantics', (
    tester,
  ) async {
    final handle = tester.ensureSemantics();
    await tester.pumpWidget(
      host(const AnField(label: 'Kind', value: 'function')),
    );
  ...
  testWidgets('hint renders below the label and is in the SR label', (
    tester,
  ) async {
    final handle = tester.ensureSemantics();
    await tester.pumpWidget(
      host(const AnField(label: 'Timeout', hint: 'seconds', value: '30')),
    );
  ...
  testWidgets('child slot (value == null) renders the control, no pencil', (
    tester,
  ) async {
    await tester.pumpWidget(
      host(
    ...
    expect(
      ...
      findsNothing,
    ); // no edit affordance for a slot field
  });

  testWidgets('editable but null onChanged → read-only (no pencil)', (
    tester,
  ) async {
    await tester.pumpWidget(
      host(const AnField(label: 'Name', value: 'x', editable: true)),
    );
  ...
  testWidgets(
    'editing in a NARROW row with a multi-word label does not overflow (Save stays on-panel)',
    (tester) async {
      ...
      expect(
        ...
        isNull,
        reason: 'editing a long-label field in a narrow row must not overflow',
      );
      ...
      expect(
        saveRight,
        lessThanOrEqualTo(panelRight + 0.5),
        reason: 'Save must stay on-panel, not pushed off the right',
      );
    },
  );
}
