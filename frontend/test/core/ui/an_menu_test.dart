...
void main() {
  ...
  testWidgets(
    'tapping the anchor opens the menu; items + section label render',
    (tester) async {
  ...
  testWidgets('picking an item fires onTap and closes the menu', (
    ...
    await tester.pumpWidget(
      host(
        menu(
          entries: [AnMenuItem(label: 'Edit', onTap: () => picked++)],
        ),
      ),
    );
    await tester.tap(find.text('Open'));
  ...
  testWidgets('keepOpen item stays open after a tap (multi-check toggle)', (
    tester,
  ) async {
    var toggles = 0;
    await tester.pumpWidget(
  ...
  testWidgets('disabled item does not fire / does not close', (tester) async {
    ...
    await tester.pumpWidget(
      host(
        menu(
          entries: [
            AnMenuItem(label: 'Archive', disabled: true, onTap: () => picked++),
          ],
        ),
      ),
    );
  ...
  testWidgets(
    'menu hugs content width — a short item is narrower than the max',
    (tester) async {
      await tester.pumpWidget(
        host(menu(entries: const [AnMenuItem(label: 'Sort')])),
      );
      await tester.tap(find.text('Open'));
      await tester.pumpAndSettle();
      ...
      expect(
        shortW,
        lessThan(AnSize.menuMaxWidth),
        reason: 'hugs content, not always the 360 max',
      );
      expect(shortW, greaterThanOrEqualTo(AnSize.menuMinWidth));
    },
  );

  testWidgets('menu width clamps to the max for a very long label', (
    tester,
  ) async {
    await tester.pumpWidget(
      host(
        menu(
          entries: const [
            AnMenuItem(
              label:
                  'An extremely long menu item label well past the maximum menu width by a wide margin',
            ),
          ],
        ),
      ),
    ...
    expect(
      ...
      AnSize.menuMaxWidth,
      reason: 'long label clamps to max',
    );
  });

  testWidgets(
    'an open menu dismisses when an ancestor scrolls (platform-standard, not stranded)',
    (tester) async {
      ...
      scroll.jumpTo(
        80,
      ); // ancestor scrolls → the menu must dismiss (not float over the wrong content) 祖先滚动→关
      await tester.pumpAndSettle();
      expect(
        find.text('Edit'),
        findsNothing,
        reason: 'scrolling the ancestor dismisses the open menu',
      );
    },
  );
  ...
  testWidgets(
    ...
    (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          ...
          home: Scaffold(
            body: Align(
              alignment: Alignment
                  .topLeft, // trigger hard against the left edge 触发器贴左缘
              child: menu(
                entries: const [
                  AnMenuItem(label: 'Edit'),
                  AnMenuItem(label: 'Delete'),
      ...
      expect(
        ...
        reason:
            'default alignEnd would extend LEFT off-screen → must flip/clamp',
      );
    },
  );

  testWidgets(
    'opening takes focus; closing hands it back to the pre-open holder (WCAG 2.4.3)',
    (tester) async {
        ...
        host(
          Column(
            ...
            children: [
              ...
              AnMenu(
                entries: const [
                  AnMenuItem(label: 'One'),
                  AnMenuItem(label: 'Two'),
                ],
                anchorBuilder: (context, toggle, isOpen) {
                  openMenu = toggle;
      ...
      expect(
        probe.hasFocus,
        isFalse,
        reason: 'overlay FocusScope + first-item autofocus take focus on open',
      );
      ...
      expect(
        ...
        isTrue,
        reason:
            'focus handed back to the trigger context on close, not dropped to root',
      );
    },
  );
  group('AnMenuSurface.caretPlacement 光标锚定落点 (A-104)', () {
    const anchor = Rect.fromLTWH(
      40,
      100,
      2,
      16,
    ); // a caret rect: left 40, top 100, bottom 116
    test('hangs BELOW by default (room below)', () {
      final p = AnMenuSurface.caretPlacement(
        anchor: anchor,
        rows: 3,
        layerHeight: 1000,
      );
