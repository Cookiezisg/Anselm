...
void main() {
  List<Widget> cells(int n) => [
    for (var i = 0; i < n; i++) Container(key: ValueKey('c$i'), height: 20),
  ];
  double cellW(WidgetTester tester, int i) =>
  ...
  Future<void> pumpAt(
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(body: AnAutoGrid(children: children)),
      ),
    );
  }

  testWidgets(
    'fits N equal columns by width (gap-compensated, columns fill the row)',
    (tester) async {
      ...
      for (var i = 0; i < 6; i++) {
        expect(
          cellW(tester, i),
          292.0,
          reason: 'all columns equal width, filling the row',
        );
      }
    },
  );

  testWidgets('collapses to ONE column when narrower than minColWidth', (
    tester,
  ) async {
  ...
  testWidgets(
    'few children in a wide container stretch to fill (auto-fit, not auto-fill)',
    (tester) async {
  ...
  testWidgets('unbounded width → single-column fallback, no crash', (
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: Row(children: [AnAutoGrid(children: cells(3))]),
        ), // Row → unbounded maxWidth
      ),
    );
    expect(tester.takeException(), isNull);
    for (var i = 0; i < 3; i++) {
