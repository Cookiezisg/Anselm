...
void main() {
  ...
  testWidgets(
    'every segment is ONE node that carries its own label (never an unnamed button)',
    (tester) async {
  ...
  testWidgets(
    'only the SELECTED segment says selected; the other says nothing at all',
    (tester) async {
      final handle = tester.ensureSemantics();
      await tester.pumpWidget(host(seg()));
      await tester.pumpAndSettle();
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.tap(
        find.text('Alpha'),
      ); // already selected → deliberately not tappable 已选中段刻意不可点
      expect(picked, [2]);
    },
  );
}
