...
Node n(String id, NodeKind k, {String? ref}) =>
    Node(id: id, kind: k, ref: ref ?? '${k.name}_$id');
Edge e(String id, String from, String to, {String? port}) =>
    Edge(id: id, from: from, fromPort: port, to: to);

final Graph branchGraph = Graph(
  nodes: [
...
void main() {
  Widget host(Widget child, {Size size = const Size(900, 600)}) =>
      TranslationProvider(
        child: MaterialApp(
  ...
  group('AnGraphCanvas', () {
    testWidgets('renders every node card (id + ref) and port pills', (
      tester,
    ) async {
    ...
    testWidgets('auto-fits on first frame: whole content visible, scale capped', (
      tester,
    ) async {
      await tester.pumpWidget(host(AnGraphCanvas(graph: branchGraph)));
      await tester.pump();
      ...
      for (final node in branchGraph.nodes) {
        expect(
          canvasBox.contains(tester.getCenter(find.text(node.id))),
          isTrue,
        );
      }
    });
    ...
    testWidgets('node tap reports id; background tap reports null', (
      ...
      await tester.pumpWidget(
        host(AnGraphCanvas(graph: branchGraph, onNodeTap: (id) => tapped = id)),
      );
      await tester.pump();
      await tester.tap(find.text('run_tests'));
      expect(tapped, 'run_tests');
      await tester.tapAt(
        tester.getRect(find.byType(AnGraphCanvas)).bottomRight -
            const Offset(8, 8),
      );
    ...
    testWidgets(
      'enter-editor action shows only when both label and handler exist',
      (tester) async {
        var entered = false;
        await tester.pumpWidget(
          host(
        ...
        await tester.pumpWidget(
          host(AnGraphCanvas(graph: branchGraph, framed: true)),
        );
        await tester.pump();
      ...
      await tester.pumpWidget(
        host(
          Align(
            ...
            child: AnGraphCanvas(graph: branchGraph, framed: true),
          ),
        ),
      );
      await tester.pump();
    ...
    testWidgets('hostile ids, unknown kind and dangling edges stay safe', (
      ...
      expect(tester.takeException(), isNull);
      expect(
        find.text('<b>not</b> & html'),
        findsOneWidget,
      ); // literal, never parsed literal 渲染
    ...
    testWidgets('swapping in a different graph re-fits (demo setGraph contract)', (
      ...
      expect(
        longK,
        lessThan(smallK),
      ); // 30-node chain must shrink to fit 长链必缩小入框
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.pumpWidget(
          host(AnGraphCanvas(graph: makeGraph()), size: const Size(760, 560)),
        );
        await tester.pump();
        final after = viewOf(tester).getTranslation();
    ...
    testWidgets('wheel zoom keeps the scene point under the cursor still', (
      tester,
    ) async {
      await tester.pumpWidget(host(AnGraphCanvas(graph: branchGraph)));
      await tester.pump();
    ...
    testWidgets('selection ring follows a controlled prop change', (
      tester,
    ) async {
      bool ringed(WidgetTester t, String id) {
        final container = t.widget<Container>(
          find
              .ancestor(of: find.text(id), matching: find.byType(Container))
              .first,
        );
      ...
      await tester.pumpWidget(
        host(
          AnGraphCanvas(graph: branchGraph, selectedNodeId: 'branch_result'),
        ),
      );
      await tester.pump();
    ...
    testWidgets(
      'system text scaling never overflows the geometry-locked cards',
      (tester) async {
        await tester.pumpWidget(
          TranslationProvider(
            child: MaterialApp(
