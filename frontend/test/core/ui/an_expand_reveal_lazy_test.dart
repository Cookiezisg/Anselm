...
void main() {
  ...
  testWidgets('collapsing settles back to zero builds (no per-rebuild body cost)', (tester) async {
    var built = 0;
    Widget host(bool open) => MaterialApp(
          home: AnExpandReveal.builder(
            open: open,
            duration: Duration.zero, // instant → no lingering animation frames 即时收
            childBuilder: (context) {
              built++;
              return const Text('body');
            },
