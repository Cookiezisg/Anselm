...
void main() {
  ...
  testWidgets('value is flush-right (right edge at the row right edge)', (tester) async {
    await tester.pumpWidget(host(const AnLeadValue(
      leading: Text('Name'),
    ...
    expect((rowRight - valueRight).abs(), lessThan(1.0),
        reason: 'value must be flush against the row right edge, not parked mid-row');
  });
  ...
  testWidgets('value uses MORE than half when leading is short (dynamic, not 50/50)', (tester) async {
    const longish = 'a-medium-length-value-well-over-half-the-row-width';
    await tester.pumpWidget(host(const AnLeadValue(
      leading: Text('ID'),
      trailing: Text(longish, maxLines: 1, overflow: TextOverflow.ellipsis),
    )));
    final rowRect = tester.getRect(find.byType(AnLeadValue));
    final valueLeft = tester.getRect(find.text(longish)).left;
    expect(valueLeft, lessThan(rowRect.center.dx),
        reason: 'with a short leading the value should span past the row midpoint — a 50/50 slot would forbid it');
  });
  ...
  testWidgets('long leading ellipsizes (no overflow), value stays flush-right', (tester) async {
    await tester.pumpWidget(host(const AnLeadValue(
      leading: Text('an-extremely-long-leading-label-that-would-eat-the-whole-row-if-it-were-not-capped',
  ...
  testWidgets('afterValue pins far-right, value flush against it', (tester) async {
    await tester.pumpWidget(host(const AnLeadValue(
      leading: Text('Key'),
      trailing: Text('val'),
    ...
    expect(valueRight, lessThanOrEqualTo(afterLeft + 0.5),
        reason: 'the value must sit to the left of the afterValue slot, not overlap it');
  });
  ...
  testWidgets('survives an intrinsic pass — IntrinsicHeight + Row(stretch) + divider (no crash)', (tester) async {
      ...
      IntrinsicHeight(
        child: Row(
          ...
          children: const [
            ...
            Expanded(child: AnLeadValue(leading: Text('Kind'), trailing: Text('function'))),
          ],
        ),
      ),
    ));
    expect(tester.takeException(), isNull, reason: 'AnLeadValue must support intrinsics (no LayoutBuilder)');
    ...
    await tester.pumpWidget(MaterialApp(
      ...
      home: Scaffold(
        body: SingleChildScrollView(
          ...
          child: const AnLeadValue(leading: Text('Name'), trailing: Text('value')),
        ),
      ),
    ));
    expect(tester.takeException(), isNull, reason: 'unbounded width must degrade, not crash');
