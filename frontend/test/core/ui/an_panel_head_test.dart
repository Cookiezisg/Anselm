...
void main() {
  ...
  testWidgets('renders the icon + title; ⋯ opens the menu; ✕ fires onClose', (
    tester,
  ) async {
    var closed = 0;
    var picked = 0;
    ...
    expect(
      find.byIcon(AnIcons.activity),
      findsOneWidget,
    ); // the identity glyph 身份字形
  ...
  testWidgets('empty menuEntries → no ⋯ button (only ✕)', (tester) async {
    ...
    await tester.pump();
    expect(
      find.byIcon(AnIcons.more),
      findsNothing,
    ); // no overflow when there are no panel actions 无动作则无 ⋯
    expect(find.byIcon(AnIcons.close), findsOneWidget);
  });

  testWidgets('sub band renders when provided; absent when null (零人话律)', (
    tester,
  ) async {
    await tester.pumpWidget(
      host(
    ...
    await tester.pumpWidget(
      host(AnPanelHead(icon: AnIcons.activity, title: 'Activity')),
    );
    await tester.pump();
  ...
  testWidgets(
    'geometry lock: icon lands at island + 12 (right-island single-source law)',
    (tester) async {
      await tester.pumpWidget(
        host(AnPanelHead(icon: AnIcons.activity, title: 'PROBE')),
      );
      await tester.pump();
      ...
      expect(
        ...
        reason:
            'panel-head icon flush with the island pad edge (single-source law)',
      );
    },
  );
  ...
  // ✕ 字形落 meta 右缘铁线附近。
  testWidgets(
    'right-edge lock: trailing ✕/⋯ box flush at island edge; ✕ glyph on the meta iron line',
    (tester) async {
      await tester.pumpWidget(
        host(
          Column(
      ...
      final moreBox = find.ancestor(
        of: find.byIcon(AnIcons.more),
        matching: find.byType(AnButton),
      );
      ...
      expect(
        ...
        moreOrLessEquals(AnSpace.s12, epsilon: 1),
        reason:
            '✕ button box flush at the island content edge (no head trailing pad)',
      );

      // (b) the row + ledger meta both land on the iron line = island 12 + row-family s8 (right 缘铁线).
      const ironLine = AnSpace.s12 + AnSpace.s8;
      expect(
        insetOf(find.text('Ran ×2')),
        moreOrLessEquals(ironLine, epsilon: 1.5),
      ...
      final glyphInset =
          islandRight - tester.getRect(find.byIcon(AnIcons.close)).right;
      expect(
        (glyphInset - ironLine).abs(),
        lessThanOrEqualTo(3),
        reason:
            '✕ glyph within 3px of the meta iron line (was 6px inboard of it)',
      );

      // (d) ⋯ rides just left of ✕ (one md-box step) — never the old 8px-further-inboard cluster. ⋯ 紧邻 ✕ 左。
      expect(
        insetOf(moreBox),
        moreOrLessEquals(AnSpace.s12 + AnSize.control, epsilon: 1),
