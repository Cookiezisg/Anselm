...
void main() {
  group('binTrackEvents (pure)', () {
    test(
      'always produces exactly binCount hourly cells with contiguous bounds',
      () {
    ...
    test('a run lands in the hour its startedAt falls into', () {
      ...
      final bins = binTrackEvents(
        ...
        binCount: 25,
        runs: [_run(at, AnStatus.done)],
      );
      final withRun = bins.where((b) => b.runs.isNotEmpty).toList();
      expect(withRun, hasLength(1));
      expect(
        withRun.single.start.isAfter(at) ||
            withRun.single.start.isAtSameMomentAs(at),
        isFalse,
    ...
    test(
      'the bin boundary is HALF-OPEN [start, end): a tick on the edge lands in the LATER bin',
      () {
        ...
        final bins = binTrackEvents(
          ...
          binCount: 25,
          runs: [_run(edge, AnStatus.done)],
        );
        final idx = bins.indexWhere((b) => b.runs.isNotEmpty);
        expect(
          bins[idx].start,
          edge,
          reason: '边界刻度归 [start,end) 的那一格(其 start == 刻度)',
        );
      },
    );

    test(
      'within a bin the WORST status wins — a later green cannot erase a real failure',
      () {
        ...
        expect(
          bin.runs.first.status,
          AnStatus.err,
          reason: '桶内失败在前(=hover 卡读序)',
        );
      },
    );
    ...
    test(
      'anything outside [start, end) is dropped — an unplaceable event is never faked into place',
      () {
        final bins = binTrackEvents(
          start: _start,
          end: _end,
        ...
        expect(
          bins.every((b) => b.runs.isEmpty),
          isTrue,
          reason: '窗外放不下就不渲,绝不硬塞',
        );
      },
    );

    test(
      'a zero/negative window places nothing rather than dividing by it',
      () {
        expect(
          binTrackEvents(
            start: _end,
        ...
        expect(
          binTrackEvents(
            ...
            binCount: 25,
            runs: [_run(_start, AnStatus.done)],
          ),
          isEmpty,
        );
      },
  ...
  group('AnScheduleTrack (widget)', () {
    testWidgets('empty lanes render nothing — no ghost axis, no orphan ruler', (
      tester,
    ) async {
      await tester.pumpWidget(
        _host(AnScheduleTrack(lanes: const [], now: _now)),
      );
      expect(tester.getSize(find.byType(AnScheduleTrack)).height, 0);
    });

    testWidgets(
      'a lane draws 25 whole-hour cells regardless of density (dense = sparse in geometry)',
      (tester) async {
        await tester.pumpWidget(
          _host(
            AnScheduleTrack(
              lanes: [
                _lane(
                  ...
                  runs: [
                    for (var h = 0; h < 24; h++)
                      _run(_now.subtract(Duration(hours: h)), AnStatus.done),
                  ],
                ),
              ],
              now: _now,
    ...
    testWidgets(
      'the fill colour is the WORST outcome; an empty hour is a faint outline, never a colour',
      (tester) async {
        await tester.pumpWidget(
          _host(
            AnScheduleTrack(
              lanes: [
                _lane(
                  ...
                  runs: [
                    ...
                    _run(
                      _now.subtract(const Duration(hours: 3, minutes: 20)),
                      AnStatus.err,
                    ),
                  ],
                ),
        ...
        final filled = decos
            ...
            .toList();
        expect(
          filled.any((d) => d.color == AnStatus.err.tone.softBg(c)),
          isTrue,
    ...
    testWidgets(
      'a missed tick lays a ✕ SHAPE over the cell — not just grey (WCAG 1.4.1)',
      (tester) async {
        await tester.pumpWidget(
          _host(
            AnScheduleTrack(
              lanes: [
                _lane(
                  'a',
                  'A',
                  missed: [_now.subtract(const Duration(hours: 6))],
                ),
              ],
              now: _now,
              onBin: (_, _) {},
        ...
        expect(
          find.byIcon(AnIcons.close),
          findsOneWidget,
          reason: 'missed 必须有 ✕ 形状',
        );
      },
    );

    testWidgets(
      'the future segment: a hollow ○ + words / «已暂停» on a paused lane / blank when none',
      (tester) async {
        await tester.pumpWidget(
          _host(
            AnScheduleTrack(
        ...
        expect(
          find.textContaining('14:32', findRichText: true),
          findsOneWidget,
        );
        expect(
          find.textContaining('每 15 分钟', findRichText: true),
          findsOneWidget,
        );
        // A hollow ○ marks the forecast (a ring, never a solid dot). 空心 ○ 标预告。
        expect(
          find.byType(AnStatusDot),
          findsOneWidget,
          reason: '未来段一枚空心 ○(过去格是 Container 不是点)',
        );
    ...
    testWidgets(
      'a PAUSED lane greys but stays on the board with its name (判决①)',
      (tester) async {
        await tester.pumpWidget(
          _host(
            AnScheduleTrack(
    ...
    group('keyboard: ONE tab stop (roving tabindex)', () {
          ...
          lanes: [
            for (var l = 0; l < 8; l++)
              _lane(
                ...
                future: TrackFuture(
                  at: _now.add(const Duration(minutes: 5)),
                  time: '14:35',
                  relative: '(5m)',
                  schedule: 'x',
                ),
      ...
      testWidgets('8 lanes × (24 cells + future ○) is ONE Tab stop, not 200', (
        tester,
      ) async {
      ...
      testWidgets(
        '←→ walk a lane in time order (bin → future ○); ↑↓ land on the SAME hour of the next lane',
        (tester) async {
          await tester.pumpWidget(
            _host(
              Column(
          ...
          expect(
            FocusManager.instance.primaryFocus?.debugLabel,
            'AnScheduleTrack (b, 24)',
          );
        },
      );

      testWidgets(
        'running off the LAST lane hands focus back to the framework (never trapped)',
        (tester) async {
          await tester.pumpWidget(
            _host(
              Column(
    ...
    testWidgets(
      'launch pad: a CONTENT cell reports its bin; an empty / missed-only cell does NOT',
      (tester) async {
        TrackBin? tapped;
    ...
    testWidgets(
      'column heads slide with the grid and name every hour; midnight wears the date anchor '
      '(v2:列头逐小时标注,0 点=日期锚;三锚刻度眉/now 线已退役)',
      (tester) async {
        await tester.pumpWidget(
          _host(
            AnScheduleTrack(
              lanes: [_lane('a', 'A')],
              now: _now,
              binHeadLabel: (bin) => bin.start.hour == 0
        ...
        expect(
          find.text('14'),
          findsNWidgets(2),
          reason: '每小时自报姓名(25 格跨天,14 时出现两次)',
        );
        expect(find.text('7/16'), findsOneWidget, reason: '0 点格标日期锚');
        expect(
          find
              .byType(CustomPaint, skipOffstage: false)
              .evaluate()
              .where((e) => '${e.widget}'.contains('_TrackFurniture')),
          isEmpty,
          reason: 'now 线 painter 已退役(右锚下右缘即现在)',
        );
      },
    );

    testWidgets(
      'the grid scrolls sideways ANCHORED AT THE NEWEST EDGE; names and forecast stay frozen '
      '(v2:右锚横滚,车道与预告两翼冻结)',
      (tester) async {
        await tester.pumpWidget(
          _host(
        ...
        expect(
          find.descendant(of: scrollable, matching: find.text('泳道甲')),
          findsNothing,
        );
        expect(
          find.descendant(
            of: scrollable,
            matching: find.textContaining('15:00', findRichText: true),
          ),
          findsNothing,
        );
    ...
    testWidgets(
      'a cell is the Matrix-View square (controlSm), not the v1 strip (v2 格视觉并入矩阵)',
      (tester) async {
        await tester.pumpWidget(
          _host(
            AnScheduleTrack(
              lanes: [
                _lane(
                  'a',
                  'A',
                  runs: [_run(DateTime(2026, 7, 16, 14, 10), AnStatus.done)],
                ),
              ],
              now: _now,
            ),
        ...
        expect(
          tester.getSize(cells.first),
          const Size(AnSize.controlSm, AnSize.controlSm),
          reason: '24×24 方格,同矩阵',
        );
      },
    );
    ...
    testWidgets(
      'arrows DRAG THE VIEWPORT: walking left past the visible edge scrolls older hours in '
      '(矩阵立法:defaultTraversalRequestFocusCallback 拖视口)',
      (tester) async {
        await tester.pumpWidget(
          _host(
            AnScheduleTrack(
        ...
        ); // cursor in — the NEWEST bin (visible) 光标入轨=最新格
        await tester.pump();
        ...
        for (var i = 0; i < 15; i++) {
          await tester.sendKeyEvent(LogicalKeyboardKey.arrowLeft);
          await tester
              .pumpAndSettle(); // each step settles its scroll animation 每步滚动动画落定
          final box =
          ...
          expect(
            ...
            reason: '第 ${i + 1} 步:光标格左缘出视口',
          );
          expect(
            cell.right,
            lessThanOrEqualTo(viewport.right + 0.5),
            reason: '第 ${i + 1} 步:光标格右缘出视口',
          );
        }
    ...
    testWidgets(
      'each lane is ONE semantics container reading its summary (§12)',
      (tester) async {
        ...
        expect(
          find.bySemanticsLabel('数据清洗,24 小时 1 次运行'),
          findsOneWidget,
