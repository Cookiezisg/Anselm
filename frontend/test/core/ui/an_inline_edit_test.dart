...
void main() {
  ...
  testWidgets(
    'style + minHeight parameterize the row (H2 title rename, no font jump on toggle)',
    (tester) async {
      ...
      expect(
        tester.getSize(find.byType(AnInlineEdit)).height,
        AnSize.islandHead,
      );
  ...
  testWidgets(
    'editing draws the edit frame (bordered) without growing the row height',
    (tester) async {
      await tester.pumpWidget(
        host(
          AnInlineEdit(
      ...
      expect(
        bordered,
        isNotEmpty,
        reason: 'edit mode shows the bordered frame',
      );
  ...
  testWidgets(
    'H2 edit frame fits the zero-slack production row height (no clip past the row)',
    (tester) async {
      ...
      final style = AnText.h2.weight(FontWeight.w600);
      final minH =
          style.fontSize! * (style.height ?? 1.0) + AnSize.editBoxPadY * 2;
      await tester.pumpWidget(
        host(
          AnInlineEdit(
            value: 'Title',
      ...
      expect(
        ...
        lessThanOrEqualTo(rowH + 0.5),
        reason:
            'the edit frame must not exceed the zero-slack H2 row height (no clip)',
      );
    },
  );
  ...
  testWidgets(
    ...
    (tester) async {
      await tester.pumpWidget(
        host(
          AnInlineEdit(value: 'Hello', startEditing: true, onCommit: (_) {}),
        ),
      );
      await tester.pump();
      final sel = tester
      ...
      expect(
        sel.end,
        'Hello'.length,
      ); // whole value selected — Finder/F2 rename convention 全选
    },
  );

  testWidgets('Enter commits the typed value and returns to idle', (
    tester,
  ) async {
    String? committed;
    await tester.pumpWidget(
  ...
  testWidgets(
    'onAbort fires on Esc and on Cancel (a host can drop its editing state)',
    (tester) async {
      var aborts = 0;
      await tester.pumpWidget(
        host(
  ...
  testWidgets(
    'disabled renders a read-only title — NO edit affordance (no phantom greyed pencil)',
    (tester) async {
      await tester.pumpWidget(
        host(AnInlineEdit(value: 'X', enabled: false, onCommit: (_) {})),
      );
      await tester.pump();
      ...
      expect(
        find.byType(AnInput),
        findsNothing,
      ); // and it never enters edit 且不进编辑
      expect(find.text('X'), findsOneWidget);
    },
  );

  testWidgets(
    'A2 blur-commit ON — an outside tap commits (commitOnTapOutside:true)',
    (tester) async {
      String? committed;
      await tester.pumpWidget(
        host(
      ...
      expect(
        find.byType(AnInput),
        findsNothing,
      ); // back to the resting title 回静态
    },
  );

  testWidgets(
    'A2 blur-commit OFF (default) — an outside tap must NOT silently rename',
    (tester) async {
      String? committed;
      await tester.pumpWidget(
        host(
      ...
      expect(
        committed,
        isNull,
      ); // a stray click must NOT commit — pins the rail/entity rename behaviour 点别处不静默改名
      expect(
        find.byType(AnInput),
        findsOneWidget,
      ); // still editing (only Enter/✓/Esc commit) 仍编辑
    },
  );

  testWidgets(
    'long content caps at the space the affordance leaves, then scrolls — no overflow',
    (tester) async {
      ...
      await tester.pumpWidget(
        host(
          AnInlineEdit(
            value:
                'A very long title being edited that grows past the cap and must scroll, not overflow',
            startEditing: true,
            onCommit: (_) {},
          ),
          width: 220,
      ...
      expect(
        ...
        lessThan(80),
        reason:
            'field must cap at the leftover space, not take its content width',
      );
    },
  );

  testWidgets(
    'empty value while editing keeps a clickable min-width field (no collapse)',
    (tester) async {
      await tester.pumpWidget(
        host(AnInlineEdit(value: '', startEditing: true, onCommit: (_) {})),
      );
      await tester.pumpAndSettle();
      expect(tester.takeException(), isNull);
      expect(find.byType(AnInput), findsOneWidget);
