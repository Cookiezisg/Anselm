...
void main() {
  ...
  testWidgets('tall content scrolls in a single scroll region (no overflow)', (
    tester,
  ) async {
  ...
  testWidgets('content column is clamped to 720 + centered in a wide ocean', (
    tester,
  ) async {
    await tester.pumpWidget(
      host(
    ...
    expect(
      width,
      lessThanOrEqualTo(AnSize.content + 0.5),
      reason: 'content clamped to the 720 max-width',
    );
    expect(
      width,
      lessThan(900),
      reason: 'clamped, not the full ocean width (centered)',
    );
  });
  ...
  group('the 720 reading column is absolute (用户 0717 判决 — 判决③ 全宽破例作废)', () {
    testWidgets(
      'every section is clamped to 720 — a wide child cannot widen the page',
      (tester) async {
        await tester.pumpWidget(
          host(
            AnPage(
        ...
        for (final k in ['a', 'b', 'c']) {
          expect(
            tester.getSize(find.byKey(Key(k))).width,
            lessThanOrEqualTo(AnSize.content + 0.5),
    ...
    testWidgets(
      'the page body scrolls ONLY vertically — it physically cannot scroll sideways',
      (tester) async {
        await tester.pumpWidget(
          host(
            AnPage(
        ...
        expect(
          sv.scrollDirection,
          Axis.vertical,
          reason: 'AnPage body 单轴纵向,横滚在物理上不可能',
        );
        expect(
          find.byType(SingleChildScrollView),
          findsOneWidget,
    ...
    testWidgets(
      'a narrow ocean never overflows (the 720 clamp is a MAX, not a floor)',
      (tester) async {
        await tester.pumpWidget(
          host(
            AnPage(
  ...
  testWidgets(
    'on a desktop target only AnPage\'s RawScrollbar paints (no inherited Material Scrollbar)',
    (tester) async {
      ...
      try {
        ...
        expect(
          ...
          findsNothing,
          reason: 'inherited Material Scrollbar suppressed',
        );
        expect(
          find.byType(RawScrollbar),
          findsOneWidget,
          reason: 'only AnPage\'s deliberate overlay bar',
        );
      } finally {
