...
void main() {
  Widget host(Widget child, {double width = 320, bool reduced = false}) =>
      MaterialApp(
  ...
  testWidgets(
    'types progressively; a11y exposes the FULL phrase, not the half-typed string',
    (tester) async {
      final handle = tester.ensureSemantics();
      await tester.pumpWidget(
        host(const AnTypewriter(['Hello world'], loop: false)),
      );
      await tester.pump();
      await tester.pump(
      ...
      expect(
        tester.getSemantics(find.byType(AnTypewriter)).label,
        'Hello world',
      );
      ...
      ); // let the controller complete + stop
    },
  );

  testWidgets(
    'reduced-motion shows the full primary phrase, steady, and settles',
    (tester) async {
      await tester.pumpWidget(
        host(
          const AnTypewriter(['Welcome', 'Second'], loop: true),
          reduced: true,
        ),
      );
  ...
  testWidgets(
    'grapheme-safe: an emoji phrase types without splitting a cluster',
    (tester) async {
      const phrase = 'Hi 👋🏽 ok';
  ...
  testWidgets(
    'onDone fires once when a non-looping run finishes (after type + hold)',
    (tester) async {
      var done = 0;
      await tester.pumpWidget(
        host(AnTypewriter(const ['Hi'], loop: false, onDone: () => done++)),
      );
      await tester.pump();
      await tester.pump(
        const Duration(milliseconds: 300),
      ); // typing (2 chars) + not yet held out 打字中
      expect(done, 0);
      await tester.pump(
        const Duration(milliseconds: 1600),
      ); // hold ends → completed → post-frame 完成
      await tester.pump();
      expect(done, 1);
      await tester.pump(
        const Duration(seconds: 1),
      ); // stays 1 — no re-fire 不重触发
      expect(done, 1);
    },
  );

  testWidgets(
    'onDone fires under reduced motion (static render is the finished state)',
    (tester) async {
      var done = 0;
      await tester.pumpWidget(
        host(
          AnTypewriter(const ['Hi'], loop: false, onDone: () => done++),
          reduced: true,
        ),
      );
      await tester.pump();
