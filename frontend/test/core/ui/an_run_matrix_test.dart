...
Finder get _cells =>
    find.byWidgetPredicate((w) => w is AnInteractive && w.onTap != null);
...
void main() {
  testWidgets('empty rows/cols render nothing', (tester) async {
    await tester.pumpWidget(
      _host(
        AnRunMatrix(rows: const [], cols: const [], cellStatus: (_, _) => null),
      ),
    );
    expect(
      tester.getSize(find.byType(AnRunMatrix)).height,
      0,
      reason: '零行零列=零高,不留空框废墟',
    );
  });

  testWidgets(
    'a SPARSE cell renders an empty square — never a colour, never a lie',
    (tester) async {
      await tester.pumpWidget(
        _host(
          AnRunMatrix(
            ...
            cellStatus: (runId, nodeId) =>
                runId == 'fr_2' && nodeId == 'analyze'
                ? null
      ...
      expect(
        find.byTooltip('未及'),
        findsOneWidget,
        reason: '稀疏格说「未及」,不说成功也不说失败',
      );
    },
  );

  testWidgets(
    'a still-running column grows NO duration bar (no zero that reads as «instant»)',
    (tester) async {
      await tester.pumpWidget(
        _host(
          AnRunMatrix(
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(colors, {
        AnStatus.done.tone.softBg(c),
        AnStatus.err.tone.softBg(c),
      }, reason: '条=最终状态淡色(与格子同族)——完成淡绿/失败淡红,绝不再是无差别的淡蓝');
    },
  );

  testWidgets(
    'the selection indicator lives ON TOP and reserves its pixels when idle '
    '(选中现墨杠,平时透明占位不跳变)',
    (tester) async {
      Widget grid({String? selectedRun}) => _host(
        AnRunMatrix(
          rows: const [MatrixRowHead(nodeId: 'x')],
          cols: _cols,
          cellStatus: (_, _) => const MatrixCellState(status: 'completed'),
      ...
      final c = tester.element(find.byType(AnRunMatrix)).colors;
      Finder inkBar() => find.byWidgetPredicate(
        (w) =>
            w is Container &&
  ...
  testWidgets(
    '«×N» rides IN the cell for loop iterations; a single round grows no badge',
    (tester) async {
      await tester.pumpWidget(
        _host(
          AnRunMatrix(
      ...
      expect(
        find.text('3'),
        findsNWidgets(2),
        reason: 'iterations>1 才渲 ×N(两列各一格)',
      );
  ...
  testWidgets(
    'an UNKNOWN status folds neutral — a cell never paints a success it never had',
    (tester) async {
      ...
      await tester.pumpWidget(
        _host(
          AnRunMatrix(
            rows: const [MatrixRowHead(nodeId: 'x')],
            cols: const [
              RunColumn(id: 'fr_1', status: 'completed', elapsedMs: 10),
            ],
            cellStatus: (_, _) =>
                const MatrixCellState(status: 'SOME_FUTURE_WORD'),
          ),
        ),
      );
  ...
  group('three selection grains (§12)', () {
    testWidgets('tapping a CELL reports (run, node)', (tester) async {
      ...
      expect(got, (
        'fr_1',
        'fetch',
      ), reason: '点格必须报出 (run, node) 两个键——一个都不能少,且不能对调');
    });

    testWidgets(
      'tapping a ROW head reports the node; tapping a COL head reports the run',
      (tester) async {
        String? row;
        String? col;
        await tester.pumpWidget(
    ...
    testWidgets(
      'selection highlights the picked cell / row without moving anything',
      (tester) async {
        await tester.pumpWidget(
          _host(
            AnRunMatrix(
        ...
        expect(
          tester.getRect(find.text('analyze')),
          before,
  ...
  group('a11y: row-level, because the desktop transports no table (§12)', () {
    testWidgets(
      'rows carry the PATTERN as a summary; only the cursor + the selection speak',
      (tester) async {
        ...
        await tester.pumpWidget(
          _host(
            AnRunMatrix(
              rows: _rows,
              cols: _cols,
              cellStatus: (_, nodeId) => nodeId == 'fetch'
                  ? const MatrixCellState(status: 'completed')
                  : null,
              onCell: (_, _) {},
              onCol: (_) {},
              ...
              coordinateLabel: (r, rc, c, cc) =>
                  '第 ${r + 1} 行 共 $rc 行，第 ${c + 1} 列 共 $cc 列',
            ),
          ),
        );
        ...
        expect(
          find.bySemanticsLabel('fetch fr_2 completed 第 1 行 共 2 行，第 2 列 共 2 列'),
          findsOneWidget,
          reason:
              '光标格必须报出坐标——桌面 role/indexInParent 全不过 embedder ABI,坐标只能在 label 里',
        );
        // Every other square is SILENT: 480 unstructured nodes is a wall, not access.
        // 其余方块**沉默**:480 个无结构节点是墙,不是可访问性。
        expect(
          find.bySemanticsLabel('analyze fr_2 未及'),
          findsNothing,
          reason: '非光标非选区的格不带节点——替它们说话的是行摘要',
        );
        expect(
          find.bySemanticsLabel('run fr_2 running'),
          findsOneWidget,
          reason: '列头各是一次 run,是新闻',
        );
    ...
    testWidgets(
      'the SELECTED cell keeps its node even when the cursor is elsewhere',
      (tester) async {
        final handle = tester.ensureSemantics();
        await tester.pumpWidget(
          _host(
        ...
        expect(
          find.bySemanticsLabel('analyze fr_1 第 2 行，第 1 列'),
          findsOneWidget,
        );
        handle.dispose();
      },
    ...
    testWidgets('the cursor label rides the FOCUSABLE node, not a child of it', (
      tester,
    ) async {
      final handle = tester.ensureSemantics();
      await tester.pumpWidget(
      ...
      expect(
        tester.getSemantics(find.bySemanticsLabel('fetch fr_1 completed')),
  ...
  group('keyboard: ONE tab stop (roving tabindex)', () {
    ...
    testWidgets('a 20×24 grid is ONE Tab stop, not 527', (tester) async {
      await tester.pumpWidget(
        _host(
          AnRunMatrix(
            rows: [
              for (var i = 0; i < 24; i++) MatrixRowHead(nodeId: 'node_$i'),
            ],
            cols: [
              for (var i = 20; i > 0; i--)
                RunColumn(id: 'fr_$i', status: 'completed', elapsedMs: i),
            ],
            cellStatus: (_, _) => const MatrixCellState(status: 'completed'),
            onCell: (_, _) {},
            onCol: (_) {},
    ...
    testWidgets(
      'Tab lands ON the cursor cell, and the NEXT Tab leaves the grid entirely',
      (tester) async {
        await tester.pumpWidget(
          _host(
            Column(
    ...
    testWidgets(
      'arrows walk the cursor; a move never selects (ARIA keeps the two axes apart)',
      (tester) async {
        ...
        expect(picked, [
          ('fr_1', 'analyze'),
        ], reason: 'Enter 激活光标格,报出 (run, node)');
      },
    );

    testWidgets(
      'the grid NEVER traps: an arrow off the edge hands focus back to the framework',
      (tester) async {
        await tester.pumpWidget(
          _host(
            Column(
    ...
    testWidgets(
      'the cursor is addressed by ID: a new run prepending a column cannot slide it',
      (tester) async {
        Widget grid(List<RunColumn> cols) => _host(
          AnRunMatrix(
            rows: _rows,
            cols: cols,
            cellStatus: (_, _) => const MatrixCellState(status: 'completed'),
    ...
    testWidgets(
      'arrows are VISUAL: under RTL a Row mirrors, so → walks the other way',
      (tester) async {
        await tester.pumpWidget(
          _host(
            Directionality(
    ...
    testWidgets(
      'an inert grid (no handlers) is a picture — zero stops, no ghost cursor',
      (tester) async {
        await tester.pumpWidget(
          _host(
            AnRunMatrix(
  ...
  group('a11y announcement: macOS only, and that is not a preference', () {
    ...
    testWidgets(
      'macOS: the cursor announces itself — its engine bridge drops FOCUS_CHANGED',
      (tester) async {
    ...
    testWidgets(
      'windows/linux: NOT announced — the focus notification already read it once',
      (tester) async {
        final said = hook(tester);
        await tester.pumpWidget(grid());
        await tester.sendKeyEvent(LogicalKeyboardKey.tab);
  ...
  testWidgets(
    'stress: the full 20-column cap × 24 rows lays out without overflow',
    (tester) async {
      await tester.pumpWidget(
        _host(
          AnRunMatrix(
            rows: [
              for (var i = 0; i < 24; i++)
                MatrixRowHead(nodeId: 'node_$i', kind: 'action'),
            ],
            cols: [
              for (var i = 20; i > 0; i--)
                RunColumn(
      ...
      expect(
        ...
        isNull,
        reason: '20 列 × 24 行=格阵上限,在 720 阅读列内必须不溢出',
      );
    },
  );
  ...
  group('width lives inside the grid (用户 0717 判决 — 判决③ 全宽破例作废)', () {
    ...
    Widget grid({
      void Function(String, String)? onCell,
      VoidCallback? onNearOldestEdge,
    ...
    Finder hBar() => find.byWidgetPredicate(
      (w) => w is RawScrollbar && w.child is ScrollConfiguration,
    );

    testWidgets(
      'the grid scrolls sideways in its OWN container — the host never does',
      (tester) async {
        ...
        // 宿主(页面替身)的滚动器仍是纵向的。
        final hostSv = tester.widget<SingleChildScrollView>(
          find
              .ancestor(
                of: find.byType(AnRunMatrix),
                matching: find.byType(SingleChildScrollView),
    ...
    testWidgets(
      'the bar IS the discoverability: it paints when there is more, and only then',
      (tester) async {
        ...
        var painted = tester
            ...
            .controller!;
        expect(
          painted.position.maxScrollExtent,
          greaterThan(0),
          reason: '真有得滚→thumb 有内容可画',
        );
    ...
    testWidgets(
      'exactly ONE thumb — the local AnScrollBehavior kills the inherited desktop bar',
      (tester) async {
    ...
    void expectCursorVisible(WidgetTester tester, String why) {
      ...
      expect(
        cell.left,
        greaterThanOrEqualTo(viewport.left - 0.5),
        reason: '$why:光标格左缘跑到视口左外',
      );
      expect(
        cell.right,
        lessThanOrEqualTo(viewport.right + 0.5),
        reason: '$why:光标格右缘跑到视口右外',
      );
    }
    ...
    for (final w in [640.0, 300.0]) {
      testWidgets(
        'arrow keys drag the viewport along — the cursor is NEVER walked out of sight (host ${w.toInt()})',
        (tester) async {
          ...
          for (var i = 0; i < 19; i++) {
            ...
            expectCursorVisible(
              tester,
              '第 ${i + 1} 次左移',
            ); // every step, not just the end 每一步
          }
    ...
    testWidgets('first frame anchors at the NEWEST end — no post-frame jump', (
      tester,
    ) async {
      await tester.pumpWidget(_host(grid(), width: 640));
      ...
      final newest = tester.getRect(
        find.byKey(const ValueKey(('node_0', 'fr_20'))),
      );
      final viewport = tester.getRect(
        find.descendant(
          of: find.byType(AnRunMatrix),
      ...
      expect(
        find.byKey(const ValueKey(('node_0', 'fr_1'))).hitTestable(),
        findsNothing,
    ...
    testWidgets(
      'nearing the OLDEST edge fires onNearOldestEdge ONCE per approach, and re-arms',
      (tester) async {
        var fired = 0;
        ...
        ctl.jumpTo(
          max - 8,
        ); // jitter inside the threshold — must NOT refire 阈内抖动不复发
        await tester.pump();
    ...
    testWidgets(
      'prepending an OLDER page moves NOTHING on screen (reverse-scroll geometry)',
      (tester) async {
        ...
        await tester.pumpWidget(
          _host(grid(cols: cols(12)), width: 640),
        ); // 19 cols, overflows 溢出
        final ctl = hCtl(tester);
        ...
        final rectBefore = tester.getRect(
          find.byKey(const ValueKey(('node_0', 'fr_25'))),
        );

        await tester.pumpWidget(
          _host(grid(cols: cols(1)), width: 640),
        ); // +11 older cols 前插十一列
        await tester.pump();
        expect(hCtl(tester).offset, offsetBefore, reason: '前插旧页 offset 一动不动');
        expect(
          tester.getRect(find.byKey(const ValueKey(('node_0', 'fr_25')))),
          rectBefore,
    ...
    testWidgets('loadingOlder renders the working spinner at the oldest edge', (
      tester,
    ) async {
