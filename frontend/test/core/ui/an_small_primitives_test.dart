...
void main() {
  ...
  testWidgets('AnHeatBar width scales with fraction and floors at 0.15', (
    tester,
  ) async {
  ...
  testWidgets('AnQuoteBar renders its child under a left rule', (tester) async {
    ...
    expect(
      (deco.border! as Border).left.width,
      AnSize.quoteBar,
    ); // the quote-bar rule 引用条宽
  });

  testWidgets('AnHoverSurface fills surfaceHover only when active', (
    tester,
  ) async {
    await tester.pumpWidget(
      host(const AnHoverSurface(active: true, child: Text('a'))),
    );
    expect(
      decoOf(tester, find.byType(AnHoverSurface))!.color,
      AnColors.light.surfaceHover,
    );

    await tester.pumpWidget(
      host(const AnHoverSurface(active: false, child: Text('b'))),
    );
    expect(
      decoOf(tester, find.byType(AnHoverSurface))!.color,
      isNull,
    ); // transparent at rest 静息透明
  });

  testWidgets(
    'AnTonedPanel carries the tone border at the card radius on surface',
    (tester) async {
      await tester.pumpWidget(
        host(
          AnTonedPanel(
            borderColor: AnColors.light.danger,
      ...
      expect(
        ...
        AnColors.light.danger,
      ); // the semantic tone edge tone 边
      expect(
        deco.borderRadius,
        BorderRadius.circular(AnRadius.card),
      ); // card-16, not chip-12 机器窗圆角
    },
  );

  testWidgets(
    'AnPopSurface is the floating white island: surface + chip radius + line hairline + shadowPop',
    (tester) async {
      ...
      expect(
        deco.borderRadius,
        BorderRadius.circular(AnRadius.chip),
      ); // chip radius
      expect(
        (deco.border! as Border).top.color,
        AnColors.light.line,
  ...
  testWidgets(
    'AnWashHighlight renders its child; reduced-motion collapses to the end (no wash)',
    (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          theme: AnTheme.light(),
          home: const MediaQuery(
            ...
            child: Scaffold(
              body: Center(child: AnWashHighlight(child: Text('landed'))),
            ),
          ),
        ),
      );
