...
void main() {
  ...
  Widget speaker(void Function(BuildContext) speak) => Builder(builder: (context) {
  ...
  group('announce — new information, all platforms', () {
    testWidgets('posts the message politely by default', (tester) async {
      ...
      await tester.pumpWidget(host(speaker((c) => AnA11y.announce(c, 'Saved'))));
    ...
    testWidgets('carries assertive through to the channel', (tester) async {
      final handle = tester.ensureSemantics();
      final said = probeAnnouncements(tester);
      await tester.pumpWidget(host(speaker(
          (c) => AnA11y.announce(c, 'Boom', assertiveness: Assertiveness.assertive))));
      await tester.pumpAndSettle();
  ...
  group('announceFocusMove — a macOS-only patch, not a preference', () {
    testWidgets('macOS: pushes, because its bridge drops FOCUS_CHANGED', (tester) async {
      final handle = tester.ensureSemantics();
      final said = probeAnnouncements(tester);
      await tester.pumpWidget(host(speaker((c) => AnA11y.announceFocusMove(c, 'Row 2'))));
      await tester.pumpAndSettle();
      expect(said.map((a) => a.toString()), ['polite: Row 2'],
          reason: 'macOS 上被聚焦的节点是哑的,不推则光标移动全程无声');
      handle.dispose();
    }, variant: TargetPlatformVariant.only(TargetPlatform.macOS));

    testWidgets('windows/linux: silent — the focus notification already read it', (tester) async {
      final handle = tester.ensureSemantics();
      final said = probeAnnouncements(tester);
      await tester.pumpWidget(host(speaker((c) => AnA11y.announceFocusMove(c, 'Row 2'))));
      await tester.pumpAndSettle();
  ...
  test('no widget writes Semantics(selected:) by hand — AnA11y.selected is the only ruling', () {
    const seam = 'lib/core/ui/an_a11y.dart';
    final offenders = <String>[];
    for (final f in Directory('lib').listSync(recursive: true).whereType<File>()) {
      if (!f.path.endsWith('.dart') || f.path.endsWith('.g.dart') || f.path.endsWith('.freezed.dart')) {
        continue;
      }
      ...
      for (final m in RegExp(r'Semantics\(').allMatches(src)) {
        ...
        final sel = RegExp(r'(?<![\w.])selected:\s*([^,\n]*)').firstMatch(args);
        if (sel == null) continue;
        if (sel.group(1)!.contains('AnA11y.selected')) continue;
        offenders.add('${f.path}: Semantics(selected: ${sel.group(1)!.trim()})');
      }
    }
    expect(offenders, isEmpty,
        reason: 'Semantics 的 selected 只许经 AnA11y.selected 出闸(否则 false 会被念成「已选中」);\n'
  ...
  group('selected — «say no by not saying» (pinned-engine workaround)', () {
    ...
    test('false becomes null — an explicit false is announced as SELECTED on mac/win',
        () => expect(AnA11y.selected(false), isNull));
    test('null stays null — «selection is not a concept here»',
