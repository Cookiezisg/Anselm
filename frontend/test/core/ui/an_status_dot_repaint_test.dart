...
void main() {
  ...
  testWidgets('a running dot is repaint-isolated; a static dot is not', (
    tester,
  ) async {
    ...
    expect(
      withRun,
      greaterThan(withStatic),
      reason:
          'the breathing run dot must be repaint-isolated; the static dot must not pay for a layer',
    );
  });

  testWidgets(
    'reduced-motion collapses the run dot to a static, un-isolated face',
    (tester) async {
      await tester.pumpWidget(
        host(const AnStatusDot(AnStatus.run), reduced: true),
      );
