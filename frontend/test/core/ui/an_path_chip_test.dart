...
void main() {
  testWidgets('shows the basename, not the full path', (tester) async {
    await tester.pumpWidget(_host(const AnPathChip(path: '/Users/x/project/src/parser/lexer.dart')));
    await tester.pumpAndSettle();
    expect(find.text('lexer.dart'), findsOneWidget);
    expect(find.text('/Users/x/project/src/parser/lexer.dart'), findsNothing); // full path only on hover
  });

  testWidgets('a trailing slash / no slash / partial path degrade gracefully', (tester) async {
  ...
  testWidgets('tap copies the FULL path (not the basename)', (tester) async {
    ...
    await tester.pump(const Duration(seconds: 1)); // let the ✓-revert timer fire before teardown
