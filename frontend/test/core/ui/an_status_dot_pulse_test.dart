...
void main() {
  Widget host(Widget child, {bool reduced = false, bool tickerOn = true}) =>
      MaterialApp(
  ...
  testWidgets(
    'a static dot does ZERO animation work: clock never starts, no frames requested',
    (tester) async {
  ...
  testWidgets(
    'a run dot breathes on arrival, then degrades past idleAfter and goes fully silent',
    (tester) async {
      final clock = PulseClock(
      ...
      expect(
        clock.idle,
        isFalse,
      ); // arrival poked the clock — breathing 出现即起搏,在呼吸
      ...
      expect(
        ...
        isFalse,
        reason:
            'past idleAfter a lone run dot must request nothing — the T1 cure IS the stop',
      );
    },
  );

  testWidgets(
    'a rebuild reaching a live run dot re-arms the idle window (stream heartbeat)',
    (tester) async {
      final clock = PulseClock(
        period: const Duration(milliseconds: 100),
        idleAfter: const Duration(milliseconds: 300),
      ...
      ); // t=450 > original deadline 300 原线已过
      expect(clock.idle, isFalse, reason: 'activity must sustain the breath');
      await tester.pump(
        const Duration(seconds: 1),
      ); // quiet — now it rests 安静后归息
      expect(clock.idle, isTrue);
    },
  );

  testWidgets(
    'reduced motion: a run dot neither subscribes nor pokes (double gate)',
    (tester) async {
      final clock = PulseClock();
      await tester.pumpWidget(
        host(AnStatusDot(AnStatus.run, clock: clock), reduced: true),
      );
      await tester.pump(const Duration(seconds: 1));
  ...
  testWidgets(
    'off-stage (TickerMode off): a run dot stays static — no poke, no subscription',
    (tester) async {
      final clock = PulseClock();
      await tester.pumpWidget(
        host(AnStatusDot(AnStatus.run, clock: clock), tickerOn: false),
      );
      await tester.pump(const Duration(seconds: 1));
      expect(clock.idle, isTrue);
      expect(
