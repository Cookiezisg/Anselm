...
void main() {
  ...
  Future<TestGesture> scrollBy(WidgetTester tester, Offset anchor, int frames) async {
  ...
  testWidgets('idle: enter/exit pass straight through (no ancestor motion)', (tester) async {
    final log = <String>[];
    ...
    await tester.pump();
    expect(log, ['enter'], reason: 'over the target, nothing scrolling → immediate enter');

    await mouse.moveTo(const Offset(400, 560)); // off the target, over the bottom spacer
    await tester.pump();
    expect(log, ['enter', 'exit'], reason: 'leaving the target while idle → immediate exit');
  });

  testWidgets('scrolling: the exit is FROZEN (not dispatched mid-scroll)', (tester) async {
    final log = <String>[];
    final controller = ScrollController();
    addTearDown(controller.dispose);
    await tester.pumpWidget(harness(controller: controller, log: log));
    ...
    expect(log, ['enter'], reason: 'exit must be deferred while the ancestor is scrolling');

    await g.panZoomEnd();
    await mouse.removePointer(); // avoid a settle-time flush racing the assertion above 先断指针免落定 flush 抢跑
    await tester.pumpAndSettle();
  });

  testWidgets('settle: the frozen exit FLUSHES once motion stops', (tester) async {
    final log = <String>[];
    final controller = ScrollController();
    addTearDown(controller.dispose);
    await tester.pumpWidget(harness(controller: controller, log: log));
    ...
    expect(log, ['enter', 'exit'], reason: 'the deferred exit must flush exactly once on settle');
  });

  testWidgets('re-bind: a swapped ScrollPosition is tracked, so the freeze still engages', (tester) async {
    final log = <String>[];
    ...
    final g = await scrollBy(tester, centre, 5);
    expect(log, ['enter'],
        reason: 'the freeze must follow the swapped ScrollPosition (c2), not the stale c1');
    await g.panZoomEnd();
    await tester.pumpAndSettle();
    expect(log, ['enter', 'exit'], reason: 'and still flush on settle after the re-bind');
  });
}
