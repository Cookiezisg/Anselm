...
void main() {
  ...
  testWidgets('renders rows; read-only value shows; empty → em-dash', (
    tester,
  ) async {
  ...
  testWidgets(
    'read-only row is a single merged "label: value" semantics node',
    (tester) async {
      final handle = tester.ensureSemantics();
      await tester.pumpWidget(
        host(const AnKv(rows: [AnKvRow('Type', 'function')])),
      );
  ...
  testWidgets(
    'editable row: pencil → edit → Enter emits the whole list with that row updated',
    (tester) async {
      ...
      ); // only the editable row has a pencil
      await tester.pump();
      await tester.enterText(find.byType(TextField), 'new');
      await tester.testTextInput.receiveAction(TextInputAction.done);
      await tester.pumpAndSettle();
      expect(emitted, isNotNull);
      expect(emitted!.map((r) => r.value), [
        'new',
        'function',
  ...
  testWidgets('null onChanged → all rows read-only even if marked editable', (
    tester,
  ) async {
    await tester.pumpWidget(
      host(const AnKv(rows: [AnKvRow('Name', 'x', editable: true)])),
    );
    expect(
      find.byIcon(AnIcons.edit),
      findsNothing,
    ); // no pencil without onChanged
  });

  testWidgets(
    'mono value uses mono+tabular style; a NON-mono value is still tabular (值列 tabular 铁律)',
    (tester) async {
      await tester.pumpWidget(
        host(const AnKv(mono: true, rows: [AnKvRow('Run', 'run_3a9f0e88')])),
      );
      final mono = tester.widget<Text>(find.text('run_3a9f0e88'));
      expect(
        mono.style?.fontFamily,
        AnText.mono.fontFamily,
        reason: 'mono value uses the monospace family',
      );
      expect(
        mono.style?.fontFeatures,
        contains(const FontFeature.tabularFigures()),
        reason: 'mono value is tabular',
      );
      // The value column is tabular UNCONDITIONALLY — even without mono (the documented 铁律). 非 mono 也 tabular。
      await tester.pumpWidget(
        host(const AnKv(rows: [AnKvRow('Name', 'normalize')])),
      );
      final plain = tester.widget<Text>(find.text('normalize'));
      expect(
        plain.style?.fontFeatures,
        contains(const FontFeature.tabularFigures()),
  ...
  testWidgets('values of editable + read-only rows share one flush-right column', (
    tester,
  ) async {
        ...
        AnKv(
          ...
          rows: const [
            AnKvRow(
              ...
              'normalize-input',
              editable: true,
            ), // editable (pencil) display value
            AnKvRow('Created', '2026-06-24'), // read-only value
          ],
        ),
      ),
    );
    final editableValueRight = tester
        .getRect(find.text('normalize-input'))
        .right;
    final readonlyValueRight = tester.getRect(find.text('2026-06-24')).right;
    expect(
      (editableValueRight - readonlyValueRight).abs(),
      lessThan(1.0),
