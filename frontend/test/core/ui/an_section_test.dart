...
void main() {
  ...
  testWidgets(
    'caption: visual label is uppercased, header reads original case',
    (tester) async {
      final handle = tester.ensureSemantics();
      await tester.pumpWidget(
        host(const AnSection(label: 'inputs', children: [Text('body')])),
      );
      expect(find.text('INPUTS'), findsOneWidget); // visual uppercased
      expect(find.text('body'), findsOneWidget);
      final node = tester.getSemantics(
        find.bySemanticsLabel('inputs'),
      ); // SR hears original case, not "INPUTS"
      expect(node.flagsCollection.isHeader, isTrue);
      handle.dispose();
    },
  );

  testWidgets('plain: document-tier heading (not uppercased), header flagged', (
    tester,
  ) async {
    final handle = tester.ensureSemantics();
    await tester.pumpWidget(
    ...
    expect(
      tester
          .getSemantics(find.bySemanticsLabel('Schedule'))
          .flagsCollection
          .isHeader,
      isTrue,
    );
  ...
  testWidgets('actions render at the head right; body children present', (
    tester,
  ) async {
    await tester.pumpWidget(
      host(
    ...
    (tester) async {
      ...
      await tester.pumpWidget(
        host(
          const AnSection(
            label: 'I/O',
            semanticLabel: 'Inputs and outputs',
            children: [Text('b')],
          ),
        ),
      );
  ...
  testWidgets(
    'head reading order: the header (label) is read BEFORE the actions (WRK-038 §G3-a invariant)',
    (tester) async {
      final handle = tester.ensureSemantics();
      await tester.pumpWidget(
        host(
      ...
      expect(
        iAdd,
        greaterThan(iHeader),
        reason: 'label-Expanded physically precedes trailing actions',
      );
      handle.dispose();
    },
  );
  ...
  testWidgets(
    'empty-string label with actions → actions-only head, no phantom empty header',
    (tester) async {
      final handle = tester.ensureSemantics();
      await tester.pumpWidget(
        host(
