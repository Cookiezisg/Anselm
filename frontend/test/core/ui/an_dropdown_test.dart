...
void main() {
  ...
  testWidgets(
    'the menu uses the shared AnMenuSurface + AnMenuRow standard (rows match AnMenu)',
    (tester) async {
      await tester.pumpWidget(
        host(AnDropdown<String>(options: opts, value: 'a', onChanged: (_) {})),
      );
      await tester.tap(find.byType(AnDropdown<String>));
      await tester.pumpAndSettle();
  ...
  testWidgets('menu is keyboard-navigable — arrow focuses a row, Enter selects', (
    tester,
  ) async {
    ...
    expect(
      picked,
      isNotNull,
      reason: 'arrow-down then Enter should select a row',
    );
  });
  ...
  testWidgets('empty menu explains why there is nothing to choose', (
    ...
    await tester.pumpAndSettle();
    expect(find.byType(AnMenuSurface), findsOneWidget);
    expect(find.text('No MCP servers configured yet'), findsOneWidget);
    expect(picked, isNull);
    expect(tester.takeException(), isNull);
  });

  testWidgets(
    'block dropdown in a wide container opens menu without non-normalized constraints',
    (tester) async {
  ...
  testWidgets('ghost (compact trigger) menu fits rich rows — no overflow', (
    tester,
  ) async {
  ...
  testWidgets('massive option list opens and scrolls without overflow', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      host(AnDropdown<String>(options: many, value: '0', onChanged: (_) {})),
    );
    await tester.tap(find.byType(AnDropdown<String>));
    await tester.pumpAndSettle();
    expect(tester.takeException(), isNull);
