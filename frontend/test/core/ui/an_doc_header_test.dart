...
void main() {
    ...
    await tester.pumpWidget(
      host(
        const AnDocHeader(
          ...
          tags: [
            'x',
          ], // even with tags present, showTags:false suppresses the row 即便有标签也不渲
        ),
      ),
    );
  ...
  testWidgets(
    'document face: tags row renders when editable (onMetaChanged given)',
    (tester) async {
      await tester.pumpWidget(
        host(
          AnDocHeader(
  ...
  testWidgets('read-only (no onMetaChanged) + no tags → no tags row', (
    tester,
  ) async {
    await tester.pumpWidget(
      host(const AnDocHeader(crumbs: [AnCrumb('Documents')], name: 'Doc')),
    );
  ...
  testWidgets('title rename fires onMetaChanged {name: ...}', (tester) async {
    ...
    final titlePencil = find.descendant(
      of: find.byType(AnInlineEdit).first,
  ...
  testWidgets(
    'a non-empty title/description/tags render VALUES, not the guides',
    (tester) async {
      await tester.pumpWidget(
        host(
          AnDocHeader(
  ...
  testWidgets('tapping the dummy add-tag pill opens the AnTags input field', (
    tester,
  ) async {
    await tester.pumpWidget(
      host(
