...
void main() {
  ...
  (String, TextStyle?, GestureRecognizer?) spanWhere(
    WidgetTester tester,
    String needle,
  ...
  group('links', () {
    ...
    testWidgets(
      'allowed scheme fires the callback exactly once; style is accent',
      (tester) async {
        final taps = <String>[];
    ...
    testWidgets(
      'javascript:/data:/file:/weird/relative are inert (callback never fires)',
      (tester) async {
        final taps = <String>[];
        const md =
            '[jsA](javascript:alert(1))\n\n[jsB](JAVASCRIPT:x)\n\n[dataC](data:text/html,x)\n\n'
            '[fileD](file:///etc/passwd)\n\n[weirdE](weird://x)\n\n[relF](/relative)';
        await tester.pumpWidget(
          host(AnMarkdown(md, onLinkTap: (u, t) => taps.add(u))),
        );
        for (final label in [
          'jsA',
          'jsB',
  ...
  group('two-weight rule', () {
    testWidgets(
      '**bold** lands w400 AND the wght axis (the axis is what actually renders)',
      (tester) async {
        ...
        expect(
          ...
          reason:
              'without the axis the pinned wght(300) wins and bold renders as w300',
        );
    ...
    testWidgets(
      '# heading downshifts to readingH1 22/w400 (never 32, never >w400)',
      (tester) async {
  ...
  group('scale (reading vs embedded)', () {
    ...
    testWidgets(
      ...
      (tester) async {
        const md = '# H1\n\n## H2\n\n### H3\n\n#### H4\n\nplain body';
        await tester.pumpWidget(
          host(const AnMarkdown(md, scale: AnMarkdownScale.embedded)),
        );
    ...
    testWidgets(
      'block gap tightens one tier: reading 12 (AnFlow.block) → embedded 8 (AnGap.stack)',
      (tester) async {
    ...
    testWidgets(
      'inline code drops a rung: reading mono 13 → embedded codeInline 12',
      (tester) async {
        await tester.pumpWidget(host(const AnMarkdown('use `retries` here')));
        expect(
          tester.widget<Text>(find.text('retries')).style?.fontSize,
          13,
        ); // AnText.mono
        await tester.pumpWidget(
          host(
    ...
    testWidgets(
      'fenced code rides the scale: reading → codeReading 13 (reading:true), embedded → code 12 (reading:false)',
      (tester) async {
        await tester.pumpWidget(host(const AnMarkdown('```py\nprint(1)\n```')));
        expect(
          tester.widget<AnCodeEditor>(find.byType(AnCodeEditor)).reading,
          isTrue,
        );
        await tester.pumpWidget(
          host(
  ...
  group('tables', () {
    ...
    testWidgets(
      'md table → AnProseTable hairline grid; :-:/--: drive per-column align; cells parse rich',
      (tester) async {
        const md =
            '| a | b | c |\n|:--|:-:|--:|\n| **x** | 2 | 3 |\n| 4 | 5 | 6 |';
        ...
        expect(
          find.byType(AnThinTable),
          findsNothing,
        ); // AnThinTable retired from the markdown path
        final table = tester.widget<Table>(
          find.descendant(
            of: find.byType(AnProseTable),
    ...
    testWidgets(
      'cells with inline-code / link WidgetSpans render without throwing (intrinsic-width guard)',
      (tester) async {
    ...
    testWidgets(
      'ragged rows normalize to the header column count (no crash, uniform Table rows)',
      (tester) async {
        const md = '| a | b |\n|---|---|\n| 1 | 2 | extra |\n| x |';
        await tester.pumpWidget(host(const AnMarkdown(md)));
        final table = tester.widget<Table>(
  ...
  group('injection / safety', () {
    testWidgets(
      'raw HTML renders literally (<script>, <u> — UnderLineMd stripped)',
      (tester) async {
        await tester.pumpWidget(
          host(const AnMarkdown('<script>alert(1)</script> and <u>plain</u>')),
        );
        expect(
    ...
    testWidgets(
      'images never fetch: zero Image widgets, inert placeholder chip instead',
      (tester) async {
        await tester.pumpWidget(
          host(const AnMarkdown('![alt](https://evil.example/x.png)')),
        );
        expect(find.byType(Image), findsNothing);
        expect(find.byIcon(AnIcons.image), findsOneWidget);
        expect(
          find.textContaining('evil.example'),
          findsOneWidget,
        ); // the URL is visible, not fetched
      },
    );
  });

  testWidgets('theme flip regenerates spans (link color follows dark accent)', (
    tester,
  ) async {
    await tester.pumpWidget(host(const AnMarkdown('[x](https://x.dev)')));
    expect(spanWhere(tester, 'x').$2?.color, AnColors.light.accent);
    await tester.pumpWidget(
      host(const AnMarkdown('[x](https://x.dev)'), theme: AnTheme.dark()),
    );
    await tester.pumpAndSettle();
    expect(
      spanWhere(tester, 'x').$2?.color,
      AnColors.dark.accent,
      reason:
          'isSame() compares style — a colourless style would leave stale light-theme spans',
    );
  });
}
