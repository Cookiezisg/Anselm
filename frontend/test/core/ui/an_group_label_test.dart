...
void main() {
  ...
  testWidgets('default padding follows proximity (more above, less below)', (
    tester,
  ) async {
    await tester.pumpWidget(host(const AnGroupLabel('inputs')));
    expect(
      labelPadding(tester).padding,
      const EdgeInsets.fromLTRB(AnSpace.s4, AnSpace.s8, AnSpace.s4, AnSpace.s4),
    );
  });

  testWidgets(
    'padding override drops the rail insets (AnSection caption reuse path)',
    (tester) async {
      await tester.pumpWidget(
        host(const AnGroupLabel('inputs', padding: EdgeInsets.zero)),
      );
