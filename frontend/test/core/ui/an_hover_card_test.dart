...
void main() {
        ...
        home: Scaffold(
          body: Center(
            child: AnHoverCard(
              ...
              child: const SizedBox(width: 24, height: 24, child: ColoredBox(color: Color(0xFFEEEEEE))),
            ),
          ),
        ),
      );
  ...
  testWidgets('the card sizes to its CONTENT — never the full stage (0718 大白片回归锁)',
      (tester) async {
    ...
    await tester.pump();
    final card = find.ancestor(of: find.text('card body'), matching: find.byType(Container)).first;
    final rect = tester.getRect(card);
    expect(rect.width, lessThanOrEqualTo(AnSize.menuMaxWidth),
        reason: '卡宽 ≤ maxWidth token(绝不铺满舞台)');
    expect(rect.height, lessThan(200), reason: '卡高按内容(一行文字),绝非全屏白板');
  });

  testWidgets('dwell manners: no card before the dwell elapses; exit hides it', (tester) async {
    await tester.pumpWidget(host());
  ...
  testWidgets('the card is pure display: IgnorePointer + ExcludeSemantics (不可交互、不重复朗读)',
      (tester) async {
    await tester.pumpWidget(host());
    await hover(tester);
    await tester.pump(AnMotion.dwell);
    await tester.pump();
    expect(
        find.ancestor(of: find.text('card body'), matching: find.byType(IgnorePointer)), findsWidgets,
        reason: '整卡 IgnorePointer(光标进不了卡,不与格争 hover)');
    expect(
        find.ancestor(of: find.text('card body'), matching: find.byType(ExcludeSemantics)),
        findsWidgets,
