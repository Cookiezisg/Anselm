...
Node n(String id, NodeKind k, {int x = 0, int y = 0}) => Node(
  id: id,
  kind: k,
  ref: '${k.name}_$id',
...
void main() {
  ...
  testWidgets(
    'C-016: a drag pointer-move rebuilds ZERO node cards (drag isolation), yet still commits',
    (tester) async {
      ...
      expect(
        ...
        0,
        reason:
            'a drag pointer-move must not rebuild any node card (isolation)',
      );
  ...
  testWidgets('a tap near an edge selects it', (tester) async {
    ...
    final mid =
        (tester.getCenter(find.text('a')) + tester.getCenter(find.text('b'))) /
        2;
  ...
  testWidgets('a small node drag moves it without also toggling selection', (
    tester,
  ) async {
    (String, NodePosition)? moved;
    ...
    expect(
      tapped,
      'sentinel',
    ); // onNodeTap NOT fired → selection not toggled 选中未被切换
  });
  ...
  testWidgets('handles do not appear when not editable', (tester) async {
    ...
    var moved = false;
    await tester.pumpWidget(
      host(AnGraphCanvas(graph: g, onNodeMoved: (_, _) => moved = true)),
    );
    await tester.pump();
    ...
    expect(
      moved,
      isFalse,
    ); // read-only: drag pans the canvas, never moves a node 只读:拖平移画布
    await gesture.removePointer();
  });
}
