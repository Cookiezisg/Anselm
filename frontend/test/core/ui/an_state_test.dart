...
void main() {
  ...
  testWidgets('empty renders title + hint + action; and stays SILENT', (
    tester,
  ) async {
  ...
  testWidgets(
    'error is MONOCHROME (glyph = inkFaint, never danger) and ANNOUNCES itself',
    (tester) async {
      final handle = tester.ensureSemantics();
      final said = probeAnnouncements(tester);
      await tester.pumpWidget(
      ...
      expect(
        glyph.color,
        isNot(AnColors.light.danger),
      ); // decision ①: red is reserved for AnCallout
      expect(
        tester.getSemantics(find.byType(AnState)).label,
        'Failed. Retry.',
      ); // findable 可被找到
  ...
  testWidgets(
    'an in-place loading→error flip announces the NEW sentence (the reader is sitting on it)',
    (tester) async {
      final handle = tester.ensureSemantics();
      final said = probeAnnouncements(tester);
      await tester.pumpWidget(
        host(const AnState(kind: AnStateKind.loading, title: 'Loading…')),
      );
  ...
  testWidgets(
    'loading spins normally, freezes to a static glyph under reduced-motion',
    (tester) async {
      await tester.pumpWidget(
        host(const AnState(kind: AnStateKind.loading, title: 'Loading…')),
      );
      ...
      await tester.pumpWidget(
        host(
          const AnState(kind: AnStateKind.loading, title: 'Loading…'),
          reduced: true,
        ),
      );
      await tester.pumpAndSettle(
      ...
      expect(
        find.byType(CircularProgressIndicator),
        findsNothing,
      ); // froze to a static glyph
      expect(find.byIcon(AnIcons.spin), findsOneWidget);
    },
  );

  testWidgets('long title + hint wrap and stay centered, no overflow', (
    tester,
  ) async {
    await tester.pumpWidget(
      host(
        const AnState(
          kind: AnStateKind.error,
          title:
              'A long error title that must wrap and stay centered without overflowing the column',
          hint:
              'An equally long explanatory hint that wraps onto several centered lines and never overflows.',
        ),
        width: 240,
      ),
    );
