...
void main() {
  ...
  testWidgets(
    'renders each segment + a faint «/» separator BETWEEN them (原语渲分隔符)',
    (tester) async {
      await tester.pumpWidget(
        host(const AnCrumbs([AnCrumb('Scheduler'), AnCrumb('数据清洗流水线')])),
      );
  ...
  testWidgets('an onTap-less segment is INERT (not a button, no AnInteractive)', (
    tester,
  ) async {
    // The direct-parent kind / a root you're already at has no navigable target → plain text, not a
    // control. 无去处的段=纯文本、非控件。
    await tester.pumpWidget(
      host(const AnCrumbs([AnCrumb('Entities'), AnCrumb('Function')])),
    );
  ...
  testWidgets(
    'a clickable segment IS an AnInteractive (click cursor + button semantics)',
    (tester) async {
      await tester.pumpWidget(
        host(AnCrumbs([AnCrumb('Scheduler', onTap: () {})])),
      );
  ...
  testWidgets(
    'a deep chain folds its middle to «…», keeping the first + the direct parent (Notion 同款)',
    (tester) async {
