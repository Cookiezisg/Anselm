...
void main() {
  ...
  group('AnWindow', () {
    testWidgets(
      'maxHeight clamp is silent-SAFE: a tall Column child never RenderFlex-overflows (复审 #3)',
      (tester) async {
    ...
    testWidgets(
      'unbounded-width host (Row rigid slot) shrink-wraps instead of throwing (复审 #7)',
      (tester) async {
        await tester.pumpWidget(
          _host(Row(children: const [AnWindow(child: Text('w'))])),
        );
        await tester.pump();
  ...
  group('AnLedgerRow', () {
    testWidgets(
      '280px host + 3 chips: ellipsizes, never overflows (复审 HIGH #2)',
      (tester) async {
        await tester.pumpWidget(
          _host(
            Column(
        ...
        expect(
          ...
          isNull,
        ); // rigid chips used to overflow 180px here 旧刚性 chips 曾溢出
      },
    );
  });
  ...
  group('AnLiveTail', () {
    testWidgets(
      'prose face shows the BOTTOM of overflowing text (复审 HIGH #1: was head-frozen)',
      (tester) async {
        final long = List.generate(40, (i) => '第 $i 句蒸馏。').join(' ');
        await tester.pumpWidget(
          _host(AnLiveTail(long, style: AnLiveTailStyle.prose)),
        );
        await tester.pump();
        ...
        expect(
          pos.maxScrollExtent,
          greaterThan(0),
        ); // content really overflows the clamp 内容真超钳
      },
    );

    testWidgets(
      'bare face drops the window shell (thinking stays inline prose, 批1)',
      (tester) async {
        await tester.pumpWidget(
          _host(
            const AnLiveTail(
              '思考中的一句话',
              style: AnLiveTailStyle.prose,
              bare: true,
            ),
          ),
        );
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.pumpWidget(
          _host(
            const AnLiveTail('short', style: AnLiveTailStyle.prose, bare: true),
          ),
        );
        await tester.pump();
        ...
        final long = List.generate(40, (i) => '第 $i 句蒸馏。').join(' ');
        await tester.pumpWidget(
          _host(AnLiveTail(long, style: AnLiveTailStyle.prose, bare: true)),
        );
        await tester.pump();
        await tester.pump();
    ...
    testWidgets(
      'mono face clips each logical line to one visual line (复审 #23)',
      (tester) async {
        ...
        final texts = tester
            ...
            .where((t) => t.maxLines == 1);
        expect(
          texts.length,
          greaterThanOrEqualTo(2),
        ); // per-line clipping 每行单行裁
      },
    );
  });
  ...
  group('AnVersionDiff live', () {
    testWidgets(
      'live rows render dels-then-adds through the SETTLED row structure (统一向落定对齐)',
      (tester) async {
        await tester.pumpWidget(
          _host(
            const AnVersionDiff(
    ...
    testWidgets(
      'live empty stream renders NOTHING (复审 #29: no bar-only shell)',
      (tester) async {
        await tester.pumpWidget(
          _host(const AnVersionDiff(live: true, before: '', after: '')),
        );
        await tester.pump();
    ...
    testWidgets(
      'live face is bounded by the stick viewport (复审 #6: no unbounded wall)',
      (tester) async {
        final long = List.generate(60, (i) => 'line $i').join('\n');
        await tester.pumpWidget(
          _host(AnVersionDiff(live: true, before: '', after: long)),
        );
        await tester.pump();
  ...
  group('AnLiveTail O(tail)', () {
    testWidgets(
      'a huge buffer renders only its tail — the head owns the slice (批1 复审)',
      (tester) async {
        ...
        await tester.pumpWidget(
          _host(AnLiveTail(huge, style: AnLiveTailStyle.mono, tailLines: 3)),
        );
        await tester.pump();
        expect(find.text('line 49999'), findsOneWidget); // the newest line 最新行
        expect(
          find.text('line 49996'),
          findsNothing,
        ); // beyond the 3-line tail 尾外
    ...
    testWidgets(
      'bare + windowed faces both survive a single giant no-newline line (帽兜底)',
      (tester) async {
        final spam =
            'x' *
            200000; // one logical line — the char cap is the real bound 单逻辑行,靠字符帽
        await tester.pumpWidget(
          _host(AnLiveTail(spam, style: AnLiveTailStyle.prose, bare: true)),
        );
        await tester.pump();
        await tester.pump();
        expect(tester.takeException(), isNull);
  ...
  group('AnFocusRing', () {
    testWidgets(
      ...
      (tester) async {
        await tester.pumpWidget(
          _host(
            const AnFocusRing(active: false, child: AnWindow(child: Text('卡'))),
          ),
        );
        await tester.pump();
        Container ringBox() => tester.widget<Container>(
          find
              .descendant(
                of: find.byType(AnFocusRing),
        ...
        await tester.pumpWidget(
          _host(
            const AnFocusRing(active: true, child: AnWindow(child: Text('卡'))),
          ),
        );
        await tester.pump();
  ...
  group('AnChip', () {
    testWidgets(
      'copy taps flash ✓ in the PERMANENT glyph slot (width never jumps, 复审 #13)',
      (tester) async {
        ...
        await tester.pumpWidget(
          _host(const AnChip('fr_1', copyValue: 'fr_full', mono: true)),
        );
        await tester.pump();
        final before = tester.getSize(find.byType(AnChip));
        expect(
          find.byIcon(AnIcons.copy),
          findsOneWidget,
        ); // idle affordance 静息示能
        await tester.tap(find.byType(AnChip));
        await tester.pump();
        expect(find.byIcon(AnIcons.check), findsOneWidget); // ✓ flash
        expect(
          tester.getSize(find.byType(AnChip)),
          before,
        ); // same slot, same width 原槽同宽
      },
    );
    ...
    testWidgets('批5: dot slot renders the typed status dot before the label', (
      tester,
    ) async {
      await tester.pumpWidget(
        _host(
      ...
      expect(
        tester.getTopLeft(find.byType(AnStatusDot)).dx,
        lessThan(tester.getTopLeft(find.text('completed')).dx),
      );
    });

    testWidgets('批5: an EMPTY label renders icon-only with no orphan gap', (
      tester,
    ) async {
      await tester.pumpWidget(
        _host(const AnChip('', copyValue: 'v', look: AnChipLook.outlined)),
      );
      await tester.pump();
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.pumpWidget(
          _host(const AnChip('本机', semanticLabel: '存储域: 本机')),
        );
        await tester.pump();
  ...
  group('AnAttachmentThumb onRemove a11y 批5', () {
    testWidgets(
      'the remove button is a LIVE semantic node (never excluded with the pixels)',
      (tester) async {
        await tester.pumpWidget(
          _host(
            AnAttachmentThumb(
        ...
        await tester.pump();
        expect(
          find.bySemanticsLabel('移除附件'),
          findsOneWidget,
        ); // 复审 MED:曾整体被 ExcludeSemantics 剥除
      },
    );
  });
  ...
  group('AnLedgerRow 批6 三槽', () {
    testWidgets(
      'sub renders under the primary, left-aligned with it (lead offset structural)',
      (tester) async {
        await tester.pumpWidget(
          _host(
            const AnLedgerRow(
        ...
        await tester.pump();
        expect(
          tester.getTopLeft(find.text('chat 触发')).dx,
          tester.getTopLeft(find.text('fnexec_01')).dx,
        );
        // measure sits LEFT of the meta iron line. measure 居铁线之左。
        expect(
          tester.getTopRight(find.text('942ms')).dx,
          lessThan(tester.getTopLeft(find.text('2m')).dx),
        );
      },
    );
    ...
    testWidgets(
      'expandChild insets EQUALLY from both edges — the phantom frame s8, lead or not '
      '(WRK-070 0718 用户裁,推翻批6 的仅左 lead 沟缩进)',
      (tester) async {
    ...
    testWidgets(
      'a lead-less row expands with the SAME both-side inset (缩进不再随 lead)',
      (tester) async {
        await tester.pumpWidget(
          _host(
            const AnLedgerRow(
        ...
        expect(
          tester.getTopLeft(find.text('BODY')).dx -
              tester.getTopLeft(find.byType(AnLedgerRow)).dx,
          AnSpace.s8,
    ...
    testWidgets(
      'sub is trimmed for render AND geometry: leading \\n paints, whitespace-only stays one line (批6 复审)',
      (tester) async {
        ...
        // 纯空白 sub 不得切双行几何留幽灵道。
        expect(
          tester.getSize(find.byKey(const Key('ws'))).height,
  ...
  group('AnLedgerList 批6', () {
    testWidgets('caps at N, escape reveals the rest in place', (tester) async {
      await tester.pumpWidget(
        _host(
          AnLedgerList(
            cap: 2,
            children: [for (var i = 0; i < 5; i++) AnLedgerRow(primary: 'r$i')],
          ),
        ),
      );
      await tester.pump();
    ...
    testWidgets(
      'exactly-cap shows no escape; cap+1 does (批6 复审 — a reused State kept _showAll and voided this pin)',
      (tester) async {
        ...
        await tester.pumpWidget(
          _host(
            AnLedgerList(
              ...
              cap: 2,
              children: [
                AnLedgerRow(primary: 'a'),
                AnLedgerRow(primary: 'b'),
              ],
            ),
          ),
        );
  ...
  group('AnLadder 批6', () {
    testWidgets('numbers every rung; the last rung has no descent line', (
      tester,
    ) async {
      await tester.pumpWidget(
        _host(
          const AnLadder(children: [Text('one'), Text('two'), Text('three')]),
        ),
      );
      await tester.pump();
      expect(find.text('1'), findsOneWidget);
      ...
      expect(
        find.descendant(
          ...
          matching: find.byWidgetPredicate(
            (w) => w is Expanded && w.child is Container,
          ),
        ),
        findsNWidgets(2),
      );
  ...
  group('AnKvRow.flag 批6', () {
    testWidgets('renders ✓/— and speaks localized yes/no (never «check mark»)', (
      tester,
    ) async {
      await tester.pumpWidget(
        _host(
    ...
    testWidgets(
      'the flag row wears the family inset — edges align with sibling rows (批6 复审)',
      (tester) async {
        await tester.pumpWidget(
          _host(
            const AnKv(
              rows: [AnKvRow('plain', 'v'), AnKvRow.flag('listening', true)],
            ),
          ),
        );
        await tester.pump();
        ...
        expect(
          tester.getTopLeft(find.text('listening')).dx,
          tester.getTopLeft(find.text('plain')).dx,
        );
      },
    );
  });
  ...
  group('AnKeycap 批5c', () {
    testWidgets(
      'tri-state renders and taps; the keycap is NEVER focusable (host owns the keyboard)',
      (tester) async {
        var taps = 0;
    ...
    testWidgets(
      'compact tier (0719): per-key caps at rest — one 20-high plate per fragment, fits a 32 row',
      (tester) async {
        var taps = 0;
        await tester.pumpWidget(
          _host(
    ...
    testWidgets(
      'recording keeps the wide plate (the big form is recording-dedicated)',
      (tester) async {
        await tester.pumpWidget(
          _host(
            const AnKeycap(
  ...
  group('AnSwatch 批5c', () {
    testWidgets('pick cell speaks selected semantics; dot face is inert', (
      tester,
    ) async {
      await tester.pumpWidget(
        _host(
      ...
      expect(
        tester.getSize(find.byType(AnSwatch).last),
        const Size(AnSize.swatch, AnSize.swatch),
      );
    });
  });
  ...
  group('AnFollowPill.jump 批5', () {
    testWidgets('static: renders label + never subscribes to the pulse clock', (
      tester,
    ) async {
      var taps = 0;
      await tester.pumpWidget(
        _host(AnFollowPill.jump(label: '回到最新', onTap: () => taps++)),
      );
      await tester.pump();
  ...
  group('AnCodeEditor 批2', () {
    ...
    testWidgets(
      'zero-jump: live and settled faces have the SAME frame height at the same tier (缺口A)',
      (tester) async {
    ...
    testWidgets(
      'live face slices its own O(tail): huge code renders, gutter numbers stay honest',
      (tester) async {
  ...
  group('AnVersionDiff 批2', () {
    testWidgets(
      'zero-jump: live and settled diff faces have the SAME frame height (复审: diff 孪生件同病)',
      (tester) async {
        ...
        expect(
          await frameH(live: false),
          await frameH(live: true),
        ); // 落定瞬间不再矮 32px
      },
    );

    testWidgets('bounded host + maxHeight stays silent-safe (复审: 裸钳曾溢出)', (
      tester,
    ) async {
  ...
  group('AnCodeEditor live 增量与换源 批2', () {
    testWidgets(
      'gutter numbering stays honest across APPEND frames (kills the += → = mutation)',
      (tester) async {
        String make(int lines) => List.generate(
          lines,
          (i) => 'ln${(i + 1).toString().padLeft(5, '0')}',
        ).join('\n');
    ...
    testWidgets(
      'a same-State SOURCE SWAP recounts from scratch (复审: 仅比切点的守卫铸假行号)',
      (tester) async {
        ...
        await tester.pumpWidget(
          _host(
            AnCodeEditor(code: a, live: true, maxHeight: AnSize.codeViewport),
          ),
        );
        await tester.pump();
        ...
        await tester.pumpWidget(
          _host(
            AnCodeEditor(code: b, live: true, maxHeight: AnSize.codeViewport),
          ),
        );
        await tester.pump();
        final gutter = tester
        ...
        expect(
          (start - (freshHeadLines + 1)).abs(),
          lessThan(3),
        ); // within alignment slack 对齐容差
      },
    );

    testWidgets(
      'bounded host + maxHeight stays silent-safe (复审: 裸钳曾溢出 152px)',
      (tester) async {
        ...
        await tester.pumpWidget(
          _host(
            SizedBox(
              height: 120,
              child: AnCodeEditor(code: code, maxHeight: AnSize.codeViewport),
            ),
          ),
        );
        await tester.pump();
  ...
  group('AnStatBar', () {
    testWidgets(
      'leading credential renders BEFORE the status badge (批3: the bar subject leads)',
      (tester) async {
        await tester.pumpWidget(
          _host(
            const AnStatBar(
        ...
        final badgeX = tester
            .getTopLeft(
              find.text(
                Translations.of(
                  tester.element(find.byType(AnStatBar)),
                ).status.done,
              ),
            )
            .dx;
  ...
  group('AnSpinner 批7', () {
    testWidgets(
      'spins live; freezes to the still glyph under assistive tech (orAssistive gate, B-054)',
      (tester) async {
  ...
  group('装饰循环 orAssistive 门 批7 复审', () {
    testWidgets(
      'AnFollowPill live + AnRadarSweep freeze under assistive tech (立法3, mutation-locked)',
      (tester) async {
  ...
  group('AnFadeRiseIn 批7', () {
    testWidgets(
      'animates entry once; renders static (no ticker) under reduced motion',
      (tester) async {
  ...
  group('AnDropVeil 批7', () {
    testWidgets('veils at the token alpha and never intercepts the pointer', (
      tester,
    ) async {
      var taps = 0;
      await tester.pumpWidget(
      ...
      final box = tester.widget<ColoredBox>(
        find.descendant(
          of: find.byType(AnDropVeil),
          matching: find.byType(ColoredBox),
        ),
      );
      expect(
        box.color.a,
        closeTo(AnOpacity.veil, 0.005),
      ); // token alpha, not a private literal 走档
