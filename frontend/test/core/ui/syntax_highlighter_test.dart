...
void main() {
  ...
  TextSpan spanFor(List<TextSpan> s, String text) => s.firstWhere((e) => e.text == text);
  Color? colorOf(List<TextSpan> s, String text) => spanFor(s, text).style?.color;

  test('empty input → no spans', () {
    expect(highlightCode('', colors: c), isEmpty);
  });

  test('LOSSLESS: concatenated span text equals the input (no glyph dropped or duplicated)', () {
    for (final src in <String>[
      ...
      expect(joined(highlightCode(src, colors: c)), src, reason: 'must round-trip: $src');
    }
  });
  ...
  test('identifier followed by ( → function colour; otherwise plain', () {
    ...
    expect(spanFor(plain, 'foo').style?.color, isNull, reason: 'not a call → plain, inherits ambient');
  });
  ...
  test('CEL expression colours keyword + function + string together (unified tokenizer covers CEL)', () {
  ...
  test('lang param is accepted and does not change v1 tokenization (unified)', () {
