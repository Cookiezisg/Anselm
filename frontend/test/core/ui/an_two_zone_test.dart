...
void main() {
  ...
  testWidgets('label renders and trailing is pinned to the right edge', (
    tester,
  ) async {
    ...
    expect(
      (zoneRight - trailRight).abs(),
      lessThan(1.0),
      reason: 'trailing pinned right',
    );
  });
  ...
  testWidgets('long label ellipsis-truncates without overflow', (tester) async {
    await tester.pumpWidget(
      host(
        const AnTwoZone(
          label: Text(
            'A very very very long primary label that must ellipsis',
            maxLines: 1,
  ...
  testWidgets(
    'long meta is capped (≤45%) so it never crowds out the label — no overflow',
    (tester) async {
      const longMeta =
          'an_extremely_long_meta_identifier_that_would_overflow_if_uncapped_0123456789';
      await tester.pumpWidget(
        host(
          const AnTwoZone(
            label: Text(
      ...
      expect(
        find.text('Primary'),
        findsOneWidget,
      ); // label survives, not crowded out
