...
void main() {
  test('decodes a full assistant terminal turn', () {
    final m = ChatMessage.fromJson({
      ...
      'blocks': [
        ...
        },
        {
          ...
          'content': '答案。',
          'status': 'completed',
          'createdAt': '2026-07-02T10:00:02Z',
          'updatedAt': '2026-07-02T10:00:03Z',
        },
      ],
    });
    ...
    expect(m.blocks.map((b) => b.type), [
      'reasoning',
      'text',
    ]); // wire order preserved 保序
  ...
  test(
    'decodes a user turn with frozen attachment + mention snapshots (incl. unavailable stub)',
    () {
      final m = ChatMessage.fromJson({
        'id': 'msg_2',
        'conversationId': 'cv_1',
        ...
        'attrs': {
          ...
          'mentions': [
            ...
            },
            {
              ...
              'id': 'ag_9',
              'name': '(unavailable)',
            }, // no content key 无 content 键
          ],
        },
      });
      expect((m.attrs?['attachments'] as List).cast<String>(), [
        'att_1',
        'att_2',
      ]);
      final mentions = (m.attrs?['mentions'] as List)
          .cast<Map<String, dynamic>>();
      expect(mentions.first['name'], 'sync_inventory');
      expect(
        mentions.last.containsKey('content'),
        isFalse,
      ); // availability signal 可用性信号
