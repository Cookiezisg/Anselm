...
beforeEach(() => {
  usePaneStore.setState({ activeConv: null, openPanes: [], activeNarrowPane: null });
...
describe("FlowRunDetail", () => {
  ...
  it("triageSuccess_setsActiveConv_opensChatPane_andToasts", async () => {
    useFlowRun.mockReturnValue({ data: { ...BASE_RUN, status: "failed" } });
    render(<FlowRunDetail runId="fr_xy" onBack={() => {}} />);
    await userEvent.click(screen.getByText("AI 排查"));
    await waitFor(() => expect(usePaneStore.getState().activeConv).toBe("cv_triage_new"));
  ...
  it("dagNodes_renderedWithLabelAndKind", () => {
    ...
    // Labels appear in both DAG nodes and Gantt rows — assert at least one of each
  ...
  it("waitingApproval_approvalBannerVisible", () => {
    ...
    // "Approve me" appears in banner, DAG node, Gantt — verify ≥1
