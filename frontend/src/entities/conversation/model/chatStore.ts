...
import type { BlockType, BlockStatus, MessageRole, MessageStatus } from "./types";

// ── Store-internal shapes (differ from API Message/Block in types.ts) ──
// The runtime store keeps blocks.children/message.blocks as ID arrays,
// not embedded objects. These are intentionally separate from the REST
// API types to avoid structural mismatch.
...
export interface ChatMessage {
  ...
  blocks: string[];
  attachments: AttachmentRefRaw[];
  attrs: Record<string, unknown> | null;
}

interface AttachmentRefRaw {
  attachmentId: string;
  fileName?: string;
  mimeType?: string;
}
...
// Payload shapes from SSE eventlog events.
...
// REST history message shape (from useConversationMessages / hydrateConv call).
...
interface RestBlock {
  ...
  error?: string | null;
  attrs?: Record<string, unknown> | null;
  innerMessage?: RestMessage;
  [key: string]: unknown;
}

export interface ChatState {
  convs: Record<string, ChatConvState>;
  hydratedConvs: Set<string>;
  ensureConv(convId: string): void;
  resetConv(convId: string): void;
  resetAll(): void;
  hydrateConv(convId: string, messages: RestMessage[]): void;
  onMessageStart(convId: string, e: MessageStartEvent): void;
  onMessageStop(convId: string, e: MessageStopEvent): void;
  onBlockStart(convId: string, e: BlockStartEvent): void;
  onBlockDelta(convId: string, e: BlockDeltaEvent): void;
  onBlockStop(convId: string, e: BlockStopEvent): void;
}

function emptyConv(): ChatConvState {
...
interface DeltaItem { convId: string; blockId: string; delta: string }
let pendingDeltas: DeltaItem[] = [];
let rafHandle: ReturnType<typeof setTimeout> | number = 0;
let storeApiRef: UseBoundStore<StoreApi<ChatState>> | null = null;
...
function cancelFlush() {
  if (!rafHandle) return;
  if (typeof cancelAnimationFrame !== "undefined") cancelAnimationFrame(rafHandle as number);
  else clearTimeout(rafHandle as ReturnType<typeof setTimeout>);
  rafHandle = 0;
}
...
function flushDeltas() {
  ...
  const byConv = new Map<string, Map<string, string>>();
...
export const useChatStore = create<ChatState>()((set, get) => ({
  ...
  hydrateConv(convId, messages) {
    ...
    const installMessage = (m: RestMessage, parentBlockId: string | null) => {
  ...
  onMessageStop(convId, e) {
    ...
    set((s) => {
      ...
      messages.set(e.id, {
        ...
        errorCode: e.errorCode || cur.errorCode,
        errorMessage: e.errorMessage || cur.errorMessage,
...
export function selectTopMessageIds(convId: string, state: ChatState): string[] {
  return state.convs[convId]?.topMsgIds || EMPTY_IDS;
}
export function selectBlock(convId: string, blockId: string, state: ChatState): ChatBlock | null {
  return state.convs[convId]?.blocks.get(blockId) || null;
}
export function selectChildIds(convId: string, parentId: string, state: ChatState): string[] {
