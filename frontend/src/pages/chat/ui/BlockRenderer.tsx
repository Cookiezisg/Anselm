...
const TextBlock = memo(function TextBlock({ convId, blockId }: { convId: string; blockId: string }) {
...
const ReasoningBlock = memo(function ReasoningBlock({ convId, blockId, defaultOpen }: { convId: string; blockId: string; defaultOpen?: boolean }) {
...
const ToolCallBlock = memo(function ToolCallBlock({ convId, blockId, defaultOpen }: { convId: string; blockId: string; defaultOpen?: boolean }) {
      ...
      <div className="blk-tool-head" onClick={() => setOpen((o) => !o)}>
        ...
        <div className="blk-tool-meta">
          <div className="blk-tool-name">
            <code>{blockAttrs?.tool || "tool"}</code>
            {blockAttrs?.executionGroup && (
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--fg-faint)" }}>
                group {blockAttrs.executionGroup}
              </span>
            )}
          </div>
          {blockAttrs?.summary && (
            <div className="blk-tool-summary">{blockAttrs.summary}</div>
          )}
        </div>
...
const ToolChildBlock = memo(function ToolChildBlock({ convId, blockId }: { convId: string; blockId: string }) {
...
// ── ProgressBlock ───────────────────────────────────────────────────
const ProgressBlock = memo(function ProgressBlock({ convId, blockId }: { convId: string; blockId: string }) {
  const { t } = useTranslation("conv");
...
// ── ToolResultBlock ────────────────────────────────────────────────
const ToolResultBlock = memo(function ToolResultBlock({ convId, blockId }: { convId: string; blockId: string }) {
  const { t } = useTranslation("conv");
  const block = useChatStore((s) => selectBlock(convId, blockId, s));
  if (!block) return null;
...
const SubagentBlock = memo(function SubagentBlock({ convId, blockId, depth = 0 }: { convId: string; blockId: string; depth?: number }) {
  const { t } = useTranslation("conv");
  const block = useChatStore((s) => selectBlock(convId, blockId, s));
...
// ── CompactionBlock ────────────────────────────────────────────────
const CompactionBlock = memo(function CompactionBlock({ convId, blockId }: { convId: string; blockId: string }) {
  const { t } = useTranslation("conv");
  const block = useChatStore((s) => selectBlock(convId, blockId, s));
  const [open, setOpen] = useState(false);
  if (!block) return null;
...
export function BlockList({ convId, blockIds, depth = 0, defaultOpenTools = false }: { convId: string; blockIds: string[]; depth?: number; defaultOpenTools?: boolean }) {
