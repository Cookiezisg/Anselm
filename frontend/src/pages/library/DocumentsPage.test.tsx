...
const mockUseDocumentTree = useDocumentTree as any;
const mockUseDocument = useDocument as any;
const mockUseCreateDocument = useCreateDocument as any;
const mockUseUpdateDocument = useUpdateDocument as any;
const mockUseDeleteDocument = useDeleteDocument as any;
...
beforeEach(() => {
  ...
  updateMutate      = vi.fn();
  delMutate         = vi.fn();
  mockUseDocumentTree.mockReturnValue({ data: TREE, isLoading: false });
  mockUseDocument.mockReturnValue({ data: null, isLoading: false });
  mockUseCreateDocument.mockReturnValue({ mutateAsync: createMutateAsync });
  mockUseUpdateDocument.mockReturnValue({ mutate: updateMutate, isPending: false });
  mockUseDeleteDocument.mockReturnValue({ mutate: delMutate });
  useToastStore.setState({ toasts: [] });
});

describe("DocumentsPage", () => {
  it("treeLoading_showsLoadingHint", () => {
    mockUseDocumentTree.mockReturnValue({ data: undefined, isLoading: true });
  ...
  it("activeDocLoaded_rendersTitleInput_andEditor", () => {
    ...
    const titleInput = document.querySelector(".doc-page-title-input") as HTMLInputElement;
