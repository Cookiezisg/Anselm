// shared/bridge/wails — base URL resolution. Module state is reset between
// tests via dynamic re-import so each spec sees a fresh _baseUrl=null.
// Migrated from src/bridge/wails.test.js (4b.5 recovery).

import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";

let bridge: typeof import("./wails");
beforeEach(async () => {
  vi.resetModules();
  bridge = await import("./wails");
});

afterEach(() => {
  delete (window as Window & typeof globalThis).go;
});
...
describe("wails bridge", () => {
  ...
  it("initBaseUrl_withWailsRuntime_resolvesToLocalhostPort", async () => {
    (window as Window & typeof globalThis).go = { main: { App: { GetBackendPort: async () => 9876 } } };
    const r = await bridge.initBaseUrl();
  ...
  it("apiUrl_wailsMode_returnsAbsoluteUrl", async () => {
    (window as Window & typeof globalThis).go = { main: { App: { GetBackendPort: async () => 7788 } } };
    await bridge.initBaseUrl();
