...
/// and shuts it down with the app — zero backend changes (it already reads `ANSELM_ADDR`
/// / `ANSELM_DATA_DIR` and serves `GET /api/v1/health`, verified in the backend).
///
/// Resolution order:
///  1. `ANSELM_BACKEND_URL` env — the dev escape hatch: attach to an already-running
...
///     `ANSELM_ADDR=127.0.0.1:<port>`, launch, then poll `/api/v1/health` until 200.
///
/// 以 sidecar 模型托管 Go 后端(ADR 0004 §1)。后端是独立 localhost HTTP+SSE server;客户端拉起、
/// 健康门控、随 app 关停——零后端改动(它已读 `ANSELM_ADDR`/`ANSELM_DATA_DIR`、提供
/// `GET /api/v1/health`,已核实)。解析序:① `ANSELM_BACKEND_URL` dev 逃生口(挂到已跑后端、不 spawn);
/// ② spawn 内置二进制(Dart 抢临时端口→`ANSELM_ADDR` 注入→轮询 /health 至 200)。
class BackendController {
  ...
  /// executable (packaging concern). Ignored when `ANSELM_BACKEND_URL` is set.
  ///
  ...
  Future<void> start() async {
    try {
      final external = Platform.environment['ANSELM_BACKEND_URL'];
  ...
  Future<String> _spawn() async {
    ...
    final env = {'ANSELM_ADDR': '127.0.0.1:$port'};
    final dir = dataDir;
    if (dir != null) env['ANSELM_DATA_DIR'] = dir;
