...
class ApiException implements Exception {
  ...
  /// branches on the handful in [AnselmErr]; the rest are shown via [message].
  ///
  /// 稳定 `<ENTITY>_<REASON>` wire code(全局唯一,~256 登记)。UI 只 branch [AnselmErr]
  /// 里的少数,其余经 [message] 呈现。
  final String code;
  ...
  factory ApiException.transport(String message) =>
      ApiException(code: AnselmErr.transport, message: message, httpStatus: 0);
...
/// 生成;此为驱动 UX 流程的手挑子集。
abstract final class AnselmErr {
