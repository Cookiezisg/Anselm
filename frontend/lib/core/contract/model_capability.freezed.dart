...
mixin _$ModelCapability {
 ...
 String get apiKeyId; String get keyName; String get provider; String get modelId; String get displayName; int get contextWindow; int get maxOutput; int get textInputLimit; int get multimodalInputLimit; bool get vision; bool get video; bool get audio; bool get nativeDocs;/// Whether the model can call tools — i.e. whether it can drive the agent runtime at all.
///
...
/// 「不在那儿」,没有任何途径知道为什么(H12-b)。
 bool get tools; int get maxMediaParts; int get maxMediaBytes; List<ModelKnob> get knobs;
...
@useResult
$Res call({
 String apiKeyId, String keyName, String provider, String modelId, String displayName, int contextWindow, int maxOutput, int textInputLimit, int multimodalInputLimit, bool vision, bool video, bool audio, bool nativeDocs, bool tools, int maxMediaParts, int maxMediaBytes, List<ModelKnob> knobs
});
...
@optionalTypeArgs TResult maybeWhen<TResult extends Object?>(TResult Function( String apiKeyId,  String keyName,  String provider,  String modelId,  String displayName,  int contextWindow,  int maxOutput,  int textInputLimit,  int multimodalInputLimit,  bool vision,  bool video,  bool audio,  bool nativeDocs,  bool tools,  int maxMediaParts,  int maxMediaBytes,  List<ModelKnob> knobs)?  $default,{required TResult orElse(),}) {final _that = this;
switch (_that) {
case _ModelCapability() when $default != null:
return $default(_that.apiKeyId,_that.keyName,_that.provider,_that.modelId,_that.displayName,_that.contextWindow,_that.maxOutput,_that.textInputLimit,_that.multimodalInputLimit,_that.vision,_that.video,_that.audio,_that.nativeDocs,_that.tools,_that.maxMediaParts,_that.maxMediaBytes,_that.knobs);case _:
  return orElse();
...
class _ModelCapability implements ModelCapability {
  const _ModelCapability({required this.apiKeyId, this.keyName = '', this.provider = '', required this.modelId, this.displayName = '', this.contextWindow = 0, this.maxOutput = 0, this.textInputLimit = 0, this.multimodalInputLimit = 0, this.vision = false, this.video = false, this.audio = false, this.nativeDocs = false, this.tools = true, this.maxMediaParts = 0, this.maxMediaBytes = 0, final  List<ModelKnob> knobs = const <ModelKnob>[]}): _knobs = knobs;
...
/// 两个事实,而选择器直说是哪一个。此前目录**整个丢掉**这些模型,且丢得看不见——那个模型就是
/// 「不在那儿」,没有任何途径知道为什么(H12-b)。
@override@JsonKey() final  bool tools;
...
@JsonKey(includeFromJson: false, includeToJson: false)
@override
int get hashCode => Object.hash(runtimeType,apiKeyId,keyName,provider,modelId,displayName,contextWindow,maxOutput,textInputLimit,multimodalInputLimit,vision,video,audio,nativeDocs,tools,maxMediaParts,maxMediaBytes,const DeepCollectionEquality().hash(_knobs));
