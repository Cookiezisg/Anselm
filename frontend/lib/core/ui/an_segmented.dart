...
class AnSegmentedOption<T> {
  const AnSegmentedOption({required this.value, required this.label, this.icon});
...
/// The horizontal segmented control (2–4 options) — the settings rows' enum picker. A sunken pill
...
class AnSegmented<T> extends StatelessWidget {
  const AnSegmented({
    ...
    this.semanticLabel,
    super.key,
  }) : assert(options.length >= 2 && options.length <= 4, '2–4 segments 段数 2–4');
              ...
              Row(children: [
                for (final (i, o) in options.indexed)
                  Expanded(
                    child: AnInteractive(
                      enabled: enabled,
                      onTap: enabled && i != index ? () => onChanged(o.value) : null,
                              ...
                              children: [
                                if (o.icon != null) ...[
                                  Icon(o.icon, size: AnSize.iconSm,
                                      color: active ? c.ink : (hovered ? c.inkMuted : c.inkFaint)),
