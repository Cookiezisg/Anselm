...
abstract final class AnIcons {
  ...
  static final IconData chevronLeft = _thin(
    LucideIcons.chevronLeft,
  ); // calendar month back 日历翻上月
  static final IconData chevronDown = _thin(LucideIcons.chevronDown);
  static final IconData calendarRange = _thin(
    LucideIcons.calendarRange,
  ); // time-range picker capsule 时间范围胶囊
  static final IconData calendarToday = _thin(
    LucideIcons.calendarCheck,
  ); // calendar «back to today» 日历回今天
  ...
  static final IconData copy = _thin(
    LucideIcons.copy,
  ); // code-block / value copy-to-clipboard 复制
  static final IconData expand = _thin(LucideIcons.maximize2);
  static final IconData plus = _thin(
    LucideIcons.plus,
  ); // New / add (sidebar New row, row-add) 新建/添加
  static final IconData zoomIn = _thin(
    LucideIcons.zoomIn,
  ); // graph canvas zoom 图画布放大
  static final IconData zoomOut = _thin(
    LucideIcons.zoomOut,
  ); // graph canvas zoom 图画布缩小
  static final IconData search = _thin(LucideIcons.search);
  static final IconData check = _thin(LucideIcons.check);
  static final IconData panelLeft = _thin(
    LucideIcons.panelLeft,
  ); // collapse/reopen the left island 左岛收起/展开
  static final IconData panelRight = _thin(
    LucideIcons.panelRight,
  ); // toggle the right island 右岛切换
  ...
  static final IconData pin = _thin(
    LucideIcons.pin,
  ); // pinned conversations 置顶对话
  static final IconData archive = _thin(
    LucideIcons.archive,
  ); // archive a conversation 归档对话
  ...
  static final IconData locate = _thin(
    LucideIcons.locate,
  ); // Cast「跳到发生处」jump-to-occurrence 定位
  static final IconData eye = _thin(
    LucideIcons.eye,
  ); // sidestage follow-mode 侧幕跟随三档
  static final IconData activity = _thin(
    LucideIcons.activity,
  ); // sidestage auto-reveal (follow) 侧幕自动展示(脉冲)
  static final IconData unfold = _thin(
    LucideIcons.unfoldVertical,
  ); // sidestage expand-all 展开全部
  static final IconData fold = _thin(
    LucideIcons.foldVertical,
  ); // sidestage collapse-all 收起全部
  static final IconData open = _thin(
    LucideIcons.arrowUpRight,
  ); // Cast「去实体页」cross-ocean nav 外跳
  ...
  static final IconData download = _thin(
    LucideIcons.download,
  ); // WebFetch / install_mcp_server 抓取/安装
  static final IconData layers = _thin(
    LucideIcons.layers,
  ); // stage_workflow 暂存版本
  static final IconData pause = _thin(
    LucideIcons.circlePause,
  ); // deactivate_workflow 停用
  static final IconData ban = _thin(
    LucideIcons.ban,
  ); // kill_workflow / KillShell 强杀
  static final IconData store = _thin(
    LucideIcons.store,
  ); // list_mcp_marketplace 市场
  static final IconData unplug = _thin(
    LucideIcons.unplug,
  ); // uninstall_mcp_server 卸载
  static final IconData model = _thin(LucideIcons.cpu); // get_model_config 模型配置
  static final IconData capability = _thin(
    LucideIcons.badgeCheck,
  ); // capability_check_workflow 能力体检
  static final IconData relations = _thin(
    LucideIcons.share2,
  ); // get_relations 依赖关系
  static final IconData memory = _thin(
    LucideIcons.bookMarked,
  ); // write/read_memory 记忆
  static final IconData ask = _thin(
    LucideIcons.messageCircleQuestion,
  ); // ask_user 提问
  static final IconData gavel = _thin(LucideIcons.gavel); // decide_approval 裁决
  static final IconData inbox = _thin(
    LucideIcons.inbox,
  ); // list_approval_inbox 审批收件箱

  // ── settings 设置 ──
  static final IconData laptop = _thin(
    LucideIcons.laptop,
  ); // device-scope prefs 本机域
  static final IconData workspaceScope = _thin(
    LucideIcons.briefcase,
  ); // workspace-scope 工作区域
  static final IconData machineScope = _thin(
    LucideIcons.hardDrive,
  ); // machine-wide settings.json 全机域
  static final IconData undo = _thin(
    LucideIcons.undo2,
  ); // single-row reset 单项重置
  ...
  static final IconData keyboard = _thin(
    LucideIcons.keyboard,
  ); // shortcuts panel 快捷键
  static final IconData globe = _thin(
    LucideIcons.globe,
  ); // network panel 网络(settings)
  ...
  static final IconData codeBlock = _thin(
    LucideIcons.code,
  ); // fenced code block 代码块
  static final IconData divider = _thin(
    LucideIcons.minus,
  ); // horizontal rule 分隔线
  static final IconData table = _thin(LucideIcons.table); // table block 表格块
  static final IconData todo = _thin(
    LucideIcons.listTodo,
  ); // task checkbox block 待办
  static final IconData circle = _thin(
    LucideIcons.circle,
  ); // an empty/pending mark (todo pending) 空/待办标记
  ...
  // ── composer (chat input) ──
  static final IconData mention = _thin(
    LucideIcons.atSign,
  ); // @ mention trigger @提及
  static final IconData attach = _thin(
  ...
  static final IconData timeout = _thin(
    LucideIcons.timer,
  ); // duration budget / timed-out (批7 B-047) 时限/超时
  ...
  static final IconData iterate = _thin(
    LucideIcons.sparkles,
  ); // AI edit (≠ forge: rebuild env) AI 编辑
  ...
  static final IconData image = _thin(
    LucideIcons.image,
  ); // markdown image placeholder chip 图片占位
  static final IconData taskOpen = _thin(
    LucideIcons.square,
  ); // md task list, unchecked 任务未勾
  static final IconData taskDone = _thin(
    LucideIcons.squareCheck,
  ); // md task list, checked 任务已勾
  ...
  static final IconData fileMissing = _thin(
    LucideIcons.fileX,
  ); // tombstone: deleted/404 墓碑
  ...
  static final Map<String, IconData> _byKey = {
    ...
    'doc': doc,
    'document': doc,
    'entities': entities, // 'document' = backend EntityKind wire 后端实体 kind 线缆值
  ...
  static IconData entityKindGlyph(String wireKind) =>
      byKey(wireKind.toLowerCase());
  ...
  static final Map<String, IconData> _toolExact = {
    ...
    'replay_flowrun': history,
    // workflow 生命周期:暂存=层 / 激活=运行 / 停用=暂停 / 杀=禁 / 能力体检=徽
    'stage_workflow': layers,
    'activate_workflow': run,
    'deactivate_workflow': pause,
  ...
  static IconData toolIcon(String name) {
    ...
    if (RegExp(r'execution|_call|flowrun|firing|activation').hasMatch(n)) {
      return history;
    }
