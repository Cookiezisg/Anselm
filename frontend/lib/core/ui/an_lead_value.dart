...
class AnLeadValue
    extends SlottedMultiChildRenderObjectWidget<AnLeadValueSlot, RenderBox> {
  const AnLeadValue({
  ...
  @override
  SlottedContainerRenderObjectMixin<AnLeadValueSlot, RenderBox>
  createRenderObject(BuildContext context) => _RenderLeadValue(wrap: wrap);
  ...
  void updateRenderObject(
    ...
    SlottedContainerRenderObjectMixin<AnLeadValueSlot, RenderBox> renderObject,
  ) {
...
class _RenderLeadValue extends RenderBox
    with SlottedContainerRenderObjectMixin<AnLeadValueSlot, RenderBox> {
  ...
  double computeMinIntrinsicWidth(double height) =>
      _intrinsicWidth((c) => c.getMinIntrinsicWidth(height));
  @override
  double computeMaxIntrinsicWidth(double height) =>
      _intrinsicWidth((c) => c.getMaxIntrinsicWidth(height));
  ...
  double computeMinIntrinsicHeight(double width) =>
      _intrinsicHeight((c) => c.getMinIntrinsicHeight(double.infinity));
  @override
  double computeMaxIntrinsicHeight(double width) =>
      _intrinsicHeight((c) => c.getMaxIntrinsicHeight(double.infinity));
  ...
  void performLayout() {
    ...
    if (aL != null) {
      aL.layout(
        BoxConstraints(maxWidth: w, maxHeight: hMax),
        parentUsesSize: true,
      );
      aLW = aL.size.width;
    }
    ...
    if (leading != null) {
      leading.layout(
        BoxConstraints(maxWidth: avail * _leadingMaxFraction, maxHeight: hMax),
        parentUsesSize: true,
      );
      leadW = leading.size.width;
    }
    final valueMax = math.max(
      0.0,
      avail - leadW,
    ); // value owns the true remainder 值拿真实剩余
    var valW = 0.0;
    if (value != null) {
      value.layout(
        _wrap
            ...
            : BoxConstraints(
                maxWidth: valueMax,
                maxHeight: hMax,
              ), // content width, flush-right 内容宽贴右
        parentUsesSize: true,
      );
    ...
    if (leading != null) {
      _pd(leading).offset = Offset(0, _wrap ? 0 : centerY(leading));
    }
    if (aL != null) _pd(aL).offset = Offset(leadW + gapL, centerY(aL));
    if (value != null) {
      final vx = _wrap
          ? (leadW + gapL + aLW + _gapMid)
          : (w - aVW - gapV - valW);
