...
Stylesheet buildAnEditorStylesheet(AnColors colors, {AnFace? prose}) {
  ...
  TextStyle ink(TextStyle s) =>
      applyContentFace(prose, s).copyWith(color: colors.ink);

  return Stylesheet(
    inlineTextStyler: (attributions, existingStyle) =>
        anInlineTextStyler(colors, attributions, existingStyle),
  ...
  for (final a in attributions) {
    if (a == boldAttribution) {
      ...
      s = s.copyWith(
        ...
        fontVariations:
            const [], // clear the UI VF wght — the mono face isn't that axis 清变量轴
      );
...
double _headingTopFor(Document doc, DocumentNode node) {
  ...
  if (index > 0 && _isHeaderNode(doc.getNodeAt(index - 1))) {
    return AnFlow.headingStack;
  }
...
List<StyleRule> _rules(
  AnColors colors,
  TextStyle Function(TextStyle) ink,
  AnFace? prose,
) {
  return [
    ...
    StyleRule(
      ...
      (doc, node) => {
        Styles.padding: CascadingPadding.only(
          top: isQuoteContinuation(doc, node) ? 0 : AnFlow.block,
        ),
      },
    ),
    ...
    StyleRule(
      ...
      (doc, node) => {
        ...
        Styles.dotColor: colors
            .inkFaint, // chat parity — list markers ride inkFaint (an_markdown.dart)
      },
    ),
    StyleRule(
      ...
      (doc, node) => {
        Styles.padding: CascadingPadding.only(
          top: isQuoteContinuation(doc, node) ? 0 : AnFlow.listItem,
        ),
      },
    ),
    ...
    StyleRule(
      ...
      (doc, node) => {
        Styles.padding: const CascadingPadding.only(top: AnFlow.block),
        Styles.textStyle: applyContentFace(
          prose,
          AnText.reading,
    ...
    StyleRule(
      ...
      (doc, node) => {
        ...
        TableStyles.headerTextStyle: applyContentFace(
          prose,
          AnText.reading,
        ).weight(AnText.emphasisWeight).copyWith(color: colors.ink),
        TableStyles.cellPadding: const CascadingPadding.symmetric(
    ...
    StyleRule(
      ...
      (doc, node) => {
        Styles.padding: const CascadingPadding.only(
          bottom: AnInset.pageBottom + AnGap.section,
        ),
      },
    ),
  ];
