...
// ChatThinking — the reasoning block's "whisper + flow-window" grammar (NOT a tool_call card). Shown on the
// gallery's white cell (= the real white ocean). Labels are English for now (i18n later, user's call); dev
// strings are i18n-exempt. The live flow-window's top fade + bottom-pin is best seen with prose > 5 lines.
//
// ChatThinking——推理块的「低语+流窗」语法(非 tool_call 卡)。衬白 cell(=真实白海洋)。标签暂英文(i18n 回头
// 再说,dev 串豁免)。流窗的顶部渐隐+底部钉住,正文超 5 行才看得出。

const double _thinkW = 620;

// A long reasoning stream (Chinese prose is model CONTENT, not a UI label) — wraps past 5 lines so the live
// window clips + top-fades. 长推理流(中文是模型内容非 UI 标签),超 5 行→裁+渐隐。
const String _long =
...
final GalleryItem chatThinkingGalleryItem = GalleryItem(
  'ChatThinking 推理块',
  '低语 + 左 rail + 限高滚动窗(非卡):思考中显最后 5 行、可上滑回看、顶边窄渐隐;想完收成一行 thought for Ns · expand',
  [
    GallerySpecimen('思考中 · 滚动窗(超 5 行 → 钉底,可上滑,顶边窄渐隐)',
        (_) => _thinking(text: _long, streaming: true), span: true, maxWidth: _thinkW),
    GallerySpecimen('思考中 · 短(未满 5 行,全显不裁不渐隐)',
