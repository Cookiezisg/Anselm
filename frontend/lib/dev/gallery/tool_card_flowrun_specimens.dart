...
BlockNode _replay(String result) =>
    BlockNode(id: 'tc_replay', kind: BlockKind.toolCall)
      ..status = 'completed'
      ..content = {
        'name': 'replay_flowrun',
      ...
      ..children.add(
        BlockNode(id: 'tr_replay', kind: BlockKind.toolResult)
          ..status = 'completed'
          ..content = {'content': result},
      );

BlockNode _trigger(String args, String result) =>
    BlockNode(id: 'tc_trig', kind: BlockKind.toolCall)
      ..status = 'completed'
      ..content = {'name': 'trigger_workflow', 'arguments': args}
      ..children.add(
        BlockNode(id: 'tr_trig', kind: BlockKind.toolResult)
          ..status = 'completed'
          ..content = {'content': result},
      );

BlockNode _getFlowrun(String result) =>
    BlockNode(id: 'tc_gfr', kind: BlockKind.toolCall)
      ..status = 'completed'
      ..content = {
        'name': 'get_flowrun',
        'arguments': '{"flowrunId":"fr_9a8b7c6d5e4f3a2b"}',
      ...
      ..children.add(
        BlockNode(id: 'tr_gfr', kind: BlockKind.toolResult)
          ..status = 'completed'
          ..content = {'content': result},
      );
...
final toolCardFlowrunGalleryItem = GalleryItem(
  ...
  [
    ...
    GallerySpecimen(
      'replay · 仍失败(auto-expand,失败节点红摘要置顶)',
      (c) => ChatToolCard(
        node: _replay(
          _run(
            'failed',
    ...
    GallerySpecimen(
      'replay · 等待审批(节点 parked,run 头仍 running)',
      (c) => ChatToolCard(
        node: _replay(
          _run('running', [
            _node('trigger', 'trigger', 'completed'),
    ...
    GallerySpecimen(
      'replay · 80 封顶(nodeSummary 诚实账 + 循环轮次)',
      (c) => ChatToolCard(
        node: _replay(
          _run(
            'completed',
            [
              _node(
                ...
                'failed',
                iteration: 41,
                error: 'transient: retry budget hit',
              ),
    ...
    GallerySpecimen(
      'trigger_workflow · 带 payload(启动凭据 + get_flowrun 指路)',
      (c) => ChatToolCard(
        node: _trigger(
    ...
    GallerySpecimen(
      'trigger_workflow · 空 payload(明说不装空树)',
      (c) => ChatToolCard(
        node: _trigger(
    ...
    GallerySpecimen(
      'get_flowrun · 完成(run 头条 + 节点台账 + 触发器出处)',
      (c) => ChatToolCard(
        node: _getFlowrun(
          _run('completed', [
            _node('trigger', 'trigger', 'completed'),
    ...
    GallerySpecimen(
      'get_flowrun · 失败(auto-expand + run 级 error 窗)',
      (c) => ChatToolCard(
        node: _getFlowrun(
          _run(
            'failed',
    ...
    GallerySpecimen(
      'get_flowrun · 等待审批(running + parked 节点)',
      (c) => ChatToolCard(
        node: _getFlowrun(
          _run('running', [
            _node('trigger', 'trigger', 'completed'),
