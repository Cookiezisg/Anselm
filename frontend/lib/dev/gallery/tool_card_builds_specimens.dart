  ...
  final r = BlockTreeReducer()
    ...
    ..apply(
      const StreamEnvelope(
        ...
        frame: FrameDelta(
          chunk:
              '{"summary":"Create the quarterly rollup","ops":[{"op":"set_meta","name":"quarterly_rollup",'
...
final toolCardBuildsGalleryItem = GalleryItem(
  ...
  [
    GallerySpecimen(
      'args 流入中 · 活代码窗(本族主打:代码随打字流进窗)',
      (c) => ChatToolCard(node: _streamingBuild()),
      span: true,
    ),
    GallerySpecimen(
      '已创建函数 · v1 · env 就绪(展开:高亮代码+结果条)',
      (c) => ChatToolCard(
        node: _call(
          'fn-ok',
          'create_function',
          args: _fnArgs,
          summary: 'Create the quarterly rollup',
          result:
    ...
    GallerySpecimen(
      'env 失败 · 半成功(危险色回执,自动展开,envError 红)',
      (c) => ChatToolCard(
        node: _call(
          'fn-env',
          'create_function',
    ...
    GallerySpecimen(
      '已创建函数 · env 自愈(装错重试→治好,EnvFixTimeline)',
      (c) => ChatToolCard(
        node: _call(
          'fn-heal',
          'create_function',
    ...
    GallerySpecimen(
      '已更新处理器 · crashed(真 brick:env 好但 __init__ 坏,红 warning 自动展开)',
      (c) => ChatToolCard(
        node: _call(
          'hd-crash',
          'edit_handler',
          args:
              '{"handlerId":"hd_5c4b3a2f1e0d9c8b","ops":[{"op":"add_method","method":{"name":"charge","body":"raise RuntimeError()"}}]}',
    ...
    GallerySpecimen(
      '已更新处理器 · stopped(良性:纯改名、从未 spawn,静音徽不报警)',
      (c) => ChatToolCard(
        node: _call(
          'hd-stop',
          'edit_handler',
          args:
              '{"handlerId":"hd_5c4b3a2f1e0d9c8b","ops":[{"op":"set_meta","name":"billing-webhook"}]}',
          summary: 'Rename the handler',
          result:
              '{"id":"hd_5c4b3a2f1e0d9c8b","version":3,"envStatus":"ready","opsApplied":1,"runtimeState":"stopped"}',
        ),
      ),
      span: true,
    ),
    GallerySpecimen(
      ...
      (c) => ChatToolCard(
        node: _call(
          ...
          result:
              '{"id":"ag_9f8e7d6c5b4a3f2e","versionId":"agv_8877665544332211","version":3}',
        ),
      ),
      span: true,
    ),
    GallerySpecimen(
      '已创建文档(markdown 内容;散文输出→无结果条)',
      (c) => ChatToolCard(
        node: _call(
          'doc',
          'create_document',
          ...
          result:
              'Created document "季度汇总口径" (id=doc_5566778899aabbcc, path=/specs/季度汇总口径)',
        ),
      ),
      span: true,
    ),
    GallerySpecimen(
      '已创建工作流 · v1 · 未激活(★两幕生长:展开→图回放生长)',
      (c) => ChatToolCard(
        node: _call(
          'wf',
          'create_workflow',
          ...
          result:
              '{"id":"wf_00ff00ff00ff00ff","versionId":"wfv_1122334455667788","version":1,"active":false,"lifecycleState":"inactive"}',
        ),
      ),
      span: true,
      stress: true,
    ...
    GallerySpecimen(
      '已更新工作流 · v5(★morph 花名册:绿添 · 琥珀改 · 红删划线)',
      (c) => ChatToolCard(
        node: _call(
          'wfe',
          'edit_workflow',
          ...
          result:
              '{"id":"wf_00ff00ff00ff00ff","versionId":"wfv_5566778899aabbcc","version":5}',
        ),
      ),
      span: true,
      stress: true,
    ...
    GallerySpecimen(
      '已创建控制 · 决策梯(有序 port/when/emit + 否则钉底)',
      (c) => ChatToolCard(
        node: _call(
          'ctl',
          'create_control',
          ...
          result:
              '{"id":"ctl_7d4c9e0112233445","activeVersionId":"ctlv_1122334455667788","version":1}',
        ),
      ),
      span: true,
    ),
    GallerySpecimen(
      '已创建审批 · 表单预览(审批人视角 + 规则条 + mock 决策)',
      (c) => ChatToolCard(
        node: _call(
          'apf',
          'create_approval',
          ...
          result:
              '{"id":"apf_2e9b5c3344556677","activeVersionId":"apfv_1122334455667788","version":1}',
        ),
      ),
      span: true,
    ),
    GallerySpecimen(
      '已创建技能 · 稿子 + allowedTools 警示药丸(预授权=权限让渡)',
      (c) => ChatToolCard(
        node: _call(
          'skl',
          'create_skill',
    ...
    GallerySpecimen(
      '已创建触发器 · webhook(可复制 URL + 密钥 + 签名算法)',
      (c) => ChatToolCard(
        node: _call(
          'trg-hook',
          'create_trigger',
    ...
    GallerySpecimen(
      '已创建触发器 · fsnotify(路径 + 事件 chips + glob)',
      (c) => ChatToolCard(
        node: _call(
          'trg-fs',
          'create_trigger',
    ...
    GallerySpecimen(
      '已更新触发器 · sensor(目标药丸 + 轮询周期 + CEL 条件/输出 · 热更新)',
      (c) => ChatToolCard(
        node: _call(
          'trg-sensor',
          'edit_trigger',
