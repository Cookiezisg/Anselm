...
BlockNode _todoWrite(List<Map<String, String>> items) =>
    BlockNode(id: 'tc_tw', kind: BlockKind.toolCall)
      ..status = 'completed'
      ..content = {
        'name': 'todo_write',
      ...
      ..children.add(
        BlockNode(id: 'tr_tw', kind: BlockKind.toolResult)
          ..status = 'completed'
...
final toolCardTodoGalleryItem = GalleryItem(
  ...
  [
    GallerySpecimen(
      'todo_write · 混合状态(进行中显 activeForm)',
      (c) => ChatToolCard(
        node: _todoWrite([
          {'content': '盘点后端契约', 'status': 'completed'},
          {
            'content': '设计 tool 卡',
            'activeForm': '正在设计 tool 卡',
            'status': 'in_progress',
          },
