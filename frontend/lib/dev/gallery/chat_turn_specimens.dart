...
const double _blockGap =
    AnSpace.s12; // between blocks within one assistant turn 轮内块间距(更紧)
...
final GalleryItem chatTurnGalleryItem = GalleryItem(
  ...
  [
    GallerySpecimen(
      '迷你对话(节奏)',
      (c) => const _MiniTranscript(),
      span: true,
      maxWidth: _turnW,
    ),
    GallerySpecimen(
      '用户泡 · 短',
      (c) => _user(c, '帮我把 sync_inventory 加上失败重试'),
      span: true,
      maxWidth: _turnW,
    ),
    GallerySpecimen(
      ...
      (c) => _user(
        c,
        '这个 workflow 昨晚跑到第 3 个节点就失败了,你能不能先帮我看下是哪个 handler 抛的错、再决定要不要加重试',
      ),
      span: true,
      maxWidth: _turnW,
    ),
    GallerySpecimen(
      '用户泡 · 发送中(淡显)',
      (c) => _user(c, '第 3 次还失败呢?', sending: true),
      span: true,
      maxWidth: _turnW,
    ),
    GallerySpecimen(
      ...
      (c) => _assistant(c, const [
        '好的,我给 sync_inventory 加了指数退避重试——最多 3 次,间隔 1s→2s→4s。',
      ]),
      span: true,
      maxWidth: _turnW,
    ),
    ...
    GallerySpecimen(
      ...
      (c) => _user(
        c,
        'https://example.com/a/really/really/long/url/that/must/wrap/inside/the/bubble/not/blow/it/out?x=1&y=2&z=3',
      ),
      stress: true,
      span: true,
      maxWidth: _turnW,
...
class _MiniTranscript extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final turns = <Widget>[
      ...
      _assistant(context, const [
        '好的,我给 sync_inventory 加了指数退避重试——最多 3 次,间隔 1s→2s→4s。',
      ]),
      _user(context, '第 3 次还失败的话,能不能自动开个 issue?'),
      _assistant(context, const [
        '可以。我在最后一次失败的分支上挂了 create_issue,标题带上 flowrun id,方便你回溯。',
      ]),
    ];
    return Column(
