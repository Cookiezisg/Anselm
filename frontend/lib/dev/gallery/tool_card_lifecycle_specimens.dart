...
final toolCardLifecycleGalleryItem = GalleryItem(
  ...
  [
    GallerySpecimen(
      'revert_function · ⤺ v3(倒带 + 注记)',
      (c) => ChatToolCard(
        node: _call(
          'revert_function',
          '{"functionId":"fn_1a2b"}',
          '{"id":"fn_1a2b","activeVersionId":"fnv_9","version":3}',
        ),
      ),
      span: true,
    ),
    GallerySpecimen(
      'delete_function · 删除审计(3 处引用受影响 + 跳修药丸)',
      (c) => ChatToolCard(
        node: _call(
          'delete_function',
          '{"functionId":"fn_1a2b"}',
    ...
    GallerySpecimen(
      'restart_handler · 绿壳红事实(error 键→失败态自动展开)',
      (c) => ChatToolCard(
        node: _call(
          'restart_handler',
          '{"handlerId":"hd_5c4b"}',
    ...
    GallerySpecimen(
      'deactivate_workflow · draining 半态(琥珀,不自动展开)',
      (c) => ChatToolCard(
        node: _call(
          'deactivate_workflow',
          '{"workflowId":"wf_00ff"}',
          '{"workflowId":"wf_00ff","lifecycleState":"draining"}',
        ),
      ),
      span: true,
    ),
    GallerySpecimen(
      'kill_workflow · 杀停 3 个在途运行(红核自动展开)',
      (c) => ChatToolCard(
        node: _call(
          'kill_workflow',
          '{"workflowId":"wf_00ff"}',
          '{"workflowId":"wf_00ff","killed":3}',
        ),
      ),
      span: true,
    ),
    GallerySpecimen(
      'update_function_meta · 纯改名(动态动词 已改名 + 单端 → b)',
      (c) => ChatToolCard(
        node: _call(
          'update_function_meta',
          '{"functionId":"fn_1a2b","name":"data-fetcher"}',
          '{"id":"fn_1a2b","name":"data-fetcher"}',
        ),
      ),
      span: true,
    ),
    GallerySpecimen(
      'activate_workflow · 上线 → 监听中',
      (c) => ChatToolCard(
        node: _call(
          'activate_workflow',
          '{"workflowId":"wf_00ff"}',
          '{"workflowId":"wf_00ff","lifecycleState":"active","active":true}',
        ),
      ),
      span: true,
    ),
