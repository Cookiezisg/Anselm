...
BlockNode _get(String name, String args, Map<String, dynamic> record) => BlockNode(id: 'tc_$name', kind: BlockKind.toolCall)
  ..status = 'completed'
...
final toolCardDossierGalleryItem = GalleryItem(
  ...
  [
    GallerySpecimen('get_function_execution · 成功(输入/输出 + 日志抽屉 + 出处行)',
        (c) => ChatToolCard(node: _get('get_function_execution', '{"executionId":"fnexec_1a2b3c4d"}', {
              'id': 'fnexec_1a2b3c4d', 'functionId': 'fn_1', 'versionId': 'fnv_3', 'status': 'ok', 'triggeredBy': 'chat',
              'input': {'url': 'https://x.io/api', 'retries': 3}, 'output': {'status': 200, 'bytes': 18422},
              'logs': 'fetch page 1/3\nfetch page 2/3\nfetch page 3/3\ndone', 'elapsedMs': 942,
              'startedAt': '2026-07-05T14:03:00Z', 'endedAt': '2026-07-05T14:03:01Z',
              'conversationId': 'conv_abc123', 'messageId': 'msg_def456', 'createdAt': '2026-07-05T14:03:01Z',
            })),
        span: true),
    GallerySpecimen('get_function_execution · 失败(auto-expand + errorMessage + flowrun 出处)',
        (c) => ChatToolCard(node: _get('get_function_execution', '{"executionId":"fnexec_bad"}', {
              'id': 'fnexec_bad', 'functionId': 'fn_1', 'status': 'failed', 'triggeredBy': 'workflow',
              'input': {'url': 'https://down'}, 'errorMessage': 'ConnectionError: max retries exceeded (3)',
              'elapsedMs': 30021, 'startedAt': '2026-07-05T14:03:00Z', 'endedAt': '2026-07-05T14:03:30Z',
              'flowrunId': 'fr_9a8b7c6d5e4f3a2b', 'flowrunNodeId': 'fetch', 'flowrunIteration': 0,
              'createdAt': '2026-07-05T14:03:30Z',
            })),
        span: true),
    GallerySpecimen('get_handler_call · method() 头徽 + instanceId',
        (c) => ChatToolCard(node: _get('get_handler_call', '{"callId":"hdcall_1"}', {
              'id': 'hdcall_1', 'handlerId': 'hd_1', 'status': 'ok', 'method': 'charge', 'instanceId': 'inst_88a1',
              'triggeredBy': 'agent', 'input': {'account': 'acct_88'}, 'output': {'result': 18240.5},
              'elapsedMs': 610, 'startedAt': '2026-07-05T14:03:00Z', 'endedAt': '2026-07-05T14:03:00Z',
              'createdAt': '2026-07-05T14:03:00Z',
            })),
        span: true),
    GallerySpecimen('get_mcp_call · 失败 + stderr 尾独立染红段',
        (c) => ChatToolCard(node: _get('get_mcp_call', '{"callId":"mcl_1"}', {
              'id': 'mcl_1', 'serverId': 'mcp_acme', 'tool': 'fetch_page', 'status': 'failed', 'triggeredBy': 'agent',
              'input': {'url': 'https://x'}, 'errorMessage': 'tool call failed',
              'logs': 'connecting to acme…\ninvoking fetch_page\n'
        ...
        span: true),
    GallerySpecimen('get_activation · fired(returnValue + payload + 扇出 + 触发器出处)',
        (c) => ChatToolCard(node: _get('get_activation', '{"activationId":"act_1"}', {
              'id': 'act_1', 'triggerId': 'trg_7a8b9c0d1e2f3a4b', 'kind': 'sensor', 'fired': true, 'firingCount': 2,
              'returnValue': {'temp': 31.4, 'threshold': 30}, 'payload': {'manual': false, 'temp': 31.4},
              'createdAt': '2026-07-05T14:03:00Z',
            })),
        span: true),
    GallerySpecimen('get_activation · not fired(condition false,无因果链)',
        (c) => ChatToolCard(node: _get('get_activation', '{"activationId":"act_2"}', {
              'id': 'act_2', 'triggerId': 'trg_7a8b9c0d1e2f3a4b', 'kind': 'sensor', 'fired': false, 'firingCount': 0,
              'detail': 'condition evaluated false', 'returnValue': {'temp': 22.0, 'threshold': 30},
