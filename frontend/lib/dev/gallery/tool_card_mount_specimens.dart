...
BlockNode _call(String name, String args, String result) =>
    BlockNode(id: 'tc_${name.hashCode}', kind: BlockKind.toolCall)
      ...
      ..children.add(BlockNode(id: 'tr_${name.hashCode}', kind: BlockKind.toolResult)
        ..status = 'completed'
...
final toolCardMountGalleryItem = GalleryItem(
  ...
  [
    GallerySpecimen('mcp__linear__create_issue(server/tool chip + 纯字符串结果)',
        (c) => ChatToolCard(
            node: _call('mcp__linear__create_issue', '{"title":"Fix rollup","team":"ENG"}',
                'Created issue ENG-421: "Fix rollup"\nURL: https://linear.app/eng/issue/ENG-421')),
        span: true),
    GallerySpecimen('mcp__linear__x · 服务器离线(MCP 错误红,自动展开)',
        (c) => ChatToolCard(
            node: _call('mcp__linear__create_issue', '{"title":"x"}',
                'mcp server "linear" is not connected: MCP_SERVER_NOT_CONNECTED')),
        span: true),
    GallerySpecimen('billing_webhook__charge(handler.method() chip + {result} JSON 树)',
        (c) => ChatToolCard(
            node: _call('billing_webhook__charge', '{"amount":4200,"currency":"USD"}',
