...
import 'an_relation_graph_specimens.dart';
...
final GalleryCategory _perf = GalleryCategory('性能 Perf', AnIcons.trigger, [
  GalleryItem(
    'W0 流式压力床',
    '1MB content / 50 op每秒 / 5000 词 prompt——增量 args 会话 + revision 记忆化走真卡;▶ 开跑,HUD 绿(<16ms)即门禁过',
    perfSpecimens,
  ),
]);
...
final GalleryCategory _notifications =
    GalleryCategory('通知 Notifications', AnIcons.bell, [
      GalleryItem(
        'NotificationRow 通知行',
        '未读点·tone 图标·{类}「{名}」{动词}·相对时间 + 可选灰详情行;已读整行灰留列表;hover→mark-read',
        notificationRowSpecimens,
      ),
    ]);
...
// 朴素 KV 文档概览(无专属 hero)。
final GalleryCategory _entityViz = GalleryCategory(
  '实体可视化 Entity Viz',
  AnIcons.entities,
  [
    anRelationGraphGalleryItem,
    anScheduleTrackGalleryItem,
    anRunMatrixGalleryItem,
    GalleryItem(
      'AnGraphCanvas 编排图画布',
      'workflow hero:节点卡 + 正交圆角边 + 回边虚线弧 + 平移缩放 fit;framed=实体页预览框',
      [
        GallerySpecimen(
          '线性 (trigger→action→agent)',
          (_) => AnGraphCanvas(graph: _gLinear, framed: true),
          span: true,
        ),
        GallerySpecimen(
          '分支+端口+回边 (pr_merge_flow)',
          (_) => AnGraphCanvas(graph: _gBranch, framed: true),
          span: true,
        ),
        GallerySpecimen(
          '纵向 TB (回边走右通道)',
        ...
        GallerySpecimen(
          '手摆 pos 优先 (不重排)',
          (_) => AnGraphCanvas(graph: _gPinned, framed: true),
          span: true,
        ),
        GallerySpecimen(
          '选中 + 进入编辑器',
        ...
        GallerySpecimen(
          '编辑器形态 (非 framed 满幅)',
          (_) => AnGraphCanvas(graph: _gBranch),
          span: true,
          height: 420,
        ),
        GallerySpecimen(
        ...
        GallerySpecimen(
          '运行中 (taken 加粗+彗星+running 呼吸)',
          (_) => AnGraphCanvas(
            graph: _gBranch,
            framed: true,
            run: const GraphRunState(
              nodes: {
                'on_pr_merged': GraphNodeRun.completed,
                'run_tests': GraphNodeRun.completed,
                'branch_result': GraphNodeRun.running,
              },
          ...
          (_) => AnGraphCanvas(
            ...
            run: const GraphRunState(
              nodes: {
                'on_pr_merged': GraphNodeRun.completed,
                'run_tests': GraphNodeRun.completed,
                'branch_result': GraphNodeRun.completed,
                'approve_rollback': GraphNodeRun.parked,
              },
              iters: {
                'on_pr_merged': 1,
                'run_tests': 1,
        ...
        GallerySpecimen(
          '空图',
          (_) => AnGraphCanvas(graph: const Graph(), framed: true),
          stress: true,
          span: true,
        ),
        GallerySpecimen(
          '海量 (40 节点扇出)',
          (_) => AnGraphCanvas(graph: _gHuge, framed: true),
          stress: true,
          span: true,
        ),
        GallerySpecimen(
          'unknown kind + 超长 + 注入',
          (_) => AnGraphCanvas(graph: _gHostile, framed: true),
          stress: true,
          span: true,
        ),
      ],
    ...
    GalleryItem(
      'AnNodeGantt 节点甘特',
      'flowrun 逐节点时段条:状态色 + ×N 循环 + parked 等待框 + 未运行占位',
      [
        GallerySpecimen(
          '完成/失败/循环×3/parked/未运行',
          (_) => Padding(
...
final List<GanttRow> _ganttSample = [
  const GanttRow(
    ...
    ref: 'trg_3a1f',
    status: 'completed',
    segments: [GanttSegment(0.0, 0.08)],
    parked: false,
    iterations: 1,
  ),
  const GanttRow(
    ...
    status: 'completed',
    segments: [
      GanttSegment(0.1, 0.2),
      GanttSegment(0.34, 0.2),
      GanttSegment(0.58, 0.18),
    ],
    parked: false,
    iterations: 3,
  ),
  const GanttRow(
    nodeId: 'branch_result',
    kind: NodeKind.control,
    ref: 'ctl_7d4c',
    status: 'completed',
    segments: [GanttSegment(0.78, 0.06)],
    parked: false,
    iterations: 1,
  ),
  const GanttRow(
    ...
    ref: 'apf_2e9b',
    status: 'parked',
    segments: [GanttSegment(0.86, 0.12)],
    parked: true,
    iterations: 1,
  ),
  const GanttRow(
...
Node _gn(String id, NodeKind k, String ref, {NodePosition? pos}) =>
    Node(id: id, kind: k, ref: ref, pos: pos);
Edge _ge(String id, String from, String to, {String? port}) =>
    Edge(id: id, from: from, fromPort: port, to: to);

final Graph _gLinear = Graph(
  nodes: [
    _gn('on_tick', NodeKind.trigger, 'trg_3a1f9c2b'),
    _gn('fetch', NodeKind.action, 'fn_5b2e1a77'),
    _gn('summarize', NodeKind.agent, 'ag_9c4d21aa'),
  ],
  edges: [_ge('e1', 'on_tick', 'fetch'), _ge('e2', 'fetch', 'summarize')],
);

final Graph _gBranch = Graph(
  nodes: [
...
final Graph _gPinned = Graph(
  nodes: [
    _gn(
      'webhook',
      NodeKind.trigger,
      'trg_11aa22bb',
      pos: const NodePosition(x: 0, y: 90),
    ),
    _gn(
      'etl',
      NodeKind.action,
      'fn_33cc44dd',
      pos: const NodePosition(x: 260, y: 0),
    ),
    _gn(
      'review',
      NodeKind.approval,
      'apf_55ee66ff',
      pos: const NodePosition(x: 260, y: 180),
    ),
    _gn(
      'publish',
      NodeKind.action,
      'fn_77aa88bb',
      pos: const NodePosition(x: 520, y: 90),
    ),
  ],
  edges: [
    _ge('e1', 'webhook', 'etl'),
  ...
  nodes: [
    ...
    for (var i = 0; i < 13; i++) ...[
      _gn(
        'fan$i',
        NodeKind.action,
        'fn_${i.toRadixString(16).padLeft(8, '0')}',
      ),
      _gn('mid$i', NodeKind.agent, 'ag_${i.toRadixString(16).padLeft(8, '0')}'),
      _gn(
        'end$i',
        NodeKind.action,
        'fn_${(i + 100).toRadixString(16).padLeft(8, '0')}',
      ),
    ],
  ],
  edges: [
...
final Graph _gHostile = Graph(
  nodes: [
    _gn(
      'trigger_with_an_unreasonably_long_node_identifier_name',
      NodeKind.trigger,
      'trg_ffffffffffffffff_more_more_more',
    ),
...
final GalleryCategory _toolCards =
    GalleryCategory('工具卡 Tool Cards', AnIcons.tool, [
...
final GalleryCategory _chatRail = GalleryCategory('对话 Chat', AnIcons.chat, [
  GalleryItem(
    '侧幕 Sidestage(W1 原语)',
    '右岛伴生舞台的地基件:演员表行(新鲜度晕/多动词/墓碑)+频道条+跟随/人闸药丸+诚实丝带;编排在真壳验',
    sidestageSpecimens,
  ),
  GalleryItem('会话行 Conversation row', '状态点 · 标题 · 时间/⋯;rail 列表的一行', [
    GallerySpecimen(
      '普通 (无点)',
      (_) => const AnRow(label: '周报初稿整理', meta: '1 小时前'),
      span: true,
      maxWidth: _railW,
    ),
    GallerySpecimen(
      '生成中 (蓝呼吸)',
      (_) => const AnRow(dot: AnStatus.run, label: '竞品日报流程', meta: '刚刚'),
      span: true,
      maxWidth: _railW,
    ),
    GallerySpecimen(
    ...
    GallerySpecimen(
      ...
      (_) => const AnRow(
        dot: AnStatus.done,
        label: 'AI 编辑 · sync_inventory 加重试',
        meta: '10 分前',
      ),
      span: true,
      maxWidth: _railW,
    ...
    GallerySpecimen(
      '已归档 (灰标记)',
      (_) => const AnRow(dot: AnStatus.idle, label: '旧版迁移笔记', meta: '上月'),
      span: true,
      maxWidth: _railW,
    ),
    GallerySpecimen(
    ...
    GallerySpecimen(
      '无 lead 槽 (leadless · 大纲目录)',
      (_) => const AnRow(label: '目录行 · 无图标槽顶格', leadless: true),
      span: true,
      maxWidth: _railW,
    ),
    GallerySpecimen(
      '超长截断',
      (_) => const AnRow(
        label: '一个非常非常长的对话标题，应当省略号截断而不撑破侧栏宽度无限延伸下去',
        meta: '3 天前',
      ),
      stress: true,
      span: true,
    ...
    GallerySpecimen(
      '注入转义',
      (_) => const AnRow(label: '<b>not</b> & <i>html</i> 注入标题', meta: '上周'),
      stress: true,
      span: true,
      maxWidth: _railW,
    ),
  ...
  GalleryItem(
    'ChatContextMark 上下文压缩低语',
    '回合间的系统时间轴标记(发丝线夹 layers 图标 + 本地化文案,count 从后端英文 marker 解出);不借 thinking 左轨',
    [
      GallerySpecimen(
        '带数量',
        (_) => const ChatContextMark(
          marker:
      ...
      GallerySpecimen(
        '无数量 (裸标签)',
        (_) => const ChatContextMark(marker: 'Context compacted.'),
        span: true,
      ),
      GallerySpecimen(
        '窄宽截断',
        (_) => const ChatContextMark(
          marker:
              'Context compacted — 128 earlier blocks folded into the running summary.',
        ),
        span: true,
        stress: true,
        maxWidth: 260,
  ...
  GalleryItem('AnAttachmentChip 待发附件', 'composer 附件条的一枚;上传中/就绪/失败(点体重试)', [
    ...
    GallerySpecimen(
      ...
      (_) => AnAttachmentChip(
        kind: 'text',
        filename:
            'a-very-long-file-name-that-must-ellipsis-instead-of-blowing-out.txt',
        meta: '4 KB',
        onRemove: () {},
      ),
      stress: true,
      ...
      (_) => AnMentionPanel(
        items: const [
          AnMentionRowData(
            kind: 'function',
            name: 'sync_inventory',
            description: '同步库存到仓库主档',
          ),
          AnMentionRowData(
            kind: 'handler',
            name: 'on_order_created',
            description: '订单创建后的入库钩子',
          ),
          AnMentionRowData(
            kind: 'agent',
            name: 'report_writer',
            description: '周报/日报撰写代理',
          ),
          AnMentionRowData(
            kind: 'workflow',
            name: 'daily_digest',
            description: '每日竞品摘要流水线',
          ),
        ],
        activeIndex: 1,
        onPick: (_) {},
      ...
      (_) => AnMentionPanel(
        items: const [
          AnMentionRowData(
            kind: 'agent',
            name:
                'a_very_long_entity_name_that_must_ellipsis_not_overflow_the_panel_row',
...
_g1Controls = GalleryCategory('基础控件 Controls', AnIcons.sliders, [
    ...
    GallerySpecimen(
      ...
      (_) => const SizedBox(
        ...
        child: Row(
          ...
          children: [Text('A'), AnDivider.vertical(), Text('B')],
        ),
      ),
    ),
  ]),
  ...
  GalleryItem(
    'AnQuoteBar 引用左条',
    '唯一引用/旁白文法:lineStrong 左细条 + 左内距(markdown 引用 / 人闸回显共用)',
    [
      GallerySpecimen(
        'quote',
        (_) => const AnQuoteBar(child: Text('引用的一段旁白,静默声。')),
        span: true,
      ),
    ],
  ),
  GalleryItem(
    'AnHoverSurface hover 填充',
    '可点行的圆角 hover 填充(active→surfaceHover);AnCard 描边 hover 的行背景对偶',
    [
      GallerySpecimen(
        'rest',
        (_) => const AnHoverSurface(
          active: false,
          child: Padding(padding: AnInset.tight, child: Text('static row')),
        ),
      ),
      GallerySpecimen(
        'active',
        (_) => const AnHoverSurface(
          active: true,
          child: Padding(padding: AnInset.tight, child: Text('hovered row')),
        ),
      ),
    ],
  ),
  GalleryItem(
    'AnTonedPanel 语义面板',
    'card-16 白面 + 语义 tone 发丝边 + 卡内距;普通装饰容器(可嵌 AnWindow,人闸壳)',
    [
      GallerySpecimen(
        'danger 边',
        (context) => AnTonedPanel(
          borderColor: context.colors.danger,
          child: const Text('危险决策面板'),
        ),
      ),
      GallerySpecimen(
        'accent 边',
        (context) => AnTonedPanel(
          ...
          child: const Text('询问面板'),
        ),
      ),
    ],
  ),
  GalleryItem(
    'AnWashHighlight 落点洗亮',
    '一次性 accentSoft 填充洗亮(先驻留后淡出);跳转/滚动落点到达高亮',
    [
      GallerySpecimen(
        'wash (加载即播一次)',
        (_) => const AnWashHighlight(
          child: Padding(padding: AnInset.card, child: Text('刚跳到这行')),
        ),
        span: true,
      ),
    ],
  ...
  GalleryItem(
    'AnPopSurface 浮层面',
    '唯一浮层白岛(surface + chip 圆角 + line 发丝边 + shadowPop 抬起);菜单/下拉/编辑器格式条·链接条同一盒',
    [
      GallerySpecimen(
        'pop surface',
        (_) => const AnPopSurface(
          child: Padding(padding: AnInset.card, child: Text('浮起的白岛面')),
        ),
      ),
    ],
  ),
  GalleryItem('AnStatusDot', '语义状态点(run 呼吸)+ raw 直喂色形(批5:珠串/色点/fire 记号唯一实现)', [
    for (final s in AnStatus.values)
      GallerySpecimen(s.name, (_) => AnStatusDot(s)),
    ...
    GallerySpecimen(
      'raw swatch 档 (色板)',
      (context) => AnStatusDot.raw(context.colors.accent, size: AnSize.swatch),
    ),
    GallerySpecimen(
      'raw dotSm 档 (芯片内点)',
      (context) => AnStatusDot.raw(context.colors.warn, size: AnSize.dotSm),
    ),
  ]),
  GalleryItem('AnChip', '状态/标签药丸 + 可选点', [
    ...
    GallerySpecimen(
      'danger',
      (_) => const AnChip('failed', tone: AnTone.danger),
    ),
    GallerySpecimen(
      'accent',
      (_) => const AnChip('active', tone: AnTone.accent),
    ),
    GallerySpecimen(
      'dot=done',
      (_) => const AnChip(
    ...
    GallerySpecimen(
      ...
      (_) => const AnChip(
        'running',
        tone: AnTone.accent,
        dot: AnStatusDot(AnStatus.run),
      ),
    ),
    GallerySpecimen(
      '超长截断',
      (_) => const AnChip(
        'a-very-long-tag-that-must-truncate-not-blow-out',
        tone: AnTone.ok,
      ),
      stress: true,
      maxWidth: 150,
    ...
    GallerySpecimen(
      '注入转义',
      (_) => const AnChip('<b>not</b> & <i>html</i>', tone: AnTone.warn),
      stress: true,
    ),
    GallerySpecimen(
      'outlined (轻列表芯片)',
      (_) => const AnChip('util', look: AnChipLook.outlined),
    ),
    GallerySpecimen(
      'outlined + mono + copy (id 芯片)',
      (_) => const AnChip(
        'fn_3a9f0e88',
        look: AnChipLook.outlined,
        mono: true,
        copyValue: 'fn_3a9f0e88',
      ),
    ),
    GallerySpecimen(
      'strikethrough (删除态)',
    ...
    GallerySpecimen(
      ...
      (_) => const AnChip(
        ...
        look: AnChipLook.outlined,
        mono: true,
        copyValue: '/src/parser/lexer.dart',
        tooltip: '/src/parser/lexer.dart',
      ),
    ),
    GallerySpecimen(
      'icon-only (空标签纯示能)',
      (_) => const AnChip(
        '',
        look: AnChipLook.outlined,
        copyValue: 'the-full-payload',
      ),
    ),
  ]),
  GalleryItem(
    'AnKeycap 键帽',
    '快捷键弦控件脸(0719 紧凑档):静息=逐键小帽([⌘][B],20 高 mono 12、s2 间距,行回 32 节律);录制/冲突=宽板专属;刻意不可聚焦(键盘归宿主,settings 焦点序教训)、不进 AnChip(输入控件非标签)',
    [
      GallerySpecimen(
        'idle 逐键帽 (⌘⇧B)',
        (_) => AnKeycap('⌘⇧B', keys: const ['⌘', '⇧', 'B'], onTap: () {}),
      ),
      GallerySpecimen(
        'idle 单帽兜底 (无 keys)',
        (_) => AnKeycap('⌘K', onTap: () {}),
      ...
      GallerySpecimen(
        'recording (录制中,宽板)',
        (_) => const AnKeycap('按下快捷键…', state: AnKeycapState.recording),
      ),
      GallerySpecimen(
        'error (冲突,宽板)',
        (_) => const AnKeycap('按下快捷键…', state: AnKeycapState.error),
      ),
    ],
  ),
  GalleryItem(
    'AnSpinner 转圈',
    '唯一小型不定转圈(批7 B-071):icon 档淡环 strokeWidth 2;a11y 门=orAssistive(装饰循环冻成静态 spin 字形);行脚/面板加载用,整面占位归 AnState',
    [
      GallerySpecimen('default (icon 16)', (_) => const AnSpinner()),
      GallerySpecimen('iconLg 20', (_) => const AnSpinner(size: AnSize.iconLg)),
      GallerySpecimen(
        'stateIcon 40 (AnState loading 同款)',
        (_) => const AnSpinner(size: AnSize.stateIcon),
      ),
    ],
  ),
  GalleryItem(
    'AnFadeRiseIn 入场淡升',
    '仅入场一次的淡入上移(批7 B-056,chat landing 升格):mid/easeOut/默认 s6;reduced 静态;与命中列级联(单控制器错峰)角色不同',
    [
      GallerySpecimen(
        'replayable (换 key 重播)',
        (_) => AnFadeRiseIn(key: UniqueKey(), child: const Text('欢迎回来')),
      ),
    ],
  ),
  GalleryItem(
    'AnDropVeil 拖放面纱',
    '拖放悬停面纱(批7 B-070,chat 升格):AnOpacity.veil 表面洗微透底+居中字形提示;IgnorePointer 指针穿透,文案字形归调用方',
    [
      GallerySpecimen(
        'over content (盖在内容上)',
        (context) => SizedBox(
  ...
  GalleryItem(
    'AnSwatch 色板件',
    '「色即内容」原语(与 AnStatusDot 语义状态色两类):身份点 dot 10 / 取色格 pick 22 + 选中环 ring 档',
    [
      GallerySpecimen(
        'dot (身份点)',
        (context) => AnSwatch(
          parseHexColor(kAvatarPalette.first, context.colors.accent),
          ...
          children: [
            for (final (i, hex) in kAvatarPalette.indexed)
              Padding(
                ...
                child: AnSwatch(
                  parseHexColor(hex, context.colors.accent),
                  selected: i == 1,
                  onTap: () {},
                ),
              ),
  ...
  GalleryItem(
    'AnInlineCapsule 行内药囊',
    '唯一贴基线文内壳(WidgetSpan 专用):软 tone 底+tag 圆角+宿主字体;{{CEL}} 琥珀囊/[[id]] 药丸/编辑器提及皆骑它',
    [
      GallerySpecimen(
        'accent (默认,[[id]] 药丸)',
        (context) => Text.rich(
          TextSpan(
            children: [
              TextSpan(
                text: '引用 ',
                style: AnText.reading.copyWith(color: context.colors.ink),
              ),
              const WidgetSpan(
                alignment: PlaceholderAlignment.middle,
                child: AnInlineCapsule('quarterly-fix.md'),
              ),
              TextSpan(
                text: ' 的模板。',
                style: AnText.reading.copyWith(color: context.colors.ink),
        ...
        (context) => Text.rich(
          TextSpan(
            children: [
              ...
              const WidgetSpan(
                alignment: PlaceholderAlignment.middle,
                child: AnInlineCapsule('{{amount}}', tone: AnTone.warn),
              ),
              TextSpan(
                text: ' 时进审批。',
                style: AnText.reading.copyWith(color: context.colors.ink),
      ...
      GallerySpecimen(
        '带字形 (提及,经 AnRefPill.inline)',
        (_) => AnInlineCapsule('sync_inventory', icon: AnIcons.function),
      ),
      GallerySpecimen(
        '超长换行 (禁截断)',
        (context) => SizedBox(
              ...
              children: [
                const WidgetSpan(
                  ...
                  child: AnInlineCapsule(
                    'an-extremely-long-inline-reference-name-that-wraps-not-truncates',
                  ),
                ),
              ],
            ),
  ...
  GalleryItem('AnGroupLabel', '极薄分组小标题', [
    GallerySpecimen(
      'default',
      (_) => const AnGroupLabel('Entities'),
      span: true,
    ),
    GallerySpecimen(
      '超长截断',
      (_) => const AnGroupLabel(
        'a very long section caption that should ellipsis instead of wrapping',
      ),
      stress: true,
      maxWidth: 150,
    ),
  ...
  GalleryItem('AnButton', '统一动作钮:变体/尺寸/图标/态', [
    ...
    GallerySpecimen(
      'surface·灰洗底 (Stop/Retry hover 钮)',
      (context) => ColoredBox(
    ...
    GallerySpecimen(
      'size=sm',
      (_) => AnButton(label: 'Small', size: AnButtonSize.sm, onPressed: () {}),
    ),
    GallerySpecimen(
      'disabled',
      (_) => const AnButton(label: 'Disabled', onPressed: null),
    ),
    GallerySpecimen(
      'block',
      (_) => AnButton(
    ...
    GallerySpecimen(
      '超长截断',
      (_) => AnButton(
        label: 'a-really-long-button-label-that-must-ellipsis-within-its-box',
        block: true,
        onPressed: () {},
      ),
      stress: true,
  ...
  GalleryItem('AnInput', '值叶子:单行/多行/等宽', [
    ...
    GallerySpecimen(
      'compact 24 · 「#」记号占位(0718,AnPager 跳页格档)',
      (_) => const AnInput(
        placeholder: '#',
        compact: true,
        tabular: true,
    ...
    GallerySpecimen(
      'mono',
      (_) => const AnInput(initialValue: 'fn_3a9f', mono: true),
    ),
    GallerySpecimen(
      'readonly',
      (_) => const AnInput(initialValue: 'read only', readOnly: true),
    ),
    GallerySpecimen(
      'disabled',
      (_) => const AnInput(initialValue: 'disabled', enabled: false),
    ),
    GallerySpecimen(
      'multiline full',
      (_) => const AnInput(
    ...
    GallerySpecimen(
      ...
      (_) => const AnInput(
        initialValue:
            'this-is-an-extremely-long-single-line-value-that-should-scroll-horizontally-and-never-overflow-the-bordered-box',
        block: true,
      ),
      stress: true,
      maxWidth: 180,
  ...
  GalleryItem('AnActionGroup', '动作组:对齐/间距/换行', [
    GallerySpecimen(
      'default',
      (_) => AnActionGroup([
        AnButton(label: 'Cancel', onPressed: () {}),
        AnButton(
          label: 'Save',
          variant: AnButtonVariant.primary,
          onPressed: () {},
    ...
    GallerySpecimen(
      ...
      (_) => AnActionGroup(
        [
          AnButton(label: 'A', size: AnButtonSize.sm, onPressed: () {}),
          AnButton(label: 'B', size: AnButtonSize.sm, onPressed: () {}),
        ],
        end: true,
        compact: true,
      ),
    ...
    GallerySpecimen(
      'stack',
      (_) => AnActionGroup([
        AnButton(label: 'First', block: true, onPressed: () {}),
        AnButton(label: 'Second', block: true, onPressed: () {}),
      ], stack: true),
      span: true,
    ),
  ]),
  GalleryItem('AnEditAffordance', '就地编辑触发器原语:铅笔 → 取消/保存', [
    GallerySpecimen(
      'idle (铅笔)',
      (_) => AnEditAffordance(editing: false, onEdit: () {}),
    ),
    GallerySpecimen(
      'editing (取消/保存)',
      (_) => AnEditAffordance(editing: true, onCommit: () {}, onAbort: () {}),
    ),
  ]),
  GalleryItem('AnInlineEdit', '就地重命名:文字 → 自适应框(增长→封顶→截断,按钮跟随→钉右)', [
    GallerySpecimen(
      'idle (点铅笔进编辑)',
      (_) => AnInlineEdit(value: 'Untitled workflow', onCommit: (_) {}),
    ),
    GallerySpecimen(
      'editing 态',
      (_) => AnInlineEdit(
    ...
    GallerySpecimen(
      '超长·idle (省略+铅笔钉右)',
      (_) => AnInlineEdit(
        value: 'A very long entity title that must ellipsis when idle',
        onCommit: (_) {},
      ),
      stress: true,
      maxWidth: 220,
    ...
    GallerySpecimen(
      '超长·editing (框封顶→按钮钉右→横滚)',
      (_) => AnInlineEdit(
        value:
            'A very long title being edited that grows, caps at the row, then scrolls',
        startEditing: true,
        onCommit: (_) {},
      ),
      stress: true,
  ...
  GalleryItem('AnDropdown', '受控单选下拉 + 富行菜单', [
    ...
    GallerySpecimen(
      'single value(无 meta)',
      (_) => const _DropdownDemo(initial: 'med', simple: true),
    ),
    GallerySpecimen(
      'placeholder',
      (_) => const _DropdownDemo(initial: null, simple: true),
    ),
    GallerySpecimen(
      'ghost',
      (_) => const _DropdownDemo(initial: 'ag', ghost: true),
    ),
    GallerySpecimen(
      'disabled',
      (_) => const AnDropdown<String>(
        options: [],
        value: null,
        onChanged: null,
    ...
    GallerySpecimen(
      'block',
      (_) => const _DropdownDemo(initial: 'wf', block: true),
      span: true,
    ),
    GallerySpecimen(
      '两区都超长',
      (_) => AnDropdown<String>(
        options: const [
          AnDropdownOption(
            value: 'x',
            label:
                'An extremely long entity name that must ellipsis on the left',
            meta: 'a_very_long_identifier_that_also_truncates',
          ),
        ],
        value: 'x',
        onChanged: (_) {},
    ...
    GallerySpecimen(
      '海量选项',
      (_) => const _DropdownDemo(initial: '0', massive: true),
      stress: true,
    ),
  ]),
]);
...
// ── G2 — Feedback states ──
final GalleryCategory
_g2Feedback = GalleryCategory('反馈态 Feedback', AnIcons.info, [
          ...
          child: Stack(
            children: [
              Padding(
                ...
                child: Text(
                  [
                    for (var i = 0; i < 8; i++) 'scrolling content line $i',
                  ].join('\n'),
                  style: AnText.body.copyWith(color: context.colors.inkMuted),
                ),
              ),
              Positioned(
                ...
                right: 0,
                height: 20,
                child: AnEdgeFade(fromTop: true, color: context.colors.surface),
              ),
              Positioned(
                bottom: 0,
                left: 0,
  ...
  GalleryItem('AnCallout', '通栏语气提示条:图标 + 文案 + 动作 + 关闭', [
    GallerySpecimen(
      'info',
      (_) => const AnCallout('Heads up — this workflow has unsaved changes.'),
      span: true,
    ),
    GallerySpecimen(
      'ok',
    ...
    GallerySpecimen(
      'warn',
      (_) => const AnCallout(
        'The sandbox runtime is not installed yet.',
    ...
    GallerySpecimen(
      'danger',
      (_) => const AnCallout(
        'Deploy failed — the trigger could not reach the handler.',
    ...
    GallerySpecimen(
      'title + body',
      (_) => const AnCallout(
        'Re-run skipped nodes, or replay the whole flow from the failed step.',
        title: 'Run finished with 2 failures',
        severity: AnCalloutSeverity.warn,
      ),
      span: true,
    ),
  ...
  GalleryItem('AnState', '空/载/错 整块占位:图标 + 标题 + 提示 + 动作', [
    GallerySpecimen(
      ...
      (_) => AnState(
        ...
        hint: 'Create your first Function to get started.',
        action: AnButton(
          label: 'New Function',
    ...
    GallerySpecimen(
      'loading',
      (_) => const AnState(kind: AnStateKind.loading, title: 'Loading…'),
      span: true,
    ),
    GallerySpecimen(
      'error',
      (_) => AnState(
        ...
        hint: 'Check the backend is running, then try again.',
        action: AnButton(label: 'Try again', onPressed: () {}),
      ),
      span: true,
    ),
    GallerySpecimen(
      ...
      (_) => AnState(
        kind: AnStateKind.error,
        fatal: true,
        title: "Can't reach the local engine",
        hint: 'The backend did not start.',
        detail: 'dial tcp 127.0.0.1:7777: connection refused',
        action: AnButton(
          label: 'Retry',
          variant: AnButtonVariant.primary,
          onPressed: () {},
    ...
    GallerySpecimen(
      ...
      (_) => const AnState(
        ...
        title: 'Nothing here',
        hint: 'This panel has no content.',
      ),
      span: true,
    ),
    GallerySpecimen(
      ...
      (_) => const AnState(
        kind: AnStateKind.error,
        title:
            'A long error title that should wrap and stay centered without overflowing',
        hint:
            'An equally long explanatory hint sentence that must wrap onto several centered lines within the capped content column and never overflow.',
      ),
      stress: true,
      maxWidth: 260,
    ),
  ...
  GalleryItem('AnStepper', '步骤进度:done/current/upcoming(1-based,可点跳回)', [
    ...
    GallerySpecimen(
      '海量步 (4/10)',
      (_) => const AnStepper(count: 10, current: 4),
      stress: true,
      maxWidth: 200,
    ),
  ]),
  ...
  GalleryItem(
    'AnShimmerText',
    '「正在忙」文字流光:光带扫过字形(降级/inactive=静态);chat thinking · 运行中 tool 名复用',
    [
      GallerySpecimen(
        'active · meta',
        (c) => AnShimmerText(
          'thinking',
          style: AnText.meta.copyWith(color: c.colors.inkMuted),
        ),
        span: true,
      ),
      GallerySpecimen(
  ...
  GalleryItem('AnTypewriter', '打字机:type→hold→delete→循环(降级=静态主句)', [
    ...
    GallerySpecimen(
      'emoji 字素',
      (_) => const AnTypewriter(['Ready 👋🏽 to go'], loop: false),
    ),
    GallerySpecimen(
      '无光标',
      (_) =>
          const AnTypewriter(['No caret here'], loop: false, showCaret: false),
    ),
  ]),
  GalleryItem('AnTags', '可编辑标签集:药丸 + 健康点 + 内联添加(重复拒+闪)', [
    GallerySpecimen(
      'multi (可编辑)',
      (_) => const _TagsDemo(initial: ['agent', 'workflow']),
      span: true,
    ),
    GallerySpecimen(
      'single (单值)',
      (_) => const _TagsDemo(initial: ['medium'], single: true),
    ),
    GallerySpecimen('空 (placeholder)', (_) => const _TagsDemo()),
    GallerySpecimen(
      'readOnly + tone/health',
      (_) => const AnTags(
        readOnly: true,
        tags: [
          AnTag('passed', tone: AnTone.ok, health: AnStatus.done),
          AnTag('pending', tone: AnTone.warn, health: AnStatus.wait),
          AnTag('failed', tone: AnTone.danger, health: AnStatus.err),
        ],
      ),
      span: true,
    ),
    GallerySpecimen(
      'reading (内容 KV 值位:24 高/13 字)',
      (_) => const AnTags(
        readOnly: true,
        reading: true,
        tags: [AnTag('util'), AnTag('io')],
      ),
      span: true,
    ),
    GallerySpecimen(
...
// ── G3 — Rows & cards ──
final GalleryCategory
_g3RowsCards = GalleryCategory('行与卡 Rows & Cards', AnIcons.entities, [
  GalleryItem(
    'AnSunkenPanel 凹陷面板',
    'surfaceSunken 底 + chip 圆角 + s12/s8 内距;灰凹面退役后唯一住户=聊天用户泡(机器窗一律 AnWindow)',
    [
      GallerySpecimen(
        '用户泡 (唯一灰幸存者)',
        (_) => const AnSunkenPanel(
          child: Text(
            'A contained, non-interactive well one rung below the surface.',
          ),
        ),
        span: true,
      ),
  ...
  GalleryItem(
    'AnFormField 纵向表单字段',
    '标签在上 + 可选 desc + 可选类型徽章 + block 控件在下(区别于横向 AnField)',
    [
      GallerySpecimen(
        'label + 控件',
        (_) => const AnFormField(
          label: 'Method',
          child: AnInput(placeholder: 'select…', block: true),
        ),
        span: true,
      ),
      GallerySpecimen(
        ...
        (_) => const AnFormField(
          label: 'Payload',
          desc: 'JSON body sent to the workflow',
          child: AnInput(multiline: true, block: true),
        ),
        span: true,
      ),
      GallerySpecimen(
        'label + 类型徽章 (labelTrailing)',
        (context) => AnFormField(
          label: 'city',
          labelTrailing: Text(
            'string',
      ...
      GallerySpecimen(
        ...
        (_) => const AnFormField(
          label:
              'an-extremely-long-field-label-that-must-ellipsis-not-overflow-the-column',
          child: AnInput(block: true),
        ),
        stress: true,
        maxWidth: 240,
        span: true,
  ...
  GalleryItem('AnRefPill', '实体提及药丸:类型图标 + 文案;id 非空可点(派 {kind,id})、空=纯标注', [
    ...
    GallerySpecimen(
      'document (纯提及)',
      (_) => const AnRefPill(kind: 'document', label: 'spec.md'),
    ),
    GallerySpecimen(
      'approval (可点)',
      (_) => AnRefPill(
    ...
    GallerySpecimen(
      '纯标注 (无 id 不可点)',
      (_) => const AnRefPill(kind: 'handler', label: 'on-webhook'),
    ),
    GallerySpecimen(
      '未知 kind (兜底 ?)',
      (_) => const AnRefPill(kind: 'quasar', label: 'unknown-kind'),
      stress: true,
    ),
    GallerySpecimen(
      '超长截断',
      (_) => const AnRefPill(
        kind: 'workflow',
        label:
            'an-extremely-long-entity-reference-name-that-must-ellipsis-within-its-cap',
      ),
      stress: true,
      maxWidth: 180,
    ),
    GallerySpecimen(
      '注入转义',
      (_) => const AnRefPill(kind: 'agent', label: '<b>not</b> & <i>html</i>'),
      stress: true,
    ),
    GallerySpecimen(
      'inline 行内脸 (嵌文本基线)',
      (context) => Text.rich(
        TextSpan(
          children: [
            ...
            const WidgetSpan(
              alignment: PlaceholderAlignment.middle,
              child: AnRefPill.inline(
                kind: 'function',
                label: 'sync_inventory',
              ),
            ),
            TextSpan(
              text: ' 再对账。',
  ...
  GalleryItem('AnSection', '小节:caption/plain 标题 + 无边内容 + 右侧 actions', [
    GallerySpecimen(
      ...
      (_) => const AnSection(
        label: 'Inputs',
        children: [
          AnInput(placeholder: 'name'),
          AnInput(placeholder: 'value'),
        ],
      ),
      span: true,
    ),
    GallerySpecimen(
      ...
      (_) => AnSection(
        ...
        children: const [AnChip('NODE_ENV=prod'), AnChip('REGION=us')],
      ),
      span: true,
    ),
    GallerySpecimen(
      ...
      (_) => const AnSection(
        ...
        children: [
          Text('A document-tier section leans on whitespace, not rule lines.'),
        ],
      ),
      span: true,
    ),
    GallerySpecimen(
      ...
      (_) => const AnSection(
        ...
        children: [
          Text(
            'A quiet lowercase faint-meta heading over a tight body — run terminal output / trace / nodes sections.',
          ),
        ],
      ),
      span: true,
    ...
    GallerySpecimen(
      ...
      (_) => AnSection(
        actions: [
          AnButton(label: 'New', size: AnButtonSize.sm, onPressed: () {}),
        ],
        children: const [Text('Head with no label still renders its actions.')],
      ),
      span: true,
    ),
    GallerySpecimen(
      ...
      (_) => const AnSection(
        label:
            'a very long section caption that must ellipsis instead of wrapping the head',
        children: [Text('body')],
      ),
      stress: true,
      maxWidth: 220,
    ...
    GallerySpecimen(
      '注入转义',
      (_) => const AnSection(
        label: '<b>not</b> & <i>html</i>',
        children: [Text('escaped label')],
      ),
      stress: true,
      span: true,
    ),
  ...
  GalleryItem('AnAutoGrid', '响应式块网格:auto-fit 等宽列(1fr 填满)、窄塌 1 列、每行按内容高', [
    GallerySpecimen(
      ...
      (_) => const AnAutoGrid(
        children: [
          _GridCell('input', height: 72),
          _GridCell('output', height: 48),
          _GridCell('env', height: 96),
          _GridCell('schedule', height: 56),
          _GridCell('triggers', height: 64),
          _GridCell('mounts', height: 40),
        ],
      ),
      span: true,
    ),
    GallerySpecimen(
      '少块拉伸 (2 块·1fr)',
      (_) =>
          const AnAutoGrid(children: [_GridCell('left'), _GridCell('right')]),
      span: true,
    ),
    GallerySpecimen(
      '单卡 (1 块拉满)',
      (_) => const AnAutoGrid(children: [_GridCell('only')]),
      span: true,
    ),
    GallerySpecimen(
      '海量 (12 块·多行流)',
      (_) => AnAutoGrid(
        children: [
          for (var i = 0; i < 12; i++)
            _GridCell('blk $i', height: (40 + (i % 4) * 16).toDouble()),
        ],
      ),
      stress: true,
      span: true,
    ...
    GallerySpecimen(
      '窄塌 1 列',
      (_) => const AnAutoGrid(children: [_GridCell('a'), _GridCell('b')]),
      stress: true,
      maxWidth: 200,
      span: true,
    ),
  ...
  GalleryItem('AnKv', '紧凑定义列表:key 左·value 右;可编辑行就地编辑(铅笔→框/下拉)', [
    ...
    GallerySpecimen(
      ...
      (_) => const AnKv(
        mono: true,
        rows: [AnKvRow('Run', 'run_3a9f0e88'), AnKvRow('Hash', 'a1b2c3d4e5f6')],
      ),
      span: true,
    ),
    GallerySpecimen(
      ...
      (_) => const AnKv(
        rows: [
          AnKvRow(
            'Description',
            'A deliberately long value that should wrap onto several lines instead of truncating.',
            wrap: true,
          ),
        ],
      ),
    ...
    GallerySpecimen(
      ...
      (_) => const AnKv(
        dense: true,
        rows: [AnKvRow('Status', 'running'), AnKvRow('Elapsed', '2.4s')],
      ),
      span: true,
    ),
    GallerySpecimen(
      ...
      (_) => const AnKv(
        rows: [
          AnKvRow(
            'an-extremely-long-key-name-that-must-ellipsis',
            'and-an-equally-long-value-that-also-truncates-on-the-right',
          ),
        ],
      ),
      stress: true,
  ...
  GalleryItem('AnEditableValue', '双锚就地编辑核(被 Kv/Field 消费;此处直展编辑态)', [
    GallerySpecimen(
      'editing (input)',
      (_) => const _EditableDemo(value: '0.85', startEditing: true),
      span: true,
    ),
    GallerySpecimen(
      'select (常驻下拉)',
      (_) => const _EditableDemo(
        value: 'medium',
        editor: AnEditKind.select,
        options: _effortOptions,
      ),
  ...
  GalleryItem('AnField', '键值大行:label(+hint)左 + 值右;可编辑 / 只读 / 控件槽 三态', [
    GallerySpecimen(
      '可编辑值',
      (_) => const _FieldDemo(label: 'Name', value: 'normalize-input'),
      span: true,
    ),
    GallerySpecimen(
      'label + hint',
      (_) => const _FieldDemo(
        label: 'Timeout',
        hint: 'seconds before the run is aborted',
        value: '30',
      ),
      span: true,
    ),
    ...
    GallerySpecimen(
      '只读',
      (_) => const AnField(label: 'Kind', value: 'function'),
      span: true,
    ),
    GallerySpecimen(
      'dense (chrome 档 13)',
      (_) => const AnField(label: 'Kind', value: 'function', dense: true),
      span: true,
    ),
    GallerySpecimen(
      '控件槽 (value=null)',
      (_) => const AnField(
        label: 'Visibility',
        child: _DropdownDemo(initial: 'med', simple: true),
      ),
      span: true,
    ),
    GallerySpecimen(
      '超长 label/value',
      (_) => const _FieldDemo(
        label: 'an-extremely-long-field-label-that-must-ellipsis',
        value: 'and-a-very-long-value-that-truncates-on-the-right',
      ),
      stress: true,
      maxWidth: 280,
      span: true,
  ...
  GalleryItem('AnCard', '有边卡片容器:normal / accent / 可选 / 紧凑', [
    GallerySpecimen(
      ...
      (_) => const AnCard(
        child: Text(
          'A bordered card collects settings / MCP config / onboarding choices.',
        ),
      ),
      span: true,
    ),
    GallerySpecimen(
      'accent 变体',
      (_) => const AnCard(
        variant: AnCardVariant.accent,
        child: Text('Editing — accent border draws focus.'),
      ),
      span: true,
    ),
    GallerySpecimen('可选 (点选)', (_) => const _CardSelectDemo(), span: true),
    GallerySpecimen(
      'selected (静态·2px accent 边)',
      (_) => AnCard(
        selectable: true,
        selected: true,
        onSelect: () {},
    ...
    GallerySpecimen(
      ...
      (_) => AnCard(
        child: Row(
          children: [
            Icon(AnIcons.mcp, size: AnSize.icon),
            const SizedBox(width: AnSpace.s8),
            const Expanded(
              child: Text('Horizontal card — compose a Row as the child'),
            ),
            AnButton(label: 'Edit', size: AnButtonSize.sm, onPressed: () {}),
          ],
        ),
      ),
      span: true,
    ...
    GallerySpecimen(
      'pad=tight',
      (_) => const AnCard(pad: AnCardPad.tight, child: Text('Tight inset')),
      span: true,
    ),
  ]),
  GalleryItem('AnInfoCard', '无边信息单元:head(icon+title+meta)+ body + actions', [
    GallerySpecimen(
      ...
      (_) => AnInfoCard(
        ...
        meta: 'UTC',
        child: const AnKv(
          rows: [AnKvRow('Cron', '0 0 * * *'), AnKvRow('Next run', 'in 3h')],
        ),
      ),
      span: true,
    ),
    GallerySpecimen(
      ...
      (_) => AnInfoCard(
        ...
        child: const AnKv(
          rows: [
            AnKvRow('NODE_ENV', 'production'),
            AnKvRow('REGION', 'us-east'),
          ],
        ),
      ),
      span: true,
    ...
    GallerySpecimen(
      ...
      (_) => const AnInfoCard(
        child: Text(
          'A headless info unit — just body content, organised by whitespace.',
        ),
      ),
      span: true,
    ),
    GallerySpecimen(
      '超长 title + meta',
      (_) => const AnInfoCard(
        title: 'an-extremely-long-info-card-title-that-must-ellipsis',
        meta: 'and-a-long-meta',
        child: Text('body'),
      ),
      stress: true,
      maxWidth: 260,
  ...
  GalleryItem(
    'AnRow',
    '核心行:lead(dot/icon↔chevron)+ label + trail(meta↔hover actions)',
    [
      GallerySpecimen(
        'icon + meta',
        (_) => AnRow(
      ...
      GallerySpecimen(
        ...
        (_) => AnRow(
          ...
          label: 'on-webhook',
          hint: 'Fires when an external HTTP request hits the mounted path.',
          onSelect: () {},
        ),
        span: true,
      ),
      ...
      GallerySpecimen(
        ...
        (_) => AnRow(
          ...
          label:
              'an-extremely-long-row-label-that-must-ellipsis-not-overflow-the-row',
          meta: 'now',
          onSelect: () {},
        ),
        stress: true,
  ...
  GalleryItem('AnRowDetail', '可展开详情行:点行展开下方详情(AnExpandReveal 高度揭示)', [
    GallerySpecimen(
      'expandable (点开/收起)',
      (_) => const _RowDetailDemo(),
      span: true,
    ),
  ]),
  GalleryItem(
    'AnDisclosure',
    '披露组:常驻旋转 chevron 头(icon/label/可选尾随)+ AnExpandReveal body(流式轨迹/日志)',
    [
      GallerySpecimen(
        'reasoning (点开/收起)',
        (_) => _DisclosureDemo(label: 'reasoning', icon: AnIcons.reasoning),
        span: true,
      ),
      GallerySpecimen(
        'tool_call + danger 尾随',
  ...
  GalleryItem('AnThinTable', '对齐多列(非表格、无 chrome):首列吃富余 + 其余贴内容、表头灰 meta', [
    GallerySpecimen(
      '对齐多列 (只读)',
      (_) => const AnThinTable(columns: _tableCols, rows: _tableRows),
      span: true,
    ),
    GallerySpecimen(
      '可点选行',
    ...
    GallerySpecimen(
      ...
      (_) => AnThinTable(
        ...
        rows: [
          for (var i = 0; i < 14; i++)
            {'name': 'job-$i', 'kind': 'function', 'runs': '$i'},
        ],
      ),
      stress: true,
      span: true,
    ...
    GallerySpecimen(
      ...
      (_) => const AnThinTable(
        ...
        rows: [
          {
            'name': 'an-extremely-long-entity-name-that-must-ellipsis',
            'kind': 'workflow',
            'runs': '999999',
          },
        ],
  ...
  GalleryItem(
    'AnProseTable',
    '有框正文表(与文档编辑器表逐像素 1:1):发丝网格 + 强调表头 + 12h/6v 紧内距 + 富单元格(调用方渲)',
    [
      GallerySpecimen(
        'bordered grid (reading cells)',
        (_) => const _ProseTableDemo(),
        span: true,
      ),
    ],
  ),
...
// ── G4 — Navigation & shell ──
final GalleryCategory
_g4NavShell = GalleryCategory('导航与壳 Nav & Shell', AnIcons.grip, [
  GalleryItem(
    'AnFloatingBar 浮动工具条',
    'surface 药丸 + 发丝边 + float 阴影;浮在繁忙内容(图画布)上;段间放 AnDivider.vertical',
    [
      GallerySpecimen(
        '缩放簇 + 分隔 + 动作',
        (_) => AnFloatingBar(
  ...
  GalleryItem(
    'AnInspector',
    '右岛内容壳:head(icon+title)+ 滚动块流 body(不画岛皮,AnIsland 供)· headless 占满自管',
    [
      GallerySpecimen(
        'head + body (in island)',
        (_) => SizedBox(
            ...
            child: AnInspector(
              ...
              children: const [
                ...
                AnInfoCard(
                  title: 'Source',
                  child: AnKv(
                    rows: [AnKvRow('Lines', '42'), AnKvRow('Lang', 'CEL')],
                  ),
                ),
              ],
            ),
      ...
      GallerySpecimen(
        'headless (slot fills + self-manages)',
        (_) => const SizedBox(
          width: 320,
          child: AnIsland(
            child: AnInspector(
              ...
              child: AnState(
                ...
                size: AnStateSize.inset,
                title: 'Headless',
                hint: 'slot self-draws its head + scroll',
              ),
            ),
          ),
        ),
      ...
      GallerySpecimen(
        ...
        (_) => SizedBox(
          ...
          child: AnIsland(
            child: AnInspector(
              title:
                  'an-extremely-long-inspector-title-that-must-ellipsis-in-the-head',
              icon: AnIcons.workflow,
              children: [
  ...
  GalleryItem(
    'AnInspectorHead',
    '右岛统一头带:小字 label(meta/inkFaint) + 动作槽 + ✕ 收岛 + 可选 meta 次行;无分隔线',
    [
      GallerySpecimen(
        'label + subhead + close',
        (_) => SizedBox(
        ...
        (_) => SizedBox(
          ...
          child: AnIsland(
            child: AnInspectorHead(
              ...
              label:
                  'an-extremely-long-node-identifier-that-must-ellipsis-and-hold',
              subLeading: '智能体',
        ...
        (_) => SizedBox(
          ...
          child: AnIsland(
            child: AnPanelHead(
              icon: AnIcons.activity,
              title:
                  'an-extremely-long-panel-title-that-must-ellipsis-and-hold-the-two-buttons',
          ...
          children: [
            for (final t in const [
              ...
              AnInfoCard(
                title: t,
                child: const AnKv(
                  rows: [AnKvRow('key', 'value'), AnKvRow('another', 'thing')],
                ),
              ),
          ],
        ),
  ...
  GalleryItem(
    'AnMenu',
    '浮层菜单(on AnPopover):分组小标题 + icon/check/meta + danger/disabled(多选 keepOpen)',
    [
      GallerySpecimen(
        'actions menu (⋯) — tap to open',
        (_) => const _MenuActionsDemo(),
      ),
      GallerySpecimen(
        'sliders (multi-check, keepOpen)',
        (_) => const _MenuSlidersDemo(),
      ),
      GallerySpecimen(
        'match anchor width (workspace 切换器) — tap to open',
        (_) => const _MenuMatchWidthDemo(),
        span: true,
      ),
    ],
  ),
  GalleryItem(
    'AnTabs',
    '文字下划线切换器:灰→选中黑 + 弹簧下划线;IndexedStack panes 隐藏不销毁;多 tab 横滚',
    [
      GallerySpecimen(
        'tabs (underline + panes)',
        (_) => const _TabsDemo(),
        height: 220,
        span: true,
      ),
  ...
  GalleryItem(
    'AnSidebarList',
    '左岛侧栏:New + 域内过滤(sliders 菜单)+ groups→types→rows 递归树(文档树可折叠、可拖拽重排:插线=重排/中段=嵌入)',
    [
      GallerySpecimen(
        'sidebar (filter + tree + select + drag)',
        (_) => const _SidebarDemo(),
        height: 420,
        span: true,
      ),
      GallerySpecimen(
        'row 改名中 (就地编辑态)',
        (_) => const _SidebarDemo(editingId: 'fn1'),
        height: 420,
        span: true,
      ),
    ],
  ...
  GalleryItem('AnOceanHeader', '海洋页头:面包屑 + H2 标题(可就地改名)+ 右动作 + meta', [
    GallerySpecimen(
      'editable (crumb + H2 + actions + meta)',
      (_) => const _OceanHeaderDemo(),
      span: true,
    ),
    GallerySpecimen(
      'read-only',
  ...
  GalleryItem(
    'AnDocHeader',
    '阅读尺度页头(AnOceanHeader 的阅读列对位件):灰面包屑 + 可改名 readingH1 大标题 + 可编 reading 描述 + 可编标签行;供 document/skill,skill 走 showTags:false 无标签。空字段引导律:空标题=灰「未命名」占位、空简介=灰「添加简介…」、空标签=灰 dummy 药丸「添加标签」(穿目标形态、可点入编辑)',
    [
      GallerySpecimen(
        'document (crumb + 大标题 + 描述 + tags)',
        (_) => const _DocHeaderDemo(),
        span: true,
      ),
      GallerySpecimen(
        '空字段引导 (灰占位 / 添加简介… / dummy 药丸)',
        (_) => const _DocHeaderDemo(empty: true),
        span: true,
      ),
      GallerySpecimen(
        'skill (无 tags,标题不可改名)',
  ...
  GalleryItem(
    'AnLazyIndexedStack',
    '懒 IndexedStack:槽首显才建、建后常驻(保 State/滚动位/展开态);未显槽=零成本盒。中心海洋切换用它(切走再回瞬时且不丢滚动位)',
    [
      GallerySpecimen(
        'interactive (切槽:看懒挂 + 计数保活)',
        (_) => const _LazyStackDemo(),
        height: 220,
        span: true,
      ),
    ],
  ...
  GalleryItem(
    'AnOceanSwitcher',
    '左岛海洋切换器:选中展开标签 + 匹配几何滑动(旧的收起、新的展开,单药丸滑动+变宽,整行回流);降级=瞬切',
    [
      GallerySpecimen(
        'interactive (点不同海洋,看滑动)',
        (_) => const _OceanSwitcherDemo(),
        span: true,
      ),
      GallerySpecimen(
        'rest · Chat',
        (_) => AnOceanSwitcherFrame(
          items: _oceanItems,
          fromIndex: 0,
          toIndex: 0,
          t: 1,
  ...
  GalleryItem(
    'AnBrandIcon.mark',
    '裸品牌 mark(6 方块·无白底无边框·inkMuted 灰):供全屏/Win-Linux 窗控品牌区行内,和 nav 描边图标**同列同视觉大小**。此格并排真 nav 图标(16px 盒描边示意),比对 mark 方块可见大小与描边字形一致(_markContentRatio 收)',
    [
      GallerySpecimen(
        'mark vs 真 nav 图标(16 盒对比)',
        (_) => const _BrandMarkDemo(),
        span: true,
      ),
    ],
  ),
  GalleryItem(
    'AnBrandIcon.brand 品牌资产',
    '随包品牌 SVG(assets/brand,lobe-icons MIT + simple-icons CC0,LICENSES.md 载明):ink 着色灰底盘,brandIconOr 经注册表解析 slug,**缺者一律首字母圆徽兜底**(每卡必有图标位)。消费方=模型与密钥 provider 区 + MCP 双列卡',
    [
      GallerySpecimen(
        'LLM 厂牌 (lobe-icons)',
        (_) => Wrap(
          ...
          children: [
            for (final s in const [
              ...
              AnTooltip(
                message: s,
                child: brandIconOr(s, fallbackLabel: s),
              ),
          ],
        ),
        span: true,
  ...
  GalleryItem(
    '品牌 lockup',
    'AnWindowControls 全屏/Win-Linux 露出的「Anselm」锁定组合:灰裸 mark + Newsreader(OFL 衬线)wordmark,纯净无点缀。真机对齐(mark 落 nav 图标列)+ 竖向节奏见侧栏',
    [
      GallerySpecimen(
        '灰 mark + Newsreader wordmark (ink)',
        (_) => const _BrandLockupDemo(),
        span: true,
      ),
    ],
  ),
  GalleryItem(
    'AnWorkspaceButton',
    '左岛底栏 workspace 触发钮:头像 + 名字(省略)+ chevron;点开快捷操作菜单(isOpen 高亮 + chevron 翻转)',
    [
      GallerySpecimen(
        'default',
        (_) => SizedBox(
          width: 200,
          child: AnWorkspaceButton(name: 'Personal', onTap: () {}),
        ),
        span: true,
      ),
      GallerySpecimen(
      ...
      GallerySpecimen(
        ...
        (_) => SizedBox(
          width: 180,
          child: AnWorkspaceButton(
            name: 'A very long workspace name that must ellipsize',
            onTap: () {},
          ),
        ),
        stress: true,
  ...
  GalleryItem(
    'AnSidebarFooter',
    '左岛底栏:workspace 菜单 | 设置格 | 通知格(两格独立高亮 + 未读红点)',
    [
      GallerySpecimen('idle', (_) => _footerDemo(), span: true),
      GallerySpecimen(
        'settings active (齿轮高亮)',
        (_) => _footerDemo(settingsActive: true),
        span: true,
      ),
      GallerySpecimen(
        'notifications active + 红点',
        (_) => _footerDemo(notificationsActive: true, unread: 3),
        span: true,
      ),
    ],
  ),
...
_g5CodeData = GalleryCategory('代码与数据 Code & Data', AnIcons.function, [
  GalleryItem(
    'AnCodeBlock 只读代码块',
    'AnCodeSurface + 标准内距 + mono 文本;给不需要编辑器 chrome 的输出/数据块',
    [
      GallerySpecimen(
        '输出块',
        (_) => const AnCodeBlock('run finished\nexit code 0'),
        span: true,
      ),
      GallerySpecimen(
        'json 数据',
      ...
      GallerySpecimen(
        'bare (无框)',
        (_) => const AnCodeBlock('a bare mono line, no frame', bare: true),
        span: true,
      ),
      GallerySpecimen(
        '超长 (软换行)',
  ...
  GalleryItem(
    'AnCodeChip',
    '行内代码芯片:mono + padded 圆角 sunken 胶囊;chat 与文档编辑器静置态的唯一真相',
    [
      ...
      GallerySpecimen(
        '长标识',
        (_) => const AnCodeChip('a_very_long_identifier_name'),
        span: true,
      ),
  ...
  GalleryItem('AnCodeEditor', '唯一代码块/轻编辑:高亮 + 行号 + 顶栏;只读/可编辑/内联/换行/seamless 嵌入', [
    GallerySpecimen(
      'python 只读 (机器窗档 12)',
      (_) => const AnCodeEditor(code: _pyCode, lang: 'py'),
      span: true,
    ),
    GallerySpecimen(
      'reading (内容档 13 — markdown/概览代码)',
      (_) => const AnCodeEditor(code: _pyCode, lang: 'py', reading: true),
      span: true,
    ),
    GallerySpecimen(
      'CEL (插值上色)',
      (_) => const AnCodeEditor(code: _celCode, lang: 'cel'),
      span: true,
    ),
    GallerySpecimen(
      'json 只读',
      (_) => const AnCodeEditor(code: _jsonCode, lang: 'json'),
      span: true,
    ),
    ...
    GallerySpecimen(
      'compact',
      (_) => const AnCodeEditor(code: _celCode, lang: 'cel', compact: true),
      span: true,
    ),
    GallerySpecimen(
      'inline (无框)',
      (_) => const AnCodeEditor(
        code: 'input.x > 0 && has(node.y)',
        lang: 'cel',
        inline: true,
      ),
      span: true,
    ...
    GallerySpecimen(
      ...
      (_) => const AnCodeEditor(
        code:
            'x = "a really long single line that exceeds the editor width and must scroll horizontally without wrapping or overflow"',
        lang: 'py',
      ),
      stress: true,
      span: true,
    ...
    GallerySpecimen(
      '空',
      (_) => const AnCodeEditor(code: '', lang: 'py'),
      stress: true,
      span: true,
    ),
    GallerySpecimen(
      '海量行 (内容高、父滚动)',
      (_) => AnCodeEditor(
        code: [for (var i = 0; i < 60; i++) 'line_$i = step($i)'].join('\n'),
        lang: 'py',
      ),
      stress: true,
      span: true,
  ...
  GalleryItem('AnFadeCollapse 渐隐收合', '超长内容收合 + 底部渐隐 + 展开/收起;是否可收由调用方按内容尺寸判定', [
    GallerySpecimen(
      ...
      (_) => AnFadeCollapse(
        ...
        collapseLabel: '收起',
        child: AnCodeEditor(
          code: [for (var i = 0; i < 40; i++) 'line_$i = step($i)'].join('\n'),
          lang: 'py',
        ),
      ),
      span: true,
    ...
    GallerySpecimen(
      'collapsible=false (裸渲)',
      (_) => const AnFadeCollapse(
        collapsible: false,
        expandLabel: '展开全部',
        collapseLabel: '收起',
  ...
  GalleryItem('AnJsonTree', '唯一 JSON/结构化展示:可折叠树 + 类型着色(TreeSliver 虚拟化);只读', [
    GallerySpecimen(
      'flowrun 节点结果',
      (_) => const AnJsonTree(data: _flowrunNode),
      span: true,
      height: 280,
    ),
    GallerySpecimen(
      '数组根',
      (_) =>
          const AnJsonTree(data: ['alpha', 'beta', 'gamma'], rootLabel: 'tags'),
      span: true,
      height: 160,
    ),
    GallerySpecimen(
      '深嵌套 (open-depth)',
      (_) => const AnJsonTree(data: _deepJson, openDepth: 5),
      span: true,
      height: 220,
    ),
    GallerySpecimen(
      '无根行 (showRoot=false)',
      (_) => const AnJsonTree(data: _flowrunNode, showRoot: false),
      span: true,
      height: 240,
    ),
    GallerySpecimen(
    ...
    GallerySpecimen(
      '无效 JSON',
      (_) => const AnJsonTree(jsonString: '{ bad json,, }'),
      span: true,
      height: 80,
    ),
    GallerySpecimen(
    ...
    GallerySpecimen(
      '海量键 (截断)',
      (_) =>
          AnJsonTree(data: {for (var i = 0; i < 80; i++) 'key_$i': 'value_$i'}),
      stress: true,
      span: true,
      height: 260,
    ),
    GallerySpecimen(
      '注入转义',
      (_) => const AnJsonTree(
        data: {'html': '<b>not</b> & <i>x</i>', 'tmpl': '\${raw} {{cel}}'},
      ),
      stress: true,
      span: true,
      height: 140,
  ...
  GalleryItem(
    'AnVersionDiff',
    '版本 diff:单框 unified(增绿删红)+ 行内高亮(唯一 tokenizer);只读',
    [
      GallerySpecimen(
        '范围 + 说明 + 计数',
        (_) => const AnVersionDiff(
          before: _diffBefore,
          after: _diffAfter,
          lang: 'py',
          range: 'v3 → v4',
      ...
      GallerySpecimen(
        ...
        (_) => const AnVersionDiff(
          before: 'x = short',
          after:
              'x = "a really long replacement line that exceeds the diff width and must scroll horizontally"',
          lang: 'py',
        ),
        stress: true,
        span: true,
...
// ── G6 — Top-band messages + dialog ──
final GalleryCategory _g6Overlays = GalleryCategory(
  '浮层 Overlays',
  AnIcons.more,
  [
    GalleryItem(
      'AnApprovalCapsule',
      '审批顶带岛:与普通通知共用 36px 冠部和关闭铁线,正文只向下生长;等待/在途/失败/批准/驳回全状态',
      [
        GallerySpecimen(
          '等待决策',
          (_) => AnApprovalCapsule(
            title: 'approve_deploy',
            question: 'Deploy **v2.4.0** to production? 42 files changed.',
            pendingLabel: '等待审批',
            busyLabel: '正在裁决',
            approveLabel: '通过',
            rejectLabel: '驳回',
    ...
    GalleryItem(
      'AnNoticeCapsule',
      '统一顶带消息胶囊:事件通知 + 操作反馈唯一即时面——tone 点+一句话+查看尾+常驻 X;自驱登-停-退,hover 暂停',
      [
        GallerySpecimen(
          'danger event',
          (_) => AnNoticeCapsule(
    ...
    GalleryItem(
      'AnNoticeQueueTail',
      '候场尾:最多两颗 tone 点;更多折为 32px 数字槽,hover/focus 原位变居中的 28×28 方形 X 并清空顶带',
      [
        GallerySpecimen(
          '1 pending',
          (_) => AnNoticeQueueTail(
            cues: const [
              NoticeCue(id: 'a', tone: AnTone.ok, kind: NoticeKind.pill),
            ],
            overflowCount: 0,
            clearLabel: '清空顶部通知,共 2 条',
            onClear: () {},
          ),
          span: true,
        ),
    ...
    GalleryItem(
      'AnDialog (confirm)',
      '模态确认框:遮罩 + 居中卡 + 焦点陷阱 + Escape;命令式 confirm()',
      [
        GallerySpecimen(
          '危险确认 (删除)',
          (_) => const _DialogTriggerDemo(tone: AnDialogTone.danger),
          span: true,
        ),
        GallerySpecimen(
          '主操作确认 (无说明)',
          (_) => const _DialogTriggerDemo(
            tone: AnDialogTone.primary,
            withMessage: false,
          ),
          span: true,
        ),
...
// AnDialog command-trigger demo — confirm() resolves true/false; the result enters the top band.
// dialog 命令式触发演示——confirm() 返 true/false,结果进入顶带。
        ...
        return AnButton(
          ...
          onPressed: () async {
            ...
            final ok = await ctrl.confirm(
              ...
              message: withMessage
                  ? (danger ? '此操作不可撤销,确定删除该实体吗?' : '将把改动写入当前版本。')
                  : null,
            ...
            ref
                ...
                .show(ok ? '已执行' : '已取消', tone: ok ? AnTone.ok : AnTone.none);
          },
        );
      },
    );
...
class _CodeEditDemoState extends State<_CodeEditDemo> {
  ...
  @override
  Widget build(BuildContext context) => AnCodeEditor(
    code: _code,
    lang: 'cel',
    editable: true,
...
class _MenuSlidersDemoState extends State<_MenuSlidersDemo> {
  ...
  void _toggle(String k) =>
      setState(() => _on.contains(k) ? _on.remove(k) : _on.add(k));
  @override
  Widget build(BuildContext context) => AnMenu(
    ...
    entries: [
      ...
      AnMenuItem(
        ...
        checked: _on.contains('recent'),
        keepOpen: true,
        onTap: () => _toggle('recent'),
      ),
      AnMenuItem(
        label: 'Name',
        checked: _on.contains('name'),
        keepOpen: true,
        onTap: () => _toggle('name'),
      ),
      ...
      AnMenuItem(
        label: 'Show versions',
        checked: _on.contains('versions'),
        keepOpen: true,
        onTap: () => _toggle('versions'),
      ),
      AnMenuItem(
        label: 'Show status dots',
        checked: _on.contains('dots'),
        keepOpen: true,
        onTap: () => _toggle('dots'),
      ),
    ],
...
final List<AnOceanItem> _longOceanItems = [
  ...
  AnOceanItem(
    id: 'long',
    icon: AnIcons.workflow,
    label: 'A very long ocean name that must clip',
  ),
...
class _ProseTableDemo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final c = context.colors;
    final head = AnText.reading
        .weight(AnText.emphasisWeight)
        .copyWith(color: c.ink);
    final body = AnText.reading.copyWith(color: c.ink);
    Widget cell(String s, TextStyle style, TextAlign align) =>
        Text(s, style: style, textAlign: align);
    List<Widget> row(String a, String b, String d, TextStyle s) => [
      cell(a, s, TextAlign.left),
      cell(b, s, TextAlign.center),
      cell(d, s, TextAlign.right),
    ];
    return AnProseTable(
      rows: [
...
class _DisclosureDemoState extends State<_DisclosureDemo> {
  ...
  Widget build(BuildContext context) {
    ...
    return AnDisclosure(
      ...
      trailing: widget.danger
          ? const AnChip('dangerous', tone: AnTone.danger)
          : null,
      open: _open,
      onToggle: () => setState(() => _open = !_open),
      child: Text(
        'node.exec("deploy", env)\n  → streaming output…',
...
class _CardSelectDemoState extends State<_CardSelectDemo> {
  ...
  Widget build(BuildContext context) => AnCard(
    ...
    child: Text(
      _sel ? 'Selected — tap to deselect' : 'Selectable — tap to select',
    ),
  );
}
...
class _TabsDemoState extends State<_TabsDemo> {
  ...
  @override
  Widget build(BuildContext context) =>
      AnTabs(items: _items, value: _v, onSelect: (k) => setState(() => _v = k));
}
...
class _DemoNode {
  _DemoNode(this.id, this.label, [List<_DemoNode>? children])
    : children = children ?? [];
  final String id;
  final String label;
...
class _SidebarDemoState extends State<_SidebarDemo> {
  ...
  void _opt(String k) =>
      setState(() => _opts.contains(k) ? _opts.remove(k) : _opts.add(k));
  ...
  void _drop(String dragged, String target, AnRowDropZone zone) => setState(() {
    ...
    if (node == null) return;
    if (!_place(_docs, node, target, zone)) {
      _docs.add(node); // detached fallback — re-append 兜底回挂
    }
  });
  ...
  SidebarRow _toRow(_DemoNode n) => SidebarRow(
    ...
    label: n.label,
    icon: AnIcons.doc,
    children: [for (final c in n.children) _toRow(c)],
  );
  ...
  Widget build(BuildContext context) {
    ...
    return AnSidebarList(
      ...
      menuEntries: [
        ...
        AnMenuItem(
          label: 'Recently updated',
          checked: _opts.contains('updated'),
          keepOpen: true,
          onTap: () => _opt('updated'),
        ),
        AnMenuItem(
          label: 'Name',
          checked: _opts.contains('name'),
          keepOpen: true,
          onTap: () => _opt('name'),
        ),
        const AnMenuSection('Display'),
        AnMenuItem(
          label: 'Show versions',
          checked: _opts.contains('versions'),
          keepOpen: true,
          onTap: () => _opt('versions'),
        ),
        AnMenuItem(
          label: 'Show status dots',
          checked: _opts.contains('status'),
          keepOpen: true,
          onTap: () => _opt('status'),
        ),
      ],
...
class _DocHeaderDemoState extends State<_DocHeaderDemo> {
  ...
  late List<String> _tags = widget.empty
      ? const []
      : const ['design', 'backend'];
  @override
