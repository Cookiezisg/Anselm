...
BlockNode _call(
  ...
  if (progress != null) {
    node.children.add(
      BlockNode(id: 'pr_$name', kind: BlockKind.progress)
...
final chatToolCardGalleryItem = GalleryItem(
  ...
  [
    GallerySpecimen(
      'args 流入中(动词流光,无展开体)',
      (c) => ChatToolCard(node: _streamingArgs()),
      span: true,
    ),
    GallerySpecimen(
      '执行中(已关未果,读秒在 3s 后浮现)',
    ...
    GallerySpecimen(
      'ask_user · 已回答·选项(选中章 + 余淡出)',
      (c) => ChatToolCard(
        node: _call(
          'ask_user',
          args:
              '{"message":"这几张发票币种不一致,按哪种本位币归一?","options":["人民币 CNY","美元 USD","欧元 EUR"]}',
          result: '美元 USD',
        ),
      ),
      span: true,
    ...
    GallerySpecimen(
      'decide_approval · 已批准(判词章 + 理由 + 后果条)',
      (c) => ChatToolCard(
        node: _call(
          'decide_approval',
          args:
              '{"flowrunId":"flr_7a1b2c3d4e5f6a7b","nodeId":"approve_spend","decision":"yes","reason":"金额在季度预算内,且申请人是授权审批人,批准放行。"}',
          result:
    ...
    GallerySpecimen(
      'decide_approval · 已否决(红章 + 理由 + failed 后果)',
      (c) => ChatToolCard(
        node: _call(
          'decide_approval',
          args:
              '{"flowrunId":"flr_7a1b2c3d4e5f6a7b","nodeId":"approve_spend","decision":"no","reason":"超出本季度预算上限,需先走额度调整流程。"}',
          result:
              '{"flowrun":{"status":"failed"},"nodes":[{"status":"completed"},{"status":"failed"}]}',
        ),
      ),
      span: true,
    ),
    GallerySpecimen(
      'decide_approval · NOT_PARKED(产品正常态,友好呈现非红崩)',
      (c) => ChatToolCard(
        node: _call(
          'decide_approval',
          args:
              '{"flowrunId":"flr_7a1b2c3d4e5f6a7b","nodeId":"approve_spend","decision":"yes"}',
    ...
    GallerySpecimen(
      'list_approval_inbox · 待审薄表(ref/摘要/等待/run,最久在最上)',
      (c) => ChatToolCard(
        node: _call(
          'list_approval_inbox',
          result:
    ...
    GallerySpecimen(
      '成功 · JSON 结果(点行展开:意图/参数/结果树)',
      (c) => ChatToolCard(
        node: _call(
          'create_function',
          args:
              '{"ops":[{"op":"set_meta","name":"quarterly_rollup","description":"按季度聚合发票"},{"op":"set_code","code":"def rollup(): ..."}]}',
    ...
    GallerySpecimen(
      ...
      (c) => ChatToolCard(
        node: _call(
          ...
          resultError:
              'WORKFLOW_GRAPH_CYCLE: node "sync" reaches itself via "retry" — break the loop or mark it as an intended cycle',
        ),
      ),
      span: true,
    ),
    ...
    GallerySpecimen(
      'MCP 动态 · 离谱名与深 JSON(schema-less 兜底)',
      (c) => ChatToolCard(
        node: _call(
          'mcp__context7__resolve_library_docs____v2',
