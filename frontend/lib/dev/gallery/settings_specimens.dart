...
class _LiveState<T> extends State<_Live<T>> {
  ...
  Widget build(BuildContext context) =>
      widget.builder(value, (v) => setState(() => value = v));
}

final GalleryCategory settingsCategory = GalleryCategory(
  '设置 Settings',
  AnIcons.gear,
  [
    GalleryItem('AnSwitch', '布尔开关——kit 唯一正统(30×18 药丸,accent 填充,reduced 直跳)', [
      GallerySpecimen(
        ...
        (_) => _Live<bool>(
          initial: true,
          builder: (v, set) =>
              AnSwitch(value: v, onChanged: set, semanticLabel: 'demo'),
        ),
      ),
      GallerySpecimen(
        'off',
      ...
      GallerySpecimen(
        'disabled-on',
        (_) => const AnSwitch(value: true, semanticLabel: 'locked'),
      ),
      GallerySpecimen(
        'disabled-off',
        (_) => const AnSwitch(value: false, semanticLabel: 'locked'),
      ),
    ]),
      ...
      GallerySpecimen(
        ...
        (_) => AnSegmented<int>(
          options: const [
            AnSegmentedOption(
              value: 0,
              label: 'a-very-long-segment-label-that-must-ellipsize',
            ),
    ...
    GalleryItem('AnSecretField', '密钥输入——默认掩码+可见切换;粘贴 trim;三条可兑现承诺', [
      GallerySpecimen(
        'empty',
        (_) => const AnSecretField(
          placeholder: 'sk-…',
          revealLabel: 'reveal',
          concealLabel: 'conceal',
        ),
        span: true,
      ),
      GallerySpecimen(
    ...
    GalleryItem('AnMeter', '用量条——accent→warn(0.85)→danger(0.97);null=空轨', [
      GallerySpecimen(
        '正常',
        (_) => const AnMeter(ratio: 0.42, label: '2 100 / 5 000 · 8/1 重置'),
        span: true,
      ),
      GallerySpecimen(
        'warn',
        (_) => const AnMeter(ratio: 0.9, label: '4 500 / 5 000'),
        span: true,
      ),
      GallerySpecimen(
        'danger',
        (_) => const AnMeter(ratio: 0.99, label: '4 950 / 5 000'),
        span: true,
      ),
      GallerySpecimen(
        '未知',
        (_) => const AnMeter(ratio: null, label: '——'),
        span: true,
      ),
      GallerySpecimen(
        '零',
        (_) => const AnMeter(ratio: 0),
        stress: true,
        maxWidth: 160,
      ),
    ]),
