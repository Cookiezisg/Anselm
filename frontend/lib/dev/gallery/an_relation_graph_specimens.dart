...
final _edges = [
  ...
  _e('e14', 'workflow', 'wf_batch', 'handler', 'hd_ftp'), // the second, disconnected component
];
...
const _paletteKinds = [
  'workflow', 'agent', 'function', 'handler', 'trigger',
  'control', 'approval', 'mcp', 'skill', 'memory', 'document',
];
      ...
      for (final k in _paletteKinds)
        Row(mainAxisSize: MainAxisSize.min, children: [
          Container(
            width: 14,
            height: 14,
            decoration: BoxDecoration(color: RelationGraphConfig.fogColor(k), shape: BoxShape.circle),
          ),
...
final anRelationGraphGalleryItem = GalleryItem(
  ...
  [
    GallerySpecimen('雾彩色板一览(kind 管色相 · 低饱和同明度 · 独立于提示色)', _fogPalette, span: true),
    GallerySpecimen('观赏态 framed(默认焦点=fn_core,柔光+涟漪衰减)', (_) => AnRelationGraph(
          nodes: _nodes, edges: _edges, framed: true, focusId: 'fn_core',
          nodeSemanticLabel: _nodeLabel, edgeSemanticLabel: _edgeLabel,
          onExpand: () {}, expandLabel: '展开',
        ), span: true),
    GallerySpecimen('换焦(焦点=ag_writer,涟漪整体移位)', (_) => AnRelationGraph(
          nodes: _nodes, edges: _edges, framed: true, focusId: 'ag_writer',
          nodeSemanticLabel: _nodeLabel, edgeSemanticLabel: _edgeLabel,
        ), span: true),
    GallerySpecimen('选中枢纽(accent 环 + 焦点)', (_) => AnRelationGraph(
          nodes: _nodes, edges: _edges, framed: true, selectedId: 'fn_core', focusId: 'fn_core',
    ...
    GallerySpecimen('探索态(非 framed 满幅 + 缩放条)', (_) => AnRelationGraph(
          nodes: _nodes, edges: _edges, toolbar: true, selectedId: 'ag_writer', focusId: 'ag_writer',
          onNodeTap: (_) {}, nodeSemanticLabel: _nodeLabel, edgeSemanticLabel: _edgeLabel,
    ...
    GallerySpecimen('孤立实体(不进力场·底部安静带) + 单边叶子', (_) => AnRelationGraph(
          nodes: [
            _n('function', 'fn_a', 'lonely-fn'), // zero-degree isolate → quiet band
