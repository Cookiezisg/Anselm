...
const _esc = '\x1B';
final _term =
    '$_esc[32m✓$_esc[0m compiled 42 modules\n'
...
final _longCode = [
  for (var i = 1; i <= 40; i++)
    i == 40
        ? 'return merged  # newest line 最新行'
        : 'step_$i = transform(step_${i - 1})',
].join('\n');
const _prose =
    '归因先行:issue_date 未做时区归一,跨年边界上 Q4 与次年 Q1 混桶。'
...
final tongguiCodexCategory = GalleryCategory('同轨法典', AnIcons.entities, [
    ...
    [
      GallerySpecimen(
        ...
        (c) => AnWindow(
          header: Text(
            '\$ npm test',
            style: AnText.code.copyWith(color: c.colors.ink),
          ),
          actions: [AnChip('copy', icon: AnIcons.copy, copyValue: 'npm test')],
          child: Text(
            _term.replaceAll(RegExp('\\x1B\\[[0-9;]*m'), ''),
      ...
      GallerySpecimen(
        '钳高+折叠(超高 FadeCollapse)',
        (c) => AnWindow(
          maxHeight: AnSize.proseClamp,
          collapsible: true,
          child: Text(
            List.filled(18, _prose).join('\n\n'),
            style: AnText.reading.copyWith(color: c.colors.inkMuted),
          ),
        ),
        span: true,
  ...
  GalleryItem(
    ...
    [
      GallerySpecimen(
        'AnCodeEditor 落定脸(高亮+行号)',
        (c) => AnCodeEditor(code: _code, lang: 'python', reading: true),
        span: true,
      ),
      GallerySpecimen(
        'AnCodeEditor live 脸(全量+高亮+行号,超视口贴底跟最新行)',
        (c) => AnCodeEditor(code: _longCode, lang: 'python', live: true),
        span: true,
      ),
      GallerySpecimen(
        'AnVersionDiff live 两幕(与落定同行结构,仅行序不同)',
        (c) => const AnVersionDiff(
          live: true,
          lang: 'python',
        ...
        (c) => Wrap(
          ...
          children: [
            AnChip(
              'fr_demo_8f3a92',
              mono: true,
              copyValue: 'fr_demo_8f3a92c41b77e5d2',
              icon: AnIcons.copy,
            ),
            AnChip(
              truncate('exec_0123456789abcdef0123456789abcdef', AnTrunc.id),
              mono: true,
            ),
  ...
  GalleryItem(
    ...
    [
      ...
      GallerySpecimen(
        'AnFieldSection(标签在上,吃 IO 段/意图行)',
        (c) => AnFieldSection(
          label: '输出',
          child: AnWindow(
            child: Text(
      ...
      GallerySpecimen(
        'AnLedgerRow ×3(状态点居左,右缘一条线)',
        (c) => Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            AnLedgerRow(
              lead: const AnStatusDot(AnStatus.done),
              primary: 'exec_01H8…f2',
              chips: const [AnChip('42ms', tone: AnTone.none)],
              meta: '2 分钟前',
            ),
            AnLedgerRow(
              lead: const AnStatusDot(AnStatus.err),
              primary: 'exec_01H8…e9',
              chips: const [AnChip('SyncError', tone: AnTone.danger)],
              meta: '5 分钟前',
            ),
            AnLedgerRow(
              lead: const AnStatusDot(AnStatus.wait),
              primary: 'node.approval_gate',
              meta: '等待审批',
            ),
          ],
        ),
      ...
      GallerySpecimen(
        'AnLedgerRow 批6 三槽:sub 副行 / danger 副行 / measure 计量',
        (c) => Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            const AnLedgerRow(
            ...
            const AnLedgerRow(
              ...
              primary: 'charge',
              sub: 'HANDLER_RPC_TIMEOUT: charge() exceeded 30s',
              subTone: AnTone.danger,
              meta: '5 分钟前',
            ),
            AnLedgerRow(
              primary: 'search_tools',
              mono: true,
              disclose: true,
              chips: const [AnChip('2 args', tone: AnTone.none)],
              onTap: () {},
              expanded: true,
              expandChild: Text(
                '披露体两侧等退假想框 s8;disclose 行由原语渲左岛箭头 morph(悬停换、展开旋 90°)',
      ...
      GallerySpecimen(
        'AnLadder 判别梯骨架(序号圆+降线+内容槽)',
        (c) => AnLadder(
          children: [
            Text(
              'amount > 10000 → approval',
              style: AnText.code.copyWith(color: c.colors.inkMuted),
            ),
            Text(
              'amount > 1000 → review',
    ...
    [
      GallerySpecimen(
        ...
        (c) => AnStatBar(
          ...
          stats: const [
            AnStat('v4', tabular: true),
            AnStat('env ready', tone: AnTone.ok),
            AnStat('1.2s', tabular: true),
          ],
          chips: [
            AnChip(
              'sync_inventory',
      ...
      GallerySpecimen(
        ...
        (c) => const AnStatBar(
          status: AnStatus.err,
          stats: [
            AnStat('3 步', tabular: true),
            AnStat('↑1840 ↓620', tabular: true),
          ],
  ...
  GalleryItem(
    ...
    [
      GallerySpecimen(
        'term(ANSI 色+折叠)',
        (c) => AnLiveTail(_term, tailLines: 4),
        span: true,
      ),
      GallerySpecimen(
        'mono(便笺逐笔)',
      ...
      GallerySpecimen(
        'prose(蒸馏流,贴底钳高,溢出顶渐隐)',
        (c) => AnLiveTail(
          List.filled(6, _prose).join(' '),
          style: AnLiveTailStyle.prose,
        ),
        span: true,
      ),
      GallerySpecimen(
        'prose bare(thinking 的内联无框脸)',
        (c) => AnLiveTail(
          List.filled(6, _prose).join(' '),
          style: AnLiveTailStyle.prose,
          bare: true,
