...
const double _composerW = 560; // narrowed toward the 720 reading column feel for the gallery cell 逼近阅读列观感

final GalleryItem chatComposerGalleryItem = GalleryItem(
  'AnComposer 发送框',
  '演变输入:单行药丸 ↔ 多行卡片 · @/📎 · send↔stop · 聚焦 accent 光环 · landing 浮起',
  [
    GallerySpecimen('空态 · 单行药丸 (send 藏)', (_) => const _ComposerSpecimen(), span: true, maxWidth: _composerW),
    GallerySpecimen('聚焦看光环 (点进去↑)', (_) => const _ComposerSpecimen(hint: '点这里 → 看 accent 光环'),
        span: true, maxWidth: _composerW),
    GallerySpecimen('有字 · 单行 (send ↑)', (_) => const _ComposerSpecimen(seed: '帮我看下这个 workflow 为什么失败', send: true),
        span: true, maxWidth: _composerW),
    GallerySpecimen('多行 · 卡片 reflow',
        (_) => const _ComposerSpecimen(seed: '第一行……\n第二行,继续写更多内容,看它换行后钮组落到下面一排、圆角从药丸渐变到卡片\n第三行', send: true),
        span: true, maxWidth: _composerW),
    GallerySpecimen('生成中 · stop', (_) => const _ComposerSpecimen(generating: true), span: true, maxWidth: _composerW),
    GallerySpecimen('带附件条', (_) => const _ComposerSpecimen(seed: '看下这两个文件', send: true, attachments: true),
        span: true, maxWidth: _composerW),
    GallerySpecimen('landing · 浮起药丸', (_) => const _ComposerSpecimen(floating: true, hint: '下午好 · 今天想做点什么?'),
        span: true, maxWidth: _composerW),
    GallerySpecimen('超长 URL 不撑破',
        (_) => const _ComposerSpecimen(seed: 'https://example.com/a/really/really/long/url/that/must/wrap/inside/the/box/not/blow/it/out?x=1&y=2', send: true),
        stress: true, span: true, maxWidth: _composerW),
  ],
);
...
class _ComposerSpecimenState extends State<_ComposerSpecimen> {
  late final TextEditingController _ctrl = TextEditingController(text: widget.seed);
