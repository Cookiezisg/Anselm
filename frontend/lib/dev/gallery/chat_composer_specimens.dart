...
// (generating/recording). The recording specimen also mounts the real strip area with duration +
// wave. No specimen auto-focuses — a focused TextField blinks its caret forever, which would hang
// the matrix test's pumpAndSettle.
//
// 聊天发送框各态。dev 明文串豁免 i18n。可交互:点入见 accent 聚焦光环、打字看药丸↔卡片演变。send/stop 由宿主定
// (原语只渲 trailing),故每态给对的那个:空=无 / 可语音=mic / 有字=send↑ / 生成中或录音中=stop。录音 specimen
// 也挂真实 strip 区域展示时长+波形。无 specimen 自动聚焦(聚焦的输入框光标永远闪 → 挂死 matrix 的 pumpAndSettle)。
