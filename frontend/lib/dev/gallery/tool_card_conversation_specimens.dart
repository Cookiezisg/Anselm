...
final toolCardConversationGalleryItem = GalleryItem(
  ...
  [
    GallerySpecimen(
      'manage · rename(已重命名对话 + 标题 chip + 状态回显)',
      (c) => ChatToolCard(
        node: _call(
          'manage_conversation',
          '{"action":"rename","title":"发票对账排查"}',
          '{"conversationId":"cv_1","action":"rename","title":"发票对账排查","archived":false,"pinned":false}',
        ),
      ),
      span: true,
    ),
    GallerySpecimen(
      'manage · archive(状态回显 + 归档产品事实)',
      (c) => ChatToolCard(
        node: _call(
          'manage_conversation',
          '{"action":"archive"}',
          '{"conversationId":"cv_1","action":"archive","title":"旧线程","archived":true,"pinned":false}',
        ),
      ),
      span: true,
    ),
    GallerySpecimen(
      'manage · 软失败(ctx 无对话 → 中性已调用 + 原文)',
      (c) => ChatToolCard(
        node: _call(
          'manage_conversation',
          '{"action":"pin"}',
    ...
    GallerySpecimen(
      'list_conversations · 迷你 rail(置顶字形 / 归档徽 / 时间 · 续页注记)',
      (c) => ChatToolCard(
        node: _call(
          'list_conversations',
    ...
    GallerySpecimen(
      'search_conversations · 命中门(snippet 次行 + ×N chunks + 截断注记)',
      (c) => ChatToolCard(
        node: _call(
