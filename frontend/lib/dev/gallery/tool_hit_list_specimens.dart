...
final toolHitListGalleryItem = GalleryItem(
  ...
  [
    ...
    GallerySpecimen(
      '本地超封顶 · 逃生口(前 3 · 共 4,点→全量 JSON)',
      (c) => ToolHitList(
        rows: _fnHits,
        cap: 3,
        rawJson:
            '{"count":4,"functions":[{"id":"fn_1a2b","name":"fetch_with_retry"},'
    ...
    GallerySpecimen(
      '无面板 kind · 行惰性(block/memory 不放死链)',
      (c) => ToolHitList(
        rows: [
          ToolHitRow(
            glyph: AnIcons.entityKindGlyph('block'),
            title: 'block: 定价规则草案',
            kind: 'block',
            id: 'bk_1',
          ),
          ToolHitRow(
            glyph: AnIcons.entityKindGlyph('memory'),
            title: 'memory: 用户偏好',
            kind: 'memory',
            id: 'mem_1',
          ),
