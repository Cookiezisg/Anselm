...
MatrixCellState? _cell(String runId, String nodeId) {
  ...
  if (!(reached[runId]?.contains(nodeId) ?? false)) return null; // 未及 — sparse, no cell 稀疏无格
  if (nodeId == 'analyze' && (runId == 'fr_8' || runId == 'fr_6')) {
    ...
    return const MatrixCellState(status: 'failed', iterations: 3, label: 'analyze · 失败 · ×3 轮');
  }
...
final anRunMatrixGalleryItem = GalleryItem(
  ...
  [
    ...
    GallerySpecimen(
        ...
        (_) => Padding(
              ...
              child: AnRunMatrix(
                ...
                cols: _cols(),
                cellStatus: _cell,
                selection: const MatrixSelection(flowrunId: 'fr_8', nodeId: 'analyze'),
                onCell: (_, _) {},
                onCol: (_) {},
                onRow: (_) {},
                notReachedLabel: '未及',
        ...
        (_) => SingleChildScrollView(
              child: Padding(
                ...
                child: AnRunMatrix(
                  rows: [
                    for (var i = 0; i < 24; i++)
                      MatrixRowHead(nodeId: 'node_$i', kind: i.isEven ? 'action' : 'agent'),
                  ],
                  cols: [
                    for (var i = 20; i > 0; i--)
                      RunColumn(
                  ...
                  cellStatus: (runId, nodeId) =>
                      MatrixCellState(status: runId.endsWith('5') ? 'failed' : 'completed'),
                  onCell: (_, _) {},
                  onCol: (_) {},
                  onRow: (_) {},
                  notReachedLabel: '未及',
                  ...
                  // 穿过这张床要按 527 次 Tab。
                  cellSemanticLabel: (col, row, cell) => '${row.nodeId} ${col.id} ${cell?.status}',
    ...
    GallerySpecimen(
        ...
        (_) => Padding(
              ...
              child: AnRunMatrix(
                rows: const [
                  MatrixRowHead(
                      nodeId: 'a_very_long_node_identifier_that_must_ellipsize', kind: 'action'),
    ...
    GallerySpecimen(
        ...
        (_) => Padding(
              padding: const EdgeInsets.all(AnSpace.s16),
              child: AnRunMatrix(rows: const [], cols: const [], cellStatus: (_, _) => null),
            ),
        stress: true),
  ],
);
