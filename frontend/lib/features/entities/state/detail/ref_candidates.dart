...
final refTargetsProvider = FutureProvider.autoDispose
    .family<List<RefCandidate>, RefFamily>((ref, family) async {
      ...
      if (kind != null) {
        ...
        return [
          for (final r in page.items)
            (id: r.id, name: r.name.isEmpty ? r.id : r.name, meta: null),
        ];
      }
...
final refMembersProvider = FutureProvider.autoDispose
    .family<List<RefCandidate>, ({RefFamily family, String target})>((
      ref,
      key,
    ) async {
      final repo = ref.watch(entityRepositoryProvider);
...
final controlProvider = FutureProvider.autoDispose.family<ControlLogic, String>(
  (ref, controlId) => ref.watch(entityRepositoryProvider).getControl(controlId),
);
...
final controlPortsProvider = FutureProvider.autoDispose
    .family<List<String>, String>((ref, controlId) async {
      final control = await ref.watch(controlProvider(controlId).future);
      return [
        for (final b in control.activeVersion?.branches ?? const <Branch>[])
          b.port,
      ];
    });
