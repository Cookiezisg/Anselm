...
class WorkflowOverview extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final run = ref.watch(
      runTerminalProvider(EntityRef(EntityKind.workflow, wf.id)),
    );
    final overlay =
        (g == null || run.flowrunId == null || run.flowNodes.isEmpty)
        ? null
        : deriveRunState(
            g,
            rows: run.flowNodes,
            runStatus: run.flowrunStatus.isEmpty && run.isRunning
                ? 'running'
                : run.flowrunStatus,
          );
        ...
        AnSection(
          ...
          children: [
            AnRow(
              ...
              label: wf.needsAttention
                  ? (wf.attentionReason ?? d.val.needsAttention)
                  : d.val.noAlerts,
              passive: true,
            ),
          ],
