...
class TriggerOverview extends StatelessWidget {
  ...
  Widget _config(BuildContext context) {
    ...
    String? intervalStr() => cfgMap['intervalSec'] == null ? null : '${cfgMap['intervalSec']}s';
    ...
    String? eventList() {
      ...
      return e is List && e.isNotEmpty ? e.map((x) => x.toString()).join(', ') : null;
    }
    final (String code, String lang, List<(String, String?)> rows) = switch (trigger.kind) {
      // cron config is just the expression — the backend reads nothing else (config.go:56). cron 只读 expression。
      TriggerSource.cron => (cfg('expression'), 'cron', const <(String, String?)>[]),
      // The mounted path an external caller POSTs to (copy it to configure the caller). 外部调用方 POST 的挂载路径。
      TriggerSource.webhook => ('/api/v1/webhooks/${trigger.id}/${cfg('path')}', 'url', [
          (d.trigger.signatureAlgo, opt('signatureAlgo')),
          (d.trigger.signatureHeader, opt('signatureHeader')),
        ]),
      TriggerSource.fsnotify => (cfg('path'), 'path', [
          (d.trigger.events, eventList()),
          (d.trigger.pattern, opt('pattern')),
        ]),
      TriggerSource.sensor => (cfg('condition'), 'cel', [
