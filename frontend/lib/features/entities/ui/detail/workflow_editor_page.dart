...
class WorkflowEditorPage extends ConsumerWidget {
  ...
  Widget _leftChrome(
    ...
    return Positioned(
      ...
      left: AnSpace.s8,
      height: AnSize
          .titlebar, // centered controls sit on the traffic-lights' horizontal line 控件居中落灯线
      child: Row(
    ...
    final VoidCallback? onSave = (!dirty || saving)
        ...
        : () async {
            ...
            if (ok) {
              ref
                  .read(noticeCenterProvider.notifier)
                  .show(e.saved, tone: AnTone.ok);
            } else if ((st2?.saveError ?? '').isNotEmpty) {
              ref
                  .read(noticeCenterProvider.notifier)
                  .show(st2!.saveError!, tone: AnTone.danger);
            }
          };
    return Positioned(
      top: 0,
...
class _CollapsibleInspector extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final content = AnInspector(
      headless: true,
      child: WorkflowEditorInspector(entityRef: entityRef),
    );
