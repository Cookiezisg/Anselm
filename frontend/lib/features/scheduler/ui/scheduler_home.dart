...
class _SchedulerHomeViewState extends ConsumerState<SchedulerHomeView> {
  ...
  Widget build(BuildContext context) {
        ...
        return Center(
          child: AnState(
            ...
            action: AnButton(
              label: t.retry,
              onPressed: () =>
                  ref.read(schedulerRailProvider.notifier).refresh(),
            ),
          ),
        );
      }
    ...
    final row = data.workflows
        .where((w) => w.id == widget.workflowId)
        .firstOrNull;
    if (row == null) {
    ...
    final myTriggers = [
      for (final e in data.edges)
        if (e.fromId == row.id && triggersById[e.toId] != null)
          triggersById[e.toId]!,
    ];
...
class _HealthHeadState extends ConsumerState<_HealthHead> {
  ...
  Future<void> _runNow() async {
    ...
    try {
      ...
      await ref
          .read(schedulerRunTableProvider(widget.row.id).notifier)
          .refetchTop();
  ...
  Widget build(BuildContext context) {
    ...
    final rangeStats = stamped != null && stamped.range == range
        ? stamped.stats
        : null;
    ...
    return Column(
      ...
      children: [
        ...
        AnOceanHeader(
          ...
          crumbs: [
            AnCrumb(t.home.crumbRoot, onTap: () => context.go('/scheduler')),
          ],
          title: widget.row.name,
          meta: [
            AnChip(
              lifecycleWord,
              tone: widget.row.lifecycleState == 'inactive'
                  ? AnTone.none
                  : AnTone.accent,
            ...
            AnTimeRangePicker(
              value: range,
              onChanged: (r) =>
                  ref.read(schedulerTimeRangeProvider.notifier).set(r),
            ...
            Text(
              t.home.statsLine(
                rate: successRate != null
                    ...
                    : '—',
                avg: avgMs != null
                    ? fmtDuration(Duration(milliseconds: avgMs))
                    : '—',
              ),
...
class _RunTableZoneState extends ConsumerState<_RunTableZone>
    with BatchZone<_RunTableZone> {
  DateTime? _lastTapAt;
  String? _lastTapId;
  ...
  String _flagshipPath(String frId) =>
      '/scheduler/w/${widget.workflowId}/runs/$frId';
  ...
  Future<void> _replayOne(Flowrun run) async {
    ...
    try {
      ...
      if (mounted) {
        ref
            .read(noticeCenterProvider.notifier)
            .show(t.replayed, tone: AnTone.ok);
      }
    } on ApiException catch (e) {
      if (!mounted) return;
      ref
  ...
  Future<void> _batchReplay(List<Flowrun> rows) async {
    ...
    if (!ok || !mounted) return;
    final (done, lost, err) = await runBatch<Flowrun>(
      targets,
      (r) => r.id,
      (r) => repo.replayRun(r.id),
    );
    if (!mounted) return;
    summaryNotice(
  ...
  Future<void> _batchCancel(List<Flowrun> rows) async {
    ...
    if (!ok || !mounted) return;
    final (done, ended, err) = await runBatch<Flowrun>(
      targets,
      (r) => r.id,
      (r) => repo.cancelRun(r.id),
    );
    if (!mounted) return;
    summaryNotice(
  ...
  Widget build(BuildContext context) {
            ...
            Align(
              ...
              child: AnButton(
                ...
                onPressed: () => ref.invalidate(
                  schedulerRunTableProvider(widget.workflowId),
                ),
              ),
            ),
          ],
    ...
    final selectable =
        s.filter == RunStatusFilter.failed ||
        s.filter == RunStatusFilter.running;
    ...
    final failedLabel = s.failedCountCapped
        ? '${SchedulerRunTableController.probeCap}+'
        : '${s.failedCount}';
    final runningLabel = s.runningCountCapped
        ? '${SchedulerRunTableController.probeCap}+'
        : '${s.runningCount}';
      ...
      children: [
                ...
                SizedBox(
                  ...
                  child: AnSegmented<RunStatusFilter>(
                    ...
                    options: [
                      ...
                      AnSegmentedOption(
                        value: RunStatusFilter.failed,
                        label: t.filterFailed(n: failedLabel),
                      ),
                      AnSegmentedOption(
        ...
        if (s.rows.isEmpty)
          ...
          for (final run in s.rows)
            AnExpandReveal(
              open: !leaving.contains(run.id),
              child: _row(context, s, run),
            ),
  ...
  Widget _row(BuildContext context, RunTableState s, Flowrun run) {
    ...
    final showCheck =
        selectable && (selected.isNotEmpty || hovered) && !isPending;
    ...
    return AnHoverRegion(
      ...
      child: AnLedgerRow(
        ...
        expandBuilder: (_) =>
            RunPeekCard(workflowId: widget.workflowId, flowrunId: run.id),
        ...
        lead: isPending
            ...
            ? AnBatchCheck(
                ...
                semanticLabel: context.t.scheduler.overview.selectRow(
                  name: run.id,
                ),
...
AnTimeRangePickerStrings _rangeStrings(BuildContext context) {
  ...
  return AnTimeRangePickerStrings(
    ...
    monthTitle: (m) =>
        t.monthTitle(y: '${m.year}', m: t.months.split(',')[m.month - 1]),
...
class _MatrixZone extends ConsumerWidget {
          ...
          ? Row(
              children: [
                ...
                AnButton(
                  ...
                  size: AnButtonSize.sm,
                  onPressed: () =>
                      ref.invalidate(schedulerMatrixWindowProvider(workflowId)),
                ),
              ],
            )
  ...
  Widget _grid(BuildContext context, WidgetRef ref, MatrixWindowState s) {
    ...
    final byKey = {
      for (final cell in m.cells) '${cell.flowrunId}/${cell.nodeId}': cell,
    };
    ...
    return AnRunMatrix(
      ...
      rows: [
        for (final r in m.rows) MatrixRowHead(nodeId: r.nodeId, kind: r.kind),
      ],
      cols: [
          ...
          RunColumn(
            ...
            label:
                '${t.matrixColA11y(
                  ...
                  d: col.elapsedMs == null ? t.matrixRunning : fmtDuration(Duration(milliseconds: col.elapsedMs!)),
                )}\n${col.flowrunId}',
          ),
      ],
      cellStatus: (frId, nodeId) {
        ...
        if (cell == null) {
          return null; // 未及 — sparse, and that IS the answer 稀疏即答案
        }
        return MatrixCellState(
          ...
          label: t.matrixCellA11y(
            node: nodeId,
            status: flowrunStatusWord(context.t, cell.status),
            n: '${cell.iterations}',
          ),
        );
      },
      ...
      onCell: (frId, nodeId) => context.go(
        Uri(path: flagship(frId), queryParameters: {'node': nodeId}).toString(),
      ),
      onNearOldestEdge: s.hasMore
          ? () => ref
                .read(schedulerMatrixWindowProvider(workflowId).notifier)
                .loadOlder()
          : null,
      ...
      coordinateLabel: (r, rc, c2, cc) => t.matrixCoordA11y(
        r: '${r + 1}',
        rows: '$rc',
        c: '${c2 + 1}',
        cols: '$cc',
          ...
          AnAutoGrid(
            children: [
              for (final trigger in triggers)
                _TriggerCard(trigger: trigger, now: now),
            ],
          ),
      ],
    );
...
class _TriggerCardState extends ConsumerState<_TriggerCard> {
  ...
  Future<void> _resume() =>
      _flip((repo) => repo.resumeTrigger(widget.trigger.id));

  Future<void> _flip(
    Future<TriggerEntity> Function(SchedulerRepository) op,
  ) async {
    final notices = ref.read(noticeCenterProvider.notifier);
    setState(() => _busy = true);
    try {
        ...
        Text(
          [
            if (nextFire != null && nextFire.isAfter(widget.now))
              t.home.nextFire(
                d: fmtWaited(nextFire.difference(widget.now)),
                at: fmtDateTime(nextFire),
              ),
            if (trigger.lastFiredAt != null)
              t.home.lastFired(
                d: fmtWaited(widget.now.difference(trigger.lastFiredAt!)),
              )
            else
              t.home.neverFired,
          ].join(' · '),
      ...
      child: Row(
        ...
        children: [
          Expanded(
            ...
            // 恢复). 暂停灰显只罩信息簇,控件保持全活。
            child: paused
                ? Opacity(opacity: AnOpacity.disabled, child: info)
                : info,
          ),
