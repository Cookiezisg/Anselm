...
Widget? _glance(BuildContext context, WidgetRef ref) {
  ...
  final segs = <String>[
    ...
    if ((stats?.consecutiveFailures ?? 0) > 0)
      t.glanceStreak(n: stats!.consecutiveFailures),
  ];
      ...
      children: [
        AnKv(
          rows: [
            ...
            AnKvRow(
              t.replayHistory(n: '${run.replayCount}'),
        ...
        if (run.pinnedRefs.isNotEmpty) ...[
          ...
          AnSection(
            label: t.pinnedRefsHead,
            variant: AnSectionVariant.plain,
            children: [
              AnKv(
                dense: true,
                rows: [
                  for (final e in run.pinnedRefs.entries)
                    AnKvRow(e.key, e.value, mono: true),
                ],
              ),
            ],
          ),
        ...
        if (error != null && !d.orphan) ...[
          if (!_hasErrorText && run.pinnedRefs.isEmpty)
            const SizedBox(height: AnGap.section),
          AnButton(
            label: t.triage,
            icon: AnIcons.chat,
...
class _NodeInspectorFaceState extends ConsumerState<_NodeInspectorFace> {
  ...
  Future<void> _replay() async {
    ...
    await _act(
      (repo) => repo.replayRun(widget.data.run.id),
      lost: t.notReplayable,
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (rows.isEmpty) {
      ...
      return _Face(
        ...
        children: [
          AnStatBar(
            leading: [
              AnChip(widget.nodeId, mono: true, look: AnChipLook.outlined),
            ],
          ),
          const SizedBox(height: AnGap.block),
          Text(
            inferredRunningNodes(
                  d.graph ?? const Graph(),
                  d.merged,
    ...
    final node = rows.firstWhere(
      (n) => n.iteration == widget.iteration,
      orElse: () => rows.last,
    );
    final activity = [
    ...
    return _Face(
      ...
      children: [
        AnStatBar(
          ...
          chips: [
            if (node.ref.isNotEmpty)
              AnChip(
                truncate(node.ref, AnTrunc.id),
                mono: true,
            ...
            children: [
              AnKv(
                ...
                rows: [
                  ...
                  AnKvRow(
                    context.t.scheduler.run.kvStatus,
                    runStatusWord(context.t, activity.status),
                  ),
                ],
              ),
            ],
        ...
        if (parked) ...[
          if (node.result.isEmpty &&
              !(activity != null && activity.execId.isNotEmpty))
            const SizedBox(height: AnGap.section),
          ApprovalGate(
            parked: node,
            busy: _busy,
