...
/// One trigger seed — the entity's static identity plus the `nextFireAt` it WOULD project when
/// unpaused. Kept apart from [TriggerEntity] because a paused trigger reads `nextFireAt` absent on the
/// wire, yet the repo must still know the tick to reveal it when the demo resumes the trigger.
/// 单条 trigger 种子:静态身份 + 未暂停时会投影的 nextFireAt。不用 TriggerEntity 本体,因为暂停态线缆上
/// nextFireAt 缺席,而 demo 恢复它时仓储仍需知道那个刻度。
class SchedulerTriggerSeed {
  const SchedulerTriggerSeed({
    required this.id,
    required this.name,
    required this.kind,
...
/// The PURE-DATA seed a [FixtureSchedulerRepository] plays back — every list the repository used to
/// hard-code, lifted into one value so a second dataset (the marketing `story`) can be injected while
/// the stateful grammar (decide / cancel / replay / runNow / kill / pause / resume, paging, filters,
/// derived stats and matrix) stays in ONE implementation. [SchedulerSeed.demo] is the original battery,
/// verbatim, so the default constructor's behaviour is unchanged.
/// 纯数据种子:仓储原本硬编码的每张表抬成一个值,好让第二套数据集(story)注入,而有状态文法(决断/取消/重放/
/// 立即运行/终止/暂停/恢复、分页、过滤、派生统计与矩阵)只留一份实现。demo() 逐字保留原电池,默认行为不变。
class SchedulerSeed {
  const SchedulerSeed({
    required this.workflows,
    required this.stats,
    required this.completedSince,
    required this.runs,
    required this.inbox,
    required this.versions,
    required this.nodesFor,
    required this.triggers,
    required this.schedule,
    required this.firings,
    required this.edges,
    required this.workflowCreatedAt,
    required this.triageConversationId,
    this.matrixRows = const {},
  });

  /// The rail rows, in wire order. rail 行,线缆序。
  final List<SchedulerWorkflowRow> workflows;

  /// Static per-workflow stats; `running` and `parkedNodes` are OVERLAID by the repo from the live run
  /// and inbox state, so a seed never has to keep them in step. 静态逐 workflow 统计;running/parkedNodes
  /// 由仓储按活状态覆写,种子无需自己对齐。
  final List<WorkflowRunStats> stats;

  /// `totals.completedSince` — a bare number on the wire, kept bare here. 线缆裸数,照裸。
  final int completedSince;

  /// Seed run rows BEFORE the stateful status overlay (newest first, plus any generated history).
  /// 有状态覆写前的种子 run 行(新→旧)。
  final List<Flowrun> runs;

  /// Parked approvals with their workflow join. 带 workflow join 的 parked 审批。
  final List<SchedulerInboxRow> inbox;

  /// Every pinned version the dataset knows; a workflow's active version is the first one that names
  /// it. 数据集知道的全部钉版;workflow 的活跃版本=首个指向它的版本。
  final List<WorkflowVersion> versions;

  /// Per-run node rows (the linked pane's gantt/graph, the matrix cells, the activity aggregation).
  /// Receives the seed inbox so a parked run can surface its own gate row.
  /// 逐 run 节点行(联动格、矩阵格、活动聚合);传入 inbox 好让 parked run 露出自己的闸行。
  final List<FlowrunNode> Function(Flowrun run, List<SchedulerInboxRow> inbox)
  nodesFor;

  final List<SchedulerTriggerSeed> triggers;
  final List<SchedulePoint> schedule;
  final List<Firing> firings;
  final List<EntityRelation> edges;

  /// Every workflow entity's createdAt (one instant is enough for a demo). workflow 实体的 createdAt。
  final DateTime workflowCreatedAt;

  /// The conversation `:triage` hands back. :triage 返回的对话 id。
  final String triageConversationId;

  /// Optional per-workflow matrix row order (workflowId → rows). The wire law mints rows from the
  /// node rows' first appearance, so a branch no run has ever reached has no row at all; a story
  /// that wants the WHOLE pinned graph on the row axis (never-run branches read as «not reached»)
  /// declares it here. Absent → the first-appearance law, unchanged; first-seen nodes missing
  /// from the declared list are appended so a cell can never lose its row.
  /// 可选的逐 workflow 矩阵行序。线缆律按节点行首次出现铸行,从未跑到的分支根本没有行;想让整张钉版图
  /// 上行轴(未跑分支读作「未及」)的故事在此声明。缺省=首次出现律不变;声明外的首见节点追加,格不失行。
  final Map<String, List<MatrixRow>> matrixRows;

  /// The original WRK-069 §15 battery — unchanged data, now expressed as a seed. 原电池,数据不变。
  factory SchedulerSeed.demo() => SchedulerSeed(
    workflows: _demoWorkflows(),
    stats: _demoStats(),
    completedSince: 23,
    runs: _demoRuns(),
    inbox: _demoInbox(),
    versions: [_demoVersion()],
    nodesFor: _demoNodesFor,
    triggers: _demoTriggers(),
    schedule: _demoSchedule(),
    firings: _demoFirings(),
    edges: _demoEdges,
    workflowCreatedAt: _shift(_anchor.subtract(const Duration(days: 30))),
    triageConversationId: 'cv_demo000000000001',
  );
}

class FixtureSchedulerRepository implements SchedulerRepository {
  /// Defaults to the original demo battery so every existing caller is untouched. 默认原电池。
  FixtureSchedulerRepository({SchedulerSeed? seed})
    : _seed = seed ?? SchedulerSeed.demo();

  final SchedulerSeed _seed;
  ...
  Future<List<SchedulerWorkflowRow>> listWorkflows() async => [
    for (final w in _seed.workflows)
  ...
  Future<SchedulerStats> stats(
    ...
    final all = <WorkflowRunStats>[
      // running / parkedNodes are overlaid from the live run + inbox state so the demo stays honest
      // after cancel / decide. running/parkedNodes 按活状态覆写,动作之后 demo 不撒谎。
      for (final row in _seed.stats)
        row.copyWith(
          running: _runningCount(row.workflowId),
          parkedNodes: _parkedCountForWorkflow(row.workflowId),
        ),
    ];
    ...
    return SchedulerStats(
      totals: SchedulerTotals(
        running: running,
        completedSince: _seed.completedSince,
  ...
  int _parkedCountForWorkflow(String workflowId) => _seed.inbox
      .where(
        (r) =>
  ...
  List<Flowrun> _allRuns() {
    final runs = <Flowrun>[..._manualRuns, ..._seed.runs];
    return [
      for (final r in runs)
        Flowrun(
          id: r.id,
  ...
  WorkflowEntity _workflowEntity(String id) {
    final row = _seed.workflows.where((w) => w.id == id).firstOrNull;
    if (row == null) {
      throw const ApiException(
    ...
    final created = _seed.workflowCreatedAt;
  ...
  Future<FlowrunComposite> getRunFull(String flowrunId) async {
    ...
    return FlowrunComposite(
      flowrun: run,
      nodes: _seed.nodesFor(run, _seed.inbox),
    );
  }
  ...
  Future<List<FlowrunActivityRow>> listActivity(String flowrunId) async {
    ...
    for (final n in _seed.nodesFor(run, _seed.inbox)) {
  ...
  Future<String> triageRun(String flowrunId) async =>
      _seed.triageConversationId;

  @override
  Future<List<TriggerEntity>> listTriggers() async => [
    for (final t in _seed.triggers) _trigger(t),
  ];

  TriggerEntity _trigger(SchedulerTriggerSeed t) {
    final paused = _paused[t.id] ?? t.paused;
  ...
  Future<TriggerSchedule> triggerSchedule({
    ...
    final points = [..._seed.schedule]..sort((a, b) => a.at.compareTo(b.at));
  ...
  Future<FlowrunMatrix> runMatrix(List<String> flowrunIds) async {
    ...
    final seen = <String>{};
    // A declared row order goes first (one workflow per matrix call, so the first run's workflow
    // names the order). 声明的行序先铺(一次矩阵调用只涉一条 workflow,首个 run 的 workflow 定序)。
    for (final r
        in _seed.matrixRows[runs.firstOrNull?.workflowId] ??
            const <MatrixRow>[]) {
      if (seen.add(r.nodeId)) rows.add(r);
    }
  ...
  @override
  Future<List<EntityRelation>> workflowTriggerEdges() async => _seed.edges;
}

SchedulerRepository demoSchedulerRepository() => FixtureSchedulerRepository();

// ── The original demo battery, as seed builders (data unchanged). 原电池的种子构造器(数据不变)。 ──

List<SchedulerWorkflowRow> _demoWorkflows() => [
  SchedulerWorkflowRow(
    id: 'wf_clean',
    name: '数据清洗流水线',
...
List<WorkflowRunStats> _demoStats() => [
...
List<SchedulerInboxRow> _demoInbox() => [
  SchedulerInboxRow(
    node: FlowrunNode(
...
/// wf_clean's v7 — the one graph the demo's runs walked; the rest are bare entities (no active
/// version → the honest «no graph» sentence). wf_clean 带 4 节点图;其余裸实体。
WorkflowVersion _demoVersion() {
...
List<FlowrunNode> _demoNodesFor(Flowrun run, List<SchedulerInboxRow> inbox) {
  ...
  switch (run.status) {
    ...
    case 'running':
      ...
      final parked = inbox.where((r) => r.node.flowrunId == run.id).firstOrNull;
      return [
        node(
          'fetch',
          'action',
...
List<SchedulerTriggerSeed> _demoTriggers() => [
  ...
  SchedulerTriggerSeed(
    ...
    createdAt: _shift(_anchor.subtract(const Duration(days: 12))),
    lastFiredAt: _shift(_anchor.subtract(const Duration(hours: 2))),
  ),
  SchedulerTriggerSeed(
    id: 'tr_cron_report',
    name: '每周一 08:00',
  ...
  SchedulerTriggerSeed(
    ...
    createdAt: _shift(_anchor.subtract(const Duration(days: 20))),
    lastFiredAt: _shift(
      _anchor.subtract(const Duration(hours: 1, seconds: 12)),
    ),
    nextFireAt: _shift(_anchor.add(const Duration(hours: 5))),
  ),
  ...
  SchedulerTriggerSeed(
    ...
    createdAt: _shift(_anchor.subtract(const Duration(days: 90))),
    ...
    // 就是这条链在自相矛盾。
    lastFiredAt: _shift(_anchor.subtract(const Duration(hours: 26))),
    nextFireAt: _shift(_anchor.add(const Duration(hours: 16))),
    paused: true,
  ),
];

List<SchedulePoint> _demoSchedule() => [
...
const _demoEdges = <EntityRelation>[
  EntityRelation(
    id: 'rel_1',
    kind: 'equip',
    fromKind: 'workflow',
