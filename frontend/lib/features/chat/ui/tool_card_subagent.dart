...
ToolReceipt? subTraceReceipt(Translations t, String output) {
  ...
  if (o.containsKey('subagentRuns')) {
    final n = o['count'] is int
        ? o['count'] as int
        : (o['subagentRuns'] as List?)?.length ?? 0;
    return (
      text: t.chat.tool.subTraceRuns(n: '$n'),
  ...
  if (o.containsKey('blocks')) {
    ...
    return (
      text: t.chat.tool.transcriptSteps(n: '$n'),
      tone: ToolReceiptTone.none,
    );
  }
  return null;
...
Widget getSubTraceBody(BuildContext context, ToolCardState state) {
  ...
  final runs =
      (o['subagentRuns'] as List?)?.whereType<Map>().toList() ?? const [];
  if (runs.isEmpty) {
    return Text(
      t.chat.tool.subTraceNoRuns,
      style: AnText.meta.copyWith(color: c.inkFaint),
    );
  }
  return AnWindow(
          ...
          RunLedgerRow(
            ...
            chips: [
              if ((r['blockCount'] is int ? r['blockCount'] as int : 0) > 0)
                AnChip(
                  t.chat.tool.transcriptSteps(n: '${r['blockCount']}'),
                  tone: AnTone.none,
                ),
            ],
          ),
