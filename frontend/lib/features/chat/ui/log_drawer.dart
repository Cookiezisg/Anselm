...
const String mcpStderrSeparator =
...
({String head, int omitted, String tail}) capLog(String log) {
  if (log.length <= AnCap.logHead + AnCap.logTail) {
    return (head: log, omitted: 0, tail: '');
  }
  return (
...
class _LogDrawerState extends State<LogDrawer> {
  ...
  Widget build(BuildContext context) {
    ...
    final sepIdx = widget.splitStderr
        ? widget.logs.indexOf(mcpStderrSeparator)
        : -1;
    final main = sepIdx >= 0 ? widget.logs.substring(0, sepIdx) : widget.logs;
    final stderr = sepIdx >= 0
        ? widget.logs.substring(sepIdx + mcpStderrSeparator.length).trimLeft()
        : null;
          ...
          ? Column(
              ...
              children: [
                AnWindow(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (capped.omitted > 0) ...[
                        Text(
                          t.chat.tool.logOmitted(n: '${capped.omitted}'),
                ...
                if (stderr != null && stderr.isNotEmpty) ...[
                  ...
                  Text(
                    t.chat.tool.dossierStderr,
                    style: AnText.meta.copyWith(color: c.danger),
                  ),
                  const SizedBox(height: AnSpace.s2),
                  AnWindow(
                    child: Text(
                      stderr.length > AnCap.stderrTail
                          ? stderr.substring(stderr.length - AnCap.stderrTail)
                          : stderr,
