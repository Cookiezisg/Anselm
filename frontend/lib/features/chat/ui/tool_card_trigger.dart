...
List<Widget> _faceOf(
  ...
  switch (kind) {
    ...
    case 'webhook':
      ...
      final hasSecret =
          config['secret'] != null && '${config['secret']}'.isNotEmpty;
      ...
      return [
        ...
        if (id.isNotEmpty && path.isNotEmpty)
          AnChip(
            'POST /api/v1/webhooks/$id/$path',
            look: AnChipLook.outlined,
            mono: true,
            copyValue: 'POST /api/v1/webhooks/$id/$path',
          )
            ...
            child: Wrap(
              ...
              children: [
                ...
                if (pattern.isNotEmpty)
                  Text(pattern, style: AnText.mono.copyWith(color: c.inkFaint)),
              ],
            ),
          ),
      ];
      ...
      return [
        Wrap(
          ...
          children: [
            if (targetKind.isNotEmpty && targetId.isNotEmpty)
              AnRefPill(
                kind: targetKind,
                label: method.isEmpty ? targetId : '$targetId.$method()',
                id: targetId,
              ),
            if (interval != null)
              AnChip(t.chat.tool.trgEvery(n: '$interval'), tone: AnTone.none),
          ],
        ),
        if (condition.isNotEmpty)
          Padding(
            padding: const EdgeInsets.only(top: AnSpace.s4),
            child: Text(
              '${t.chat.tool.trgCondition} · $condition',
      ...
      return [
        AnWindow(
          child: SizedBox(
            ...
            child: AnJsonTree(data: config, showRoot: false),
          ),
        ),
      ];
  }
