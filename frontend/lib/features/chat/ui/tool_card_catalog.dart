...
final ToolCardSpec genericToolCardSpec = ToolCardSpec(
  verb: (t, {required bool live}) =>
      live ? t.chat.tool.calling : t.chat.tool.called,
  target: (s) => s.toolName,
);
...
ToolCardSpec _search({
  ...
  receipt:
      ...
      (t, s) => countReceipt(
        s.resultText,
        countLabel: (n) => countLabel(t, n),
        noneLabel: t.chat.tool.noMatches,
      ),
  body: body,
);
...
ToolCardSpec _build({
  ...
  verb: (t, {required bool live}) => create
      ? (live
            ? t.chat.tool.creatingKind(kind: kind(t))
            : t.chat.tool.createdKind(kind: kind(t)))
      : (live
  ...
  receipt:
      ...
      (t, s) {
        ...
        final v = out['version'];
        final envFailed =
            out['envStatus'] == 'failed' ||
            (out['envError'] as String?)?.isNotEmpty == true;
        if (envFailed) {
          return (text: t.chat.tool.envFailed, tone: ToolReceiptTone.danger);
        }
        ...
        if (out['runtimeState'] == 'crashed') {
          return (
            text: t.chat.tool.runtimeCrashed,
...
ToolCardSpec _entitySearch({
  ...
  verb: (t, {required bool live}) {
    ...
    return listOnly
        ? (live
              ? t.chat.tool.listingKind(kind: kw)
              : t.chat.tool.listedKind(kind: kw))
        : (live
              ? t.chat.tool.searchingKind(kind: kw)
              : t.chat.tool.searchedKind(kind: kw));
  },
  verbOf: listOnly
      ? null
  ...
  receipt: (t, s) => searchReceipt(
    s.resultText,
    listKey: listKey,
    hits: (n) => t.chat.tool.hits(n: '$n'),
    hitsOfTotal: (n, total) =>
        t.chat.tool.hitsOfTotal(n: '$n', total: '$total'),
...
ToolCardSpec _entityGet({
  ...
  verb: (t, {required bool live}) => live
      ? t.chat.tool.viewingKind(kind: kind(t))
      : t.chat.tool.viewedKind(kind: kind(t)),
  target: (s) {
...
ToolCardSpec _revert({
  ...
  verb: (t, {required bool live}) => live
      ? t.chat.tool.revertingKind(kind: kind(t))
      : t.chat.tool.revertedKind(kind: kind(t)),
  target: (s) => s.arg(idKey),
  receipt: (t, s) =>
      revertReceipt(s.resultText, rewind: (v) => t.chat.tool.rewind(v: '$v')),
  body: (context, s) {
...
ToolCardSpec _delete({
  ...
  receipt: (t, s) => deleteReceipt(
    ...
    affected: (n) => t.chat.tool.depsAffected(n: '$n'),
    agentForm: agentForm,
  ),
...
ToolReceipt? _lifecycleWord(
  ...
  return o != null && o[key] == expect
      ? (text: label, tone: ToolReceiptTone.none)
      : null;
}
...
ToolReceipt? _deactivateReceipt(Translations t, String output) {
  final ls = _result(output)?['lifecycleState'] as String?;
  if (ls == 'inactive') {
    return (text: t.chat.tool.offline, tone: ToolReceiptTone.none);
  }
  if (ls == 'draining') {
    return (text: t.chat.tool.draining, tone: ToolReceiptTone.warn);
  }
  return null;
}
...
ToolReceipt? _configKeysReceipt(Translations t, String argsText) {
  try {
    ...
    if (d is Map && d['config'] is Map) {
      ...
      return n == 0
          ? null
          : (text: t.chat.tool.nKeys(n: '$n'), tone: ToolReceiptTone.warn);
    }
  } catch (_) {}
  return null;
}
...
ToolCardSpec _meta({
  ...
  verb: (t, {required bool live}) =>
      live ? t.chat.tool.updatingMeta : t.chat.tool.updatedMeta,
  ...
  receipt: (t, s) {
    ...
    return labels.isEmpty
        ? null
        : (text: labels.join(' · '), tone: ToolReceiptTone.none);
  },
  body: (context, s) {
    final t = Translations.of(context);
    return metaDeltaBody(
      context,
      kindWire: kindWire,
      id: argString(s.argsText, idKey) ?? '',
      argsText: s.argsText,
      idKey: idKey,
      note: handlerNote ? t.chat.tool.noteMetaHandler : null,
    );
  },
);
...
final Map<String, ToolCardSpec> _catalog = {
  ...
  'Write': _fsOp(
    ...
    receipt: (t, s) {
      ...
      if (s.resultText.isNotEmpty &&
          !s.resultText.trimLeft().startsWith('Wrote ')) {
        return (text: t.chat.tool.fsUnconfirmed, tone: ToolReceiptTone.warn);
      }
      if (content == null || content.isEmpty) {
        return (text: t.chat.tool.emptyFile, tone: ToolReceiptTone.none);
      }
      return (
  ...
  'Edit': _fsOp(
    ...
    receipt: (t, s) {
      ...
      if (m != null) {
        return (
          text: t.chat.tool.edited2(n: m.group(1)!),
          tone: ToolReceiptTone.none,
        );
      }
      if (s.resultText.isNotEmpty) {
  ...
  'Glob': _search(
    ...
    receipt: (t, s) {
      ...
      return (
        text: t.chat.tool.items(n: g.truncated ? '${g.total}+' : '${g.total}'),
        tone: ToolReceiptTone.none,
      );
    },
    body: globToolBody,
  ...
  'LS': _search(
    ...
    receipt: (t, s) {
      ...
      if (ls.total == 0) {
        return (text: t.chat.tool.lsEmpty, tone: ToolReceiptTone.none);
      }
      return (
        text: t.chat.tool.items(
          n: ls.truncated ? '${ls.total}+' : '${ls.total}',
        ),
        tone: ToolReceiptTone.none,
      );
    },
  ...
  // ── F4 builds 构建族 ──
  'create_function': _build(
    kind: (t) => t.chat.tool.kind.function,
    create: true,
  ),
  'edit_function': _build(
    kind: (t) => t.chat.tool.kind.function,
    create: false,
    editIdKey: 'functionId',
  ),
  ...
  'create_approval': _build(
    kind: (t) => t.chat.tool.kind.approval,
    create: true,
    body: approvalFormBody,
  ),
  'edit_approval': _build(
    kind: (t) => t.chat.tool.kind.approval,
    create: false,
    editIdKey: 'approvalId',
    body: approvalFormBody,
  ),
  ...
  'search_function': _entitySearch(
    ...
    body: searchHitBody(
      listKey: 'functions',
      cap: 20,
      row: (t, h) => entityHitRow('function', h),
    ),
  ),
  'search_handler': _entitySearch(
    ...
    body: searchHitBody(
      listKey: 'handlers',
      cap: 20,
      row: (t, h) => entityHitRow('handler', h),
    ),
  ),
  ...
  'search_control': _entitySearch(
    ...
    body: searchHitBody(
      listKey: 'controls',
      cap: 20,
      row: (t, h) => entityHitRow('control', h),
    ),
  ),
  'search_approval': _entitySearch(
    ...
    body: searchHitBody(
      listKey: 'approvals',
      cap: 20,
      row: (t, h) => entityHitRow('approval', h),
    ),
  ),
  'search_documents': _entitySearch(
    ...
    body: searchHitBody(
      listKey: 'documents',
      cap: 20,
      row: (t, h) => entityHitRow('document', h),
    ),
  ),
  ...
  'list_attachments': _entitySearch(
    ...
    body: searchHitBody(
      listKey: 'attachments',
      cap: 30,
      row: (t, h) => attachmentListRow(h),
    ),
  ),

  // ── F06 entity-get: «正在查看 X → 已查看 X»; chip settles id→name; the four-part exhibit body.
  // F06 实体 get:动词带类名词 + chip 落定换名 + 四段陈列体。──
  'get_function': _entityGet(
    kind: (t) => t.chat.tool.kind.function,
    idKey: 'functionId',
    body: f06GetBodies['get_function']!,
  ),
  'get_handler': _entityGet(
    kind: (t) => t.chat.tool.kind.handler,
    idKey: 'handlerId',
    body: f06GetBodies['get_handler']!,
  ),
  'get_agent': _entityGet(
    kind: (t) => t.chat.tool.kind.agent,
    idKey: 'agentId',
    body: f06GetBodies['get_agent']!,
  ),
  'get_workflow': _entityGet(
    kind: (t) => t.chat.tool.kind.workflow,
    idKey: 'workflowId',
    body: f06GetBodies['get_workflow']!,
  ),
  'get_control': _entityGet(
    kind: (t) => t.chat.tool.kind.control,
    idKey: 'controlId',
    body: f06GetBodies['get_control']!,
  ),
  'get_approval': _entityGet(
    kind: (t) => t.chat.tool.kind.approval,
    idKey: 'approvalId',
    body: f06GetBodies['get_approval']!,
  ),
  'get_skill': _entityGet(
    kind: (t) => t.chat.tool.kind.skill,
    idKey: 'name',
    body: f06GetBodies['get_skill']!,
  ),
  'get_trigger': _entityGet(
    kind: (t) => t.chat.tool.kind.trigger,
    idKey: 'triggerId',
    body: f06GetBodies['get_trigger']!,
  ),
  ...
  'read_document': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.readingDoc : t.chat.tool.readDoc,
    target: (s) {
      // settled: the `# <name>` heading; live: the args id. 落定:# 标题;live:args id。
      final first = s.resultText.startsWith('# ')
  ...
  'read_attachment': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.readingAtt : t.chat.tool.readAtt,
    target: (s) => s.arg('id'),
  ...
  'revert_function': _revert(
    kind: (t) => t.chat.tool.kind.function,
    kindWire: 'function',
    idKey: 'functionId',
  ),
  'revert_handler': _revert(
    kind: (t) => t.chat.tool.kind.handler,
    kindWire: 'handler',
    idKey: 'handlerId',
    handler: true,
  ),
  'revert_agent': _revert(
    kind: (t) => t.chat.tool.kind.agent,
    kindWire: 'agent',
    idKey: 'agentId',
  ),
  'revert_workflow': _revert(
    kind: (t) => t.chat.tool.kind.workflow,
    kindWire: 'workflow',
    idKey: 'workflowId',
  ),
  'revert_control': _revert(
    kind: (t) => t.chat.tool.kind.control,
    kindWire: 'control',
    idKey: 'controlId',
  ),
  'revert_approval': _revert(
    kind: (t) => t.chat.tool.kind.approval,
    kindWire: 'approval',
    idKey: 'approvalId',
  ),
  ...
  'delete_agent': _delete(
    kind: (t) => t.chat.tool.kind.agent,
    idKey: 'agentId',
    agentForm: true,
  ),
  'delete_workflow': _delete(
    kind: (t) => t.chat.tool.kind.workflow,
    idKey: 'workflowId',
  ),
  'delete_control': _delete(
    kind: (t) => t.chat.tool.kind.control,
    idKey: 'controlId',
  ),
  'delete_approval': _delete(
    kind: (t) => t.chat.tool.kind.approval,
    idKey: 'approvalId',
  ),
  ...
  'stage_workflow': _action(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.staging : t.chat.tool.staged,
    idKey: 'workflowId',
    kindWire: 'workflow',
    receipt: (t, s) =>
        _lifecycleWord(s.resultText, 'staged', true, t.chat.tool.staged2),
    note: (t) => t.chat.tool.noteStage,
  ),
  'activate_workflow': _action(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.activatingWf : t.chat.tool.activatedWf,
    idKey: 'workflowId',
    kindWire: 'workflow',
    receipt: (t, s) =>
        _lifecycleState(s.resultText, {'active': t.chat.tool.listening2}),
  ),
  'deactivate_workflow': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.deactivatingWf : t.chat.tool.deactivatedWf,
  ...
  'kill_workflow': _action(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.killingWf : t.chat.tool.killedWf,
    idKey: 'workflowId',
    kindWire: 'workflow',
    receipt: (t, s) => killReceipt(
      s.resultText,
      killedN: (n) => t.chat.tool.killedN(n: '$n'),
      none: t.chat.tool.noInflight,
    ),
  ...
  'restart_handler': _action(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.restarting : t.chat.tool.restarted,
    idKey: 'handlerId',
    kindWire: 'handler',
    receipt: (t, s) => restartReceipt(
      s.resultText,
      label: (rs) => rs,
      errored: (e) => '${t.chat.tool.restartFailed}: $e',
    ),
    resultFailed: (s) =>
        (_result(s.resultText)?['error'] as String?)?.isNotEmpty == true,
    note: (t) => t.chat.tool.noteRestart,
    noteColor: (c) => c.colors.warn,
  ),
  ...
  'update_handler_meta': _meta(
    kind: (t) => t.chat.tool.kind.handler,
    kindWire: 'handler',
    idKey: 'handlerId',
    handlerNote: true,
  ),
  'update_agent_meta': _meta(
    kind: (t) => t.chat.tool.kind.agent,
    kindWire: 'agent',
    idKey: 'agentId',
  ),
  'update_handler_config': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.configuring : t.chat.tool.configured,
    target: (s) => s.arg('handlerId'),
    receipt: (t, s) => _configKeysReceipt(t, s.argsText),
    body: (context, s) => lifecycleRefNote(
      context,
      kind: 'handler',
      id: argString(s.argsText, 'handlerId') ?? '',
      note: Translations.of(context).chat.tool.noteConfig,
      noteColor: context.colors.warn,
    ),
  ),
  ...
  'manage_conversation': ToolCardSpec(
    verbOf: (t, s, {required bool live}) =>
        manageConversationVerb(t, s, live: live),
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.cvManaging : t.chat.tool.cvManaged,
  ...
  'list_conversations': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.cvListing : t.chat.tool.cvListed,
    target: (s) {
  ...
  'search_conversations': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.cvSearching : t.chat.tool.cvSearched,
    target: (s) {
      final q = s.arg('query');
  ...
  'run_function': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.runningFn : t.chat.tool.ranFn,
  ...
  'call_handler': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.callingMethod : t.chat.tool.calledMethod,
    target: (s) {
  ...
  'invoke_agent': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.invokingAgent : t.chat.tool.invokedAgent,
  ...
  'fire_trigger': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.firingTrigger : t.chat.tool.firedTrigger,
  ...
  'trigger_workflow': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.triggeringWf : t.chat.tool.triggeredWf,
  ...
  'search_function_executions': _searchLog(
    running: (t) => t.chat.tool.searchingFnExec,
    done: (t) => t.chat.tool.searchedFnExec,
    chipArg: 'functionId',
    body: fnExecBody,
  ),
  'search_handler_calls': _searchLog(
    running: (t) => t.chat.tool.searchingHdCalls,
    done: (t) => t.chat.tool.searchedHdCalls,
    chipArg: 'handlerId',
    body: hdCallsBody,
  ),
  'search_agent_executions': _searchLog(
    running: (t) => t.chat.tool.searchingAgentExec,
    done: (t) => t.chat.tool.searchedAgentExec,
    chipArgs: const ['agentId', 'conversationId', 'flowrunId'],
    body: agentExecBody,
  ),
  'search_mcp_calls': _searchLog(
    running: (t) => t.chat.tool.searchingMcpCalls,
    done: (t) => t.chat.tool.searchedMcpCalls,
    chipArg: 'serverId',
    body: mcpCallsBody,
  ),
  // Count families (no aggregates — never fabricate a ✓/✗ split): flowruns / firings / activations.
  'search_flowruns': _countLog(
    running: (t) => t.chat.tool.searchingFlowruns,
    done: (t) => t.chat.tool.searchedFlowruns,
    chipArg: 'workflowId',
    listKey: 'runs',
    body: flowrunsBody,
  ),
  'search_firings': _countLog(
    running: (t) => t.chat.tool.searchingFirings,
    done: (t) => t.chat.tool.searchedFirings,
    chipArg: 'triggerId',
    listKey: 'firings',
    body: firingsBody,
  ),
  'search_activations': _countLog(
    running: (t) => t.chat.tool.searchingActivations,
    done: (t) => t.chat.tool.searchedActivations,
    chipArg: 'triggerId',
    listKey: 'activations',
    body: activationsBody,
  ),
  ...
  'get_function_execution': _getRecord(
    running: (t) => t.chat.tool.gettingFnExec,
    done: (t) => t.chat.tool.gotFnExec,
    chipArg: 'executionId',
    receipt: execRecordReceipt,
    failed: execRecordFailed,
    body: getFnExecBody,
  ),
  'get_handler_call': _getRecord(
    running: (t) => t.chat.tool.gettingHdCall,
    done: (t) => t.chat.tool.gotHdCall,
    chipArg: 'callId',
    receipt: execRecordReceipt,
    failed: execRecordFailed,
    body: getHdCallBody,
  ...
  'get_mcp_call': _getRecord(
    running: (t) => t.chat.tool.gettingMcpCall,
    done: (t) => t.chat.tool.gotMcpCall,
    chipArg: 'callId',
    receipt: execRecordReceipt,
    failed: execRecordFailed,
    body: getMcpCallBody,
  ...
  'get_activation': _getRecord(
    running: (t) => t.chat.tool.gettingActivation,
    done: (t) => t.chat.tool.gotActivation,
    chipArg: 'activationId',
    receipt: activationFireReceipt,
    failed: activationRecordFailed,
    body: getActivationBody,
  ),
  ...
  'get_flowrun': _getRecord(
    running: (t) => t.chat.tool.gettingFlowrun,
    done: (t) => t.chat.tool.gotFlowrun,
    chipArg: 'flowrunId',
    receipt: getFlowrunReceipt,
    failed: getFlowrunFailed,
    body: getFlowrunBody,
  ),
  ...
  'get_agent_execution': _getRecord(
    running: (t) => t.chat.tool.gettingAgentExec,
    done: (t) => t.chat.tool.gotAgentExec,
    chipArg: 'executionId',
    receipt: execRecordReceipt,
    failed: execRecordFailed,
    body: getAgentExecBody,
  ),

  // ── F12 relations + F13 mcp-mgmt + capability/model (B7.2 ecosystem tail) ──
  'get_relations': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.gettingRelations : t.chat.tool.gotRelations,
    target: (s) {
      final id = s.arg('id');
      return id == null ? null : (truncate(id, AnTrunc.id));
    },
  ...
  'list_mcp_marketplace': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.browsingMarket : t.chat.tool.browsedMarket,
    receipt: (t, s) => marketplaceReceipt(t, s.resultText),
    body: marketplaceBody,
  ),
  'get_model_config': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.gettingModelConfig : t.chat.tool.gotModelConfig,
  ...
  'todo_write': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.todoWriting : t.chat.tool.todoWrote,
    receipt: (t, s) =>
        todoReceipt(t, argsJson: s.argsText, rendered: s.resultText),
    body: todoWriteBody,
  ),
  'todo_read': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.todoReading : t.chat.tool.todoRead,
  ...
  'get_subagent_trace': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.gettingSubTrace : t.chat.tool.gotSubTrace,
    target: (s) {
  ...
  'ask_user': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.asking : t.chat.tool.answered,
    awaitingVerb: (t) => t.chat.tool.awaitingAnswer,
    terminalVerb: (t, s) {
      if (s.resultText.startsWith(declinedProsePrefix)) {
        return t.chat.tool.skipped;
      }
      if (s.resultText.trim() == askEmptyAnswerProse) {
        return t.chat.tool.emptyAnswer;
      }
    ...
    receipt: (t, s) {
      ...
      if (s.resultText.startsWith(declinedProsePrefix) ||
          s.resultText.trim() == askEmptyAnswerProse) {
        return null;
      }
  ...
  'decide_approval': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.deciding : t.chat.tool.decided,
    ...
    receipt: (t, s) {
      ...
      try {
        ...
        final status = fr is Map<String, dynamic>
            ? fr['status'] as String?
            : null;
        if (status != null && status.isNotEmpty) {
          return (text: status, tone: ToolReceiptTone.none);
        }
      } catch (_) {}
      return null;
    },
  ...
  'list_approval_inbox': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.clearing : t.chat.tool.cleared,
    receipt: (t, s) {
      try {
        final d = s.resultObj; // C-028: memoized decode 记忆化解码
        final count = d is Map<String, dynamic>
            ? (d['count'] as num?)?.toInt()
            : null;
        if (count != null) {
          return (
            text: count == 0
                ? t.chat.tool.inboxEmpty
                : t.chat.tool.inboxCount(n: '$count'),
            tone: ToolReceiptTone.none,
          );
        }
      } catch (_) {}
  ...
  'Bash': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.runningCmd : t.chat.tool.ranCmd,
    ...
    verbOf: (t, s, {required bool live}) {
      final bg =
          RegExp(r'"run_in_background"\s*:\s*true').hasMatch(s.argsText) ||
    ...
    receipt: (t, s) => bashReceipt(
      ...
      backgroundLabel: (id) =>
          t.chat.tool.bashBackground(id: truncate(id, AnTrunc.id)),
    ),
  ...
  'BashOutput': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.polling : t.chat.tool.polled,
    target: (s) {
      final id = s.arg('bash_id');
      return id == null ? null : (truncate(id, AnTrunc.id));
    },
    receipt: (t, s) => statusReceipt(
      s.resultText,
    ...
    resultFailed: (s) =>
        s.resultText.startsWith('Background shell process not found'),
  ),
  ...
  'write_memory': ToolCardSpec(
    ...
    resultFailed: (s) =>
        s.resultText.trimLeft().startsWith('Cannot save memory'),
  ),
  'read_memory': ToolCardSpec(
    ...
    hasBodyOf: (s) =>
        !s.resultText.contains('not found') ||
        parseMemoryTemplate(s.resultText) != null,
  ),
  ...
  'WebFetch': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.fetchingWeb : t.chat.tool.fetchedWeb,
    target: (s) {
      final url = s.arg('url');
  ...
  'WebSearch': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.searchingWeb : t.chat.tool.searchedWeb,
    target: (s) {
      final q = s.arg('query');
    ...
    resultFailed: (s) => switch (webSearchOutcome(s.resultText)) {
      WebSearchOutcome.noBackend ||
      WebSearchOutcome.misconfig ||
      WebSearchOutcome.providerFail => true,
      _ => false,
    },
  ),
  ...
  'search_tools': ToolCardSpec(
    verb: (t, {required bool live}) =>
        live ? t.chat.tool.searchingTools : t.chat.tool.searchedTools,
    target: (s) {
      final q = s.arg('query');
      if (q == null || q.isEmpty) return null;
      return '"${truncate(q, AnTrunc.line)}"';
