...
const int _depCap = 24; // pill Wrap cap (impact ledger; the count line still reports the full N). 药丸封顶。
...
Widget lifecycleRefNote(BuildContext context, {required String kind, required String id, String? note, Color? noteColor}) {
    ...
    children: [
      ...
      if (note != null && note.isNotEmpty)
        Padding(
          ...
          child: Text(note, style: AnText.meta.copyWith(color: noteColor ?? c.inkFaint)),
        ),
    ],
  );
}
...
Widget Function(BuildContext, ToolCardState) deleteBody({bool agentForm = false}) => (context, state) {
      final deps = agentForm ? parseAgentDependents(state.resultText) : parseDependents(state.resultText);
...
Widget metaDeltaBody(BuildContext context,
    {required String kindWire, required String id, required String argsText, required String idKey, String? note}) {
  final t = Translations.of(context);
  final rows = <AnKvRow>[];
