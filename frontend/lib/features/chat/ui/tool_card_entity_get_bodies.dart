...
Widget Function(BuildContext, ToolCardState) getEntityBody({
  ...
  required GetProjection Function(BuildContext context, Translations t, Map<String, dynamic> out) project,
    ...
    (context, state) {
      ...
      if (out == null) {
        return ToolWindow(child: Text(state.resultText, style: AnText.code, maxLines: 40, overflow: TextOverflow.ellipsis));
      }
...
Widget readDocumentBody(BuildContext context, ToolCardState state) {
  ...
  if (!text.startsWith('# ')) {
    return ToolWindow(child: Text(text, style: AnText.code, maxLines: 40, overflow: TextOverflow.ellipsis));
  }
  ...
  if (text.startsWith('Attached ')) {
    ...
    return EntityGetBody(
      ...
      badges: text.contains('truncated')
          ...
          : null,
      content: [ToolWindow(child: Text(body, style: AnText.code, maxLines: 200, overflow: TextOverflow.ellipsis))],
      rawJson: text,
    );
  }
  return ToolWindow(child: Text(text, style: AnText.code, maxLines: 40, overflow: TextOverflow.ellipsis));
