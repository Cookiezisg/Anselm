...
Future<String> startConversation(
  ...
  try {
    ...
    await ref
        .read(conversationStreamProvider(conv.id).notifier)
    ...
    try {
      ...
      /* best-effort rollback — the original error still surfaces 尽力回滚,原错照冒 */
    }
    rethrow;
  }
  return conv.id;
...
final startConversationProvider =
    ...
    >(
      (ref) =>
          (text, {mentions = const [], attachmentIds = const []}) =>
              startConversation(
                ref,
                text,
                mentions: mentions,
