...
SettingsRepository demoSettingsRepository() {
  ...
  return FixtureSettingsRepository()
    ...
    ..memories.addAll([
      ...
      Memory(
        name: 'user-timezone',
        description: 'SGT (UTC+8)',
        source: 'user',
        updatedAt: at,
      ),
      Memory(
        name: 'retry-policy',
        description: '指数退避,最多 3 次;超限抛 SyncError',
        source: 'ai',
        updatedAt: at,
      ),
    ])
      ...
      McpServerStatus(
        ...
        tools: const [
          McpToolDef(
            ...
            description: 'Resolve a package to its docs id',
          ),
          McpToolDef(
            serverName: 'context7',
            name: 'get-library-docs',
            description: 'Fetch versioned library docs',
          ),
        ],
      ),
      McpServerStatus(
    ...
    ..mcpRegistry.addAll(const [
      McpRegistryEntry(
        name: 'filesystem',
        description: 'Local filesystem access (read/write within a root)',
      ),
      McpRegistryEntry(
        name: 'postgres',
