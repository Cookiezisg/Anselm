...
class SandboxRuntimesController extends AsyncNotifier<List<SandboxRuntime>> {
  @override
  Future<List<SandboxRuntime>> build() =>
      ref.watch(settingsRepositoryProvider).sandboxRuntimes();

  Future<void> install({required String kind, required String version}) async {
    await ref
        .read(settingsRepositoryProvider)
        .installRuntime(kind: kind, version: version);
    await _refresh();
  }
  ...
  Future<void> _refresh() async {
    state = AsyncData(
      await ref.read(settingsRepositoryProvider).sandboxRuntimes(),
    );
  }
}
...
final sandboxEnvsProvider = FutureProvider.autoDispose
    .family<List<SandboxEnv>, String>(
      (ref, ownerKind) =>
          ref.watch(settingsRepositoryProvider).sandboxEnvs(ownerKind),
    );
