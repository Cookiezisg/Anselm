...
abstract interface class OsNotifier {
  ...
  Future<void> show({required String key, required String title, required String body, String? location});
}
...
/// a signed build (WRK-043 Developer ID chain), so notification integration is NOT in `make fe-verify`.
///
...
class LocalOsNotifier implements OsNotifier {
  ...
  Future<void> show({required String key, required String title, required String body, String? location}) async {
    await _plugin.show(
      id: key.hashCode, // a stable slot id per (type, entity) — a repeat replaces rather than piles up
      title: title,
      body: body,
