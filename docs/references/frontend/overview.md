...
- **四海洋**:`chat`(对话)· `entities`(实体:Function/Handler/Agent/Workflow)· `scheduler`(调度)· `documents`(文档),外加齿轮进的 `settings`。左岛顶部 `AnOceanSwitcher` 切换(matched-geometry 滑动药丸)。**当前只 `entities` 是完整海洋;`chat` 的左岛 rail 已建、中心海洋未建;其余占位「即将推出」**。海洋切换暂走 provider(`selectedOceanProvider`),未路由化。
