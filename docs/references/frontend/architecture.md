...
app/                       # 装配根
  ...
  workspace_gate.dart      # 冷启动工作区门控(在 startup gate 之下、Router 之上):名册解析中显"准备工作区";空名册显 onboarding；首次创建后让旧面+真 Chat 壳短暂共存，实测两端 Composer 矩形后由 paint-only 飞行层共享几何交接，壳从入场起不重挂；已有工作区直接 content-in，reduced motion 直达
  workspace_onboarding.dart # 零工作区单页创建面:连续白面;左为弹性且封顶的 Rijksmuseum 公版工作场景+档案签，右为恒宽 460 的裸 mark+ANSELM、恒英文标题与无 lead 真 AnComposer；作品封顶后的宽窗余量回到跨页两侧留白；自带临时 Overlay(WorkspaceGate 尚扣着 Router/Navigator 时承接 tooltip/图标钮)
...
core/                      # 跨切共享层(不依赖上层)
  ...
  workspace/               # workspace_bootstrap(冷启动列名册:有行激活首行;空集返 null 扣在 onboarding,显式 create 落行后直接设 active;**read 非 watch**——生产者出环,否则切换被拽回首个)+ workspace_create_control(首次页/设置共用的 AnComposer 行为宿主:零 lead/attachment/voice,输入后蓝色回车钮,trim/Enter·Shift+Enter·IME/单飞/WORKSPACE_NAME_CONFLICT 本地化)+ workspace_journey(首启共享几何缝:起终 Composer GlobalKey + 终点 paint opacity；两端 EditableText/IME 不跨树搬家)+ workspace_switch(已有世界间热切换,三拍:**①同瞬** go('/') 离旧深链 **并清 core/shell 的 lastChatThread 记忆**[右岛保活面板的绑定——两半齐清,下一帧才放干净全部对话域 provider];**②post-frame** 才设 id+name——「先离开」是先一**帧**、不是先一条语句:provider 重建在下一 widget 帧**之前** flush,翻转瞬间仍被监听的对话域 provider 会带旧 id 在新 ws 下重跑=404[WRK-083 L1,守卫 hot_switch_test 线缆断言];③其余交响应级联;feature 粘性态[chat landing 模型/打字机队列]各自 watch id 自愈)
  ...
  sse/                     # 3 流地基:frame(线缆 + seq 派生 durable) · sse_parser · sse_connection(重连 + 410 续传 + full-jitter + bearer) · sse_gateway(per-scope/per-kind demux,Riverpod 之下;懒建 controller 带 `onCancel` 自摘登记,故 **dispose 必须先快照再关**——边遍历活 map 边 close 会在迭代器脚下改集合,0725 真机关停实锤)。**410 配对律(WRK-083 L7)**:`resync(stream)` 只是把「缺口发生过」说出来,**接住它是消费方的责任**——凡靠某条流的**信号**保鲜的 provider,必须同时订阅**那条流**的 resync 并重取(缺口里的信号永远没了)。故三个仓各暴露一条与自己 `lifecycleSignals`(皆骑 notifications 流)**配对**的 `lifecycleResync`,六处消费方(chat rail·对话头·实体列表·实体详情·library 页树·skill 列表)逐一订阅;chat 的 `transcriptResync` 是**另一条**流(messages,活态点),两者不可互相顶替。源码守卫 `test/guards/lifecycle_resync_guard_test.dart` 逐文件钉死这对配对
  ...
  media/                   # 媒体一族(WRK-082 批B'):media_ref(MediaRef 文法前端半——`attachmentId`/`att_<16hex>`/collectMediaRefs ≤8,**后端 pkg/mediaref 的逐条孪生件**,含**字符串形**识别〔receipt 跨 workflow 节点就是以文本走的〕;两边不一致即出现「模型看得见而界面渲不出」) · media_source(MediaSource 端口 + ApiMediaSource + mediaMetaProvider〔**成功才** keepAlive:附件行不可变,而缓存失败会把一次超时焊成永久墓碑〕——附件是**平台**资源,故各面读媒体不必 import chat) · media_cards(AnMediaRefCard 按**附件行的 mime** 分发〔非 receipt 提示、非逐面自定〕+ AnMediaRefStrip/`forPayload` 展开形〔带间距宿主里空结果不得占槽〕;chat 工具卡 / flowrun 节点检查器 / 实体调试台 / approval 门同吃这一族,不变量④) · media_video(内联播放:海报→点击才构造 controller;**走带控件随播放器出现**) · **media_player_chrome**(`AnVideoControls`:播放/暂停/**重播** + 可拖进度条〔用包自带的 `VideoProgressIndicator`,不手搓手势→时长换算〕+ 位置/时长 + 全屏入口。**B1 人眼验收补的洞**:此前渲的是**裸的** `VideoPlayer(controller)`,而官方 `video_player` 自己不带任何控件,于是一段片子播完停在最后一帧、不能暂停不能重播,想再看只能重载对话——widget 一直在树上且是对的,故任何断言都看不见它) · **media_viewer**(`openImageViewer`/`openVideoViewer`:**放大**视图,直接构造 `RawDialogRoute`〔与 `an_dialog.dart` 同一套:自带遮罩/Escape/点遮罩/`closedLoop` 焦点环/**`Material(transparency)` 免黄下划线**/自补 `namesRoute`〕,但用**自己的** `AnColors.scrimMedia`〔亮 0.82 / 暗 0.88〕而非共享的 `scrim`〔0.28/0.50〕——两种遮罩要的是相反的东西:确认框背后的上下文**让人安心**,而查看器里产物**就是**整个屏幕、任何透出来的东西都在和它抢〔真机全屏播片时 composer 与左岛清晰可读、还和走带条撞在一起〕。图走 `InteractiveViewer` 且按**全分辨率**解码〔与封顶缩略图不同的 ImageCache 键〕;视频**共用那一个活 controller**,故位置与播放态天然不会走偏。一条路由两种载荷,使「放大」这个手势在卡族渲染的每一处含义一致) · attachment_image_provider(按 id 键的 ImageProvider,解码期按显示宽下采样) · **朗读缝(批C)**:`readAloudAvailable`/`readAloud` 两方法 + `readAloudAvailableProvider`(keepAlive,**自吞探测失败返 false**——可用性失败的答案是「没有按钮」而非错误态,让失败透出去会上膛 Riverpod 自动重试、在一个「不能用就该缺席」的入口背后挂一条退避循环)
...
dev/                       # dev 工具:gallery_main（组件画廊）· demo_main（真壳+fixture+跳门控）· onboarding_main（make onboard:真 WorkspaceGate 创建→真壳过渡,fixture 隔离、零后端/零持久化）
...
无边框**不透明白窗**:左岛(`AnIsland` 卡,**弹性 240–400 默认 320、可拖 + 可收起**)· 敞开海洋(窗体白面、无卡,内容列**弹性 480–720**)· 右岛(`AnIsland` 卡,**弹性 280–640 默认 320、可拖**——拖拽实时钳到「海洋保底」动态上限:余宽 = 窗宽 − 左岛现宽 − oceanMin − 间距);四周 8px + 岛间 8px(左右岛 grip 各兼间距,同一套把手文法)。**右岛内容随海洋**(app 的 `_InspectorStack`,懒保活栈——四张脸首揭才建、建后常驻折叠,RI 病灶②):chat=侧幕(`StagePanel`,绑**活选区、以 `lastChatThreadProvider` 线程记忆托底**——去别的海洋看一眼侧幕不卸;记忆是 core/shell provider 而非私有 widget 态,**随 workspace 死**:watch activeWorkspace 换世界即弃 + 热切换第①拍显式清[WRK-083 L1——私有记忆会活过切换,其四个对话域 provider 在新 ws 下重取旧对话])· entities=run 终端(`RunTerminal`)· scheduler=run 卷宗双脸(`SchedulerRunInspector`)· documents=属性面板(`LibraryInspector`:页 name/desc/tags 分部 PATCH、skill frontmatter PUT 全覆盖);壳在"当前海洋有选中 且 未收起"时揭示。**状态由 app 持有、props 喂入,`AnShell` 不沾 Riverpod**(`shellChromeProvider` 左收起/左宽/右宽[持久化,键 `an.side.collapsed/w/rightw`(SettingsPrefs 中央键表,fy.* 旧键一次性迁移);右宽全海洋一份——右岛是同一件 chrome,分海洋的只有收起轴]· `shellHeadProvider` 浮层头面包屑 · **`rightPanelCollapsedProvider` 右收起——shell 级、`core/shell/right_panel.dart`、跨海洋共享**· **`lastChatThreadProvider` 右岛 chat 脸线程记忆——shell 级、`core/shell/inspector_memory.dart`,app 在每次进入对话时上闩**· `selectedOceanProvider`/`notificationsOpenProvider` 左岛两轴 · `activeWorkspaceNameProvider` 底栏名)。
  - **左岛内容(`app_shell.dart` 装配,自上而下,均 gallery-first 的 kit 原语)**:chrome bar(红绿灯 + 收起钮)→ **海洋切换器 `AnOceanSwitcher`**(顶部 4 海洋 chat/entities/scheduler/documents 图标钮,选中展开标签;**matched-geometry 滑动药丸**[单药丸滑动+变宽、旧收新展、整行回流,无水珠收颈];`selectedIndex=-1` 无选中态;横滚不裁)→ **中段**(当前海洋的 rail,走与中心同款懒保活栈 `_RailStack`[S3,`AnLazyIndexedStack` 槽序同 `_OceanStack`]——rail 首访才建、切海洋常驻折叠后,滚动位/展开态不丢;铃开时换成通知托盘[暂态接管、不入栈])→ **底栏 `AnSidebarFooter`**(`AnWorkspaceButton` workspace 快捷菜单[`AnMenu matchAnchorWidth` 与钮等宽下拉:全量切换 /「新建」直达 Settings→Workspaces→addWorkspace 规范创建面 / 工作区设置] | 设置格 | 通知格[红点])。**左岛两条独立轴**:① **选中海洋** `selectedOceanProvider`(顶部 4 + 齿轮进的 `settings`,驱动 rail + 中心;在 settings 时顶部切换器无选中、齿轮高亮)② **通知托盘** `notificationsOpenProvider`(正交,铃 toggle,**接管左岛中段 rail、不动中心**;**点任一海洋[顶部 4 或齿轮]即收起**)。**`chat`/`entities`/`documents` 三海洋已建**(真 ConversationRail/EntityRail/LibraryRail + 各中心),`scheduler` + 通知托盘外的占位 = 「即将推出」;workspace 名经 `activeWorkspaceNameProvider`(冷启动 bootstrap 设、底栏显,空回退默认标签)。**海洋切换暂走 provider(未路由化,后续并入 go_router)**;`selectedOceanProvider` **首次创建成功落 `chat` 初始页,已有工作区启动则恢复上次海洋(SettingsPrefs `an.ocean` 持久化,镜像 `shellChromeProvider` 左岛恢复;恢复只设海洋、不设过期选区 id,不与 URL 相顶;抢先手动切换优先于异步恢复)**。
...
- **三个启动面(规范,永不增 per-feature 入口)**:
  ...
  - `make -C frontend onboard` — 首启创建→普通 Chat 的整段视觉验收(`lib/dev/onboarding_main.dart`):每次空名册,真实 `WorkspaceGate` + 真实壳,fixture 隔离、零后端/零持久化。
