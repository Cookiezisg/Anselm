...
| Composer | 文本、`@` 提及、文件选择/粘贴/拖放附件、模型选择；发送与停止；生成中 Enter 入队，队列可编辑、删除和取回；回合收到 durable completed close 后必须回到发送态，不能因已水化的 user echo 与 optimistic pending bubble 重叠而永久停在“停止生成”；图片媒体准备期间，附件 chip 显示可读的取消动作，取消/失败后显示可读的重试动作 |
| 回合操作 | 复制整回合、分叉、重试换模型、编辑重发、同一逻辑回合的版本翻页；历史版本仍可读，不把 `superseded_by` 当软删；最后用户回合的编辑重发入口即使其后已有 assistant 答复也常显，不依赖不可发现的 hover；分叉点击后立即保留固定几何的「正在分叉」忙碌态并阻止重复创建，直到导航或错误收口 |
| 朗读 | 只有服务端确认当前 workspace 有语音路由时，落定回合的动作行才显示朗读入口；workspace 切换、模型密钥变更和免费档开通（包括冷启动 `WorkspaceBootstrap` 直接调用的 provision）会在 widget build 之外重探可用性，loading/失败均诚实保持无入口，不把一次旧 workspace 的结果带过来；首次合成是可能较慢的真实网络动作，入口在原几何内显示统一 spinner 和本地化“准备朗读中”语义并锁住重复点击，完成后进入播放；服务端命中缓存仍校验附件，陈旧附件映射会在重新合成前修复，避免同一段文本反复付费，普通存储错误不伪装成缓存未命中；实时语音 WebSocket 握手期的网关拒绝只接受闭集产品错误，额度耗尽和安装禁用明确不可重试，限流才允许稍后重试，不把上游散文或泛化断线文案展示给用户 |
| 驻地 | 对话可挂工作目录；三态入口；在访达/终端打开、切换/退出驻地、切分支、新建分支、创建 worktree；脏区切分支直接拒绝。阻断切换的提示必须在菜单现有宽度内完整可读，不能把“下一步”截成省略号。 |
| 工具与人在环 | 工具卡按工具族渲染，默认收起；失败与交互门自动展开；用户停止执行中的工具时必须显示中性取消态，不得显示 `Search failed` 或 `context canceled` 等内部错误；坏参数也必须安全降级并显示后端错误，不能把 transcript widget 树打崩，也不能因此回显敏感参数；危险调用、提问与审批都走同一 interaction broker；`replay_flowrun` 被后端拒绝时明确显示“未执行重放”，不得沿用“已重放运行”；可空 query 的实体搜索才可切到“列”声道，`search_documents` 的 query 必填，畸形空参数必须仍显示“搜索失败”；`search_conversations` 命中卡逐项显示标题、snippet、匹配块数和消息锚点（消息 ID 芯片可复制，点击该行直接执行 transcript deep-jump；标题命中没有消息锚点时只打开对话），正文不能把 5 个命中压成 2 个，也不能把 opaque ID 脱敏成 `the requested item` 坏占位；`delete_document` 的 not-found completed 软失败必须显示失败动词与原始证据，不得显示“已删除”或“软删除,可恢复”；`Subagent` 在 `subagent_type` 校验失败时必须显示“校验失败 · 未启动”，不展示 `get_subagent_trace` 回放提示，也不把校验错误当作子代理回答；系统抑制的重复工具调用保留在线缆与 durable 证据，但不在 transcript 追加第二条“未执行”噪音卡，首个真实调用的结果是用户可见真相 |
| 右岛 | 触点台账 + 流式侧幕；只在存在 Activity 时可揭示，详见 [`chat-sidestage.md`](chat-sidestage.md) |

### 用户可见导航词汇

左侧导航的容器名是 `Library`，它同时承载 Documents 和 Skills；`Documents` 只表示
Library 内的资源类型，不是左侧导航项。Chat 右侧的近期变更侧幕明确标为 `Activity`，
显示当前对话触碰过的动作和实体。用户询问“刚刚发生了什么变化”时，助手应先指向
`Activity`；用户要浏览或编辑结果时，再指向 `Library`。

MCP 或传输失败必须先呈现一条平静的人话摘要；原始协议、RPC、EOF、stack trace 与 stderr
只进入可选的技术详情，助手正文不得重复抄写 raw 错误，除非用户明确要求技术细节。助手
正文只负责指向上方工具卡，标题、行动提示与原始详情均由工具卡承载；右岛只呈现失败工具
和人话状态。这样三处各自承担清晰职责，不把同一故障渲染成多份互相竞争的主文案。
协议 token 的识别必须按独立 token 判断，不能用无边界子串匹配；例如普通替代路径
`LibreOffice` 不得因为包含字母序列 `EOF` 而被投影成 MCP 失败交接文案。
...
- `ChatRepository` 是唯一数据缝；`LiveChatRepository` 接 HTTP/SSE，`FixtureChatRepository` 驱动 demo 与测试，两者保持同一契约形状。普通发送成功后若首条 user SSE 回声因新线程订阅竞态丢失，transcript 用 REST 头做窄对账，不让耐久 user 行与 optimistic bubble 同时可见；若回声（包括 ephemeral `open`/`delta`）在 REST 水化期间进入 prelude，且同一 block 已落在 `settled`，任何迟到 frame 都不得重建重复的 live 节点，但仍必须用 durable 回声归并 pending bubble，不能让回合状态永久停在生成中；**若迟到的是 message 根的 durable `close`，即使根已经在 `settled`，也必须把 close 的 status、错误和终态 content 合并回该根；它是水化竞态下的终态真相，不是重复渲染。**失败气泡的 retry 保持同一气泡，仍由 SSE/重同步收口。自动标题在头部与 rail 同步做一次性揭示，头部标题槽预留最终宽度，模型选择器不能因逐字揭示而横向移动，揭示结束后也不得换槽；utility 无正文时使用首句本地兜底，长请求在可读边界加省略号，不把半个词当作标题。列表 provider 对每条 pinned/驻地轴独立保存 cursor 和 `total`，任何查询轴变化都丢弃旧 cursor 后重新取首屏；生命周期变更只合帧刷新响应头总数，不用已加载 rows 猜段头；驻地组的 `lastMessageAt` 变化（包括一条回合完成）也会合帧重读服务端投影，保证“最近活跃”组头不会停在旧位置。
- 驻地 Git 动作的失败通知必须在顶带固定最大宽度内完整呈现“发生了什么 + 下一步”；英文不得依赖一行省略号隐藏恢复动作。`CONVERSATION_GIT_FAILED` 必须保留 backend 转发的完整 stderr（包括指出实际占用 worktree 的路径），不得只取第一行；需要更长解释时放到可展开的详情面，而不是扩大顶带或让用户猜下一步。`CONVERSATION_WORKTREE_RESIDENCY_UPDATE_FAILED` 是另一种语义：worktree 已经创建但对话仍在旧驻地，必须明确提示用户先从菜单切入，禁止复用“什么都没有改动”的失败文案或诱导重复创建。
...
- 文本模型返回暂时限流时，主时间线只显示本地化的“服务暂时繁忙，请稍后重试”提示；`LLM_RATE_LIMITED` 与上游原文仍留在耐久诊断字段和五通道台账中，不把内部码或 HTTP 细节交给用户，也不在后台静默放大重试。
- 实时语音握手拒绝可能早于原生录音器初始化完成；启动窗口只缓存闭集错误，待 recorder 生命周期建立后统一 teardown；已建立的语音 socket 若同时触发 `onError` 与 `onDone`，按 generation 单飞，只允许一个恢复者执行一次重连或 recorder teardown，避免并发收口制造隐藏 `PlatformException`。服务端拒绝空/超 256 KiB 音频帧与未知控制帧时，前端保留已转写草稿并给出可重试的人话提示，不把协议错误降成无原因的启动失败。
- 侧幕失败丝带按动作语义分流：`create_*` 只说明未创建实体，`edit_*` 才说明上一版仍是真相，执行类工具（例如 `invoke_agent`、`run_function`、`call_handler`）只说明运行失败并指向下方错误；执行失败不得复用草稿/版本文案。执行工具的 `tool_result` 外层可能仍是 `completed`，必须继续读取正文 `ok:false` 或 agent 的 `status=failed|timeout`，否则 Activity 会错误显示 `Ran` 且失去失败行清除出口。`edit_approval` 失败时，侧幕与中心工具卡都必须明确上一版仍有效，禁止把未保存 payload 渲染成带批准/拒绝按钮的审批预览。
...
- **DB 行是真相、流只为实时**：`seq>0` 的 durable 帧可推进水位；`seq=0` 的 delta/tick/interaction 只改瞬时态。410 后重取 REST，再从新水位续流。若断点恰好落在 `tool_call` 已闭合而 `tool_result` 尚未写入前端内存，之后抵达的 assistant 根 `close` 只有回合元数据，不能单独让执行卡落定；根 `close` 发现仍有未闭合执行结果时必须触发一次 REST 水化，以持久行重新接地并清除 Live 幽灵。正常已闭合的回合不重复水化，避免每次收尾产生闪烁或额外请求。
- 水化后迟到的 message 根 durable `close` 合并进已存在的 settled 根时，必须递增该根 revision 使落定行身份缓存失效；这样终态提示立即可见，同时不让其它落定行随流式帧重建。
...
- 当用户明确询问图片或视频、但当前模型无原生视觉输入时，助手必须直接说明“当前模型不能看见/检查像素”，不得猜测或命名媒体内容、声称能访问文件，或退回通用上传确认/重复附件 ID；同时给出切换视觉模型或让用户描述/粘贴内容的下一步。
