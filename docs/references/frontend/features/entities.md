...
- **关系图（主角）**：`AnRelationGraph` 观赏态 framed 预览（720 列内定高、框内平移缩放）。数据源 `relGraphProvider`（`GET /relgraph` 全景快照，autoDispose 每显重取）经 `structuralSubgraph` 过滤到**结构边 equip/link + 结构节点**（溯源 create/edit 边 + 对话节点默认不入场=毛线球放大器）。点节点→进探索态并选中（`/entities/graph?sel=kind:id`），hover 框角「展开↗」→进探索态无选中（`/entities/graph`）。
...
**探索态**（全页 `/entities/graph`，`EntitiesGraphPage`，`ui/graph/`）：**独立无边框 Scaffold 路由，非三岛壳**——宽度对齐 workflow 图编辑器先例（满铺画布 + 透明浮层 chrome + 真可收右岛 `AnIsland`+共享 `rightPanelCollapsedProvider`）。`AnRelationGraph` 满幅（浮缩放条）；左簇 `AnWindowControls` 预留红绿灯 + 返回 + **「显示溯源」开关**（默认关，开后 `?showProvenance` 纳入 conversation 节点 + create/edit 边=「这场对话生了哪些零件」家谱）；底部**kind 图例即显隐过滤**（一行色点 `_LegendChip`，点击 toggle `hiddenKinds`，纯渲染过滤、布局不变；图例兼职过滤器不另设两件）；右岛**实体卡**（`GraphEntityCard`：kind 色字形+名+vN+描述[rail kind 经 `entityRowFetchProvider` 取]+**关系分组** 装备了/被引用/链接[`relationGroupsFor`，逐行 `AnRefPill` 可点=图内 fly-to `revealId`+`revealToken` 平移居中]+「打开详情→」进实体页）。**Esc 回总览**；选区单向派生自 URL 的 `?sel`。

**关系图原语 `AnRelationGraph`**（core/ui）+ 力导向核 `force_layout`（core/graph 纯模型，自研理由/三工程律见 [`design-system.md`](../design-system.md) §5）+ kind 色/词单源 `entityKindColor`/`entityKindWord`（core/ui）。demo 关系图种子 `demoRelGraph()`（2 星型枢纽 fn_normalize/hd_slack + skill/mcp/document 边 + 入度 0 的 workflow 源 + 对话溯源边），`demo_fixture_test` 锁种子。
...
`AnSidebarList`(rail)· `AnPage`(海洋滚动脚手架)· `AnOceanHeader` · `AnTabs(flow)` · `AnInspector`(右岛壳)· `AnRow`/`AnRowDetail` · `AnKv`/`AnField`/`AnEditableValue`(就地编辑)· `AnCodeEditor` · `AnGraphCanvas`(workflow hero)· `AnFadeCollapse`(代码收合)· `AnCallout`(envError)· `AnVersionDiff` · `AnStatusDot`/`AnChip`。**全部来自 gallery,零手搓**。
