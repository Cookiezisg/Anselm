...
- Advanced limits 是机器级单一配置：页面由 `GET /limits/schema` 的字段元数据驱动、由 `GET /limits` 水化具体值；单字段修改使用 dotted key 对应的部分嵌套 PATCH。输入框按回车只提交一次并以权威 GET 收敛，点按移出也提交一次；Reset all 必须先确认，成功后重新读取全部服务端默认值，不能由前端猜默认或留下重复写入。
