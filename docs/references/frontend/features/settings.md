...
| 资源 | **模型与密钥** Models&keys | `panels/models_keys_panel.dart` | 旗舰面板,**四区**(0719 重构):①受管免费档卡(未开通 CTA / 配额 meter / 预算横幅)②提供商区——**品牌 logo 密钥行**(受管锁顶 + 探测点尾端常驻 + Test/Edit/Delete hover,**行点击 = 编辑**),添加流程从**供应商市场**起步(H12-e:目录里 173 家**一上来全铺开**,文法与 MCP 市场 `McpMarket` **刻意相同**——`AnAutoGrid(minColWidth: AnSize.block)` 双列品牌卡 + `autofocus` 搜索**逐渐收窄**〔同时匹配 id 与显示名:知道 `togetherai` 的人不该还要猜我们把它渲成「Together AI」〕+ 空结果 `AnState(empty, inset)` + hover/focus **揭示 CTA**〔常驻布局不重排、隐时惰化使静止点击透传给整卡〕+ 整卡点击进表单。**不分组、不维护名单**——分组本身就是一张要人维护的表。卡上两个事实:**模型数**〔把一手厂商与聚合器分开的唯一东西;`models == 0` 意为「没有目录清单」而非「零个模型」,故**不渲那一行**〕与**未验证徽**〔`curated=false` = 靠机械 `npm` → 方言映射抵达、我们从没试过,用户有权在**花掉一把 key 之前**知道〕;已有 key 的家戴「已配置」脸。旧脸是个没有搜索的 logo `Wrap`——十家时对、173 家时不能用,而且是**任何像素断言都抓不到**的那种不能用:它渲得好好的,只是把用户要的那家推到屏幕外)。表单每一栏用目录值预填、可改;`api` 为空的家 base URL 必填,**模板家显示 `baseUrlHint` 形状**(光一句「必填」没法照着做——用户不知道该填的是他自己的 resource 名)。**失败分三种**(WRK-085 §7):**key 不对**(后端自己的消息,绝不替换——它可能是唯一一句真话)/ **这家我们没试过**(`curated=false` 时加一句诚实的第二行)/ **地址不对**(鉴权失败 + base URL 是**用户自填**时,`diagCheckBaseUrl` 与 base URL 栏的 `baseUrlSuspect` 一起**指向那一栏**——一个指向真实但错误地方的 URL,答的 401 与一把坏 key 一模一样)。凭证控件**按 `credential` 分叉**:Vertex 收的是**服务账号 JSON 文件**,给它一个写着「API key」的框会让用户去找一样他项目里从没签发过的东西,故换成「粘 JSON + 选文件」并在**存下之前**验形。ollama/custom baseUrl 硬必填才解锁保存,保存即 `:test`(飞行中转圈)③场景默认三行——收起一句话摘要,点开进**可复用三段面板 `ModelPickerPanel`**(凭证→模型[上下文窗+视觉/文档徽]→原生 knobs 通用渲染[enum 下拉/bool 开关/int 数字,default 预填],应用 `{apiKeyId,modelId,options}`;零可用引导跳密钥区)④搜索区(search 类 key 一层默认选择)。`KeyForm` 仍是 **S-3 状态机**(首次 POST 绑 id、此后一律 PATCH;secret 提交即清)。未配默认对话模型 = 人话句 + `MODEL_NOT_CONFIGURED` 收 tooltip。 |
...
| 系统 | **工作区** Workspaces | `panels/workspaces_panel.dart` | 名册(色点 + 当前高亮,**点行 = 热切换**)+ 新建(复用 `core/workspace/workspace_create_control.dart` 的无 lead `AnComposer` 行为宿主,无创建前色盘；输入后浮出蓝色回车钮,成功自动切到新 workspace 的普通 Chat 初始页)+ 推入编辑(改名同步底栏 / 六预设色改色 / **页尾危险区** `AnTypeToConfirm` 输名解锁,stats 真数字入散文;**当前 ws 与最后一个绝不给删**)。左下 workspace 菜单的「新建」直达同一推入页,不另造浮层表单。 |
...
- **创建与热切换**(`core/workspace/`):`workspace_bootstrap` 用 `ref.read` 出反应环；有名册行即激活首行,空集停在 onboarding。首次显式创建直接摊平 id/name、先把海洋定到 Chat，再由 `workspace_journey` 让旧面与新壳短暂共存，以两端实测 Composer 矩形完成共享几何交接；Shell 初挂后不重挂，reduced motion 直达。`workspace_create_control` 被 onboarding 与设置共用,视觉直接复用真 `AnComposer`(无 @、附件、语音),普通 focus/输入期抑制 accent halo、仅 Enter 提交期点亮；统一 trim/Enter 创建/Shift+Enter 换行/IME 合成守卫/单飞，并只按稳定码 `WORKSPACE_NAME_CONFLICT` 映射「该工作区已存在」。已有世界间切 workspace → `dioProvider`/`sseGateway` 重建(**脉搏在 dio 层**,每切换新 Dio + `onDispose(close)`——放 client 层会在共享 Dio 上叠旧拦截器捏已废 Ref 必炸,回归钉入 `hot_switch_test`)→ 全 Live repo → 全 server-state provider 零逐处接线级联重取；粘性态(landingModel/titleReveals)各自 watch id 自愈;选区全 URL 派生故 `go('/')` 即清。`workspace_switch.dart`:同 id 短路 + 第①拍同瞬 go('/') 离深链**并清右岛线程记忆**(`core/shell/inspector_memory.dart`,WRK-083 L1)+ 第②拍 **post-frame** 设 id——「先离开」是先一帧,翻转时仍被监听的对话域 provider 会带旧 id 重跑。设置新建成功先选 `chat` 再复用这套编舞。
...
- **13 面板数据电池**(0719 P0 防线,`settings_demo_fixture_test.dart`):每面板在**真 demo override 清单**(`demoOverrides`,与 `make -C frontend demo` / capture 同源)下 pump,断言种子数据渲出、无错误/空态脸——capture 手抄 override 子集漂移(六面板假「坏」的 0719 根因)从此过不了门禁;capture_demo 已改吃 `demoOverrides` 单源。
