...
- **磁盘回收**（T4/WRK-070，不是 D1 例外）：删行本身**一字节都不还给文件系统**——SQLite `DELETE` 只把页移到 freelist（`auto_vacuum` 默认 `NONE`），存储面板的「Run 历史保留」若只删行则腾不出磁盘。故库跑在 **`auto_vacuum=INCREMENTAL`**（`infra/db` DSN 首位设定 + 存量 `NONE` 库 boot 时一次 `VACUUM` 迁移），且 `sweepRetention` 在一趟清理真删了行后调**一次** `ReclaimFreePages` 把腾出的页还给 OS（`wal_checkpoint(TRUNCATE)` + 越过回收闸才 `incremental_vacuum`——死空间 ≥25% 或 ≥128MiB，稳态 churn 复用不折腾）。**回收不删任何逻辑行**（只归还已腾空的页），故**不是物理删例外、无需在此新立法**；机制详见 [foundation/platform-pkgs.md](foundation/platform-pkgs.md#infradb) `infra/db` 节。
