...
| 表 | 关键列 | 说明 |
|---|---|---|
| `conversations` | title · auto_titled · system_prompt · **summary / summary_covers_up_to_seq**（压缩器写）· attached_documents(json) · archived/pinned · model_override(json) · **last_message_at**（最近活跃排序键：创建时=now、chat 每条消息刷；列表索引 `idx_conversations_ws_list (ws, pinned DESC, last_message_at DESC, id DESC)` + keyset 游标键此列）· `unread`（持久布尔，0/1：有完成的 assistant 回复未看=1[rail 绿点]。TouchLastMessage 折进同一 UPDATE 原子设——用户发送=0、assistant **完成**终态=1、非完成终态=0；MarkSeen[`:seen` 动作]清 0；创建默认 0。**非排序/游标键、无索引**；存布尔非时间/seq watermark→绕开墙上时钟比较，重启照样在）· **`forked_from_conversation_id` / `forked_from_message_id`**（分叉血缘，`POST /{id}:fork` 写：源线程 + 前缀复制停在的那条消息[含它]。**列演化经 `ALTER TABLE … ADD COLUMN` 两句**、不并进 `CREATE`，使已有安装下次启动补列，重复执行靠 `db.Migrate` 把 "duplicate column name" 视作已应用；用 `TEXT NOT NULL DEFAULT ''` 而非可空——domain 字段是纯 string 且 `''` 已表达「不是分叉」，可空只会为同一种不存在多买一种拼法。fork 时一次写定、永不更新；源日后被删则 id 悬空、UI 只是不显血缘行。**刻意无索引**：每次只读当前线程自己这一行、从不作过滤条件。系统写、线缆只读、不入 PATCH）· **`work_dir`**（**驻地**——对话可选的工作目录，`POST/PATCH {workDir}` 写，`''` = 未挂，WRK-077 WD1）。**列演化经 `ALTER TABLE … ADD COLUMN` 一句**、不并进 `CREATE`，与上方分叉血缘两列同一条结果幂等径（第三次使用）；用 `TEXT NOT NULL DEFAULT ''` 而非可空——`''` 已表达「未挂」，可空只会为同一种「不存在」多买一种拼法 + 给每条 WD1 之前的旧行一次 nil 扫描。写时经 `fspath.Expand` **归一化**（展开 `~`、`Clean`），故落库的恒是真绝对路径、每个下游读者（`ExpandIn` / `Inside` / Bash 的 `cmd.Dir`）不必再猜一遍；非绝对即 422 `CONVERSATION_INVALID_WORK_DIR`，**存在性刻意不校验**（目录被移走/删掉是合法可渲染状态，答案在派生投影 `GET /{id}/workdir` 的 `exists`、不在这一列）。**它不是权限模型**：往外读不受影响，只有往外**写**升级为人闸。**本批刻意无索引**——唯一读者是当前线程自己的头行（按主键、一行、每回合一次）；**WD1.5** 要按本列**分组**出 rail（`GET /conversations/workdir-groups` + `?workDir=`），`(workspace_id, work_dir)` 索引到那时才挣得回它的钱，现在就加是为一个还没人发的查询预付插入代价。用户可改（与 `title`/`model_override` 同走 PATCH 面），**不是**系统写字段 | 软删；`cv_`；三条排序覆盖索引各对一种 `?sort`：活跃 `idx_conversations_ws_list (…, last_message_at DESC, id DESC)` · name `idx_conversations_ws_title (…, title COLLATE NOCASE ASC, id ASC)` · created `idx_conversations_ws_created (…, created_at DESC, id DESC)`——每种 keyset 走各自列、避免全表扫 + filesort（R12 族） |
