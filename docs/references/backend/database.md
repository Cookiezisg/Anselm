...
- [`../references/backend/domains/`](../references/backend/domains/) — 每个 domain 的详设计（含完整 struct + 业务规则）

**定位**：**一眼看到全仓有哪些表 + 关键约束**。struct / 索引细节 / CHECK 约束原文、schema_extras 补丁，**去 references/backend/domains/ 看**。
...
详见 [`../references/backend/domains/user.md`](../references/backend/domains/user.md) §8。
...
#### `api_keys` ✅
详见 [`../references/backend/domains/apikey.md`](../references/backend/domains/apikey.md) §11。
...
#### `model_configs` ✅（2026-05-28 model selection redesign：列名 `provider` → `api_key_id`；2026-06-02：`options` 列）
详见 [`../references/backend/domains/model.md`](../references/backend/domains/model.md) §11。
主键 `mc_<16hex>`；软删（`deleted_at`）；GORM 全唯一索引 `UNIQUE(user_id, scenario)`（partial UNIQUE 暂缓，见 model.md §17 决定）。Scenario 白名单 app 层校验，DB 不 CHECK（3 值：`dialogue`/`utility`/`agent`）。**列名**：`api_key_id TEXT NOT NULL`（引用 `api_keys.id`，无 GORM FK 声明 per V1.2 D4；app 层 RefScanner 走 RESTRICT —— 删 api_key 前扫此列）。**2026-06-02 新增列**：`options TEXT`（JSON `map[string]string`；GORM `serializer:json`；存 provider/model 原生选项，如 `thinking` / `reasoning_effort` / `context`）。`model_cap_overrides` 已删除；模型能力与可选项统一来自 `internal/pkg/modelcatalog`。

#### `conversations` ✅
详见 [`../references/backend/domains/conversation.md`](../references/backend/domains/conversation.md) §8。
主键 `cv_<16hex>`；软删（`deleted_at`）；`user_id` 索引 + **`archived` 索引（§17.12，2026-05-17）**。字段：`system_prompt TEXT`（对话级自定义系统提示词，可为空）/ `auto_titled BOOLEAN`（标记标题是 AI 自动生成的还是用户手动改的）/ **`summary TEXT default ''`**（V1.2 §1 final-sweep；compaction running summary，anchored-append。空 = 尚未发生压缩）/ **`summary_covers_up_to_seq INT64 default 0`**（V1.2 §1；该 summary 覆盖到哪个 block seq；下次压缩从此 seq+1 起算）/ **`attached_documents TEXT default '[]'`**（Phase 5 §14.5c；GORM serializer:json，挂载文档 ID + includeSubtree refs）/ **`archived BOOLEAN default false`**（§17.12，2026-05-17；归档隐藏；List 默认 `WHERE archived = false`，UI 切到归档面板用 `?archived=true`）/ **`pinned BOOLEAN default false`**（§15.6，2026-05-17；List `ORDER BY pinned DESC, created_at DESC, id DESC`；testend sidebar 客户端同 sort + 📌 标识）/ **`model_override TEXT`**（§12.3；GORM serializer:json；**JSON shape `{apiKeyId, modelId, options?}`**；nil = 用全局 dialogue scenario；`llmclient.ResolveDialogueWithOverride` 优先 conv override；PATCH F1 走 `keys.ResolveCredentialsByID` 校验 apiKeyId 存在 + 跨用户隔离 → 404 `API_KEY_NOT_FOUND`）。Title 允许空字符串，首轮完成后 auto-titling goroutine 回写。
...
#### `functions` ✅
详见 [`../references/backend/domains/function.md`](../references/backend/domains/function.md) §5.1 + redesign topic [`02-function.md`](../archive/forge-redesign-2026-05/02-function.md) §5.1。
...
#### `function_versions` ✅
详见 [`../references/backend/domains/function.md`](../references/backend/domains/function.md) §5.2 + redesign topic [`02-function.md`](../archive/forge-redesign-2026-05/02-function.md) §5.2。
...
- **`env_id`** TEXT 索引（`fnenv_<16hex>`,每 Version 行独立生成,跟 version_id 1:1 但**解耦**;D-redo-8 每版本独立 venv,sandbox 跨域共用时各家命名空间隔离。详 [`../archive/forge-redesign-2026-05/discussions/2026-05-12-env-and-sse-rework.md`](../archive/forge-redesign-2026-05/discussions/2026-05-12-env-and-sse-rework.md) §D）
...
每 Version 独立 venv(D-redo-8),env_status 字段对当前 version 状态零歧义。env 装配同步发生在 `create_function` / `edit_function` 工具内(D-redo-9),失败由 LLM tool 走内部 env-fix loop(maxAttempts=3,详 [`../archive/forge-redesign-2026-05/02-function.md`](../archive/forge-redesign-2026-05/02-function.md) §10)。accepted 版本上限 `functiondomain.AcceptedVersionCap = 50`,超限硬删最旧。

#### `function_executions` ✅ (D22)
详见 [`../references/backend/domains/function.md`](../references/backend/domains/function.md) §5.3 + redesign topic [`08-executions.md`](../archive/forge-redesign-2026-05/08-executions.md) §4.1。
...
#### `handlers` ✅
详见 [`../references/backend/domains/handler.md`](../references/backend/domains/handler.md) §8.1。
...
#### `handler_versions` ✅
详见 [`../references/backend/domains/handler.md`](../references/backend/domains/handler.md) §8.2。
...
AcceptedVersionCap = 50/handler。Env 装配跟 function 同模式 — 同步发生在 `create_handler` / `edit_handler` 工具内,失败走内部 env-fix loop(详 [`../archive/forge-redesign-2026-05/03-handler.md`](../archive/forge-redesign-2026-05/03-handler.md) §11)。

#### `handler_calls` ✅ (D22)
详见 [`../references/backend/domains/handler.md`](../references/backend/domains/handler.md) §8.3 + [`08-executions.md`](../archive/forge-redesign-2026-05/08-executions.md) §4.2。
...
#### `workflows` ✅
详见 [`../references/backend/domains/workflow.md`](../references/backend/domains/workflow.md) §8.1。
...
#### `workflow_versions` ✅
详见 [`../references/backend/domains/workflow.md`](../references/backend/domains/workflow.md) §8.2。
...
**2026-06-02 model options**:`Graph.Nodes[i].modelOverride` (JSON 内嵌字段) 形状 `{apiKeyId, modelId, options?}`;仅 `agent`/`llm` 节点生效。删 api_key 走 RESTRICT:apikey RefScanner 用 LIKE `%"apiKeyId":"<id>"%` over `workflow_versions.graph` JSON 扫,JOIN `workflows.user_id` 限定用户(`workflow_versions` 表本身无 user_id 列)。
...
#### `flowruns` ✅
详见 [`../references/backend/domains/flowrun.md`](../references/backend/domains/flowrun.md) §2.1。
...
#### `flowrun_nodes` ✅
详见 [`../references/backend/domains/flowrun.md`](../references/backend/domains/flowrun.md) §2.2 + [`08-executions.md`](../archive/forge-redesign-2026-05/08-executions.md) §4.5。
...
#### `mcp_calls` ✅ (D22)
详见 [`../references/backend/domains/mcp.md`](../references/backend/domains/mcp.md) + [`08-executions.md`](../archive/forge-redesign-2026-05/08-executions.md) §4.3。
...
详见 [`../references/backend/domains/skill.md`](../references/backend/domains/skill.md) + [`08-executions.md`](../archive/forge-redesign-2026-05/08-executions.md) §4.4。
...
> 不持久化的兄弟域：**ask**（AskUserQuestion 会合）—— `app/ask` 持有 in-memory `pending` map（toolCallID → channel），无 DB 表，无 entity；详见 [`../references/backend/domains/ask.md`](../references/backend/domains/ask.md)。
...
- **mcp 配置**：`~/.forgify/mcp.json` 文件 source of truth（Claude Desktop schema 兼容）；server runtime 状态在内存。详见 [`../references/backend/domains/mcp.md`](../references/backend/domains/mcp.md) §5。
- **skill 文件**：`~/.forgify/skills/<name>/SKILL.md` 文件系统 source of truth；fsnotify 维护内存 cache。详见 [`../references/backend/domains/skill.md`](../references/backend/domains/skill.md) §5。
- **catalog**：纯派生，不落盘、不缓存——开聊时按需现查 function/handler/skill/mcp 四源 mechanical 拼装。详见 [`../references/backend/domains/catalog.md`](../references/backend/domains/catalog.md)。
