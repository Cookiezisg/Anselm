...
>
> **列演化两径**（SQLite 现实）：加列走 `ALTER TABLE ADD COLUMN`（结果幂等，`duplicate column name` = 已应用；先例 flowruns `origin`、triggers `paused`/`missed_checked_at`、conversations `forked_from_conversation_id`/`forked_from_message_id`/**`work_dir`**、messages `superseded_by`）；**CHECK 加词无法 ALTER**，须**整表重建**——`db.MigrateRebuild(table, marker, stmts…)` 查 `sqlite_master` 现行 DDL，仅当标记词缺席才在单事务内建新表→逐列拷贝→删旧→改名→重建索引（结果幂等：全新安装的 CREATE 已含新词 → 永不重建；重建后每次启动 no-op）。`trigger_firings.status += 'missed'` 是首例，`flowrun_nodes.status += 'cancelled'` 第二例，**`message_blocks.type += 'marker'`（六 → 七，WRK-077 WD1）第三例**——立法见 messages 节的 `marker` 专条。
...
- **磁盘回收**（T4/WRK-070，不是 D1 例外）：删行本身**一字节都不还给文件系统**——SQLite `DELETE` 只把页移到 freelist（`auto_vacuum` 默认 `NONE`），存储面板的「Run 历史保留」若只删行则腾不出磁盘。故库跑在 **`auto_vacuum=INCREMENTAL`**（`infra/db` DSN 首位设定，新库天生如此），且 `sweepRetention` 在一趟清理真删了行后调**一次** `ReclaimFreePages` 把腾出的页还给 OS（`wal_checkpoint(TRUNCATE)` + 越过回收闸才 `incremental_vacuum`——死空间 ≥25% 或 ≥128MiB，稳态 churn 复用不折腾）。另有用户主动通路：存储面板「压缩数据库」按钮（`POST /storage:compact`）跑一次**全量 `VACUUM`** 回收全部死空间、并顺带把 mode=0 库升级到 `INCREMENTAL`（`GET /storage-stat` 诚实显示大小 + 可回收死空间）。**`VACUUM`/`incremental_vacuum` 都不删任何逻辑行**（只归还已腾空的页），故**不是物理删例外、无需在此新立法**；机制详见 [foundation/platform-pkgs.md](foundation/platform-pkgs.md#infradb) `infra/db` 节。
...
| 表 | 关键列 | 说明 |
|---|---|---|
| `conversations` | title · auto_titled · system_prompt · **summary / summary_covers_up_to_seq**（压缩器写）· attached_documents(json) · archived/pinned · model_override(json) · **last_message_at**（最近活跃排序键：创建时=now、chat 每条消息刷；列表索引 `idx_conversations_ws_list (ws, pinned DESC, last_message_at DESC, id DESC)` + keyset 游标键此列）· `unread`（持久布尔，0/1：有完成的 assistant 回复未看=1[rail 绿点]。TouchLastMessage 折进同一 UPDATE 原子设——用户发送=0、assistant **完成**终态=1、非完成终态=0；MarkSeen[`:seen` 动作]清 0；创建默认 0。**非排序/游标键、无索引**；存布尔非时间/seq watermark→绕开墙上时钟比较，重启照样在）· **`forked_from_conversation_id` / `forked_from_message_id`**（分叉血缘，`POST /{id}:fork` 写：源线程 + 前缀复制停在的那条消息[含它]。**列演化经 `ALTER TABLE … ADD COLUMN` 两句**、不并进 `CREATE`，使已有安装下次启动补列，重复执行靠 `db.Migrate` 把 "duplicate column name" 视作已应用；用 `TEXT NOT NULL DEFAULT ''` 而非可空——domain 字段是纯 string 且 `''` 已表达「不是分叉」，可空只会为同一种不存在多买一种拼法。fork 时一次写定、永不更新；源日后被删则 id 悬空、UI 只是不显血缘行。**刻意无索引**：每次只读当前线程自己这一行、从不作过滤条件。系统写、线缆只读、不入 PATCH）· **`work_dir`**（**驻地**——对话可选的工作目录，`POST/PATCH {workDir}` 写，`''` = 未挂，WRK-077 WD1）。**列演化经 `ALTER TABLE … ADD COLUMN` 一句**、不并进 `CREATE`，与上方分叉血缘两列同一条结果幂等径（第三次使用）；用 `TEXT NOT NULL DEFAULT ''` 而非可空——`''` 已表达「未挂」，可空只会为同一种「不存在」多买一种拼法 + 给每条 WD1 之前的旧行一次 nil 扫描。写时经 `fspath.Expand` **归一化**（展开 `~`、`Clean`），故落库的恒是真绝对路径、每个下游读者（`ExpandIn` / `Inside` / Bash 的 `cmd.Dir`）不必再猜一遍；非绝对即 422 `CONVERSATION_INVALID_WORK_DIR`，**存在性刻意不校验**（目录被移走/删掉是合法可渲染状态，答案在派生投影 `GET /{id}/workdir` 的 `exists`、不在这一列）。**它不是权限模型**：往外读不受影响，只有往外**写**升级为人闸。**WD1.5 起有索引** `idx_conversations_ws_workdir (workspace_id, work_dir, last_message_at DESC) WHERE deleted_at IS NULL`——本列从 WD1 的「每回合按主键读一行」变成了「每次 rail 绘制都对整个 workspace 做一次 GROUP BY」，正是 WD1 说过「索引挣得回它的钱」的那种负载。**一个结构服务两条读**：`GET /conversations/workdir-groups` 的 GROUP BY 把 `(workspace_id, work_dir)` 当索引区间走、并从尾键直接拿 `MAX(last_message_at)` 而不碰表；`?workDir=<path>` 则以页所需的确切顺序扫同一索引里一段连续区间。`pinned` **刻意不进键**:置顶谓词切掉的是单用户量级的寥寥几行，把它放在 `work_dir` 之前会毁掉分组的区间扫。DDL 语句排在它所依赖的 `ADD COLUMN` **之后**（语句有序、按序应用;给还不存在的列建索引会在全新安装上失败）。用户可改（与 `title`/`model_override` 同走 PATCH 面），**不是**系统写字段 | 软删；`cv_`；四条索引:三条排序覆盖索引各对一种 `?sort`：活跃 `idx_conversations_ws_list (…, last_message_at DESC, id DESC)` · name `idx_conversations_ws_title (…, title COLLATE NOCASE ASC, id ASC)` · created `idx_conversations_ws_created (…, created_at DESC, id DESC)`——每种 keyset 走各自列、避免全表扫 + filesort（R12 族）;第四条 `idx_conversations_ws_workdir (ws, work_dir, last_message_at DESC)` 服务 WD1.5 的驻地分组与 `?workDir=` 组内翻页（见 `work_dir` 列） |
...
| `attachments` | sha256(内容寻址，非唯一) · filename · mime_type · kind(image/document/text/audio/video/other) · size_bytes · blob 字节在 infra/fs/blob 按 sha256 寻址 · **溯源三列**(WRK-082 H5.7)`source`(产地自己的名字，与 receipt 同一套词表：generate_image/generate_speech/generate_video/function_artifact/handler_artifact/mcp/read_aloud；``=用户上传) + `origin_conversation_id` + `origin_flowrun_id` + `origin_tool_call_id`(铸它的那次执行；四列均 NOT NULL DEFAULT ''、**刻意不建索引**，在 `Upload` 这一处从 ctx 盖上) | 软删；`att_`；≤50MB。**刻意没有保留线,附件永不自动删除**（WRK-082 H5.9 裁定）——flowrun 有保留线是因为 run 记录是**机器无界生成的历史**，不清会真失控；附件是**人的动作产生的内容**，量由人决定、且用户在存储面板看得见它（受管档封顶 10 条视频/天 ≈ 30MB/天 是**持续满额**的极端，真实使用一年几百 MB）。而方向不对称：**删除不可逆且静默，不删可逆**。为对称而给附件配一条线，正是设计原则 #6 说的没有物理价值的校验。真出现容量痛点时，正确的下一步是**带预览的手动清理**（先列出「没有被任何对话/文档引用」的那些），**不是**按时间清扫——四个月前那张照片仍在显示它的对话里，也可能嵌在某份文档的 `![](anselm://media/…)` 里。前三列**只记录、不执行**；**`origin_tool_call_id` 是唯一被执行的那列**(WRK-082 H5.8)——`ToolResultContentParts` 只展开「本次工具调用自己铸出」的附件，其余当文本。审计实证:全部 125 个工具里只有 `inspect_media` 与 `read_attachment` 会正当回显自己没铸的 id，而**两者的描述都明写不得把字节倾倒进对话**，故这条收紧是**修好了两份正被静默违反的契约**。另两个消费入口(agent payload / 附件文档)仍走未收窄的 `ToContentParts`——收窄它们会让「附一个文件」整个坏掉 |
...
| `gen_spend` | provider · model · category(CHECK image/speech/video) · **units**(该品类原生计费单位:图=张、语音=字符 rune、视频=秒) · **est_pusd**(估算价,pico-USD;**0 = 诚实的未知、绝不是免费**) · conversation_id · tool_call_id · created_at | **直连侧生成支出台账**(WRK-082 H10)。Log 表:**无 `deleted_at`、无更新路径**(D1);索引 `idx_gsp_day (ws, created_at)` 支撑按日聚合,别无读形态故别无索引。**受管调用不入表**——网关权威记账、桌面已在免费档配额卡展示,再记即数两遍。**刻意不配保留线**:一行约百字节、按人的动作尺度累积,重度使用一年 MB 级,配旋钮是校验剧场(#6),同「附件永不自动删」(H5.9)之判断。**两条诚实律**:用量恒真(数出来的)、金额恒为估算(手写价目表,与网关 `assumed-` 卡同源同债)。**没有 `voice` 品类**——参考音色只在受管档(`voice-enrollment` 不收字节、只取地址,而直连用户的音频在回环 sidecar 的 bearer 之后**没有地址可给**),故直连台账里永远不会出现音色行,给 CHECK 留一个没人写的成员等于让 schema 撒谎;`gsp_` |
| `voices` | name(用户起的名) · provider · **upstream_id**(该家铸出的 id,合成时作为 `voice` 传) · source_attachment_id(克隆自哪段音频) · created_at | **克隆音色**(WRK-082 H9)。**行是指针、不是资源**——资源住在上游。由此推出三条:`UNIQUE idx_voice_name (ws, name)`(同名登记两次会让第一次在上游变成**孤儿**,而本地再没有东西够得着它)· **硬删**(软删的指针会继续占着唯一名、同时指向一个用户以为已经没了的音色)· 删除定义为「**先上游、后行**」(行是唯一知道 upstream_id 的东西)。**上限是行数、不是日计数**:音色是**库存**(创建花一次钱、一直存在到被删),不是配额——每 workspace 2 个,满了要删一个才腾得出位;`vce_` |

> **`message_blocks.type` CHECK 六 → 七：`marker`**（WRK-077 WD1，D 系列立法）。`marker` 是线程上的持久**行内标记**：关于**对话本身**的某件事变了，而日后往回翻的读者必须能看见它变在**哪里**。载荷全在 `attrs`——`{kind, …}`，今天恰有一种 kind：**`{kind:'workdir', from, to}`**（驻地在线程中途被切换，故这条线以上的每次工具调用都跑在别处；`from` 空 = 首次挂载、`to` 空 = 退出驻地，**两端都有意义故存一对、不只存新值**）。`content` **按契约为空**：标签由客户端据 attrs 渲染，使它以用户自己的语言出现——Go 侧写一句话等于把某一种语言硬编码进一条持久行（i18n 铁律）。**kind 放在 attrs、而非铸 N 个块型**：CHECK 是封闭集、加词要付一次整表重建，故一个 `marker` 型配一个开放的 attrs kind，意味着**下一个**持久标记（换模型、改 system prompt）是纯 app 层改动。
>
> **D1 合宪**：`marker` 是**追加**（`CreateMessage` 落一个合成 assistant 回合 + 一个块，与 compaction 锚`contextmgr.writeAnchor` 完全同形），零删除、零改写。它对 LLM 不可见**不靠过滤**、靠 `BlocksToAssistantLLM` 是**类型白名单**（text/reasoning/tool_call/tool_result）——marker 在里面根本没有 case，故它不可能进入任何 prompt；同理它**不建**场次条锚点（锚点是你**跳过去**的地方，而一行标记不是目的地，故 `api.md` 里 6 种锚点词汇保持封闭）。**空线程不落标记**：标记是「之前」与「之后」之间的界线，而首条消息还没发的线程没有「之前」——而首发之前就挂驻地恰恰是**常见**路径（先选目录、再开口）。
>
> **重建（第三次使用 `db.MigrateRebuild`）**：SQLite 无法 ALTER CHECK，故已有安装必须**整表重建**——`bootstrap.openDB` 在 `Migrate` **之后**跑 `MigrateRebuild("message_blocks", "'marker'", …)`，**结果幂等**（仅当 `sqlite_master` 现行 DDL 缺该带引号的标记词才重建；全新安装的 `CREATE` 已含它 → 永不重建，重建后每次启动 no-op）。标记词**带引号**故不可能误配到 `parent_block_id` 之类的标识符。本表比前两例（`trigger_firings.status += 'missed'` · `flowrun_nodes.status += 'cancelled'`）多两处需留意：①它装的是**用户说过的话**，拷错不是 bug、是数据丢失；②`superseded_by` 那句 ALTER 落在 **`messages`**、不在 `message_blocks` 上，故本表现行形状**就是**它的 `CREATE`（13 列全内联、无 ALTER 补的列要记）。两个索引随表落、逐个重建，其中 `idx_blocks_conv_seq` 是 **UNIQUE**——一条线程全部排序契约压在它身上。守卫 = `store/messages/rebuild_test.go` 的**等价性**门禁（照 `trigger/rebuild_test.go` 写）：「老安装」夹具从**现行** Schema **派生**（只把 `,'marker'` 拿掉）而非手抄，随后逐 PRAGMA 断言「升级后的表 == 全新安装的表」（列/类型/可空/默认/索引），故往 `CREATE` 加一列却忘了重建会在此挂掉——而不是从一个真实数据库里静默删掉那一列。
