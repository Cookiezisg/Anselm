...
详见 [`../references/backend/domains/model.md`](../references/backend/domains/model.md) §5（ModelCapOverride struct）。
...
**2026-05-28 model selection redesign**:`Graph.Nodes[i].modelOverride` (JSON 内嵌字段) 形状 `{apiKeyId, modelId}`;仅 `agent`/`llm` 节点生效。删 api_key 走 RESTRICT:apikey RefScanner 用 LIKE `%"apiKeyId":"<id>"%` over `workflow_versions.graph` JSON 扫,JOIN `workflows.user_id` 限定用户(`workflow_versions` 表本身无 user_id 列)。
