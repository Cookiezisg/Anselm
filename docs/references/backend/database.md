...
| `gen_spend` | provider · model · category(CHECK image/speech/video/**voice**) · **units**(该品类原生计费单位:图=张、语音=字符 rune、视频=秒、音色=**登记个数**) · **est_pusd**(估算价,pico-USD;**0 = 诚实的未知、绝不是免费**) · conversation_id · tool_call_id · created_at | **直连侧生成支出台账**(WRK-082 H10)。Log 表:**无 `deleted_at`、无更新路径**(D1);索引 `idx_gsp_day (ws, created_at)` 支撑按日聚合,别无读形态故别无索引。**受管调用不入表**——网关权威记账、桌面已在免费档配额卡展示,再记即数两遍。**刻意不配保留线**:一行约百字节、按人的动作尺度累积,重度使用一年 MB 级,配旋钮是校验剧场(#6),同「附件永不自动删」(H5.9)之判断。**两条诚实律**:用量恒真(数出来的)、金额恒为估算(手写价目表,与网关 `assumed-` 卡同源同债——**唯一例外是音色**,$0.2/个逐字印在官方价目页上,故它在两仓的价目行都不背对账债)。**`voice` 行永不冲销**:删掉一个音色收回的是库存位、不是费用,一个「删除即退钱」的支出视图会告诉用户账单降了而它并没有;`gsp_` |
