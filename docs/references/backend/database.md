...
| `agent_executions` | model_id（实际跑的模型）· status · triggered_by(chat/workflow/manual，**无 agent**——员工不调员工) · input/output · **transcript(json，完整 block 序列——运行的自包含耐久记录，不入 message_blocks)** · 溯源 6 列（含 flowrun 3 列：id/node_id/iteration） | 同款 |
