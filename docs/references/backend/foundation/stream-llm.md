...
- **anselm（内置免费档）**：`anselm.go` embed `deepseekProvider` 复用 OpenAI-compatible streaming/tools/reasoning wire，仅覆盖 identity/header 与模型描述。公开模型仅 `anselm-auto`：保守 `256K/32K` 上下文外壳、原生图片+MP4 视频、每回合最多 8 个/3MiB 解码媒体、当前 audio=false、无 knobs。`infra/deviceproof` 持一把加密落盘的 Ed25519 安装私钥；`Transport` 给 install/chat/quota/models probe 逐请求签 method + authority/target + exact body hash + server nonce/jti。`InstallClient` 登记公钥并返回公开 `installId`，该 id 骑受管 api_key 行但不是凭证；无 Bearer 兼容。网关 402 / 流内 `BUDGET_EXHAUSTED` → `ErrQuotaExhausted`（自有 Code、非重试）。
