...
- **deepseek 全文本 parts 坍缩**：user 回合的 `Parts` 中无 image_url 存活时（如图片已被非视觉能力降级成文本占位）以 `\n\n` join **坍缩回字符串 `content`**——纯文本端点（anselm 免费网关继承本构造器）拒收数组形 `content`，且冻结附件逐回合重放，数组形会让该对话每一回合永远 400。有图仍走数组多模态形。
