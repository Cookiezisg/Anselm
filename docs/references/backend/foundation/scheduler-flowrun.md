...
- 端点：`GET/POST /flowruns` · `GET /flowruns/{id}`（头+全节点行）· `POST /flowruns/{id}:replay` · `GET /flowrun-inbox` · `POST /flowruns/{id}/approvals/{node}:decide` → [api.md](../api.md)。
- 码：`FLOWRUN_*` 5 个 → [error-codes.md](../error-codes.md)。
