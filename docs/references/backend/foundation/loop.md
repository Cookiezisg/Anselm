...
无表无端点。回合级错误码（MAX_STEPS_REACHED / TOOL_ERROR_STORM / LLM_STREAM_ERROR；LoadHistory 失败走通用 INTERNAL_ERROR）落 message.error_code（与 HTTP wire code 两个命名空间，见 [chat.md](../domains/chat.md)#6）。ToolProgress（ctx 注入的进度 writer）是工具流 progress 块的唯一通道。
