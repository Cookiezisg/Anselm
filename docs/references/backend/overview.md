...
- **错误系统**：一个类型（`pkg/errors`）、一种造法（`errorspkg.New(kind, code, msg)`）、265 码全 registry 登记；HTTP 读 Kind/Code、LLM 读 Message；机械守卫防回退（`standard_test.go`：sentinel 全用 errorspkg.New · 码全库唯一 · transport 走 FromDomainError）。
