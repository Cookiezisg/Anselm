...
attachment：`POST /attachments`（上传）· `GET /{id}` · `GET /{id}/content` · `DELETE /{id}`。上传与 metadata GET 返回附件行字段外，另带可选 `preparation`：`{status,target?,width?,height?,mimeType?,sizeBytes?,errorCode?}`，其中 image 会认领/暴露 `model-default` 代理准备态（`pending|running|ready|failed`），非 image 为 `not_required`，状态侧车失败为 `unavailable` 且不影响附件元数据可用性。
