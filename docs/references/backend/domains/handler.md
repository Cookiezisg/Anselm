...
用户构建的**有状态** Python 类：每个 handler 跑**一个长生命周期的常驻进程**（像 MCP server）——开局/首调 spawn、跨调用保活（`self.xxx` 状态留存）、edit（改类代码/schema）/改 config/crash 时重启（**纯 meta 改名/改述不重启**——见 §5）、退出软件才优雅关闭。**所有调用方（chat/agent/workflow）共享这一个实例**（真共享状态）。代码层级：`domain/handler` → `app/handler`（16 文件，最大的实体 app）→ `infra/store/handler` + `infra/handler`（RPC 客户端）+ `app/tool/handler`（11 工具）。
...
- **Edit/Revert 必重启实例**（**纯 meta 编辑除外**）：常驻进程不会自己换代码。例外——`Edit` 的 ops **全是 `set_meta`** 时，改的是 Handler 行的 name/description/tags（非版本化类快照），常驻实例跑的东西没变，故**只更行、不铸版本、不重启**——否则一次改名就无谓抹掉有状态实例的内存态（agent 又无别的改名路径）。真代码/schema op 仍重启。
- **`Client` 只有 StreamCall 一个调用动词**（统一入口，progress 回调 nil 即非流式）。

## 6. 契约（引用）

端点 → [api.md](../api.md)#handler（注意 config 三端点 + `:restart`）· 表 → [database.md](../database.md)#handler · 码 → [error-codes.md](../error-codes.md)（domain `HANDLER_*` 17 + infra `HANDLER_CLIENT_*` 5——RPC 客户端原语独立命名空间 + 工具校验 5）· 事件 → [events.md](../events.md)（10 个通知——实体里最多，含 restarted/config_updated/config_cleared/crashed）。LLM 工具 11 个。
