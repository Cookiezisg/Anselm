...
resolve handler → 解析 method spec（校验 + 墙钟 deadline：method 自带 `timeout`(ms) 或全局 `HandlerCallSec` 默认兜底，使每次调用有界）→ `manager.Get`（懒 spawn；**失败也记账**——Get 出错=坏 `__init__`/config 缺/env 未就绪/sandbox 挂，记成 `status=failed` 的 Call 行使其现身调用历史 + `failedCount` + `:triage`，`recordCall` 容忍 nil 实例、instanceId 留空，否则该次失败无迹可查）→ **一律 `StreamCall`**（非流式 method 无 yield 自然退化为普通返回）：yield 三写到 entities run 终端 + 调用方 progress sink + 限长 logtail；调用窗口同时在实例 stderr 扇出上挂 sink（print()/日志 → chat progress + run 终端 + logtail；**有注入 secret 时该 sink 经 `scrubbingWriter` 源头掩码**——print 携注入 secret 绝不明文流上 messages/entities SSE 或持久 progress block，F108 补 F82 只盖 recordCall 审计副本的漏；**窗口归属**：同实例并发调用各收各窗口的行、明示可能串扰；收尾留 30ms stderr 宽限——stdout/stderr 两管道乱序，先于 return 写出的 print 可能后到）→ `recordCall`（Detached ctx，best-effort；溯源 6 列从 ctx 读，同 function（含 `flowrun_iteration`——回边 loop 同节点多轮的审计行靠它区分、1:1 join frn，F175-M12）；**持久化的 `errorMessage` 用 `errorspkg.Surface` 清洗**（F159）——与实时 LLM 错误面一致、渲 Message+Details（含 traceback）但剥 Go 路径面包屑（"handler.Client:"/"context deadline exceeded"），否则 get_handler_call/REST/LLM 读回的耐久记录会漏内部面包屑；**记账前把本实例 sensitive init-arg 的解密值从 error/logs/output 掩成 `********`**——平台抹自己注入密钥的防御纵深（用户代码若把它泄进 traceback/print；只能抹平台已知的注入密钥、非任意用户密钥；config-at-rest 加密 + 读时掩码另算，F82）；`logs` 随行落盘，List 置空、单条 Get 携带）。**错误面也掩、非只审计副本**（F164）：旧 recordCall 只掩持久副本的 errMsg，故返给调用方（`call_handler`→LLM/用户）的**实时**错误仍明文漏密；且 spawn 失败路径以 `inst==nil` 记账、连那道掩码都被绕过。现 `scrubErr`（掩结构化错误 Details 里抬上来的 traceback + cause、保留外层 Code 供 errors.Is）在**构造处**施加——`spawn.go` init 失败 `Wrap` 后即掩（覆盖 spawn 路径 inst==nil）、`call.go` method 路 `mapCallErr` 后即掩——使审计记录与实时返回错误两面皆净。TriggeredBy 空时按 ctx 推（subagent→agent，否则 chat）。
...
- **REST list `?search`**：`GET /handlers?search=<term>` 服务端 `name` 大小写不敏感子串（orm `WhereLike`，与其余 3 实体 list 同构）。
