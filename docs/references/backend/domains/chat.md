...
- **Tools 每步重算**：resident + `search_tools` + **能力工具（CapabilityTools 缝,WRK-082 批B）** + 本对话近期已 activated 的 lazy 工具。能力工具是**逐请求 resident**——仅当运行期路由存在才注入(诚实缺席,`tool/generate.Router.ImageAvailable/SpeechAvailable(ctx)` 每步重估、下一步生效)。**H11 起六个生成工具与朗读全部只在受管档**(`route.provider == "anselm"`),直连方言整体删除:判据是**成本线**——每一种生成能力都要一条**我们手写并维护**的方言,而本项目付过学费的线缆意外全落在那一侧(同一供应商两个域名对工具参数一个发增量一个发累积、TTS 只有 WebSocket、`voice-enrollment` 只收公网 URL);「读」(文本 + 多模态**输入**)的能力来自目录、方言是每家都同意的那一种,故 BYOK 全开。**诚实缺席因此变成「没有受管 install 就整族不存在」**——那是一个合法状态、由 testend 的 `*_HonestAbsence` 场景断言,不是缺陷。**X→X 一族(H9)同缝、谓词更窄**:`edit_image`／`animate_image`／`enroll_voice` 各带 `EditAvailable`／`VideoEditAvailable`／`VoiceCloneAvailable`——**会画不等于会改、会说不等于会克隆**,故家里不支持时该工具**根本不出现**,而不是出现了必然失败;`ErrNoEditRoute` 与 `ErrNoImageRoute` 刻意分开(「你没有出图 key」与「你的出图 key 那家不会改图」是两个事实);完整 schema 随请求走、不进 lazy 简介表(模型必须**知道**自己会画,无发现舞步)。注意一个刻意边界:能力工具**不入** `GET /tools` 静态目录(它按 ctx 存在,目录是静态集);subagent 侧经 `SetMultimodal` 吃同一份 `capabilityTools`(见 [subagent.md](subagent.md))。inactive inventory 在 system prompt 只放 name + ≤180 rune 首行简介；`search_tools` 的 durable tool_result 只返 `{name,purpose}` 激活清单，完整 schema 仅在**下一请求的 tools 字段出现一次**，不再双份占上下文。AgentState 用 16 项 recency set，重新发现刷新、最旧淘汰，防长会话最终退化为“全工具常驻”。**AutoActivator**——LLM 直接点名 lazy 工具时自动激活（免先跑 search_tools）。
