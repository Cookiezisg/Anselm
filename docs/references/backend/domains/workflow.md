...
节点 `Input` 的 CEL **按 node id 引用上游结果**（`reviewer.score`、`fetch.body`），而非某个全局 payload。create/edit 时，每条 Input CEL 用一个 **ScopedEnv** 编译——其根 = **该节点的祖先 node id（+ 恒有的 `ctx`）**（`compileGraphCEL`，R0067/D8）。

- ⇒ **祖先可见性 lint（R0067/D8）**：一个节点只可引用**保证在它之前已完成**的节点（其祖先）的结果，不能引用任意存在节点。引用非祖先 → 那名字在祖先 env 里未声明 → **编译当场失败**——无需走 CEL AST（绕开原 TODO 以为要的 `cel.ReferencedRoots`）。
- **祖先** = 沿全图边（含**回边**）反向可达的所有节点（`workflowdomain.Ancestors`）：故循环内可携带引用（B 读上一轮的 C）、环上节点可引用自己上一轮——对齐运行时 `scopeFor`「循环内取当前轮、循环外取固定 result」。
- 错误两段区分（`compileGraphCEL`）：引用**根本不存在**的名字 → `invalid CEL`（全图 env 那段）；引用**存在但非祖先**的节点 → `references a non-ancestor node`（祖先 env 那段，错误列出可见祖先供作者/LLM 自纠）。语法错 / 未知函数（如 `now()`）同样编写期快速失败。
- 为何重要：运行时 `scopeFor` 把**所有** completed 节点塞进 scope，故引用并行分支节点能不能读到全靠批次顺序——**非确定**。本 lint 把这类坑提前到 create/edit 拦死，保证图确定可重放。
...
| **② app CEL + 祖先可见性** (`compileGraphCEL`，pkg/cel **祖先 scoped env**) | 每条 `node.Input` 表达式语法 + **只可引用祖先 node id**（R0067/D8，见 §3.1）→ 失败映射 `WORKFLOW_INVALID_GRAPH`（违例 node/field + 可见祖先在 `details`） | create/edit（快速失败） |
...
## 12. 后续波次已落地（本节原为 Deferred，现全数完成）

本模块只 STORE + VALIDATE + PIN 图本身、不**解释**它；曾推迟的配套现已全部落地：
- **执行**：durable 解释器 / scheduler ✅ R0049（走 pin 的图，import `ValidateGraph`/`BackEdges`/`BuildPinClosure`/`WorkflowReader`）。
- **flowrun**：执行实例 + 节点结果记忆化 ✅ R0048（`fr_`/`frn_` 两表；**事件日志 `fre_` / `apv_` 投影方案被否**，改记忆化——见 flowrun.md）。
- **祖先可见性 CEL lint** ✅ R0067/D8（§3.1；逐节点祖先 scoped env，绕开 AST 抽取）。
- **M7 中央装配** ✅ R0061（`RefResolver` 直查 7 实体 Service、`WorkflowTools` 进 `Toolset.Lazy`、`WorkflowHandler` 注册、`workflowstore.Schema` 进 migrate）。
- **`:trigger` 执行入口** ✅ R0066/D1（+ `:stage`/`:activate`/`:deactivate`/`:kill`，见 §7）；**`:iterate` AI 编辑** ✅ R0065（aispawn）。
