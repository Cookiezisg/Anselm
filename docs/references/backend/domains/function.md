...
> **driver stdout/stderr 分隔（SSE-B/R0062）**：driver 在调用期间把函数 `sys.stdout` 重定向到 stderr，再把 JSON 结果打回真正的 stdout——这既让 **stdout 保持单一干净 JSON**（`res.Stdout` 可解析；**修了用户 `print()` 与结果交错破坏解析的 bug**），又把函数自己的 `print()` 引到 stderr，被工具层经 `SpawnOpts.StreamErr` → `loop.ToolProgress` **实时流在 run_function tool_call 下**（用户看函数日志滚动）。
