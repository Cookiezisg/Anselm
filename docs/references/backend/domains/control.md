...
`id` · `workspace_id` · `control_id` · **`version`**(单调号) · **`branches`**(json) · `change_reason` · `forged_in_conversation_id`(relation 边用) · 时间戳。`UNIQUE(control_id, version)`。

### 2.3 `Branch`（分支结构，存于 `branches` json）
```go
type Branch struct {
    Port string            `json:"port"`           // 出口名（图把它连到下游节点；可连回上游=循环）
    When string            `json:"when"`            // 布尔 CEL 守卫（first-true-wins；末条恒 "true" 兜底）
    Emit map[string]string `json:"emit,omitempty"`  // 字段→CEL（重塑下游 payload；空=透传）
}
```
...
| 层 | 管什么 | 何时 |
|---|---|---|
| **① domain 结构** (`ValidateBranches`) | 非空 · 每个 port 非空且**唯一**（图要可区分寻址每个出口）· 末条 `when:"true"` **兜底**（防全 false 无路） | create/edit |
| **② app CEL** (`pkg/cel` 编译) | 每个 `when` / `emit` 表达式语法 + 未知函数（如 `now()`）→ `ErrInvalidCEL` | create/edit（快速失败） |
...
create/edit 直接传**完整 branch 组**——branches 是一个原子整体，function 那套增量 `ops` 框架对它无价值（`set_branches` = 全量替换），故**刻意简化**为整组传入。`emit` 里引用的字段与具体 workflow 的 payload 形状耦合，故 control 实体的**复用主要在同一 workflow 内**，价值在「编排-锻造分离 + 统一节点心智」而非跨 workflow 共享。
...
- **relation**：control 是第 **10** 个 EntityKind（前缀 `ctl_`）；`create`/`edit` 边由被锻造实体自述（对话 → 版本）。**ctl 不产出边**——`when`/`emit` 只读 payload，不引用任何其他实体；`workflow → ctl` 监听/引用边由 workflow 侧在波次 4 产。
...
```
取 ctl_ 的 pin 版本（pinned_callables）→ Service.Resolve(id, versionId) 拿 branches
  → 逐分支求 when（first-true-wins）→ 命中分支算 emit → 记 branch_taken{port, payload}
  → 沿该 port 的下游边走（port 连回上游 = 结构化循环，emit 携带循环状态如 attempt+1）
```

**确定性**（durable replay 硬约束）：`when`/`emit` 只读已记账的 payload/ctx、禁墙钟（`now()`），故 `branch_taken` 记下的 emit 后 payload 在重放时抄账、不分叉。详 [`17-execution-contract.md`](../../../working/workflow-revamp/17-execution-contract.md) §3/§4。
