...
端点（CRUD + `:edit`/`:revert`/`:iterate` + versions）→ [api.md](../api.md) · 表 `control_logics`/`control_logic_versions` → [database.md](../database.md) · 码 `CONTROL_*` 9+4 → [error-codes.md](../error-codes.md) · ID：`ctl_`/`ctlv_`。catalog/mention/relation 三适配器同构；版本 cap 50 放过 active。
