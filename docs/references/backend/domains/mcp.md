...
> **🔧 限制优化（2026-05-31，limits-optimization）**：`defaultCallTimeout` 30→180s（agent-recovery 高默认；MCP 工具可能自调 LLM / 爬虫，超时把控制权还给 agent 而非挂死）。详 [`../archive/limits-optimization-2026-05/`](../archive/limits-optimization-2026-05/)。
...
- [`../references/backend/database.md`](../references/backend/database.md) — 无新表（mcp.json 是 source）
- [`../references/backend/error-codes.md`](../references/backend/error-codes.md) — mcp ×14（已接 errmap）
- [`../references/backend/events.md`](../references/backend/events.md) — `mcp_server` per-name entity-state 通知（不再发全量快照）
...
**表**：`mcp_health_history`（见 `references/backend/database.md`）— 无软删，按 `(user_id, server_name, checked_at)` 索引；id 前缀 `mch_`（§S15）。
...
**Wire path**：`/api/v1/notifications` 全局通道 + 客户端按 `type=mcp_server` 过滤；`id` 是 server name 用于增量更新。详 [`../references/backend/events.md`](../references/backend/events.md) notifications 协议章。
