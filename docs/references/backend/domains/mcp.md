...
## 2. 物理模型：`mcp_servers` + `mcp_calls` 两张表

`mcp_servers` = 已装 server 的配置清单；`mcp_calls`（`mcl_`，C4/R0063）= 工具调用审计 log（append-only D1，见 §10.5）。无健康历史表（砍，live 计数在内存 `ServerStatus`，见 §10）。
...
- **`mcp_health_history` 表（`mch_`）**：健康历史删（live 计数在内存 `ServerStatus`）。~~`mcp_calls` 全删~~ → **C4/R0063 重建**：`mcp_calls`（`mcl_`）调用审计回归（见 §10.5），但 LLM 检索工具仍不建。
...
### 10.5 C4/R0063 补全：`mcp_calls` 调用审计 + sensor mcp 目标 + entities run 终端

MCP 此前是唯一**零执行记录**的可执行实体（fn 有 `function_executions`、hd 有 `handler_calls`、ag 有 `agent_executions`），用户拍板补全：

- **`mcp_calls`（`mcl_`）**：每次 `CallTool` 记一行（best-effort detached，对标 handlerapp.recordCall）。对标 `handler_calls` 精简——无 `version_id`（mcp 无版本）/ 无 `instance_id` / 无 flowrun 列（CallTool 无入参携带，不留死列）。列：server_id · tool · status(4 CHECK) · triggered_by(4 CHECK) · input(JSON) · output(text) · error_message · elapsed_ms · started/ended_at · conversation/message/tool_call_id。
- **`CallTool` 签名 += `triggeredBy`**：HTTP=`manual` / workflow dispatcher=`workflow` / sensor=`workflow` / chat 动态工具=`""`（service 从 ctx 推：subagent→`agent`，否则 `chat`）。
- **读取**：`GET /api/v1/mcp-servers/{name}/calls`（`?tool&status&triggeredBy` + cursor/limit）。**不建 LLM 检索工具**（mcp server 是外部黑盒，LLM 调试价值低；与 §10 砍 `search_executions` 决策一致）。
- **sensor mcp 目标接通**：trigger config 早声明 `targetKind=mcp`（method=工具名）但 invoker 缺分支直接报错——`sensorInvoker` 补 `case "mcp"`（结果经 `toResultMap` 以 `{text:…}` 喂 CEL 谓词）。
- **entities run 终端（SSE-C）**：`CallTool` 把 server 的 progress notifications tee 到 `entities` 流 `{mcp, serverID}` 的 `run` 节点（叠在 chat sink 之上；懒开，无进度不发帧），实体面板小终端实时滚。
