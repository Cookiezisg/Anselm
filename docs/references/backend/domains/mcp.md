...
- **安装计划投影**（工单⑨,`POST /mcp-registry:plan`）：`PlanFromRegistry` 把 domain `Plan()`（node>python>docker>dotnet 轻量优先选包,否则首个 remote）的结果投影成 `RegistryPlan{transport, runtime?, oauth, envVars, prerequisite?}`——市场安装表单装前拿到「后端会怎么装 + 要收集哪些 env(isSecret→掩码/required→星标)」,选包逻辑单源服务端、客户端绝不复刻;零副作用,`envVars` 恒 `[]` 非 null;未知条目 404 `MCP_REGISTRY_NOT_FOUND`、无可跑 package 422 `MCP_NO_RUNNABLE_PACKAGE`。
...
- **媒体入口（WRK-082 批B'）**：`client.CallTool` 返 `(text, []Media, error)`——SDK 的 ImageContent/AudioContent 以解码字节出infra;app 层经 `SetUploader`(attachment 服务结构满足)把每件落**一等附件**并把 MediaRef receipt(`{attachmentId,…,source:"mcp_media"}`)追加进文本结果——模型与一切渲染面拿到唯一媒体货币,不再是 `[image: png]` 占位符。逐项 best-effort:单件落库失败保留该件占位叙事、绝不失败整个调用;无 uploader(未装配)整体退回占位(诚实降级)。
...
- **调用记账**（`recordCall`）：mcp_calls Log 表，溯源 6 列与其它执行单元对齐（conversation/message/toolCall 从 ctx + **flowrun 3 列**——id/node_id/iteration，调度器派发注入；`flowrun_iteration` 使回边 loop 同节点多轮的审计行可区分、1:1 join frn，F175-M12）；`logs` 列存 progress 通知、**失败时附 server stderr 尾**（8KiB，server 级、标注可能早于本次调用）；默认 call 超时 180s（MCP 工具可能调 LLM/爬虫，长顶棚把控制权还给 agent）。
