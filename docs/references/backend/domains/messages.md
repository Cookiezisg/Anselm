...
**`Repository`（7 法）—— 两段式写对应 loop.Host 契约**：
...
| `SumTokens(convID)` | 求和对话 input+output token（usage 端点，R0057）。空对话 → (0,0)。 |
| `UpdateBlocksContextRole(ids, role)` | 批量改 block 的 `context_role`（contextmgr R0059 唯一对 message_blocks 的写：投影变更、不改 content）。空 ids no-op。 |
...
- **压缩水位（contextmgr R0059）**：`UpdateBlocksContextRole` 是压缩器对 `message_blocks` 的唯一写（hot→warm/cold/archived，仅投影、不改 content）。「已并入摘要」的**真相源是 `conversation.SummaryCoversUpToSeq` 水位**而非 archived 标记——chat `LoadHistory` 丢弃 `seq ≤ 水位` 的块（崩溃安全：压缩器先写 summary+水位、再 best-effort 翻 archived 标记，故崩溃不重复计数）。详见 `compaction.md`。
