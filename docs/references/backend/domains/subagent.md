...
触发源：LLM 在 chat agent 循环里调 Subagent 工具
  → transport 层：无（system tool 不走 HTTP）
    → app 层：app/tool/subagent.SubagentTool.Execute
        ...
        → 从 ctx 读 effective override：parentModelOverride = reqctxpkg.GetModelOverride(ctx)
            （chat runner 在 agentCtx 上 stash 过 conv.ModelOverride；subagent 链中也通过 subCtx 继续传递）
        → subagentapp.Service.Spawn(parentCtx, type, prompt, opts, parentModelOverride)
            → registry.Get(type) → SubagentType（找不到 → ErrTypeNotFound）
            → llmclientpkg.ResolveDialogueWithOverride(parentCtx, parentModelOverride, picker, keys, factory)
                 → bundle{Client, APIKeyID, Provider, ModelID, Key, BaseURL}
            ...
            → subCtx = parentCtx
                ...
                + WithModelOverride(parentModelOverride)   // 让嵌套 subagent 也读得到同一 override
...
func (s *Service) Spawn(
    ...
    typeName, prompt string,
    opts SpawnOpts,
    parentModelOverride *modeldomain.ModelRef,   // 父 conv 的 ModelOverride;chain 继承
) (*SpawnResult, error)
```

**`parentModelOverride` 参数（2026-05-28 model selection redesign）**：subagent 在语义上是用户对话的延伸——用户为主对话挑了 Opus，subagent 也用 Opus。chat runner 在 agentCtx 上 stash `conv.ModelOverride`（`reqctxpkg.WithModelOverride`）；`SubagentTool.Execute` 从 ctx 读 effective override 透传给 `Spawn` 的此参数；`Spawn` 内部一方面用它走 `ResolveDialogueWithOverride`（subagent 用同 (apiKeyId, modelId)），另一方面继续 stash 进 subCtx 让嵌套 subagent 一层层继承。详 §Model resolution（下一节）。
...
## 6.5 Model resolution（2026-05-28 redesign：subagent 折进 dialogue scenario，继承父 conv override）

**心智模型**：subagent 在语义上是用户对话的延伸——不该有独立的"subagent 档"模型。用户给主对话挑了 Opus，subagent 也用 Opus。

**实现 chain**：
1. 用户在 conv header 改 ModelOverride → `PATCH /api/v1/conversations/{id}` 写 `conversations.model_override = {apiKeyId, modelId}`
2. chat runner `processTask` 加载 conv → `reqctxpkg.WithModelOverride(agentCtx, conv.ModelOverride)`
3. 主 LLM 调 `Subagent` tool → `SubagentTool.Execute` 从 ctx 读 `parentModelOverride = reqctxpkg.GetModelOverride(ctx)`
4. `Service.Spawn(parentCtx, type, prompt, opts, parentModelOverride)` 收到非 nil override
5. Spawn 内：
   - `bundle, err := llmclientpkg.ResolveDialogueWithOverride(parentCtx, parentModelOverride, picker, keys, factory)` → 同主对话用同一 (apiKeyId, modelId)
   - `subCtx = reqctxpkg.WithModelOverride(subCtx, parentModelOverride)` → 嵌套 subagent 继续读得到

**嵌套 subagent 的"父 conv"指最外层**：subagent 内若再 spawn subagent（V1 物理排除挡住；hypothetical），深层 subagent 读到的仍是最外层 conv 的 override——因为 chain 是顺着 subCtx 透传，不是每层重新查 parent conv。

**override == nil 时**：fallback 到 `picker.PickForDialogue(ctx)` —— Layer 1 默认 dialogue 档。

**为什么不用 utility 档**：utility 是"工具内部活儿"（autoTitle / compaction / search rerank），subagent 不是工具内部，是用户对话的语义延伸。两档独立配置（用户分别为 dialogue / utility 配模型），互不影响。
...
func (t *SubagentTool) Execute(ctx context.Context, argsJSON string) (string, error) {
    ...
    // 3. Spawn — 从 ctx 读 effective override 透传给 Spawn 第 5 参数（subagent chain 继承）
    parentModelOverride := reqctxpkg.GetModelOverride(ctx)
    res, err := t.svc.Spawn(ctx, args.SubagentType, args.Prompt, subagentapp.SpawnOpts{MaxTurns: args.MaxTurns}, parentModelOverride)
    if err != nil {
