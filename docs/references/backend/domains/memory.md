...
- **工具（R0044 已建）**：`read/write/forget_memory`（`app/tool/memory`）包 app 的 Get/Upsert/Delete；write 恒 `source=ai`、**不暴露 `pinned`**（用户专属），且**无 `type`**（四分类已砍）。装入 `Toolset.Lazy`、经 `search_tools` 浮现留 M7。
