...
- [`references/backend/`](./references/backend/) — 每个 domain 的服务契约索引（一眼清单）
- [`references/backend/domains/`](./references/backend/domains/) — 每个 domain 的详细设计
...
| 2026-04-24 | **文档结构重组**：`backend-rewrite.md` → `backend-design.md`；分册迁入 `references/backend/`；详设计迁入 `references/backend/domains/` |
...
| 2026-05-03 | **[doc]** 流水线迭代 1 设计文档：`archive/test-pipeline-iteration-documents/01-foundation-and-coverage.md`，~13h / 7 phase / fake LLM + 真 sandbox 双层 / ~80 测覆盖目标。完整方案见该文件 |
...
| 2026-05-10 | **[doc]** forge_redesign trinity architecture spec — Function/Handler/Workflow 三位一体 (`f98c152`) + plan 01 Function domain (`41d9212`) + plans 02-06 完整路线 (`2a0a1a0`)。Phase 4/5 重定向到 forge 重设计。完整方案见 `documents/version-1.2/archive/forge-redesign-2026-05/`。 |
...
| 2026-05-12 | **[feat]** forge_redesign Plan 01 交付（13 commits）——function trinity 替代 forge：domain+store + apply.go 6-op 引擎 + 9 LLM tools + 11 端点 + 14 sentinel + function_executions 表（D22）+ Phase 7 删 forge ~2500 LOC（4 包）。4 pipeline test。新 function.md + 删 forge.md。设计见 [forge_redesign/](./archive/forge-redesign-2026-05/)。 |
...
| 2026-05-12 | **[doc]** Plan 03 redesign 落档(b0af578/89b0d4c/25db693)— 讨论 §A-I + Doc A env 模型 9 files + Doc B SSE 三流 6 files;CLAUDE.md §E1 双→三协议。完整方案 [`discussions/2026-05-12-env-and-sse-rework.md`](./archive/forge-redesign-2026-05/discussions/2026-05-12-env-and-sse-rework.md)。 |
...
| 2026-05-19 | **[feat]** relation domain（跨实体关系图，§16 R1-R4）落地：relationdomain/store/app + `pkg/wikilink` 解析 `[[id]]`（5 实体）+ trinity version 表加 `ForgedInConversationID`。18 个 source-domain hook（CRT/Delete 级联，`SetRelationSyncer` 反注入避循环）+ 3 端点 + 4 sentinel。domain/store/app/wikilink 单测全绿，17 测试包无回归。设计见 [`relation.md`](./references/backend/domains/relation.md)。欠 R5 pipeline test。 |
| 2026-05-13 | **[feat]** forge_redesign Plan 05 交付（17 commits）——workflow execution plane（trinity 最后一块，trigger→scheduler→flowrun 单向链）：flowrun domain+store；trigger（cron/fsnotify/webhook/manual 4 listener，robfig/cron v3 首引入）；scheduler（7-gate StartRun + topo Kahn + 13 节点 dispatcher + retry/timeout/approval pause + 跨进程 rehydrate §6.1）；mcp_calls/skill_executions 两表（D22）；6 LLM tools + 8 端点 + 14 sentinel。134 单测 + 38 pipeline 全绿。trinity 完工。设计见 [`05-execution-plane.md`](./archive/forge-redesign-2026-05/05-execution-plane.md)。 |
...
| 2026-05-26 | **[feat] 阶段 5** 严格化 + 文档体系：`steiger` + `eslint-plugin-boundaries`（6 层注册 + 单向规则）+ `tsc --noEmit`；`make lint-frontend` 与 `staticcheck` 同等地位。文档体系建立：`frontend-design.md`（架构愿景）+ `references/frontend/{fsd-layers,entity-types,cross-cutting}.md`（3 契约索引）+ `references/frontend/slices/`（38 slice 详设计）。 |
...
| 2026-05-28 | **[doc] §S14 + §F1 全量文档同步**（本 rollup commit）：`references/backend/domains/{model,chat,subagent,conversation,web,compaction,apikey,workflow}.md` 全量同步；`references/backend/{api-design,error-codes,database-design}.md` 同步；`frontend-prd.md` §17 + §12；`references/frontend/entity-types.md` ModelRef + Scenario + ModelConfig；4 个 frontend-design-documents 同步（model-config / conversation / workflow / feature-onboarding / feature-settings）。`make verify` + `make matrix` 全绿。|
...
| 2026-05-31 | **[design]** workflow-revamp 第二轮交叉审 batch 全落(文档已迁 `documents/working/workflow-revamp/`;`00`/`05`/`06`/`08`/`11`/`12`):**A1** 修我自己写的矛盾——parked approval 重放按**它 pinned 的老图拓扑**续(`version_id` 钉图;"永远 prod"只作用 callable 解析、不作用 graph;`06` 与 `11` 对齐)· **A2+A5 journal 写入契约**(reviewer #1 风险):`00` 新增「Journal 写入契约 + replay key」段 —— per-flowrun 串行写 + seq 事务内分配 + `UNIQUE(flowrun_id,node_id,iteration_key,type)` record-once + signal/timer first-wins + agent 子步键 `(…,turn,tool_call_id)` + **eventlog(UI 投影)≠ flowrun_events(durable 真相)**;schema 同步加 `timer_armed/fired`·`agent_step_*` 事件 + UNIQUE(`00`/`11`)· **A3 不丢→不丢+不重**:`trigger_firings` 加 `dedup_key` + UNIQUE,onFire 重复材化幂等(崩在 append→bump 之间 / catchup 重算去重)· **A4 durable timer = 节点级 gate**(`at` 绝对 / `after` 相对,arm 时把绝对 deadline 记 journal、重放不重算 `now()`;与 approval timeout 同原语;替掉"wait 砍了"含糊;`00`+`05`)· **B1** 工期表单列 N1(outputSchema 强制,2-3 天)· **B2** 引擎块拆 4 子块 + 按上限重估(总 → ~22.5-26.5 天)· **C1** document `move` = `edit` op + `ForgeOpApplied{op:move}` · **C2** 进度 tick 标 `delivery: ephemeral`、不参与 N7 `Last-Event-ID` 重放 · 顺手清 `is_from_listener` 半残留(全 derivable from trigger 节点 kind)。grep 自查 is_from_listener / A1 矛盾 / forge-4 全清。**仍设计阶段、未实现。**|
| 2026-05-31 | **[design]** workflow-revamp 第三轮(unbiased 评审)batch + **对标标准目录**(新增 `16-standard-conformance.md`;`00`/`03`/`04`/`05`/`06`/`07`/`11`/`12`)。**先对标** van der Aalst workflow-patterns + Temporal 四原语 —— 系统扫标准模式缺口(比 reviewer 抽查全),新挖出 WP11 / WP19。**A-1(最严重,我漏 + 过度声称)**:case 后菱形 merge 用 **active-branch join(WP5 simple-merge)** —— case 给未走分支下发 skip、join 只等被激活入边;改"join=await 全部"为"await 被激活"、修"空票由构造消失"(漏 join 条件菱形会死锁)· **A-5(用户拍板采纳标准)**:flowrun **启动时 pin 图 + callable 版本快照**(= Temporal versioning),在途不漂移;"永远 prod" 缩回引用/编排语义、执行 pin(`00`§3/确定性/schema `pinned_callables`,`03`/`06` 对齐)· **A-2(我上轮契约 bug)**:record-once UNIQUE 只对**结果类 kind**,attempt(node_started/failed)append-many 带 `attempt` 留 retry 痕(`00`/`11`)· **A-3**:trigger 失败规约到 **listener/收件箱层**(trigger 非 activity,retry 在 `trigger_schedules` 不在节点 config;`07`)· **A-4** agent 节点无 retry 旋钮(用 tool 节点)/ **A-6** approval `fail`→`status=failed`+flowrun failed / **A-7** 删 sub-workflow(只 forge 工具)/ **C-3** 清 `agent_uses_agent`(6 种非 7)· **WP11** 定 implicit termination / **WP19-20** cancel 粒度(`flowrun_cancelled` 事件 + ctx.Done + parked 标 cancelled)· **C-2** continue-as-new 定阈值 · **C-1** tick 就放 notifications(用户:没事儿)。grep 自查 A-1/A-5/A-2/agent_uses_agent/7种 全 flip 干净。**抽象未变(durable + 可归约图),修法全是"采纳标准"非堵窟窿;仍设计阶段。**|
| 2026-05-31 | **[doc-fix]** 同步 `CLAUDE.md` 文档地图到新 Diátaxis 结构(`documents/` → `docs/`:concepts/decisions/how-to/references/working)。顶部文档地图 + "改代码前必做" 用户已迁;本次补 §S14 / §F1 / N7 里的 bare 旧文件名 —— `progress-record.md`→`changelog.md`、`database-design`→`database`、`api-design`→`api`、`events-design`→`events`、`backend-design`→`architecture`、N7 SSE 协议详见 → `docs/references/backend/events.md`。grep 自查 CLAUDE.md 无残留旧路径 token(version-1.2 / service-*-documents / adhoc-* 全清)。|
| 2026-05-31 | **[design]** workflow-revamp 第四轮(unbiased 评审)→ **改打法:DRY 收口 + 承重机制写成契约**(新增 `17-execution-contract.md` 为单一事实源;`00`/`01`/`07`/`11`/`13`/`15`)。第四轮逮出的全是**我前几轮的传播漏**——根因是 **00/11 各存一份 schema/状态/join → 必漂移**(不是手滑):`11` 三处仍"汇合 await 全部"(漏 case active-branch)、`11` approval 仍 3 态、字段名 `ref` vs `agentRef`/`callable` 漂移、`00:183` record-once UNIQUE 仍 `kind` 且缺 generation。**不再逐条补 → 把承重契约收口到 `17` 一处**:§1 schema(flowruns+`pinned_callables` / flowrun_events 全 event-type + record-once 仅结果类 `UNIQUE(…,type,generation)` / approvals 5 态 / trigger_firings+`dedup_key`+claim lease)· §2 journal 写入 · §3 **join 激活契约(根治 a1:AND-join await 全部 ✗ case active-branch await 被激活,由 `branch_taken` 重放推导)** · §4 **replay-reset(a2:`:replay` 写 `replay_started`+`generation++`,终态只看最高代,UNIQUE 含 generation)** · §5 **作用域变量(c2:= 上游 `node_completed.result`,不另立 var store)** · §6 **触发 dispatch(a4/a5:claim 原子单事务 / `dedup_key` per-source / polling=`(cursor,段内 index)` 不要事件 ID)** · §7 **节点 config 字段名 canon(c1:agent=`agentRef`/tool=`callable`/approval 端口=`yes`/`no`)** · §8 **校验三层职责(c3:JSON-shape / capability_check 不查类型 / 运行时 outputSchema)** · §9 approval 状态机+tick(a3/a6:tick 有 seq 但不进 replay buffer)。**DRY 落地**:`11` schema/join/approval 全改引用 17(顶部"canon 在 17、以 17 为准"指针 + 修 3 处 await-all→active-branch + approval 5 态)、`00` schema/journal/join 指 17 并修 `kind`→`type,generation`、`13`/`15` 字段名对齐 17 §7、`07`(replay-reset)/`01`(claim/dedup)加回指。grep 自查:await-all 仅余正确的 AND-split 普通节点 fork-join、3 态 approval / `{ref}` / `,kind)` 全清、17 被 6 文档引用。**a1-a6/c1-c3 九条全收口于一处契约,根治 00/11 反复漂移;b1-b5 属实现量(已在 `12` 工期表),非 doc-fixable。仍设计阶段、未实现。**|
| 2026-05-31 | **[impl]** workflow-revamp **M1**(首段实现代码,take-over):durable journal 地基 —— `flowrun_events`(+`dedup_key` partial unique)/ `approvals` / trigger schema 实体 + journal store(`AppendEvent` record-once + `LoadJournal`);3 正确性命脉 TDD 绿(record-once 去重 / seq 严格单调 / `signal_received` first-wins,证伪 approval 双点火)。契约钉于 ADR 016-022 + `17` 重写。strangler-fig:旧 scheduler 待 M2 删。|
| 2026-06-01 | **[impl]** workflow-revamp **M2–M5**(durable 引擎接管 + 控制流 + approval + trigger 收件箱):interpreter agenda 执行器接管 `executeRun`(ADR-016,strangler 旧 topo-walk);CEL 替 text/template;case + fork-join + **active-branch join(A-1 菱形死锁根治)** + loop(`iteration_key`,ADR-017)+ 崩溃重放确定性(属性测试);approval 端到端 durable(park=`signal_awaited` / resume=`signal_received`,journal 真相);M5 trigger `firings` 收件箱 store(`AppendFiring` dedup + `ClaimFiring` 单事务,ADR-021)。~12 interpreter 单测 + approval/cross pipeline 绿。详 `docs/working/workflow-revamp/IMPLEMENTATION-LOG.md`。|
| 2026-06-01 | **[review+fix]** workflow-revamp **对抗式实现 review 第 1 轮**(52-agent workflow,审真实代码非设计):34 confirmed(5 blocker + 14 major),7 refuted(含怀疑论者正确驳回 M6-deferred 的 generation 假 blocker)。**逮到 approval 实际是坏的 + 我的"M4 E2E 绿"是 false-green**(等一个从不被设的 status、从不断言 downstream 真跑)。root-fix 全部**可达** blocker×5 + major×8,每个 TDD red→green:① approval 端口 canon `yes/no`(validator 落后 17 §7)② 单执行者 CAS claim(并发 resume TOCTOU 双 walk)③ cancel 区分 / emit 不吞错 / ctx wire ④ 拒 multi-trigger ⑤ `needed()` loop-header/body ⑥ number 归一(approval-resume 重 walk 的 float64+int CEL 崩)。loop authoring / approvals row / M5 dispatcher / M6 generation 按 §S20 明确 deferred(latent / 无消费者 / 未 wire)。round 2 review 进行中。|
| 2026-06-01 | **[review+fix]** workflow-revamp **对抗式实现 review 第 2 轮**(19-agent;验证 R1 修复无 regression + 找 R1 漏的新 surface + **压测 deferred 判断**):12 confirmed(2 blocker + 3 major + 5 minor + 1 residual)+ 1 refuted;**loop-deferral charter 返 not-a-finding**——独立确认 loop 真 latent、defer 正确。逮到 R1 漏的:**前端 approval UI 全坏**(decision 发 approve/reject 但后端要 approved/rejected → approve+reject 按钮全 400)。root-fix(每个 TDD):① number 归一不对称(R1 漏 fresh activity output)② create.go tool 仍教 LLM 用 approved/rejected 端口(AI authoring 主路径全失败)③ cancel 不了 awaiting_signal parked run(zombie)④ interpreter 无视 DryRun(预览真跑副作用)⑤ **running 崩溃 zombie**(我 wrongly-deferred 到 M6,实为可达+阻塞串行)⑥ 前端 decision/status canon(997 前端测试绿)。**关键洞察:deferred 的 approvals store 其实有消费者(前端 approval banner/inbox)→ 提升其前端阶段优先级。** 收敛趋势:R1 34 → R2 12。详 IMPLEMENTATION-LOG 第 2 轮总结。|
| 2026-06-01 | **[feat]** workflow-revamp **approvals 投影 store(17 §9)**——R2 揭示它有真消费者(前端 approval banner/inbox 需知道哪个 node parked),建之:`ApprovalRepository` port(Park/Decide/CancelParked/ListParked)+ `infra/store/approval`;`Approval` 实体加 `user_id`(inbox scope)+ `UNIQUE(flowrun_id,node_id)`(park upsert 重放幂等);interpreter park 时写行(prompt/allowReason 取自 node config,与 signal_awaited 同 DB 窗口),ResumeApproval 翻 approved/rejected(审计 reason+decided_at),Cancel 翻 cancelled(best-effort 投影,journal 仍是执行真相);新端点 **`GET /api/v1/approvals`**(当前用户 parked 列表,前端 inbox 数据源)。3 store TDD + approval E2E 锁定 park→row→inbox→decide 全链。main.go/harness 接 `SetApprovals`。|
| 2026-06-01 | **[feat]** workflow-revamp **loop authoring 接通(ADR-017)**——loop 此前"建得出但创建被拒":validator Kahn 拒一切环,interpreter 却支持 loop(R1 的分歧)。根治:`workflowdomain.BackEdges` 一个 DFS back-edge 检测**共享**给 validator + interpreter(原各一份、会漂),两者对"哪些是回边"一致;`detectCycle` 排除 reducible 单入口回边再 Kahn(reducible loop 过、不可达/irreducible 环仍拒)。再加 **replay-3 payload-merge**:activity 输出 / case emit 现在**合并**到 inbound payload(原 wholesale 替换 → loop-body activity 丢 counter、loop 一轮就退)。配合 R1 needed() header/body + R2 number 归一,case-back-edge loop 现在**能创建也能执行**(含 body activity)。TDD:reducible loop 接受 / 不可达环拒 / loop-body-activity counter 存活。(剩:nested loop 仍 1-D-unsupported,authoring 未显式拒。)|
| 2026-06-01 | **[feat]** workflow-revamp **M5 durable trigger dispatch(ADR-021)**——trigger 此前 fire-and-forget(onFire→StartRun,崩溃即丢)。现端到端 durable:`onFire → scheduler.OnTriggerFired → AppendFiring(先持久化再动作)→ DispatchPending` 经**单事务 claim**(`buildRun` 校验 → `ClaimFiring` 认领+`tx.Create(run)` 一个 tx → `spawnRun`)—— 无 claimed-without-flowrun 残留、竞争失败跳过、workflow-gone 的 firing shed(终态不丢)。**boot catchup**:`RehydrateOnBoot` 重排 crash 遗留 pending firing,单事务 claim 保证幂等(started 不再 pending → 不重复)。`StartRun` 重构成 `buildRun`+`spawnRun`(直接路径与派发路径共用);`FiringInbox` port(domain/trigger);`trigger_firings` 加 `trigger_kind` + 表注册。ClaimFiring 回调里的 gorm.DB 是单事务既定 trade-off(firings+flowruns 同库一 tx)。TDD:OnTriggerFired 单事务建 flowrun 恰一次、重排不重复;cross+api/workflow pipeline 绿。(剩:cron-tick catchup 确定性 / polling 游标 / overlap 超 serial。)**revamp 里程碑:M0-M5 core ✅ + loop ✅ + approvals ✅;剩 M6/M7/M8 + 前端 banner rewire。**|
| 2026-06-01 | **[feat]** workflow-revamp **M6 lifecycle + `:replay` + failures API(ADR-019)** —— ① **failures API** `GET /flowruns/{id}/failures`:从 journal 取 node_failed,排除被同代/更高代 node_completed 取代的(highest-generation,ADR-019)。② **`:replay`** `POST /flowruns/{id}:replay`:重跑失败 run —— `ReplayRun` 校验 terminal-failed → `BumpGeneration`(generation++ + status→running 一条 UPDATE)→ journal `replay_started` → 重驱 interpreter;interpreter 现给每个事件盖 generation(`WithGeneration(run.Generation)`),重跑记录与上一代区分,copy-hit 取最高代(成功步抄、失败步及下游在新代重跑)。`ErrNotReplayable`(422 `FLOWRUN_NOT_REPLAYABLE`)。③ **lifecycle drain**:`runWG` 跟踪每个 run goroutine,`Drain(ctx)` 优雅关停等 in-flight 完成、超时取消(写终态),wire 进 main.go shutdown,clean SIGTERM 不留 running zombie;ResumeApproval 也走 `ExecuteFn` seam(一致+可测)。TDD:replay bump gen + 重驱 / 非失败拒 / Drain 取消 in-flight / failures highest-gen。cross+api/workflow+errcodes pipeline 绿。**M0-M6 ✅ + loop ✅ + approvals ✅;剩 M7/M8 + 前端 rewire。**|
| 2026-06-01 | **[feat]** workflow-revamp **前端 approvals-banner rewire(端到端闭环)** —— banner 此前靠节点 status `waiting_approval` 过滤渲染,但 interpreter **从不发**该节点态(durable 模型下 approval 是 journal 的 signal_awaited + approvals 投影,非节点 status)→ banner 永不出现(死逻辑)。根治:新 `useApprovalInbox()`(GET `/api/v1/approvals`)+ `Approval`/`ApprovalStatus` TS 类型 + `qk.approvals()`;`ApprovalBanner` 改**自取 inbox、按 runId + status==="parked" 过滤**(去 `nodes` prop),行接 `Approval`(prompt/nodeId/allowReason),reason 按钮 gate 在 `allowReason`;approve/reject onSuccess 加 `invalidate(qk.approvals())` → 决策后 inbox 重取、该行不再 parked、banner 行消失。barrel 导出 `useApprovalInbox`/`Approval`/`ApprovalStatus`;entity-types / cross-cutting / flowrun / page-execute 四文档同步。前端门禁全绿:tsc 0 / eslint 0-err / steiger clean / **996 vitest**(ApprovalBanner 12 测重写为 inbox-mock + FlowRunDetail mock 补 `useApprovalInbox`)。**M0-M6 ✅ + loop ✅ + approvals 端到端 ✅;剩 M7/M8。**|
| 2026-06-01 | **[doc-fix]** approvals rewire 顺带修既有前端 doc 漂移:entity-types `ApproveNodeVars` 端点 `…/nodes/{nodeId}:approve` → 实际 `…/approvals/{nodeId}`、FlowRunStatus 补 `awaiting_signal`、`ApprovalStatus` 新枚举;flowrun.md decision 值 `approve/reject` → `approved/rejected`、7→8 hooks。另 CLAUDE.md 4 处不存在的 make 目标改正:前端 `make lint-frontend`/`make test-frontend` → `make lint`/`make web`、后端"改代码前必做"/原则四的 `make test-backend` → `make unit`(实测三者均 "No rule to make target")。|
| 2026-06-01 | **[feat]** workflow-revamp **M8 trace API(observability;08 §6)** —— **`GET /api/v1/flowruns/{id}/trace`**:把 flowrun journal(durable 真相,seq 序)投影成编排 UI 的节点诊断数据 —— 每条 = 一次 node_started/completed(activity input/result)/ branch_taken(case)/ signal_awaited·received(approval),loop 多轮按 `iterationKey` 区分;`?nodeId=X` 过滤到单节点;**只读、绝不碰运行引擎**(对位 08【CANON-X4】"不开第 4 条 SSE,运行时状态复用 notifications+eventlog,UI 重连从 trace 全量补")。`schedulerapp.GetTrace` + `TraceEntry`(镜像 M6 `ListFailures` 的 journal-read 模式);handler/route 挂上。TDD:单测 filter+seq-order+loop-iter+result-payload;HTTP E2E(trig→variable→completed)assert 200/seq 序/nodeId 过滤/未知节点空。`make matrix` 计入(`// covers: GET …/trace`)。顺带补 flowrun 域文档缺失的 failures/replay/approvals/trace 四端点行(M4/M6 域文档同步 gap)+ 清 `loop_body_test.go` 既有 ST1012。cross pipeline 绿、staticcheck clean。**这是 useFlowrunTicker 的后端半;前端 ticker 是后续任务。M8 observability 件 ✅。**|
| 2026-06-01 | **[feat]** workflow-revamp **M8 live runtime tick(端到端;08 CANON-X4)** —— 用户推翻"留 fresh session"判断,逐件做完。①**ephemeral 投递类(前置)**:`Bridge.PublishEphemeral`(+ domain Bridge 接口 + pkg Publisher,best-effort log-only)实时扇出但**不分配 seq、不入 replay buffer、永不阻塞**(`select{case ch<-env: default:}` 满则丢);envelope `Seq=0` 标记,notifications SSE handler 对 `Seq==0` **省 `id:` 行**(不移动 Last-Event-ID;durable 仍带 seq+重放)。这正是此前杀掉复用 `Publish` 的硬约束解(不驱逐 replay-ring、不背压引擎)。②**interpreter 集成**:`WithTick(fn)` + nil-safe `emitTick`;`activityRun` 在 node_started 后推 `running`、node_completed 后(及 replay copy-hit)推 `ok`、dispatch 错推 `failed`——回调是 walk 唯一新增,非阻塞、不改控制流。③**Service 接线**:`executeRun` 把 `.WithTick` 接到 `PublishEphemeral(detachedCtx,"flowrun",runID,{action:tick,workflowId,nodeId,status,iterationKey})`。④**前端**:现有 `useNotifications` flowrun 处理已 invalidate flowruns/flowrun(id)/flowrunNodes(id)→画布实时刷;唯一改:`action==="tick"` invalidate 但**不计未读**(ephemeral 非用户向)。TDD:bridge ephemeral(live-only/Seq0/无buffer/不耗seq)+ interpreter tick 序列(running→ok / running→failed)+ HTTP E2E(订阅→触发→断言 Seq0 tick)+ 前端(tick 不增未读、lifecycle 通知仍增)。新 cross seam `scheduler_notifications:runtime_tick` 注册 + `make matrix` 收录。events.md §11.2a(ephemeral delivery class)+ flowrun 域 + cross-cutting 同步。**M8 trace ✅ + tick ✅;剩 M7 sub-step replay + M5/M6 refinements。**|
| 2026-06-01 | **[feat]** workflow-revamp **M7 agent sub-step replay(ADR-010;replay-via-history,非改 loop 拷贝逻辑)** —— agent 节点 crash-replay 不再从头重跑已完成的 LLM turn + tool call。**刻意不重构共享 `loop.Run` 的拷贝命中逻辑**(那 loop 与 chat 共用,改 body 风险高——正是我此前标注的高风险路径),改用**历史重建 + 每步记账钩子**:①`loop.Run` 仅加一个可选 `StepRecorder`(type-asserted,chat host 不实现→对 chat 字节级不变),tool-step 完成后调 `RecordStep`;②interpreter 给每个 activity dispatch 传 `AgentSubStepJournal`(`DispatchInput.AgentSubSteps`,按 flowrunID/nodeID/iter/generation 作用域,仅 agent dispatcher 用),`agentHost.RecordStep`→journal `agent_step_completed`(Turn=绝对步序,Result=assistant+tool 结果);③`:replay` 时 `agentHost.LoadHistory` 把已记账步经 `loop.BlocksToAssistantLLM` 重建成 `[prompt, asst0, results0, …]` 历史前置,loop 从断点续——完成步是历史消息、**不重 dispatch**(工具副作用不重演);`LoadSteps` 按 turn 取最高 prior-gen(ADR-019),replay-of-replay 仍见每个早先 copy-hit 步;maxTurns 按已 replay 步扣减;中途未记账步重跑整步(at-least-once,工具须幂等)。TDD:loop 钩子每 tool-step 触发一次(非终态文本步)/ agentSubSteps 跨代 record→load 往返(JSON 往返 Block)/ merge-by-turn-highest-gen / LoadHistory 前置重建步 / HTTP E2E(fake-LLM agent 节点→trace 见 agent_step_completed)。新 cross seam `workflow_scheduler:agent_substep_replay` + matrix。17 §4(把假想 `loop.Run(replayed []AgentStep)` 改为实际机制)+ 02-agent-node 实现注同步。**M7 ✅;剩 M5/M6 refinements(最后一件)。**|
| 2026-06-01 | **[bug-fix]** scheduler `fakeRepo.Get` 返回**共享 `*FlowRun` 指针**的既有 latent data race —— run goroutine 锁内 `UpdateStatus` 写 `run.Status` 时,poller(如 `TestExecuteFn_PanicRecover_FinalizesFailed`)在锁外读返回指针的 `.Status` 字段,竞争。`make unit` 不带 `-race`、`make mock` 只跑 `./test/...` 不跑 internal 单测 → 此竞争一直没被门禁抓到;M7 收尾手动 `go test -race ./internal/app/scheduler/` 才暴露(非 M7 回归:该测试 override `ExecuteFn` 为纯 panic,我的代码不运行;且我未触碰 scheduler_test.go/dispatch.go)。根因修:`Get` 返回**副本**(`cp := *run`)——正确的 repo 语义(调用方 Get-then-Update,不经 Get 指针改)。`-race -count=3` 全 scheduler 包绿。|
| 2026-06-01 | **[feat]** workflow-revamp **M5/M6 refinement:cron-tick catchup 确定性(最后一件)** —— firing 的 `dedup_key` 原对所有 kind 都用 `wf|node|wall-clock-nanos`,导致 cron missed-tick **catch-up 补跑**同一刻度时不与已触发去重(wall-clock 各异 → 同一调度刻度跑两次)。修:`OnFireFunc(…, dedupKey string)` 让每个 listener 给出天然幂等键 —— **cron 按调度刻度**(`cronDedupKey`=`wf|node|cron|<tick.Truncate(分钟).Unix()>`;live 用 `now`、catch-up 用 missed `next`,都截断到分钟=ParseStandard 分辨率 → 同刻度的 live + catch-up 撞键);fsnotify/webhook 传 `""`(每事件/请求真独立 → app 回退 per-event wall-clock 键)。firing inbox 本就 `UNIQUE(wf,node,dedup_key)` + `AppendFiring` 幂等(冲突返已存行)→ tick-keyed cron firing 现去重 double-run。TDD:`cronDedupKey` 每分钟刻度确定、跨 tick/node/wf 不同;cron/fsnotify/webhook/app-trigger 全测绿、staticcheck clean。**polling cursor + overlap 不是 refinement 而是未来功能(§S20):无 polling listener(只 cron/fsnotify/webhook),`polling_states` 表/store 是 stopgap 但无触发器;cursor 预设了不存在的 listener,overlap-beyond-serial 是增强 —— 都留作真新功能(起点是 polling listener 本身)。** **revamp M0-M8 全 ✅;durable 执行引擎功能完整、崩溃可恢复、可观测、两轮深审 + TDD + 全 -race pipeline。**|
| 2026-06-01 | **[bug-fix]** chat cancel **取消对话时最终消息真丢失** + `make mock` 偶发挂 `TestChat_CancelDuringSecondLLMCall`。根因:eventlog emitter 的 block-row dual-write 不对称 —— `StopBlock` 的 `FinalizeStop` 故意用 `context.Background()` detach(§S9 注释在案),但 **`DeltaBlock` 的 `AppendDelta` + `saveBlockRow` 的 `SaveBlock` 用 caller(stream loop)ctx**。cancel 对话时,in-flight delta 的 DB 写用已取消 ctx 跑 → rollback → **生产:partial message 丢;测试(:memory: `SetMaxOpenConns(1)`):database/sql 把单连接判 bad 丢弃 → 之后全部"no such table"(含已 detach 的 SaveMessage/FinalizeStop/MaybeCompact)→ 偶发挂**。修:两个 streaming dual-write 改 `context.WithoutCancel(ctx)`(脱离 cancel、保留 user_id/conversation_id 值;比 Background 更安全)—— 持久化与 stream cancel 解耦(对齐 StopBlock 既有 §S9 模式)。TDD:cancelled ctx 下 delta 仍持久化(red→green);eventlog 全包 + chat cancel `-race×5` 绿、staticcheck clean。**注:chat 域 bug,排查 revamp mock flake 时发现,与 revamp 无关;finalize 写本就已 detach,唯一漏网是 streaming 写。**|
| 2026-06-01 | **[bug-fix]** `make mock` 偶发挂 `TestCatalog_IncludesFunctionAndHandlerItems`(超时类)。根因:每个 `th.New(t)` 用 per-test `t.TempDir()` 把 mise binary 重解压一遍(~67MB + codesign);`-race(~2x)-p1(串行)` 全套 16+ 次解压把 wall-clock 推向 10m suite-timeout,timeout 命中谁谁挂(故"偶发")。修(用户方向②,复用既有 `WithSandboxDataDir` 机制):harness 默认 honor `FORGIFY_TEST_SANDBOX_DIR`(显式 opt > env > per-test tempdir);`make mock` 设一个 per-run 共享临时 dir(跑完清理,保留退出码)→ **mise 全套只解压一次**。实测 `test/cross` **125s → 32s**、全套 5.4min,timeout 压力消失。per-test sandbox env 仍按 owner id 隔离(无 non-gated 测试做全局 env 列表,已核)。|
| 2026-06-01 | **[bug-fix]** 上条提速后暴露 `TestCompaction_FullCompactSummaryReachesNextTurn` 的 pre-existing async-race:compaction(`MaybeCompact`)是聊天轮次后**异步** goroutine 跑,但测试 `WaitForAssistantTerminal` 只等轮次、随即读 `conv.Summary`/archived 计数 → 读到 compaction 完成前的空态(日志:断言后几 ms 才 `compaction complete blocks_archived:2`)。修:断言前 **poll until archived>0**(10s deadline,真相是 DB 行数非计时器)。`DemoteOnly` 有多轮循环给异步留时间 + 断言 `archived==0`,无此 race,不动。修后 `make mock` 全套绿(32 包、0 fail、6.4min)。|
| 2026-06-01 | **[bug-fix]** 清 `app/handler` 两个既有 staticcheck 债:`call.go` `stderrRing.w` 未用字段(旧 index-based ring 残留,现 slice-based Write 不用)→ 删;`config_test.go` SA4006 `missing`(partially_configured case 算了不读、随即被下个 case 重赋)→ 按测试意图补断言 `missing==[dsn]`(与 case1/3 对称:填非必填 schema 后 dsn 仍缺)。`staticcheck ./internal/app/handler/` exit 0、`make unit` 全绿(200 包)。|
| 2026-06-01 | **[feat]** **生产级完整交付 — 全量 gap 修复**。审计后发现的所有缺口一次性修干净：**后端**：① `Approval.Deadline`/`TimeoutBehavior` 字段 + 解释器 park 时写 deadline + `expiry.go` 到期检查器(每 30s 扫 `approvals WHERE deadline<now`，先写 `signal_received(source=timeout)` journal 再翻 projection)；② `TriggerSchedule.LastFiredAt` 持久化：`ScheduleStore` port + `UpsertSchedule`/`GetSchedule`/`UpdateLastFiredAt` store 方法；`RegisterTrigger` 从 DB 种 `lastFire`；`onFire` 后 best-effort 持久化；`cron.RegisterWithLastFire` 接种；③ `state.go` 删 `buildNodeInput()` 永恒返 `{}` 的 stub；④ `interpreter.go` 三个完全重复的 event 提取函数合并为 `extractResultsByType` 工厂；⑤ approval config `timeoutSec` 类型断言由静默失败改为 switch + warn-log；⑥ `expiry.go` 持久化顺序修正：journal 先写(durable truth)才翻 projection(防 journal nil 时已写投影)。**前端**：① ExecuteOverview status filter key `waiting_approval`→`awaiting_signal`（filter predicate 兼容两个别名）；② "Webhook"/"available" 硬编码字符串改 `t()`；③ KpiStrip 死 `onOpen` prop 删；④ ApprovalBanner `onError` 加 `instanceof Error` guard。**testend**：① tsconfig 加 `@entities` 等 5 个路径别名(typecheck 恢复 0-error)；② ApprovalsQueue `status=paused`→`awaiting_signal`；③ FlowRunDetail approve endpoint + decision 值修正 + `onError` + 区分 loading/invalid-id；④ ApprovalsQueue 加 `isLoading`/`isError`/`JSON.stringify` try-catch；⑤ Profile/Memory/AsksPending 硬编码 queryKey→`qk.*`；⑥ FlowRuns/StatusBadge 加 `awaiting_signal`。所有门禁全绿：`make mock`(32 包 0 fail)、`make web`(998 test)、`tsc frontend+testend 0-error`、testend build clean。|
| 2026-06-01 | **[doc-fix]** 全量文档同步：`flowrun.md` 补 Approval 超时(Deadline/TimeoutBehavior/expiry checker)+ 错误码 6→7(+FLOWRUN_NOT_REPLAYABLE)；`trigger.md` 修正 "lastFire 内存" 误导说法 → 补 TriggerSchedule 持久化 + ScheduleStore port + RegisterWithLastFire；frontend `slices/flowrun.md` 补 `useFlowRunTrace` hook + `TraceEntry` 类型 + hook 计数 8→9；`entity-types.md` 补 TraceEntry 行；`page-execute.md` 补 trace inspector / Journal tab 数据流 + 状态过滤说明；`fsd-layers.md` 补 workflow-edit 5-kind palette 说明；`cross-cutting.md` 已在上轮 revamp session 更新(approvals invalidation/flowrun tick)，本次核查无新 gap。|
| 2026-06-01 | **[bug-fix]** **trigger 断连根治（二次审计发现：上条 "生产级完整交付" 实为过度声称）**。审计 1:1 对照 revamp doc 01/17 发现:`RegisterTrigger` **生产路径从无调用方**（仅测试调），`Activate` handler 只 `UpdateMeta(enabled=true)`、注释谎称 "listeners now registering" —— **cron/fsnotify/webhook/polling 对真实 workflow 全不触发**，boot 也不按 CANON-BOOT 重挂。修:workflowapp 加中性 `TriggerSync` port + `TriggerNodeInfo`（不引 triggerdomain）+ `extractTriggers`（兼容 `kind`/`triggerType` 双写法 + 嵌套/扁平 spec）+ `syncActiveTriggers` helper，挂进 **Create / AcceptPending / UpdateMeta(enabled 切换)** 三处生命周期；triggerapp.Service 实现该 port（`SyncWorkflowTriggers`，acyclic：workflow 永不引 trigger）；main.go + harness wire `SetTriggerSync` + boot 为每个 enabled workflow 重挂 listener（CANON-BOOT a）。测试:`extractTriggers` 5 单测 + `trigger_registration_pipeline_test.go` 2 e2e（create 自动注册 cron / deactivate 撤 + reactivate 复原）。`make mock` 全绿。|
| 2026-06-01 | **[feat]** **polling 触发器按契约完整重写（二审：上条一审 polling 字段名全错 + TypeError + 不可达）**。一审 stub 用 `callable`/`intervalSec`、调 `{"cursor"}`（签名是 `poll(lastCursor)`）、function domain 无 kind 字段。按 doc 01/17 重写：① `FunctionVersion` 加 `Kind`(normal\|polling, CHECK) + `PollingInterval`(duration 串)，version 级，GORM AutoMigrate 自动加列；② `apply.go` 加 `set_kind`/`set_polling_interval` op + draft 字段 + 编辑 carry-forward；③ Create/Edit 持久化 + `normalizeKind` + 新 `ActiveVersion()` 公共方法；④ `functionapp.PollingAdapter`(Interval 确认 kind=polling+解析间隔 / Poll 传 `{"lastCursor"}` 跑 `def poll`)；⑤ `infra/trigger/polling` 重写：`PollingFunction` port，Register 从 `spec.functionRef` 解析→按 PollingInterval 设 ticker，runPoll 每事件 onFire+去重 `cursor\|index`+推进 `polling_states.cursor`；⑥ `SetPollingFunction` 替 `SetPollingCallable`；⑦ create/edit_function 工具描述补 set_kind/set_polling_interval + 修正签名。测试：`polling_ops_test.go` 3 单测 + `polling_trigger_pipeline_test.go` 2 e2e（functionRef→kind=polling 注册成功 / normal 函数 register fail-soft）。`make mock` 全绿。**跟进**：accept-time capability_check 对 functionRef kind=polling 的前向校验（register-time 已 fail-soft 拒绝，accept 层是额外一层）。|
| 2026-06-01 | **[bug-fix]** 二审 "其余造假项" 三连修：① **useFlowrunTicker 删死代码**（一审建了 hook 谎称 "wired"，实零 import；实时画布其实早由 `useNotifications` invalidate `flowrunNodes` 驱动，hook 是冗余复制）→ 删 `frontend/src/app/sse/useFlowrunTicker.ts`，998 前端测试仍绿（证实真无消费者）；② **triggerNodeId 后端打通**（一审前端发了、后端 `FireManual` 解码后丢弃）：`StartRunOptions.TriggerNodeID`→`buildRun` 写 `run.TriggerNodeID`→interpreter `WithTriggerNode`+`selectTriggerNode`（指定/回落/stale 三态）→`StartRunFromNode` port + `FireManual(wf, nodeId, input)` + HTTP 正常/dryRun 两路传 `req.TriggerNodeID`；测试 `triggernode_test.go` 3 个；③ **timer gate 补测试 + 标注 loop 限制**：`timergate_test.go` 4 个（past-at 立即 fire / after 真等 / 无 config 不 arm / 重放不重等）；`activityRun` 注释明标 loop/parallel 体内（旧 subdag 路径）不识别 at/after = known no-op（非 silent）。`make mock` + `make web` + `make lint` 全绿。|
| 2026-06-01 | **[bug-fix]** 二审 (phase 4 逐文档核) 揪出两个**测试没覆盖到的真 bug**：① **agentRef 节点必炸**：`dispatch_agent.go` 用 `context.Background()` 调 `GetAgentConfig`，但 agent store `Get` 按 `RequireUserID`+`WHERE user_id=?` scope → 任何引用 Agent 实体的 agent 节点都报 "missing user id"。修：传 run ctx；回归测试 `dispatch_agent_test.go`（fake resolver 捕获 ctx 的 userID，改前 ""、改后 u_test）。② **timer gate 提前点火**：deadline 用 `RFC3339`（秒精度）存，`now+0.4s` 截断到整秒 → 落到 now 之前 → 不等待（按 now 小数位 flaky，秒级 timer 最多早 1s）。修：arm 存 + wait 解析全改 `RFC3339Nano`；`TestTimerGate_AfterWaitsThenFires` -count=2 稳定绿。③ **approval 超时字段名/类型错**：interpreter 读 `config.timeoutSec`(数字)，但 doc 05/17 §7 canon 是 `config.timeout`(duration 串如 "30d")→ 按契约写的 approval 永不超时。修：`approvalTimeout` helper 读 canon `timeout`(支持 Go duration + "Nd" 天，因 ParseDuration 无 day 单位)、`timeoutSec` 数字作兼容别名；测试 `TestApprovalTimeout_DurationStringCanon` 8 例。三 bug 均为一审"完整交付"遗留，二审测试逼出。|
| 2026-06-01 | **[bug-fix]** **agent 工具不可发现（用户实测：AI 说"不能直接创建 agent"，退而求其次建 agent 节点）**。6 个 agent forge 工具（create/search/get/edit/accept/delete_agent）已注册在 lazy 组 "agent"，但披露双缺：① 常驻 `toolsSection`（每轮都给 LLM 看）完全不提 agent；② `categoryLabels` 没有 "agent" 条目 → 能力索引渲染成裸 "agent (6)" 无描述。AI 不知道有 agent 工具组可激活，且误以为 agent 只是 workflow 节点。修：`toolsSection` 把 quadrinity 列全 `function/handler/workflow/agent` 并注明 "agent = 一等可复用 AI worker 实体，直接 forge，不只是 workflow 节点"；`categoryLabels["agent"]` 加描述。另修 create_agent 的 next_step 误导提示（引用了不存在的 run_agent 工具）。**这批工具此前零测试**（"为什么测试是这样"）——补 `chat/agent_disclosure_test.go`（守披露）+ `agentforge/tools_test.go`（create_agent 端到端建实体 + search/get 找到 + 拒 ag_ tool ref）。|
...
| 2026-04-24 | 文档结构三层分工：`backend-design.md`（规范） / `references/backend/`（索引） / `references/backend/domains/`（详设计） / `references/changelog.md`（进展） |
