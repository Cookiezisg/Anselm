...
audience: [human, ai]
landed-into: docs/references/backend/domains/ · docs/archive/multimodal-output/README.md（承接输入侧收口）· docs/working/app-hardening/ACCEPTANCE-GUIDE.md（验收清单已迁出）
---
...
- ASR 按官方时长口径入账：开始前按会话上限悲观 reserve，结束后按实际转发 PCM 时长向上取整 settle；
...
upstream delta/error→client、nonce refresh 与控制帧白名单路径；gateway 已接受 `cancel` 并只关闭本次 ASR 会话、不向上游转发未知控制事件；sidecar 与 gateway 都对 client/upstream 两条 WebSocket leg 发送 ping，并用 message/pong 滚动 30s 读 deadline，同时保留 2min 会话绝对上限；网关 ASR 会话已经接入同一 quota/ledger：按 `qwen3-asr-flash-realtime` 新加坡时长费率预留 120s，实际按成功转发的 PCM16/16k/mono 字节换算秒数 settle，无音频或拨号前失败 rollback，总音频字节超过预留上限会拒绝继续转发并返回稳定 `SPEECH_AUDIO_TOO_LONG`；网关新增显式付费 `make qwen-asr-evals` live 入口，有 `EVALS_ASR_WAV` 时要求真实转写文本，否则只做 protocol/connect smoke。跨进程 retry draft 恢复在 V1 已落地；未验收：真 key eval 尚未在本机/服务器实际执行。
...
> **2026-07-25 订正**:用户确认本节决策**已全部拍板**,且 C1→V2 的代码正是按这些默认执行的。此前仍标
> 「是否确认」的条目是**标记滞后于事实**,非未决——现统一改为「已确认」。此举只是让文档追上已发生的事,
> **不代表战役完成**:`landed-into` 与归档属于收口仪式,C 尚有余项、E/F 未动,故刻意不做。

| # | 建议默认 | 待确认 |
|---:|---|---|
| 1 | 主路由采用 §3.1：DeepSeek text、Qwen3.7 visual、Omni perception | 已确认 |
| 2 | Qwen 是唯一视觉主 route，无 fallback | 已确认 |
| 3 | text/visual 主 Agent 均发布真实 1M；Omni 明示 64K 感知器 | 已确认 |
| 4 | Anselm Auto 用网关 profile；外部模型自然撞墙透明恢复并学习安全预算，高置信后才主动治理 | 已确认 |
| 5 | 原件长期只在本地；公网只放 proxy/片段短租约 | 已确认 |
| 6 | 图片默认长边约 2048；文字截图优先无损 | 已确认 |
| 7 | 长视频默认关键帧+音轨+按需片段，不整段反复送 | 已确认 |
| 8 | 麦克风主按钮=听写，停止后可编辑，不自动发送 | 已确认 |
| 9 | 原始音频通过附件/“录制为音频附件”，不靠长按麦克风 | 已确认 |
| 10 | 本战役不做语音输出；消息朗读/TTS 后续独立排期 | 已确认 |
| 11 | 免费档媒体使用新加坡 private object storage 短租约 | 已确认 |
