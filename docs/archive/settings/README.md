...
- **S5 沙箱面板 ✅(2026-07-09)**:`core/contract/sandbox.dart` 四 DTO(Runtime/RuntimeAvailability[pinned→下拉/否则自由输入]/Env[五 owner]/Bootstrap)+repo 面 10 方法+`sandbox_providers`(runtimes AsyncNotifier 变更即重取/envs per-owner family/bootstrap+available FutureProvider);`SandboxPanel`——引导健康门(ok=false 红卡+重试)/磁盘 AnMeter/运行时(全机域徽+装[推入表单:kind 下拉+version pinned 下拉或自由]/删[409 `SANDBOX_ENV_IN_USE` 诚实提示])/**五 owner 环境 tab**(function|handler|mcp|skill|conversation;状态点+deps 数+尺寸+runningPid>0 标运行中+删[「下次执行自动重建」])/GC(N 天 AnInput+回收 toast{removed};红框「立即全部回收」=0 天两步)。s5_sandbox 电池 4 测(引导失败+重试/运行时装删+409/环境 tab 状态点/GC toast);**真机全脸**:真沙箱磁盘 520.6MB+真运行时列表(embedmodel/llamasrv/node 22 真尺寸)+五 owner tab+域徽。i18n sandbox 块双语。settings_shell placeholder 测改指向仍未建的 shortcuts。

- **S6 快捷键+收尾 ✅(2026-07-09,拍板 #4,模块收官)**:**全局命令收编表**——`core/shortcuts/shortcut_catalog.dart`(`ShortcutCommand` 枚举 6 命令[toggleLeft/Right·openSettings·zoomIn/Out/Reset]+`ShortcutChord` 平台归一[cmd=mac ⌘/其余 Ctrl,`toActivator()` 映 meta·control]+稳定序列化[用 `keyId` 数字、绝不用 label]+人读 `display`[⌘⇧B]+`kShortcutDefaults` **唯一声明处**);`shortcut_bindings.dart`(`ShortcutBindings` Notifier:默认表叠加用户覆写[单 JSON 存 `an.shortcuts`,**只存非默认覆写**→resetAll 干净回落]+`conflictFor(排除自身)`+rebind/reset/resetAll);**根生成而非内联**——`core/shortcuts/global_shortcuts.dart`(`GlobalShortcuts` ConsumerWidget)watch `shortcutBindingsProvider` **从目录生成 CallbackShortcuts**,由 `app.dart` 挂在 **app 根、autofocus Focus 之上**(zoom 绑定从 `app.dart` 内联搬来收编,散落 SingleActivator 清零→「catalog==已绑」门成立、改绑=一次 map 编辑;handler 全为纯 provider/静态调用故无需壳 context);`settings_prefs` 加 `an.shortcuts` 键、`settings_catalog` 该面板 `ownedKeys` 补它(三相等门)。**面板**——`ShortcutsPanel` 每命令一行(当前键帽[`AnText.mono`]+`Focus.onKeyEvent` 捕获下一组合键[须带修饰键否则红字提示/与别命令冲突则拒并说明是谁/Esc 取消]+modified 竖条+单项重置+全机域徽+全部重置);录制态蓝框「Press a new chord…」。i18n shortcuts 块双语。s6 电池 3 测(每命令渲键帽/录入即改绑/**回归:录制完成后本行交还键盘**)+`global_shortcuts` 电池 2 测(冷启动 autofocus 无点击即触发/改绑键即时换默认键);**真机全链**(release+真 sidecar):6 键帽正确渲(⌘B/⌘\/⌘,/⌘=/⌘−/⌘0)→点键帽入录制蓝框→送 ⌘J 改绑→键帽变 ⌘J+现单项重置→**重启后 ⌘J 持久**→**热生效实证**:⌘J 收/展左岛(截图)、旧默认 ⌘B 无效→全部重置回 ⌘B→**冷启动无点击 ⌘B 即收/展左岛(截图)**。**真机修出两处真 bug**:①`_ShortcutRow._onKey` 无 `_recording` 守卫且录完不 `unfocus`→本行录后仍持焦点,吞掉后续每次组合键(静默把命令再改绑成杂散键)、饿死壳全局快捷键(真机首验即抓:改绑后 ⌘J 不生效);修=`_onKey` 非录制态返 `ignored`+`_stop()` 交还焦点,回归测钉入(录完送异键、绑定须不变)。②**焦点序回归(S6 引入)**——CallbackShortcuts 曾放 `app_shell` 内,落在 `app.dart` autofocus Focus **之下**;而 CallbackShortcuts 只对「从持焦点子孙冒泡上来的按键」触发,故冷启动全局键全被饿死、要先点一下壳才活;修=抽 `GlobalShortcuts` 挂 app 根 autofocus **之上**(恢复 S6 前正确层序,同时保留目录驱动),回归测钉入(autofocus 无点击即触发)。fe-verify 3312 全绿,analyze 净。
