...
audience: [human, ai]
landed-into: docs/references/frontend/overview.md
---
...
| **Notifications(通知账本 + 顶带舞台)** | ✅ | 左岛铃托盘保全量持久事件，顶带是全 app 唯一即时出口：后台事件与操作反馈共用一张舞台，当前卡恒定居中，候场=`• / • • / • • +N`，单条 ✕ 与 `+N→✕` 快照清场均不碰左岛账本；私有双 `ListQueue` 无 cap、UI 恒看 current+两 cue+计数，审批优先候场但不硬切当前；未聚焦仍走 OS。右上 toast 展示层物理退役，overlay 仅留 confirm | [`features/notifications`](../../references/frontend/features/notifications.md) · 建造史 [`WRK-058`](../../archive/notifications/README.md) / [`WRK-074`](../../archive/notice-stage/README.md) |
...
| [`WRK-074`](../../archive/notice-stage/README.md) | 顶带唯一即时消息舞台(共享 NoticeCenter + 两点/+N 候场 + 快照清场 + 操作反馈收口) |
