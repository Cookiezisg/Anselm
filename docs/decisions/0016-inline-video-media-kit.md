...
`AttachmentAudioDriver` 这层缝早就在(注释写明存在理由就是换后端),`media_kit` 也确实能同时做音视频。
但**证据只覆盖 macOS**,而音频**今天在三平台都好好的**。

拿一个能用的功能去赌一个只在一个平台上验过的原生栈,换来的只是「少维护一套」——不划算。缝的意义是让这次
交换**随时可以廉价地做**,不是让我们现在就必须做。

## 影响 / Consequences

- **原生依赖进来了**:`media_kit_libs_video` 每平台分发 libmpv。安装包变大;**Linux 构建要 libmpv 开发包**。
  这是本决策买下的代价,写在这里以免将来有人以为它是免费的。
...
| 同批把音频也换到 media_kit | 证据只覆盖 macOS,而音频三平台都好好的;缝的意义是让交换随时廉价,不是现在必须做 |
