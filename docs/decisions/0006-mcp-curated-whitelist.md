...
经一次系统调研（每 server 一个 agent 调查 + 2 轮对抗 cross-check，共 297 个 agent；结论见 [`working/mcp-oauth-support`](../working/mcp-oauth-support/README.md)）按一条判据分类：
...
- **档 2（后续）= 10 个 oauth-dcr**：待新建 MCP OAuth 2.1 + PKCE + DCR 客户端（桌面 loopback 回调 + 经 Flutter 通道拉浏览器 + token 加密存储/自动刷新，规格见 working/mcp-oauth-support §1–§5）后再纳入白名单。
