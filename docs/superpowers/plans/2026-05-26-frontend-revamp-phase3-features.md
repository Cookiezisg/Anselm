...
## 阶段 3 完成说明(2026-05-26)

**8 feature 全部抽取完毕**:send-message / onboarding / forge-iterate / forge-review / workflow-edit / settings / ask-user / entity-link。每个 feature 均有 `model/use<X>.ts` + `index.ts`。

**feature→store 豁免清单(留阶段4)**:

| feature | hook | 豁免 import |
|---|---|---|
| send-message | useSendMessageFlow | store/ui (pushToast, setActiveConv) |
| onboarding | useOnboardingFlow | store/ui (pushToast) + store/settings (set) + components/overlays/onboarding-strings (ACCENTS, PROVIDER_DEFAULT_MODEL) |
| forge-iterate | useForgeIterate | store/ui (pushToast, setActiveConv) |
| forge-review | useForgeReview | store/ui (pushToast) |
| forge-review | useForgeBatchDelete | store/ui (pushToast) |
| workflow-edit | useWorkflowEdit | store/ui (pushToast) |
| settings | useAccountManager | store/settings (useSettings) + store/ui (pushToast) |
| ask-user | useAskUserAnswer | store/ui (pushToast, pendingAsk, close) |

所有豁免均有 `// TODO(阶段4)` 标记。`npx eslint src/features` = 0 error。

**forge.js iterate 残壳已清**:阶段2 遗留的 `useIterateForge` 实现已在 Task 3.3 搬进 `features/forge-iterate/model`。

**组件原位瘦身,迁目录留阶段4**:所有组件仍在 `panes/`/`components/` 原位,onClick/onSubmit 只调 feature hook 意图级 API。

**残留业务清单(非错误,已知合理)**:
- `ChatPane.jsx` line 43-48:convError 上来自愈 effect(GET 对话本身 404 → setActiveConv(null) + invalidate)。该 effect 依赖 `convError`(来自 `useConversation` query),早于用户发送触发,留组件使 hook 无需感知 query 状态。行为不变优先,留阶段4 统一身份/资源自愈层。
- `WorkflowDetail.jsx` line 30+99:`pushToast` 直接在组件调用用于 onDeploy stub(`() => pushToast(...)`)。deploy 功能尚未实现,属于临时 UI stub 非编排,留阶段4 deploy feature 建立时一并抽取。

**全量验证结果**:tsc 0 / vitest 752 passed / build ✓ / make lint-frontend 0 error (47 pre-existing warnings) / make dev 冒烟:backend :8742 健康 + Vite :5173 HTML 正常,无 feature import 解析错误。
