...
- ✅ **STEP 5.5 壳 chrome 对齐 demo**（用户复审追加,2 段调研 workflow `w27smg8so` 深读 demo + 4 篇 best-practice）——**已落**:
  ...
  - **顶控对齐**:chrome bar 与浮层头同高 `islandHead`(44)、同带;**左岛去顶 pad**(`AnIsland` top:0)使 chrome 抵岛顶 → 收起钮与 OS 红绿灯居中对齐(真机截图核对修正,研究预判的 32↔52pt 坑);红绿灯仍 OS 画(macos_window_utils,不抄 demo 假点)。⌘B 切左 / ⌘\ 切右(`app_shell.dart` `CallbackShortcuts`,meta+control 双绑)。
  ...
  - **Gate 达成**:`make verify` **828 测绿**(新增 shell_chrome provider 测 4 + OceanBreadcrumb widget 测 1 + AnShell 收起/panel-right widget 测 2;既有 AnShell 测补 `TranslationProvider`)+ **真机截图**(`shot_app_real.sh` 真窗口核对红绿灯 + 收起钮对齐)。
