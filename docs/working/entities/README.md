...
- ✅ **STEP 5 执行 + 右岛 run-terminal** — **已落**（2 决策已拍板:① agent 轨迹 = **完整 block-tree**[非 flat 转录]、② 入参表单 = **类型化逐字段**;第二轮契约调研 workflow `woh8w8dwz`[5 后端契约 + 2 前端现状 + best-practice]+ Explore[messages 块契约],字节级折入)。
  ...
  - **state**（`features/entities/state/run/`）:`RunTerminalController`（`Notifier<RunTerminalState>`）接 4 execute 方法 + 先订 `panelSignals(scope)` 再发执行(捕获执行期帧)、按 kind 分流(fn/hd run-node delta→文本 / agent 块帧→`BlockTreeReducer` / wf tick→live 节点 + 触发后 `getFlowrun` 取 durable 节点);高频 body 在 `CoalescingNotifier<RunStream>`(L2,每帧≤1 重画、不搅生命周期态);cancel = seq-guard 丢弃陈旧结果(同步动词后端续跑)。
  ...
  - **接线**:`AnOceanHeader` 动词 CTA → `runTerminalProvider.openFor(ref)`(原禁用占位激活);`AppShell` 转 `ConsumerWidget`、`inspectorOpen` watch `runTerminalProvider.open`、`inspector: AnInspector(headless, child: RunTerminal())`(右岛揭示/收起)。
