...
## 当前前线（2026-08-02 03:47）

第六批已完成 **50 / 50**，中央账本 `350 judgments`，锚点校准有效，警报复审后 clean。`TOOL-055 edit_approval`、
`TOOL-056 revert_approval`、`TOOL-057 delete_approval`、`TOOL-058 search_workflow`、`TOOL-059 get_workflow` 与
`TOOL-060 create_workflow` 已完成 formal 红路径、修复和正负五通道重跑；统一长门禁和一次性提交正在执行，下一前线为 `TOOL-061 edit_workflow`，门禁完成前不启动。
...
`TOOL-060` 的最终语义是显式保留 metadata，且在执行边界窄兼容 hosted model 的精确 stringified tags/ops；`ValidateInput` 在 mutation 前要求 `description`、`tags`、`changeReason` 三键出现，空值仅允许 `""`/`[]`，formal-132 只执行一次 mutation，REST/SQLite/SSE/UI/wire 一致。随后以无 App 的真实后端 cleanup session 清掉遗留 acceptance conversation（DELETE=204→GET=404），SQLite 仅留契约要求的最后 workspace。第六批已达到 50 格，现在运行统一长门禁；门禁通过前不提交、不进入 TOOL-061。
