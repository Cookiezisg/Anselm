...
## 当前前线覆盖声明（2026-08-19 01:49）

`SURF-037 entities/rail-workflow` 已在真实 App session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-011009` 完成：真实 workflow rail 从 inactive 激活到 `active / Engaged: Listening`，折叠/重开后路由与状态保持，Manual trigger 真实输入后右岛显示 `Done`、`fr_0c9afd489615f725`、`last ok 164ms`，四个节点均 `completed`、`No alerts`。首轮红 session=`20260819-005343` 暴露 fixture 把 approval 输入错接到 `route.amount`；停线修正为 `work.amount`/`work.message`，新 workspace、新 SQLite、新 binary 重跑通过，红证据不入绿账。录屏 `523.665000s / 2560x1584 / H.264 / 60fps`，backend `673` 行无应用红线，frontend 已复核 AXTree tooling churn 且无应用红线，ssetap 三流连接、entities `run_started→run_terminal(completed)` 无 gap，llmtap managed proof/install/models 全 `200`，SQLite integrity=`ok`。证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-011009/evidence/SURF-037-entities-rail-workflow-five-channel.md`，最终帧=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-011009/evidence/SURF-037-final.png`，五格=`G1/F1/B2/C4/G1`，警报复审=`/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-037-ledger-alarm-reaudit.md`，最终 `alarms.py check`=`clean (2135 judgments)`；批次四十由 `40→50/50`，统一长门禁和提交已解锁；下一原子前线为 `SURF-038 entities/rail-control`。P12 400+ Journey 继续按用户裁定推迟二期。

EP-230–EP-251、EP-220 及既有绿色项保持五格绿；EP-252–EP-257 debug-only 五格为 L1–L3 绿、L4/L5 按边界记 `na`，其余已列 SURF 面（含 SURF-003、SURF-028–SURF-037）均完成五级裁决。批次三十六、三十七、三十八、三十九、四十已 `50/50` 收口并完成各自统一门禁；正式 ledger=`2135 judgments`，COVERAGE=`848 rows / 420 judged rows / 0 tombstones`，anchors=`10/10`，alarms=`clean`。根 `make verify`、backend 非缓存全量 Go、台架 `42/42` 和黑盒 testend 两次稳定复跑均通过；首轮黑盒红保留在 LOG，不调整门禁或源码来修绿。当前只剩工作树归属审计、staged diff 复核和提交；通过后继续 `SURF-038`。
