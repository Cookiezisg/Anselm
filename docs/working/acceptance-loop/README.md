...
## 当前精确状态（2026-08-31 · EDGE-241 master key 边界完成；EDGE-236/242 强制交互后置；批次 87 已达 77/50）
...
### 5.2 Day 0 当前状态(整体重述,2026-08-31 EDGE-240 五格收口、EDGE-236 后置；批次 87 73/50)

当前唯一权威状态以 `COVERAGE.md`、`ledger-sequence.json`、`judgments.jsonl` 及脚本实时重算为准：清册 `848` 行，`736` 行五级结算，`112` 行仍开放；`3872` 个单元已结算、`368` 个单元开放；`manual_queue=173`，其中实际需要用户强制交互的 `forced_queue=26`。`EDGE-240|ADD COLUMN 结果幂等` 已完成 `L2=F1/L3=na/L4=na/L5=na`：真实 App 从缺少分叉血缘/驻地列的旧 SQLite 安装启动，首次 boot 补齐三列及依赖索引，第二次启动重复执行 ADD COLUMN 仍正常；原有 conversation、索引、`integrity_check` 与 `foreign_key_check` 均正确，真实 App 两次稳定进入 Chat，无空白或卡死。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-165453`，正式证据=`testend/rig/formal-evidence/EDGE-240-add-column-idempotency-real-app-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-240-ledger-alarm-reaudit-real-app-20260831.md`。L3-L5 的 `na` 是适用性边界，不是降低标准：该后台迁移没有独立用户动作、视觉对象或可发现入口。顺序门已跳过需要真实父进程 `kill -9` 和强制交互的 `EDGE-236|父进程死人开关`，当前自主前线为 `EDGE-241|换 master key 种子`；该强制项正式留在人工尾队，不阻塞自主推进。批次 87 已由 `69` 推进至 `73/50`，本格前置的统一长门禁已通过。强制队列项目继续不写绿格、不降低标准；人工后置不是 waiver，自动回归不得冒充真实 App 产品证据。P12 的 400+ Journey 扩写按用户裁定推迟二期。
