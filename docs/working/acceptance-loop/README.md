...
- 新语义扫描当前清册后，仍有 `93` 个自主未收口项目行（`298` 个未收口单元）；人工队列中的 `EDGE-031|回合收尾期单槽缓冲`、`EDGE-030|生成中再 Send`、`EDGE-033|关页不留 streaming 孤儿`、`EDGE-037|归档对话发消息自动解档`、`EDGE-038|:retry 重生成分支`、`EDGE-039|:retry 编辑重发分支`、`EDGE-251|删最后一个 workspace`、`EDGE-254|keyset 排序切换丢游标`、`EDGE-256|驻地目录被移走`、`EDGE-257|脏区切分支被拒`、`EDGE-258|新建分支不受脏区门` 与 `EDGE-329|快捷键录制后吞键` 继续后置。此前尝试越过 `EDGE-251` 直接写 EDGE-040 被正式顺序门拒绝；`EDGE-253|单连接 panic 事务砖化` 与 `EDGE-255|PageAsc collation 不一致` 已完成普通/race 回归及四格适用性复核，`EDGE-254` 的 L2 已完成契约验证、L3-L5 留在人工队列，`EDGE-256/257/258` 保留完整 L2-L5 现场验收，下一自动前线待顺序门重新计算。
...
- `EDGE-257|脏区切分支被拒` 与 `EDGE-258|新建分支不受脏区门` 的 conversation service/HTTP 普通与 race 回归通过；两项的拒绝/放行文案、用户现场反馈、视觉和发现性仍完整后置。`gen_coverage.py` 已同步 `848` 行，下一自动前线待顺序门重新计算。
