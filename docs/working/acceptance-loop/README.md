...
- `EDGE-266|空线程/重复 PATCH 不落 marker` 的 conversation service 普通/race 与真实黑盒 `TestChatWorkDir_MidThreadSwitchLeavesADurableMarker` 普通/race 回归通过；空线程首次挂载与同路径重复 PATCH 均无 marker、无生命周期动作，后续真实切换仍正常落 marker。无痕 no-op 的真实 App 反馈、视觉和发现性仍需五通道现场观察，故整项进入人工后置队列，不作 `na` 降级。独立复核=`testend/rig/formal-evidence/EDGE-266-ledger-alarm-reaudit-20260830.md`；下一自动前线由顺序门重新计算。
