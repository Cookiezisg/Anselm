...
## 当前精确状态（2026-08-31 · EDGE-235 关停预算格完成；EDGE-236 强制交互后置；批次 87 已达 60/50）
...
> **本段是当前权威快照，覆盖下方旧条目中的 next 指针和历史计数；历史快照不回写。**
 ...
 - Edge210、Edge234 与 Edge235 的连续写账均触发 `gap-too-fast`/`discovery-collapse`；每次均用真实证据独立复审后逐条 ack，未改阈值、曲线算法、CODEX、锚点、五级标准或顺序门。当前 `RIG_HOME=/private/tmp/anselm-rig-formal-20260831-11 python3 testend/rig/alarms.py check`=`clean (113 live judgments; 4240 baseline judgments excluded from drift curves)`，锚点=`10/10`，`gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`。
 - 权威清册重算为 `848` 行：`732` 行五级已结算、`116` 行仍开放；`3856` 个单元已结算、`384` 个单元开放。`manual_queue=173`、`forced_queue=26`；`EDGE|父进程死人开关` 已正式提升为 forced 项，当前顺序门跳过它并推进下一个无需用户操作的自主前线。P12 的 400+ Journey 扩写按用户裁定推迟二期。
 - 批次 87 已由 `8/50` 推进至 `60/50`；已达到统一长门禁触发点，长门禁通过前不提交。强制队列项目继续不写绿格、不降低标准，人工后置不是 waiver，不以 `na`、本地 mock 或旧 session 冒充真实 App 产品证据。
...
### 5.2 Day 0 当前状态(整体重述,2026-08-31 EDGE-235 L2-L4 收口、EDGE-236 后置；批次 87 60/50)

当前唯一权威状态以 `COVERAGE.md`、`ledger-sequence.json`、`judgments.jsonl` 及脚本实时重算为准：清册 `848` 行，`732` 行五级结算，`116` 行仍开放；`3856` 个单元已结算、`384` 个单元开放；`manual_queue=173`，其中实际需要用户强制交互的 `forced_queue=26`。`EDGE-235|关停预算格` 已完成 `L2=F2/L3=A4/L4=C4/L5=na`，真实 App 停修复验、三路 SSE、backend、frontend console、LLM tap、录屏与 SQLite 真相均绑定正式 session；正式证据=`testend/rig/formal-evidence/EDGE-235-shutdown-budget-real-app-fixed-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-235-ledger-alarm-reaudit-real-app-20260831.md`。顺序门已跳过 forced 的 `EDGE-236|父进程死人开关`，当前自主前线为 `EDGE-237|坏 settings.json`；前者需要真实父进程 `kill -9` 和强制交互，按用户授权留在人工尾队，不阻塞先推进后者。上一批次 85 已完成 `50/50`，批次 86 已完成 `52/50`，当前批次 87 为 `60/50`，执行统一长门禁前不提交。强制队列项目继续不写绿格、不降低标准；人工后置不是 waiver，自动回归不得冒充现场证据。P12 的 400+ Journey 扩写按用户裁定推迟二期。
