...
## 当前精确状态（2026-08-31 · EDGE-240 ADD COLUMN 结果幂等完成；EDGE-236 强制交互后置；批次 87 已达 73/50）
...
> **本段是当前权威快照，覆盖下方旧条目中的 next 指针和历史计数；历史快照不回写。**
 ...
 - 权威清册重算为 `848` 行：`736` 行五级已结算、`112` 行仍开放；`3872` 个单元已结算、`368` 个单元开放。`manual_queue=173`、`forced_queue=26`；`EDGE|父进程死人开关` 已正式提升为 forced 项，当前顺序门跳过它并推进下一个无需用户操作的自主前线。P12 的 400+ Journey 扩写按用户裁定推迟二期。
 - 批次 87 已由 `8/50` 推进至 `73/50`；统一长门禁已在 `69/50` 完成，本格完成后仍保持原门禁结果有效。强制队列项目继续不写绿格、不降低标准，人工后置不是 waiver，不以 `na`、本地 mock 或旧 session 冒充真实 App 产品证据。
