...
## 当前精确状态（2026-08-31 · EDGE-239 CHECK 加词整表重建完成；EDGE-236 强制交互后置；批次 87 已达 69/50）
...
> **本段是当前权威快照，覆盖下方旧条目中的 next 指针和历史计数；历史快照不回写。**
 ...
 - Edge210、Edge234、Edge235 与 Edge239 的连续写账均触发 `gap-too-fast`/`discovery-collapse`；每次均用真实证据独立复审后逐条 ack，未改阈值、曲线算法、CODEX、锚点、五级标准或顺序门。当前 `RIG_HOME=/private/tmp/anselm-rig-formal-20260831-11 python3 testend/rig/alarms.py check`=`clean (125 live judgments; 4240 baseline judgments excluded from drift curves)`，锚点=`10/10`，`gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`。
 - 权威清册重算为 `848` 行：`735` 行五级已结算、`113` 行仍开放；`3868` 个单元已结算、`372` 个单元开放。`manual_queue=173`、`forced_queue=26`；`EDGE|父进程死人开关` 已正式提升为 forced 项，当前顺序门跳过它并推进下一个无需用户操作的自主前线。P12 的 400+ Journey 扩写按用户裁定推迟二期。
 - 批次 87 已由 `8/50` 推进至 `69/50`；统一长门禁已达到触发点，当前正在为 `EDGE-239` 做收口审计。强制队列项目继续不写绿格、不降低标准，人工后置不是 waiver，不以 `na`、本地 mock 或旧 session 冒充真实 App 产品证据。
...
### 5.2 Day 0 当前状态(整体重述,2026-08-31 EDGE-239 五格收口、EDGE-236 后置；批次 87 69/50)

当前唯一权威状态以 `COVERAGE.md`、`ledger-sequence.json`、`judgments.jsonl` 及脚本实时重算为准：清册 `848` 行，`735` 行五级结算，`113` 行仍开放；`3868` 个单元已结算、`372` 个单元开放；`manual_queue=173`，其中实际需要用户强制交互的 `forced_queue=26`。`EDGE-239|CHECK 加词整表重建` 已完成 `L2=F1/L3=na/L4=na/L5=na`：真实 App 从旧 schema SQLite 安装启动，boot migration 恢复三处 CHECK marker，行数、索引、`integrity_check` 与 `foreign_key_check` 均正确；真实 App 的 startup spinner 后稳定进入 Storage & logs/Chat，无空白或卡死。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-162419`，正式证据=`testend/rig/formal-evidence/EDGE-239-check-rebuild-migration-real-app-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-239-ledger-alarm-reaudit-real-app-20260831.md`。L3-L5 的 `na` 是适用性边界，不是降低标准：该后台迁移没有独立用户动作、视觉对象或可发现入口。顺序门已跳过需要真实父进程 `kill -9` 和强制交互的 `EDGE-236|父进程死人开关`，当前自主前线为 `EDGE-240|ADD COLUMN 结果幂等`；该强制项正式留在人工尾队，不阻塞自主推进。批次 87 已由 `65` 推进至 `69/50`，正在执行本批次统一长门禁与工作树审计。强制队列项目继续不写绿格、不降低标准；人工后置不是 waiver，自动回归不得冒充真实 App 产品证据。P12 的 400+ Journey 扩写按用户裁定推迟二期。
