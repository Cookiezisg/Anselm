...
- **最新精确快照（EDGE-289 调度后，按修复后的 judge.py 重算）**：`848` 行、`41` 个人工后置项、`111` 个未收口行；自主队列剩余 `70` 行、`206` 个未收口单元，下一自动前线为 `EDGE-222|生成 origin 从凭证派生`。本轮修复了 `L#:measure:...→note:` provisional 记录曾被误当 settled 的 gate 漏检；此前的 `62/176` 快照与现行 provisional-`na` 判定不一致，已纠正；以本条、`judge.py` 和 `ledger-sequence.json` 为准。
...
- 本轮 gate 修复证据=`testend/rig/formal-evidence/WRK-087-judge-measure-provisional-20260830.md`；`testend/rig/test_judge.py` 普通回归 20/20 通过，measure 指针后的 provisional note 现在会重新打开顺序门，明确适用的 `na` 仍保持 settled。
