...
- **最新精确快照（EDGE-288 调度后）**：`848` 行、`40` 个人工后置项、`102` 个未收口行；自主队列剩余 `62` 行、`176` 个未收口单元，下一自动前线为 `EDGE-289|@ 一个 fork skill`。本节下方较早的长快照是调度前历史记录，以本条和 `ledger-sequence.json` 为准。
...
- `EDGE-288|fork skill 无 runner` 的 skill service 与激活 HTTP 合同普通/race 回归通过；缺 runner、缺 agent 和非法 agent 均明确拒绝，但真实 App 的不可用反馈、文案、视觉和发现性仍后置。独立复核=`testend/rig/formal-evidence/EDGE-288-ledger-alarm-reaudit-20260830.md`；下一自动前线为 `EDGE-289|@ 一个 fork skill`。
