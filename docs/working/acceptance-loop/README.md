...
### 5.2 Day 0 当前状态(整体重述,2026-08-19 01:49 批次四十已统一收口待提交)

`SURF-037 entities/rail-workflow` 已在全新数据目录和真实 managed gateway 上完成正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-011009`，workspace=`ws_244fe36c98fbe921`。真实 App 从 Workflow rail 进入、激活为 `active / Engaged: Listening`、折叠再展开，随后在右岛 Manual trigger 输入真实 JSON 并点击 `Trigger`；最终显示 `Done`、flowrun=`fr_0c9afd489615f725`、`last ok 164ms`、四个节点全部 `completed`、`No alerts`。首轮红 session=`20260819-005343` 暴露 fixture 把 approval 输入错接到 `route.amount`；已停线修正为真实上游 `work.amount`/`work.message`，新 workspace、新 SQLite、新 binary 重跑通过，红证据保留且不入绿账。

五通道封口：`screen.mov`=`523.665000s / 2560x1584 / H.264 / 60fps`；backend `673` 行无应用级 WARN/ERROR/panic/FATAL/stack trace；frontend `161` 行无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception 应用红线，已知 AXTree tooling churn 由同 session `frontend-ax-review.md` 明确复核；ssetap 独立连接 notifications/messages/entities 三流，entities durable `7..10` 且 `run_started → run_terminal(completed)`，无 gap；llmtap 的 managed proof/install/models 全 `200`，无伪造 completion；REST、SQLite `PRAGMA integrity_check=ok`、SSE terminal 和最终 UI 对账一致。正式证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-011009/evidence/SURF-037-entities-rail-workflow-five-channel.md`，最终帧=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-011009/evidence/SURF-037-final.png`；五格按 `G1/F1/B2/C4/G1` 写入。警报复审=`/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-037-ledger-alarm-reaudit.md`，`gap-too-fast` 与 `discovery-collapse` 已按原阈值独立复审并 ack，最终 `alarms.py check`=`clean (2135 judgments)`。

EP-230–EP-251、EP-220 及既有绿色项保持五格绿；EP-252–EP-257 debug-only 五格为 L1–L3 绿、L4/L5 按边界记 `na`，其余已列 SURF 面（含 SURF-003、SURF-028–SURF-037）均完成五级裁决。批次三十六、三十七、三十八、三十九、四十均已完成统一门禁并收口；批次四十为 `50/50`，正式 ledger=`2135 judgments`，COVERAGE=`848 rows / 420 judged rows / 0 tombstones`，anchors=`10/10`，alarms=`clean`。根 `make verify`、backend 非缓存全量 Go、台架 `42/42` 和黑盒 testend 稳定复跑均通过；首轮黑盒红已原样记入 LOG，未改门禁阈值或源码来修绿。当前只剩本批次工作树归属审计、staged diff 复核和提交；提交后下一原子前线为 `SURF-038 entities/rail-control`。P12 的 400+ Journey 继续按用户裁定推迟二期，一期仍以 COVERAGE 为覆盖真相源。
