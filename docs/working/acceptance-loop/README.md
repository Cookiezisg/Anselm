...
## 当前精确状态（2026-08-31 · EDGE-213 未开通读配额完成；批次 86 已封存，批次 87 开始）
 ...
 - 权威清册重算为 `848` 行：`719` 行五级已结算、`129` 行仍开放；`3804` 个单元已结算、`436` 个单元开放。`manual_queue=173`、`forced_queue=24`；其中 `105` 行属于当前可自主推进的普通前线，下一项为 `EDGE-214|开通降级不挂 boot`，另有 `24` 行保留在强制人工队列。P12 的 400+ Journey 扩写按用户裁定推迟二期。
 - 上一批次 85 已满 `50/50`，统一长门禁已通过并封存于 `testend/rig/formal-evidence/batch-85-unified-gate-20260831.md`；批次 86 已由 `0/50` 推进至 `52/50`，统一长门禁已通过并封存于 `testend/rig/formal-evidence/batch-86-unified-gate-20260831.md`，下一批次从 `0/50` 开始。强制队列项目继续不写绿格、不降低标准，人工后置不是 waiver，不以 `na`、本地 mock 或旧 session 冒充真实 App 产品证据。
