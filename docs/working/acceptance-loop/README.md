...
### 5.2 Day 0 当前状态(整体重述,2026-08-03 03:44)

**前线增量（2026-08-03 03:44）。** `TOOL-070 get_flowrun` 已完成真实正式绿验收。session `/private/tmp/anselm-rig-formal-20260801-3/sessions/20260803-033531` 以修复后台架二进制、真实 Flutter App、真实受管网关、Computer Use、连续窗口录屏、backend/frontend journal、三路独立 SSE witness 和 LLM tap 完成；真实用户请求只调用一次 `get_flowrun`，没有 retry、search、mutation 或第二次 inspect。目标真实 run `fr_31606084bcee949b` 的 REST 真相为 `completed`、91 个节点全部 `completed`；inline projection 按产品边界展示 80/91，并将剩余 11 个节点指向 REST 分页。

本格先冻结了一个真实产品红线：修复前的真实大运行画面出现 `Run summary for the requested item` 和用户正文中的 `fnv_...` 版本号。修复为 loop 确定性出口对 `Run summary for <opaque-or-placeholder>` 做语义归一化、把 pinned function version 变成人话，并将 `fnv_` 纳入跨 delta 整行缓冲；补充完整块收尾、跨 chunk、flowrun summary 与 pinned reference 的 Go regression。修复后二进制真实重跑后，画面显示自然的 `Flowrun report`，精确 ID 仍只在相邻 tool card 的 Copy 和审计面保留。

修复后的正式录屏封口 `327.648333s`；Computer Use 最终画面显示 `Completed · 80/91`、`Run status: completed`、`True nodeSummary total: 91 nodes` 和 `Projection capped: Yes`，没有 placeholder、opaque version、重复活动或布局溢出。正式证据为 `/private/tmp/anselm-rig-formal-20260801-3/sessions/20260803-033531/evidence/tool-070-formal-acceptance.md`。

五通道交叉核对：messages durable seq `1..20`、notifications durable seq `1..2` 连续，三流均连接；可见 reasoning/text 重建结果不含 `the requested item`、`the referenced item`、`fnv_`、`wfv_`、`apf_`、`apfv_` 或 `get_flowrun tool`；backend 无 panic/FATAL/WARN/ERROR，frontend 无 Flutter/Dart/RenderFlex/Unhandled 红线，LLM challenge/install/models/chat 响应全为 200。REST 分页两页合计 91 个节点且全部 completed，raw tool result 保留完整机器值，UI/SSE close/REST 三方一致。五级 `G1/F2/A5/C4/G2` 已由 judge 落账，中央账本为 **390 judgments**；锚点 10/10 复核有效，`gap-too-fast` 与 `discovery-collapse` 已根据本次完整录屏、修复前红证据和五通道复审 ack，`alarms.py check` clean。第七批 **50 / 50**，统一长门禁、完整 testend、工作树审计和提交现在解锁；下一前线 `TOOL-071 search_flowruns`。
...
| 产品主循环 | ✅ 已从第一条未裁决格启动；前置真实切片持续覆盖 onboarding、chat、composer、toc、log drawer、Read、Write、Edit、LS、Glob、Grep、Bash、BashOutput、KillShell、ask_user、todo_write、todo_read 以及 function 全生命周期。此前各工具切片中的产品语义、错误态和视觉问题均按 stop-and-fix 冻结、修复并真实重跑；formal-103 至 formal-145 已完成 control、approval、workflow search、workflow get、create_workflow、edit_workflow、revert_workflow、delete_workflow、capability_check_workflow、trigger_workflow、stage_workflow、activate_workflow 与 deactivate_workflow；TOOL-069 kill_workflow 的审批胶囊溢出和 TOOL-070 get_flowrun 的 summary/redaction 泄漏均已冻结、修复、补守卫并真实重跑。当前中央账本 390 judgments，锚点有效、警报 clean；第七批 **50 / 50**，统一长门禁与提交已解锁，下一前线 TOOL-071。 |
