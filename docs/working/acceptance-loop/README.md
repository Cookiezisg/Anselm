...
## 当前精确状态（2026-08-30 · EDGE-031 真实台架复验与批次收口）

- 账本清册共 `848` 行；当前 `675` 行完全结算、`173` 行仍有 `457` 个未收口单元。`manual_queue` 已与清册重新对齐为 `173` 条唯一人工尾队，自主未收口单元为 `0`，下一人工项为 `EDGE|视频轮询超时诚实话`。所有必须用户物理按键、系统权限、安全确认或物理网络切换的项目继续后置；这不是 waiver，也不降低五级标准。
...
- `manual_queue` 对 `(family,item)` 强制唯一；重复登记属于策略损坏并 fail-closed，避免长周期运行中尾队长度和前线顺序漂移。回归锁定在 `testend/rig/test_judge.py` 的 `test_sequence_policy_rejects_duplicate_manual_items`。
- provisional-N/A 词表覆盖“没有独立 formal rig 五通道 session”这类缺证据措辞；这类格子必须重新进入人工尾队，不能被误算为已结算。
- 台架回归：`python3 -m unittest testend/rig/test_judge.py -v` 的 `21/21`、JSON 校验和 `py_compile` 均通过。当前批次既有记录为 `50/50`；本轮只调整人工调度，不把 `EDGE-056` 的遮挡 session 计入覆盖格，统一长门禁仍受证据标准约束；标准、法典、证据要求和最终收口条件不变。

## 当前自动前线重述（2026-08-30 · EDGE-172 产品现场受系统窗口遮挡后置；继续自主前线）
...
### 5.2 Day 0 当前状态(整体重述,2026-08-30 EDGE-031 L3 收口；批次 50/50)
