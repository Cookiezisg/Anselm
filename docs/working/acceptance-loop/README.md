...
## 当前精确状态（2026-08-31 · EDGE-249 后台 context 播种边界完成；批次 87 已达 101/50）
...
清册准确重算为 `848` 行：`743` 行五级已结算、`105` 行开放；`3900` 个单元已结算、
`340` 个单元开放。`manual_queue=173`、`forced_queue=26`；批次 87=`101/50`，下一
自主原子继续按顺序门从首个未结算的自主 `~` 格选择。EDGE-247 的真实 session、五通道日志、
...
### 5.2 Day 0 当前状态(整体重述,2026-08-31 EDGE-249 收口、人工尾队后置；批次 87 101/50)

当前唯一权威状态以 `COVERAGE.md`、`ledger-sequence.json`、`judgments.jsonl` 及脚本实时重算为准：清册 `848` 行，`743` 行五级结算，`105` 行仍开放；`3900` 个单元已结算、`340` 个单元开放；`manual_queue=173`，其中实际需要用户强制交互的 `forced_queue=26`。`EDGE-247|ServeMux 纯文本 404/405 改写`、`EDGE-248|客户端断连与请求超时` 与 `EDGE-249|后台裸 ctx 播种缺失` 均已按各自边界收口：EDGE-249 的 focused race 回归证明裸 `context.Background()` 访问 workspace-scoped store 会返回 `MISSING_WORKSPACE_ID`，生产后台入口使用逐 workspace `Detached` context，L2-L5 是逐级正式适用性 `na`，不把内部接线回归冒充用户界面证据。正式复核=`testend/rig/formal-evidence/EDGE-249-applicability-reaudit-20260831.md`，写账触发的两条统计警报已独立复核并销账，最终 `alarms.py check`=`clean (159 live judgments; 4240 baseline judgments excluded)`。顺序门已跳过仍需用户强制交互的 26 项，下一自主前线为 `EDGE-254|keyset 排序切换丢游标`；强制项继续留在人工尾队，不阻塞自主推进。批次 87 已由 `97` 推进至 `101/50`，未改阈值、法典、锚点、五级标准或顺序 gate。P12 的 400+ Journey 扩写按用户裁定推迟二期。
