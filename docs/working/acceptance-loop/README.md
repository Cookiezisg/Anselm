...
- **最新精确快照（EDGE-286 调度后）**：`848` 行、`38` 个人工后置项、`102` 个未收口行；自主队列剩余 `64` 行、`184` 个未收口单元，下一自动前线为 `EDGE-287|run_skill_script 扩展名不支持`。本节下方较早的长快照是调度前历史记录，以本条和 `ledger-sequence.json` 为准。
