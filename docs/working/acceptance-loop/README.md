...
用户裁定推迟二期。
 ...
 - `EDGE-264|「这里没有 git」四情形` 已完成真实 App 五级验收。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-224712`：真实 App 挂载普通目录并打开工作目录菜单，首轮发现 Git 区域静默隐藏，停止推进并修为只读 `Not a Git repository` + `Choose a repo`；最终帧完整显示，无省略、溢出或重排。普通目录、未挂载、路径消失三态分别执行 switch/create/add，九次真实 HTTP 写动作均返回 `422 CONVERSATION_WORK_DIR_NOT_GIT_REPO`，无分支、worktree、marker 或驻地误写；无 Git 二进制由空 `PATH` 的真实进程测试覆盖，同样三动作统一拒绝。五通道 `rig-check`/`rig-down` 通过，录屏=`71.790000s`，backend 无应用红线，SSE 三流连接，frontend 仅已知 macOS IMK 诊断，LLM wire 真实 managed challenge/install/models 通过；本路径无 chat completion，不虚构。L2=`F2`、L3=`B2`、L4=`C5`、L5=`G1`；正式证据=`testend/rig/formal-evidence/EDGE-264-not-a-git-four-states-real-app-20260831.md`，session witness=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-224712/evidence/EDGE-264-not-a-git-real-app.md`，终帧=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-224712/evidence/EDGE-264-not-a-git-menu-fixed.jpeg`，账本复审=`testend/rig/formal-evidence/EDGE-264-ledger-alarm-reaudit-20260831.md`；focused backend 普通/race、frontend `chat_work_dir_button_test.dart`=`24` 项与 `make gen` 通过，清册现为 `✓✓✓✓✓`。

 - `EDGE-263|worktree 建成后切驻地失败` 已完成真实 App 五级验收。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-221640`：真实 App 在 `/private/tmp/anselm-edge263-repo.qGepjk` 的 `main` 驻地中完成真实 managed-gateway 基线聊天，随后用 Computer Use 输入 `partial` 并提交 worktree 创建；故障注入只让最后一步驻地持久化失败。首轮真实现场暴露后端未映射错误和前端虚假文案「什么都没有改变」，停止推进并保留红帧；修复为 `CONVERSATION_WORKTREE_RESIDENCY_UPDATE_FAILED` 及明确的半成功恢复文案后重跑。最终 frame=`testend/rig/formal-evidence/EDGE-263-worktree-update-failure-fixed.jpg`，弹窗保留输入并完整显示 worktree 已创建、本对话仍在原目录、应从菜单切入。Git/REST 证明原仓库仍为 `main`，`...-partial` worktree 与 `wt/partial` 均保留；重复请求返回 `409 CONVERSATION_WORKTREE_EXISTS`，没有二次创建。五通道 `rig-check`/`rig-down` 通过，录屏=`234.660000s`，backend 无 panic/fatal/unmapped/application error，SSE 三流连接且失败动作无伪驻地 mutation，frontend 仅有已知 macOS IMK 诊断，LLM wire 含 managed bootstrap 与两次真实 chat completion。L2=`F2`、L3=`B2`、L4=`C5`、L5=`G1`；正式证据=`testend/rig/formal-evidence/EDGE-263-worktree-update-failure-real-app-20260831.md`，session witness=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-221640/evidence/EDGE-263-worktree-update-failure-real-app.md`，红证据=`testend/rig/formal-evidence/EDGE-263-worktree-update-failure-red.jpg`，账本复审=`testend/rig/formal-evidence/EDGE-263-worktree-update-failure-ledger-alarm-reaudit-20260831.md`；focused backend 普通/race 与 frontend `chat_work_dir_button_test.dart`=`24` 项通过，清册现为 `✓✓✓✓✓`。

 - `EDGE-262|worktree 分支已存在` 已完成真实 App 五级验收。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-215145`：真实 App 先复用已存在的 `wt/kept` 分支并创建 sibling worktree，再对在外部目录占用的 `wt/occupied` 分支提交创建；backend 真实记录 `422 CONVERSATION_GIT_FAILED`。首轮真实复验发现前端只显示 Git stderr 第一行，停止推进并保留红帧；修复 `_gitReason` 保留完整 stderr 后重跑，弹窗完整显示占用目录路径，无裁切。Git 主仓库仍为 `main`，外部 `wt/occupied` 与已创建的 `wt/kept` 均保持，目标 `...-occupied` 未生成；正式抽帧=`testend/rig/formal-evidence/EDGE-262-branch-occupied-fixed.jpg`，红帧=`testend/rig/formal-evidence/EDGE-262-branch-occupied-red.jpg`。五通道 `rig-check`/`rig-down` 通过，frontend 仅有已知 macOS IMK 诊断，无应用级 Dart/Flutter/RenderFlex/overflow/Unhandled 红线，本路径无 chat completion，未虚构该通道；focused `chat_work_dir_button_test.dart`=`23` 项通过。L2=`F2`、L3=`B2`、L4=`C5`、L5=`G1`；正式证据=`testend/rig/formal-evidence/EDGE-262-branch-occupied-real-app-20260831.md`，session witness=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-215145/evidence/EDGE-262-branch-occupied-real-app.md`，账本复审=`testend/rig/formal-evidence/EDGE-262-branch-occupied-ledger-alarm-reaudit-20260831.md`；清册现为 `✓✓✓✓✓`。

 - `EDGE-261|worktree 目录已存在` 已完成真实 App 五级验收。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-213052`：真实 App 在 Git `main` 驻地中对已存在的 sibling 目录提交 `taken`，backend 真实记录 `409 CONVERSATION_WORKTREE_EXISTS`；对话框保留输入并完整展示「目录已存在，请换名称或从菜单切入」。占用目录 sentinel hash 保持不变，未生成 `wt/taken` 分支或 worktree；最终抽帧=`testend/rig/formal-evidence/EDGE-261-worktree-directory-exists-fixed.jpg`。五通道 `rig-check`/`rig-down` 通过，frontend 仅有已知 macOS IMK/TSM 诊断，无应用级 Dart/Flutter/RenderFlex/overflow/Unhandled 红线，本路径无 chat completion，未虚构该通道。L2=`F2`、L3=`B2`、L4=`C5`、L5=`G1`；正式证据=`testend/rig/formal-evidence/EDGE-261-worktree-directory-exists-real-app-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-261-worktree-directory-exists-ledger-alarm-reaudit-20260831.md`；清册现为 `✓✓✓✓✓`。
 ...
 - `EDGE-246|DNS rebinding 防护` 已完成真实 App 的 L2-L5。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-184415-edge246`：appproxy 只对第一次真实 `GET /api/v1/conversations` 将出站 Host 改为 `evil.example.com`，连接仍到 conductor 持有的真实 backend；backend journal 真实返回 `403 FORBIDDEN_BAD_HOST`，一次 Retry 后规范 Host 恢复 `200`。Computer Use 确认错误态只显示可理解的列表失败与 `Try again`，不泄漏 Host、路由或堆栈；点击后恢复 `Recents 1` 与 `演示对话`。录屏=`3104x1844/60fps/62.178333s`，`measure latency`=`100.0ms`，稳定帧无白闪、裁切、重排或自主跳变。L2=`F2`、L3=`A4`、L4=`C4`、L5=`G1`，正式证据=`testend/rig/formal-evidence/EDGE-246-dns-rebinding-real-app-20260831.md`，告警复审=`testend/rig/formal-evidence/EDGE-246-discovery-alarm-reaudit-20260831.md`；清册现为 `✓✓✓✓✓`，下一自主原子为 `EDGE|ServeMux 纯文本 404/405 改写`。
 ...
 - `EDGE-258|新建分支不受脏区门` 已完成真实 App 五级验收。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-205254` 在真实 Git 脏驻地（`main` + 未跟踪 `in-progress.txt`）中打开菜单，清楚展示 `Uncommitted changes`、禁用的已有分支切换提示和可用的 `New branch...`；对话框说明未提交改动会随新分支带走且不会冲突。Computer Use 用真实键盘输入 `feature/edge258` 并创建成功；后端投影和 `git branch --show-current` 均为该分支，未提交文件仍在。重开菜单显示 `Branch feature/edge258`，最终帧无裁切、空白或重排；focused conversation/HTTP/gitinfo 回归、五通道、录屏=`3104x1844/60fps/201.396667s`、`rig-check`/`rig-down` 均通过，frontend 仅有已知 macOS IMK 诊断，无应用级 Dart/Flutter/RenderFlex/overflow/Unhandled 红线；本格不虚构 chat completion。L2=`F2`、L3=`B2`、L4=`C5`、L5=`G1`；正式证据=`testend/rig/formal-evidence/EDGE-258-dirty-create-branch-real-app-20260831.md`，session witness=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-205254/evidence/EDGE-258-dirty-create-branch-real-app.md`，终帧=`testend/rig/formal-evidence/EDGE-258-dirty-create-branch-final.png`，告警复审=`testend/rig/formal-evidence/EDGE-258-dirty-create-branch-ledger-alarm-reaudit-20260831.md`；清册现为 `✓✓✓✓✓`，下一自主原子按顺序门重算。
 - `EDGE-259|切分支名拼错` 已完成真实 App 五级验收。正式 red session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-210547` 在真实 App 已打开分支菜单时删除本地 `typo-target`，保留同名 `remotes/origin/typo-target`，再点击菜单旧行；backend 真实返回 `404 CONVERSATION_BRANCH_NOT_FOUND`，前端第一轮暴露顶带副本被省略为 `That branch is gone. Reopen the menu to ...`，停止推进并保留红帧。修复为 `Branch gone. Reopen.`，不扩大 340px 顶带，并以渲染测试锁定一行完整可见；fixed session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-211452` 重跑同一动作，顶带完整显示，workdir/HEAD 仍为 `main`，Git 仅有远端 ref，未发生 DWIM 跟踪分支创建。两次 `rig-check`/`rig-down`、backend/SSE/frontend/LLM 五通道均有记录；fixed 录屏=`3104x1844/60fps/75.398333s`，frontend 无应用级 Dart/Flutter/RenderFlex/overflow/Unhandled 红线。L2=`F2`、L3=`B2`、L4=`C5`、L5=`G1`；正式证据=`testend/rig/formal-evidence/EDGE-259-branch-typo-real-app-20260831.md`，红/修复帧=`testend/rig/formal-evidence/EDGE-259-branch-typo-red.png`、`testend/rig/formal-evidence/EDGE-259-branch-typo-fixed.png`，告警复审=`testend/rig/formal-evidence/EDGE-259-branch-typo-ledger-alarm-reaudit-20260831.md`；清册现为 `✓✓✓✓✓`，下一自主原子为 `EDGE|worktree 目录已存在`。
 - `EDGE-257|脏区切分支被拒` 已完成真实 App 五级验收。首轮真实 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-202851` 发现脏驻地菜单的阻断文案被固定宽度省略为 `Commit or stash your changes first, then switc...`，停止推进并保留红帧；修复为 `Commit or stash changes before switching` 后，正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-203407` 的 Computer Use 看到完整文案，键盘遍历也能到达 `New branch...` 与 `Open a worktree for this conversation...`，未执行误操作。后端真实重检 dirty 状态并维持 422 `CONVERSATION_WORK_DIR_DIRTY`，前端失败不改投影；focused conversation/HTTP/gitinfo 回归通过。五通道、录屏=`3104x1844/60fps/218.643333s`、`rig-check`/`rig-down` 均通过；frontend 仅有已单独复核的 AX observer churn 与已知 macOS IMK 诊断，无 Dart/Flutter/RenderFlex/overflow/Unhandled 应用红线；本格不虚构 chat completion。L2=`F2`、L3=`B2`、L4=`C5`、L5=`G1`；正式证据=`testend/rig/formal-evidence/EDGE-257-dirty-switch-branch-real-app-20260831.md`，session witness=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-203407/evidence/EDGE-257-dirty-switch-branch-real-app.md`，红/修复帧=`testend/rig/formal-evidence/EDGE-257-dirty-switch-branch-red-20260831.jpg`、`...-fixed-20260831.jpg`，告警复审=`testend/rig/formal-evidence/EDGE-257-dirty-switch-branch-ledger-alarm-reaudit-20260831.md`；清册现为 `✓✓✓✓✓`。
 - `EDGE-256|驻地目录被移走` 已完成真实 App 五级验收。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-201616`：真实 App 先挂载 `/private/tmp/anselm-edge256-workdir.QmsUCq`，再在台架外将目录移走；Computer Use 打开驻地菜单后仍看到原路径、`This directory no longer exists`，Finder/Terminal 动作禁用，而 Switch/Leave 保持可用。5 秒抽帧未见未挂状态闪回、空白或探测导致的重排；backend 同场真实返回 `200` 的缺失投影，三路 SSE、frontend console、managed bootstrap LLM tap、录屏与 `rig-check`/`rig-down` 均可追溯。L2=`F2`、L3=`B2`、L4=`C5`、L5=`G1`；正式证据=`testend/rig/formal-evidence/EDGE-256-moved-workdir-real-app-20260831.md`，完整 session 证据=`sessions/20260831-201616/evidence/EDGE-256-moved-workdir-real-app.md`，告警复审=`testend/rig/formal-evidence/EDGE-256-ledger-alarm-reaudit-20260831.md`；清册现为 `✓✓✓✓✓`。
 ...
 - 告警在每次写账后按机制进入复审：EDGE-257 的四次 `discovery-collapse` 均按原阈值独立复核并 ack，复审记录=`testend/rig/formal-evidence/EDGE-257-dirty-switch-branch-ledger-alarm-reaudit-20260831.md`；未改阈值、法典、锚点、五级标准或顺序门。
 - 告警在每次写账后按机制进入复审：EDGE-258 的四次统计告警均按原阈值独立复核并 ack，复审记录=`testend/rig/formal-evidence/EDGE-258-dirty-create-branch-ledger-alarm-reaudit-20260831.md`；未改阈值、法典、锚点、五级标准或顺序门。
 - 告警在每次写账后按机制进入复审：EDGE-259 的 `discovery-collapse` 与最终 `gap-too-fast`/`discovery-collapse` 均按原阈值独立复核并 ack，复审记录=`testend/rig/formal-evidence/EDGE-259-branch-typo-ledger-alarm-reaudit-20260831.md`；未改阈值、法典、锚点、五级标准或顺序门。
 - 权威清册重算为 `848` 行：`749` 行五级已结算、`99` 行仍开放；`3931` 个单元已结算、`309` 个单元开放。`manual_queue=173`、`forced_queue=26`；P12 的 400+ Journey 扩写按用户裁定推迟二期。当前自动前线已推进为 `EDGE|「这里没有 git」四情形`，强制人工交互仍后置；本轮以真实 App、五通道和红绿 stop-and-fix 证据收口，没有用 focused 回归替代产品证据。
 - 批次 87 已由 `125/50` 推进至 `133/50`；统一长门禁不因本次追加四格而提前运行，仍按每 50 格执行；本轮未以警报压制、旧证据覆盖或标准降级推进。

## 近期完成项历史快照

 - `EDGE-240|ADD COLUMN 结果幂等` 已完成真实 App 的五级验收：从缺少分叉血缘/驻地列的旧 SQLite 安装启动，首次 boot 补齐三列及 `idx_conversations_ws_workdir`，第二次启动对同一组 `ALTER TABLE ... ADD COLUMN` 的 `duplicate column name` 精确视为已应用；原有 conversation、索引与 SQLite integrity 均保持正确。真实 App 两次均稳定进入 Chat，startup 过渡无空白或卡死。L2=`F1`；L3-L5=`na`，因为该后台迁移没有独立用户动作、视觉对象或可发现入口。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-165453`，正式证据=`testend/rig/formal-evidence/EDGE-240-add-column-idempotency-real-app-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-240-ledger-alarm-reaudit-real-app-20260831.md`；清册为 `✓✓~~~`。
 - `EDGE-241|换 master key 种子` 已完成边界验收：不同 seed 的 AES-GCM 旧密文拒绝解密；真实隔离旧库在错误 seed 下于 bootstrap 阶段因加密 `device-proof.key` 认证失败而退出，未伪造 App/SSE/LLM 产品闭环。按 ADR 0008，`ANSELM_MASTER_KEY` 不是产品入口，主密钥轮换不属于用户支持路径；L2-L5 以具体适用性理由记 `na`，不把通用启动失败页冒充恢复体验。真实负向 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-170033`，正式证据=`testend/rig/formal-evidence/EDGE-241-master-key-real-app-boundary-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-241-ledger-alarm-reaudit-20260831.md`；清册保持 `✓~~~~`（L1=`F5`，L2-L5 为 settled `na`）。
 - `EDGE-239|CHECK 加词整表重建` 已完成真实 App 的五级验收：从真实旧 schema SQLite 安装启动，boot migration 恢复 `trigger_firings='missed'`、`flowrun_nodes='cancelled'`、`message_blocks='marker'` 三处 CHECK，保留行数、索引与 SQLite integrity；真实 App 稳定进入 Storage & logs/Chat，startup spinner 过渡无空白或卡死。L2=`F1`；L3-L5=`na`，因为该后台迁移没有独立用户动作、视觉对象或可发现入口。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-162419`，正式证据=`testend/rig/formal-evidence/EDGE-239-check-rebuild-migration-real-app-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-239-ledger-alarm-reaudit-real-app-20260831.md`；清册为 `✓✓~~~`。
 - `EDGE-235|关停预算格` 已完成真实 App 的 L2-L4，L5 以明确适用性边界收口。初轮 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-145945` 真实显示 3 个在途 workflow，SIGTERM 后 backend 有序退出、三路 SSE clean EOF、SQLite 保留可恢复的 `running` 真相且无 SIGKILL；初轮发现“已运行后失联”误显示为“backend didn't start”，停止推进修复 `BackendFailureReason`，复验 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-150438` 的真实 App 显示“本地引擎已停止响应。点击重试以重新连接。” L2=`F2`、L3=`A4`、L4=`C4`；L5=`na` 仅因内部预算生命周期没有独立 discoverability 表面。正式证据=`testend/rig/formal-evidence/EDGE-235-shutdown-budget-real-app-fixed-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-235-ledger-alarm-reaudit-real-app-20260831.md`；清册为 `✓✓✓✓~`。

 - `EDGE-210|免费档配额耗尽` 已完成真实 App 的 L2-L5 五级验收。正式 HTTP 402 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-142137`、流内 `BUDGET_EXHAUSTED` session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-142343` 均使用真实 managed gateway 的 challenge/install/models 链路，llmtap 只对聊天 completion 做受控协议故障注入；前者为 `402 QUOTA_EXHAUSTED`，后者为 `200` 流内耗尽事件。stop-and-fix 修复主时间线泄漏内部码和 provider 原文，最终 App 只显示可行动的双语免费档额度说明及 `Settings → Models & keys` 路径。本格不声称真实消耗网关额度。证据=`testend/rig/formal-evidence/EDGE-210-freetier-quota-exhausted-fixed-real-app-20260831.md`，红证据=`testend/rig/formal-evidence/EDGE-210-quota-copy-red-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-210-ledger-alarm-reaudit-fixed-real-app-20260831.md`，法条=`F2/A4/C4/G1`；清册现为 `✓✓✓✓✓`。
 - `EDGE-234|三步优雅关停` 已完成真实 App 的 L2-L4，L5 以明确适用性边界收口。formal session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-140051-edge234`：真实 App、backend、三路 resident SSE、frontend console、llmtap 与 H.264 录屏由同一 conductor 托管；收到 SIGTERM 后先取消请求上下文，三路 SSE 均 clean EOF，backend 完成 graceful shutdown，owned processes 全部退出且无 SIGKILL escalation。backend/frontend 无应用红线，`rig-check`/`rig-down` 通过。L2=`F2`、L3=`A4`、L4=`C4`；L5=`na` 仅因内部生命周期动作没有用户寻找的功能入口，不把台架日志冒充发现性证据。正式证据=`testend/rig/formal-evidence/EDGE-234-graceful-shutdown-real-app-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-234-ledger-alarm-reaudit-real-app-20260831.md`；清册现为 `✓✓✓✓~`，下一自主前线=`EDGE|关停预算格`。
 - Edge210、Edge234、Edge235、Edge239 与 Edge240 的连续写账均触发 `gap-too-fast`/`discovery-collapse`；每次均用真实证据独立复审后逐条 ack，未改阈值、曲线算法、CODEX、锚点、五级标准或顺序门。当前 `RIG_HOME=/private/tmp/anselm-rig-formal-20260831-11 python3 testend/rig/alarms.py check`=`clean (129 live judgments; 4240 baseline judgments excluded from drift curves)`，锚点=`10/10`，`gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`。
 - 权威清册重算为 `848` 行：`737` 行五级已结算、`111` 行仍开放；`3876` 个单元已结算、`364` 个单元开放。`manual_queue=173`、`forced_queue=26`；`EDGE|父进程死人开关` 与 `EDGE|keychain 铸钥只对全新安装` 均保留在 forced 队列，当前顺序门跳过它们并推进下一个无需用户操作的自主前线。P12 的 400+ Journey 扩写按用户裁定推迟二期。
 - 批次 87 已由 `8/50` 推进至 `77/50`；统一长门禁已在 `69/50` 完成，本格完成后仍保持原门禁结果有效。强制队列项目继续不写绿格、不降低标准，人工后置不是 waiver，不以 `na`、本地 mock 或旧 session 冒充真实 App 产品证据。

 - `EDGE-196|受管 remote media lease` 已完成真实 App 的 L2-L5 五级验收。正式 session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260831-033112`：真实文件选择器附加 PNG 图片，受管 gateway 完成 upload create、分片 PUT、complete，助手返回 `RECEIVED`。
 - `EDGE-197|lease 临期刷新` 已完成真实 App 的 L2-L5 五级验收。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-064421`：真实 App 附加 PNG 后执行真实 sandbox function `3650s`，跨过首个受管媒体 lease 的临界窗口；下一步请求前自动重新 staging，同一图片 `724` bytes 与 SHA-256 保持一致但取得新 lease，第二次 chat completion 返回 `200`，App 最终显示 `DONE`。证据=`sessions/20260831-064421/evidence/EDGE-197-long-react-boundary-green-20260831.md`，法条=`F2/A4/C4/G1`。
 - `EDGE-198|staging 失败大声失败` 已完成真实 App 的 L2-L5 五级验收。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-081342`：真实 App 上传 PNG 后由本地 llmtap 对真实上游路径 `/v1/media/uploads` 注入一次 503，错误终态只显示“附件无法准备，因此回复无法继续”，不泄漏内部码、路径、message id 或 provider 原文；点击 Retry 后同一附件完成重新 staging，真实模型调用 `inspect_media` 并完成回复。证据=`sessions/20260831-081342/evidence/EDGE-198-attachment-staging-failure-green.md`，红证据=`testend/rig/formal-evidence/EDGE-198-real-app-raw-error-red-20260831.md`，法条=`F2/A4/C4/G1`。
 - `EDGE-199|代理图未 ready` 已完成真实 App 的 L2-L5 五级验收。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-084340`：台架仅设置 `ANSELM_RIG_MEDIA_PROCESS_DELAY_MS=15000` 延迟真实媒体处理；真实 App 在 chip 仍显示 `Preparing media…` 时发送 `12000x12000`、`2995581` bytes 的 JPEG，LLM wire 收到同尺寸的原图 staging（不是后续 `943140` bytes、`2048x2048` 代理），约 15 秒后 derivative 才 durable `ready`。App 正常完成回复，五通道、`rig-check`、`rig-down` 均通过；证据=`sessions/20260831-084340/evidence/EDGE-199-attachment-proxy-not-ready-green.md`，账本警报复审=`testend/rig/formal-evidence/EDGE-199-real-app-ledger-alarm-reaudit-20260831.md`，法条=`F2/A4/C4/G1`。前一轮 PNG 仅命中 ready 路径的事实仍保留在 `EDGE-199-real-app-ready-not-fallback-20260831.md`，不与本次回退证据混淆。
 - `EDGE-200|blob GC 只在 boot 跑` 已完成真实 App 的 L2/L3 五通道闭环；L4/L5 是明确适用性边界而非 waiver。三次真实台架 session=`20260831-085128`、`20260831-085315`、`20260831-085630` 复用同一 data dir：先上传四个同 SHA 附件并全部软删，删除窗口内 blob 仍在；下一次 boot 回收孤儿；再建立一个活动引用并删除重复引用，第三次 boot 保留 blob。最终 session 的真实 App、backend、三路 SSE、frontend、managed gateway tap、录屏和 `rig-check` 均通过；证据=`testend/rig/formal-evidence/EDGE-200-real-app-blob-gc-green-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-200-real-app-ledger-alarm-reaudit-20260831.md`，法条=`F2/measure:edge200-attachment-blob-gc-boot-only`。当前 `✓✓✓~~`：存储维护没有用户可见状态/回收结果 UI，L4；也没有需要用户发现内部 boot GC 时机的入口，L5，均以具体理由记 `na`。
 - `EDGE-201|缺失/不可读 blob` 已完成真实 App 的 L2-L5 五级验收。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-090201`：物理删除一个附件 blob、保留 metadata，再用同一真实回合同时引用缺失附件和正常文本附件；App 明确显示前者已不可用，后者按原顺序被读取并精确回显，assistant 以 completed 收尾。backend 的三条 WARN 均明确指向本次物理故障注入，无 ERROR/panic；SSE close seq=`21/22`、LLM tap 两次真实网关 completion=`200`、frontend 无 Flutter/Dart 异常；五通道、`rig-check`、`rig-down` 均通过。证据=`testend/rig/formal-evidence/EDGE-201-real-app-missing-blob-green-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-201-real-app-ledger-alarm-reaudit-20260831.md`，法条=`F2/measure:edge201-attachment-missing-blob-replay/C4/G1`。
 - `EDGE-202|音频 playback token 过期` 已完成真实 App 的 L2-L5 五级验收。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-091019`：真实 WAV 卡片首次播放走过 playback lease、`Range 0-1` 与完整读取；台架 TTL=`1500ms` 的独立 lease 在过期后返回 `404`；切换离开再回到对话，真实 Play 控件重新签发 lease 并再次读取 `206`，最终卡片显示 `0:02` 与进度线。没有把播放器本地缓存误报成过期错误；backend 无 ERROR/panic，frontend 无 Flutter/Dart 异常，五通道、`rig-check`、`rig-down` 均通过。证据=`testend/rig/formal-evidence/EDGE-202-real-app-audio-playback-expiry-green-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-202-real-app-ledger-alarm-reaudit-20260831.md`，法条=`F2/measure:edge202-attachment-audio-playback-token-expiry/C4/G1`。
 - `EDGE-203|非 audio 签发 playback` 已完成真实 App 的 L2-L5 五级验收。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-091904`：真实 PNG 经过 managed gateway 上传并在真实对话中显示为图片卡片，界面没有音频播放入口；对同一图片强行请求 playback lease 返回 `415 ATTACHMENT_PLAYBACK_UNSUPPORTED`，没有签发播放 token。backend 无 ERROR/panic，SSE 收到用户与 assistant 的完整 completed close、最终 durable seq=`9`，frontend 无 Flutter/Dart 异常，五通道、`rig-check`、`rig-down` 均通过。证据=`testend/rig/formal-evidence/EDGE-203-real-app-non-audio-playback-green-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-203-real-app-ledger-alarm-reaudit-20260831.md`，法条=`F2/measure:edge203-non-audio-playback-honesty/C4/G1`。
 - `EDGE-204|朗读缓存命中` 已完成真实 App 的 L2-L5 五级验收。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-092531`：真实 workspace 确认朗读可用，真实 App 首次点击朗读显示准备态、完成一次 managed speech 合成并播放；第二次点击直接复用本地音频附件，不再产生 `read-aloud:read` 或 `audio/speech` 请求；独立同文本同音色 API 对返回 `cached=false → true`、附件 ID 不变且只发生一次语音上游调用。backend 无 ERROR/panic，SSE 对话完整收尾，frontend 无 Flutter/Dart 异常，五通道、`rig-check`、`rig-down` 均通过。证据=`testend/rig/formal-evidence/EDGE-204-real-app-readaloud-cache-green-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-204-real-app-ledger-alarm-reaudit-20260831.md`，法条=`F2/measure:edge204-readaloud-cache-hit/C4/G1`。
 - `EDGE-205|朗读缓存 LRU 淘汰` 已完成真实 App 的 L2-L5 五级验收。正式 session=`20260831-094003`：在仅用于验收的 `5,000,000` 字节预算下，通过 managed gateway 生成四个真实 WAV 产物，真实 SQLite 字节证明 A→B→C→D 依次驱逐最久未使用缓存与旧附件软删除；生产默认仍为 `50 MiB`。随后真实 App 再生成并播放第五个产物，证明淘汰后用户路径仍可用；backend 无 ERROR/panic，SSE、frontend console、LLM wire、录屏五通道齐全，`rig-check`、`rig-down` 均通过。证据=`testend/rig/formal-evidence/EDGE-205-real-app-readaloud-cache-lru-green-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-205-real-app-ledger-alarm-reaudit-20260831.md`，法条=`F2/measure:edge205-readaloud-cache-lru/C4/G1`。下一自主前线为 `EDGE-206|朗读长度上限`。
 - `EDGE-206|朗读长度上限` 已完成真实 App 的 L2-L5 五级验收。初始 session=`20260831-095420` 证明 exactly `4001` 个 CJK rune 在 sidecar 以 `0ms` 返回 `400 READALOUD_TEXT_TOO_LONG`，LLM wire 没有任何 `/v1/audio/speech`；该 session 同时诚实记录了旧 UI 只显示泛化失败、composer 提交探针未成功的红事实。stop-and-fix 让共享前端上限与后端一致，超限朗读入口保持可见但禁用，并以 `Too long to read aloud in one go (maximum 4,000 characters)` 解释原因；修复后二次真实 App session=`20260831-100536` 的 AX 树、录屏和点击后无副作用共同证明用户不会再看到模糊失败 toast，也不会发起 speech 请求。五通道、`rig-check`、`rig-down` 均通过，frontend 仅保留已知 macOS IMK 平台诊断，无 Flutter/Dart 应用红线。证据=`testend/rig/formal-evidence/EDGE-206-real-app-readaloud-length-limit-green.md`、`testend/rig/formal-evidence/EDGE-206-real-app-readaloud-length-limit-ui-green-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-206-real-app-ledger-alarm-reaudit-20260831.md`、`testend/rig/formal-evidence/EDGE-206-real-app-ledger-alarm-reaudit-ui-20260831.md`，法条=`F4/A1/C4/G1`；清册现为 `✓✓✓✓✓`，下一自主前线为 `EDGE-208|origin_tool_call_id 收窄展开`。
 - `EDGE-208|origin_tool_call_id 收窄展开` 已完成真实 App 的 L2-L5 五级验收。首个真实 session=`20260831-101345` 诚实发现了验收 fixture 的 `1x1` PNG 被真实 vision route 以 `400 invalid_request` 拒绝；该红场保留为 stop-and-fix 证据，没有冒充 provenance 失败。修正 fixture 为 provider 可接受的 `32x32` PNG 后，fresh session=`20260831-101803` 从普通 Chat 入口完成 `Searched tools` → `Searched function` → `Ran function edge208_provenance_probe` → `EDGE208DONE`；tool result 同时回显 foreign user-upload attachment 与当前调用铸出的 function artifact，LLM wire 的追加 native media parts 只有后者，foreign ID 仅保留为文本。真实 App、五通道 journal、三路 SSE、managed LLM wire、录屏和 `rig-check`/`rig-down` 均通过，frontend 无 Flutter/Dart 应用红线。证据=`testend/rig/formal-evidence/EDGE-208-real-app-provenance-red-20260831.md`、`testend/rig/formal-evidence/EDGE-208-real-app-provenance-green-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-208-real-app-ledger-alarm-reaudit-20260831.md`，法条=`F2/A4/C4/G1`；清册现为 `✓✓✓✓✓`，下一自主前线由 formal sequence gate 重算。
 - `EDGE-212|瞬时失败绝不轮换` 已完成真实 App 的 L2-L5 五级验收。正式 session=`20260831-103415`：llmtap 对真实受管 `GET /v1/quota` 注入一次 `429`，Models & keys 首帧显示“暂时读不到配额，现有设备注册未改动，请稍后重试”，点击 Repair 后 quota 恢复为 `12 / 1B`，受管 install `ins_f83...6d35` 保持不变，LLM wire 没有 `/v1/install`。初始旧文案的误导性已作为红证据保留并 stop-and-fix；真实 App、五通道 journal、三路 SSE、录屏、managed LLM wire、`rig-check`/`rig-down` 和 focused widget/contract tests 均通过，frontend 无 Flutter/Dart 应用红线。证据=`testend/rig/formal-evidence/EDGE-212-real-app-transient-copy-red-20260831.md`、`testend/rig/formal-evidence/EDGE-212-real-app-transient-no-rotation-green-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-212-real-app-ledger-alarm-reaudit-20260831.md`，法条=`F2/A4/C4/G1`；清册现为 `✓✓✓✓✓`，下一自主前线为 `EDGE-213|未开通读配额`。
 - `EDGE-213|未开通读配额` 已完成真实 App 的 L2-L5 五级验收。正式 session=`20260831-104036`：以 `RIG_SEED=0` 启动空工作区，真实 App 创建 `EDGE213 no-provision` 后进入 Models & keys；受控不可用上游使两次后台 provision 尝试失败，REST `GET /freetier/quota` 精确返回 `404 FREETIER_NOT_PROVISIONED`，界面显示 `Enable free tier`、空 model keys 与 `Not set` defaults，绝不渲染伪造的 `0 / limit`。五通道、录屏、`rig-check`/`rig-down` 和后台/前端 focused 证据均通过；该场景明确不把关闭上游冒充真实网关成功。证据=`testend/rig/formal-evidence/EDGE-213-real-app-quota-not-provisioned-green-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-213-real-app-ledger-alarm-reaudit-20260831.md`，法条=`F2/A4/C4/G1`；清册现为 `✓✓✓✓✓`，下一自主前线为 `EDGE-214|开通降级不挂 boot`。
 - `EDGE-214|开通降级不挂 boot` 已完成真实 App 的 L2-L5 五级验收。正式 session=`20260831-110713`：以 `RIG_SEED=0` 启动空数据目录，受控关闭上游使 managed install 失败；真实 App 创建 `EDGE214 degraded boot` 后仍释放到完整 Chat 壳，主导航与 Composer 可用，没有无限等待、错误死端或伪造配额。无机器指纹、install 失败、持久化冲突三条分支由 `freetier_test.go` 的 focused/race 回归覆盖，现场只对 install 失败作真实五通道证明，不把关闭上游冒充真实网关成功。证据=`testend/rig/formal-evidence/EDGE-214-real-app-provision-degraded-green-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-214-real-app-ledger-alarm-reaudit-20260831.md`，法条=`F2/A4/C4/G1`；清册现为 `✓✓✓✓✓`，下一自主前线为 `EDGE|旋转 key 重探失败`。
 - `EDGE-217|旋转 key 重探失败` 已完成真实 App 的 L2-L5 五级验收。正式 session=`20260831-112438-edge217`：真实 App 在 Models & keys 打开同一条 BYOK key 的编辑表单，输入新的受控测试密钥并提交；旋转先落盘，随后探针失败，耐久行显示新 mask 与 `testStatus=error`。stop-and-fix 修复了原本只显示 `api key probe failed`、无法区分“保存失败”和“探针失败”的产品反馈；修复后最终帧明确显示 `The key was saved, but its connectivity probe failed...`，并保留后端真实错误与可重试按钮。Computer Use 的 URL 字符输入在本机折叠 `:`/`/`，故证据只主张“探针失败”，不冒充特定 TCP 端口拒绝。证据=`testend/rig/formal-evidence/EDGE-217-real-app-key-rotation-probe-failure-fixed-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-217-real-app-key-rotation-ledger-alarm-reaudit-20260831.md`，法条=`F2/A4/C4/G1`；清册现为 `✓✓✓✓✓`，下一自主前线按序重算。
 - `EDGE-218|播种只填未设` 已完成真实 App 的 L2-L5 五级验收。正式 session=`20260831-113541-edge218`：真实 App 创建空工作区后，在 Models & keys 选择受控显式 Dialogue 模型 `gpt-4o · EDGE218 Explicit`，再调用真实 `POST /freetier:provision`；最终 REST/SQLite 与 App 同时显示 Dialogue 仍为显式选择，其余五个未设 scenario 才填入 managed `anselm-auto`。五通道台架、录屏、`rig-check`/`rig-down` 通过；本场景没有聊天 completion，不作虚构。证据=`testend/rig/formal-evidence/EDGE-218-real-app-seed-defaults-unset-only-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-218-real-app-seed-defaults-ledger-alarm-reaudit-20260831.md`，法条=`F2/A4/C4/G1`；清册现为 `✓✓✓✓✓`，下一自主前线按序重算。
 - `EDGE-219|native knob 校验` 已完成真实 App 的 L2-L5 五级验收。正式 session=`20260831-115738-edge219`：真实 App 从启动时读取受控 BYOK key，在 Models & keys → Dialogue 选择 `gpt-5.5`，逐帧展示 `Reasoning effort` 与 `Verbosity` 两个已发布原生旋钮，选择 `high/high` 后保存；同一真实 backend 对未知旋钮返回 `422 MODEL_OPTION_UNSUPPORTED`，对已知旋钮非法值返回 `400 MODEL_OPTION_VALUE_INVALID`，合法值返回 `200` 且 REST 持久状态一致。Computer Use 的 JSON 标点注入限制单独排除，不冒充 UI 错误证据。证据=`testend/rig/formal-evidence/EDGE-219-real-app-native-knobs-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-219-real-app-native-knobs-ledger-alarm-reaudit-20260831.md`，法条=`F2/A4/C4/G1`；清册现为 `✓✓✓✓✓`，下一自主前线按序重算。
 - `EDGE-220|未探测/custom 模型` 已完成真实 App 的 L2-L4，L5 以明确适用性边界收口。正式 session=`20260831-121247`：真实 App 在空 options 的 ModelRef 已写入后，从 Chat 发送请求；custom 未探测 model 的真实调用按设计 fail-loud，durable/SSE 保留 `LLM_STREAM_ERROR` 与连接细节。stop-and-fix 修复了主时间线泄漏原始 `127.0.0.1` URL 和 socket 错误的问题，fresh App 终帧只显示可行动的人话；设置页同时诚实显示该 custom 未探测模型不进入 probe-OK 选择器。证据=`testend/rig/formal-evidence/EDGE-220-real-app-unprobed-custom-model-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-220-real-app-ledger-alarm-reaudit-20260831.md`，法条=`F2/A4/C4`，L5=`~`；清册现为 `✓✓✓~`，下一自主前线按序重算。
 - `EDGE-221|写时校 apiKeyId 存在性` 已完成真实 App/API 五通道现场。正式 session=`20260831-122143`：真实 App 完成 onboarding 后，同一 sidecar 对 conversation override、agent override、workspace scenario default、workspace search default 四个写入口送入不存在的 `aki_missing_edge221`，全部在写入时返回统一 `404 API_KEY_NOT_FOUND`；回查 REST/SQLite 确认原有配置不变、悬挂引用为零。该 seam 没有独立用户等待、视觉产物或可发现 invalid-key 入口，L3-L5 以明确适用性 `na` 收口，不是 waiver。证据=`testend/rig/formal-evidence/EDGE-221-real-app-write-time-key-existence-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-221-ledger-alarm-reaudit-20260831.md`，法条=`F2/A4/C4/G1`；清册现为 `✓✓~~~`，下一自主前线按序重算。
 - `EDGE-225|能力工具诚实缺席` 已完成真实 App 的 L1-L5 五级验收。首个 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-122936` 证明只删除 `defaultImage` 不足以构造“无出图路由”，managed fallback 仍会真实出图；该场景被判为无效前置条件，未写绿。修正后的正式 session=`.../sessions/20260831-123648` 在新 workspace 中设置非图像 BYOK placeholder 为 `defaultImage`，保持 Dialogue 走 managed route；真实 App 不注入 `generate_image`，不触发 `/v1/images/generations`，并向用户显示准确的 `Settings → Models & keys → Image generation` 配置路径。首轮误导性“安装 MCP”指引被 stop-and-fix 修为正确的内建设置指引，代码=`backend/internal/app/chat/prompt.go`、回归=`backend/internal/app/chat/chat_test.go`、同步文档=`docs/references/backend/domains/chat.md`。五通道、`rig-check`/`rig-down`、frontend/backend 健康均通过；证据=`testend/rig/formal-evidence/EDGE-225-real-app-image-capability-honest-absence-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-225-ledger-alarm-reaudit-20260831.md`，法条=`E4/F2/A4/C4/G1`；清册现为 `✓✓✓✓✓`，下一自主前线=`EDGE-228|ASR sidecar 无受管凭证`。
 - `EDGE-228|ASR sidecar 无受管凭证` 已完成真实 App 的 L2，并以明确适用性边界收口 L3-L5。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-125311-edge228`：`RIG_SEED=0` 创建空工作区，受控关闭 `127.0.0.1:9` 仅用于让 managed provision 确定性失败；这次不声称成功调用真实网关。真实 App 进入空白 Chat 后，AX 树和关键帧均只有 `Mention an entity`、`Attach files` 与文本输入，没有麦克风/语音入口；frontend、backend、三路 SSE、LLM wire、60fps 录屏和 `rig-check`/`rig-down` 属于同一封存 session，收台无残留。L2 按 `F2` 通过；L3/L4/L5 分别按 `A4/C4/G1` 记有理由的适用性 `na`，不把 absence seam 冒充语音操作、视觉产物或可发现入口。正式证据=`testend/rig/formal-evidence/EDGE-228-real-app-no-managed-asr-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-228-ledger-alarm-reaudit-20260831.md`；下一自主前线=`EDGE-229|多块 TTS PCM 拼接`。
 - `EDGE-229|多块 TTS PCM 拼接` 已完成真实 App 的 L2-L5 五级验收。显式工具指令 session=`/private/tmp/anselm-rig-formal-20260831-12-edge229/sessions/20260831-130336` 与自然语言主 session=`/private/tmp/anselm-rig-formal-20260831-12-edge229b/sessions/20260831-130955` 均经真实受管 gateway 产生 5 次 `/v1/audio/speech` 分块调用；自然语言场景没有写工具名，只说“做成可播放音频”，App 仍完成一次 `generate_speech` 并返回单一 WAV。最终附件 `6,513,164` bytes、`24kHz/16bit/mono`、`135.69s`，五块实际 PCM 之和等于最终 data 区，只有一个 RIFF/WAVE 头。loading 帧显示 `Synthesizing speech…`、计时和 Stop，完成帧显示 `WAV · 2:16` 播放卡；五通道、`rig-check`/`rig-down`、frontend/backend 健康均通过。正式证据=`testend/rig/formal-evidence/EDGE-229-real-app-tts-pcm-concat-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-229-ledger-alarm-reaudit-20260831.md`，法条=`measure:edge229-tts-pcm-concat/F2/A4/C4/G1`；清册现为 `✓✓✓✓✓`，下一自主前线按序重算为 `EDGE|模型目录运行时刷新失败`。
 - `EDGE-230|ParseWAV 遍历 chunk 表` 已完成真实 App 的 L2，并以明确适用性边界收口 L3-L5。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-132910-edge230`：真实 App 从自然语言“make a playable audio version of the complete passage”进入朗读能力，未在用户文本中指定工具名；真实 managed gateway 返回 3 个 `/v1/audio/speech=200`，llmtap 在本 session 显式插入 `LIST/fact` 后，最终 durable WAV 仍只有一个 `RIFF` 与一个 `data` chunk。三个实际 PCM 区=`1,350,240 + 1,384,800 + 414,720 = 3,149,760` bytes，最终 blob 的 data 区完全相等；App 终态显示约 `65s` 单一 WAV 附件。第一轮注入器误判真实 data 长度哨兵的红证据保留，修复后单测和全新 session 重跑均已封存。正式证据=`testend/rig/formal-evidence/EDGE-230-parse-wav-chunks-injected-real-app-20260831.md`，红证据=`testend/rig/formal-evidence/EDGE-230-parse-wav-chunks-injection-missed-red-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-230-ledger-alarm-reaudit-20260831.md`；L2=`measure:edge230-parse-wav-chunks`，L3/L4/L5 为内部 parser 无独立对应表面的适用性 `na`；清册现为 `✓✓~~~`，下一自主前线=`EDGE|模型目录运行时刷新失败`。
 - `EDGE-230|ParseWAV 遍历 chunk 表` 已完成真实 App 的 L2，并以明确适用性边界收口 L3-L5。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-132910-edge230`：真实 App 从自然语言“make a playable audio version of the complete passage”进入朗读能力，未在用户文本中指定工具名；真实 managed gateway 返回 3 个 `/v1/audio/speech=200`，llmtap 在本 session 显式插入 `LIST/fact` 后，最终 durable WAV 仍只有一个 `RIFF` 与一个 `data` chunk。三个实际 PCM 区=`1,350,240 + 1,384,800 + 414,720 = 3,149,760` bytes，最终 blob 的 data 区完全相等；App 终态显示约 `65s` 单一 WAV 附件。第一轮注入器误判真实 data 长度哨兵的红证据保留，修复后单测和全新 session 重跑均已封存。正式证据=`testend/rig/formal-evidence/EDGE-230-parse-wav-chunks-injected-real-app-20260831.md`，红证据=`testend/rig/formal-evidence/EDGE-230-parse-wav-chunks-injection-missed-red-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-230-ledger-alarm-reaudit-20260831.md`；L2=`measure:edge230-parse-wav-chunks`，L3/L4/L5 为内部 parser 无独立对应表面的适用性 `na`；清册现为 `✓✓~~~`，下一自主前线=`EDGE|模型目录运行时刷新失败`。
 - `EDGE-232|模型目录运行时刷新失败` 已完成真实 App 的 L2-L3，L4/L5 以明确适用性边界收口。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-134318-edge232`：只将后台 models.dev fetch 指向关闭的 `127.0.0.1:9`，真实 managed gateway 仍完成 challenge/install/models/quota 与两次 chat completion；boot 后约 30 秒 backend 明确记录 `previous catalog kept`，Settings → Models & keys 仍显示 managed model、六个 scenario defaults，真实聊天返回 `EDGE232CHATOK`。录屏 `147.233333s / 3104x1844 / 60fps`，三路 SSE 各连接一次并 clean EOF，frontend 无 Flutter/Dart/布局红线；正式证据=`testend/rig/formal-evidence/EDGE-232-model-catalog-refresh-failure-real-app-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-232-ledger-alarm-reaudit-20260831.md`；L2=`E7`、L3=`A4`，L4/L5 是无独立视觉成品/入口的内部路径适用性 `na`，清册现为 `✓✓✓~~`，下一前线=`EDGE|boot 顺序 SweepMisfires`。
 - `EDGE-232|模型目录运行时刷新失败` 已完成真实 App 的 L2-L3，L4/L5 以明确适用性边界收口。正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-134318-edge232`：只将后台 models.dev fetch 指向关闭的 `127.0.0.1:9`，真实 managed gateway 仍完成 challenge/install/models/quota 与两次 chat completion；boot 后约 30 秒 backend 明确记录 `previous catalog kept`，Settings → Models & keys 仍显示 managed model、六个 scenario defaults，真实聊天返回 `EDGE232CHATOK`。录屏 `147.233333s / 3104x1844 / 60fps`，三路 SSE 各连接一次并 clean EOF，frontend 无 Flutter/Dart/布局红线；正式证据=`testend/rig/formal-evidence/EDGE-232-model-catalog-refresh-failure-real-app-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-232-ledger-alarm-reaudit-20260831.md`；L2=`E7`、L3=`A4`，L4/L5 是无独立视觉成品/入口的内部路径适用性 `na`，清册现为 `✓✓✓~~`，下一前线=`EDGE|boot 顺序 SweepMisfires`。
 - `EDGE-233|boot 顺序 SweepMisfires` 已完成真实 App 的 L2-L5 五级验收。setup session=`/private/tmp/anselm-rig-ep233-setup-20260831/sessions/20260831-135329` 建立真实 active cron listener，formal session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-135552-edge233` 在同一 data dir 重启；boot 后 SQLite 有 2 条 `missed` firing、均无 `flowrun_id`，workflow 没有被补跑。Scheduler 真实终帧显示 `Missed · 24h: 1` 与 `0 runs in 24h: 0 ok, 0 failed, 1 missed`，下一刻度仍可见；录屏 `58.195000s / 3104x1844 / 60fps`，三路 SSE 各连接一次并 clean EOF，frontend/backend 无应用红线。正式证据=`testend/rig/formal-evidence/EDGE-233-boot-sweep-order-real-app-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-233-ledger-alarm-reaudit-real-app-20260831.md`，法条=`F2/A4/C4/G1`；清册现为 `✓✓✓✓✓`，下一前线=`EDGE|三步优雅关停`。
 - `EDGE-227|语音配额与限流分流` 暂不进入自主前线：本地分类/映射回归已通过，但完整 L2-L5 需要麦克风权限和真实受管 ASR 网关主动返回额度耗尽、限流、封禁三类拒绝；API Serve 当前没有安全的客户端演练开关。按用户“需要我强行交互的放到最后”裁定，已从候选顺序移入 `forced_queue`，保留 L1 与未完成 L2-L5；不以本地假网关或单测写绿。
 - 本轮修复同时加固账本自身：`judge.py` 现在把“本格本批没有独立……”这类缺真实证据备注识别为 provisional `~`，避免 L2 写入后错误跳过仍未验证的 L3-L5；新增回归测试覆盖该中文变体。原有“本格没有独立……”适用性说明仍保持明确 `na` 语义，不被过度匹配。
 - EDGE-196 的法条=`F2/A4/C4/G1`，四个等级均来自同一真实 session 的独立证据；五通道 `rig-check`/`rig-down` 通过，SSE 的 user close→assistant reasoning/text→completed close 完整，LLM wire 的 `image_url` 仅为 `/v1/media/leases/...` 相对路径，无绝对 URL、`data:image`、base64 或图片字节，backend 无 ERROR/panic，frontend 无 Flutter/Dart 异常，收台后进程归零。证据=`sessions/20260831-033112/evidence/EDGE-196-L{2,3,4,5}.md`，产品复核=`testend/rig/formal-evidence/EDGE-196-managed-media-lease-real-app-20260831.md`。
 - EDGE-196 的真实产品复核确认受管媒体走 resumable upload 后以短期相对 lease 交付模型，产品界面保留附件缩略图和消息上下文；本次不把远端 lease 内容下载另行夸大为独立证据，未降低法典、阈值、锚点、五级标准或 formal sequence。
 - 本次五十二次写账后的 `RIG_HOME=/private/tmp/anselm-rig-formal-20260831-11 python3 testend/rig/alarms.py check` 已在独立 re-audit=`testend/rig/formal-evidence/EDGE-213-real-app-ledger-alarm-reaudit-20260831.md` 后 clean，未改阈值；`gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`，锚点=`10/10`。新台架账本按仓内规定由 `rebuild_ledger.py --write --acknowledge-history` 从当前 COVERAGE 建立带 hash 的历史基线，未手工写账或绕过连续性 gate。
 - 权威清册重算为 `848` 行：`731` 行五级已结算、`117` 行仍开放；`3853` 个单元已结算、`387` 个单元开放。`manual_queue=173`、`forced_queue=25`；自主候选已耗尽，当前按清册顺序推进普通后置项，另有 `25` 行保留在强制人工队列。P12 的 400+ Journey 扩写按用户裁定推迟二期。
 - 上一批次 85 已满 `50/50`，统一长门禁已通过并封存于 `testend/rig/formal-evidence/batch-85-unified-gate-20260831.md`；批次 86 已由 `0/50` 推进至 `52/50`，统一长门禁已通过并封存于 `testend/rig/formal-evidence/batch-86-unified-gate-20260831.md`；当前批次 87 已由 `8/50` 推进至 `48/50`，未满 `50` 格不执行统一长门禁、不提交。强制队列项目继续不写绿格、不降低标准，人工后置不是 waiver，不以 `na`、本地 mock 或旧 session 冒充真实 App 产品证据。

## 历史前线快照（2026-08-31 · EDGE-189 Changed 队满丢事件 L5 完成；批次 20/50）

- `EDGE-189|Changed 队满丢事件` 已完成真实 App 的 L2-L5 五级验收。正式 session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260831-002700`：真实洪峰并发创建 1500 篇文档，全部 HTTP 201，后端记录 435 次非阻塞队列丢弃；SIGKILL 后以同一数据目录重启，boot reconcile 将相关搜索 chunk 从 `2728` 补齐至 `3000`，覆盖全部 1500 篇文档。三条 SSE 断线重连，LLM wire 响应为 200，真实 App 恢复后搜索并读出 `EDGE189 live recovery sentinel 1499`；录屏逐帧确认 thinking/工具进度、长等待可取消、完成态布局和自然语言入口。L2-L5 证据分别为该 session 下的 `...-l2-real-app-20260831.md`、`...-l3-real-app-20260831.md`、`...-l4-real-app-20260831.md`、`...-l5-real-app-20260831.md`，`judge.py`=`F2/A4/C4/G1`；人为 SIGKILL 产生的前端断线/重连噪声已在证据中明示，不冒充正常日志。
- `EDGE-189` 每次写账后的 `discovery-collapse` 均由独立复审记录=`testend/rig/formal-evidence/EDGE-189-ledger-alarm-reaudit-real-app-20260831.md`、`...-l3-ledger-alarm-reaudit-real-app-20260831.md`、`...-l4-ledger-alarm-reaudit-real-app-20260831.md`、`...-l5-ledger-alarm-reaudit-real-app-20260831.md` 串行 ack；四次均未改告警阈值、算法、CODEX、锚点、五级标准或顺序 gate。最终 `gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`，`alarms.py check`=`clean`。下一自主前线为 `EDGE|sifter 缺席回退` L2；强制人工首项仍为 `EDGE|视频轮询超时诚实话`，批次由 `16/50` 推进至 `20/50`，未满 50 格不执行统一长门禁、不提交。
- 账本清册共 `848` 行；当前 `699` 行完全结算、`149` 行仍有 `483` 个未收口单元，已结算单元 `3757` 个。`manual_queue` 是 `173` 条唯一历史候选，实际需要用户强制交互的 `forced_queue` 是其中 `24` 条；其中 `125` 行仍可自主推进，强制人工首项仍为 `EDGE|视频轮询超时诚实话`。`EDGE|每租户模板 URL` 和 `EDGE|MCP 市场缺必填 env` 继续留在 forced queue，不写绿格、不降低标准；人工后置不是 waiver，历史候选但不在 `forced_queue` 的项目按顺序门正常自主推进。
- `EDGE-188|密文红线` 已完成真实 App 的 L3 进行态验收。第二个正式 session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260831-000912`：发送后录屏在 `1.1s` 捕获 `thinking` 可见反馈，随后进入搜索工具活动并完成零命中解释；wire 没有 `Read/LS/Glob/Grep` 文件系统调用，也没有重复同一搜索。证据=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260831-000912/evidence/EDGE-188-search-encrypted-redline-l3-real-app-20260831.md`，`judge.py`=`EDGE|密文红线 L3 ✓ (A4)`。本格只判定 >1s 操作有明确进行态，不把首个文字 token冒充 100ms A1 反馈；USB 配件授权仍是后置强制交互，不影响本格 App 窗口证据。
- `EDGE-188|密文红线` 已完成真实 App 的 L4 视觉 craft 验收。稳定尾帧=`/private/tmp/edge188-second-final-0831.png`，源自正式 session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260831-000912`：结果层级、实体表格列宽/行高、中英文混排、换行、Composer 和无遮挡收尾均通过 C4；证据=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260831-000912/evidence/EDGE-188-search-encrypted-redline-l4-real-app-20260831.md`，`judge.py`=`EDGE|密文红线 L4 ✓ (C4)`。L4 只判定完成态视觉，不重复挪用 L2/L3 事实；本轮 `discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-188-ledger-alarm-reaudit-real-app-20260831.md` ack，未改任何阈值或标准。
- `EDGE-188|密文红线` 已完成真实 App 的 L5 可发现性验收。默认 Chat 中 `Chat`、`New chat` 与 Composer 直接可见，用户无需知道工具名、命令或 Settings 即可用自然语言完成搜索并理解零命中解释；证据=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260831-000912/evidence/EDGE-188-search-encrypted-redline-l5-real-app-20260831.md`，`judge.py`=`EDGE|密文红线 L5 ✓ (G1)`。L5 只判定入口与路径发现性；新增告警已由独立复审=`testend/rig/formal-evidence/EDGE-188-l5-ledger-alarm-reaudit-real-app-20260831.md` ack，标准、阈值、法典、锚点和顺序门不变。
- `EDGE-188|密文红线` 已完成真实 App 的 L2 数据真相验收。正式 session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260830-235206`：合成 API key、webhook trigger config、MCP env 的密文值经 REST 搜索均为零命中；真实 App 精确密文查询返回 `0`，公开 trigger 名称返回 `1` 个 `webhook`。五通道 journal、三流 SSE、managed LLM tap、窗口录屏均属于同一 session，正式证据=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260830-235206/evidence/EDGE-188-search-encrypted-redline-l2-real-app-20260830.md`。自由编排首次出现的重复检索与停止体验已如实记录，不能替代后续 L3/L4/L5 判断。
- `EDGE-188` 本轮台架曾被 `UserNotificationCenter` 瞬态窗口挡住 `rig-check`，末帧复核无遮挡；`frontend.log` 保留一条 macOS IMK mach-port 错误，未见 Flutter exception。该环境问题不被隐瞒，也不被本次 L2 绿格当作视觉通过；若影响后续逐帧等级，须先修复台架/环境并以 fresh session 重跑。
- `EDGE-185|异查询游标` 已完成真实 App + 本地真实 sidecar 的 L2 事实闭环。正式 session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260830-233247`：App 从普通 Chat 入口真实完成搜索目标；同一 sidecar 对 `q=demo` 取得 cursor 后，带到 `q=different` 返回 `400 SEARCH_CURSOR_INVALID`，同查询带合法 `=` padding 仍返回下一页。五通道 `rig-check` 通过、录屏=`186.960000s / 3104x1844 / 60fps`，收台后 conductor 进程归零。正式证据=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260830-233247/evidence/EDGE-185-search-cursor-query-binding-l2-real-app-20260830.md`。
- `EDGE-185` 的 L3-L5 已通过适用性 gate 收口为 `~`，不是 UI 通过：统一搜索 cursor 仅存在于 REST 合同，Flutter 当前没有把该 continuation 或 `SEARCH_CURSOR_INVALID` 做成独立控件、视觉状态或导航入口；实体/对话 rail 使用各自列表协议。依据=`testend/rig/formal-evidence/EDGE-185-applicability-boundary-20260830.md`；未来若公开统一搜索分页或错误状态，必须撤销三项裁决并重跑。
- `EDGE-186|:reindex 并发与就地重建` 的 L2-L5 已通过适用性 gate 收口为 `~`：该动作是无用户产物的内部维护 API，Flutter 没有调用点、控件、状态卡片或导航入口；focused/真实黑盒仍覆盖 single-flight、跨 workspace 隔离、就地 reconcile 与 `204/409` 合同。依据=`testend/rig/formal-evidence/EDGE-186-applicability-boundary-20260830.md`；未来若公开入口或进度状态，必须撤销四项裁决并重跑。
- `EDGE-187|fts_schema_version 不匹配` 的 L2-L5 已通过适用性 gate 收口为 `~`：该动作是 boot 阶段的派生索引迁移 seam，Flutter 只消费迁移后的搜索结果，不接收迁移状态；focused boot 回归仍覆盖旧投影清理、当前版本写入和 live source 重建。依据=`testend/rig/formal-evidence/EDGE-187-applicability-boundary-20260830.md`；未来若公开迁移状态或等待反馈，必须撤销四项裁决并重跑。
- `EDGE-185..187` 的连续适用性写账触发 `pass-burst`，已按独立复审=`testend/rig/formal-evidence/EDGE-185-187-ledger-alarm-reaudit-20260830.md` ack；未改阈值、算法、法典、锚点或顺序 gate。当前 `gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`，`alarms.py check`=`clean (2176 live judgments; 2300 baseline judgments excluded)`，`git diff --check` 通过。未满 `50` 格不执行统一长门禁、不提交；P12 的 400+ Journey 扩写继续按用户裁定推迟二期。
- `EDGE-184|短词 LIKE 回退` 已完成真实短 token、长短 token 合取及真实 App L2-L5 收口。独立 session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260830-225024` 中，`q=qx` 的 LIKE 回退返回并高亮目标，`q=forecast qx` 仅返回同时含两者的文档；App 从普通 Chat 入口完成搜索、读取和排除解释。正式证据=`testend/rig/formal-evidence/EDGE-184-search-short-token-like-fallback-l{2,3,4,5}-real-app-20260830.md`，四级已写账；该批次统一门禁已封存，当前新批次为 `1/50`。
- 统一门禁首轮已真实跑完 backend black-box、前端生成/分析/测试、docs、demo；前端唯一实际失败为启动错误提示丢失开发启动指引，以及 MCP 卡片统计段合并后无法独立定位。已恢复完整双语启动指引，并将 MCP 状态/工具/调用拆成独立文本节点；定点测试、完整前端 `5452` 测试和第二轮根 `make verify` 均已通过。正式证据=`testend/rig/formal-evidence/batch-84-unified-gate-20260830.md`，批次 `52/50` 已封存。
- `EDGE-183|换 embedder 重嵌` 已完成真实 builtin→Ollama 切换、真实 `embeddinggemma:latest` 重嵌、当前模型隔离及真实 App 语义召回的 L2-L5 收口。独立 session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260830-223728` 中，Ollama 向量覆盖当前 `35/35` 个文档，旧 builtin 仅有 `1` 条且不对应现存 `search_docs`，不会进入当前 provider 查询；App 从自然目标发现 `search_documents`→`read_document`，真实回答目标文档。正式证据=`testend/rig/formal-evidence/EDGE-183-search-embedder-switch-reembed-l{2,3,4,5}-real-app-20260830.md`，四级均已写账，警报复核待本轮统一门禁执行。
- `EDGE-182|cosineFloor 噪声闸` 已完成真实红→修→fresh session→L2-L5 收口。红 session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260830-221606` 的真实统一搜索把自然乱码 `flomptar quendel vaxori` 返回为 3 个无关实体，最高 raw cosine=`0.721914`；这是真实产品缺陷，不因 App 上层最终显示“无结果”而豁免。修复在 semantic-only、无词法证据路径增加 `semanticMargin=0.03`，仅拒绝平坦高基线候选，保留真实语义召回；未改验收标准、CODEX、锚点、警报阈值或顺序门。
- 修复后的 fresh session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260830-222330` 由真实 Go sidecar、Flutter App、EmbeddingGemma、managed gateway、三路 SSE、LLM tap、frontend console 和 Computer Use 录屏组成。真实 REST 对自然乱码返回 `total=0`，语义查询仍找到 `EDGE182 Semantic Recall Fixture`；App 从新对话显示诚实空结果，并完成不同措辞的语义解释。录屏=`209.861667s`，收台后进程归零，五通道非空且无应用级前端红线。正式 L2-L5 证据=`testend/rig/formal-evidence/EDGE-182-search-cosine-floor-noise-gate-l2-fixed-real-app-20260830.md`、`...-l3-fixed-real-app-20260830.md`、`...-l4-fixed-real-app-20260830.md`、`...-l5-fixed-real-app-20260830.md`；红证据保留。`pass-burst` 已由 `testend/rig/formal-evidence/EDGE-182-ledger-alarm-reaudit-real-app-20260830.md` 独立复审并 ack，`anchors.py check=10/10`、`alarms.py check=clean`。
- `EDGE-181|整批 embed upsert 全失败` 已完成真实 App L2-L5 收口。独立数据副本在真实 `search_embeddings` 表安装受控 SQLite 写失败 trigger，真实 worker 首轮记录 `upsert failed` 与 `batch fully failed to persist; aborting backfill round, next kick retries`；真实文档 PATCH 触发第二次独立 kick 后累计失败计数固定为 `3`/abort=`2`，没有热循环。lexical search 仍返回 `EDGE181` 文档与高亮，App 保持正常 Chat 空态，无前端红线；真实 backend、三路 SSE、frontend、managed LLM tap、Computer Use 录屏和收台进程归零均属于 session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260830-220606`。正式 L2-L5 证据分别为 `testend/rig/formal-evidence/EDGE-181-search-embed-upsert-all-fail-l2-real-app-20260830.md`、`...-l3-real-app-20260830.md`、`...-l4-real-app-20260830.md`、`...-l5-real-app-20260830.md`；该 trigger 是确定性的真实 store 写失败注入，不冒充物理磁盘满。
- `EDGE-180|embedder 孤儿回收` 已完成真实 kill-9→同数据目录重启→真实搜索恢复闭环。第一段真实 App session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260830-215722` 启动真实 `EmbeddingGemma`，PID=`62153`；对 backend=`61568` 发真正 `SIGKILL` 后确认 `llama-server` 孤儿仍在、PID 文件仍指向它，App 自动进入 `Can't reach the local engine` 错误门。第二段真实 App session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260830-215934` 使用同一数据目录重新启动，真实搜索返回结果，backend journal 明确记录回收 `62153`，新 embedder=`62689` ready，恢复后的 App 回到正常 Chat。两段均由 Computer Use、窗口录屏、backend、三路 SSE、frontend console、managed LLM tap 组成真实台架证据，不以 sleep/fake fixture 冒充；正式 L2-L5 证据分别为 `testend/rig/formal-evidence/EDGE-180-search-embedder-orphan-reap-l2-real-app-20260830.md`、`...-l3-real-app-20260830.md`、`...-l4-real-app-20260830.md`、`...-l5-real-app-20260830.md`。`COVERAGE EDGE-180=✓✓✓✓✓`，旧 L1 与 provisional NA 指针保留为审计历史。
- `EDGE-178|搜索 embedder 缺席降级` L3 已在同一正式真实 App session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-205857` 收口。真实 App 走完 embedder=`off` 的词法 fallback 聊天路径，`measure latency` 以发送前稳定帧为锚点测得首个超过阈值的实质反馈为 `1.4s`；完成后的 `32` 帧稳定尾段在正文 ROI、阈值 `0.0005` 下 `measure diff` 无输出。录屏=`171.086667s / 3104x1844 / 60fps`，backend=`271` 行、frontend=`5` 行、SSE=`294` 行、LLM wire=`28` 行，均无应用级红线且归属于同一 manifest。按 `B2` 写入 L3，证据=`testend/rig/formal-evidence/EDGE-178-search-embedder-off-fallback-l3-real-app-20260830.md`；设置面没有 search/embedder 控件的事实仍保留，L4/L5 另有独立证据。
- `EDGE-178` L3 写账后的 `discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-178-l3-ledger-alarm-reaudit-real-app-20260830.md` ack；`anchors.py check`=`10/10`、`alarms.py check`=`clean (2136 live judgments; 2300 baseline judgments excluded)`、`gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`。未改阈值、算法、法典、锚点或顺序门。本批次由 `25→26/50`，未满 50 格不执行统一长门禁、不提交；P12 的 400+ Journey 扩写继续按用户裁定推迟二期。
- `EDGE-178|搜索 embedder 缺席降级` L4 已在同一正式真实 App session 收口。稳定尾段真实画面复核了正文层级、列表缩进、行内 code 胶囊、island/card/pill 圆角层级、Composer 内缩、长标题省略和无遮挡；没有 clipping、overlap、残留 loading 或 fallback 专属视觉噪声。按 `C4` 写入 L4，证据=`testend/rig/formal-evidence/EDGE-178-search-embedder-off-fallback-l4-real-app-20260830.md`；该证据不把缺失 Settings search/embedder 控件冒充为 L5 通过。
- `EDGE-178` L4 写账后的 `discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-178-l4-ledger-alarm-reaudit-real-app-20260830.md` ack；`anchors.py check`=`10/10`、`alarms.py check`=`clean (2137 live judgments; 2300 baseline judgments excluded)`、`gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`。未改阈值、算法、法典、锚点或顺序门。本批次由 `26→27/50`，未满 50 格不执行统一长门禁、不提交；当前 formal sequence 下一自主前线为 `EDGE|搜索 embedder 缺席降级` L5，P12 的 400+ Journey 扩写继续按用户裁定推迟二期。
- `EDGE-178|搜索 embedder 缺席降级` L5 已在同一正式真实 App session 收口。新 workspace 从 onboarding 后的空白 Chat 开始，用户无需读文档、知道内部术语或寻找不存在的 search/embedder 设置，即可直接提出检索目标；模型真实走 `search_documents` → `read_document` 并给出 lexical/semantic 的诚实边界。按 `G1` 写入 L5，证据=`testend/rig/formal-evidence/EDGE-178-search-embedder-off-fallback-l5-real-app-20260830.md`；这验证的是自动能力的可发现性和用户目的达成，不是内部开关可见性。
- `EDGE-178` L5 写账后的 `discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-178-l5-ledger-alarm-reaudit-real-app-20260830.md` ack；`anchors.py check`=`10/10`、`alarms.py check`=`clean (2138 live judgments; 2300 baseline judgments excluded)`、`gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`。未改阈值、算法、法典、锚点或顺序门。本批次由 `27→28/50`，未满 50 格不执行统一长门禁、不提交；当前 formal sequence 下一自主前线为 `EDGE|首用下载途中关停` L2，P12 的 400+ Journey 扩写继续按用户裁定推迟二期。
- `EDGE-179|首用下载途中关停` L2 已在正式 session=`/private/tmp/anselm-rig-formal-20260801-4/sessions/20260830-212132` 收口。真实 App、Go sidecar、三路 SSE witness、managed gateway LLM wire tap 和 60 fps 录屏均已接线；通过真实首次 builtin embedder 下载窗口观察到 `engine.status=downloading` 后向 backend 发送 `SIGTERM`。backend journal 记录 installer `context canceled`、`provider unavailable, staying lexical` 和 sandbox 收口，关停约 `118ms`；三路 SSE 均连接、frontend console 无应用级红线、managed gateway challenge/install/models 均为 `200`，录屏已封存且进程组归零。正式证据=`testend/rig/formal-evidence/EDGE-179-search-first-download-shutdown-l2-real-app-20260830.md`，按 `F2` 写入 L2。
- `EDGE-179|首用下载途中关停` L3 首次真实 App 复验失败，停在 stop-and-fix。新 session=`/private/tmp/anselm-rig-formal-20260801-5/sessions/20260830-213111` 完成 onboarding 并真实观察到 `engine.status=downloading`；SIGTERM 后录屏仍停在正常空白 Chat，没有断连状态或恢复入口，而 `frontend.log` 在 sidecar 退出后持续出现 `76` 行三路 SSE `Connection refused`。后端本身已记录下载取消和有序退出，故红点在前端健康/产品反馈，不是后端关停。红证据=`testend/rig/formal-evidence/EDGE-179-search-first-download-shutdown-l3-red-20260830.md`，按 `A4` 写入失败；不把缺反馈记为 `na`，不填 L4/L5。修复方向是外接后端持续健康监督，复验前不得继续 formal sequence。
- `EDGE-179|首用下载途中关停` L3 已在全新 session=`/private/tmp/anselm-rig-formal-20260801-6/sessions/20260830-214132` 修复复验通过。真实 App 完成 onboarding 后触发真实 `engine.status=downloading`，SIGTERM 后约 `39ms` 退出；App 在检测窗口后切换到全屏 `Can't reach the local engine` 错误门，显示清晰的不可用说明和 `Retry`，不再停留在假正常 Chat。frontend console 仅有检测窗口内 `8` 行连接拒绝，5 秒后仍为 `8` 且不再增长；三路前端 SSE 随 gateway dispose 停止，独立 ssetap 的重试记录未混算为 UI 行为。正式证据=`testend/rig/formal-evidence/EDGE-179-search-first-download-shutdown-l3-fixed-20260830.md`，按 `A4` 写入 L3；旧红证据保留，L4/L5 仍开放。
- `EDGE-179|首用下载途中关停` L4 已基于同一新 session 的独立视觉复核收口。关停后 `67s..80s` 抽取的 `13` 张 1fps 稳定帧在 ROI=`200,150,2700,1500` 执行 `measure diff` 无输出；错误图标、标题、说明和 `Retry` 层级居中、留白均匀、无 clipping/overlap/reflow，正文/危险图标/按钮文字对比度分别为 `17.01:1/5.38:1/4.70:1`。正式证据=`testend/rig/formal-evidence/EDGE-179-search-first-download-shutdown-l4-real-app-20260830.md`，按 `C4` 写入 L4；L5 仍开放。
- `EDGE-179|首用下载途中关停` L5 已基于同一新 session 的从零路径收口。全新 workspace 经 onboarding 后，用户不读内部文档、不知道 `ANSELM_BACKEND_URL`，在真实 backend 失联后直接看到 `Can't reach the local engine`、清楚的不可用说明和 `Retry`；Computer Use AX 树、录屏与五通道 journal 一致。正式证据=`testend/rig/formal-evidence/EDGE-179-search-first-download-shutdown-l5-real-app-20260830.md`，按 `G1` 写入 L5；EDGE-179 五级现已完整收口，L3 红证据及修复历史仍保留。
- `EDGE-179` 的 L2 证据明确披露 App 录屏全程停在 onboarding；本轮不声称用户在 UI 中看到了下载进度或关停提示，因此 L3 的可见终态时序、L4 的视觉 craft、L5 的 discoverability 继续开放。L2 写账后的 `discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-179-ledger-alarm-reaudit-20260830.md` ack；`anchors.py check`=`10/10`、`alarms.py check`=`clean (2139 live judgments; 2300 baseline judgments excluded)`、`gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`。未改阈值、算法、法典、锚点、五级标准或顺序门；本批次由 `28→29/50`，未满 50 格不执行统一长门禁、不提交，当前 formal sequence 下一自主前线为 `EDGE|首用下载途中关停` L3，P12 的 400+ Journey 扩写继续按用户裁定推迟二期。
- 本轮修正了账本 provisional-NA 识别器的词序漏洞：`没有本格独立...` 原先未被识别为“尚缺现场证据”，导致一批 L3-L5 被错误视为已结算。新增规则与回归测试后，历史这些格子按真实状态重新开放；没有删除裁决、没有改五级标准，旧统计 `3794/446` 作废，以脚本重算的 `3703/537` 为当前唯一权威。随后真实 L5 发现按自然目标找到 MCP 后，模型误把它说成 workflow-only，已保留红证据并完成 stop-and-fix。
- `EDGE-174|MCP 进度关联` 已在正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-181342` 完成真实 App L2-L4：同一聊天回合经 `search_tools` 激活两个动态 MCP 工具，并以同一 `execution_group=1` 请求 `progress_alpha(label=alpha)` 与 `progress_beta(label=beta)`；messages/entities 两路 SSE 各自记录三条独有 progress，alpha/beta 无串台，App 最终展示两个结果。实际 MCP 投递在本轮观察为串行，证据只证明 per-call 关联，不冒充并发吞吐。L3 按 `B2`、L4 按 `C4` 收口；正式证据分别为=`testend/rig/formal-evidence/EDGE-174-mcp-progress-correlation-real-app-20260830.md`、`testend/rig/formal-evidence/EDGE-174-mcp-progress-correlation-l3-real-app-20260830.md`、`testend/rig/formal-evidence/EDGE-174-mcp-progress-correlation-l4-real-app-20260830.md`。
- `EDGE-174` L5 新开正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-184652` 发现真实产品失败：自然语言请求成功进入 App，模型用 `search_blocks` 找到两个 connected MCP tool，却错误告知用户只能放进 workflow，没有执行工具或返回 progress。红证据=`testend/rig/formal-evidence/EDGE-174-mcp-progress-correlation-l5-discovery-red-20260830.md`；L5 暂不计入结算，必须修复后以全新 session 重跑。
- `EDGE-174` L5 已在全新正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-190430` 收口：自然语言请求经 `search_tools` 找到并直接调用 `progress_alpha`，真实 MCP progress 三步与最终结果均到达 App；随后不重复执行工具，使用展示名 `edge174` 查询调用历史，返回唯一真实 `ok` 记录并与 backend、SSE、LLM wire 一致。录屏最终态、backend journal、frontend console、三路 SSE、managed LLM wire 均属于同一 manifest，未见应用级前端红线或数据矛盾；L5 按 `G1` 写入绿格，证据=`testend/rig/formal-evidence/EDGE-174-mcp-progress-correlation-l5-real-app-20260830.md`。此前两次红场与两次 stop-and-fix 保留为审计历史，不被覆盖。
- `EDGE-174` L5 红账后的 `pass-burst`、`discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-174-l5-discovery-red-ledger-alarm-reaudit-20260830.md` ack；`anchors.py check`=`10/10`，`alarms.py check`=`clean (2126 live judgments; 2300 baseline judgments excluded)`。未改阈值、算法、法典、锚点或顺序门。
- `EDGE-174` L5 修复后产生的 `discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-174-l5-ledger-alarm-reaudit-real-app-20260830.md` ack；最终 `anchors.py check`=`10/10`，`alarms.py check`=`clean (2127 live judgments; 2300 baseline judgments excluded)`。未改阈值、算法、法典、锚点或顺序门；本批次由 `17→18/50`，未满 50 格不执行统一长门禁、不提交。
- `EDGE-174` L3 写账后的 `discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-174-l3-ledger-alarm-reaudit-real-app-20260830.md` ack；`anchors.py check`=`10/10`，`alarms.py check`=`clean (2124 live judgments; 2300 baseline judgments excluded)`，阈值、算法、法典、锚点和顺序门未改。
- `EDGE-175|MCP 失败附 stderr 尾` 已在正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-182257` 完成真实 App L2：真实 stdio MCP 在 `tools/call` 中写入 180 行 stderr 后崩溃，App 明确显示 EOF 失败，MCP call detail 真实返回 `status=failed`、有界 server-level stderr tail 及“可能早于本次调用”标注。五通道证据=`testend/rig/formal-evidence/EDGE-175-mcp-stderr-tail-real-app-20260830.md`；L3-L5 仍开放，且 App 未内联完整 stderr 尾的边界已明确保留。
- `EDGE-175` L3 已在同一封存正式 session 收口：从调用到失败反馈的录屏抽样约 `100ms` 出现可见状态，`t=66..75s` 的 2fps 稳定尾段在正文 ROI 内 `measure diff` 零变化；早于稳定窗口的标题/内容收尾变化已明确披露并排除，未被藏进“稳定”结论。失败卡片、EOF 文案、assistant 解释和 Activity rail 收口后保持原位，按 `B2` 写入绿格，证据=`testend/rig/formal-evidence/EDGE-175-mcp-stderr-tail-l3-real-app-20260830.md`；L4-L5 仍开放。
- `EDGE-175` 写账后的 `gap-too-fast`、`discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-175-ledger-alarm-reaudit-real-app-20260830.md` 逐条 ack；`anchors.py check`=`10/10`，`alarms.py check`=`clean (2122 live judgments; 2300 baseline judgments excluded)`，阈值、算法、法典、锚点和顺序门未改。
- `EDGE-175` L3 写账触发的 `discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-175-l3-ledger-alarm-reaudit-real-app-20260830.md` ack；最终 `anchors.py check`=`10/10`，`alarms.py check`=`clean (2128 live judgments; 2300 baseline judgments excluded)`。未改阈值、算法、法典、锚点或顺序门；本批次由 `18→19/50`，未满 50 格不执行统一长门禁、不提交。
- `EDGE-175|MCP 失败附 stderr 尾` L4 在全新正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-202150` 收口。真实 App 默认态由中心工具卡承载失败标题、行动提示和折叠的 `Technical details`；助手正文只保留一次指向卡片的本地化交接句，Activity 只显示失败工具和人话摘要。Computer Use 显式展开技术详情后，原始 `tools/call`/`EOF` 才出现；stderr 尾、SSE error close、backend journal、frontend console 与 managed LLM wire 均属于同一 session，录屏已由 `rig-down.sh` 封口至 `152.375s`，无应用级前端错误。正式证据=`testend/rig/formal-evidence/EDGE-175-mcp-stderr-tail-l4-real-app-20260830.md`，按 `E1` 写入 L4；此前 L4 红证据保留，不被绿场覆盖。
- `EDGE-175` L4 写账后的 `discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-175-l4-ledger-alarm-reaudit-final-real-app-20260830.md` ack；`anchors.py check`=`10/10`、`alarms.py check`=`clean (2130 live judgments; 2300 baseline judgments excluded)`。复审未改阈值、算法、法典、锚点或顺序门；本批次由 `19→20/50`，未满 50 格不执行统一长门禁、不提交。
- `EDGE-031|回合收尾期单槽缓冲` 在正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-140457` 完成真实 App L2/L3：修复拒绝路径缺少 `message close` 后，真实并发 `409` 同时具备 durable assistant error、SSE `open→close(error)`、无第二次 LLM 请求，App 清除 generating 状态并显示明确错误。证据=`testend/rig/formal-evidence/EDGE-031-compaction-single-slot-buffer-real-app-20260830.md`；告警复审=`testend/rig/formal-evidence/EDGE-031-ledger-alarm-reaudit-20260830.md`。
- `EDGE-033|关页不留 streaming 孤儿` 在正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-150746` 完成真实 App L2：assistant 已 open 且产生 reasoning/tool block 后关闭窗口，录屏确认窗口消失，backend 在 detached context 完成 durable close，SSE 记录唯一 assistant close，LLM challenge/chat 全 200，frontend 无应用级红线。证据=`testend/rig/formal-evidence/EDGE-033-close-window-recovery-real-app-20260830.md`；清册为 `✓✓~~~`，L3-L5 未冒充通过，告警已按原阈值复审销账。
- `EDGE-033|关页不留 streaming 孤儿` 在同一正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-152501` 完成真实 App L3：streaming 期间 Composer 保留草稿、transcript 可滚动，完成后草稿仍在且回复完整；录屏、backend、三路 SSE、frontend console、managed LLM wire 全部属于同一 session。证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-152501/evidence/EDGE-033-streaming-ui-l3.md`；清册为 `✓✓✓~~`，L4-L5 未冒充通过。此前收台漏报的 `screen.mov` 已由 fail-closed 修复拦截，重收台后录屏 `3104x1844 / 371.686667s` 可读、进程组归零。
- 同场 L3 裁决触发的 `gap-too-fast`、`discovery-collapse` 已用独立复审证据=`testend/rig/formal-evidence/EDGE-033-streaming-l3-ledger-alarm-reaudit-20260830.md` 逐条 ack，最终 `alarms.py check` clean；阈值、法典和锚点未改。
- `EDGE-037|归档对话发消息自动解档` 在正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-153718` 完成真实 App L2：同一线程先被置为 archived，真实 Composer 发送后 notifications 记录 `archived→unarchived`，messages 开启新回合，最终 REST 返回 `archived=false` 且 App 回到活动 Recents。实际发送内容因 Computer Use `set_value` 未更新 Flutter 控制器而为旧草稿 `hive`，已在证据中披露，不扩张为输入内容通过；清册为 `✓✓~~~`，L3-L5 仍开放。
- `EDGE-037` 新裁决触发的 `gap-too-fast`、`discovery-collapse` 已以该 session 的五通道证据复审并 ack，最终 `alarms.py check` clean；阈值、法典和锚点未改。
- `EDGE-037|归档对话发消息自动解档` 在正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-154644` 完成真实 App L3：本次使用真实键盘路径输入 `EDGE037 L3 archive restore probe`，归档线程发送后即时显示用户气泡与 `thinking`，回复完成后 composer、历史内容和视口稳定保留。原始 60fps 局部测量在低阈值下首反馈为 `16.7ms`；较高阈值的正文首现为 `683.3ms`，不把粗粒度 2fps 的 `500ms` 量化结果冒充首反馈。稳定段未发现非用户触发的既有内容位移，按 `B2` 写入 L3；L4-L5 仍开放。证据=`testend/rig/formal-evidence/EDGE-037-archive-send-auto-unarchive-l3-20260830.md`。
- `EDGE-037` L3 新裁决后的 `gap-too-fast`、`discovery-collapse` 已用独立复审=`testend/rig/formal-evidence/EDGE-037-archive-send-auto-unarchive-l3-ledger-alarm-reaudit-20260830.md` 逐条 ack；`anchors=10/10`，最终 `alarms.py check` clean，阈值、法典、锚点和顺序门未改。清册由 `✓✓~~~` 提升为 `✓✓✓~~`，本批次由 `3→4/50`；未满 50 格不跑统一长门禁、不提交。
- `EDGE-038|:retry 重生成分支` 首轮真实五通道复验发现并修复了一个产品缺陷：含 `Context compacted` 的线程会把 assistant-shaped compaction marker 错当成重试目标，导致真实可见回答未被 supersede。`retryTargets` 现在排除只含 `compaction`/`marker` block 的 synthetic assistant row，并由 `TestRetryTargets_IgnoresSyntheticMarkers` 锁定；修复后真实 App 重试产生 `msg_e95d53244b2fd3fe`，其 `retryOf` 指向真实旧回答 `msg_a47649d7e916d11b`，旧回答的 `supersededBy` 指向新版本，marker 保持未改。正式证据=`testend/rig/formal-evidence/EDGE-038-retry-regenerate-compaction-marker-fixed-20260830.md`，红证据=`testend/rig/formal-evidence/EDGE-038-retry-regenerate-compaction-marker-red-20260830.md`；`judge.py` 以 `F2` 写入 L2，清册由 `✓~~~~` 提升为 `✓✓~~~`，L3-L5 仍开放。
- `EDGE-038` 新裁决触发的 `gap-too-fast`、`discovery-collapse` 已用独立复审=`testend/rig/formal-evidence/EDGE-038-ledger-alarm-reaudit-20260830.md` 逐条 ack；修复后的 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-160521` 已由 `rig-down.sh` 封存，messages durable seq=`1..6` 单调、LLM chat=`200`、backend/frontend 无应用红线，最终 `alarms.py check`=`clean (2112 live judgments; 2300 baseline judgments excluded)`。本批次由 `4→5/50`；未满 50 格不跑统一长门禁、不提交。
- `EDGE-038|:retry 重生成分支` 在修复后的正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-161502` 完成 L3：真实 App 点击 Retry 后 `100ms` 内出现可见反馈；高阈值下 `2.0s` 出现工具链内容，期间持续显示 `thinking`，没有静默等待；完成后的 20 秒 2fps 稳定尾段无超过 `0.0005` 的帧间变化。关键帧复读确认菜单收起、工具链展开和收尾没有既有内容非用户触发位移，按 `B2` 写入 L3；L4/L5 保持清册已有的明确不适用 `~`，不是 craft/discoverability 通过。正式证据=`testend/rig/formal-evidence/EDGE-038-retry-regenerate-l3-real-app-20260830.md`。
- `EDGE-038` L3 新裁决后的 `gap-too-fast`、`discovery-collapse` 已以独立复审=`testend/rig/formal-evidence/EDGE-038-l3-ledger-alarm-reaudit-20260830.md` 逐条 ack；`anchors=10/10`，最终 `alarms.py check`=`clean (2113 live judgments; 2300 baseline judgments excluded)`。清册由 `✓✓~~~` 提升为 `✓✓✓~~`，本批次由 `5→6/50`；未满 50 格不跑统一长门禁、不提交。
- `EDGE-039|:retry 编辑重发分支` 在正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-163538` 完成真实 App L2：修复后用户回合在 assistant 已存在时仍显示 `Edit and resend`，进入编辑态后点击 `Resend` 实际走 `POST ...:retry`，没有退化为普通 `POST .../messages`；REST 显示旧 user/assistant 行分别由 `supersededBy` 闭合，新 assistant 带 `retryOf`，旧行仍可在版本分页中寻址。Computer Use 的 AX `set_value` 未同步 Flutter 内部 controller，实际文本因此不是精确输入字符串；该限制已在证据中披露，本格只判可发现入口、传输与版本血缘，不扩张为输入内容通过。正式证据=`testend/rig/formal-evidence/EDGE-039-retry-edit-resend-fixed-20260830.md`，红证据=`testend/rig/formal-evidence/EDGE-039-edit-resend-normal-send-red-20260830.md`；`judge.py` 按 `F2` 写入 L2，清册由 `✓·~~~` 提升为 `✓✓~~~`，L3-L5 仍开放。
- `EDGE-039` 新裁决后的 `gap-too-fast`、`discovery-collapse` 已以独立复审=`testend/rig/formal-evidence/EDGE-039-ledger-alarm-reaudit-20260830.md` 逐条 ack；`anchors=10/10`，最终 `alarms.py check`=`clean (2114 live judgments; 2300 baseline judgments excluded)`。本批次由 `6→7/50`；未满 50 格不跑统一长门禁、不提交。当前下一自主前线为 `EDGE|:retry 编辑重发分支` L3，强制人工尾队首项仍为 `EDGE|视频轮询超时诚实话`。
- `EDGE-039|:retry 编辑重发分支` 在隔离正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-165831` 完成 L3：真实键盘将 `Reply exactly L3BASELINEOK` 修改为 `Reply exactly L3BASELINEOKEDITED`，编辑态稳定保留输入与 `Cancel/Resend`；点击重发后约 `716.7ms` 出现首可见变化，约 2 秒内收到精确回显，稳定尾段 20 秒主内容 ROI 无超过 `0.0005` 的帧间变化。REST 版本链、messages durable seq=`1..54`、notifications/entities 生命周期、managed LLM wire 均属于同一 session；正式证据=`testend/rig/formal-evidence/EDGE-039-retry-edit-resend-l3-real-app-20260830.md`。
- `EDGE-039` L3 新裁决后的 `gap-too-fast`、`discovery-collapse` 已以独立复审=`testend/rig/formal-evidence/EDGE-039-l3-ledger-alarm-reaudit-20260830.md` 逐条 ack；`anchors=10/10`，最终 `alarms.py check`=`clean (2115 live judgments; 2300 baseline judgments excluded)`。清册由 `✓✓~~~` 提升为 `✓✓✓~~`，本批次由 `7→8/50`；未满 50 格不跑统一长门禁、不提交。当前下一自主前线为 `EDGE|SSE 410 SEQ_TOO_OLD 重放` L2，强制人工尾队首项仍为 `EDGE|视频轮询超时诚实话`。
- `EDGE-056|SSE 410 SEQ_TOO_OLD 重放` 在正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-171858` 完成真实 App L2：基线回合产生 messages durable seq=`1..8` 后，真实 App 的 messages SSE 被受控断开；通过真实 REST 产品入口并发建立 `80` 个对话并发送消息，使独立 ssetap 的 durable 游标推进到 `392`，越过生产环形缓存。重连由后端实际返回 HTTP `410`，正文为 `SEQ_TOO_OLD`；App 随后重拉 messages、touchpoints、interactions、todos、conversations 并恢复可用。五通道均属于同一封存 session，正式证据=`testend/rig/formal-evidence/EDGE-056-sse-seq-too-old-real-app-20260830.md`，按 `measure:edge056-sse-seq-too-old` 写入 L2，L3-L5 对该传输恢复边界明确不适用并保持 `~`。
- `EDGE-056` 写账后的 `gap-too-fast`、`discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-056-ledger-alarm-reaudit-real-app-20260830.md` 串行 ack；复审确认五通道原始证据、真实 410 和后端恢复链完整，未修改阈值、法典、锚点或顺序门。最终 `anchors.py check`=`10/10`、`alarms.py check`=`clean (2116 live judgments; 2300 baseline judgments excluded)`，`gen_coverage.py --check`=`848/848/0`。本批次由 `8→9/50`，未满 50 格不执行统一长门禁、不提交；当前下一自主前线为 `EDGE|每租户模板 URL` L2。
- `EDGE-168|每租户模板 URL` 已在真实 App MCP marketplace 打开安装计划并确认：表单展示必填 `GLEAN_MCP_URL`、示例格式和 `Connect & authorize`；该按钮会启动浏览器 OAuth 并创建持久授权，属于必须在动作时确认的安全交互。未输入租户 URL、未点击授权、未落盘 server 或 OAuth grant，不写任何绿格；本事实记录在=`testend/rig/formal-evidence/EDGE-168-real-app-forced-auth-boundary-20260830.md`，并将该项补入 `forced_queue`。因此清册仍为 `✓~~~~`，不计入本批次；顺序门下一自主前线改为 `EDGE|MCP degraded 态` L2。
- `EDGE-169|MCP degraded 态` 在正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-173628` 完成真实 App L2：通过真实 REST 产品入口接入本地受控 stdio MCP，连续三次 `boom` 失败后 App MCP 面板显示橙色 `degraded · 2 tools · 3 calls` 与可读错误；entities SSE 同步记录 `ready→degraded`。随后 `echo` 在 degraded 状态仍成功，REST 与 App 同步恢复 `ready`，entities SSE 记录 `degraded→ready`；正式证据=`testend/rig/formal-evidence/EDGE-169-mcp-degraded-real-app-20260830.md`，按 `measure:edge169-mcp-degraded` 写入 L2，L3-L5 保持开放。
- `EDGE-169` 写账后的 `gap-too-fast`、`discovery-collapse` 已用独立复审=`testend/rig/formal-evidence/EDGE-169-ledger-alarm-reaudit-real-app-20260830.md` 串行 ack；五通道、真实错误路径和恢复证据齐全，未修改阈值、法典、锚点或顺序门。最终 `anchors.py check`=`10/10`、`alarms.py check`=`clean (2117 live judgments; 2300 baseline judgments excluded)`，`gen_coverage.py --check`=`848/848/0`。本批次由 `9→10/50`，未满 50 格不执行统一长门禁、不提交；当前下一自主前线为 `EDGE|MCP 连接失败仍落盘` L2。
- `EDGE-170|MCP 连接失败仍落盘` 在正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-174239` 完成真实 App L2：坏的 stdio 命令经真实 `PUT` 后持久化为 `failed`，App 面板显示 `1 servers · 1 failed`、初始化错误和失败卡片；恢复同一路径后调用真实 `:reconnect`，状态回到 `ready`，App 显示 `1 servers · 1 ready`，真实 `echo` 调用成功。正式证据=`testend/rig/formal-evidence/EDGE-170-mcp-failed-persists-real-app-20260830.md`，按 `measure:edge170-mcp-failed-persists` 写入 L2，L3-L5 保持开放。
- `EDGE-170` 写账后的 `gap-too-fast`、`discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-170-ledger-alarm-reaudit-real-app-20260830.md` 串行 ack；真实失败持久化、App 卡片、reconnect 恢复和五通道封口均已复核，未修改阈值、法典、锚点或顺序门。最终 `anchors.py check`=`10/10`、`alarms.py check`=`clean (2118 live judgments; 2300 baseline judgments excluded)`，`gen_coverage.py --check`=`848/848/0`。本批次由 `10→11/50`，未满 50 格不执行统一长门禁、不提交；当前下一自主前线为 `EDGE|MCP 媒体逐件 best-effort` L2。
- `EDGE-171|MCP 媒体逐件 best-effort` 在正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-175550` 完成真实 App L2：第一轮真实调用发现并修复 MCP 卡片的英文单数文案 `1 calls`，第二轮用全新构建和数据目录重跑后显示 `ready · 1 tools · 1 call`。真实 MCP 返回 PNG、MP3 和一件超出 `50 MiB` 单附件上限的 PNG；前两件落为一等附件且字节 SHA-256 与 REST 行一致，失败项保留 `[image: image/png]` 占位，整次调用仍为 HTTP `200`、MCP 审计仍为 `ok`。正式证据=`testend/rig/formal-evidence/EDGE-171-mcp-media-best-effort-real-app-20260830.md`，按 `measure:edge171-mcp-media-best-effort` 写入 L2，L3-L5 保持开放。
- `EDGE-171` 写账后的 `gap-too-fast`、`discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-171-ledger-alarm-reaudit-real-app-20260830.md` 串行 ack；修复前红场、修复后真实 App、五通道证据、focused backend/frontend 回归均已复核，未修改阈值、法典、锚点或顺序门。最终 `anchors.py check`=`10/10`、`alarms.py check`=`clean (2119 live judgments; 2300 baseline judgments excluded)`，`gen_coverage.py --check`=`848/848/0`。本批次由 `11→12/50`，未满 50 格不执行统一长门禁、不提交；当前下一自主前线为 `EDGE|MCP 媒体无 uploader` L2。

自动回归复验：`EDGE-334` 的四个真实 `Kill9` 场景、`EDGE-335/337` harness race、`EDGE-338` 关闭回环网关隔离均通过；相邻后端核心包、黑盒生成/语音缺席与 workflow/chat fork 场景、前端设置/语音/转录定向测试也通过。详见 `testend/rig/formal-evidence/EDGE-334-338-current-regression-20260830.md`。随后五行台架维度 L2-L5 以具体适用性理由记为 `na`，本批达到 `47/50`；独立告警复审=`testend/rig/formal-evidence/EDGE-334-338-ledger-alarm-reaudit-20260830.md`，formal `alarms.py check` clean。人工产品格仍不越级放行。
...
非正式 smoke：当前代码经 `make -C frontend app` 启动后，用真实键盘路径完成一次受管网关 Chat，创建新线程并得到 `READY`；backend 记录 `POST /conversations=201`、`POST /messages=202` 及正常收尾，随后只读巡检 Library/Entities/Scheduler 无新增应用级红线。该 smoke 不写 COVERAGE，不替代正式五通道 L2/L3。

本轮台架修复：`rig-down.sh` 在启用录屏时对缺失、空或不可读的 `screen.mov` fail-closed；仅 `RIG_RECORD=0` 生成的 `recording.disabled` 诊断态允许无 MOV，且不能进入正式五通道裁决。`test_screen_recording.py` 共 `6/6` 通过。
...
- 这不是降低标准，也不是把格子判为 `na`：人工格仍保持未完成，最终必须用真实动作和原五通道证据收口。精确队列由 `testend/rig/ledger-sequence.json` 的 `manual_queue` 控制，损坏即 fail-closed；当前登记的人工尾队包含 `EDGE-223|视频轮询超时诚实话`、`EDGE-224|不可能的生成组合钳制`、`EDGE-225|能力工具诚实缺席`、`EDGE-226|受管档视频路由`、`EDGE-227|语音配额与限流分流`、`EDGE-228|ASR sidecar 无受管凭证`、`EDGE-229|多块 TTS PCM 拼接`、`EDGE-230|ParseWAV 遍历 chunk 表`、`EDGE-231|断网启动`、`EDGE-056|SSE 410 SEQ_TOO_OLD 重放`、`EDGE-168|每租户模板 URL`，以及其余既有后置项。
- `judge.py` 的顺序门只跳过 `forced_queue` 中有明确条目的格，先推进其他自主格；自主格耗尽后自动回到强制人工队列。历史 `manual_queue` 中已释放的普通现场项可以正常收口，不会因仍保留候选记录而阻塞；现有 `~` 裁决仍表示已记录的不适用，不会被调度规则重解释。
- `manual_queue` 对 `(family,item)` 强制唯一；`forced_queue` 是其中需要用户强制交互的显式子集，必须逐项引用 `manual_queue`，两队重复登记或悬空引用均 fail-closed。普通真实 App 现场项不再因历史 SecurityAgent 阻断而永久后置，由 Computer Use 自主推进；两队都不改变五级标准。回归锁定在 `testend/rig/test_judge.py` 的 `test_sequence_policy_rejects_duplicate_manual_items`、`test_forced_queue_releases_ordinary_ui_items` 与 `test_forced_queue_rejects_item_outside_manual_queue`。
- provisional-N/A 词表覆盖“没有独立 formal rig 五通道 session”这类缺证据措辞；这类格子必须重新进入人工尾队，不能被误算为已结算。
- 台架回归：`python3 -m unittest testend/rig/test_judge.py -q` 的 `27/27`、JSON 校验、`py_compile`、`git diff --check` 均通过；锚点重新校准为 `10/10`，`alarms.py check` clean。当前批次由既有 `50/50` 重新起算为本次 `1/50`；未满 50 格不跑统一长门禁、不提交。标准、法典、证据要求和最终收口条件不变。

## 历史前线快照（不覆盖顶部当前精确状态）

以下条目保留此前批次的审计上下文；当前前线、批次和人工尾队只以上方“当前精确状态”和
`ledger-sequence.json` 的实时重算为准。

- **最新精确快照（EDGE-190 产品现场后置，按现行 judge.py 重算）**：`848` 行、`74` 个人工后置项、`106` 个自主未收口行、`224` 个自主未收口单元；`668` 行已结算。下一自动前线为 `EDGE-191|附件 sandbox 提取路径`。`EDGE-056`、`EDGE-168` 至 `EDGE-190` 均保留 L1 与未完成现场等级，不以被 `SecurityAgent/CoreServicesUIAgent` 遮挡的 session 冒充正式证据。明确 API-only 的 `EDGE-099` 至 `EDGE-105` 仍按各自适用性理由结算，不被缺现场 marker 误拦。未把任何现场缺失改写成 `na`，也未降低五级标准；以本条、`judge.py` 和 `ledger-sequence.json` 为准。
- 本批次由 `23/50` 推进至 `27/50`：新增 1 个真实 L2 与 3 个明确适用性 `na` 单元；告警已独立复审销账，未修改阈值、算法、法典、锚点或顺序门。正式证据=`testend/rig/formal-evidence/EDGE-222-llm-generation-origin-real-app-20260830.md`，告警复审=`testend/rig/formal-evidence/EDGE-222-ledger-alarm-reaudit-20260830.md`，session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-052929`。
- `EDGE-277|文档改名子树级联` 的 `TestUpdate_PathCascade` 普通/race 与真实黑盒 `TestContractDocsAtt_DocumentNameGuardsSoftDelete` 普通/race 回归通过；重命名根节点会一致重写全部后代 `path`。真实 Library 的树刷新、路径反馈、视觉稳定性和发现性仍需五通道现场观察，故整项进入人工后置队列，不作 `na` 或 `pass` 降级。独立复核=`testend/rig/formal-evidence/EDGE-277-ledger-alarm-reaudit-20260830.md`；下一自动前线为 `EDGE-278|文档 Move 防环`。
- `EDGE-279|对话挂载的文档被删` 保留既有真实 App 五通道 `L2:F1` 证据；缺失文档警告事实已验证，但 `L3-L5` 仍缺独立顺滑、视觉 craft 和可发现性结论，故转入人工后置队列，不重复现场操作、不把临时 `na` 当完成。独立复核=`testend/rig/formal-evidence/EDGE-279-ledger-alarm-reaudit-20260830.md`；下一自动前线为 `EDGE-280|agent 知识文档被删`。
- `EDGE-280|agent 知识文档被删` 保留既有真实 App 五通道 `L2:F1`，并新增 service/黑盒普通与 race 回归确认删除后的 agent invoke 明确失败且不发 LLM 请求；`L3-L5` 仍缺独立错误反馈、视觉 craft 和可发现性结论，故进入人工后置队列。独立复核=`testend/rig/formal-evidence/EDGE-280-ledger-alarm-reaudit-20260830.md`；下一自动前线为 `EDGE-281|skill 安装炸弹护栏`。
- `EDGE-281|skill 安装炸弹护栏` 的 skillfetch/skill service 与真实 HTTP 安装主链普通/race 回归通过；symlink/越界条目丢弃、4097 条目和超过 200 MiB 解压量拒绝已锁定，但真实 App 的预览、阻断反馈、等待、视觉和发现性仍后置。独立复核=`testend/rig/formal-evidence/EDGE-281-ledger-alarm-reaudit-20260830.md`；下一自动前线为 `EDGE-282|skill 本地改动漂移`。
- `EDGE-282|skill 本地改动漂移` 的 skill service 与真实 HTTP 安装更新主链普通/race 回归通过；非 force 拒绝漂移、force 恢复上游，工具授权变更重置而未变更保留信任门；真实 App 的冲突提示、恢复选择、视觉和发现性仍后置。独立复核=`testend/rig/formal-evidence/EDGE-282-ledger-alarm-reaudit-20260830.md`；下一自动前线为 `EDGE-283|skill 路径穿越`。
- `EDGE-283|skill 路径穿越` 的 skill filesystem/service 与 skill 文件 HTTP 合同普通/race 回归通过；词法、Clean、`os.Root`、symlink 和 manifest 路由防护已锁定，但真实 App 的文件操作阻断反馈、文案、视觉和发现性仍后置。独立复核=`testend/rig/formal-evidence/EDGE-283-ledger-alarm-reaudit-20260830.md`；下一自动前线为 `EDGE-284|skill 清单拒删`。
- `EDGE-284|skill 清单拒删` 的 skill service 与 skill 文件 HTTP 合同普通/race 回归通过；files API 删除 `SKILL.md` 返回 `SKILL_FILE_PATH_INVALID` 且 skill 保持完整，但真实 App 的阻断反馈、入口引导、视觉和发现性仍后置。独立复核=`testend/rig/formal-evidence/EDGE-284-ledger-alarm-reaudit-20260830.md`；下一自动前线为 `EDGE-285|大小写不敏感 FS 上的 skill.md`。
- `EDGE-285|大小写不敏感 FS 上的 skill.md` 的 filesystem 普通/race 回归通过；小写回退可读、macOS 同 inode 不自删、大小写敏感盘清退独立残件，真实 App 的跨平台反馈、视觉和发现性仍后置。独立复核=`testend/rig/formal-evidence/EDGE-285-ledger-alarm-reaudit-20260830.md`；下一自动前线为 `EDGE-286|skill 目录前导兜底`。
- `EDGE-286|skill 目录前导兜底` 的 skill service 与 directory HTTP 合同普通/race 回归通过；单文件不加前导、捆绑文件注入目录、占位符和 agent Guide 规则一致，但真实 App 的引导文案、视觉和发现性仍后置。独立复核=`testend/rig/formal-evidence/EDGE-286-ledger-alarm-reaudit-20260830.md`；下一自动前线为 `EDGE-287|run_skill_script 扩展名不支持`。
- `EDGE-287|run_skill_script 扩展名不支持` 的真实 chat ReAct 黑盒普通/race 回归通过；不存在/越界脚本都被拒绝并回喂模型、不进入执行，但真实 App 的扩展名错误反馈、文案、视觉和发现性仍后置。独立复核=`testend/rig/formal-evidence/EDGE-287-ledger-alarm-reaudit-20260830.md`；下一自动前线为 `EDGE-288|fork skill 无 runner`。
- `EDGE-288|fork skill 无 runner` 的 skill service 与激活 HTTP 合同普通/race 回归通过；缺 runner、缺 agent 和非法 agent 均明确拒绝，但真实 App 的不可用反馈、文案、视觉和发现性仍后置。独立复核=`testend/rig/formal-evidence/EDGE-288-ledger-alarm-reaudit-20260830.md`；顺序门重算后的当前自动前线为 `EDGE-222|生成 origin 从凭证派生`。
- `EDGE-289|@ 一个 fork skill` 的后端 skill service 普通/race 与前端 `EntityMentionSource` 定向回归通过；inline skill 保留在 @ 候选，fork skill 被排除且不获得父回合预授权。真实 App 的候选排序、排除反馈、视觉 craft、文案和发现性仍后置人工队列。独立复核=`testend/rig/formal-evidence/EDGE-289-skill-fork-mention-20260830.md`；该项已登记人工后置，不改变当前自动前线 `EDGE-222|生成 origin 从凭证派生`。
- 本轮 gate 修复证据=`testend/rig/formal-evidence/WRK-087-judge-measure-provisional-20260830.md`；`testend/rig/test_judge.py` 普通回归 `21/21` 通过，新增中文 provisional 变体回归，measure 指针后的 provisional note 现在会重新打开顺序门，明确适用的 `na` 仍保持 settled。
...
- 新语义扫描当前清册后，仍有 `74` 个自主未收口项目行（`222` 个未收口单元）；人工队列中的 `EDGE-031|回合收尾期单槽缓冲`、`EDGE-030|生成中再 Send`、`EDGE-033|关页不留 streaming 孤儿`、`EDGE-037|归档对话发消息自动解档`、`EDGE-038|:retry 重生成分支`、`EDGE-039|:retry 编辑重发分支`、`EDGE-251|删最后一个 workspace`、`EDGE-254|keyset 排序切换丢游标`、`EDGE-256|驻地目录被移走`、`EDGE-257|脏区切分支被拒`、`EDGE-258|新建分支不受脏区门`、`EDGE-259|切分支名拼错`、`EDGE-261|worktree 目录已存在`、`EDGE-262|worktree 分支已存在`、`EDGE-263|worktree 建成后切驻地失败`、`EDGE-264|「这里没有 git」四情形`、`EDGE-265|切驻地落 marker 块`、`EDGE-266|空线程/重复 PATCH 不落 marker`、`EDGE-267|切分支不落 marker`、`EDGE-268|驻地分组批量归档重跑`、`EDGE-269|驻地分组批量删除范围`、`EDGE-270|空 workDir 批量动作`、`EDGE-272|分组计数跨翻页不漂移`、`EDGE-273|?workDir= 三态 presence`、`EDGE-274|立碑线程读消息`、`EDGE-275|文档超 1MB`、`EDGE-276|并发同父建文档` 与 `EDGE-329|快捷键录制后吞键` 继续后置。此前尝试越过 `EDGE-251` 直接写 EDGE-040 被正式顺序门拒绝；`EDGE-253|单连接 panic 事务砖化` 与 `EDGE-255|PageAsc collation 不一致` 已完成普通/race 回归及四格适用性复核，`EDGE-254` 的 L2 已完成契约验证、L3-L5 留在人工队列，`EDGE-256/257/258/259` 保留完整 L2-L5 现场验收；`EDGE-260|前导 - 的合法 ref` 已完成安全 seam 的普通/race 与黑盒回归，并以具体适用性理由收口 L2-L5，未把上层分支错误体验冒充完成；`EDGE-261|worktree 目录已存在` 的普通/race 与黑盒回归通过，但用户可见冲突反馈、视觉和发现性保留人工验收；`EDGE-262|worktree 分支已存在` 的普通/race 与黑盒回归通过，恢复路径的用户反馈、视觉和发现性保留人工验收；`EDGE-263|worktree 建成后切驻地失败` 的故障注入、普通/race 与黑盒回归通过，但半成功状态的用户反馈、恢复指引、视觉和发现性保留人工验收；`EDGE-264|「这里没有 git」四情形` 的普通/race 与黑盒三态写动作回归通过，未宣称当前机器上未复现的无 Git 可执行文件变体已通过；`EDGE-265|切驻地落 marker 块` 的普通/race 与真实黑盒回归通过，但历史 marker 的用户呈现、视觉和发现性保留人工验收；`EDGE-266|空线程/重复 PATCH 不落 marker` 的普通/race 与真实黑盒回归通过，但无痕 no-op 的用户反馈、视觉和发现性保留人工验收；`EDGE-267|切分支不落 marker` 的普通/race 与真实黑盒回归通过，但切分支后的用户反馈、视觉和发现性保留人工验收；`EDGE-268|驻地分组批量归档重跑` 的普通/race 与真实黑盒回归通过，但批量归档的确认反馈、视觉和发现性保留人工验收；`EDGE-269|驻地分组批量删除范围` 的普通/race 与真实黑盒回归通过，但破坏性批量删除的确认、反馈、视觉和发现性保留人工验收；`EDGE-270|空 workDir 批量动作` 的普通/race 与黑盒回归通过，但阻断反馈、文案、视觉和发现性保留人工验收；`EDGE-271|分组事务交叉核对` 的普通/race 回归通过，ABORT/IGNORE 两种半写均整批回滚，L2-L5 以无独立产品表面的具体适用性理由收口；`EDGE-272|分组计数跨翻页不漂移` 的普通/race 与黑盒跨页回归通过，但 rail 计数的滚动反馈、视觉稳定性和发现性保留人工验收；`EDGE-273|?workDir= 三态 presence` 的普通/race 与黑盒筛选回归通过，但筛选切换、空态反馈、视觉稳定性和发现性保留人工验收；`EDGE-274|立碑线程读消息` 的普通/race 与黑盒删除回归通过，但删除后的深链墓碑呈现、反馈、视觉和发现性保留人工验收；`EDGE-275|文档超 1MB` 的普通/race 与黑盒文档守卫回归通过，但超限反馈、文案、视觉和发现性保留人工验收；`EDGE-276|并发同父建文档` 的普通/race 与黑盒文档树回归通过，但并发结果排序、列表反馈、视觉秩序和发现性保留人工验收。下一自动前线为 `EDGE-277|文档改名子树级联`。
- `EDGE-271|分组事务交叉核对` 的 `TestWorkDirActions_AreAllOrNothing` 普通/race 回归通过，覆盖 archive/delete 的 `RAISE(ABORT)` 行错误与 `RAISE(IGNORE)` 静默跳过；事务错误时行、广播和关系级联均保持不变。该项是服务端事务 seam，不拥有独立用户状态、交互、视觉或发现入口，L2-L5 已以具体适用性理由收口 `na`。独立复核=`testend/rig/formal-evidence/EDGE-271-ledger-alarm-reaudit-20260830.md`；四格写入后告警复核最终 `alarms.py check` clean（`2079` live judgments，`2300` baseline 排除），下一自动前线为 `EDGE-272|分组计数跨翻页不漂移`。
...
- `EDGE-269|驻地分组批量删除范围` 的 conversation service 普通/race 与真实黑盒 `TestChatWorkDirGroups_DeleteWholeGroup` 普通/race 回归通过；删除跨归档态覆盖未置顶目标、保留置顶项，消息行与文件系统均不被改动。破坏性批量删除的真实 App 确认、反馈、视觉和发现性仍需五通道现场观察，故整项进入人工后置队列，不作 `na` 降级。独立复核=`testend/rig/formal-evidence/EDGE-269-ledger-alarm-reaudit-20260830.md`；下一自动前线由顺序门重新计算。
- `EDGE-270|空 workDir 批量动作` 的 conversation service 普通/race 与真实黑盒 `TestChatWorkDirGroups_ArchiveWholeGroup` 普通/race 回归通过；`workDir:""` 返回 `400 INVALID_REQUEST`，不会被误解释为“未挂载对话”分组。该阻断路径的真实 App 文案、反馈、视觉和发现性仍需五通道现场观察，故整项进入人工后置队列，不作 `na` 降级。独立复核=`testend/rig/formal-evidence/EDGE-270-ledger-alarm-reaudit-20260830.md`；下一自动前线由顺序门重新计算。
...
- 自主验证余量：后端与前端 `make verify` 均全绿（前端 `5448` tests）；完整 `testend`、台架 `test_judge`、JSON、Python 编译、coverage clean、anchors `10/10`、alarms clean 均通过。当前批次仍为 `47/50`，尚未触发新的统一长门禁提交；未完成的真实 App/系统交互格仍保持在人工队列，不被自动化绿灯冒充完成。
...
EDGE-196 的真实产品复核确认受管媒体以 resumable upload 后的短期相对 lease 进入模型，用户只需使用 Chat 中可见的附件入口，不接触内部 upload/lease/API；四次写账的统计警报均按原算法逐项独立复核/ack，最终 `alarms.py check`=`clean (2212 live judgments; 2300 baseline judgments excluded from drift curves)`；`gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`，anchors=`10/10`。统一门禁=`testend/rig/formal-evidence/batch-85-unified-gate-20260831.md`。未改告警阈值、法典、锚点、五级标准或顺序 gate。

`EDGE-199|代理图未 ready` 的正式 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-084340`：台架仅设置 `ANSELM_RIG_MEDIA_PROCESS_DELAY_MS=15000` 延迟真实媒体处理；真实 App 在 chip 仍显示 `Preparing media…` 时发送 `12000x12000`、`2995581` bytes 的 JPEG，LLM wire 收到同尺寸的原图 staging，不是之后生成的 `943140` bytes、`2048x2048` 代理。约 15 秒后 derivative 才 durable `ready`，App 正常完成回复；五通道、`rig-check`、`rig-down` 均通过。L2-L5 证据=`sessions/20260831-084340/evidence/EDGE-199-attachment-proxy-not-ready-green.md`，法条=`F2/A4/C4/G1`；同场四格写账触发的 `gap-too-fast` 已由=`testend/rig/formal-evidence/EDGE-199-real-app-ledger-alarm-reaudit-20260831.md` 复审并 ack，未改任何阈值或标准。前一轮 PNG 仅命中 ready 路径的记录=`testend/rig/formal-evidence/EDGE-199-real-app-ready-not-fallback-20260831.md`，不与本次回退证据混淆。

`EDGE-194|单回合媒体额度耗尽` 的正式 session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260831-031039`：真实 App 通过 macOS 文件选择器附加 8 个编号图片和第 9 个不同命名图片；LLM wire 的 user 内容有 8 个 `image_url`，第 9 个原始位置是含 `edge194-final-item.png` 的额度占位，最终回答逐字保留该文件名，没有按序号臆造。SSE 记录 9 个附件 touchpoint、reasoning/text open-delta-close 和 completed assistant close；backend/frontend 无应用级异常，录屏末帧清楚显示 9 个缩略图和正确回答，收台后进程归零。L2-L5 证据=`sessions/20260831-031039/evidence/EDGE-194-L{2,3,4,5}.md`，法条=`F2/A4/C4/G1`；产品修复复盘=`testend/rig/formal-evidence/EDGE-194-media-envelope-real-app-20260831.md`。

EDGE-194 的真实 stop-and-fix 修复了额度占位文件名被模型按序号推导的产品缺陷：`prompt.go` 增加权威文件名规则，`attachment.go` 的 item/size budget 注记明确原始位置并禁止推断/改名；focused Go 回归及真实 App 五通道复验通过。四次写账的统计警报均按原算法逐项独立复核/ack，最终 `alarms.py check`=`clean (2204 live judgments; 2300 baseline judgments excluded from drift curves)`；`gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`，anchors=`10/10`。未改告警阈值、法典、锚点、五级标准或顺序 gate。

`EDGE-193|模型能力缺失诚实降级` 的正式 session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260831-024530`：真实 App 通过 macOS 文件选择器附加图片，使用无 vision 的 `qwen2.5:0.5b`，最终直接回答当前模型不能查看图片像素，并给出切换视觉模型或描述/粘贴内容的可行动下一步。稳定尾帧同时显示附件卡、原问题和完整回答；SSE user close→assistant open/text/close 完整，LLM wire 无 `image_url`/`images` 或图片字节，backend/frontend 无应用级异常，收台后进程归零。L2-L5 证据=`sessions/20260831-024530/evidence/EDGE-193-L{2,3,4,5}.md`，法条=`F2/A4/C4/G1`；产品修复复盘=`testend/rig/formal-evidence/EDGE-193-no-vision-degrade-real-app-20260831.md`。

EDGE-193 的真实 stop-and-fix 修复了无视觉模型只复述限制、泛化附件确认和不明确下一步三个产品问题：降级指令现在位于用户问题之前，明确禁止臆测、重新上传、声称可访问文件和无关客套；focused Go/Flutter 回归及真实 App 复验均通过。四次写账触发的统计警报均按原算法逐项独立复核/ack，最终 `alarms.py check`=`clean (2200 live judgments; 2300 baseline judgments excluded from drift curves)`；未改告警阈值、法典、锚点、五级标准或顺序 gate。

`EDGE-192|不认的 mime 抽取` 的 L2-L5 使用正式 session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260831-014702`：真实无效 `.odt` 走 unsupported MIME 分支，App 显示明确的转换/粘贴/PDF 替代路径；LLM wire 无实际 `tool_calls`，原始 fixture 字节未出线，backend、SSE、managed gateway、frontend 和录屏均归属同一 session。L2-L5 证据=`sessions/20260831-014702/evidence/EDGE-192-L{2,3,4,5}.md`，法条=`F2/A4/C4/G1`；前端 `LibreOffice`/`EOF` 误判已修复并有回归测试。四个等级写账后的警报均由=`testend/rig/formal-evidence/EDGE-192-ledger-alarm-reaudit-real-app-20260831.md` 复审并 ack，最终 `alarms.py check`=`clean (2196 live judgments; 2300 baseline judgments excluded)`。

`EDGE-192|不认的 mime 抽取` 的 L2 使用正式 session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260831-014702`：真实无效 `.odt` 走 unsupported MIME 分支，App 显示明确的转换/粘贴替代路径；LLM wire 无实际 `tool_calls`，原始 fixture 字节未出线，backend、SSE、frontend、managed gateway 和录屏均归属同一 session。此前 `LibreOffice` 被前端无边界 `EOF` 检测误判为失败 handoff，已改为 token-boundary 判断并通过前端回归；正式证据=`sessions/20260831-014702/evidence/EDGE-192-L2.md`，按 `F2` 写入 `✓`。L2 写账后的警报复审=`testend/rig/formal-evidence/EDGE-192-ledger-alarm-reaudit-real-app-20260831.md`，最终 `alarms.py check`=`clean (2193 live judgments; 2300 baseline judgments excluded)`；L3-L5 不复用本格证据。

`EDGE-191|附件 sandbox 提取路径` 的正式 session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260831-012918`：真实 App 通过 macOS 文件选择器上传 `.docx`，共享 Python sandbox 状态由 `installing` 到 `ready`；LLM wire 明确包含 `text-extracted, truncated` 与头部哨兵，不包含 `400K` 后尾部哨兵。真实 App 追问结果为 `HEAD_SENTINEL: 是`、`TAIL_AFTER_400K_SENTINEL: 否`、`截断: 是`，backend、SSE、frontend console、managed LLM tap 与 `182.481667s/60fps` 录屏均属于同一 session，收台后进程归零。L2 证据=`sessions/20260831-012918/evidence/EDGE-191-L2.md`，按 `F2` 写入 `✓`；首轮模型未直接回答三问的偏差已保留在证据中，没有被冒充成完美体验。L2 写账后的两条统计警报已由=`testend/rig/formal-evidence/EDGE-191-ledger-alarm-reaudit-real-app-20260831.md` 独立复审并 ack，最终 `alarms.py check`=`clean (2189 live judgments; 2300 baseline judgments excluded)`，anchors=`10/10`，`gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`。

`EDGE-184|短词 LIKE 回退` 已完成同一正式真实 App session 的 L2-L5 收口：真实 lexical search 在 `embedder=off` 下对两字母 `qx` 走 LIKE 回退并返回 `<mark>qx</mark>`，对 `forecast qx` 走长 token MATCH + 短 token LIKE 合取，只返回 `QX deployment note`。App 从普通 Chat 入口先找到短 token，再筛选同时含两个 token 的文档并解释排除项；中文 `天气` 的 Computer Use 输入桥接实验没有被冒充为现场结果。该 session 的 `screen.mov`=`263.588333s`，messages durable 推进至 `seq=60`，三路 SSE、backend、frontend、managed LLM wire 均非空；正式 L2=`F2`、L3=`A4`、L4=`C4`、L5=`G1` 证据已封存。本批次已达到 `52/50`，统一门禁首轮发现前端回归后已修复，第二轮根门禁已通过并封存；下一格恢复为 `EDGE|每租户模板 URL` L2。

`EDGE-183|换 embedder 重嵌` 已完成同一正式真实 App session 的 L2-L5 收口：`embedder=ollama` 使用真实 Ollama `embeddinggemma:latest`，`search_embeddings` 中当前模型覆盖 `35/35` 个现存文档；旧 builtin 的单条陈旧向量不对应任何现存 `search_docs`，当前查询不会混用。真实 App 在未暴露内部 embedder 术语的情况下执行 `search_documents`→`read_document`，从“找到关于 calm/coherent/trustworthy 界面品质的说明并解释其重要性”的用户目标召回并回答目标文档；未找到的过于笼统查询仍诚实显示 0 结果。该 session 的 `screen.mov`=`356.286667s`，三路 SSE、backend、frontend、managed LLM wire 均非空且收台后进程归零；正式 L2=`F2`、L3=`A4`、L4=`C4`、L5=`G1` 证据已封存。四次写账后的警报状态需在本批次 50 格统一门禁中复核，不以当前写账动作替代复核。

`EDGE-182|cosineFloor 噪声闸` 的红场、修复与 green session 已全部封存。红 session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260830-221606` 证明真实 `search` 会把自然乱码召回为无关实体；修复后的 fresh session=`/private/tmp/anselm-rig-formal-20260801-7/sessions/20260830-222330` 证明自然乱码空结果与不同措辞语义召回同时成立。L2=`F2`、L3=`B2`、L4=`C4`、L5=`G1` 均已写入 `✓`；红证据=`testend/rig/formal-evidence/EDGE-182-search-cosine-floor-noise-gate-l2-red-real-app-20260830.md`，绿证据与警报独立复审均已留档。`pass-burst` 仅由四级连续账本写入触发，已在重读五通道与固定锚点后 ack；没有修改标准或用警报复审替代产品观察。

`EDGE-179` L2 的当前证据边界是：真实首次下载窗口确实处于 `downloading`，SIGTERM 后 installer 收到 `context canceled`，搜索 provider 诚实退回 lexical，sidecar 在约 `118ms` 内收口；同一 session 的 manifest、backend、SSE、frontend、LLM wire 和 screen.mov 均通过 L2 gate。录屏没有离开 onboarding，因此没有把后端状态伪装成界面状态；L3-L5 不提前填充。

`EDGE-174` L5 已完成 stop-and-fix 并正式放行：全新 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-190430` 中，模型经 `search_tools` 直接调用 `progress_alpha`，App 收到三步真实 progress 与最终结果；随后以展示名 `edge174` 查询历史，得到唯一 `ok` 调用记录，和五通道事实一致。按 `G1` 写入 `✓`，证据=`testend/rig/formal-evidence/EDGE-174-mcp-progress-correlation-l5-real-app-20260830.md`；先前红场、修复和复跑均保留在 `LOG.md` 与独立证据中。这是上一前线的历史收口，不改变下方本轮 `EDGE-175` 的当前状态。

`EDGE-175` L3 已完成真实 App 顺滑性验收：同一封存 session 中，失败工具卡和 Activity 反馈在约 `100ms` 抽样内出现，`t=66..75s` 稳定尾段正文 ROI 的 `measure diff` 为零；早期标题/内容收尾变化已显式从稳定窗口排除。按 `B2` 写入 `✓`，证据=`testend/rig/formal-evidence/EDGE-175-mcp-stderr-tail-l3-real-app-20260830.md`。其后 L4 已在全新 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-202150` 收口：默认态只有中心卡的人话失败摘要与一次助手交接句，raw `tools/call`/`EOF` 仅在显式展开 `Technical details` 后出现；五通道日志和封口录屏齐全。按 `E1` 写入 `✓`，证据=`testend/rig/formal-evidence/EDGE-175-mcp-stderr-tail-l4-real-app-20260830.md`。

`EDGE-175` L5 已在全新真实 App session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-204436` 收口：首轮真实执行一次 `edge175/crash_with_stderr` 并失败；第二轮用户明确要求完整详情后，模型先用同 server、同 tool、`status=failed` 的 `search_mcp_calls(limit=1)` 找到最新记录，再用 `get_mcp_call` 读取持久化 `logs`，没有重新执行 MCP 工具。真实画面显示 `failed / crash_with_stderr / chat` 历史记录、`logs · 102 lines`、server-level stderr 尾部和 `may predate this call` 边界；五通道、封口录屏和无前端应用红线均已核对。按 `G1` 写入 `✓`，正式证据=`testend/rig/formal-evidence/EDGE-175-mcp-stderr-tail-l5-real-app-history-20260830.md`；L5 的 stop-and-fix 红证据仍保留。`anchors.py check`=`10/10`，`alarms.py check`=`clean (2131 live judgments; 2300 baseline judgments excluded)`。

`EDGE-177|无可跑 package` L3 已按正式 curated marketplace 的适用性边界收口：产品只暴露可规划条目，不存在真实 App 的 no-runnable package 等待或错误反馈时序对象，既有 evidence 明确说明这是不可达状态而非缺少现场证据。按 `na` 写入 `~`；L4 也已按同一边界收口：产品不产生该状态的错误表单，因此没有可评价的视觉 craft 成品；L5 同理，正式目录不提供用户可发现的无可跑 package 目标，故不存在 discoverability 评价对象。当前下一项由 formal sequence 选择；本批次为 `24/50`，未满 50 格不执行统一长门禁、不提交。

`EDGE-178|搜索 embedder 缺席降级` L2 已完成全新真实 App 五通道验收。由于当前 Settings 海洋没有搜索引擎面板，本次先通过 workspace-scoped 本地 API 将机器级 `embedder` 设为 `off`，再由真实 App 完成用户路径：创建 fixture 后，聊天实际调用 `search_documents` 命中 `EDGE178LEXICALFALLBACK`，再调用 `read_document` 读取正文；最终回答正确区分“词法搜索可用”和“仅凭该精确 token 不能推断语义搜索状态”。这证明语义层关闭时数据真相和用户检索目标仍成立，但不冒充用户可发现/切换设置的结论；该边界保留给后续对应设置面覆盖。正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-205857`，证据=`testend/rig/formal-evidence/EDGE-178-search-embedder-off-fallback-l2-real-app-20260830.md`，按 `F2` 写入 L2；SSE messages durable `1..33`，notifications `1..3`，三流已连接无 gap，managed LLM wire 全部 `200`，backend/frontend 无应用级红线，录屏 `171.086667s` 已封口且进程归零。

`EDGE-178` L2 写账触发的 `discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-178-l2-ledger-alarm-reaudit-real-app-20260830.md` ack；`anchors.py check`=`10/10`，最终 `alarms.py check`=`clean (2135 live judgments; 2300 baseline judgments excluded)`，`gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`。未改阈值、算法、法典、锚点或顺序门；本批次由 `24→25/50`，未满 50 格不执行统一长门禁、不提交。P12 的 400+ Journey 扩写继续按用户裁定推迟二期。

`EDGE-178` L3 已在同一正式 session 收口：真实 App 中从发送前稳定帧到首个超过 `0.0005` 阈值的实质反馈为 `1.4s`，完成后的 `32` 帧稳定尾段 `measure diff` 无输出；`B2` 证据=`testend/rig/formal-evidence/EDGE-178-search-embedder-off-fallback-l3-real-app-20260830.md`。这只覆盖动态反馈与稳定性，设置面没有 search/embedder 控件的事实仍单独披露，不能与 L4 craft 或 L5 discoverability 的结论混淆。

`EDGE-178` L3 写账触发的 `discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-178-l3-ledger-alarm-reaudit-real-app-20260830.md` ack；`anchors.py check`=`10/10`、最终 `alarms.py check`=`clean (2136 live judgments; 2300 baseline judgments excluded)`、`gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`。未改阈值、算法、法典、锚点或顺序门；本批次由 `25→26/50`，未满 50 格不执行统一长门禁、不提交。当前 formal sequence 下一自主前线为 `EDGE|搜索 embedder 缺席降级` L4；P12 的 400+ Journey 扩写继续按用户裁定推迟二期。

`EDGE-178` L4 已在同一正式 session 收口：稳定尾段真实画面通过 `C4` 复核正文层级、列表缩进、行内 code 胶囊、island/card/pill 圆角层级、Composer 内缩、长标题省略和无遮挡；证据=`testend/rig/formal-evidence/EDGE-178-search-embedder-off-fallback-l4-real-app-20260830.md`。缺失 Settings search/embedder 控件的事实仍保留，L5 的从零发现性不能由本格代填。

`EDGE-178` L4 写账触发的 `discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-178-l4-ledger-alarm-reaudit-real-app-20260830.md` ack；`anchors.py check`=`10/10`、最终 `alarms.py check`=`clean (2137 live judgments; 2300 baseline judgments excluded)`、`gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`。未改阈值、算法、法典、锚点或顺序门；本批次由 `26→27/50`，未满 50 格不执行统一长门禁、不提交。当前 formal sequence 下一自主前线为 `EDGE|搜索 embedder 缺席降级` L5；P12 的 400+ Journey 扩写继续按用户裁定推迟二期。

`EDGE-178` L5 已在同一正式 session 收口：新 workspace 从 onboarding 后的空白 Chat 开始，用户直接提出检索目标即可完成 lexical fallback，模型没有要求内部术语或 settings 操作，并对 semantic 状态保持诚实边界；`G1` 证据=`testend/rig/formal-evidence/EDGE-178-search-embedder-off-fallback-l5-real-app-20260830.md`。缺失内部 Settings 开关不属于该自动能力的发现性要求，未被隐藏或改写。

`EDGE-178` L5 写账触发的 `discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-178-l5-ledger-alarm-reaudit-real-app-20260830.md` ack；`anchors.py check`=`10/10`、最终 `alarms.py check`=`clean (2138 live judgments; 2300 baseline judgments excluded)`、`gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`。未改阈值、算法、法典、锚点或顺序门；本批次由 `27→28/50`，未满 50 格不执行统一长门禁、不提交。当前 formal sequence 下一自主前线为 `EDGE|首用下载途中关停` L2；P12 的 400+ Journey 扩写继续按用户裁定推迟二期。

本轮 `EDGE-039|:retry 编辑重发分支` L2 已在正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-163538` 完成：真实 App 中 assistant 已存在时，最后一个 user 回合仍显示 `Edit and resend`，进入编辑态后点击 `Resend` 实际调用 `POST ...:retry`；REST 版本链确认旧 user/assistant 由 `supersededBy` 闭合，新 assistant 带 `retryOf`，旧行仍可寻址。AX `set_value` 未同步 Flutter 内部 controller，精确输入文本因此不作为本轮证据；正式证据=`testend/rig/formal-evidence/EDGE-039-retry-edit-resend-fixed-20260830.md`，红证据=`testend/rig/formal-evidence/EDGE-039-edit-resend-normal-send-red-20260830.md`，按 `F2` 写入 L2，清册为 `✓✓~~~`，L3-L5 保持开放。

本轮 `EDGE-039` L3 的正式证据为=`testend/rig/formal-evidence/EDGE-039-retry-edit-resend-l3-real-app-20260830.md`：真实键盘编辑、约 `716.7ms` 首反馈、约 2 秒精确回显，完成后的 20 秒稳定尾段无超过 `0.0005` 的 ROI 帧间变化，清册为 `✓✓✓~~`，L4-L5 仍开放。

本轮两次写账后的 `gap-too-fast`、`discovery-collapse` 均已由各自独立复审证据逐条 ack；本次 L3 复审=`testend/rig/formal-evidence/EDGE-039-l3-ledger-alarm-reaudit-20260830.md`，`anchors.py check`=`10/10`，`alarms.py check`=`clean (2115 live judgments; 2300 baseline judgments excluded)`，`gen_coverage.py --check`=`848/848/0`。本批次为 `8/50`，未满 50 格不执行统一长门禁、不提交；当前下一自主前线为 `EDGE|SSE 410 SEQ_TOO_OLD 重放` L2；P12 的 400+ Journey 扩写按用户裁定推迟二期。

本轮 `EDGE-056|SSE 410 SEQ_TOO_OLD 重放` 的正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-171858` 已封存：真实 App 基线回合后，独立 messages ssetap 见 durable seq=`1..392`，后端真实返回 HTTP `410 SEQ_TOO_OLD`，App 重新读取五类 REST 真相并恢复可用；backend、三路 SSE、frontend console、managed LLM wire 与可读 `screen.mov` 均归属于同一 manifest。正式证据=`testend/rig/formal-evidence/EDGE-056-sse-seq-too-old-real-app-20260830.md`，按 `measure:edge056-sse-seq-too-old` 写入 L2；L3-L5 对该传输恢复边界明确不适用并保持 `~`。写账后的两项统计告警已用=`testend/rig/formal-evidence/EDGE-056-ledger-alarm-reaudit-real-app-20260830.md` 独立复审并串行 ack，未修改任何阈值、法典、锚点或顺序门；最终 `anchors.py check`=`10/10`、`alarms.py check`=`clean (2116 live judgments; 2300 baseline judgments excluded)`。

本批次由 `8→9/50`，未满 50 格不执行统一长门禁、不提交；当前 formal sequence 下一自主前线为 `EDGE|MCP degraded 态` L2。`EDGE|每租户模板 URL` 原本仅在 `manual_queue`，本次真实 App 观察到其 `Connect & authorize` 会创建持久 OAuth grant，已补入 `forced_queue`；它仍是 `✓~~~~`，不以未完成项改写成 `na` 或 `pass`。后续如需浏览器授权或其他安全确认，继续留在人工尾队。P12 的 400+ Journey 扩写按用户裁定推迟二期。

本轮 `EDGE-169|MCP degraded 态` 的正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-173628` 已封存：真实 App 面板观察到三次 MCP `boom` 失败后 `ready→degraded`、橙色状态点与失败文案，degraded 状态仍可调用 `echo` 并恢复 `ready`；entities SSE、backend journal、frontend console、managed LLM wire 和可读 `screen.mov` 均属于同一 manifest。正式证据=`testend/rig/formal-evidence/EDGE-169-mcp-degraded-real-app-20260830.md`，按 `measure:edge169-mcp-degraded` 写入 L2；L3-L5 保持开放。写账后的告警复审=`testend/rig/formal-evidence/EDGE-169-ledger-alarm-reaudit-real-app-20260830.md` 已按原阈值完成并 ack，`anchors.py check`=`10/10`、`alarms.py check`=`clean (2117 live judgments; 2300 baseline judgments excluded)`。

本批次由 `9→10/50`，未满 50 格不执行统一长门禁、不提交；当前 formal sequence 下一自主前线为 `EDGE|MCP 连接失败仍落盘` L2。`EDGE|每租户模板 URL` 已因真实 App 观察到持久 OAuth 授权动作而归入 `forced_queue`，仍保持 `✓~~~~`，不等待动作之外的替代证据。P12 的 400+ Journey 扩写按用户裁定推迟二期。

本轮 `EDGE-170|MCP 连接失败仍落盘` 的正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-174239` 已封存：真实 App 经坏 stdio 命令接入后显示 `1 servers · 1 failed`、初始化错误和失败卡片；补回同一路径后，真实 `:reconnect` 恢复为 `ready`，App 显示 `1 servers · 1 ready`，真实 `echo` 调用成功。backend journal、三路 SSE、frontend console、managed LLM wire 与可读 `screen.mov` 均归属于同一 manifest，正式证据=`testend/rig/formal-evidence/EDGE-170-mcp-failed-persists-real-app-20260830.md`，按 `measure:edge170-mcp-failed-persists` 写入 L2；L3-L5 保持开放。

`EDGE-170` 写账后的 `gap-too-fast`、`discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-170-ledger-alarm-reaudit-real-app-20260830.md` 串行 ack；真实失败持久化、App 卡片、reconnect 恢复和五通道封口均已复核，`anchors.py check`=`10/10`、`alarms.py check`=`clean (2118 live judgments; 2300 baseline judgments excluded)`，`gen_coverage.py --check`=`848/848/0`，未修改阈值、法典、锚点或顺序门。本批次由 `10→11/50`，未满 50 格不执行统一长门禁、不提交；当前 formal sequence 下一自主前线为 `EDGE|MCP 媒体逐件 best-effort` L2。P12 的 400+ Journey 扩写按用户裁定推迟二期。

本轮 `EDGE-171|MCP 媒体逐件 best-effort` 的正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-175550` 已封存：首轮真实 App 发现 MCP 卡片英文单数 `1 calls` 并 stop-and-fix；第二轮全新构建后显示 `ready · 1 tools · 1 call`。真实 MCP 返回 PNG、MP3 和超出 `50 MiB` 单附件上限的 PNG；前两件成为一等附件且 REST 内容 SHA-256 与记录一致，失败项保留 `[image: image/png]` 占位，整次调用 HTTP `200` 且审计 `ok`。backend journal、三路 SSE、frontend console、managed LLM wire 与可读 `screen.mov` 均归属于同一 manifest，正式证据=`testend/rig/formal-evidence/EDGE-171-mcp-media-best-effort-real-app-20260830.md`，按 `measure:edge171-mcp-media-best-effort` 写入 L2；L3-L5 保持开放。

`EDGE-171` 写账后的 `gap-too-fast`、`discovery-collapse` 已由独立复审=`testend/rig/formal-evidence/EDGE-171-ledger-alarm-reaudit-real-app-20260830.md` 串行 ack；修复前红场、修复后真实 App、五通道证据、focused backend/frontend 回归均已复核，`anchors.py check`=`10/10`、`alarms.py check`=`clean (2119 live judgments; 2300 baseline judgments excluded)`，`gen_coverage.py --check`=`848/848/0`，未修改阈值、法典、锚点或顺序门。本批次由 `11→12/50`，未满 50 格不执行统一长门禁、不提交；当前 formal sequence 下一自主前线为 `EDGE|MCP 媒体无 uploader` L2。P12 的 400+ Journey 扩写按用户裁定推迟二期。

`EDGE-172` L2 的适用性裁决已写账：目标环境是内部 service/REST 未注入 MediaUploader 的构造，而正式桌面 composition root 总是注入 attachment service；产品没有用户动作或支持的运行时配置能进入该状态，因此 L2 不是缺证据而是确实不适用。L3-L5 分别同理不拥有桌面交互、视觉成品或用户发现入口，四格边界写入 `testend/rig/formal-evidence/EDGE-172-mcp-media-no-uploader-20260825.md`，但仍逐格落账，不把一条说明当四格完成。

`EDGE-172` 当前清册为 `✓~~~~`（L1 pass、L2 na、L3-L5 的既有明确不适用裁决保持 `~`），本批次由 `12→13/50`，未满 50 格不执行统一长门禁、不提交；当前 formal sequence 下一自主前线为 `EDGE|MCP 进度关联` L2。L2 的适用性裁决已按独立复审证据复核，阈值、法典、锚点和五级标准不变。P12 的 400+ Journey 扩写按用户裁定推迟二期。

本轮 `EDGE-174|MCP 进度关联` 的正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-181342` 已封存：真实 App 在同一聊天回合激活并请求 `progress_alpha` 与 `progress_beta`，messages 与 entities 两路 SSE 分别记录 alpha/beta 的三步 progress 及各自结果，无交叉污染；App 完成态显示两个结果与 `edge174 Ran x2`。本轮实际 MCP 投递为串行，故 L2 只结算 per-call token 关联，不扩张为并发性能结论。L3 复用同一原始 `60fps` 录屏，10fps 动作锚点测得约 `400ms` 首可见反馈，25 秒稳定正文 ROI 在阈值 `0.0005` 下无帧间变化，按 `B2` 结算顺滑；正式证据分别为=`testend/rig/formal-evidence/EDGE-174-mcp-progress-correlation-real-app-20260830.md`、`testend/rig/formal-evidence/EDGE-174-mcp-progress-correlation-l3-real-app-20260830.md`，L4-L5 保持开放。

`EDGE-174` L3 写账后的告警已由独立复审=`testend/rig/formal-evidence/EDGE-174-l3-ledger-alarm-reaudit-real-app-20260830.md` ack；复审确认 session 内五通道证据、10/10 锚点、B2 法条和测量产物齐全，未修改阈值、算法、法典、锚点或顺序门。最终 `alarms.py check`=`clean (2124 live judgments; 2300 baseline judgments excluded)`，`gen_coverage.py --check`=`848/848/0`。本批次由 `15→16/50`，未满 50 格不执行统一长门禁、不提交；当前 formal sequence 下一自主前线为 `EDGE|MCP 进度关联` L4。P12 的 400+ Journey 扩写按用户裁定推迟二期。

本轮 `EDGE-175|MCP 失败附 stderr 尾` 的正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-182257` 已封存：真实 App 通过动态工具激活 `edge175/crash_with_stderr`，调用中 stdio server 写入 180 行诊断 stderr 后退出，真实 App 显示明确 EOF 失败；真实 `GET /api/v1/mcp-calls/mcl_c24f9dd09e527624` 返回 `status=failed`、有界 `server stderr tail` 及 `may predate this call` 标注，尾部保留到第 179 行。messages/entities SSE 的失败终态、managed LLM wire、backend journal、frontend console 和可读 `screen.mov` 均归属于同一 manifest。正式证据=`testend/rig/formal-evidence/EDGE-175-mcp-stderr-tail-real-app-20260830.md`，按 `measure:edge175-mcp-stderr-tail` 写入 L2；L3-L5 保持开放。App 当前只展示 EOF 与“inspect the error below”，没有内联完整 stderr 尾；该高层产品边界已诚实记录，不扩张为更高级别通过。

`EDGE-175` 写账后的两项告警已由独立复审=`testend/rig/formal-evidence/EDGE-175-ledger-alarm-reaudit-real-app-20260830.md` 逐条 ack；复审确认五通道证据、session 内证据、10/10 锚点与既有法条齐全，未修改阈值、算法、法典、锚点或顺序门。最终 `alarms.py check`=`clean (2122 live judgments; 2300 baseline judgments excluded)`，`gen_coverage.py --check`=`848/848/0`。本批次由 `14→15/50`，未满 50 格不执行统一长门禁、不提交；当前 formal sequence 下一自主前线为 `EDGE|MCP 市场缺必填 env` L2。P12 的 400+ Journey 扩写按用户裁定推迟二期。

账本门禁修复后，旧统计不再有效：`judge.py` 新增 `没有本格独立` provisional-NA 词序识别及专门回归，重新识别出历史清册中 94 个此前被错误当作已结算的现场证据缺口。该修复只收紧“是否已观察”的机械判定，不更改任何法条、阈值或历史 journal；当前清册真实重算为 `684` 行完全结算、`164` 行开放、`3700/540` 单元，当前 formal sequence 前线为 `EDGE|MCP 进度关联` L3。批次仍为 `15/50`，不因统计纠正虚增批次。

`EDGE|MCP 市场缺必填 env` 的真实 App marketplace 安装提交属于 `install_mcp_server` dangerous 动作，会创建持久 server 配置/外部能力并要求用户确认；本轮按“强制交互统一后置”机制加入 `forced_queue`，不以 focused 422 或后台 REST 调用冒充真实 App L2。清册仍为 `✓~~~~`，没有新增通过格；`EDGE|无可跑 package` 的 L2 已按 curated catalog 不可达边界写入明确 `na` 并完成告警复审，顺序门现回到 `EDGE|MCP 进度关联` L3，`forced_queue=24`，批次保持 `15/50`。

#### 历史前线快照（保留审计链，非当前状态）

`EDGE-031|回合收尾期单槽缓冲` 已完成本批次最后新增的真实 App L2 与 L3：在正式 ledger 根下启动真实 Anselm App、真实受管网关、SSE 三流 witness、frontend console、LLM tap 和窗口录像，先从 App 提交 accepted turn，再在其收尾窗口直接提交第二条请求。第二条真实 HTTP 请求返回 `409 STREAM_IN_PROGRESS`；修复后的拒绝路径同时落 durable assistant error 并发送 `message open→close(error)`，前端显示明确错误、清除 generating 状态且不残留 thinking 气泡；LLM tap 未出现第二次模型请求。清册=`✓✓✓~~`；L4-L5 仍未完成，不能把这次错误闭环证据冒充视觉 craft 或从零可发现性结论。

正式 L2/L3 证据=`testend/rig/formal-evidence/EDGE-031-compaction-single-slot-buffer-real-app-20260830.md`，session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260830-140457`；告警复审=`testend/rig/formal-evidence/EDGE-031-ledger-alarm-reaudit-20260830.md`。`judge.py` 已写入 `L2 ✓ (F1)` 与 `L3 ✓ (A5)`，锚点=`10/10`，`alarms.py check`=`clean (2107 live judgments; 2300 baseline judgments excluded)`，`gen_coverage.py --check`=`848/848/0`。

当前精确前线：`ledger-sequence.json` 的人工队列为 `173` 条唯一登记，全部剩余未收口格都在队列内；清册 `848` 行、`457` 格未收口，`675` 行已结算，`173` 行仍有未收口等级。自主前线已清空，下一项为人工队列首项 `EDGE|视频轮询超时诚实话`；用户要求后置的物理按键、系统授权、安全确认、物理网络切换和真实产品视觉/交互项目不会被自动化回归或本次 EDGE-031 现场绕过。批次已达到 `50/50`，现在执行统一长门禁、完整回归、告警复核、工作树审计和提交；P12 的 400+ Journey 扩写继续按用户裁定推迟二期。

按用户授权，人工后置队列仍不参与自主前线选择；所有需要用户物理按键、系统授权、安全确认、物理网络切换或其他强制交互的动作统一保留到自主前线耗尽后的人工尾队。`EDGE-223|视频轮询超时诚实话` 至 `EDGE-231|断网启动` 以及既有人工项均保留 L1 与未完成 L2-L5，不以自动化替代真实产品证据。`EDGE-225` 与 `EDGE-228` 的真实 App 试跑已写入非放行证据，均因系统窗口覆盖录屏而不能写绿；`EDGE-227` 的完整语音路径因系统麦克风权限和真实网关拒绝场景后置；`EDGE-229/230` 的真实语音产物路径不能以本地 parser/PCM 测试替代；`EDGE-231` 的物理断网冷启动不以死端口替代。`EDGE-232` 的模型目录缓存优先与刷新失败静默降级 focused/race 回归已通过；`EDGE-233` 的 ReattachActive→SweepMisfires 顺序与 misfire focused/race 回归已通过；`EDGE-235` 的关停预算 golden test 普通/race 回归已通过；`EDGE-236` 的 backend parent-watch 普通/race 回归已通过，前端 Dart 侧因工具链缺席未执行；`EDGE-237` 的 settings Load/Boot malformed-file 普通/race 回归已通过；`EDGE-238` 的 limits/network/retention 整体写与跨段保留普通/race 回归已通过；`EDGE-239` 的三张旧库 CHECK 整表重建与数据/索引幂等普通/race 回归已通过；`EDGE-240` 的重复 ADD COLUMN 结果幂等与真实错误保留普通/race 回归已通过；`EDGE-241` 的 AES-GCM 换钥拒解旧密文与 bootstrap 健康普通/race 回归已通过；`EDGE-242` 因 Flutter/Dart 工具链缺席未执行 keychain 测试；`EDGE-244` 的 bearer middleware/router 与真实 HTTP contract 普通/race 回归已通过；`EDGE-245` 的 workspace middleware/router 与真实 HTTP contract 普通/race 回归已通过，缺失或非法 workspace 不进入隔离 handler 并返回 `UNAUTH_NO_WORKSPACE`；`EDGE-246` 的 loopback Host middleware/router 普通/race 回归已通过，非 loopback Host 返回 `FORBIDDEN_BAD_HOST` 且 Host 门优先；`EDGE-247` 的 ServeMux envelope/router、Allow 头和平台合同普通/race 回归已通过，未知路由与错误方法分别返回 `ROUTE_NOT_FOUND`/`METHOD_NOT_ALLOWED`；`EDGE-248` 的 response error-map 普通/race 回归已通过，断连/超时分别映射 `CLIENT_CLOSED`/`REQUEST_TIMEOUT`；`EDGE-249` 的 bootstrap background-context focused/race 回归已通过，裸 context 返回 `MISSING_WORKSPACE_ID`，逐 workspace Detached ctx 可读到数据；`EDGE-250` 的 workspace cascade 普通/race 与真实 HTTP contract 回归已通过，被删 workspace 及其 function/conversation 不可达且 keeper 存活；`EDGE-290` 的 skill filesystem/app 普通/race 回归已通过，结构化编辑保留未知 frontmatter 键、键序与注释，安装 provenance 不写回原始 SKILL.md；`EDGE-291` 的 memory app focused/race 回归与既有真实 App L2 五通道证据均已核对，AI 更新内容/描述时保留 `pinned=true` 与 `source=user`；`EDGE-292` 的 todo app focused/race 回归与既有真实 App L2 五通道证据均已核对，0-open 时抑制 reminder 但 `todo_read` 仍返回已完成项；`EDGE-293` 的 relation ripple 与 chat touchpoint contract 普通/race 回归及既有真实 App L2 证据已通过，删除前快照入向依赖、删除后一次聚合 `dependency_broken`，拒绝/失败不造幽灵删除足迹；`EDGE-294` 的 danger gate 与 touchpoint contract 普通/race 回归及既有真实 App L2 证据已通过，拒绝 delete_agent 不执行工具也不写 deleted 触点，真实执行失败仍记 executed；`EDGE-295` 的 touchpoint failure contract 普通/race 回归已通过，真实执行后即使 function 抛错仍保留 executed 触点与 actor/entity 快照；`EDGE-296` 的 touchpoint hydrate handler 普通/race 与真实 contract 普通/race 回归已通过，已触碰实体删除后借用历史名称，未触碰实体保持诚实空名；`EDGE-297` 的真实装配工具目录穷尽性普通/race 门禁已通过，新增工具若未进入覆盖目录会在启动期失败；`EDGE-298` 的通知分径后端存储/HTTP 普通/race 与真实黑盒普通/race 已通过，权威 `unread-count` 与已读操作保持一致；`EDGE-302` 的 OS 通知边界仍需真实失焦 App 与 macOS 系统通知现场，统一保留人工尾队；`EDGE-299`、`EDGE-300`、`EDGE-301` 的 Flutter 单测可经 mise 工具链运行，后续继续自动验证。以上均已有 L1/L2 证据或明确后置记录，但真实 App/五通道的剩余层级仍后置。当前自主前线为 `EDGE-299|顶带 5000 条积压`。清册共有 `848` 行，其中自主未收口 `51` 行、`135` 格，人工队列 `137` 项，已结算 `669` 行；批次为 `27/50`，未满 50 格不跑统一长门禁、不提交。标准、阈值、法典、证据要求和最终收口条件不变；P12 的 400+ Journey 扩写继续按用户裁定推迟二期。
