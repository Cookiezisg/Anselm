...
## 当前精确状态（2026-08-31 · EDGE-234 三步优雅关停完成；EDGE-235 当前前线；批次 87 已达 56/50）
...
> **本段是当前权威快照，覆盖下方旧条目中的 next 指针和历史计数；历史快照不回写。**
 ...
 - Edge210 与 Edge234 的连续写账均触发 `gap-too-fast`/`discovery-collapse`；各次均由独立复审逐条 ack，未改阈值、曲线算法、CODEX、锚点、五级标准或顺序门。当前 `RIG_HOME=/private/tmp/anselm-rig-formal-20260831-11 python3 testend/rig/alarms.py check`=`clean (109 live judgments; 4240 baseline judgments excluded from drift curves)`，锚点=`10/10`，`gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`。
 - 权威清册重算为 `848` 行：`731` 行五级已结算、`117` 行仍开放；`3852` 个单元已结算、`388` 个单元开放。`manual_queue=173`、`forced_queue=25`；当前自主前线为 `EDGE|关停预算格`，强制人工交互继续后置。P12 的 400+ Journey 扩写按用户裁定推迟二期。
 - 批次 87 已由 `8/50` 推进至 `56/50`；现在执行统一长门禁，门禁通过前不提交。强制队列项目继续不写绿格、不降低标准，人工后置不是 waiver，不以 `na`、本地 mock 或旧 session 冒充真实 App 产品证据。
...
### 5.2 Day 0 当前状态(整体重述,2026-08-31 EDGE-229 L2-L5 收口、EDGE-227 后置；批次 87 进行中)

当前唯一权威状态以 `COVERAGE.md`、`ledger-sequence.json`、`judgments.jsonl` 及脚本实时重算为准：清册 `848` 行，`728` 行五级结算，`120` 行仍开放；`3840` 个单元已结算、`400` 个单元开放；`manual_queue=173`，其中实际需要用户强制交互的 `forced_queue=25`，仍有 `95` 行普通自主前线开放，EDGE-225 已完成 L1-L5，EDGE-228 已完成 L2 并以明确适用性边界收口 L3-L5，EDGE-229 已完成 L1-L5，EDGE-227 已按用户裁定移入强制人工尾队，顺序门当前指向 `EDGE-230|ParseWAV 遍历 chunk 表`。`EDGE-196` 至 `EDGE-229` 中已完成的各项均以顶部逐项记录和 COVERAGE 的最新判定为准；上一批次 85 已完成 `50/50` 并封存于 `testend/rig/formal-evidence/batch-85-unified-gate-20260831.md`，批次 86 已完成 `52/50` 并封存于 `testend/rig/formal-evidence/batch-86-unified-gate-20260831.md`，当前批次 87 为 `38/50`，未满 50 格不执行统一长门禁、不提交。215/216 等仍在 `forced_queue` 的项目继续不写绿格、不降低标准；历史候选但不在 `forced_queue` 的项目按 formal sequence 正常推进。人工后置不是 waiver，不降低五级标准；物理按键、系统授权、安全确认、不可逆删除、物理网络切换以及必须依赖真实用户操作的视觉/交互判断继续后置，自动回归不得冒充现场证据。
