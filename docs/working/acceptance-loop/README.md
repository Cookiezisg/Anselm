...
- **最新精确快照（EDGE-284 调度后）**：`848` 行、`36` 个人工后置项、`102` 个未收口行；自主队列剩余 `66` 行、`192` 个未收口单元，下一自动前线为 `EDGE-285|大小写不敏感 FS 上的 skill.md`。本节下方较早的长快照是调度前历史记录，以本条和 `ledger-sequence.json` 为准。
