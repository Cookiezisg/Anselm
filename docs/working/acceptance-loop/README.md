...
## 当前精确状态（2026-08-31 · EDGE-196 受管 remote media lease L2-L5 完成；批次 50/50）
 ...
 - 四次写账触发的统计警报均按原算法打开并逐项独立复核/ack；最终 `RIG_HOME=/private/tmp/anselm-rig-formal-20260801-7 python3 testend/rig/alarms.py check`=`clean (2212 live judgments; 2300 baseline judgments excluded from drift curves)`。`gen_coverage.py --check`=`848 rows, 848 carried judgments, 0 tombstones`，锚点=`10/10`。
 - 权威清册重算为 `848` 行：`706` 行五级已结算、`142` 行仍开放；`3785` 个单元已结算、`455` 个单元开放。`manual_queue=173`、`forced_queue=24`，当前已无非人工自主前线；人工/强制首项仍为 `EDGE|视频轮询超时诚实话`。P12 的 400+ Journey 扩写按用户裁定推迟二期。
 - 本批次当前为 `50/50`，统一长门禁已通过并封存于 `testend/rig/formal-evidence/batch-85-unified-gate-20260831.md`；强制队列项目继续不写绿格、不降低标准，人工后置不是 waiver，不以 `na`、本地 mock 或旧 session 冒充真实 App 产品证据。
...
### 5.2 Day 0 当前状态(整体重述,2026-08-31 EDGE-196 L2-L5 收口；批次 50/50)

当前唯一权威状态以 `COVERAGE.md`、`ledger-sequence.json`、`judgments.jsonl` 及脚本实时重算为准：清册 `848` 行，`706` 行五级结算，`142` 行仍开放；`3785` 个单元已结算、`455` 个单元开放；`manual_queue=173`，其中实际需要用户强制交互的 `forced_queue=24`，当前已无非人工自主开放行。`EDGE-196|受管 remote media lease` 已完成真实 App L2-L5，人工尾队首项仍为 `EDGE|视频轮询超时诚实话`；本批次当前为 `50/50`，统一长门禁已通过并封存于 `testend/rig/formal-evidence/batch-85-unified-gate-20260831.md`。强制队列项目继续不写绿格、不降低标准，历史候选但不在 `forced_queue` 的项目按 formal sequence 正常推进。人工后置不是 waiver，不降低五级标准；物理按键、系统授权、安全确认、不可逆删除、物理网络切换以及必须依赖真实用户操作的视觉/交互判断继续后置，自动回归不得冒充现场证据。
