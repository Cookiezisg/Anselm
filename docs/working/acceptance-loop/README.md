...
## 当前精确状态（2026-08-31 · EDGE-259 切分支名拼错完成；批次 87 已达 121/50）
...
清册准确重算为 `848` 行：`748` 行五级已结算、`100` 行开放；`3919` 个单元已结算、
`321` 个单元开放。`manual_queue=173`、`forced_queue=26`；批次 87=`121/50`，下一
自主原子继续按顺序门从首个未结算的自主 `~` 格选择。EDGE-254、EDGE-256 与 EDGE-257 的
...
用户裁定推迟二期。
 ...
 - 权威清册重算为 `848` 行：`748` 行五级已结算、`100` 行仍开放；`3919` 个单元已结算、`321` 个单元开放。`manual_queue=173`、`forced_queue=26`；P12 的 400+ Journey 扩写按用户裁定推迟二期。当前自动前线为 `EDGE|worktree 目录已存在`，强制人工交互仍后置；本轮不以 focused 回归替代真实 App 产品证据。
 - 批次 87 已由 `117/50` 推进至 `121/50`；统一长门禁不因本次追加四格而提前运行，仍按每 50 格执行；本轮未以警报压制、旧证据覆盖或标准降级推进。
...
### 5.2 Day 0 当前状态(整体重述,2026-08-31 EDGE-254 收口、人工尾队后置；批次 87 105/50)

当前唯一权威状态以 `COVERAGE.md`、`ledger-sequence.json`、`judgments.jsonl` 及脚本实时重算为准：清册 `848` 行，`744` 行五级结算，`104` 行仍开放；`3903` 个单元已结算、`337` 个单元开放；`manual_queue=173`，其中实际需要用户强制交互的 `forced_queue=26`。`EDGE-247|ServeMux 纯文本 404/405 改写`、`EDGE-248|客户端断连与请求超时`、`EDGE-249|后台裸 ctx 播种缺失` 与 `EDGE-254|keyset 排序切换丢游标` 均已按各自边界收口。EDGE-254 初轮真实 App 发现排序切轴后保留旧中段 offset，停止推进并修复共享 `AnSidebarList`；正式复验 session=`/private/tmp/anselm-rig-formal-20260831-11/sessions/20260831-200325`，Computer Use 首项从 `Sort Probe 60` 开始，录屏抽帧无空白、旧中段闪回或二次跳回，focused conversation rail suite 21 tests 全通过。L2=`F1`、L3=`B2`、L4=`C5`、L5=`G1`，正式证据=`testend/rig/formal-evidence/EDGE-254-sort-switch-scroll-offset-fixed-20260831.md`，红证据=`testend/rig/formal-evidence/EDGE-254-sort-switch-scroll-offset-red-20260831.md`；写账后的 `discovery-collapse` 已逐级独立复核并销账，最终 `alarms.py check`=`clean (163 live judgments; 4240 baseline judgments excluded)`。顺序门下一自主前线为 `EDGE|驻地目录被移走`；强制项继续留在人工尾队，不阻塞自主推进。批次 87 已由 `101` 推进至 `105/50`，未改阈值、法典、锚点、五级标准或顺序 gate。P12 的 400+ Journey 扩写按用户裁定推迟二期。
