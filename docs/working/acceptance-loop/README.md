...
> **本段是当前权威快照，覆盖下方旧条目中的 next 指针和历史计数；历史快照不回写。**
 ...
 - 权威清册重算为 `848` 行：`732` 行五级已结算、`116` 行仍开放；`3856` 个单元已结算、`384` 个单元开放。`manual_queue=173`、`forced_queue=25`；当前自主前线为 `EDGE|父进程死人开关`，但该项需要真实父进程 `kill -9` 与强制交互，按用户授权继续后置。P12 的 400+ Journey 扩写按用户裁定推迟二期。
...
当前唯一权威状态以 `COVERAGE.md`、`ledger-sequence.json`、`judgments.jsonl` 及脚本实时重算为准：清册 `848` 行，`732` 行五级结算，`116` 行仍开放；`3856` 个单元已结算、`384` 个单元开放；`manual_queue=173`，其中实际需要用户强制交互的 `forced_queue=25`。`EDGE-235|关停预算格` 已完成 `L2=F2/L3=A4/L4=C4/L5=na`，真实 App 停修复验、三路 SSE、backend、frontend console、LLM tap、录屏与 SQLite 真相均绑定正式 session；正式证据=`testend/rig/formal-evidence/EDGE-235-shutdown-budget-real-app-fixed-20260831.md`，账本复审=`testend/rig/formal-evidence/EDGE-235-ledger-alarm-reaudit-real-app-20260831.md`。顺序门当前指向 `EDGE-236|父进程死人开关`，但该项需要真实父进程 `kill -9` 和强制交互，按用户授权留在人工尾队，不阻塞先推进其他自主项。上一批次 85 已完成 `50/50`，批次 86 已完成 `52/50`，当前批次 87 为 `60/50`，执行统一长门禁前不提交。强制队列项目继续不写绿格、不降低标准；人工后置不是 waiver，自动回归不得冒充现场证据。P12 的 400+ Journey 扩写按用户裁定推迟二期。
