...
created: 2026-08-01
reviewed: 2026-08-19
review-due: 2026-11-17
audience: [human, ai]
landed-into:
---

## 2026-08-25 · Batch 47 统一长门禁通过，待提交

- SURF-105 至 SURF-114 共 50/50 格已完成五级裁决；没有在门禁阶段新增产品判断。
- 根 `make verify` 第二轮全绿；首轮发现并修复 `tool_card_trigger.dart:26` 的 `unnecessary_cast`，没有关闭 analyzer。完整 `make -C backend testend`=`312.506s`，rig Python=`50/50`，`gen_coverage.py --check`=`848/497/0`，anchors=`10/10`，`alarms.py check`=`clean (185 live; 2300 baseline excluded)`，py_compile/bash-n/git diff/process-listener audit 全绿。
- 正式记录=`testend/rig/formal-evidence/batch-47-gate-20260825.md`。当前只剩提交；提交后按 sequence gate 决定下一原子前线。P12 的 400+ Journey 继续按用户裁定推迟二期。

## 2026-08-25 · SURF-114 stage/generic 通用舞台与 poll 终态正式五级入账，批次四十七 50/50

- `_GenericStage` 作为第 13 座共同 host 覆盖未被 bespoke body 接管的 stage route；`search_tools`、conversation、attachment 无 stage route，保持诚实缺席。`trigger_workflow` 202 回执不提前谢幕，真实终态按匹配 `flowrunId` 的 durable `run_terminal` 收卷。
- 真实 App 通过实体提及选择 disposable `surf114_poll`，只调用一次 `trigger_workflow`，flowrun=`fr_b71eebde4adf9919`，8.12 秒后 completed；节点 2、边 1，右岛从通用图和运行卷收为 settled touchpoint 摘要。直接输入带下划线 ID 的两次失败保留为 Computer Use 输入桥负边界，不计绿；fixture 收台前全部删除。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-105440`，screen=`699.328333s / 2784x1808 / 60fps`，结果帧=`evidence/frames/surf114-generic-settled.png`。messages durable=`66..74`，entities=`run_started → run tick → run_terminal`；backend/frontend 无应用级红线，LLM 正向 wire 全`200`，rig-check/rig-down 无残留。正式调查=`testend/rig/formal-evidence/SURF-114-stage-generic-investigation-20260825.md`，L2=`sessions/20260825-105440/evidence/SURF-114-stage-generic-five-channel.md`。
- 新增真实时序回归 `tool_result open → run_terminal → tool_result close` 并通过。`judge.py` 串行写入 `E2/F2/B2/C4/G1`，formal journal=`2485`（2300 baseline + 185 live），coverage=`848/497/0`，anchors=`10/10`；两条统计警报按 `SURF-114-ledger-alarm-reaudit-20260825.md` 独立复审并串行 ack，`alarms.py check` clean。达到 50 格后进入统一长门禁、完整 testend、最终审计和提交；P12 400+ Journey 继续推迟二期。

## 2026-08-25 · SURF-113 stage/mcp 接线现场与类型化工具货架正式五级入账，批次四十七 45/50

- 静态反查确认 MCP 舞台只投影 install/reconnect/create 的 typed `tools[].name`，环境键遮罩、货架最多 12 项并显示总数；install 是危险动作必须经过一次性人闸，reconnect 是安全动作。
...
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-104412`，screen=`202.041667s / 2784x1808 / 60fps`；抽帧=`sessions/20260825-104412/evidence/frames/surf113-mcp-125.png`、`surf113-mcp-175.png`。五通道：messages=`1..44`、notifications=`16..19` 连续；entities `disconnected→connecting→ready` 两次；backend 无应用红线，frontend 无 Flutter/Dart/布局/Unhandled 红线，LLM 响应全`200`，收台无残留。正式调查=`testend/rig/formal-evidence/SURF-113-stage-mcp-investigation-20260825.md`，L2=`sessions/20260825-104412/evidence/SURF-113-stage-mcp-five-channel.md`。
- focused Flutter=`30` 项、Go MCP app/tool/infra 全绿。`judge.py` 写入 `E2/F2/B2/C4/G1`，中央账本 `2475→2480 judgments`；`gen_coverage.py --check`=`848/496/0`，anchors=`10/10`。写账触发的 `gap-too-fast`/`discovery-collapse` 依据 `SURF-113-ledger-alarm-reaudit-20260825.md` 独立复审并串行 ack，未改阈值/算法/CODEX/锚点/gate，最终 `alarms.py check` clean；批次四十七由 `40→45/50`，未到第 50 格不跑统一长门禁、不提交；下一正式前线=`SURF-114 stage/generic`。P12 400+ Journey 继续推迟二期。

## 2026-08-25 · SURF-112 stage/memory 记忆笺与用户图钉边界正式五级入账，批次四十七 40/50

- 静态反查确认 Memory 舞台只读 slug、摘要和正文，落定显示结果条；pin/unpin 是用户 REST-only，不在舞台渲染控制；`write_memory` 更新已有记忆必须保留既有 `Pinned` 与 `Source`。
- 真实 App 一次读取 `handoff-note`，舞台显示 slug、来源和完整正文；随后先一次 `search_tools` 激活 `write_memory`，再一次 `write_memory` 更新 `release-rule`，无 retry、重复 mutation 或其它记忆操作。REST 证明 description 不变、`pinned=true`、`source=user`，正文更新正确。对 `handoff-note` REST pin/unpin 均 200，notifications 只有无 `inbox` 的 frame-only `memory.updated`。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-103249`，screen=`257.440000s / 2696x1720 / 60fps`；抽帧=`sessions/20260825-103249/evidence/frames/surf112-memory-read.png`、`surf112-memory-write-170.png`。五通道：messages=`1..36`、notifications=`16..22` 连续；backend 无 WARN/ERROR/panic/fatal；frontend 无 Flutter/Dart/Unhandled 红线，仅已知 IMK 平台噪声；LLM 响应全`200`；收台无残留。正式调查=`testend/rig/formal-evidence/SURF-112-stage-memory-investigation-20260825.md`，L2=`sessions/20260825-103249/evidence/SURF-112-stage-memory-five-channel.md`。
- focused Go memory/tool tests 通过，Flutter `stages_w5_test.dart` + `scene_from_truth_test.dart` 共 23 项通过。`judge.py` 写入 `E2/F2/B2/C4/G1`，中央账本 `2470→2475 judgments`；`gen_coverage.py --check`=`848/495/0`，anchors=`10/10`。写账触发的 `gap-too-fast`/`discovery-collapse` 依据 `SURF-112-ledger-alarm-reaudit-20260825.md` 独立复审并串行 ack，未改阈值/算法/CODEX/锚点/gate，最终 `alarms.py check` clean。批次四十七由 `35→40/50`，未到 50 格不跑统一长门禁、不提交；下一正式前线=`SURF-113 stage/MCP`。P12 400+ Journey 继续推迟二期。

## 2026-08-25 · SURF-111 stage/skill 正文、占位符与安装信任门正式五级入账，批次四十七 35/50

- 静态反查确认 Skill 舞台为 metadata header + 真 Markdown prose；installed skill 的 allowed-tools 只有在 `toolsApproved=true` 后才可呈琥珀预授权，未批显示中性逐次确认。`activate_skill` 四类占位符与捆绑目录锚点已核对。
- 真实 App 创建 `surf111runbook` 后保留输入桥丢 `$`/下划线的负事实；精确 body 经 REST 真相面修正，再由 App `get_skill` 读取、`activate_skill` 传 `daily/review`，主内容与右侧 stage 同时显示真实展开结果。另以本地 tarball 安装 `surf111-installed`，真实读取未批、批准后重读，stage 分别显示中性与琥珀授权。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-101708`，screen=`531.340000s / 2784x1808`，messages=`1..75`、notifications=`16..28` 连续唯一；backend 无 WARN/ERROR/panic/fatal，frontend 仅已知 IMK 噪声，LLM 观测响应全`200`，rig-down 无残留。调查=`testend/rig/formal-evidence/SURF-111-stage-skill-investigation-20260825.md`，L2=`sessions/20260825-101708/evidence/SURF-111-stage-skill-five-channel.md`。
- 五级=`E2/F2/B2/C4/G1`；formal journal=`2470`（2300 baseline + 170 live），coverage=`848/494/0`，anchors=`10/10`。`gap-too-fast`/`discovery-collapse` 已按 `SURF-111-ledger-alarm-reaudit-20260825.md` 独立复审并串行 ack，未改阈值/算法/CODEX/锚点/gate，最终 `alarms.py check` clean；批次四十七由 `30→35/50`，不跑统一长门禁、不提交；下一前线=`SURF-112 stage/memory`。P12 400+ Journey 继续推迟二期。

## 2026-08-25 · SURF-110 stage/agent 四槽创建与局部编辑正式五级入账，批次四十七 30/50

- 静态反查确认 Agent 舞台同时显示 `prompt`、`tools`、`knowledge`、`modelOverride` 四槽；live 保留未触槽旧真相，settled 使用有界 prompt 视口。`create_agent` 的 knowledge/tools/modelOverride 精确形状与 `edit_agent` 局部合并语义已锁定。
- 真实 App 保留 knowledge 字符串化和缺 modelOverride.apiKeyId 的失败路径，均在 mutation 前失败且没有实体副作用；另保留一次停止重复推理造成的 context-canceled finalize WARN。修正后只创建 `surf110-planner` v1，挂载 greet、上手指南和 `anselm-auto`，随后只改 prompt 生成 v2，所有其它字段保持不变。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-100206`，screen=`572.313333s / 2784x1808`，messages=`1..77`、entities=`7..10`、notifications=`16..19` 连续唯一，LLM 观测响应全`200`；backend 仅已知负路径 WARN，frontend 仅已知 IMK 平台噪声；REST、SSE、UI、LLM wire 一致，rig-down 无残留。调查=`testend/rig/formal-evidence/SURF-110-stage-agent-investigation-20260825.md`，L2=`sessions/20260825-100206/evidence/SURF-110-stage-agent-five-channel.md`。
- 五级=`E2/F2/B2/C4/G1`；formal journal=`2465`（2300 baseline + 165 live），coverage=`848/493/0`，anchors=`10/10`。`gap-too-fast`/`discovery-collapse` 已按 `SURF-110-ledger-alarm-reaudit-20260825.md` 独立复审并串行 ack，未改阈值/算法/CODEX/锚点/gate，最终 `alarms.py check` clean；批次四十七由 `25→30/50`，不跑统一长门禁、不提交；下一前线=`SURF-111 stage/skill`。P12 400+ Journey 继续推迟二期。

## 2026-08-25 · SURF-109 stage/handler 生命周期与 RFC-7396 编辑正式五级入账，批次四十七 25/50

- 静态反查确认 Handler 舞台契约：`set_init_args_schema(args)`、`set_init(initBody)`、`add_method(method)`、`update_method(name + patch)`、`set_shutdown(shutdownBody)`；timeout 以毫秒落盘、以秒钟词展示，sensitive init arg 掩码。focused Flutter=`12/12`，Go handler/tool 定向测试通过。
- 真实 App 先保留错误 `set_method`、非法名称和 nested `method` 的失败路径，后端 WARN、SSE error close、活动区失败卡均可追溯。模型自纠后创建 `surf109_notifier` v1，再以正确 RFC-7396 patch 编辑为 v2：`send` 的 `30s/sent` 变为 `45s/updated`，输入输出、init/shutdown、敏感标记和非敏感 `region` 保持不变；重新查看实体后右侧舞台已跟随 v2，v1 作为历史保留。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-095228`，screen=`334.810000s / 2784x1808 / 60fps`，截图=`sessions/20260825-095228/evidence/SURF-109-stage-handler-settled.png`；REST active v2、SSE close/entities signal、LLM arguments 和 UI 一致，env ready/runtime running；backend 无 panic/fatal，仅三条刻意失败 WARN；frontend 无 Flutter/Dart/布局/Unhandled 红线，仅已知 IMK 噪声；managed completion 全`200`；rig-down 无残留。调查=`testend/rig/formal-evidence/SURF-109-stage-handler-investigation-20260825.md`，L2=`sessions/20260825-095228/evidence/SURF-109-stage-handler-five-channel.md`。
- `judge.py` 串行写入 `E2/F2/B2/C4/G1`，formal journal=`2455→2460 judgments`，COVERAGE=`848 rows / 491→492 carried / 0 tombstones`，anchors=`10/10`。`gap-too-fast`/`discovery-collapse` 按 `SURF-109-ledger-alarm-reaudit-20260825.md` 独立复审并串行 ack，未改阈值/算法/法典/锚点/gate，最终 `alarms.py check` clean；批次四十七由 `20→25/50`，未到 50 格不跑统一长门禁、不提交；下一前线=`SURF-110`。P12 400+ Journey 继续按用户裁定推迟二期。

## 2026-08-25 · SURF-108 stage/subagent 一席一卡与真实终端活窗正式五级入账，批次四十七 20/50

- 静态反查确认 `SubagentStageBody` 只消费本行 `StageScene`；真实 `prompt` 首行是任务名唯一来源；execution phase 判 live，ReAct/progress 终端封顶 6/10 行，结算 metadata 走 nested close 与 lifted fields 双源。focused Flutter 与 Go subagent/tool tests 通过。
- 真实 App 的 Explore 负路径保留：输入桥丢 `_` 后只读子代理诚实拒绝 Bash；精确 `general-purpose` 路径真实调用 Bash，输出 `SURF108 terminal probe 1/2/3`，第三次短请求输出 `SURF108 LIVE 1/2/3`，均退出码 0。AX 观察到 live、实时聆听、正在落定和 settled；侧幕展开显示单卡、Bash、输出和退出码。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-093437`，screen=`842.563333s / 2784x1808`，截图=`sessions/20260825-093437/evidence/SURF-108-stage-subagent-settled.png`；SSE/LLM/backend/frontend 五通道分别有 `Subagent+subagent:true+Bash+progress+tool_result`、真实 upstream `200`、无 panic/fatal（仅两条故意失败路径 Grep fallback WARN）、仅 IMK host noise；rig-down 无残留。调查=`testend/rig/formal-evidence/SURF-108-stage-subagent-investigation-20260825.md`，L2=`sessions/20260825-093437/evidence/SURF-108-stage-subagent-five-channel.md`。
- `judge.py` 串行写入 `E2/F2/B2/C4/G1`，formal journal=`2450→2455 judgments`，COVERAGE=`848 rows / 490→491 carried / 0 tombstones`，anchors=`10/10`。`gap-too-fast`/`discovery-collapse` 按 `SURF-108-ledger-alarm-reaudit-20260825.md` 独立复审并串行 ack，未改阈值/算法/法典/锚点/gate，`alarms.py check` clean；批次四十七由 `15→20/50`，未到 50 格不跑统一长门禁、不提交；下一前线=`SURF-109`。P12 400+ Journey 继续按用户裁定推迟二期。

## 2026-08-25 · SURF-107 stage/trigger 四脸与 nextFireAt 真相正式五级入账，批次四十七 15/50

- 首轮真实读取 `SURF107-cron` 发现通用 ISO 脱敏器吞掉了明确的 `nextFireAt`，使用户看不到下次触发时间；该产品红事实停止计绿。修复 `backend/internal/app/loop/redact.go` 的字段级窄保护，保留 `nextFireAt`/“下次触发时间”的 direct field、翻译 table row 和 streaming chunk，普通更新时间仍脱敏；后端 redaction suite 与 trigger focused=`21/21` 通过。
- 修复后真实 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-092425` 只读重跑，App/AX、REST、SSE close、LLM wire 均给出 `2026-08-26 09:00:00 (UTC+8)`，`listening=true`、`paused=false`、`refCount=1`；四脸 setup 使用前置 `20260825-090642`，不冒充修复后画面。嵌套 sensor target、畸形 ID、输入桥残缺请求均保留为负向事实。
- 五通道封口：screen=`336.175000s`，截图=`sessions/20260825-092425/evidence/SURF-107-fixed-next-fire.png`；backend 无应用 WARN/ERROR/panic/fatal；SSE messages=`1..21`、notifications=`1..2` 单调唯一，entities 无本路径业务 durable 帧；LLM managed wiring/chat 全`200`；frontend 仅已知 IMK 平台噪声；rig-down 无残留。调查=`testend/rig/formal-evidence/SURF-107-stage-trigger-investigation-20260825.md`，L2=`sessions/20260825-092425/evidence/SURF-107-stage-trigger-five-channel.md`。
- `judge.py` 写入五格 `E2/F2/B2/C4/G1`，formal journal=`2450`（2300 baseline + 150 live），COVERAGE=`848 rows / 490 carried / 0 tombstones`，anchors=`10/10`。`gap-too-fast`/`discovery-collapse` 按 `SURF-107-ledger-alarm-reaudit-20260825.md` 独立复审并串行 ack，未改阈值/算法/法典/锚点/gate，最终 `alarms.py check` clean；批次四十七由 `10→15/50`，未满 50 格不跑统一长门禁、不提交；下一前线=`SURF-108 stage/subagent`。P12 的 400+ Journey 按用户裁定推迟二期。

## 2026-08-25 · SURF-106 stage/approval 审批预览正式五级入账，批次四十七 10/50

- 静态反查发现 hosted gateway 的 `allowReason`/`timeout` 可能为字符串 scalar，timeout 是秒数；原 stage 只读 native 值。stop-and-fix 让 live/settled 共用解析 seam，整秒转为人话时长，`0` 显示 `0s`；新增 `"true"`、`"7200"`、`2h`、备注 chip 与零值回归，focused Flutter=`22/22`。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-085143`。首轮 AX 输入桥污染和旧对话 edit 被排除；干净对话通过真实 managed gateway 只调用一次 `create_approval`，创建 `SURF106-approval-clean` v1。Computer Use 展开 Activity，逐帧确认审批人视角、amount/vendor 琥珀插值、`2h`、`2h 后自动拒绝`、`可填备注`、批准/拒绝动作；REST active v1 一致，无 clipping/overlap/reflow/非用户跳变。
- 五通道封口：screen=`362.563333s`；backend 无应用级 WARN/ERROR/panic/fatal（唯一 401 为观测器缺 workspace 探针）；SSE messages=`1..53`、notifications=`1..7`、entities=`1..2` 均唯一单调；LLM managed proof/install/models 与 9 次 completion 全 `200`；frontend 仅已知 IMK 平台噪声；rig-check/rig-down 通过且无残留。调查=`testend/rig/formal-evidence/SURF-106-stage-approval-investigation-20260825.md`，L2=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-085143/evidence/SURF-106-stage-approval-five-channel.md`。
- `judge.py` 写入 `E2/F2/B2/C4/G1`，formal journal=`2445`（2300 baseline + 145 live），COVERAGE=`848 rows / 489 carried / 0 tombstones`，anchors=`10/10`。两条统计警报按 `testend/rig/formal-evidence/SURF-106-ledger-alarm-reaudit-20260825.md` 独立复审并 ack，未改阈值/算法/法典/锚点/gate，最终 `alarms.py check` clean。批次四十七由 `5→10/50`；未满 50 格不跑统一长门禁、不提交；下一前线=`SURF-107 stage/trigger`。P12 的 400+ Journey 按用户裁定推迟二期。

## 2026-08-25 · SURF-105 stage/control 决策梯正式五级入账，批次四十七 5/50

- 静态反查确认 control stage 原先只读原生 `branches` 数组，真实 managed gateway 的闭合 JSON 字符串数组会被误当成空 ladder。stop-and-fix 增加 `controlBranchItems` 窄兼容 seam：原生数组优先，合法闭合 JSON 数组字符串才解码，部分/畸形流保持空集并按 `PartialJsonSession` 缓存；新增 stringified branches/emit/catch-all 回归，focused Flutter=`20/20`。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-083508` 真实 App + managed gateway + Computer Use + 三路 SSE witness + LLM tap + 连续录屏完成。AX `set_value` 与可视编辑器不同步先制造了残缺输入；模型停在澄清，后端在 mutation 前拒绝坏 inputs，未产生实体副作用。随后成功创建唯一 `SURF105` v1；展开成功 stage 逐帧确认 hot/normal/otherwise 三段、连续高度、独立否则徽记和透传幽灵，REST active v1 与 UI/助手正文一致。该观察器负路径保留，不计产品红绿。
- 五通道封口：screen 总时长=`540.676667s`（含 rebind segment）；backend 仅故意 validation WARN；SSE messages=`1..33`、notifications=`1..3`、entities=`1..2` 各自单调唯一；LLM managed proof/install/models 与业务 completion 全 `200`；frontend 仅正常 VM/IMK 噪声，无 Flutter/Dart/布局/Unhandled 红线；rebind 后 `rig-check`、`rig-down` 通过且无残留。证据=`testend/rig/formal-evidence/SURF-105-stage-control-investigation-20260825.md`，L2=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-083508/evidence/SURF-105-stage-control-five-channel.md`。
- `judge.py` 写入五格 `E2/F2/B2/C4/G1`，formal journal=`2440`（2300 baseline + 140 live），COVERAGE=`848 rows / 488 carried / 0 tombstones`，anchors=`10/10`。写账触发的 `gap-too-fast`/`discovery-collapse` 按 `testend/rig/formal-evidence/SURF-105-ledger-alarm-reaudit-20260825.md` 独立复审并 ack，未改阈值/算法/法典/锚点/gate，最终 `alarms.py check` clean。批次四十七由 `0→5/50`；未到 50 格不跑统一长门禁、不提交；下一前线=`SURF-106 stage/approval`。P12 的 400+ Journey 按用户裁定推迟二期。
...
- 真实 App 矛盾 probe 的可见 `编辑 ×2` 作为负事实保留，不计绿；干净正向 probe 给出完整目标正文，单次 `edit_document` 成功，文档名、正文和右侧舞台一致，无重复卡片/跳变/溢出。调查=`testend/rig/formal-evidence/SURF-103-stage-document-investigation-20260825.md`，L2=`sessions/20260825-075245/evidence/SURF-103-stage-document-five-channel.md`。
- 五通道 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-075245`：screen=`119.876667s`、backend=`249`、frontend=`4`、SSE=`420`、LLM=`28`；messages=`1..48`、notifications=`16..22`、entities=`7..8` 单调无 gap，LLM 全 `200`，frontend 仅已知 IMK 平台噪声，rig-check/rig-down 通过且无残留。
- `judge.py` 写入 `E2/F2/B2/C4/G1`，formal journal=`2430`（2300 baseline + 130 live），coverage=`848/486/0`，anchors=`10/10`。两条统计警报按 `testend/rig/formal-evidence/SURF-103-ledger-alarm-reaudit-20260825.md` 独立复审并 ack，未改阈值/算法/法典/锚点/gate，`alarms.py check` clean；下一前线=`SURF-104 stage/workflow`，当前批次=`45/50`，未到 50 格不跑统一长门禁、不提交。P12 的 400+ Journey 仍推迟二期。

## 2026-08-25 · SURF-102 stage/function 函数编辑舞台正式五级入账，批次四十六 40/50

- 静态反查确认 edit stage 冻结编辑前真相；live 依次呈现旧地层、中性 OpTicker、同壳活代码窗，settle 计算真实 before/after `+n/−m`，失败态不染成功色。focused Flutter 阶段套件=`40/40`。
- 真实 App 使用新数据目录、managed gateway、Computer Use、三路 SSE witness、LLM tap 和录屏。短编辑场次真实 v4→v5 单一 `set_code` 成功；另一场观察到 `正在修改函数…`、`edit_function 进行中`、`实时聆听中 · 落定以真相为准`，随后打开落定代码舞台，无跳变/溢出/布局红线。窄中间帧由 focused 测试补足，不冒充 App 画面。
- 长代码 probe 中模型先发错误形状调用再成功重试，backend WARN 与重进场红失败卡片如实保留；后续单列为模型工具遵循/重试呈现边界，不吞失败事实。调查=`testend/rig/formal-evidence/SURF-102-stage-function-investigation-20260825.md`，L2=`sessions/20260825-073701/evidence/SURF-102-stage-function-five-channel.md`。
- 五通道 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-073701`：backend=`883`、SSE=`1211`，messages=`1..162`、notifications=`16..43`、entities=`7..28` 单调无 gap，LLM 全 `200`，frontend 仅已知 IMK 平台噪声；rig-check/rig-down 通过且无残留。
- `judge.py` 写入 `E2/F2/B2/C4/G1`，formal journal=`2425`（2300 baseline + 125 live），coverage=`848/485/0`，anchors=`10/10`。两条统计警报按 `testend/rig/formal-evidence/SURF-102-ledger-alarm-reaudit-20260825.md` 独立复审并 ack，未改阈值/算法/法典/锚点/gate，`alarms.py check` clean；下一前线=`SURF-103 stage/document`，当前批次=`40/50`，未到 50 格不跑统一长门禁、不提交。P12 的 400+ Journey 仍推迟二期。

## 2026-08-25 · SURF-101 i18n/markdown 图片占位与长 URL 稳定性正式五级入账，批次四十六 35/50

- 静态反查确认中英文 `markdown.imageNotLoaded` 为 `image not loaded` / `图片未加载`；markdown 图片统一走不发网络请求的 `_imagePlaceholder`，focused Flutter=`32/32`，既有 widget test 锁定零 `Image` widget，新增 locale test 锁定双语值。
- 真实 App 使用全新数据目录、managed gateway、Computer Use、三路 SSE witness、LLM tap 和连续录屏，覆盖短 URL 与 298 字符长 URL。直接输入 markdown 的首条因 Computer Use 输入桥损坏标点而排除；随后真实 REST 只写入精确 durable markdown，App 重新打开并实际渲染。AX/画面均显示 `图片未加载`，长 URL 单行省略，连续帧无持续跳变、溢出或历史内容重排。证据=`sessions/20260825-072409/evidence/SURF-101-i18n-markdown-five-channel.md`，调查=`testend/rig/formal-evidence/SURF-101-i18n-markdown-investigation-20260825.md`。
- 五通道：screen=`225.711667s`、backend=`347`、frontend=`5`、SSE=`170`、llmtap=`22`；messages durable=`1..24`、notifications durable=`1..4`，entities 无业务 durable 帧；LLM 带状态记录全 HTTP `200`；frontend 仅披露 macOS IMK 平台噪声，无 Flutter/Dart/布局/Unhandled 红线；rig-check/rig-down 通过且无残留。
- `judge.py` 写入 `E2/F2/B2/C4/G1`，formal journal=`2420`（2300 baseline + 120 live），coverage=`848/484/0`，anchors=`10/10`。`gap-too-fast`/`discovery-collapse` 按 `testend/rig/formal-evidence/SURF-101-ledger-alarm-reaudit-20260825.md` 独立复审并 ack，未改阈值/算法/法典/锚点/gate，`alarms.py check` clean；下一前线=`SURF-102 stage/function`，当前批次=`35/50`，未到 50 格不跑统一长门禁、不提交。P12 的 400+ Journey 仍推迟二期。

## 2026-08-25 · 历史收口：SURF-100 i18n/appName 产品名与 onboarding wordmark 正式五级入账，批次四十六 30/50

- 静态反查确认 `appName` 在中英文 locale 均为 `Anselm`，窗口标题、onboarding、launch-at-login、通知和无障碍调用点均走生成 locale；`toUpperCase()` 形成 `ANSELM` 是明确的品牌排版。新增双语精确回归，focused Flutter=`12/12`。
- 真实 App 用全新数据目录启动，onboarding 右上角显示 `ANSELM`；创建 `SURF-100 品牌检查` workspace 后回到中文 Chat，品牌没有翻译漂移。帧=`sessions/20260825-071353/evidence/frames/SURF-100-app-name-onboarding.png`、`SURF-100-app-name-final.png`，证据=`sessions/20260825-071353/evidence/SURF-100-i18n-app-name-five-channel.md`，调查=`testend/rig/formal-evidence/SURF-100-i18n-app-name-investigation-20260825.md`。
- 五通道：screen=`75.223333s`、backend=`136`、frontend=`4`、SSE=`8`、llmtap=`10`；三路 SSE 各连接一次，本路径没有业务实体，故没有 durable business frame，不伪造 seq。backend 无应用红线；frontend 唯一 error 是已披露的 macOS `IMKCFRunLoopWakeUpReliable` 平台日志；managed proof/install/models 成功；rig-check/rig-down 通过且无残留。
- `judge.py` 写入 `E2/F2/B2/C4/G1`，formal journal=`2415`（2300 baseline + 115 live），coverage=`848/483/0`，anchors=`10/10`。`gap-too-fast`/`discovery-collapse` 按 `testend/rig/formal-evidence/SURF-100-ledger-alarm-reaudit-20260825.md` 独立复审并 ack，未改阈值/算法/法典/锚点/gate，`alarms.py check` clean；该条为历史收口，下一前线=`SURF-102`，当前批次随后推进至=`35/50`，未到 50 格不跑统一长门禁、不提交。P12 的 400+ Journey 仍推迟二期。

## 2026-08-25 · SURF-099 i18n/tree JSON 树真实截断与无障碍正式五级入账，批次四十六 25/50

- 首轮静态审查发现中文 JSON tree 无障碍标签为 `JSON 树,$count 项`；修为自然中文 `JSON 树，$count 项`，重新生成 slang，并补中英文、invalid/circular/more-items 精确回归，focused `an_json_tree_test.dart` 与 `locale_boot_test.dart` 全绿。
- 真实 App 用 2100 项列表构造 Workflow，走调度 → `surf099_tree_workflow` → 运行 → 打开 → `tree` 节点；Scheduler 右岛真实显示 `JSON 树，1 项`（顶层 `text` 键），连续滚动到 `1993..1998` 后显示 `… 101 项已省略`，与 2000 节点上限完全一致。最终帧=`sessions/20260825-065746/evidence/frames/SURF-099-i18n-tree-final.png`，正式证据=`sessions/20260825-065746/evidence/SURF-099-i18n-tree-five-channel.md`，调查=`testend/rig/formal-evidence/SURF-099-i18n-tree-investigation-20260825.md`。
- 五通道：screen=`451.910000s`、backend=`670`、SSE=`348`、frontend=`4`、llmtap=`43`；notifications/messages/entities durable=`16..26 / 1..76 / 7..24`，LLM 全 HTTP `200`，rig-check/rig-down 通过。frontend 的 macOS IMK error 平台日志已在证据中披露，不是 Flutter/Dart/布局/Unhandled 红线。
- `judge.py` 写入 `E2/F2/B2/C4/G1`，formal journal=`2410`（2300 baseline + 110 live），coverage=`848/482/0`，anchors=`10/10`。`gap-too-fast`/`discovery-collapse` 按 `testend/rig/formal-evidence/SURF-099-ledger-alarm-reaudit-20260825.md` 独立复审并 ack，未改阈值/算法/法典/锚点/gate，最终 `alarms.py check` clean；该条为历史收口，下一前线=`SURF-101`，当前批次=`30/50`，未到 50 格不跑统一长门禁、不提交。

## 2026-08-25 · 历史收口：SURF-098 i18n/status 五态日志词 stop-and-fix 后正式五级入账

- 首轮真实日志观察发现明细行显示 raw `manual · failed`，与聚合 `2 完成 / 1 失败` 不一致；修复 `log_list_provider.dart`，四类日志主行统一使用 `AnStatus.fromRaw` → `t.status.*`，detail rows 保留 raw status 诊断事实；双语 provider 与状态 focused suite 全绿。
- 修复后真实 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-064736`、workspace=`ws_4c8a6c08c07f6523`，Computer Use 观察最终三行 `manual · 完成`、`manual · 失败`、`manual · 完成`，录屏帧=`sessions/20260825-064736/evidence/frames/SURF-098-i18n-status-final.png`。正式证据=`sessions/20260825-064736/evidence/SURF-098-i18n-status-five-channel.md`，调查=`testend/rig/formal-evidence/SURF-098-i18n-status-investigation-20260825.md`。
- 五通道：screen=`71.558333s`、backend=`207`、SSE=`192`、frontend=`3`、llmtap=`13`；rig-check/rig-down 通过，应用日志无红线，managed proof/install/models 全 200。SSE 保留 fixture workflow `entry.body.count` 缺失 `body` 的真实红事实，不隐瞒、不误归因。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal journal=`2405`（2300 baseline + 105 live），coverage=`848/481/0`，anchors=`10/10`。两条统计警报按 `testend/rig/formal-evidence/SURF-098-ledger-alarm-reaudit-20260825.md` 独立复审并 ack，未改阈值/算法/法典/锚点/gate，`alarms.py check` clean；该条为历史收口，当前批次已推进至=`25/50`，下一前线=`SURF-100`。

## 2026-08-25 · 历史收口：SURF-097 i18n/graph 工作流图节点词正式五级入账

- 真实 App 进入 `surf041_terminal_workflow` 图编辑器，Computer Use 观察添加节点菜单 `触发/动作/智能体/分支/审批`，并分别选择 `entry` 与 `inspect` 验证检查器的 `触发`/`动作` 标签；未保存、未改变图。双语 `NodeKind` 六项 focused Flutter=`14/14`，录屏帧与 AX/画面复核无 clipping/overlap/reflow/非用户跳变；`未知` fallback 只由测试覆盖。
- 五通道 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-062648`：screen=`270.311667s`，backend=`375` 行无应用红线，SSE 三流真实连接，frontend=`3` 行无 Flutter/Dart/布局/Unhandled 红线，llmtap challenge/install/models/chat completion 全 `200`；`rig-check`/`rig-down` 通过。正式证据=`sessions/20260825-062648/evidence/SURF-097-i18n-graph-five-channel.md`，调查=`testend/rig/formal-evidence/SURF-097-i18n-graph-investigation-20260825.md`。
- fixture 执行真实暴露 `entry.body.*` 与当前 payload 不匹配，SSE 原样记录 `no such key: body`；该红事实归后续执行契约格，不被图编辑器 i18n 绿证据掩盖。`judge.py` 写入 `G1/F1/B2/C4/G1`，formal journal=`2400`（2300 baseline + 100 live），coverage=`848/480/0`，anchors=`10/10`。两条统计警报按 `testend/rig/formal-evidence/SURF-097-ledger-alarm-reaudit-20260825.md` 独立复审并 ack，未改阈值/算法/法典/锚点/gate，`alarms.py check` clean；该条为历史收口，当前批次=`20/50`，下一前线=`SURF-099`。

## 2026-08-25 · 历史收口：SURF-096 i18n/startup 启动门控 stop-and-fix 后正式五级入账

- 首轮真实 App-first session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-061312` 发现崩溃页把 raw backend URL/英文内部错误渲染给用户；红场排除。stop-and-fix 删除启动门 `AnState.detail` 绑定，保留诊断 journal，并补双语 startup 六键与启动门 focused Flutter=`12/12`。
- 修复后 Computer Use 真实点击 `重试`，无 workspace 场景恢复到 `创建工作区`；为满足 L2 SSE gate，预置 workspace 在 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-061754` 重跑，三路 SSE 真实连接/EOF，点击重试后进入实体总览。两轮录屏与证据均封存，不混用无 workspace 的 SSE 结论。
- 五通道：第一轮 screen=`73.485000s`、backend=`61`、frontend=`3`、SSE 无业务帧、llmtap=`1` ready；第二轮 screen=`86.055000s`、backend=`84`、SSE 三流真实连接、frontend=`3`、llmtap=`10`。两轮无应用红线，`rig-check`/`rig-down` 通过且无残留；本路径无 chat completion，不伪造 completion。正式证据=`sessions/20260825-061312/evidence/SURF-096-i18n-startup-five-level.md`，L2 补证=`sessions/20260825-061754/evidence/SURF-096-i18n-startup-five-channel.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal journal=`2395`（2300 baseline + 95 live），`gen_coverage.py --check`=`848 rows / 479 carried / 0 tombstones`，anchors=`10/10`。两条统计警报按 `testend/rig/formal-evidence/SURF-096-ledger-alarm-reaudit-20260825.md` 独立复审并 ack，未改阈值/算法/法典/锚点/gate，最终 `alarms.py check` clean。该条为历史收口；当前批次=`20/50`，下一前线=`SURF-099`。

## 2026-08-25 · SURF-095 i18n/diff stop-and-fix 后正式五级入账，批次四十六 5/50

- 首轮静态反查确认差异查看的七个双语键均经过 `AnVersionDiff`/版本手风琴，但中文 `展开 diff`、`收起 diff` 泄漏内部术语，`只显变更`偏电报化，`展开全部($n 行)`排版不自然；红缺陷在真实 App 前先修复为 `展开差异`、`收起差异`、`仅显示变更`、`展开全部（$n 行）`，重新生成 slang 并补双语精确回归。
- focused Flutter=`43/43`，包含 locale、差异渲染、版本手风琴、折叠/整份切换、菜单二入口、长文本与虚拟化。真实 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-060117`：实体版本页构造 v2 后实际观察 v1→v2 红删绿增差异；Computer Use 打开菜单验证 `收起差异/展开全部（3 行）/设为活跃版本`，整份切换后验证 `收起差异/仅显示变更/设为活跃版本`。终帧=`sessions/20260825-060117/evidence/frames/SURF-095-final.png`。
- 五通道：screen=`139.673333s / 2784x1808 / H.264`；backend=`249` 行无应用红线，SSE=`14` 行且 notifications=`16..18`、entities=`7..8` 连续，frontend=`19` 行无 Dart/Flutter/布局/Unhandled 应用红线，llmtap=`10` 行真实 gateway challenge/install/models 全 `200`；AXTree 固定签名以 session review 归类为 Computer Use 观察器噪声，未知签名仍 fail-closed。本路径无 Chat completion 需求，不伪造 completion 或 messages durable 业务帧。`rig-check`/`rig-down` 全绿，无残留进程。
- 五级=`G1/F1/B2/C4/G1`；formal journal=`2390`（2300 baseline + 90 live），`gen_coverage.py --check`=`848 rows / 478 carried / 0 tombstones`，anchors=`10/10`。`gap-too-fast`/`discovery-collapse` 按 `testend/rig/formal-evidence/SURF-095-i18n-diff-investigation-20260825.md` 独立复审并 ack，未改阈值/算法/法典/锚点/gate，`alarms.py check` clean。下一前线=`SURF-096 i18n/startup`，当前批次=`5/50`。
...
- 首轮实体真实走查发现函数详情 Logs 页的 stale freshness：先打开空日志，再从同一详情页运行无 stdout/stderr 的函数，右侧终端更新但 Logs provider 不刷新；离开重进才出现记录。红 session=`20260825-012354` 不计入判断。
- 修复共享 `backend/internal/app/entitystream/entitystream.go`：启用的空输出 `Writer.Close` 发 durable `open → close`，重复收尾保持幂等；补 `TestWriter_CloseWithoutWritesEmitsTerminal` 与 Flutter `function logs refresh after a durable empty-run close`，backend entitystream test 通过、Flutter log provider=`7/7`。
- 绿色 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-013336` 真实覆盖实体总览/关系图、函数/处理器/智能体/工作流、控制/审批/触发器详情及运行/日志/版本/空态；空函数日志同页立即从空态变为 `1 完成 / 0 失败`。证据=`sessions/20260825-013336/evidence/SURF-083-i18n-entities-five-level.md`，告警复核=`testend/rig/formal-evidence/SURF-083-ledger-alarm-reaudit-20260825.md`。
- 五通道 rig-check/rig-down 全绿：screen=`2784x1808 / 242.805s`，backend=`448` 行无 WARN/ERROR/panic/fatal，SSE=`103` 行含空运行 `open/close` 与 workflow durable 终态，frontend=`3` 行仅正常 Flutter 启动/VM，llmtap challenge/install/models/chat 全 200。五级=`G1/F1/B2/C4/G1`；正式 journal=`2330`（2300 baseline + 30 live），`gen_coverage.py --check`=`848 rows / 466 carried judgments / 0 tombstones`。
- `gap-too-fast` 按真实 stop-and-fix、原始五通道证据和回归测试独立重审后 ack；未改阈值、算法、法典、锚点或 gate。当前批次=`45/50`，未到 50 格不跑统一长门禁、不提交；下一前线由 formal sequence gate 决定，P12 400+ Journey 继续按用户裁定推迟二期。

## 2026-08-25 · SURF-082 i18n/settings 正式五级入账，批次四十四 40/50

- 首轮真实设置走查发现中文 Models & keys 场景行泄漏 `Agent`；红 session=`20260825-010946` 不计入判断。修复 `scenarioAgent`、`referenceAgent`、`referenceAgentOverride` 为 `智能体` 系列，重新生成 slang 并补双语 locale 回归。
- 修复后绿色 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-011352`，真实 App 逐一走过 13 个设置面板与偏好/资源/系统三段目录；English 真实键盘搜索 `model` 命中 Models & keys，`proxy` 命中 Network 及三个代理项。中文 grouping/hint 由 settings search 纯函数/widget 门禁覆盖；Computer Use paste 不派发 Flutter `onChanged`，归类为观察器限制。
- 正式证据=`sessions/20260825-011352/evidence/SURF-082-i18n-settings-five-level.md`，独立告警复核=`testend/rig/formal-evidence/SURF-082-ledger-alarm-reaudit-20260825.md`。五级=`G1/F1/B2/C4/G1`，五通道 rig-check/rig-down 全绿，录屏终帧只含 Anselm，frontend 仅已知 IMK host noise；focused locale=`3/3`、settings search=`all passed`。
- `gap-too-fast` 按原阈值独立复审并 ack，未改阈值、算法、法典、锚点或 gate；正式 journal=`2325`（2300 baseline + 25 live），`gen_coverage.py --check`=`848 rows / 465 carried judgments / 0 tombstones`，alarms=`clean`。当前批次=`40/50`，未到 50 格不跑统一门禁、不提交。下一前线=`SURF-083 i18n/entities`，P12 400+ Journey 继续按用户裁定推迟二期。

## 2026-08-25 · SURF-081 i18n/chat 修复双语思考标签后正式五级入账，批次四十四 35/50

- 首轮真实中文 Chat 发现 UI reasoning 标签 `thought`/`thinking` 未翻译；保留红 session=`20260825-005929`，修复 `zh_CN.i18n.json` 为 `思考`/`思考中`，重新生成 slang 并补双语回归后，以新构建 session=`20260825-010531` 重跑。English/简体中文均走过 rail、composer、历史回答、流式思考态和 turn actions；真实回答 `acceptance-ping`/`live-ping` 收口。
- 正式证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-010531/evidence/SURF-081-i18n-chat-five-level.md`；红 session 明确排除。五通道 rig-check/rig-down 全绿，最终帧只含 Anselm，frontend 仅已知 IMK host noise；focused locale=`3/3`。
- 五级=`G1/F1/B2/C4/G1`；formal journal=`2320`（2300 baseline + 20 live），`gen_coverage.py --check`=`848 rows / 464 carried judgments / 0 tombstones`；`gap-too-fast` 已由 `testend/rig/formal-evidence/SURF-081-ledger-alarm-reaudit-20260825.md` 独立复审并 ack，未改阈值、算法、法典、锚点或 gate；当前批次=`35/50`，未到 50 格不跑统一长门禁、不提交。P12 的 400+ Journey 按用户裁定推迟二期。

## 2026-08-25 · SURF-080 detail-push 十二 kind 正式五级入账，修复 stale recorder，批次四十四 30/50

- `SURF-080 settings/detail-push` 真实覆盖 12 个 kind：`addKey/editKey/sandboxInstall/mcpServer/mcpAdd/mcpImport/mcpMarket/mcpInstall/addMemory/memory/addWorkspace/workspace`。空 MCP roster 的 15 秒 registry 等待最终正常落到内置 `102` 条快照；没有第三方安装、真实凭证或 fixture 残留。
- 真实可达性夹具：临时 custom key、失败 stdio MCP、memory、非当前 workspace；逐一观察详情后删除，最终 REST 只剩一 workspace、一个 managed Anselm key、零 MCP、零 memory。正式证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-004915/evidence/SURF-080-settings-detail-push-five-level.md`，补充 detail session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-002854`。
- stop-and-fix：App 重启后 window ID 变化但 geometry 不变，旧 `rig-rebind-app.sh` 未切 recorder；`rig-check` 拒绝后修为 identity 或 geometry 任一变化都切新录制段，新增静态回归。修复后短 session 的 `760→769` rebind 和五通道 `rig-check` 全绿；有效帧抽检只含 Anselm workspace editor。
- 五级=`G1/F1/B2/C4/G1`；formal journal=`2315`（2300 baseline + 15 live），`gen_coverage.py --check`=`848 rows / 463 carried judgments / 0 tombstones`，静态 rig 回归=`37/37`；`gap-too-fast` 已由 `testend/rig/formal-evidence/SURF-080-ledger-alarm-reaudit-20260825.md` 独立复审并 ack，未改阈值、算法、法典、锚点或 gate；当前批次=`30/50`，未到 50 格不跑统一长门禁、不提交。P12 的 400+ Journey 按用户裁定推迟二期。

## 2026-08-25 · SURF-079 正式五级入账，台架录制从矩形改为窗口 ID，批次四十四 20/50

- 旧矩形 session=`20260825-000908` 的封口帧发现 Codex 宿主污染，拒绝入账；修复 `rig-up.sh`、`rig-rebind-app.sh`、`rig-check.sh` 和文档，统一使用 `screencapture -l <appWindowId>`，未知遮挡仍硬失败。
- 修复后正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260825-001953`，证据=`sessions/20260825-001953/evidence/SURF-079-settings-panel-about-five-level.md`；窗口录屏=`2784x1808 / H.264 / 49.323333s`，终帧抽检只含 Anselm About。真实验证版本/Engine、GitHub Releases `404/no published release`、`Checking…`、字体许可、诊断 clipboard 与 `Copied`；backend 无应用红线，SSE 三流 clean EOF，frontend 无 Flutter/Dart/布局红线，llmtap challenge/install/models 全 `200`，无本路径 durable frame 或 completion 伪声明。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，正式 journal=`2305→2310`，COVERAGE=`848 rows / 461→462 judged / 0 tombstones`，anchors=`10/10`。`gap-too-fast` 由 `testend/rig/formal-evidence/SURF-079-ledger-alarm-reaudit-20260825.md` 独立复审并 ack；五格连续写入是同一真实 session 的账本序列化，不改阈值、算法、法典、锚点或 gate。最终 alarms clean，`gen_coverage.py --check`=`848 rows / 462 carried judgments / 0 tombstones`；当前批次=`20/50`，下一前线为 `SURF-080`。P12 400+ Journey 按用户裁定推迟二期。

## 2026-08-25 · 账本连续性硬闸补齐，拒绝正式清册静默从零起账

- 触发原因：正式 `RIG_HOME=/private/tmp/anselm-rig-formal-20260801-3` 的历史 `judgments.jsonl` 缺失，
  但仓内 `COVERAGE.md` 仍携带 `460` 个历史裁决单元；此前直接运行 `alarms.py` 会得到没有意义的零记录 clean。
- 修复：`testend/rig/judge.py` 在正式仓内清册已有 `✓/✗/~` 而 `$RIG_HOME/judgments.jsonl` 不存在时，
  对所有新裁决统一 fail-closed；临时外部 coverage fixture 不受影响。未改变阈值、算法、锚点答案或顺序策略。
- 回归：`python3 -m unittest testend/rig/test_judge.py -v`=`12/12`；新增测试确认不会创建 journal，也不会改动清册。
- 状态：SURF-078 的五通道产品证据仍保留但未写账；批次仍=`10/50`。只有恢复历史 journal 或完成单独、有审计依据的连续性重建，才允许继续过账。

连续性审计文件为 `testend/rig/formal-evidence/ledger-continuity-audit-20260825.md`：460 个已裁决行、2355 个指针、763 个唯一路径，其中 2332 个路径已不存在；因此清册不能反推可信 journal，不能执行隐式 rebase。

## 2026-08-25 · 用户裁定接受 Git/COVERAGE 历史基线，恢复从 SURF-078 继续

- 用户明确裁定不回头重验既有覆盖，直接继续施工；这不是把缺失 runtime journal 当作存在，而是一次有记录的历史基线决策。
- `testend/rig/rebuild_ledger.py --write --acknowledge-history` 将当前 COVERAGE 的 `2300` 个 carried 单格写入
  `/private/tmp/anselm-rig-formal-20260801-3/judgments.jsonl`，每条 `source=coverage-baseline`，并生成同根
  `ledger-baseline.json`，绑定 `coverageSha256=5504e89206d98992ef43a2943a1a8d41c9fa480699a2c48ff453928ab22de0b4`、
  `gitHead=c5621d44...` 和工作树是否 dirty。
- `alarms.py check`=`0 live judgments / 2300 baseline judgments excluded`；基线不参与速率/间隔/发现率曲线，
  从 SURF-078 起的真实 judge 裁决正常进入实时曲线。COVERAGE 校验仍为 `848 rows / 460 carried judgments / 0 tombstones`。
- 账本连续性不再阻塞当前前线；SURF-078 的五级真实证据仍未写账，批次=`10/50`，不提前跑长门禁、不提交。

## 2026-08-25 · SURF-078 五级裁决恢复，继续 SURF-079

- anchors 重新校准=`10/10`；正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260824-233028`，证据=`sessions/20260824-233028/evidence/SURF-078-settings-panel-shortcuts-formal.md`。
- `judge.py` 依次写入 `G1/F1/B2/C4/G1` 五条 live judgment；L2 绑定同一封存 session，未拼接其他台架证据。
- 中途发现 baseline manifest 若要求当前 COVERAGE hash 恒等，会把合法的新 live 写入错误拒绝；已修为 baseline 单格集合子集校验，并补测试。`test_judge`=`13/13`、`test_alarms`=`1/1`、`test_scope`=`19/19`。
- 最终：`alarms.py check`=`5 live judgments / 2300 baseline judgments excluded` clean；`gen_coverage.py --check`=`848 rows / 461 carried judgments / 0 tombstones`；批次=`15/50`。下一原子前线=`SURF-079`，未到 50 格不跑统一长门禁、不提交。

## 2026-08-24 23:40 · SURF-078 真实路径完成，但历史账本缺失，拒绝伪造过账

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260824-233028`；`rig-check` live 与 `rig-down` 均通过，录屏=`492.528333s`，证据=`sessions/20260824-233028/evidence/SURF-078-settings-panel-shortcuts-formal.md`。
- 用户在真实 Anselm App 中手动完成第一行 `⌘J` 改绑；离开设置再进入后仍持久化。第二行尝试同一 `⌘J` 显示冲突且不覆盖，Escape 取消录制，Reset all 恢复六个默认值；回到 Chat 后 `⌘J` 真实收起再展开左侧岛。普通 `a` 的无修饰键拒绝也已在正式 session 观察。
- 五通道封口：backend=`581` 行、SSE=`9` 行、frontend=`4` 行、LLM=`10` 行、screen 可读；backend/frontend 无应用红线，设置路径无业务耐久 SSE 帧、无模型 completion，不作虚假声称。
- 账本 gate 审计发现当前 `/private/tmp/anselm-rig-formal-20260801-3` 没有历史 `judgments.jsonl`、`alarms.json` 或 `anchor-check.json`；`gen_coverage.py --check` 仍为 `848 rows / 460 carried judgments / 0 tombstones`，工作记录保留历史 formal ledger=`2335 judgments`。用户目录旧台架根虽找到 `judgments.jsonl`，但只有 `76` 条、覆盖 `16` 个项目，不能恢复完整连续性。本轮只新生成并通过 `10/10` anchor calibration；`alarms.py` 只能看到当前空 journal，不能代表连续运行。
- 因此本轮**没有调用 `judge.py`，没有改 COVERAGE 五格，也没有把本 session 写成绿账**。当前前线仍是 SURF-078、批次=`10/50`；下一动作是恢复/建立有审计依据的 ledger continuity，再按 `G1/F1/B2/C4/G1` 顺序过账。不要用“历史工作记录中的 2335”替代缺失的原始 journal，也不要用当前零 journal 的 alarm clean 冒充全程 clean。

## 2026-08-20 14:05 · SURF-078 快捷键面板真实走查止于 Computer Use Command 注入阻塞，未写入判决

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260820-135637`；真实 macOS App、backend、ssetap、llmtap、窗口录屏均由 conductor 归属。`rig-check` live 全绿，`rig-down` 封口；screen=`376.895000s / 2560x1584 / H.264 / 60fps`。
- 真实走过 Settings → Shortcuts：六条命令及默认逐键帽可见；点击键帽进入 `Press a new chord…`；真实普通 `a` 被拒并显示 `A chord must include a modifier (⌘/Ctrl…)`；Escape 取消并恢复键帽；Reset all 入口可达。backend=`450` 行、SSE=`9` 行、frontend=`3` 行、llmtap=`10` 行，应用红线扫描 clean。
- `sky.press_key` 尝试 `super+9`、`meta+9`、`cmd+9`、`command+9`、`ctrl+9`、`super+shift+9`，均未向 Flutter 送出 `meta/control` 状态；App 的拒绝反馈是正确行为。不能用直接改偏好或测试注入冒充成功改绑，所以成功改绑、冲突、持久化、单项 Reset 与改绑后全局键路径不判绿。
- 支持性验证：`mise exec -- flutter test test/features/settings/s6_shortcuts_test.dart test/core/shortcuts/shortcut_bindings_test.dart`=`8/8` passed；这不能替代真实 App Command 注入。证据=`sessions/20260820-135637/evidence/SURF-078-settings-panel-shortcuts-blocked.md`。
- formal ledger 仍=`2335 judgments`，COVERAGE=`848 rows / 460 judged / 0 tombstones`，批次四十四=`10/50`；当前前线原地保持 `SURF-078`，恢复真实 Command 注入后续跑。不得为此运行统一批次门禁或提交。

## 2026-08-20 14:18 · SURF-078 第二次洁净 session 复现同一输入桥阻塞，仍未写入判决

- 新鲜隔离 data root=`/private/tmp/anselm-data-surf078-20260820-r2`，session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260820-140851`；真实 App 重新启动并通过 `rig-check`，收台后录屏=`144.610000s / 2560x1584 / H.264 / 60fps`。
- 第二次真实走查仍能进入 Shortcuts、进入录制、拒绝普通 `a`、Escape 恢复默认键帽；`super+j`、`super+9`、`Meta_L+j`、`Super_L+j`、`Super_L+J`、`Super_L+B` 均未产生 Flutter `meta/control` 状态，`Super_L+B` 也未触发现有全局左岛快捷键。backend=`218`、SSE=`9`、frontend=`3`、llmtap=`10`，无应用红线。
- 这确认不是旧 session、焦点残留或数据目录污染；仍是 Computer Use macOS 键盘桥边界。证据=`sessions/20260820-140851/evidence/SURF-078-command-injection-repro.md`。不改产品代码、不直接改偏好、不写账。
- formal ledger/COVERAGE/批次保持 `2335 / 848 rows·460 judged / 10/50`，前线继续锁定 `SURF-078`，等待可产生真实 Command 事件的输入通道后原地续跑。

## 2026-08-20 14:22 · SURF-078 权限诊断：Accessibility 已开，Input Monitoring 未授权，未擅自修改

- 通过只读 Computer Use 检查 macOS System Settings：Privacy & Security → Accessibility 中 `Codex Computer Use` 为 `on`；Input Monitoring 名单只有 `Overcooked2`，没有 `Codex Computer Use`。
- 该权限可能影响 Computer Use 的键盘事件桥，但属于安全敏感系统权限；本轮不代用户点击开启、不把权限缺失改写成产品缺陷。当前前线、正式账本和批次保持不变，等待用户在动作时确认后再验证。

## 2026-08-20 14:31 · SURF-078 跨应用对照排除 Input Monitoring 假设

- 原生 System Settings 的 Input Monitoring 搜索框中，真实 `sky.press_key({key:"command+a"})` 能选中已有 `foo` 并由后续输入替换为 `bar`；Computer Use 的 Command 生成能力在原生 Cocoa 文本控件中有效。
- Anselm 独立真实 App 的 Flutter onboarding 文本框中，同样的 `command+a` 与 `ctrl+a` 均未选中已有文本：`foo` 后输入 `bar` 得到 `foobar`，随后 `ctrl+a` 后输入 `baz` 得到 `bazfoobar`。这与快捷键面板的 `meta/control` 缺失一致，指向 Computer Use 合成事件进入 Flutter macOS 引擎的边界，而非 Input Monitoring 权限缺失。
- 不开启无关的安全权限、不修改 Anselm 产品键盘语义、不用 `select_text` 或直接偏好写入冒充用户改绑。`SURF-078` 仍锁定，等待真实物理键盘输入或修复/替换合法的 Computer Use bridge 后原地复验。

## 2026-08-20 13:41 · SURF-076 L1 过账，Advanced limits 用户入口与范围确认完成

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260820-133336`；真实 App、外接 backend、独立 ssetap、llmtap 和窗口录像均由 conductor 归属。`rig-check` live 全绿后 `rig-down` 封口，`screen.mov`=`374.376667s / 2560x1584 / H.264 / 60fps`。
- 真实走过 Settings → Advanced limits，核对 17 个 server schema 字段、agent/context/timeout/tools/guards 五组、machine-wide 文案、滚动可达性、单字段编辑、非法整数与 `(0,1)` 比例回滚、Reset all 二次确认和默认值恢复。`maxSteps 25→30`、`invokeMaxTurns 10→11` 的 PATCH/GET 与兄弟字段保留由 backend journal/REST 互证。
- 五通道封口：backend=`484` 行、SSE=`15` 行、frontend=`4` 行、llmtap=`10` 行；backend/frontend 无应用红线，三路 SSE 均真实 connect，确定性 settings 路径无 completion，不伪造 LLM 证据。正式证据=`sessions/20260820-133336/evidence/SURF-076-settings-panel-limits-five-level.md`。
- `judge.py` 已写入 `SURF|settings/panel-limits L1=pass/G1`；anchors=`10/10`，`alarms.py check`=`clean (2325 judgments on record)`（写入前检查）。formal ledger=`2325→2326 judgments`，COVERAGE=`848 / 458→459 judged / 0`，批次四十四=`1/50`。本格完成后立即重读证据并确认台架收台；下一格为 L2，未跑统一批次门禁。

## 2026-08-20 13:42 · SURF-076 L2 过账，五通道数据真相一致

- 同一已封口 session 通过 `judge.py` 的 level-2 session gate：`manifest.json` 身份、`screen.mov` 可读、backend/frontend/llm/sse journal 均非空，三路 SSE 均有真实 connect。
- `SURF|settings/panel-limits L2=pass/F1` 已写入 formal ledger；formal ledger=`2325→2327 judgments`，COVERAGE=`848 / 458→459 judged / 0`，批次四十四=`2/50`。L2 后重读 evidence 与原始 journal，无追加红线；下一格为 L3，统一门禁仍后置。

## 2026-08-20 13:43 · SURF-076 L3 过账，编辑/滚动/回滚的稳定性通过

- `SURF|settings/panel-limits L3=pass/B2` 已写入。录屏中字段编辑、滚动到 timeout/tools/guards、非法值恢复和 reset dialog 收口均保持内容区域稳定，没有观察到非用户触发的既有内容位移或流式夺视口。
- formal ledger=`2325→2328 judgments`，COVERAGE=`848 / 458→459 judged / 0`，批次四十四=`3/50`。L3 后重读 `screen.mov` 可读性与 session journal，无追加红线；下一格为 L4，统一门禁仍后置。

## 2026-08-20 13:44 · SURF-076 L4 过账，Advanced limits craft bar 通过

- `SURF|settings/panel-limits L4=pass/C4` 已写入。逐帧复核 scope badge、五组 hierarchy、字段说明/单位/区间文案、修改行 reset affordance、错误 toast 和危险 reset confirmation；视觉层级清楚、间距稳定，没有把技术细节误压到用户主路径。
- formal ledger=`2325→2329 judgments`，COVERAGE=`848 / 458→459 judged / 0`，批次四十四=`4/50`。L4 后重读录屏关键画面、证据文件和法典引用，无追加红线；下一格为 L5，统一门禁仍后置。

## 2026-08-20 13:45 · SURF-076 五级闭合，统计警报独立复审并销账

- `SURF|settings/panel-limits L5=pass/G1` 已写入，入口、命名、machine-wide 解释、字段描述、校验提示和 Reset all affordance 均从真实 Settings 侧栏可达；本格五级=`G1/F1/B2/C4/G1`。
- L5 写入后的原阈值检查按机制打开 `gap-too-fast` 与 `discovery-collapse`。没有直接删警报或调阈值：重读五级证据、封口录屏、backend/SSE/frontend/LLM journal，复核 anchors=`10/10`，确认五列是同一格不同判据的逐级审查且负向输入已真实执行；随后用 `alarms.py ack` 写入独立 resolution note，未改阈值、算法、法典或 gate。最终 `alarms.py check`=`clean (2330 judgments)`。
- formal ledger=`2325→2330 judgments`，COVERAGE=`848 / 459 judged / 0`，批次四十四=`5/50`。本批远未到 50 格，不跑统一长门禁、不提交；下一原子前线为 `SURF-077`。

## 2026-08-20 13:51 · SURF-077 L1 过账，Network 入口与用户目的确认完成

- 真实 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260820-134625`；从 Settings 侧栏打开 Network，确认 machine scope、HTTP/HTTPS/Bypass 三字段、Save 和 restart caveat；空值代表 direct，填写与清空两条用户目的均真实走过。
- `SURF|settings/panel-network L1=pass/G1` 已写入；formal ledger=`2330→2331 judgments`，COVERAGE=`848 / 459→460 / 0`，批次四十四=`6/50`。五通道已封口，证据=`sessions/20260820-134625/evidence/SURF-077-settings-panel-network-five-level.md`；L1 后立即重读证据，下一格为 L2。

## 2026-08-20 13:52 · SURF-077 L2 过账，整体 PATCH 与 REST 真相一致

- 同一封口 session 通过 level-2 session gate：manifest 身份、可读 screen、五通道非空、三路 SSE connect 均满足。backend 记录填写态 full PATCH 200/GET 200，清空态 full PATCH 200/GET 200，最终 REST 为 `{}`。
- `SURF|settings/panel-network L2=pass/F1` 已写入；formal ledger=`2330→2332 judgments`，COVERAGE=`848 / 460→460 / 0`，批次四十四=`7/50`。L2 后重读 evidence 与 journal，无新增红线；下一格为 L3。

## 2026-08-20 13:53 · SURF-077 L3 过账，网络表单交互稳定

- `SURF|settings/panel-network L3=pass/B2` 已写入。录屏逐帧复核空态 placeholder、三字段输入、Save 进行中/成功提示、离开重进和清空收口；没有观察到非用户触发的既有内容位移或异常视口夺取。
- formal ledger=`2330→2333 judgments`，COVERAGE=`848 / 460→460 / 0`，批次四十四=`8/50`。L3 后重读 `screen.mov`、frontend journal 和 evidence，无新增红线；下一格为 L4。

## 2026-08-20 13:54 · SURF-077 L4 过账，Network craft bar 通过

- `SURF|settings/panel-network L4=pass/C4` 已写入。逐帧复核 machine scope badge、purpose hint、label-above field rhythm、mono 输入面、restart warning callout、Save 主次层级和成功 toast；视觉信息层级清楚，字段间距与操作反馈稳定。
- formal ledger=`2330→2334 judgments`，COVERAGE=`848 / 460→460 / 0`，批次四十四=`9/50`。L4 后重读关键画面、证据文件和法典引用，无新增红线；下一格为 L5。

## 2026-08-20 13:56 · SURF-077 五级闭合，Network 统计警报独立复审并销账

- `SURF|settings/panel-network L5=pass/G1` 已写入，入口、字段命名、Bypass 格式、Save 和「重启后生效」后果均可由新用户自行理解；本格五级=`G1/F1/B2/C4/G1`。
- L5 后原阈值检查再次打开 `gap-too-fast` 与 `discovery-collapse`。逐条重读本 session 的 204.678333s 录屏、backend 291 行、SSE 三路 connect、frontend/LLM journals 与 evidence，确认是五判据在单一封口证据上的连续落账，不是橡皮章；真实空直连/填充/离开重进/清空负向行为已执行。用 `alarms.py ack` 写 resolution note，未改阈值、算法、法典或 gate；最终 `alarms.py check`=`clean (2335 judgments)`。
- formal ledger=`2330→2335 judgments`，COVERAGE=`848 / 460 judged / 0`，批次四十四=`10/50`。本批不满 50 格，不跑统一长门禁、不提交；下一原子前线为 `SURF-078`。

## 2026-08-20 13:22 · SURF-075 出厂重置复核 stop-and-fix，恢复 LaunchServices 重启并通过 replacement rebind

- 用户再次确认真实出厂重置。第一次复核错误地使用外接 backend：App 的 `backend.stop()` 日志显示 `child=null`，数据目录未删；该 session 不计证据。切换 `RIG_APP_OWNS_BACKEND=1` 后，`/private/tmp` 又被 macOS App sandbox 拒写；改用 App 容器数据根后，发现旧 `app_relaunch.dart` 直接 exec 同 bundle 在退出竞态下不留下 replacement。两类前置失败均停止并保留，不写账。
- stop-and-fix 将 `frontend/lib/core/platform/app_relaunch.dart` 恢复为 macOS 正确的 `open -n <bundle>` LaunchServices 实例重启语义；正式台架改用默认容器数据根 `.anselm`，避免 LaunchServices 丢失自定义 debug 环境造成假失败。Dart 构建随洁净 `rig-up` 通过。
- 最终真实 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260820-132029`：用户输入 `Anselm` 并点击 `Erase everything & relaunch`；旧 App/sidecar=`76065/76097`、旧 port=`63489` 优雅收尾，`.anselm` 数据目录消失；replacement App/sidecar=`76191/76193`、port=`63673` 回到 `Create a workspace`，无旧 workspace 幽灵。`rig-rebind-app.sh` 与 `rig-check.sh` 全绿，五通道重新归属；SSE tap 已按新端口重连，录屏窗口 segment 已封口并切换。
- 这是对已入账 SURF-075 的 stop-and-fix 复核，不新增判决；formal ledger/COVERAGE/批次计数保持 `2325 / 848 rows·458 judged / 50/50`。下一原子前线仍为 SURF-076；修复后的 working diff 留待下一 50 格收口门禁统一审计。

## 2026-08-20 12:53 · SURF-076 台架前置失败后 stop-and-fix，未写入判决

- 首次正式启动使用显式 `RIG_HOME=/private/tmp/anselm-rig-formal-20260801-3`、隔离数据目录和真实 llmtap；llmtap 已启动，但 `rig-up.sh` 在外接 backend 健康探针处因 `set -u` 直接展开空 `AUTH_ARGS` 提前退出。该失败属于 conductor 仪器，不是产品证据；失败 session 不进入 COVERAGE，账本仍为 `2325 judgments`，批次四十三仍为 `50/50`。
- stop-and-fix 将 `rig-up.sh` 与 `rig-check.sh` 的 backend 请求收束到 `curl_backend`：外接 backend 不传空鉴权参数，App-owned backend 仍精确携带 Bearer token；`test_scope.py` 新增静态守卫，`bash -n` 与 rig scope/screen 测试=`20/20` 通过，进程审计无残留。
- README §5.2、LOOP 当前前线和本日志同步重述；SURF-076 保持未判，修复后从洁净台架重新启动。

## 2026-08-20 12:25 · SURF-075 正式五级入账，Storage & logs 与用户确认后的出厂重置通过，批次四十三 50/50

- 真实 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260820-121523`，data root=`/Users/sunweilin/Library/Containers/website.anselm.app/Data/.anselm-surf075-formal-r4`。真实覆盖数据目录、磁盘/数据库/附件统计、诊断复制、Run 历史保留、数据库压缩、Reset local preferences、危险区精确确认和 Factory reset；用户输入 `Anselm` 后点击 `Erase everything & relaunch`，replacement App 显示 `Create a workspace`。
- 首轮外部 backend 持有数据目录时的递归删除失败 `Operation not permitted` 保留为红诊断，不计绿。修复为异步有界删除、先停 App-owned backend、失败回到可操作危险区，并让显式 `ANSELM_DATA_DIR` 参与全新安装判定；台架增加 App/sidecar/token/SSE 原子 rebind。重启后窗口仅发生 1px 几何变化，conductor 封存旧录屏段并启动新 crop，拒绝 stale crop。
- 旧 App/sidecar=`57551/57583`、旧 port=`57881`；replacement App/sidecar=`57682/57684`、port=`58408`。录屏原段=`322.903333s`、重启后段=`17.951667s`、合并=`340.870000s / 2560x1584 / H.264 / 60fps`；`rig-check`/`rig-down`、ffprobe 和进程收台通过。backend=`119`、SSE=`966`、frontend=`227`、llmtap=`10`；三路 SSE 均连接、无 gap，managed challenge/install/models 全 `200`，应用红线扫描 clean。确定性 reset 路径无 completion，不伪造模型证据；reset 后数据 root/`anselm.db` 不存在，符合空安装尚未创建 workspace 的状态。
- 正式证据=`sessions/20260820-121523/evidence/SURF-075-settings-panel-storage-five-level.md`，警报复审=`sessions/20260820-121523/evidence/SURF-075-ledger-alarm-reaudit.md`。`judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2320→2325 judgments`，COVERAGE=`848 / 457→458 / 0`，anchors=`10/10`。写账触发的 `gap-too-fast`/`discovery-collapse` 已按独立红/绿边界、完整五通道 journals 和 anchor calibration 逐条 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2325 judgments)`；批次四十三达到 `50/50`，下一原子前线为 `SURF-076`。P12 400+ Journey 按用户裁定推迟二期。
- 批次收口门禁全部通过：focused settings/process Flutter=`40/40`，rig Python=`44/44`，root `make verify` frontend/backend/docs/demo 全绿且 Flutter 四组=`5376 tests`，完整 `make -C backend testend`=`288.411s` exit `0`；coverage/anchors/alarms、changed-Go `gofmt`、`git diff --check` 和进程审计均通过。当前只剩最终 staged diff 审计与提交。

## 2026-08-20 00:49 · SURF-074 正式五级入账，工作区创建/切换/编辑/删除与最后工作区保护通过，批次四十三 45/50

- 真实 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260820-002515`，data=`/private/tmp/anselm-data-surf074-20260820-r1`。真实走过工作区名册、最小新建、创建后落入 Chat、点行热切换、名称编辑、六色、危险区真实统计、精确名称删除和最后工作区保护；`Demo Hub` 删除后名册只剩 `Product Lab · Current`，最后编辑页没有删除区。
- 录屏=`1286.935000s / 2560x1584 / H.264 / 60fps`；backend=`1429` 行，create/activate/edit/delete=`201/200/200/204`，无 WARN/ERROR/panic/fatal/exception/traceback；SSE=`15` 行，两个 workspace scope 的三流独立连接并正常断开；frontend=`4` 行仅正常启动/VM/已知 CapsLock host noise；llmtap=`16` 行 managed challenge/install/models 全 `200`；focused Flutter=`19/19`；`rig-check`/`rig-down` 通过。
- 正式证据=`sessions/20260820-002515/evidence/SURF-074-settings-panel-workspaces-five-level.md`，警报复审=`sessions/20260820-002515/evidence/SURF-074-ledger-alarm-reaudit.md`。`judge.py` 写入 `G1/F1/B2/C4/G1`，formal ledger=`2315→2320 judgments`，COVERAGE=`848 / 456→457 / 0`，anchors=`10/10`。`gap-too-fast`/`discovery-collapse` 按 SURF-074 独立复审逐条 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2320 judgments)`；下一前线 `SURF-075 settings/panel-storage`，本批 `45/50`，50 格前不统一门禁、不提交。P12 400+ Journey 按用户裁定推迟二期。

## 2026-08-20 00:22 · SURF-073 正式五级入账，Sandbox 健康门/运行时/五 owner 环境/GC 闭环通过，批次四十三 40/50

- 真实 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260820-001124`，data=`/private/tmp/anselm-data-surf073-20260820-r1`，workspace=`ws_a8955c11bf9eccd4`，录屏=`572.355000s / 2560x1584 / H.264 / 60fps`。走过健康态、磁盘、Python 3.13 安装/删除、Python 3.12 被引用删除 `409`、未引用运行时删除取消/确认、五 owner tab、环境删除、GC 两步确认和 degraded Retry 恢复。
- backend=`726` 行，唯一 WARN 是故意制造的 sandbox bootstrap 失败；SSE=`11` 行并捕获 direct delete/GC 的 `sandbox.env_deleted`；frontend=`4` 行仅正常启动/VM/已知 CapsLock host noise；llmtap=`10` 行 managed challenge/install/models 全 `200`；focused Flutter=`47/47`；`rig-check`/`rig-down` 通过。真实键盘非法 dotnet 版本返回 `422` 与具体修复提示；Computer Use `set_value` 未触发 `onChanged` 的观察已作为仪器限制记录并清理。
- 正式证据=`sessions/20260820-001124/evidence/SURF-073-settings-panel-sandbox-five-level.md`，警报复审=`sessions/20260820-001124/evidence/SURF-073-ledger-alarm-reaudit.md`。`judge.py` 写入 `G1/F1/B2/C4/G1`，formal ledger=`2310→2315 judgments`，COVERAGE=`848 / 455→456 / 0`，anchors=`10/10`。`gap-too-fast`/`discovery-collapse` 独立复审后逐条 ack，最终 `alarms.py check`=`clean (2315 judgments)`；下一前线 `SURF-074 settings/panel-workspaces`，本批 `40/50`，50 格前不统一长门禁、不提交。P12 400+ Journey 按用户裁定推迟二期。

## 2026-08-20 00:06 · SURF-072 修复后正式五级入账，Memory 面板从 slug/置顶/编辑保护到删除闭环通过，批次四十三 35/50

- 首轮真实 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-234423` 发现两个真实产品缺陷并停下：合法 slug 输入后旧错误仍显示；详情有未保存修改时点击 Memory 面包屑直接离开。该 session 不计绿。
- 修复为 settings detail pop guard：`MemoryEditor` 缓存 detail controller，面包屑与 Escape 使用 `requestPop()`，dirty 状态经统一 `_confirmBack()`；输入 onChanged 清理旧错误。新增 `settings_shell_test.dart` 回归，锁定 Keep editing/Discard 两分支，修复期间顺手消除了 dispose 阶段错误读取 Riverpod ref 的实现缺陷。
- 修复后新 App session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-235804`，删除收口 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260820-000144`，data=`/private/tmp/anselm-data-surf072-20260819-r1`，workspace=`ws_be9766a8964b8449`，录屏=`121.830000s + 84.088333s`。真实走过空/名册、All/Pinned、搜索命中/无匹配、新建非法→合法并置顶、编辑锁名/内容保存、dirty breadcrumb、删除 Cancel/Confirm。正式证据=`sessions/20260820-000144/evidence/SURF-072-settings-panel-memory-five-level.md`，警报复审=`sessions/20260820-000144/evidence/SURF-072-ledger-alarm-reaudit.md`。
- 五通道：backend=`186/139` 行无应用红线，预期 incomplete create `400` 后补齐必填字段真实 `200`；ssetap 三流连接并捕获 `memory.created`/`memory.deleted`；frontend=`4/3` 行仅正常启动/VM/已知 IMK host noise；llmtap wiring 通过，本确定性路径无 completion 伪造；`rig-check`/`rig-down` 两轮通过。focused Flutter=`68/68`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2305→2310 judgments`，COVERAGE=`848 / 454→455 / 0`，anchors=`10/10`。`gap-too-fast`/`discovery-collapse` 按 SURF-072 独立复审逐条 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2310 judgments)`。下一前线为 `SURF-073 settings/panel-sandbox`，本批 `35/50`，50 格前不统一长门禁、不提交。P12 400+ Journey 按用户裁定推迟二期。

## 2026-08-19 23:41 · SURF-071 正式五级入账，MCP 设置面板从市场到失败审计与导入删除闭环通过，批次四十三 30/50

- `SURF-071 settings/panel-mcp` 在真实 App + managed gateway 上完成空态市场、102 条 marketplace、关键词搜索、Context7 计划页、手动 stdio 失败态、SSE/Streamable HTTP 表单切换、详情 Tools/Call history/stderr、有效 `mcp.json` 导入、同名跳过和 soft-delete。失败 MCP 真实显示 `failed` 与 sandbox 错误；外部 API key 未提交；没有产品级 stop-and-fix 缺陷。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-232740`，data=`/private/tmp/anselm-data-surf071-20260819-r1`，workspace=`ws_470266a26e97918d`，录屏=`564.475000s`。正式证据=`sessions/20260819-232740/evidence/SURF-071-settings-panel-mcp-five-level.md`，警报复审=`sessions/20260819-232740/evidence/SURF-071-ledger-alarm-reaudit.md`；`rig-check`/`rig-down` 通过，五通道 journals 封存。
- backend=`660` 行无 WARN/ERROR/panic/FATAL；ssetap 捕获两次实体 connecting→failed、`mcp.installed`×2、`mcp.removed`×2，notifications durable seq=`1..4`；frontend=`5` 行只有已知 macOS IMK host noise；llmtap managed challenge/install/models/quota 全 `200`；focused Flutter=`47/47`。中文输入法导致的第一次非法 JSON 归类为 Computer Use 输入限制，英文输入法后的有效导入成功作为产品证据。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2300→2305 judgments`，COVERAGE=`848 / 453→454 / 0`。`gap-too-fast`/`discovery-collapse` 由 SURF-071 独立复审后逐条 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2305 judgments)`；下一前线为 `SURF-072 settings/panel-memory`，本批 `30/50`，50 格前不统一长门禁、不提交。P12 400+ Journey 按用户裁定推迟二期。

## 2026-08-19 23:22 · SURF-070 正式五级入账，生成媒体从产物到播放器/全屏闭环通过，批次四十三 25/50

- `SURF-070 core/media-viewer` 在真实 managed gateway 上完成图片和视频媒体闭环。图片从生成工具卡进入全尺寸 `Media viewer`；3 秒 H.264/AAC 视频从海报进入真实原生播放器，出现可见画面、`0:00 / 0:03`、暂停、进度条、`0:03 / 0:03`、`Replay`，点击中段定位到 `0:01 / 0:03`，再进入共享 live controller 的全屏查看器并关闭回归。没有发现产品级 stop-and-fix 缺陷。
- 首轮 session 因中途重启 App 造成 manifest PID 不再代表当前窗口，L2 没有被默许；旧 session 正常封存后，以同一 data=`/private/tmp/anselm-data-surf070-20260819-r1` 起洁净 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-231808`。洁净录屏=`261.528333s / 2560x1584 / H.264 / 60fps`，workspace=`ws_3656d8ab6c6a28d2`。五通道 `rig-check` 全绿，`rig-down` 清空进程组；正式证据=`sessions/20260819-231808/evidence/SURF-070-core-media-viewer-five-level.md`，警报复审=`testend/rig/formal-evidence/SURF-070-ledger-alarm-reaudit.md`。
- backend=`159` 行无应用 WARN/ERROR/panic/FATAL；洁净 ssetap=`4` 行为 discovery+三流 connect，上一轮生成 session 保留业务 SSE；frontend=`4` 行仅正常启动/VM/已知 macOS IMK host noise；上一轮 llmtap 保留生成 wire。focused Flutter=`35/35`，附件 REST/Range、SQLite blob 和 `ffprobe` 均证明视频不是空壳。`judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2295→2300 judgments`，COVERAGE=`848 / 452→453 / 0`。两条统计警报经独立 re-audit 串行 ack，最终 `alarms.py check`=`clean (2300 judgments)`；下一前线为 `SURF-071 settings/panel-mcp`，本批 `25/50`，50 格前不统一长门禁、不提交。P12 400+ Journey 按用户裁定推迟二期。

## 2026-08-19 22:51 · SURF-069 正式五级入账，Models & keys 全资源面板通过，批次四十三 20/50

- `SURF-069 settings/panel-models-keys` 在全新 workspace `Acceptance SURF-069` 上真实覆盖受管免费档卡、克隆音色空库存、managed key、Dialogue/Utility/Agent/Image/Speech/Video 六类 scenario defaults 和 Search key 空态。六个 Change 入口真实展开/关闭；quota 与 model list refresh 真实穿过 backend 和 managed gateway，最终没有配置漂移。`2 of 2 slots free`、音频附件登记引导和 focused inventory tests 共同锁定“库存位不是日配额”的产品语义。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-224143`、data=`/private/tmp/anselm-data-surf069-20260819-r1`、workspace=`ws_b3a9e6654c009416`、录屏=`518.718333s / 2560x1584 / H.264 / 60fps`。backend=`593` 行无应用红线；ssetap=`8` 行，workspace 创建后自动发现三流，设置纯读路径无 durable 业务帧且停机正常断开；frontend=`4` 行仅已知 host noise；llmtap=`16` 行，managed proof/install/models/quota 全 `200`；focused Flutter=`55/55`、rig-check/rig-down、coverage、diff、process audit 通过。关键帧和正式证据=`sessions/20260819-224143/evidence/SURF-069-settings-panel-models-keys-five-level.md`，警报复审=`testend/rig/formal-evidence/SURF-069-ledger-alarm-reaudit.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2290→2295 judgments`，COVERAGE=`848 / 451→452 / 0`。写账触发的 `gap-too-fast`/`discovery-collapse` 已由 SURF-069 独立重审并串行 ack，未改门禁，最终 `alarms.py check`=`clean (2295 judgments)`。下一前线由 formal sequence gate 决定；批次四十三 `20/50`，50 格前不统一门禁、不提交。P12 400+ Journey 按用户裁定推迟二期。

## 2026-08-19 22:38 · SURF-068 正式五级入账，Chat 设置与真实工具取消闭环通过，批次四十三 15/50

- `SURF-068 settings/panel-chat` 在全新 workspace `Acceptance SURF-068` 上真实覆盖右岛自动登台三档、发送键两档、Web fetch `Local → Jina → Local` 和默认模型跳转行；真实聊天经 managed gateway 返回 `OK.`，Models & keys 往返后设置保持，最终 SQLite `web_fetch_mode=local`。
- 第二个真实回合触发 `Glob`，UI 显示工具 activity；由于 `Glob` 不在产品定义的 stage-worthy 集合，没有把右岛不自动展开误判为缺陷。模型把“当前工作目录”解释为 `~`，递归 Glob 等待约 53 秒后由 Computer Use 点击 Stop；UI 显示 `Interrupted`，backend `/cancel`、SSE tool/message close 均为 cancelled，无孤儿回合。该相邻工具意图/workdir 引导观察项保留，不计 Chat 设置绿格。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-221915`、data=`/private/tmp/anselm-data-surf068-20260819-r1`、workspace=`ws_02acc0a8ce4f704e`、录屏=`963.813333s / 2560x1584 / H.264 / 60fps`。backend=`1063` 行无应用红线；ssetap=`79` 行、三流真实连接；frontend=`5` 行仅已知 host noise；llmtap=`25` 行，managed proof/chat wire 响应 `200`；SQLite、focused Flutter=`97/97`、rig-check/rig-down、coverage、anchors、diff、进程审计通过。正式证据=`sessions/20260819-221915/evidence/SURF-068-settings-panel-chat-five-level.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2285→2290 judgments`，COVERAGE=`848 / 450→451 / 0`。`gap-too-fast`/`discovery-collapse` 由 `SURF-068-ledger-alarm-reaudit.md` 独立重审并串行 ack，未改门禁，最终 `alarms.py check`=`clean (2290 judgments)`。下一前线由 formal sequence gate 决定；批次四十三 `15/50`，50 格前不统一门禁、不提交。P12 400+ Journey 按用户裁定推迟二期。

## 2026-08-19 22:17 · SURF-067 正式五级入账，通知设置与真实审批胶囊闭环通过，批次四十三 10/50

- `SURF-067 settings/panel-notifications` 在全新 workspace `Acceptance SURF-067` 上真实覆盖 `All → Needs you → Silent`、系统/应用内通知、失败/审批/需关注三类胶囊登记和默认恢复。真实 approval workflow 依次验证默认审批胶囊、关闭登记后的新事件抑制、`All` 绕过分类登记，以及从胶囊点击 `Approve` 后的 `Approved → 收口` 动画和 completed flowrun。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-220500`、workspace=`ws_b987f930f7df592c`、录屏=`622.090000s / 2560x1584 / H.264 / 60fps`。backend=`572` 行无应用红线；ssetap=`21` 行且三流真实连接；frontend=`4` 行仅已知 IMK host warning；llmtap proof/install/models 全 `200`；三 run 最终 completed；focused Flutter=`56/56`；rig-check/rig-down、coverage、anchors、diff、进程审计通过。正式证据=`sessions/20260819-220500/evidence/SURF-067-settings-panel-notifications-five-level.md`。
- 首次 L2 写账被 gate 拒绝，因为录屏尚未封存；正常 `rig-down` 后以同一证据幂等重试通过。`judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2280→2285 judgments`，COVERAGE=`848 / 449→450 / 0`。`gap-too-fast`/`discovery-collapse` 由 SURF-067 独立重审并串行 ack，未改门禁，最终 `alarms.py check`=`clean (2285 judgments)`。下一前线为 `SURF-068 settings/panel-chat`；本批 10/50，50 格前不统一门禁、不提交。P12 400+ Journey 按用户裁定推迟二期。

## 2026-08-19 22:02 · SURF-066 正式五级入账，通用设置全路径通过，批次四十三 5/50

- `SURF-066 settings/panel-general` 在全新 workspace `Acceptance SURF-066` 上真实覆盖主题三档、缩放六档与当前屏幕 cap、三条字体轴、语言双写、记住窗口、开机自启和自动更新。`1.25×/1.5×` 的真实点击保持 `1.1×`，禁用态没有假成功；所有偏好收尾恢复默认。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-214739`、workspace=`ws_60c1fd52053065b7`、录屏=`673.656667s / 2560x1584 / H.264 / 60fps`。backend=`744` 行无应用红线；ssetap 三流连接且无业务耐久帧不虚构；frontend=`4` 行仅已知 IMK host warning；llmtap managed proof/install/models 全 `200`；SQLite 最终 `language=en`；focused Flutter=`38/38 + 12/12`；rig-check/rig-down、coverage、diff、进程审计通过。正式证据=`sessions/20260819-214739/evidence/SURF-066-settings-panel-general-five-level.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2275→2280 judgments`，COVERAGE=`848 / 448→449 / 0`，anchors=`10/10`。`gap-too-fast`/`discovery-collapse` 由 `SURF-066-ledger-alarm-reaudit.md` 独立复审并串行 ack，未改门禁，最终 `alarms.py check`=`clean (2280 judgments)`。下一前线为 `SURF-067`；批次四十三当前 `5/50`，50 格前不统一门禁、不提交。P12 400+ Journey 按用户裁定推迟二期。

---
## 2026-08-19 21:12 · SURF-062 正式五级入账，设置偏好目录与设置项级搜索通过，批次四十二 35/50

- `SURF-062 settings/rail-prefs` 在全新工作区 `Acceptance SURF-062` 上真实覆盖 Settings 的 Preferences 三段：General、Notifications、Chat；真实键盘输入 `theme` / `login` 后分别完成设置项结果、跨面板定位、搜索清空、目标洗亮与长页滚动定位。启动项开关 off→on→off 后 AX/画面回读一致，未留下偏好污染。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-210545`，workspace=`ws_54a9a9eaa18dc054`，录屏=`257.628333s / 2560x1584 / H.264 / 60fps`。五通道封口：backend=`297` 行无应用红线；ssetap 三流连接、本路径无业务耐久帧不虚构；frontend 仅正常启动/VM/已知 IMK host warning；llmtap managed proof/install/models 全 `200`，无本路径 completion；rig-check/rig-down、focused settings=`42/42`、analyze、coverage check、process audit 通过。证据=`sessions/20260819-210545/evidence/SURF-062-settings-rail-prefs-five-level.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2255→2260 judgments`，COVERAGE=`848 / 444→445 / 0`，anchors=`10/10`。写账触发的 `gap-too-fast`/`discovery-collapse` 由 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-062-ledger-alarm-reaudit.md` 独立复审并串行 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2260 judgments)`。下一前线为 `SURF-063`，本批未满 50 格不统一门禁、不提交。P12 400+ Journey 按用户裁定推迟二期。

## 2026-08-19 21:02 · SURF-061 正式五级入账，循环节点逐轮检查器与失败节点原地恢复通过，批次四十二 30/50

- `SURF-061 scheduler/run-inspector-node` 真实构造 loop workflow，产生 `work#0/#1/#2` 与 `route#0/#1/#2` 共 7 条 durable node rows。真实 App 旗舰显示 `work ×3`、`route ×3`，右岛 Inspector 提供 `#0/#1/#2` 切换，输出与执行日志坐标逐轮更新；失败节点同时显示全文 traceback、无结果态、Execution log 和 `Replay the failed nodes`。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-205412`，workspace=`ws_7a9b6f127158cb3c`，run=`fr_2d4323570cff2460`，录屏=`333.648333s / 2560x1584 / H.264 / 60fps`。五通道封口：backend=`578` 行无应用红线；ssetap entities=`7..58`、notifications=`16..57` 单调；frontend 仅正常启动/VM/已知 IMK host warning；llmtap managed bootstrap 全 200、无伪造 completion；SQLite/UI/REST 对账和 process leak audit 通过。整数 CEL fixture 首次失败保留，公开 control edit 改为 double literal 后重跑，不是产品 defect。
- 正式证据=`sessions/20260819-205412/evidence/SURF-061-run-inspector-node-five-level.md`，警报复审=`/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-061-ledger-alarm-reaudit.md`。`judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2250→2255 judgments`，COVERAGE=`848 / 443→444 / 0`，anchors=`10/10`；统计警报独立复审并串行 ack，最终 `alarms.py check`=`clean (2255 judgments)`。下一前线为 `SURF-062`，本批未满 50 格不统一门禁、不提交。P12 400+ Journey 继续按用户裁定推迟二期。

## 2026-08-19 20:55 · SURF-060 正式五级入账，运行卷宗入口 payload 与 AI triage 闭环通过，批次四十二 25/50

- `SURF-060 scheduler/run-inspector-dossier` 红跑发现右岛 dossier 缺少运行入口 payload；后端真实数据位于 trigger 节点 durable result。stop-and-fix 在 Error 前增加 `Entry payload` JSON 区段，读取 durable trigger result，并同步 Flutter regression 与 Scheduler 文档。
- 绿 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-204650`，数据=`/private/tmp/anselm-data-surf060-20260819-r3`，workspace=`ws_0204dbcd6673da77`，录屏=`206.026667s / 2560x1584 / H.264 / 60fps`。真实 App 从 Scheduler 打开失败 run dossier，看到 `body.index=5`、`body.mode=fail`、完整 Error、Pinned refs 和 AI triage；点击 triage 后真实受管网关回合完成，准确解释 deliberate failure 的因果链。证据=`sessions/20260819-204650/evidence/SURF-060-run-inspector-dossier-five-level.md`，红帧=`sessions/20260819-204243/evidence/SURF-060-red-dossier-missing-payload.png`。
- 五通道：backend=`366` 行无应用红线；ssetap=`270` 行，messages durable=`1..22`、notifications durable=`1..12` 单调；frontend=`3` 行仅 Dart VM service；llmtap challenge 与三次 chat completion 全 `200`；录屏、request/response body 和截图均封存，rig-down 后无残留进程。focused `scheduler_run_test.dart`=`39/39`、analyze、rig-check/rig-down、ffprobe/process leak audit 通过。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2245→2250 judgments`，COVERAGE=`848 / 442→443 / 0`，anchors=`10/10`。`gap-too-fast`/`discovery-collapse` 经 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-060-ledger-alarm-reaudit.md` 独立复审并串行 ack，未改门禁，最终 `alarms.py check`=`clean (2250 judgments)`。下一前线为 `SURF-061`，本批未满 50 格不统一门禁、不提交。P12 400+ Journey 继续按用户裁定推迟二期。

## 2026-08-19 20:40 · SURF-059 stop-and-fix 后正式五级入账，Inactive 沉底历史行不再显示 live dot，批次四十二 20/50

- 红跑 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-203033` 通过真实 API 停用带 5 次失败历史的 `surf052_failed`；Computer Use 展开 Inactive 后发现红色 live dot，违反「灰不占状态点位」。修复 `schedulerRailModel` 对 inactive 行不生成 live status dot，只保留历史时间；新增 model regression 并同步 Scheduler 文档。
- 绿跑 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-203404`、workspace=`ws_64f76fe1fdf9aa85`、数据=`/private/tmp/anselm-data-surf059-20260819-r1`。真实 App 展开显示 `surf052_failed 5m ago`、`surf052_inactive —`，两行无红色 live dot；Display options 隐藏/恢复 Inactive 通过。录屏=`209.260000s / 2560x1584 / H.264 / 60fps`，截图与正式证据=`sessions/20260819-203404/evidence/SURF-059-rail-inactive-five-level.md`。
- 五通道：backend=`324` 行无 WARN/ERROR/panic/FATAL；ssetap 三流连接并记录 settle lifecycle；frontend=`3` 行仅 VM 启动；llmtap ready、确定性路径无 completion；rig-down 后无进程残留。focused provider=`13/13`、model=`15/15`、Dart analyze、rig-check/rig-down/ffprobe/process leak audit 通过。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2240→2245 judgments`，COVERAGE=`848 / 441→442 / 0`，anchors=`10/10`。`gap-too-fast`/`discovery-collapse` 由 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-059-ledger-alarm-reaudit.md` 独立复审并串行 ack，最终 `alarms.py check`=`clean (2245 judgments)`。批次四十二由 `15→20/50`，未满 50 格不跑统一长门禁、不提交；下一原子前线为 `SURF-060 scheduler/run-inspector-dossier`。P12 400+ Journey 按用户裁定推迟二期。

## 2026-08-19 20:30 · SURF-058 正式五级入账，Never ran 折叠段与真实 run 晋级主段通过，批次四十二 15/50

- `SURF-058 scheduler/rail-never-ran` 使用全新数据=`/private/tmp/anselm-data-surf058-20260819-r1`、workspace=`ws_625a6279a8161d46`、session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-201947`。真实 App onboarding 后构造 active never-run、已有历史和 inactive 三类 workflow；首屏 `Never ran 1` 初始折叠，展开只显示一个真实子行。真实 run=`fr_da9efc70f9a08841` completed 后 durable refresh 将该 workflow 提升到主段，Never ran 段消失，Inactive 仍独立计数 1，无重复或墓碑行。
- 五通道封口：录屏=`346.326667s / 2560x1584 / H.264`；backend=`572` 行无 WARN/ERROR/panic/FATAL 应用红线；ssetap 三流连接，entities durable=`1..44`、notifications=`1..33` 单调，messages 本确定性 scheduler 路径无 durable 不虚构；frontend 无 Flutter/Dart/RenderFlex/RenderBox/assertion/Unhandled/Exception 红线，仅已知 IMK host 噪声；llmtap managed challenge/install/models 全 `200`，无 completion 伪造。证据=`sessions/20260819-201947/evidence/SURF-058-rail-never-ran-five-level.md`，警报复审=`/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-058-ledger-alarm-reaudit.md`。
- focused Scheduler provider=`13/13`、model=`14/14`，rig-check/rig-down/ffprobe/process leak audit 通过。五格=`G1/F1/B2/C4/G1`；formal ledger=`2235→2240 judgments`，COVERAGE=`848 / 440→441 judged / 0`；两条统计警报独立复审并串行 ack，最终 `alarms.py check`=`clean (2240 judgments)`。批次四十二由 `10→15/50`，未满 50 格不跑统一长门禁、不提交；下一前线由 formal sequence gate 决定。P12 400+ Journey 继续按用户裁定推迟二期。

## 2026-08-19 20:14 · SURF-057 stop-and-fix 后正式五级入账，trigger pause stale next-fire 修复，批次四十二 10/50

- 首轮红 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-200109`：真实 `:pause` 返回 `paused:true/listening:false` 且 SSE 观察 trigger/status，但 rail 仍显示 `in 3h`。根因是 `SchedulerRailController` 没有订阅 trigger status。红帧和根因见 `sessions/20260819-200109/evidence/SURF-057-red-stale-trigger-meta.md`。
- stop-and-fix 修改 `frontend/lib/features/scheduler/state/scheduler_rail_provider.dart`：新增窄 `entities/trigger` status 监听，复用 300ms 去抖 refresh；activation/firing telemetry 不触发整 rail 重取。新增 `scheduler_rail_provider_test.dart` 正负断言，并同步 `docs/references/frontend/features/scheduler.md`。
- 绿 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-200757`、workspace=`ws_751c44f801a46d07`，录屏=`200.778333s / 2560x1584 / H.264 / 60fps`。重建真实 App 后暂停态显示上次运行、恢复态显示 `in 3h`，真实 completed run=`fr_10cf32bafb1a5505` 后活动排序、Name/Recent activity、Show next fire/last-run 视图与 Never ran/Inactive 折叠均通过；最终证据=`sessions/20260819-200757/evidence/SURF-057-rail-main-five-level.md`。
- 五通道封口：绿 backend 无应用红线，ssetap 三流连接并观察 trigger status/run/approval/settle，frontend 无 Flutter/Dart/RenderFlex/RenderBox/assertion/Unhandled/Exception 红线，llmtap journal 非空且本确定性路径无 completion 诚实不伪造；rig-check/rig-down/ffprobe/process leak 通过。五格=`G1/F1/B2/C4/G1`，formal ledger=`2230→2235 judgments`，COVERAGE=`848 / 439→440 judged / 0`；警报复审=`/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-057-ledger-alarm-reaudit.md`，两条警报串行 ack，最终 `alarms.py check` clean。focused Scheduler provider=`13/13`、model=`14/14`、docs verify、rig=`42/42`、Python compile、shell syntax、`git diff --check` 通过。批次四十二由 `5→10/50`，未到 50 格不跑统一长门禁、不提交；P12 400+ Journey 继续按用户裁定推迟二期。

## 2026-08-19 19:58 · SURF-056 正式五级入账，Scheduler Overview 等人徽标与等待区双向闭环通过，批次四十二 5/50

- `SURF-056 scheduler/rail-overview-row` 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-194527`，隔离数据=`/private/tmp/anselm-data-surf056-20260819-r1`，workspace=`ws_83b4627329714f23`。真实 App 先验证空数据固定 Overview，再用公共 API 创建 parked approval `fr_01e9dd26ff49d0c4`；Overview、中心 Waiting、右上审批卡和 Waiting on you 同时显示 1。真实 Approve 后归零，选中工作流再返回 Overview 无 stale detail，第二次 parked run 再次回升为 1；settle 收口 parked 和 deliberate slow run。
- 五通道封口：录屏=`593.678333s / 2560x1584 / H.264 / 60fps`；backend=`705` 行无应用 WARN/ERROR/panic/FATAL/Unhandled/RenderFlex/RenderBox/Exception；ssetap=`113` 行，三流真实连接并观察 `workflow.approval_pending`、run start、parked/completed node 与 durable seq，`seq=0` 未推进游标；frontend 无 Flutter/Dart/RenderFlex/RenderBox/assertion/Unhandled/Exception 红线，仅已知 macOS IMK host 噪声；llmtap managed challenge/install/models 全 `200`，本路径无 completion 不伪造。证据=`sessions/20260819-194527/evidence/SURF-056-overview-five-level.md`，警报复审=`/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-056-ledger-alarm-reaudit.md`。
- 五格=`G1/F1/B2/C4/G1`；anchors=`10/10`；formal ledger=`2225→2230 judgments`，COVERAGE=`848 / 438→439 judged / 0 tombstones`；`gap-too-fast`/`discovery-collapse` 已独立复审并串行 ack，最终 `alarms.py check`=`clean (2230 judgments)`、`gen_coverage.py --check` clean。focused Scheduler=`57/57`，rig-check/rig-down/ffprobe/process leak audit 通过。批次四十二由 `0→5/50`，未到 50 格不跑统一长门禁、不提交；P12 400+ Journey 继续按用户裁定推迟二期。

## 2026-08-19 19:24 · SURF-055 正式五级入账，run relay 交棒旗舰通过，批次四十一 50/50 并完成统一长门禁

- `SURF-055 scheduler/run-relay` 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-191759`，隔离数据=`/private/tmp/anselm-data-surf055-20260819-r1`，workspace=`ws_b0ddf3d9093275fc`，workflow=`wf_83b293643efa4b62`，primary run=`fr_4cc81cf274f208f2`。真实 App 从 Scheduler workflow home 打开 run terminal，点击生产 `Open run page →` 进入 `/scheduler/runs/:frId` relay，再交棒 `/scheduler/w/{workflowId}/runs/{flowrunId}` 旗舰；最终画面清晰显示 `Done`、pinned version、2 nodes completed、Timeline、Run dossier 与 pinned refs。dead-id 负路径返回 `FLOWRUN_NOT_FOUND`，route regression 锁定为可解释句子而非空白页。
- 五通道封口：录屏=`262.258333s / 2560x1584 / H.264 / 60fps`；backend journal=`408` 行，无未解释应用红线，REST completed/pinned refs/node result 与 UI 对账；ssetap=`15` 行，目标 flowrun durable `seq=1→2→3→4` 且中间 `seq=0` delta 不推进 cursor；frontend 无 Flutter/Dart/RenderFlex/RenderBox/Unhandled/Exception/assertion 红线，仅已知 macOS IMK host 噪声；llmtap readiness 与 managed proof wiring 通过，本确定性 scheduler 路径无 completion，不伪造 LLM 证据。证据=`sessions/20260819-191759/evidence/SURF-055-run-relay-five-channel.md`，警报复审=`/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-055-ledger-alarm-reaudit.md`。
- 五格=`G1/F1/B2/C4/G1`；formal ledger=`2220→2225 judgments`，COVERAGE=`848 / 437→438 judged / 0 tombstones`，anchors=`10/10`。写账后的 `gap-too-fast`/`discovery-collapse` 已以独立证据复审并串行 ack，未改阈值、算法、法典、锚点或 gate，`alarms.py check`=`clean (2225 judgments)`，`gen_coverage.py --check` clean。统一长门禁：rig=`42/42`、focused Scheduler=`80/80`、`make verify` 全绿、`make -C backend testend` 全量通过、shell/py_compile/diff/process leak audits 通过。批次四十一从 `49→50/50`，本条记录后提交；下一前线由 formal sequence gate 决定。P12 400+ Journey 继续按用户裁定推迟二期。

## 2026-08-19 19:04 · SURF-054 正式五级入账，Run 旗舰钉版图 stop-and-fix 后可读，批次四十一 49/50
## 2026-08-19 19:04 · SURF-054 正式五级入账，Run 旗舰钉版图 stop-and-fix 后可读，批次四十一 49/50

- 首轮真实画面发现横向五节点钉版图在右岛打开时缩成不可读细线；第二轮确认旧 Flutter bundle 被复用，均保留为红/仪器证据。stop-and-fix 增加只读 `reflowPinned`，Run 旗舰在窄主轨用纵向展示同一钉版拓扑，默认图模型与编辑器仍尊重作者坐标。
- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-185802`，workspace=`ws_c170fe14c137fee5`，workflow=`wf_40a2b65d4d430fbc`，completed=`fr_f66380fb2e86dc6d`，failed comparison=`fr_f464c9dc989c810c`。第三轮新构建真实 App + managed gateway + Computer Use 完成卷宗头、钉版图、甘特、台账和 URL 单选区；graph `stage_validate` 与台账 `stage_transform` 两次选区切换均同步右岛、图和甘特，输出/执行日志对应真实节点。
- 五通道封口：录屏=`231.118333s`；backend PID `81279` 归属 `:9081`，无应用 WARN/ERROR/panic；REST 5 节点 completed，activity `206/150/149/150ms`；ssetap `47` 行并观察 failed comparison 的 `stage_validate` terminal/notification/attention；frontend 无 Flutter/Dart/overflow/assertion 红线，仅保留已知 macOS IMK host 噪声；llmtap managed challenge/install/models 全 `200`，无 completion 伪造。证据=`sessions/20260819-185802/evidence/SURF-054-run-flagship-green.md`。
- focused Flutter graph model + canvas + scheduler run 最终 `74/74`；`rig-check`、`rig-down`、anchors `10/10`、`gen_coverage.py --check` 通过。`judge.py` 写入 `G1/F1/B2/C4/G1`，formal ledger=`2215→2220 judgments`，COVERAGE=`848 / 436→437 / 0`；两条统计警报由 `SURF-054-ledger-alarm-reaudit.md` 独立复审并串行 ack，最终 `alarms.py check`=`clean (2220 judgments)`。批次四十一由 `48→49/50`，未到 50 格不跑统一长门禁、不提交；下一原子前线由 formal sequence gate 决定。

## 2026-08-19 18:39 · SURF-053 正式五级入账，多入口 Run now 与 entryNode stop-and-fix 后通过，批次四十一 48/50

- `SURF-053 scheduler/workflow-home` 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-182815`，隔离数据=`/private/tmp/anselm-data-surf053-20260819-r7`，workspace=`ws_7189416b4484a96e`，workflow=`wf_2ec6213e355a3288`。真实 Flutter macOS App + managed gateway + Computer Use 完成 onboarding、Scheduler、workflow 首页、三入口 `Run now`、选择 Webhook、成功 run 和取消 chooser。
- 首轮真实多入口 Run now 只显示 `invalid or ambiguous trigger entry node`，用户目的未达成；该红路径保留。stop-and-fix 贯通后端 Runner/service/bootstrap/HTTP/tool 的 `entryNode`，前端增加 `_RunEntryPicker` 与双语提示，重建 App 后目标 `fr_c851c8fc46870084` 的 `hook_entry → hook_work → run_terminal` 真实完成，且同一 chooser 的 Cancel 不创建额外 run。
- 五通道封口：`screen.mov`=`319.870000s / 2560x1584 / H.264 / 60fps`，外部窗口遮挡门通过；backend 无应用 WARN/ERROR/panic/fatal/exception，SQLite `integrity_check=ok` 且 foreign-key check 为空，REST/SQLite/UI 对账为 `completed=3 / failed=2 / cancelled=1`、`flowrun_nodes=11`；ssetap 三流真实连接，entities durable=`1..24`、notifications durable=`1..15` 唯一递增，目标 run 帧闭合，messages 本路径无业务 durable frame 不虚构；frontend 无 Flutter/Dart/RenderFlex/RenderBox/Unhandled/Exception 应用红线，唯一 IMKCF macOS 输入法通知已原样保留；llmtap managed proof/install/models 有状态请求全 `200`，本确定性路径无 completion 不伪造。证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-182815/evidence/SURF-053-scheduler-workflow-home-five-channel.md`。
- 锚点过期门先拒绝旧校准，使用冻结答案重新通过 `10/10` 后才写账；`judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2210→2215 judgments`，COVERAGE=`848 rows / 435→436 judged rows / 0 tombstones`。写账后的 `gap-too-fast`/`discovery-collapse` 由 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-053-ledger-alarm-reaudit.md` 独立复审并串行 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2215 judgments)`，`gen_coverage.py --check` clean。批次四十一由 `47→48/50`，未到 50 格不跑统一长门禁、不提交；下一前线为 `SURF-054`。P12 400+ Journey 按用户裁定推迟二期。

## 2026-08-19 00:50 · SURF-036 正式五级入账，Agent rail 折叠/详情保持与无状态点语义通过，批次四十 45/50

- `SURF-036 entities/rail-agent` 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-004347`，隔离数据=`/private/tmp/anselm-data-surf036-20260819-r1`。静态审查确认 Agent 没有固有 resident/lifecycle 状态点；真实 App 冷启动显示 `报表助手` 与精确计数 `1`，打开后详情显示 v1、`All mounts healthy`、Prompt、Capabilities、Interface 和右岛 `Invoke` 台。选中后折叠 Agent，详情路由与右岛保持；重新展开恢复行，再点击 Overview 后仅 Overview 选中。
- 最终画面无虚构蓝点、隐藏行伪造选中、裁切、重叠或未解释 reflow。五通道封口：录屏 `97.883333s / 2560x1584 / H.264 / 60fps`；backend `186` 行无应用红线，frontend `3` 行仅正常启动，ssetap 三流真实连接并以 EOF 收台，llmtap challenge/install/models 全 `200`；SQLite `integrity_check=ok`，agents/versions/workspaces=`1/1/1`。本格未点击 Invoke，不伪造 LLM completion。证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-004347/evidence/SURF-036-entities-rail-agent-five-channel.md`，最终帧=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-004347/evidence/SURF-036-final-agent-frame.png`。
- focused Flutter `33/33`、Dart analyze、rig-check/rig-down 通过。`judge.py` 按 `G1/F1/B2/C4/G1` 写入五格，formal ledger `2125→2130 judgments`，COVERAGE=`848/418→419/0`，anchors=`10/10`。写账触发 `gap-too-fast`、`pass-burst`、`discovery-collapse`，已以 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-036-ledger-alarm-reaudit.md` 独立复审并逐项 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2130 judgments)`。批次四十由 `40→45/50`，未到第 50 格不跑统一长门禁、不提交；下一前线为 `SURF-037 entities/rail-workflow`。P12 400+ Journey 继续按用户裁定推迟二期。

## 2026-08-19 00:40 · SURF-035 正式五级入账，Handler 运行点真相分裂 stop-and-fix 后通过，批次四十 40/50

- `SURF-035 entities/rail-handler` 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-003246`，隔离数据=`/private/tmp/anselm-data-surf035-20260819-r3`。真实 App 冷启动 Handler `order_desk` 为 stopped 灰点，点击 `place` 的 Call 后详情为 `running`、右岛返回 `{ "ok": true }`，rail 变为蓝色运行点；返回 Overview、折叠 Handler、重新展开后计数、顺序、蓝点和路由均稳定。
- 首轮红证据 session=`20260819-002301` 发现详情 running 而 rail 灰色；`20260819-002838` 证明只修前端仍无效，根因是 `GET /handlers` 列表漏传 `runtimeState`。stop-and-fix 同时补后端列表投影、前端 call 收尾列表 invalidate，并新增 Go/Flutter 回归测试与 API/实体文档同步。
- 五通道封口：录屏 `220.095000s / 2560x1584 / H.264 / 60fps`；backend `311` 行无应用红线，frontend `3` 行仅正常启动，ssetap 三流真实连接并正常收台，llmtap challenge/install/models 全 `200`；SQLite `integrity_check=ok`，handlers/versions/calls/workspaces=`1/1/1/1`。focused Flutter `45/45`、Dart analyze、Go handler/HTTP tests、rig-check/rig-down 通过。证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-003246/evidence/SURF-035-entities-rail-handler-five-channel.md`，最终帧=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-003246/evidence/SURF-035-final-handler-frame.png`。
- `judge.py` 按 `G1/F1/B2/C4/G1` 写入五格，formal ledger `2120→2125 judgments`，COVERAGE=`848/417→418/0`，anchors=`10/10`。写账触发 `gap-too-fast` 与 `discovery-collapse`，已以 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-035-ledger-alarm-reaudit.md` 独立复审并 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2125 judgments)`。批次四十由 `35→40/50`，未到第 50 格不跑统一长门禁、不提交；下一前线为 `SURF-036 entities/rail-agent`。P12 400+ Journey 继续按用户裁定推迟二期。

## 2026-08-19 00:20 · SURF-034 正式五级入账，Function rail 折叠/展开与选中路由保持通过，批次四十 35/50

- `SURF-034 entities/rail-function` 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-001201`，隔离数据=`/private/tmp/anselm-data-surf034-20260819-r1`。真实 App 展开 Function 段观察 `sync_inventory`/`greet` 与精确计数 `2`，折叠后子行消失、计数和相邻 section 保持；选中 `greet` 后再折叠，详情路由不丢，重新展开后 `greet` 恢复唯一选中态。
- 放大终帧复审无 clipping、overlap、white flash、未解释 reflow、stale route 或隐藏行伪造选中。详情页 code/interface/environment 与右侧 Run terminal 稳定，Function 计数始终为 `2`。
- 五通道封口：录屏 `256.505000s / 2560x1584 / H.264 / 60fps`；backend `344` 行、frontend `3` 行无应用红线；ssetap 三流连接并在主动收台时 EOF，本地 rail 路径无业务 mutation，不伪造 SSE frame；llmtap challenge/install/models 全 `200`，无本路径 completion；SQLite `PRAGMA integrity_check=ok`，Function/Handler/Agent/Workflow=`2/1/1/0` 与 UI 对账一致。证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-001201/evidence/SURF-034-entities-rail-function-five-channel.md`，最终帧=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-001201/evidence/SURF-034-final-function-frame.png`。
- Focused Flutter `33/33`、相关 Dart analyze、rig-check/rig-down、进程/端口收台审计均通过。`judge.py` 按 `G1/F1/B2/C4/G1` 写入五格，formal ledger `2115→2120 judgments`，COVERAGE=`848/416→417/0`，anchors=`10/10`。
- 写账触发的 `gap-too-fast`、`pass-burst` 与 `discovery-collapse` 已由 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-034-ledger-alarm-reaudit.md` 独立复审并逐项 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2120 judgments)`。批次四十由 `30→35/50`，未到 50 格不跑统一长门禁、不提交；下一前线为 `SURF-035 entities/rail-handler`。P12 400+ Journey 继续按用户裁定推迟二期。

## 2026-08-19 00:10 · SURF-033 正式五级入账，实体 rail 固定总览入口与唯一选中态通过，批次四十 30/50

- `SURF-033 entities/rail-overview-row` 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-000301`，隔离数据=`/private/tmp/anselm-data-surf033-20260818-r1`。真实 App 冷启动无选中实体时，rail 在搜索框下方显示无头 `Overview` 固定行并高亮；点击 Function `greet` 后实体唯一选中，再点击 Overview 返回实体总览，移开指针并等待稳定后原实体行恢复普通底色，没有双高亮或 stale detail route。
- 最终画面在放大帧复审：固定行与其余 rail 行等高，圆角、缩进、图标/文字中线一致，无额外分组头抢层级；实体总览主区的五张计数卡、关系图与 Recently updated 均可见。没有需要 stop-and-fix 的产品缺陷。
- 五通道封口：录屏 `221.181667s / 2560x1584 / H.264 / 60fps`；backend `310` 行、frontend `3` 行无应用红线；ssetap 三流均真实连接并在收台时 EOF，本地 rail 路径无业务 mutation，不伪造 SSE frame；llmtap 仅有真实 proof/install/models `200` bootstrap，无本路径 completion。SQLite `PRAGMA integrity_check=ok`，Function/Handler/Agent/Workflow=`2/1/1/0` 与 UI 对账一致。证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-000301/evidence/SURF-033-entities-rail-overview-row-five-channel.md`，最终帧=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-000301/evidence/SURF-033-final-overview-frame.jpeg`。
- `judge.py` 按 `G1/F1/B2/C4/G1` 写入五格；formal ledger `2110→2115 judgments`，COVERAGE=`848/415→416/0`，anchors=`10/10`。写账后的 `gap-too-fast` 与 `discovery-collapse` 已由 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-033-ledger-alarm-reaudit.md` 独立复审并 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2115 judgments)`。批次四十由 `25→30/50`，未到 50 格不跑统一长门禁、不提交；下一前线为 `SURF-034 entities/rail-function`。P12 400+ Journey 继续按用户裁定推迟二期。

## 2026-08-18 23:57 · SURF-032 正式五级入账，工作流编辑器新增节点布局与 Save 失败反馈 stop-and-fix 后通过，批次四十 25/50

- `SURF-032 entities/workflow-editor` 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260818-234952`，隔离数据=`/private/tmp/anselm-data-surf032-20260818-r1`。真实 App 从 Entities → Workflow → Open graph editor 进入编辑器，新增 Agent 节点落在现有图下方且无重叠，Ref 选中真实 `surf032_editor_agent`，点击 Save 后后端真实拒绝不可达 Agent（`node "agent" is unreachable from any trigger`），随后 Discard，SQLite 最终确认 active workflow 仍为 v2、无临时 Agent 持久化、无 v3。
- 首轮红证据发现重复选择当前 Function family 会清掉已有 target；固定新增坐标会让新卡片与现有图重叠；修复后复跑又发现 Save 失败 notice 会压住 frameless toolbar 的 Discard 边缘。stop-and-fix 修正 family reselect no-op、基于当前图几何的确定性空位布局，以及 workflow editor notice 下移到 toolbar 之下；同步 focused provider/page/router/node-picker tests 与实体文档。
- 五通道封口：录屏 `78.313333s / 2560x1584 / H.264`；backend `136` 行无 `WARN|ERROR|panic|FATAL`，frontend `3` 行无 Flutter/Dart/Unhandled 应用红线；ssetap `8` 条记录、三流连接并正常 EOF，本路径无业务 SSE frame；llmtap 保留 readiness，本路径为直接 REST 编辑且无 LLM completion。rig-check 在台架存活时通过五通道归属/健康检查，随后 rig-down 清台。证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260818-234952/evidence/SURF-032-entities-workflow-editor-five-channel.md`。
- `judge.py` 按 `G1/F1/B2/C4/G1` 写入五格；formal ledger `2105→2110 judgments`，COVERAGE=`848/414→415/0`，anchors=`10/10`。写账后的 `gap-too-fast` 与 `discovery-collapse` 已以 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-032-ledger-alarm-reaudit.md` 独立复审并 ack，最终 `alarms.py check`=`clean (2110 judgments)`。批次四十由 `20→25/50`，未到 50 格不跑统一长门禁、不提交；下一前线为 `SURF-033 entities/rail-overview-row`。P12 400+ Journey 继续按用户裁定推迟二期。

## 2026-08-18 23:14 · SURF-031 正式五级入账，Dispatch 终态竞态与 stale-listening 修复后通过，批次四十 20/50

- `SURF-031 entities/tab-dispatch` 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260818-230255`，隔离数据=`/private/tmp/anselm-data-surf031-20260818-r2`。真实 App 冷启动显示 `Idle / Listening: No / Listeners: 0`；prepare 由公共 API 真实激活四个 workflow 并触发 webhook fan-out，detail 自动变为 `Listening / Listeners: 4`。Dispatch 真实观察 `pending`、`run started`、`skipped`、`superseded`、`shed`，打开筛选菜单、展开 durable row 详情、切 Activity `Fired only`，再点击产品内 `Fire` 得到 `Fired · tra_f29b331786dc3b5c` 通知和新派发回执；settle 后 detail 回到 `Idle`。
- 首轮红证据分别抓到 fire 后 terminal filter 旧列表、scheduler 落库竞态，以及全部 listener 下线后仍显示 `Listening`。stop-and-fix：observability provider 自有订阅 fire signal，并在 `500ms / 5s` bounded window 内用 REST 真相收敛；trigger detail/rail 在 durable workflow lifecycle signal 后重读派生 `listening`。同步 entity reference 与 focused provider/widget tests。
- 五通道封口：`screen.mov`=`486.651667s / 2560x1584 / H.264 / 60fps`；backend `975` 行无 `WARN|ERROR|panic|FATAL`，frontend `3` 行无 Flutter/Dart/Unhandled 应用红线，ssetap `145` 条三流记录、entities durable `1..66`、notifications durable `1..32` 单调无 gap，llmtap readiness 保留。本路径无 LLM node，不伪造 completion。
- REST trigger 最终 `listening=false` 且 `lastFiredAt` 非空；SQLite `PRAGMA integrity_check=ok`，`trigger_firings=shed9/skipped14/started54/superseded11`、`flowruns=51 completed+3 cancelled`、`trigger_activations=23`，与最终 UI 对账一致。证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260818-230255/evidence/SURF-031-entities-tab-dispatch-five-channel.md`，最终视觉帧=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260818-230255/evidence/SURF-031-final-dispatch-frame.jpeg`。
- Focused Flutter `35/35`、相关 `flutter analyze`、fixture/rig compile、rig-down、`git diff --check` 通过。`judge.py` 按 `G1/F1/B2/C4/G1` 写入，formal ledger `2100→2105 judgments`，COVERAGE=`848/413→414/0`，anchors=`10/10`。
- 写账触发的 `gap-too-fast` 与 `discovery-collapse` 已由 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-031-ledger-alarm-reaudit.md` 独立复审并逐项 ack，未改阈值、算法、法典、锚点或 gate；最终 `alarms.py check`=`clean (2105 judgments)`。批次四十由 `15→20/50`，未到 50 格不跑统一长门禁、不提交；下一前线为 `SURF-032 entities/workflow-editor`。P12 400+ Journey 按用户裁定推迟二期。

## 2026-08-18 22:18 · SURF-030 正式五级入账，Activity 过滤器 stop-and-fix 后通过，批次四十 15/50

- `SURF-030 entities/tab-activity` 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260818-220939`，隔离数据=`/private/tmp/anselm-data-surf030-20260818-r1`。真实 App 通过 sensor trigger Activity tab 验证 `All activity`、`Fired only`、分页、`Load more` 以及 fired/not-fired 展开详情；真实 handler probe 产生 `25` 条 activation，其中 `22 fired / 3 not-fired`，关联 workflow 完成 `22` 次。
- 首轮真实画面发现普通 boxed dropdown 横跨整个阅读列，视觉上像可编辑输入框而不是轻量筛选器；按 stop-and-fix 停线，将 Activity 与 Dispatch 两处过滤器改为紧凑右对齐 ghost control，并同步实体文档和 focused widget assertion。最终画面中筛选入口保持可发现，但不再抢占标题层级。
- 五通道封口：录屏 `255.085000s / 2560x1584 / H.264 / 60fps`；backend `239` 行无 `WARN|ERROR|panic|FATAL`，frontend `3` 行无 Flutter/Dart/Unhandled 应用红线，ssetap `9` 行三流连接且 `sse-gaps=0`，llmtap readiness 保留；REST 分页/过滤与 SQLite `PRAGMA integrity_check=ok`、`trigger_activations=22/3`、`flowruns=22 completed` 和最终 UI 对账一致。
- Focused Trigger Flutter `14/14`、AnDropdown `7/7`、`flutter analyze`、fixture/rig compile、rig-check/rig-down、`git diff --check` 均通过。证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260818-220939/evidence/SURF-030-entities-tab-activity-five-channel.md`；`judge.py` 按 `G1/F1/B2/C4/G1` 写入五格，formal ledger `2095→2100 judgments`，COVERAGE=`848/412→413/0`，anchors=`10/10`。
- 写账触发的 `gap-too-fast` 与 `discovery-collapse` 已以 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-030-ledger-alarm-reaudit.md` 独立复审并 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2100 judgments)`。批次四十由 `10→15/50`，未到 50 格不跑统一长门禁、不提交；下一前线为 `SURF-031 entities/tab-dispatch`。P12 400+ Journey 按用户裁定推迟二期。

## 2026-08-18 21:51 · SURF-029 正式五级入账，Runs 驾驶舱 elapsed 语义修复后通过，批次四十 10/50

- `SURF-029 entities/tab-runs` 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260818-213748`，隔离数据=`/private/tmp/anselm-data-surf029-20260818-r2`。真实 App Runs 驾驶舱覆盖 completed、parked approval、failed/replay；在 graph 的 `gate` 节点内点击 `Approve`，parked run 真实完成并产出 `{"decision":"yes","reason":""}`。
- 首轮红证据发现 replay 后旧 UI 把人工检查/replay 间隔并入 `Elapsed`，且 action node 的 `createdAt` 不是 start stamp，导致显示虚假 `0ms`。stop-and-fix 增加 `/flowruns/{id}/activity` 分页、execution audit 聚合与节点 fallback；运行卡片拆分 `Run lifetime` 与 `Execution`，同步实体文档、i18n、focused provider 测试。修复后失败 replay 显示 `Run lifetime=1m34s`、`Execution=179ms`，approved run 显示 `Run lifetime=6m11s`、`Execution=29ms`。
- 五通道封口：录屏 `475.108333s / 2560x1584 / H.264 / 约57fps`；backend `583` 行无应用红线，frontend `3` 行只有正常 VM service，ssetap `57` 行、notifications durable `16..29`、entities durable `7..23` 连续且 `gap=0`，llmtap challenge/install/models 全 `200`；REST 三条 flowrun 为 `2 completed + 1 failed`，SQLite integrity=`ok`。Focused Function Go、实体 Flutter `16/16`、`flutter analyze`、fixture compile、rig-check/rig-down 通过。
- 证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260818-213748/evidence/SURF-029-entities-tab-runs-five-channel.md`；`judge.py` 按 `G1/F1/B2/C4/G1` 写入五格，formal ledger `2090→2095 judgments`，COVERAGE=`848/411→412/0`，anchors=`10/10`。两条统计警报以 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-029-ledger-alarm-reaudit.md` 独立复审并 ack，最终 `alarms.py check`=`clean (2095 judgments)`，未改阈值、算法、法典、锚点或 gate。本批次由 `5→10/50`，未到 50 格不跑统一长门禁、不提交；下一前线为 `SURF-030`。P12 400+ Journey 继续按用户裁定推迟二期。
...
- r83–r87 真实托管模型回合逐次冻结为红：技术 ID 形态、生命周期/目录猜测和残余占位词泄入用户文案。stop-and-fix 在完整耐久 assistant close 边界增加窄匹配 `normalizeChineseFunctionNotFoundCopy`，只规范用户叙事，不改 live delta、工具卡、审计面或 LLM wire；补 loop 守卫测试和 `docs/references/backend/domains/chat.md`，r88 为首个 clean final。
- 五通道封口：`screen.mov`=`130.230000s / 2560x1584 / H.264`，稳定 `115/120/125s` 帧的 `measure diff` 无 changed-region；backend 仅预期 `get_function function not found` WARN，无 ERROR/panic/FATAL/stack；ssetap `96` 帧，三流连接，messages durable `1..20` 单调且无 gap/reconnect/error/SEQ_TOO_OLD；frontend 仅正常 Dart VM 行；llmtap challenge/install/models 与四次 chat completion 全真实 `200`。证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260818-145523/evidence/SURF-017-r88-five-channel.md`，红证据与 stable frames 均保留。
- `judge.py` 按 `G1/F2/B2/E1/G1` 写入 `SURF-017=✓✓✓✓✓`，formal ledger `2030→2035 judgments`，COVERAGE=`848/400/0`；写账触发的 `gap-too-fast` 与 `discovery-collapse` 已按 r83–r88 红绿证据、五通道封口和 anchors `10/10` 独立复审后 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2035)`。
- 第 50 格后的首轮根 `make verify` 只暴露 frontend coverage 分母遗漏新文件 `an_animated_size_row.dart`；按守卫用 `UPDATE_COVERAGE=1` 补入后第二轮根门禁全绿。非缓存全量 Go、完整 `make -C backend testend`=`305.379s`、rig `42/42`、Python compile、shell syntax、coverage、anchors、alarms、`git diff --check` 和进程/端口收台审计全部通过。本批次三十八正式 `50/50`，下一原子前线为 `SURF-018 chat/rail-pinned`。

## 2026-08-18 06:16 · SURF-016 正式五级入账，嵌套 live 轨迹修复后批次三十八推进至 31/50

- `SURF-016 chat/nested-run-pane` 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260818-060616`，隔离数据=`/private/tmp/anselm-data-surf016-nested-run-20260818-r2`，真实 App 通过 search→invoke 两次完成 `nested_review_agent` 交接；live 帧显示 bounded nested pane、`Ran · Live` 与 `Listening live · settle follows the truth`，settled 帧显示 `Completed`、步骤/耗时/agent id/copy chip 和最终答案。
- 前置 r1=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260818-055959` 首次暴露 plain-text `invoke_agent.input` 在旧严格对象边界被拒并留下红色失败活动；该红证据保留、不计绿。stop-and-fix 只在 `invoke_agent.input` 加 object/stringified-object/plain-task 窄兼容，数组/数字/布尔仍拒绝，补 agent 守卫测试和领域文档。
- r2 新二进制的真实 LLM wire 实际出现 stringified-object 与 native-object 两种形状，两个 `agent_executions` 均 `ok`（`25172ms/16077ms`），没有 validation failure/retry。SQLite、SSE child `parentId`、UI execution id 和最终消息均对齐；messages durable `1..44` 连续，delta 均 `seq=0`，三路 SSE clean EOF。
- 五通道封口：录屏 `355.461667s`；managed challenge/install/models/chat LLM responses 全 `200`；backend/frontend journal 无未解释应用红线；`rig-down` 停清 conductor-owned server/llama/ssetap/llmtap/App/recorder。证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260818-060616/evidence/SURF-016-nested-run-pane-green.md`。
- `go test ./internal/app/tool/agent ./internal/app/loop -count=1` 通过；anchors=`10/10`。`judge.py` 按 `G1/F2/A5/C4/G1` 写入 `SURF-016=✓✓✓✓✓`，formal ledger `2025→2030 judgments`，COVERAGE=`848/398→399 judged / 0 tombstones`。写账触发 `gap-too-fast` 与 `discovery-collapse`，复审=`/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-016-ledger-alarm-reaudit.md`，按原阈值 ack 后 `alarms.py check`=`clean`；未改阈值、算法、法典、锚点或 gate。批次三十八由 `30→31/50`，未到 50 不跑统一长门禁、不提交；下一正式前线为 `SURF-017 chat/tool-cards`。P12 的 400+ Journey 扩写继续按用户裁定推迟二期。

## 2026-08-18 05:52 · SURF-015 正式五级入账，执行审计卷宗完成上下文脱敏修复后批次三十八推进至 30/50

- `SURF-015 chat/run-dossier` 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260818-053707`，隔离数据=`/private/tmp/anselm-data-surf015-run-dossier-20260818-r51`。真实 App 经受管网关找到并执行 `audit_logger`，随后打开完整 `get_function_execution` 审计卷宗；状态、触发方式、输入、输出、日志、耗时，以及 Conversation/message/tool-call 溯源控件均真实可见。
- 首轮复核发现 durable close 使用通用脱敏路径会露出模型占位词 `the requested item`，中文溯源段在无结构内容时还会留下空表壳；stop-and-fix 让 durable close 与 live delta 共用 context-aware redactor，并把空溯源段替换为指向邻近执行卡的明确说明，补 Go 回归和 chat reference 同步。
- 修复后的真实记录为 `fne_0988429b9c11bcda`、`ok`、输入 `{}`、输出 `{"items":[1,2,3],"ok":true}`、日志 `audit-start/audit-finish`、耗时 `158ms`。固定 `240px` JSON tree viewport 是既有 bounded-tree 设计，展开态 Computer Use 截图与 fresh AX 均确认三枚精确关联控件；录屏 `695.765000s / 2784x1808 / 60fps` 可读，无残留产品缺陷。
- 五通道同一 manifest 对证：backend/frontend journal 无未解释应用红线，messages/entities/notifications 三路 SSE、LLM wire、REST/SQLite 与 UI 对账一致，录屏及展开态截图封存；focused Go `internal/app/loop`、backend 全量 Go、34 个 targeted Flutter tests、anchors `10/10`、`gen_coverage.py --check` 均通过。
- `judge.py` 按 `G1/F2/A5/C4/G1` 写入 `SURF-015=✓✓✓✓✓`；formal ledger `2020→2025 judgments`，COVERAGE=`848 rows / 398 judged rows / 0 tombstones`。写账后的 `gap-too-fast` 与 `discovery-collapse` 已按原阈值完成独立复审并 ack，`alarms.py check`=`clean`，未改阈值、算法、法典、锚点或 gate。批次三十八由 `25→30/50`，未到 50 格不跑统一长门禁、不提交；下一原子前线为 `SURF-016 chat/nested-run-pane`。P12 的 400+ Journey 扩写继续按用户裁定推迟二期。

## 2026-08-17 23:13 · SURF-009 正式五级入账，审批浮层逐帧修复后批次三十八推进至 25/50

- `SURF-009 shell/flowrun-inbox` 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-225238`，隔离数据=`/private/tmp/anselm-data-surf009-flowrun-inbox-20260817-r2`。真实 App 让两个真实 flowrun 停在同一 approval，托盘显示 `Needs you 2` 与两张完整审批卡；REST 决定第一条后计数变为 `1`，再在 App 点击第二条 `Approve`，最终计数归零、两条 flowrun 均 `completed`、inbox=`[]`。
- 首次实现的高标准逐帧复核发现 approval capsule 在收起尾部存在内容被外层几何裁切的风险；stop-and-fix 让 question/actions 先按安全透明度退场、shell 再收缩，并补 enter/exit geometry regression。修复后的源分辨率帧 `t166–t170`、`t180–t183` 逐张复核：无半截按钮、裁字、边框残片、重叠或跳位。
- 五通道同一 manifest 对证：`screen.mov`=`211.723333s / 2560x1584 / H.264 / 60fps`；backend `46966` bytes 无应用 WARN/ERROR/panic/FATAL/stack trace；frontend 仅正常启动行；ssetap 三流连接，notifications durable `16..19`、entities `7..10` 单调，run delta=`seq=0`，三流 clean EOF；llmtap challenge/install/models 全 `200`，deterministic workflow 无 completion。
- `judge.py` 按 `G1/F1/B1/C2/G1` 写入 `SURF-009=✓✓✓✓✓`，正式账本 `2015→2020 judgments`，COVERAGE=`848/396→397/0`。L2 使用同 session 绑定证据；focused approval-capsule=`11/11`、notification-tray=`15/15`、Dart analyze、rig、coverage、anchors 全通过。
- 写账触发 `gap-too-fast` 与 `discovery-collapse`；复审证据=`/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-009-ledger-alarm-reaudit.md`，两项按原阈值 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2020 judgments)`。批次三十八由 `20→25/50`，未到 50 不跑统一长门禁、不提交；下一正式前线为 `SURF-015 chat/run-dossier`。P12 的 400+ Journey 扩写继续按用户裁定推迟二期。

## 2026-08-17 22:22 · SURF-008 正式五级入账，修复折叠尾帧字形裁切，批次三十八推进至 20/50

- `SURF-008 shell/notification-tray` 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-215802`，隔离数据=`/private/tmp/anselm-data-surf008-tray-20260817-r2`。真实 App 完成三时段折叠/展开、搜索与清空、Unread only、reload、每组 mark-all-read/mark-all-unread、底部滚动和最终折叠；受控 durable 行只构造时间桶，随后真实 REST Function 创建产生 Today 通知与 SSE/entity 活动，没有把 fixture 当成自然生命周期历史。
- 首次真实录屏停在一个真实产品缺陷：bare `SizeTransition` 在折叠最后阶段把通知字形高度裁成横向残片。stop-and-fix 抽取共享 `frontend/lib/core/ui/an_animated_size_row.dart`，通知托盘与 `AnSidebarList` 共用「顶锚 SizeTransition + 尾段 FadeTransition」；补回归断言，修复后真实录屏逐帧复核 `053/058/060/063/068`，58 帧只淡出不裁字，68 帧稳定收口。
- app-region `screen.mov`=`816.771667s / 2560x1584 / H.264 / 60fps`；固定过渡 `measure latency`=`16.7ms`，首对 `052→053 changedFrac=0.12633`，后续连续过渡至 68 帧。最终 SQLite=`17` 通知/`15` unread，UI=`Today 11 / Yesterday 3 / Earlier 3`；稳定帧、搜索、Unread only、每组菜单和 reload 均人工复核。
- 五通道同一 manifest 对证：backend `904` 行（`887×200/11×201/6×204`）无应用 WARN/ERROR/panic/FATAL；ssetap 三流连接，notifications durable `16..18`、entities durable `7..8`、delta `seq=0`，收台 EOF；frontend `243` 行中 `239` 条是同形 `accessibility_bridge.cc` AX tree 观测桥噪声，未知 Dart/Flutter/RenderFlex/overflow/Unhandled 红线 `0`，分类证据见 session `evidence/frontend-ax-review.md`；llmtap managed challenge/install/models 全 `200`，无 completion。
- `judge.py` 按 `G1/F1/B1/C2/G1` 写入 `SURF-008=✓✓✓✓✓`，正式账本 `2010→2015 judgments`，COVERAGE=`848/395→396/0`；focused Flutter `31/31`、Flutter analyze、Python compile、shell syntax、`git diff --check`、`gen_coverage.py --check`、anchors `10/10` 均通过。正式证据为 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-008-contract-matrix.md`、session L2/global L3/L4/L5。
- 写账触发 `gap-too-fast` 与 `discovery-collapse`；复审证据=`/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-008-ledger-alarm-reaudit.md`，两项已按原阈值 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2015 judgments)`。批次三十八由 `15→20/50`，未到 50 不跑统一长门禁、不提交；下一正式前线为 `SURF-009 shell/flowrun-inbox`。P12 的 400+ Journey 扩写继续按用户裁定推迟二期。

## 2026-08-17 21:30 · SURF-007 正式五级入账，批次三十八推进至 15/50

- `SURF-007 shell/notice-band` 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-211204`，隔离数据=`/private/tmp/anselm-data-surf007-notice-20260817-r1`。真实 App 通过四个真实失败 workflow、真实 parked approval、失败队列尾、`Clear all 4 top notifications`、Notifications `Needs you 1` 和 inbox Reject；不是 fixture 或 synthetic notification。REST 最终对账为 approval run `completed`、node `completed`、`decision=no`、parked=`[]`、unread=`29`。
- app-region `screen.mov`=`510.650000s / 2560x1584 / H.264 / 60fps`。overflow/final settled 窗口 `measure diff` 无输出；inbox 全帧四个极小诊断 diff 落在指针/caret/外部 host overlay，两个 App-content ROI 均无输出。视觉复核没有 clipping、overlap、white flash、reflow、stale approval 或输入跳变；外部噪声已在正式 evidence 中明示并排除。
- 五通道同一 manifest 对证：backend `633` 行无应用级 WARN/ERROR/panic/FATAL，frontend `3` 行仅正常启动输出；ssetap `notifications/messages/entities` 各连接一次并以 EOF clean disconnect，durable ranges 为 entities `7..38`、notifications `16..36`；llmtap managed proof challenge/install/models 全 `200`、本路径无 completion。UI action journal=`ui-actions.jsonl`，所有 mutation 后 fresh AX。
- 回归验证：通知/审批/dispatcher Flutter `44/44`、rig `42/42`、anchors `10/10`、`gen_coverage.py --check`=`848/395/0`、`alarms.py check`=`clean (2010)`、Python compile、shell syntax、`git diff --check` 均通过。`judge.py` 按 `G1/F1/B2/C2/G1` 写入五格，正式账本 `2005→2010 judgments`，COVERAGE `848/394→395/0`。
- 写账触发 `gap-too-fast` 与 `discovery-collapse`；独立复审证据=`/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-007-ledger-alarm-reaudit.md`，两项已按原阈值 ack，未改阈值、算法、法典、锚点或 gate。批次三十八由 `10→15/50`，未到 50 不跑统一长门禁、不提交；下一正式前线为 `SURF-008 shell/notification-tray`。P12 的 400+ Journey 扩写继续按用户裁定推迟二期。

## 2026-08-17 21:08 · SURF-006 正式五级入账，批次三十八推进至 10/50

- 新 recorder 的 formal session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-205331`，隔离数据=`/private/tmp/anselm-data-surf006-breadcrumb-20260817-r1`。真实 App 完成 Entities Overview/Function `greet` 的浮层 breadcrumb、点击回顶、右 inspector 收起/恢复、左 sidebar 收起/重开，并切换 Scheduler/Library/文档/Settings 验证旧海洋头清理；每个动作后重新读取 AX 状态。
- app-region `screencapture -R 80,40,1280,792` 录制 `248.645000s / 2560x1584 / H.264 MOV`；稳定窗口 `measure diff` 没有 App 内容跳变。诊断 diff 中的 Computer Use 指针光晕/晚到重绘以及一帧外部 AirPods 系统通知已在正式证据中单独声明，没有被冒充为产品结果。
- 五通道同一 manifest 对证：backend/frontend 无应用级红线；ssetap notifications/entities/messages 三流连接并 clean EOF，read-only 路径无业务帧；llmtap proof/install/models 全 `200` 且无 completion；UI、代码中的 `AnSize.islandHead=44` 契约、源码与录屏一致。focused shell/ocean/entity/scheduler/library/settings Flutter `128/128`、rig `42/42`、anchors `10/10`、`gen_coverage.py --check` 通过。
- `judge.py` 按 `G1/F1/B2/C2/G1` 写入 `SURF|shell/ocean-breadcrumb-head` 五格。正式账本 `2000→2005 judgments`，COVERAGE=`848/393→394/0`；写入触发 `gap-too-fast` 与 `discovery-collapse`，复审证据=`/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-006-ledger-alarm-reaudit.md`，两项已按原阈值 ack，最终 `alarms.py check`=`clean`。批次三十八由 `5→10/50`，未到 50 不跑统一长门禁、不提交；下一正式前线为 `SURF-007`。P12 的 400+ Journey 扩写继续按用户裁定推迟二期。

## 2026-08-17 20:46 · SURF-005 正式五级入账，批次三十八推进至 5/50

- 新 recorder 的 clean formal session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-203105`，隔离数据=`/private/tmp/anselm-data-surf005-footer-20260817-r5`。真实 App 通过 footer workspace 菜单创建/切换长名 workspace，验证 ellipsis、current marker、Workspace settings、Notifications tray `Today 9` 和返回 Chat；再从菜单命令重复进入 Workspace settings。旧 `-l` 盲区 session 与 stale system-dialog run 均冻结保留，没有混进正式证据。
- app-region `screencapture -R 80,40,1280,792` 录制 `178.115000s / 2560x1584 / H.264 MOV`；五个 settled 5 秒 60fps 窗口的 `measure diff -threshold 0.0005` 均无 changed pairs。前置 marker 的 latency 只作诊断，未冒充纯产品 A1 时延。红点 connected region=`(300,730,3,3,7)`，对比度=`5.38:1`。
- 五通道同一 manifest 对证：SQLite 两 workspace，demo `notifications=9/unread=9` 与 UI `Today 9` 对齐；backend/frontend 无红线；ssetap 三流连接并 clean EOF；llmtap proof/install/models 全 `200` 且无 completion。focused workspace/shell/notification/settings Flutter `90/90`、rig `42/42`、anchors `10/10` 通过。
- `judge.py` 按 `G1/F1/B2/C2/G1` 写入 `SURF|shell/sidebar-footer` 五格。正式账本 `1995→2000 judgments`，COVERAGE=`848/392→393/0`；写入触发 `gap-too-fast` 与 `discovery-collapse`，复审证据=`/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-005-ledger-alarm-reaudit.md`，两项已按原阈值 ack，最终 `alarms.py check`=`clean`。批次三十八由 `0→5/50`，未到 50 不跑统一长门禁、不提交；下一正式前线为 `SURF-006`。P12 的 400+ Journey 扩写继续按用户裁定推迟二期。

## 2026-08-17 20:07 · SURF-005 首轮冻结：浮层未进入主窗口录像，修复台架区域录制

- `SURF-005 shell/sidebar-footer` 首轮 formal candidate session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-195826` 由五通道 conductor 托管。真实 App 通过 workspace 菜单创建/切换两个长名 workspace、打开 Settings、打开/关闭 Notifications tray；AX 与稳定态截图均看到了菜单浮层、三项命令、未读红点和长名省略。
- Stop-and-fix 冻结原因是帧通道的观测盲区：旧 `screencapture -l <主窗口ID>` MOV 只录主窗口，工作区菜单/OverlayPortal 浮层不出现在连续录屏；这不满足逐帧标准，故没有写 `judge.py`，也没有修改 SURF-005 五格。
- 台架修复：conductor 由同一 CoreGraphics window ID 解析 `appWindowBounds`，改用 `screencapture -v -R x,y,w,h` 录 App 区域；manifest 落 window ID+geometry，`rig-check` 对区域命令 fail-closed。这样不录全桌面，且同一 App 区域内的菜单/浮层进入 MOV。`testend/rig/README.md`、`README.md` 已同步，旧历史 session 不改写。
- 本轮当前仍为 `SURF-005` 未裁决；下一动作是新 recorder 的真实 App 重跑。formal ledger=`1995`、COVERAGE=`848/392/0`、anchors=`10/10`、alarms=`clean`，本机制修复不计入 50 格批次。
...
- 根门禁首轮暴露 `backend/internal/transport/httpapi/handlers/notification.go` 的 S6 违规：`POST /notifications/unread-count` 分支直接调用 `responsehttpapi.Error`。冻结修复增加 `KindMethodNotAllowed` / `ErrMethodNotAllowed`，handler 与 `router/envelopeMuxErrors` 的 405 都统一走 `FromDomainError`，保留 `Allow` header；`error-codes.md` 的 Kind/合成码归属同步。focused errors/response/handlers/router tests、完整 Go、根门禁与 testend 均在修复后复跑通过。
- 账本审计保持 formal `1945 judgments`、COVERAGE `848/382/0`、anchors `10/10`、alarms `clean`；rig 自测 `42/42`、measure/ssetap Go tests、coverage、anchors、alarms、docs verify、`git diff --check` 均通过。完整 testend 收台后 conductor-owned backend/taps/Flutter/llama/监听端口为零；另清理明确归属的陈旧 EP-208 fixture，未把它混入本轮证据。
- 当前只剩本批工作树归属审计与提交；提交后下一原子前线为 EP-251 `GET /api/v1/notifications/stream`。P12 的 400+ Journey 扩写仍按用户裁定推迟二期。

## 2026-08-17 16:38 · EP-250 entities SSE 正式五级收口，批次三十六 51/50，触发统一门禁

- EP-250 `GET /api/v1/entities/stream` 在当前源码、真实 Flutter macOS App、真实受管 gateway、Computer Use、窗口录屏、backend/frontend journal、三路独立 SSE witness 和真实 llmtap 下完成；正式 mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-163104`，录屏 `196.013333s / 2788x1808 / 60fps`，由同一 conductor 正常封口。旧候选和第一次 channel-5 wiring fail-closed 启动只保留台架历史，不与正式证据拼接。
- 真实 App 在 Entities 中观察新 Function `ep250_entities_stream_probe_r2` 从创建到调试运行：rail/Overview 从 3 到 4，详情显示 `env ready`、Python `3.12`；点击 Run 后真实右岛显示 `Done · 74ms`、Output、Result、Logs、Recent Manual。REST/SQLite 对账执行 `fne_a713bf6b90e0f898`、`status=ok`、`triggeredBy=manual`、输出 `{"status":"entities-stream-r2"}`、日志原文与 UI 一致。
- 协议矩阵覆盖 `fromSeq=1` 回放 `seq=2..4`、`Last-Event-ID:3` 优先只回 `4`、坏 cursor live-only、缺 workspace `401 UNAUTH_NO_WORKSPACE` 和错方法 `405 METHOD_NOT_ALLOWED`。entities SSE durable `1..4` 为 build/run 两组 `open→close`，两条 delta 均 `seq=0`；notifications durable `1..3`，messages 无业务帧符合 direct Function 不创建 Chat 回合。开放 SSE 的 `curl --max-time 2` 主动截断不计为服务失败。
- 五通道对账：55 张稳定抽帧无 clipping/overlap/white flash/reflow/button drift/input jump；backend 无 panic/FATAL/application ERROR/stack trace，frontend 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception，llmtap wiring 真实通过且无 chat completion 符合不经过 LLM；三路 SSE、REST、SQLite、UI 终态一致。
- `judge.py` 按 `F1 / F2 / B2 / C5 / G1` 写入 EP-250=`✓✓✓✓✓`，formal ledger `1940→1945 judgments`，`gen_coverage.py --check`=`848/382/0`，anchors=`10/10`。写账触发 `gap-too-fast` 与 `discovery-collapse`；复审证据=`/private/tmp/anselm-rig-formal-20260801-3/evidence/EP-250-ledger-alarm-reaudit.md`，两条逐项 ack，未改阈值/算法/法典/锚点/gate，最终 `alarms.py check`=`clean (1945 judgments)`。
- entitystream/function/handlers/response/router Go tests、measure/ssetap Go tests、rig Python `42` 项、ffprobe/抽帧、PNG measure、协议/REST/SQLite 证据、coverage、anchors、alarms 与 `git diff --check` 通过。本批次由 `46→51/50`，已跨过用户规定的 50 格边界；下一步只执行统一长门禁、工作树归属审计和本批提交，完成后再进入 EP-251。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-17 16:27 · EP-249 messages SSE 正式五级收口，批次三十六 46/50

- EP-249 `GET /api/v1/messages/stream` 在当前源码、真实 Flutter macOS App、真实受管 gateway、Computer Use、窗口录屏、backend/frontend journal、三路独立 SSE witness 和真实 llmtap 下完成；正式 mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-161753`，录屏 `164.503333s / 2784x1808 / 60fps`，由同一 conductor 正常封口。早先候选 session 仅保留历史观察，不与正式证据拼接。
- 真实 App 在 Chat composer 发送 `Reply with exactly EP-249 current stream smoke passed.`，经过真实受管网关显示 thinking/search 反馈，最终精确显示 `EP-249 current stream smoke passed.`；conversation=`cv_46cea9226d0caff7`，标题从短暂 `New chat` 收敛为提示词，composer、transcript 和响应操作稳定可用。
- 协议/后端矩阵覆盖 `fromSeq=1` 回放 `seq=2..14`、`Last-Event-ID:13` 优先只回 `14`、坏 cursor live-only、缺 workspace `401 UNAUTH_NO_WORKSPACE` 和错方法 `405 METHOD_NOT_ALLOWED`。源码三条且仅三条 SSE 注册、游标优先级、`ErrSeqTooOld→410` 由 `stream_test.go` 覆盖；开放 SSE 的 `curl --max-time 2` 主动截断不计为服务失败。
- 五通道对账：messages durable `seq=1..14` 单调、`seq=0` 仅为 ephemeral delta，notifications 有 2 个 durable title 帧；SQLite user/assistant 终态与 UI/SSE close 一致，assistant `stop_reason=end_turn`；backend 无 panic/FATAL/application ERROR/stack trace，lexical fallback 与 stale handler env INFO 如实保留；frontend 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception 红线；llmtap 真实记录 3 次 chat completion，响应全 `200`，无 fixture completion。
- 稳定帧复核无 clipping、overlay、white flash、reflow 或 input jump。`judge.py` 按 `F1 / F2 / B2 / C5 / G1` 写入 EP-249=`✓✓✓✓✓`，formal ledger `1935→1940 judgments`，`gen_coverage.py --check`=`848/381/0`，anchors=`10/10`。写账触发 `gap-too-fast` 与 `discovery-collapse`；复审证据=`/private/tmp/anselm-rig-formal-20260801-3/evidence/EP-249-ledger-alarm-reaudit.md`，两条逐项 ack，未改阈值/算法/法典/锚点/gate，最终 `alarms.py check`=`clean (1940 judgments)`。
- handlers/response/router Go tests、measure/ssetap Go tests、rig Python `42` 项、ffprobe/抽帧、协议/REST/SQLite 证据、coverage、anchors、alarms 与 `git diff --check` 通过。本批次由 `41→46/50`，未到 50 格不跑统一长门禁、不提交；下一原子前线为 EP-250 `GET /api/v1/entities/stream`。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-17 16:13 · EP-248 triage 正式五级收口，批次三十六 41/50

- EP-248 `POST /api/v1/executions/{id}:triage` 在真实 Flutter macOS App、真实受管 gateway、Computer Use、窗口录屏、backend/frontend journal、三路独立 SSE witness 和真实 llmtap 下完成；正式 mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-160404`，录屏 `112.831667s / 2788x1808 / 60fps / 6134 frames`，由同一 conductor 正常封口。
- 真实产品路径从 Scheduler 打开失败的 `EP-248 Triage Failure Probe`，进入 run inspector 点击 `AI triage`。App 发出 bodyless `POST /api/v1/executions/fr_2caa9beb6062ebc5:triage`，backend 精确记录 `202 / 38 bytes / 3ms`，创建 conversation `cv_8fab97ee9f5f9222`；最终回答完整解释 workflow、`boom`、`fn_ep248_missing` 和 `action fn_ep248_missing: function not found`，不自动创建实体或隐式修复。
- 本轮只使用该单一 direct-triage session 做正式证据。旧候选中的 bodyless 400、managed install 失效、opaque ID 截断和孤立反引号保留为红史，不能与本轮拼接；修复后才重新跑通正向 endpoint。
- 五通道对账：目标 conversation 的 messages durable 序列单调 `1..29`，`seq=28` 为完整文本 close、`seq=29` 为 assistant `end_turn`；llmtap 记录 4 次真实 chat completion，响应全 `200`，最终 wire 与 SSE close、SQLite assistant block、UI 逐字一致；frontend journal 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception 红线。backend 无 panic/FATAL/application ERROR/stack trace，但如实保留隔离目录缺少 `llama-server` 的 lexical fallback WARN/INFO；auto-title 从短暂 `New chat` 收敛到 `Please diagnose this execution`。
- 原生分辨率稳定抽帧的 `measure diff` 在 `0.0005` 阈值下无输出，黑白对比基准 `16.67:1`；最终回答的标题、节点表、单行错误 code block、两种修复选择和 Copy/Fork/Retry/Read aloud 操作均可读，未见 clipping、overlap、reflow、white flash 或 input jump。
- `judge.py` 按 `F1 / F2 / B2 / C5 / G1` 写入 EP-248=`✓✓✓✓✓`，formal ledger `1930→1935 judgments`，`gen_coverage.py --check`=`848/380/0`，anchors=`10/10`。写账触发 `gap-too-fast` 与 `discovery-collapse`；复审证据=`/private/tmp/anselm-rig-formal-20260801-3/evidence/EP-248-ledger-alarm-reaudit.md`，两条逐项 ack，未改阈值/算法/法典/锚点/gate，最终 `alarms.py check`=`clean (1935 judgments)`。
- aispawn/handler/router Go tests、measure/ssetap Go tests、rig Python `42` 项、ffprobe/measure、REST/SQLite 证据、coverage、anchors、alarms 与 `git diff --check` 通过。本批次由 `36→41/50`，未到 50 格不跑统一长门禁、不提交；下一原子前线为 EP-249 `GET /api/v1/messages/stream`。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-17 15:52 · EP-247 mark-all-unread 正式五级收口，批次三十六 36/50

- EP-247 `POST /api/v1/notifications:mark-all-unread` 在真实 Flutter macOS App、真实受管 gateway、Computer Use、窗口录屏、backend/frontend journal、三路独立 SSE witness 和真实 llmtap 下完成；formal mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-152343-ep247`，录屏 `446.876667s / 2784x1808 / 60fps / 25045 frames`，由 conductor 正常封口。
- 真实 App 从左岛 bell 打开通知托盘，Today 组更多操作显示 `Mark all read` / `Mark all unread`；点击 `Mark all unread` 只提交该组 `[after,before)` 窗，菜单收束、通知行保留为审计记录。最终 SQLite/权威 unread 对账为主 workspace `65/12/53`、另一 workspace `2/2/0`（`total/unread/read`）。
- REST/后端矩阵覆盖成功 `204`、bodyless、窗口边界、重复、坏 RFC3339 `422 NOTIFICATION_INVALID_WINDOW`、坏 JSON `400 INVALID_REQUEST`、缺 workspace `401 UNAUTH_NO_WORKSPACE`、坏 Host `403 FORBIDDEN_BAD_HOST` 和错误 GET `405 METHOD_NOT_ALLOWED`；批量标记按契约没有伪造 notification SSE echo。完整 contract evidence=`/private/tmp/anselm-rig-formal-20260801-3/evidence/EP-247-contract-matrix.md`。
- 五通道封口：backend 无应用级 panic/FATAL/WARN/ERROR/exception；ssetap 两 workspace 三流六连接 clean EOF；frontend `90` 行中 87 条严格匹配已知 Computer Use AXTree stale-node 形状，`unknown_error=0`；llmtap 真实指向 `https://api.anselm.website` 且本确定性 REST/UI 路径无 completion。关键帧和 stable diff 未见 clipping/overlap/reflow/white flash/input jump。
- `judge.py` 按 `F1 / F2 / B2 / C5 / G1` 写入 EP-247=`✓✓✓✓✓`，formal ledger `1925→1930 judgments`，`gen_coverage.py --check`=`848/379/0`，anchors=`10/10`。写账触发 `gap-too-fast` 与 `discovery-collapse`；复审证据=`/private/tmp/anselm-rig-formal-20260801-3/evidence/EP-247-ledger-alarm-reaudit.md`，两条逐项 ack，未改阈值/算法/法典/锚点/gate，最终 `alarms.py check`=`clean (1930 judgments)`；AX 归类=`sessions/20260817-152343-ep247/evidence/frontend-ax-review.md`。
- 后端 notification/store/handler/router Go tests、Flutter notifications `80` 项、measure/ssetap Go tests、rig unittest `42` 项、ffprobe/measure、REST/SQLite 证据、coverage、anchors、alarms 与 `git diff --check` 通过。本批次由 `31→36/50`，未到 50 格不跑统一长门禁、不提交；下一原子前线为 EP-248。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-17 15:20 · EP-246 mark-all-read 正式五级收口，批次三十六 31/50

- EP-246 `POST /api/v1/notifications:mark-all-read` 在真实 Flutter macOS App、真实受管 gateway、Computer Use、窗口录屏、backend/frontend journal、三路独立 SSE witness 和真实 llmtap 下完成。绿色 formal mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-150224-ep246`，录屏 `318.578333s / 2784x1808 / 60fps`，由 conductor 正常 `rig-down` 封口。
- 真实 App 从左岛 bell 打开通知托盘，Today 组的 `More actions` 菜单显示 `Mark all read` / `Mark all unread`；点击 `Mark all read` 后只作用于该组时间窗，Today 收起、Yesterday 保持可见、行仍保留为审计记录。REST 矩阵覆盖 bodyless/`{}` 全账本、半开 `[after,before)`、重复、合法空窗、坏 RFC3339 `422`、坏 JSON `400`、跨 workspace、缺 workspace `401`、坏 Host `403` 和错误 GET `405`。
- SQLite 最终直接对账为主 workspace `65/65/0`、另 workspace `2/2/0`（`total/unread/read`），对应最后一次明确 restore；UI、REST 未读数、backend journal 与持久化真相一致。批量读按契约没有伪造 notification SSE echo，llmtap 只有真实上游 ready、没有伪造 completion。
- 五通道封口：backend 应用级错误 marker `0`；ssetap journal `15` 行、两个 workspace 三路流连接并 clean EOF；frontend `59` 行中 56 条严格匹配已知 Computer Use AXTree stale-node 形状，独立 `unknown_error=0`，其余为正常启动行；关键菜单/收束/稳定帧已封存，未见 clipping、overlap、reflow、white flash 或 input jump。
- `judge.py` 按 `F1 / F2 / B2 / C5 / G1` 写入 EP-246=`✓✓✓✓✓`，formal ledger `1920→1925 judgments`，`gen_coverage.py --check`=`848/378/0`，anchors=`10/10`。写账触发 `gap-too-fast` 与 `discovery-collapse`；复审证据=`/private/tmp/anselm-rig-formal-20260801-3/evidence/EP-246-ledger-alarm-reaudit.md`，两条均逐项 ack，未改阈值/算法/法典/锚点/gate，最终 `alarms.py check`=`clean (1925 judgments)`；AX 独立归类=`sessions/20260817-150224-ep246/evidence/frontend-ax-review.md`。
- 后端 notification/store/handler/router Go tests、Flutter notifications `79` 项、串行 Dart analysis、measure/ssetap Go tests、rig unittest `42` 项、ffprobe/measure、REST/SQLite 证据、coverage、anchors、alarms、docs verify 和 `git diff --check` 通过。本批次由 `26→31/50`，未到 50 格不跑统一长门禁、不提交；下一原子前线为 EP-247。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-17 14:56 · EP-245 mark-read 正式五级收口，批次三十六 26/50

- EP-245 `POST /api/v1/notifications/{id}:mark-read` 在真实 Flutter macOS App、真实受管 gateway、Computer Use、窗口录屏、backend/frontend journal、三路独立 SSE witness 和真实 llmtap 下完成。绿色 formal mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-144121-ep245`，录屏 `347.493333s / 2784x1808 / 60fps`，由 conductor 正常 `rig-down` 封口。
- 真实 App 从左岛 bell 打开通知托盘，点击 `Function “fetch_orders_01” created`；后端真实 `204`，SQLite `noti_ep243_001.read_at` 落盘，通知仍留在列表但转为已读灰色。重复标已读仍 `204`，没有重复 mutation 或重排。
- REST/SQLite matrix：主 workspace `65/47/18`，另一个 `2/2/0`；不存在与跨 workspace `404 NOTIFICATION_NOT_FOUND`，缺 workspace `401 UNAUTH_NO_WORKSPACE`，坏 Host `403 FORBIDDEN_BAD_HOST`，错误 GET `405 METHOD_NOT_ALLOWED`。原始 response files 在 session `evidence/EP-245-*.json/.headers`。
- 五通道：backend 无应用级 WARN/ERROR/panic/FATAL/stack trace；frontend 只有正常 Dart VM 行；ssetap 两 workspace 三流六连接 clean EOF，mark-read 无 SSE echo；llmtap 真实指向 `https://api.anselm.website` 且只记录 ready，本确定性 REST/UI 路径没有伪造 completion。逐帧 `frame-0050→0051` 只改变首行读态，局部测量 ROI `changedFrac=0.01112`，未见 clipping/overlap/reflow/white flash/input jump。
- `judge.py` 按 `F1 / F2 / B2 / C5 / G1` 写入 EP-245=`✓✓✓✓✓`，formal ledger `1915→1920 judgments`，`gen_coverage.py --check`=`848/377/0`，anchors=`10/10`。写账触发 `gap-too-fast` 与 `discovery-collapse`；复审证据=`/private/tmp/anselm-rig-formal-20260801-3/evidence/EP-245-ledger-alarm-reaudit.md`，两条逐项 ack 后 `alarms.py check`=`clean (1920 judgments)`，未改阈值/算法/法典/锚点/gate。
- backend notification/store/handler/router Go tests、Flutter notifications `79` 项、串行 Dart analysis、measure/ssetap Go tests、rig unittest `42` 项、ffprobe/measure、coverage、anchors、alarms 与 `git diff --check` 通过。本批次由 `21→26/50`，未到 50 格不跑统一长门禁、不提交；下一原子前线为 EP-246 `POST /api/v1/notifications:mark-all-read`。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-17 14:36 · EP-244 unread-count 正式五级收口，批次三十六 21/50

- EP-244 `GET /api/v1/notifications/unread-count` 完成真实 Flutter macOS App、真实受管 Anselm gateway、Computer Use、窗口录屏、backend/frontend journal、三路独立 SSE witness 和真实 llmtap。绿色 formal mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-142451-ep244`，红路径 mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-141809-ep244-red`；两场均由 conductor 封口。
- 首轮真实负向矩阵捕获 `POST /api/v1/notifications/unread-count` 被 generic action 路由误报为 `404 NOT_FOUND`，该红证据保留且不计绿。stop-and-fix 在 `notification.go` 增加保留 `unread-count` 的 `405 METHOD_NOT_ALLOWED` 分支，补 handler regression，API reference 同步 `Allow: GET, HEAD` 契约；修复后二进制真实重跑仅一次正确拒绝。
- 真实 App 的权威 unread 状态为 `44→43→36→48`：单条已读、Today 组全读、Today 组全未读均由真实点击驱动；冷重启后 Today `12`、Yesterday `9` 与 REST/SQLite unread `48` 重建一致。跨 workspace 为主 `65/48/17`、另一个 `2/2/0`，缺/未知 workspace `401`，坏 Host `403`。
- 五通道封口：backend `159` 行无应用级红线；ssetap 两 workspace 三流六连接 clean close；frontend 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception；llmtap 真实 managed wiring 通过，本确定性路径未伪造 completion；录屏 `93.560000s / 2784x1808 / 60fps`，收台黑帧不计产品证据，stable frames 无 clipping/overlap/reflow/white flash/input jump。
- `judge.py` 按 `F1 / F2 / B2 / C5 / G1` 写入 EP-244=`✓✓✓✓✓`，formal ledger `1910→1915 judgments`，`gen_coverage.py --check`=`848/376/0`，anchors=`10/10`。写账触发的 `gap-too-fast` 与 `discovery-collapse` 经红绿证据、五通道 journal、REST/SQLite、修复回归和锚点独立复审后逐项 ack；未改阈值/算法/法典/锚点/gate，最终 `alarms.py check`=`clean (1915 judgments)`；复审=`/private/tmp/anselm-rig-formal-20260801-3/evidence/EP-244-ledger-alarm-reaudit.md`。
- 后端 notification/store/handler/router Go tests、Flutter 通知与契约 suite `82/82`、串行 Dart analysis、measure/ssetap Go tests、rig Python `21/21`、ffprobe/measure、coverage、anchors、alarms、docs verify 和 `git diff --check` 通过。本批次由 `16→21/50`，按用户规则不跑统一长门禁、不提交；下一原子前线为 EP-245 `POST /api/v1/notifications/{id}:mark-read`。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-17 14:05 · EP-243 notifications 正式五级收口，批次三十六 16/50

- EP-243 `GET /api/v1/notifications` 完成真实 Flutter macOS App、真实受管 Anselm gateway、Computer Use、窗口录屏、backend/frontend journal、三路独立 SSE witness 和真实 llmtap。正确 session mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-140206-ep243`，冷重启 mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-140453-ep243`；两场均由 conductor 封口，首轮错误时间格式的 fixture session 已废弃，不进入 formal。
- 隔离主 workspace 写入 65 条混合类型、Today/Yesterday/Earlier 三段、交错 read/unread 的通知，另一个 workspace 仅 2 条。真实 App 从左岛 bell 打开托盘，fresh AX/frame 看到 Today `12`、Yesterday `9`，继续滚动触发第二页并到达 Earlier 的第 65 条；REST unread-count 为 `44`。冷重启后首屏与未读数保持一致，中心 Settings 内容没有被通知托盘错误替换。
- REST matrix 覆盖第一页 `limit=40`（40 条+cursor）、第二页（25 条、无 cursor）、尾 cursor 空页、`limit=201` 钳到最大值、`limit=0`/malformed cursor `400`、缺失 workspace `401 UNAUTH_NO_WORKSPACE`、坏 Host `403 FORBIDDEN_BAD_HOST`、POST `405 METHOD_NOT_ALLOWED` + GET/HEAD Allow，以及跨 workspace 只见本 workspace 2 条。组合 65 条唯一、跨页无重叠且 `createdAt` 非递增。
- 五通道封口：backend 无应用级 `WARN/ERROR/panic/FATAL/stack trace`；ssetap 两 workspace 三流均连接并 clean close，只读列表无虚构 durable SSE 事件；frontend 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception 产品红线；llmtap 真实指向 `https://api.anselm.website`，managed wiring 通过且无 completion；录屏稳定帧未见 clipping、overlap、reflow、white flash 或 input jump。正式证据集中在两场 session 与 `evidence/EP-243-*.md`。
- `judge.py` 按 `F1 / F2 / B2 / C5 / G1` 写入 EP-243=`✓✓✓✓✓`，formal ledger `1905→1910 judgments`，`gen_coverage.py --check`=`848/375/0`，anchors=`10/10`。写账触发的 `gap-too-fast` 与 `discovery-collapse` 经两场 App、REST cursor 矩阵、重启复核、稳定帧、五通道 journal、实现测试和锚点独立复审后逐项 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (1910 judgments)`；复审=`evidence/EP-243-ledger-alarm-reaudit.md`。
- 本批次由 `15→16/50`，按用户规则不运行统一长门禁、不提交；下一原子前线为 EP-244 `GET /api/v1/notifications/unread-count`。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-17 13:50 · EP-242 storage compact 正式五级收口，批次三十六 15/50

- EP-242 `POST /api/v1/storage:compact` 完成真实 Flutter macOS App、真实受管 Anselm gateway、Computer Use、窗口录屏、backend/frontend journal、三路独立 SSE witness 和真实 llmtap。真实压缩 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-133730-ep242`，冷重启 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-133942-ep242`；两场均由 conductor 封口，录屏分别为 `97.481667s` 与 `43.765000s`，`2784x1808 / 60fps`。
- 为验证真实回收而非 no-op，隔离数据目录在后端停机时只创建/删除临时 `ep242_compact_fixture` 表，形成 `page_count=4810 / freelist_count=4609 / auto_vacuum=2`；真实 App 看到 `18.8 MB / 18.0 MB reclaimable`，点击 `Compact database` 后收到 `Reclaimed 18.0 MB`，UI/REST 收敛为 `784.0 KB / 0 B`。第二次 POST 返回 `reclaimedBytes=0,migrated=false`，证明幂等。
- REST 负向矩阵为缺失/未知 workspace `401 UNAUTH_NO_WORKSPACE`、GET `405 METHOD_NOT_ALLOWED` + `Allow: POST`、坏 Host `403 FORBIDDEN_BAD_HOST`；压缩后和冷重启后 SQLite/REST 对账为 `fileBytes=802816 / page_count=196 / freelist_count=0 / workspaceCount=2 / attachmentCount=3`，live/soft-deleted 附件行均保留。
- 五通道无应用级红线：backend 真实 compact 200/18ms 且无 panic/FATAL，SSE 两 workspace 三流均 connect/disconnect clean、无伪造 durable 事件，frontend 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception，llmtap 指向真实 `https://api.anselm.website` 且两 workspace wiring 通过；稳定帧未见 clipping、overlap、reflow、white flash、input jump 或持久数据跳变。收台时最后黑帧明确排除为 teardown。
- `judge.py` 按 `F1 / F2 / B2 / C2 / G1` 写入 EP-242=`✓✓✓✓✓`，formal ledger `1900→1905 judgments`，`gen_coverage.py --check`=`848/374/0`，anchors=`10/10`。写账触发的 `gap-too-fast` 与 `discovery-collapse` 经两场 App、REST/SQLite 前后证据、五通道、实现测试和锚点复审后逐项 ack，未改阈值/算法/法典/锚点/gate，最终 `alarms.py check`=`clean (1905 judgments)`；复审=`evidence/EP-242-ledger-alarm-reaudit.md`。
- backend storage/db/handler 定向 Go tests、focused Flutter `s5_storage_limits_test.dart` `16/16`、串行 Dart analysis、录屏/ffprobe、PNG measure/contrast、rig 工具测试、SQLite fixture 前后核对、coverage、anchors、alarms 和 `git diff --check` 通过。本批次仅 `15/50`，按用户规则不跑统一长门禁、不提交；下一原子前线为 EP-243 `GET /api/v1/notifications`。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-17 13:35 · EP-241 storage-stat 正式五级收口，批次三十六 10/50

- EP-241 `GET /api/v1/storage-stat` 完成真实 Flutter macOS App、真实受管 Anselm gateway、Computer Use、窗口录屏、backend/frontend journal、三路独立 SSE witness 和真实 llmtap。完整 App mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-132248-ep241`，前置数据操作=`sessions/20260817-131821-ep241`，REST 负矩阵=`sessions/20260817-132513-ep241-matrix`。
- 真实 App 在 Settings → System → Storage & logs 从初始 `Database 784.0 KB / 0 B reclaimable`、`Attachments 0 B / 0 B reclaimable`，经真实附件上传/软删除与第二 workspace 上传后，离开再进入显示 `Database 784.0 KB / 0 B reclaimable`、`Attachments 20.0 KB / 2.0 KB reclaimable`；重启 App/sidecar 后保持。两个 workspace 的 REST 结果一致，SQLite/CAS 对账为 live `4096+16384=20480`、soft-deleted `2048`。
- REST matrix 覆盖 valid/query `200`、缺失/未知 workspace `401 UNAUTH_NO_WORKSPACE`、坏 Host `403 FORBIDDEN_BAD_HOST`、POST `405 METHOD_NOT_ALLOWED` + `Allow: GET, HEAD`。五通道无应用级 backend/frontend 红线，SSE 两 workspace 三流 clean close，llmtap managed wiring 真实通过且无伪造 completion；录屏稳定帧无 clipping、overlap、reflow、white flash 或 input jump。
- `judge.py` 按 `F1 / F2 / B2 / C2 / G1` 写入 EP-241=`✓✓✓✓✓`，formal ledger `1895→1900 judgments`，`gen_coverage.py --check`=`848/373/0`，anchors=`10/10`。写账触发的 `gap-too-fast` 与 `discovery-collapse` 经两场真实 App、独立负矩阵、SQLite/CAS、五通道和实现测试独立复审后逐项 ack，未改阈值/算法/法典/锚点/gate，最终 `alarms.py check`=`clean (1900 judgments)`；复审=`evidence/EP-241-ledger-alarm-reaudit.md`。
- backend storage/db/handler 定向 Go tests、focused Flutter `s5_storage_limits_test.dart` `16/16`、串行 Dart analysis、录屏/ffprobe、measure/contrast、rig 工具测试、coverage、anchors、alarms 和 `git diff --check` 通过。本批次仅 `10/50`，按用户规则不跑统一长门禁、不提交；下一原子前线为 EP-242 `POST /api/v1/storage:compact`。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-17 13:15 · EP-240 retention PATCH 正式五级收口，批次三十六 5/50

- EP-240 `PATCH /api/v1/retention` 完成真实 Flutter macOS App、真实受管 Anselm gateway、Computer Use、窗口录屏、backend/frontend journal、三路独立 SSE witness 和真实 llmtap。正式 mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-130251-ep240`，上一场真实保存 session=`sessions/20260817-125848-ep240-save`；两场均由 conductor 封口。
- 真实产品路径为 Settings → System → Storage & logs：从 `90 days` 选择 `180 days` 即提交 PATCH，并显示 `Retention updated`；随后验证 `Keep forever`=`0`，再恢复 `180 days`。离开、重新进入和重启 App/sidecar 后 fresh GET 与 settings snapshot 均保持 `180 days`。产品没有独立静态 Save 按钮，本格按“选择即保存”记录，不虚构 UI 控件。
- REST/后端覆盖空对象 no-op、负数/未知字段 `400 SETTINGS_RETENTION_INVALID`、缺失 workspace `401 UNAUTH_NO_WORKSPACE`；真实 sweep 删除过期 completed run、保留 running run，backend journal 记录 `runs=1 retentionDays=180`。无应用级 backend 红线；frontend 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception；三路 SSE clean close；llmtap managed wiring 真实通过且未伪造 completion；录屏稳定帧未见 clipping、overlap、reflow、white flash 或 input jump。
- `judge.py` 按 `F1 / F2 / B2 / C2 / G1` 写入 EP-240=`✓✓✓✓✓`，formal ledger `1890→1895 judgments`，`gen_coverage.py --check`=`848/372/0`，anchors=`10/10`。写账触发的 `gap-too-fast` 与 `discovery-collapse` 经两场 session、REST/fixture、五通道、实现测试和锚点独立复审后逐项 ack，未改阈值/算法/法典/锚点/gate，最终 `alarms.py check`=`clean (1895 judgments)`。
- backend retention/settings/scheduler/flowrun 定向 Go tests、focused Flutter `s5_storage_limits_test.dart` `16/16`、串行 Dart analysis、录屏/ffprobe、PNG measure/contrast、rig 自测、SQLite 前后核对、coverage、anchors、alarms 和 `git diff --check` 通过。本批次仅 `5/50`，按用户规则不跑统一长门禁、不提交；下一原子前线为 EP-241 `GET /api/v1/storage-stat`。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-17 12:32 · EP-239 retention GET 正式五级收口，批次三十五 50/50

- EP-239 `GET /api/v1/retention` 完成真实 Flutter macOS App、真实受管 Anselm gateway、Computer Use、窗口录屏、backend/frontend journal、三路独立 SSE witness 和真实 llmtap。formal mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-122156-ep239`，录屏 `125.515000s / 2784x1808 / 60fps / 7056 frames`；`rig-check`/`rig-down` 通过且 owned processes/listeners 清零。
- 真实 App 在 Settings → System → Storage & logs 读取权威 `GET`，渲染 `90 days`、`This machine` 和 retention/统计窗口语义；打开下拉但不 mutation，录屏逐帧看到 `30 days / 90 days / 180 days / Keep forever`，90 days 选中。离开并重新进入后 fresh GET 仍返回 90。REST matrix 覆盖 valid/query `200`、缺失/未知 workspace `401 UNAUTH_NO_WORKSPACE`、POST `405 METHOD_NOT_ALLOWED` + `Allow: GET, HEAD, PATCH`、坏 Host `403 FORBIDDEN_BAD_HOST`；settings snapshot 证明 sibling blocks 保持不变。
- 五通道已封口：backend 无应用级 WARN/ERROR/panic/FATAL；ssetap 三流 clean close 且只读路径无虚构 durable 事件；frontend 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception 产品红线；llmtap readiness 指向 `https://api.anselm.website` 且无 completion；sealed MOV、菜单帧、稳定帧、矩阵和正式证据均保留在 session。一次旧 AX index 复用被复核为仪器观测问题，不计产品缺陷。
- 正式按 `F1 / F2 / B2 / C2 / G1` 写入 `COVERAGE EP-239=✓✓✓✓✓`，formal ledger `1885→1890 judgments`，`gen_coverage.py --check`=`848/371/0`，anchors=`10/10`。写账触发 `gap-too-fast` 与 `discovery-collapse`，两条均依据封口 session、菜单/稳定帧、measure diff、REST/实现测试、五通道 journal 和锚点复审后 ack；未改阈值/算法/法典/锚点/gate，最终 `alarms.py check`=`clean (1890 judgments)`。
- 后端定向 Go tests、focused Flutter `s5_storage_limits_test.dart` `16/16`、串行 Dart analysis、录屏完整性、PNG `measure diff`、rig 自测 `42/42`、SQLite integrity、coverage、anchors、alarms 和 `git diff --check` 通过。批次三十五由 `45→50/50`；随后统一 `make verify`（backend/frontend/docs/demo 全绿，docs 仅仓内既有 6 条 review-due 警告）、backend `go test ./...`、完整 `make -C backend testend`（`279.171s`）、已修场景回归和残留进程审计均通过。工作树审计只暂存本轮四份 acceptance working 文档，其他团队改动未触碰；本批次验收记录已提交。P12 的 400+ Journey 扩写继续按用户裁定推迟二期。

---
## 2026-08-17 11:18 · EP-236 system/data-dir 正式五级收口，批次三十五 35/50

- EP-236 `GET /api/v1/system/data-dir` 完成真实 Flutter macOS App、真实受管 Anselm gateway、Computer Use、窗口录屏、backend/frontend journal、三路独立 SSE witness 和真实 llmtap。formal mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-111018-ep236`，录屏 `62.548333s / 2696x1720 / 60fps`；`rig-check`/`rig-down` 通过且 owned processes/listeners 清零。
- 真实 backend REST matrix：无 workspace=`401 UNAUTH_NO_WORKSPACE`；正确 workspace=`200` 并返回 `/private/tmp/anselm-rig-ep232-codex-20260817-a/data`；未知 workspace=`401`；POST=`405 METHOD_NOT_ALLOWED` + `Allow: GET, HEAD`；坏 Host=`403 FORBIDDEN_BAD_HOST`。App 的 Storage & logs 从权威 sidecar path 渲染，不使用客户端猜测。
- 首轮 stop-and-fix 记录两项：AppKit 异步 `NSWorkspace.open` 被误用成同步 Bool 导致 macOS build 失败，已改为 completion handler；真实点击又证明 `Open logs folder` 回报成功但 Finder 无可观察结果，故改成经过实测的 `Reveal logs folder`/`selectFile`，中英文 slang 生成物同步。fresh AX/frame 复核 data 与 logs 均被 Finder 选中，stable App frames 无 clipping、overlap、reflow、white flash 或按钮文字跳变；稳定帧 `measure diff` 无超阈值记录。
- 五通道：backend 无应用级 WARN/ERROR/panic/FATAL；ssetap 三流 clean close；frontend 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception；llmtap readiness 指向 `https://api.anselm.website` 且不伪造 completion。正式证据为 session `evidence/EP-236-data-dir-final-green.md`，矩阵为 `evidence/EP-236-data-dir-matrix.txt`，报警复审为 `/private/tmp/anselm-rig-formal-20260801-3/evidence/EP-236-ledger-alarm-reaudit.md`。
- 正式按 `F1 / F2 / B2 / C2 / G1` 写入 COVERAGE EP-236=`✓✓✓✓✓`，formal ledger `1870→1875 judgments`，`gen_coverage.py --check`=`848/368/0`，anchors=`10/10`。写账触发 `gap-too-fast` 与 `discovery-collapse`，均依据封口 session、stop-and-fix、AX、measure 和五通道证据独立复审后 ack；未改阈值/算法/法典/锚点/gate，最终 `alarms.py check`=`clean (1875 judgments)`。
- backend contract `go test`、focused Flutter `s5_storage_limits_test.dart` `16/16`、串行 Dart analysis、AppKit build、measure diff、rig 自测 `42/42`、coverage、anchors、alarms 与 `git diff --check` 通过。批次三十五由 `30→35/50`，未满 50 格不跑统一长门禁、不提交；下一原子前线为 EP-237。P12 400+ Journey 扩写按用户裁定推迟二期；EP-220 不删除、不重跑。

## 2026-08-17 10:34 · EP-235 version 正式五级收口，批次三十五 30/50

- EP-235 GET /api/v1/version 完成真实 Flutter macOS App、真实受管 Anselm gateway、Computer Use、窗口录屏、backend/frontend journal、三路独立 SSE witness 和真实 llmtap；formal mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-102416-ep235`，录屏 `196.810000s / 2784x1808 / 60fps`。App 从 Settings → About 真实看到 App `0.1.0`、Engine `dev`；更新点击从 `Checking…` 回到按钮并给明确离线提示，About 稳定帧无 clipping、overlap、reflow、white flash 或按钮漂浮。
- 真实 tokenless matrix：无 workspace GET=`200 {"data":{"version":"dev"}}`，POST=`405 METHOD_NOT_ALLOWED`，坏 Host=`403 FORBIDDEN_BAD_HOST`。独立 token server：缺失/错误 bearer=`401 UNAUTH_BAD_TOKEN`，正确 bearer 在无 workspace/带 workspace header 下=`200`，正确 bearer+坏 Host=`403`。真实 ldflags stamped binary 返回 `{"data":{"version":"ep235-stamped-20260817"}}`。原始矩阵、盖章回执和最终产品证据均在 formal session `evidence/`。
- 五通道：backend 无应用级 WARN/ERROR/panic/FATAL；SSE messages/entities/notifications 三流连接并 clean EOF；frontend 只有正常 conductor/Dart VM 行，无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception；llmtap 真实指向 `https://api.anselm.website`，managed wiring 存在且本只读 slice 未伪造 completion。`rig-check`/`rig-down` 正常封口，owned processes/listeners 清零。
- 正式按 `G1 / F1 / A4 / C2 / G2` 写入 COVERAGE EP-235=`✓✓✓✓✓`，formal ledger `1865→1870 judgments`，`gen_coverage.py --check`=`848/367/0`，anchors=`10/10`。写账触发 `gap-too-fast` 与 `discovery-collapse`；独立复审=`/private/tmp/anselm-rig-formal-20260801-3/evidence/EP-235-ledger-alarm-reaudit.md`，两条均串行 ack，未改阈值/算法/法典/锚点/gate，最终 `alarms.py check`=`clean (1870 judgments)`。
- backend 定向 Go tests、stamped build matrix、focused Flutter settings `25/25`、串行 Dart analysis、measure diff、rig 自测 `42/42`、coverage、anchors、alarms 和 `git diff --check` 通过。并发 `flutter analyze` 曾触发 Flutter 工具自身 ephemeral 删除竞态，未计为通过；串行 `dart analyze` 明确 `No issues found`。无产品代码 stop-and-fix。批次三十五由 `25→30/50`，未满 50 格不跑统一长门禁、不提交；下一原子前线为 EP-236 GET /api/v1/system/data-dir。P12 的 400+ Journey 扩写按用户裁定推迟二期；EP-220 不删除、不重跑。

## 2026-08-17 10:17 · EP-234 health 正式五级收口，批次三十五 25/50

- EP-234 GET /api/v1/health 完成两套真实五通道 session：backend-first formal mirror=/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-100814-ep234，录屏 109.601667s；App-first 冷启动 formal mirror=/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-101051-ep234，录屏 24.296667s。第二套让 App 先于 backend 启动并延迟 2s，真实逐帧看到稳定 connecting gate，health 200 后进入 Advanced limits，无白屏、壳泄漏、文字跳变或半成品布局。
- 真实 tokenless server 矩阵：无 workspace GET health=200 {"data":{"status":"ok"}}，POST=405 METHOD_NOT_ALLOWED，坏 Host=403 FORBIDDEN_BAD_HOST。真实启用 ANSELM_AUTH_TOKEN 的 server：缺失或错误 bearer=401 UNAUTH_BAD_TOKEN，正确 bearer 在无 workspace 及带 workspace header 下=200。原始矩阵保存在 sessions/20260817-100814-ep234/evidence/EP-234-health-tokenless.txt 与 EP-234-health-bearer.txt。
- 五通道：backend 无应用级 WARN/ERROR/panic/FATAL，SSE messages/entities/notifications 三流均连接，frontend 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception 产品红线，llmtap wiring 存在但未伪造 completion；两场 rig-check/rig-down 通过且 owned processes 清零。measure diff 对启动采样帧仅报告 connecting 到 ready 一次状态替换 changedFrac=0.79167，没有中间漂移。
- 正式按 G1 / F1 / A4 / C2 / G2 写入 COVERAGE EP-234=✓✓✓✓✓，formal ledger 1860→1865 judgments，gen_coverage.py --check=848/366/0，anchors=10/10。写账触发 gap-too-fast 与 discovery-collapse；独立复审=/private/tmp/anselm-rig-formal-20260801-3/evidence/EP-234-ledger-alarm-reaudit.md，两条均按机制 ack，未改阈值/算法/法典/锚点/gate，最终 alarms.py check=clean (1865 judgments)。
- 本轮 backend bootstrap/router/middleware Go tests、token matrix、frame measure、rig 自测 42/42、coverage、anchors、alarms、docs/working 同步和 git diff --check 通过；无产品代码 stop-and-fix。批次三十五由 24→25/50，未满 50 格不跑统一长门禁、不提交；下一原子前线为 EP-235 GET /api/v1/version。P12 的 400+ Journey 扩写按用户裁定推迟二期；EP-220 不删除、不重跑。

## 2026-08-17 10:03 · EP-233 limits reset 正式五级收口，批次三十五 24/50

- 首轮真实 App session `/private/tmp/anselm-rig-ep233-codex-20260817-a/sessions/20260817-095019` 暴露产品红线：Reset all 确认框只说明机器级，不说明当前修改会被覆盖；真实点击取消保持 `31/11`，backend 没有 reset 请求。红截图 `EP-233-red-confirm-copy.jpeg` 保留，未计绿格。
- stop-and-fix 直接补齐 `resetAllWarning` 中英文文案；`_resetAll` 的 ApiException/transport failure 先重读服务端真相，再显示本地化可重试 danger notice。Fixture 增加 reset failure/call-count 钩子；Flutter 补取消零请求、失败回滚、warning copy 三组守卫。
- 修复后真实 session `/private/tmp/anselm-rig-ep233-codex-20260817-a/sessions/20260817-095436`（formal mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-095436-ep233`）录屏 `114.575000s`：从 `31/11` 打开，取消无 mutation；确认产生一次 `POST /api/v1/limits:reset=200` + 一次权威 GET，完整 UI 回到 `25/10`、`0.8` 和 Default()，modified rails 消失。三路 SSE、backend/frontend/LLM journals、录屏均由 conductor 封口。
- 同一 data dir fresh restart session `/private/tmp/anselm-rig-ep233-codex-20260817-a/sessions/20260817-095703`（formal mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-095703-ep233`）录屏 `51.300000s`，真实新 App/后端直接显示 `25/10` 默认值且无 modified rails；三场录屏合计 `382.485s`，backend 无应用级 WARN/ERROR/panic/FATAL，ssetap 三流连接并 clean EOF，llmtap readiness wiring 存在，未伪造 completion。
- 正式按 `G1 / F1 / A4 / C2 / G2` 写入 `COVERAGE EP-233=✓✓✓✓✓`，formal ledger `1855→1860 judgments`，`gen_coverage.py --check`=`848/365/0`，anchors=`10/10`。五格写账触发 `gap-too-fast` 与 `discovery-collapse`；复审证据=`sessions/20260817-095436-ep233/evidence/EP-233-ledger-alarm-reaudit.md`，两条均按机制 ack，未改阈值/算法/法典/锚点/gate，最终 `alarms.py check`=`clean (1860 judgments)`。
- Flutter `s5_storage_limits_test.dart` `16/16`、focused `dart analyze`、backend settings/limits atomic-reset tests、i18n gen、录屏完整性、SQLite integrity、三场 rig-check/rig-down、`make -C docs verify` 与 `git diff --check` 通过；docs verify 仍仅有仓内既有 6 条 review-due 提醒。批次三十五由 `19→24/50`，未到 50 格不跑统一长门禁、不提交；下一原子前线为 EP-234 `GET /api/v1/health`。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-17 09:40 · EP-232 limits patch 正式五级收口，批次三十五 19/50

- EP-232 覆盖局部 nested merge、热换、越界 400、未知字段拒绝、workspace 门、磁盘持久化和 App 重启恢复。真实 session `/private/tmp/anselm-rig-ep232-codex-20260817-a/sessions/20260817-093330` 先将 `agent.maxSteps` `25→31` 按 Return 提交，后端只记 `PATCH=200` + 权威 `GET=200`；三条直接负向 PATCH（`maxSteps=0`、`triggerRatio=1`、unknown field）为 `400 SETTINGS_LIMITS_INVALID`，缺 workspace 为 `401 UNAUTH_NO_WORKSPACE`，后续 GET 和 settings.json 仍保留 31。初次 AX 索引未实际聚焦、没有产生 mutation，未计产品证据；随后用 fresh screenshot 坐标重跑并封存。
- 同一 settings data dir 的独立重启 session `/private/tmp/anselm-rig-ep232-codex-20260817-b/sessions/20260817-093631` 真实渲染 31；再把 `invokeMaxTurns` `10→11` 后点击走提交，只有一次 `PATCH=200` + `GET=200`，两行蓝色 modified rail 与单行回退 affordance 可见，输入列稳定。formal mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-093330-ep232`，两场录屏分别 `2784x1808 / 60fps / 128.166667s / 7300 frames` 与 `160.478333s / 9203 frames`，均由 conductor 封口。
- 五通道封口：formal backend 无应用级 `WARN/ERROR/panic/FATAL/stack trace`；ssetap 三流均连接并 clean EOF；frontend 仅正常 Flutter VM 与已知 IMK 宿主噪声，无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception；llmtap managed proof/install/models readiness `200`，没有伪造 completion；SQLite integrity=`ok`。`rig-check`/`rig-down` 两场均通过，owned processes/listeners 清零。正式证据为 `sessions/20260817-093330-ep232/evidence/EP-232-limits-patch-green.md`。
- 锚点 `10/10`、写账前 alarms clean；`judge.py` 先按序写入 L1，再因 source session 不属于 formal `RIG_HOME/sessions` 正确拒绝 L2，镜像并修正 `manifest.session` 后继续写入 `F1/A4/C2/G2`。最终 EP-232 五格 `G1/F1/A4/C2/G2`，formal ledger `1850→1855 judgments`，COVERAGE=`848 rows / 364 carried / 0 tombstones`。写账触发 `gap-too-fast` 与 `discovery-collapse`；复审证据=`sessions/20260817-093330-ep232/evidence/EP-232-ledger-alarm-reaudit.md`，逐项 ack 后 `alarms.py check`=`clean (1855 judgments)`，未改阈值、法典、锚点或 gate。
- backend settings/limits/handlers Go tests、Flutter `s5_storage_limits_test.dart` `14/14`、focused Dart analysis、录屏 tests、SQLite integrity、`gen_coverage.py --check`、两场 rig-check/rig-down、docs verify 和 `git diff --check` 通过。docs verify 仍仅有仓内既有 6 条 review-due 提醒。批次三十五由 `14/50→19/50`；未到 50 格不跑统一长门禁、不提交。下一原子前线为 EP-233 `POST /api/v1/limits:reset`。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-17 09:30 · EP-231 limits schema 正式五级收口，批次三十五 14/50

- 首轮 EP-231 红证据证明 Advanced limits UI 只展示 desc/unit/default，忽略 live schema 的 `min/max/exclusive`；stop-and-fix 在 `limits_panel.dart` 增加 inclusive/open/unbounded range 与 default 提示，越界/非有限输入在构造 PATCH 前本地拒绝，恢复权威当前值并给本地化 danger notice。补充了 inclusive、unbounded、exclusive 上界、合法 PATCH、reset 与 invalid zero-PATCH Flutter 回归，并同步 Settings 文档。
- 正式 session mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-091858-ep231`，由同一 conductor 托管真实 Flutter macOS App、Computer Use、窗口录制、backend/frontend journal、三路独立 SSE witness、真实受管 gateway 和 llmtap；录屏 `2784x1808 / 60fps / 112.850000s / 6439 frames`。有效帧为 `EP-231-limits-top.png`、`EP-231-limits-invalid-final.png`、`EP-231-limits-exclusive-invalid.png`、`EP-231-limits-bottom.png`；此前跨暂停窗口存活的 14GB 旧 session 仅保留诊断，不计正式证据。
- 真实产品路径覆盖完整 17 行 schema、五组、顶部到底部滚动、`maxSteps=0` 下界负路径和 ratio=`1` 开区间上界负路径。前者恢复 `25` 并显示 `Enter a value within ≥ 1.`，后者恢复 `0.8` 并显示 `Enter a value within (0, 1).`；backend journal 记录 schema/limits `200` 且没有 PATCH。独立 REST witness 复核 17 行唯一值、5 组、ratio 开区间、query 稳定、缺失/错误 workspace `401 UNAUTH_NO_WORKSPACE`、POST `405 METHOD_NOT_ALLOWED`；canonical hashes 与五通道事实写入正式证据。
- 五通道封口：backend 无应用级 `WARN/ERROR/panic/FATAL/stack trace`；ssetap 三流均连接并 clean close；frontend 仅正常 Flutter VM 与已知 IMK 宿主噪声，无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception；llmtap managed proof/install/models 全部上游 `200`，本只读/本地拒绝路径没有伪造 completion。`rig-check`/`rig-down` 通过，owned processes/listeners 清零。
- anchor 旧状态超过四小时时先被 gate 正确拒绝，重新校准 `10/10` 后才写账。`judge.py` 按 first-unsettled 写入 EP-231 `G1/F1/A4/C2/G2` 五格，formal ledger `1845→1850 judgments`，COVERAGE=`848 rows / 363 carried / 0 tombstones`。写账触发 `gap-too-fast` 与 `discovery-collapse`；复审证据=`sessions/20260817-091858-ep231/evidence/EP-231-ledger-alarm-reaudit.md`，逐项 ack 后 `alarms.py check`=`clean (1850 judgments)`，未改阈值、法典、锚点或 gate。
- Flutter `s5_storage_limits_test.dart` 全 suite、`limits:` focused tests、focused `dart analyze`、`gen_coverage.py --check`、`git diff --check` 通过；working 文档同步后继续执行 docs verify，保留仓内既有 6 条 review-due 提醒。批次三十五由 `9/50→14/50`；未到 50 格不跑统一长门禁、不提交。下一原子前线为 EP-232 `PATCH /api/v1/limits`。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-14 18:14 · EP-230 limits 正式五级收口，批次三十五 9/50

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260814-180804-ep230`，由同一 conductor 托管真实 Flutter macOS App、Computer Use、窗口录制、backend/frontend journal、三路独立 SSE witness、真实受管 gateway 和 llmtap；录屏 `2784x1808 / 60fps / 169.313333s / 9613 frames`，稳定帧为 `evidence/EP-230-limits-top.png`、`evidence/EP-230-limits-lower.png` 与 `evidence/EP-230-limits-search.png`。
- 真实产品目的为理解本机当前运行边界。App 从 Settings → System → Advanced limits 打开，显示 This machine 与机器级全局语义；schema 驱动的 agent/context/timeout/tools/guards 五组 17 字段完整可见，顶部/底部滚动无 clipping、overlap、reflow；设置搜索真实键盘输入 `limits` 只返回 Advanced limits，点击返回后页面稳定。
- REST `GET /api/v1/limits=200` 返回完整 live limits，`GET /api/v1/limits/schema=200` 返回 17 行 metadata，UI 数字与线缆逐项一致；无 workspace/错误 workspace=`401 UNAUTH_NO_WORKSPACE`，POST=`405 METHOD_NOT_ALLOWED` 并保留 Allow；SQLite integrity check=`ok`。
- 五通道封口：backend `259` 行无应用级 WARN/ERROR/panic/FATAL/stack trace；ssetap 独立接通 notifications/entities/messages 三流并 clean close；frontend 仅正常 Flutter VM 与已知宿主噪声，无 Dart/Flutter/RenderFlex/overflow/Unhandled/Exception；llmtap managed proof/install/models/quota 全部上游 `200`，只读 Settings 路径没有伪造 completion。`rig-check`/`rig-down` 通过，owned processes/listeners 清零。
- `judge.py` 按 first-unsettled 写入 EP-230 `G1/F1/A4/C2/G2` 五格，formal ledger `1840→1845 judgments`，COVERAGE=`848 rows / 362 carried / 0 tombstones`。写账触发 `gap-too-fast` 与 `discovery-collapse`；复审证据=`sessions/20260814-180804-ep230/evidence/EP-230-ledger-alarm-reaudit.md`，逐项 ack 后 `alarms.py check`=`clean (1845 judgments)`，未改阈值、法典或锚点。
- 后端 settings/limits/handlers/bootstrap 定向 Go tests 全通过；Flutter 设置相关 suite `51/51` 全通过；`gen_coverage.py --check`、SQLite integrity、`make -C docs verify`、正式 `rig-check`/`rig-down` 与 `git diff --check` 通过。docs verify 仅有仓内既有 6 条 review-due 提醒。本轮按用户每 50 格统一门禁规则只推进到 `9/50`，不跑统一长门禁、不提交；下一原子前线为 EP-231。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-14 18:01 · EP-229 tools 正式五级收口，批次三十五 8/50

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260814-175343-ep229`，由同一 conductor 托管真实 Flutter macOS App、Computer Use、窗口录制、backend/frontend journal、三路独立 SSE witness、真实受管 gateway 和 llmtap；录屏 `2784x1808 / 60fps / 265.446667s`，稳定帧为 `evidence/frames/EP-229-tool-picker-top.png`、`EP-229-tool-picker-empty.png` 与 `EP-229-tool-picker-final.png`。
- 真实产品目的为在 Skill Properties 的 Allowed tools 中选择内置授权。App 打开 `commit-helper` → Add a tool，Builtin 目录显示名称与一行摘要；选择 `Bash` 时出现勾选，关闭后属性面板保留可移除 Bash 药丸。真实键盘输入 `zzzz-no-match`，候选源收敛后显示 no-match/literal-add 恢复提示，不残留无关 Function/Handler 候选。
- REST `GET /api/v1/tools=200` 返回 117 个仅含 `name/summary`、按名称稳定排序且摘要非空的 descriptor；无 workspace=`401 UNAUTH_NO_WORKSPACE`，POST=`405 METHOD_NOT_ALLOWED` + `Allow: GET, HEAD`，无关 `limit/cursor` query 不改变有界目录。Skill GET 与 `SKILL.md` 最终均为 `allowedTools: ["Bash"]`，SSE 收到对应 durable `skill.updated`，SQLite integrity check=`ok`。
- 五通道封口：backend journal 398 行无应用级 WARN/ERROR/panic/FATAL/stack trace；ssetap 三流均连接，Skill 更新 durable 帧到达，entities/messages 无本轮伪造 durable mutation；frontend 仅正常 Flutter VM、CapsLock 与已知 IMK 宿主噪声，无 Dart/Flutter/RenderFlex/overflow/Unhandled/Exception；llmtap managed proof/install/models 全部上游 `200`，本只读路径没有伪造 chat completion。`rig-check`/`rig-down` 通过，owned processes/listeners 清零。
- `judge.py` 按 first-unsettled 写入 EP-229 `G1/F1/A4/C2/G2` 五格，formal ledger `1835→1840 judgments`，COVERAGE=`848 rows / 361 carried / 0 tombstones`。写账触发 `gap-too-fast` 与 `discovery-collapse`；复审证据=`sessions/20260814-175343-ep229/evidence/EP-229-ledger-alarm-reaudit.md`，逐项 ack 后 `alarms.py check`=`clean (1840 judgments)`，未改阈值、法典或锚点。
- 定向 `tool/bootstrap/handlers` Go tests、Flutter Library/Skill picker `62/62`、`gen_coverage.py --check`、`make -C docs verify`、`git diff --check` 通过。按用户每 50 格统一门禁规则，本批次仍为 `8/50`，不跑统一长门禁、不提交；下一原子前线为 EP-230。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-14 17:45 · EP-228 catalog 正式五级收口，批次三十五 7/50

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260814-173754-ep228`，由同一 conductor 托管真实 Flutter macOS App、Computer Use、窗口录制、backend/frontend journal、三路独立 SSE witness、真实受管 gateway 和 llmtap；录屏 `2784x1808 / 60fps / 194.393333s`，最终帧为 `evidence/frames/EP-228-final-top.jpeg` 与 `EP-228-final-bottom.jpeg`。
- 真实产品目的为 Chat 中按类型回答当前能力目录。App 最终展示 Agent、Document、Function、Handler、Skill 五组，`order_desk` 的 `place/cancel` 方法清晰可见；REST `GET /api/v1/catalog=200`、无关 query 同结果，缺失/未知 workspace=`401 UNAUTH_NO_WORKSPACE`，POST=`405 METHOD_NOT_ALLOWED` + `Allow: GET, HEAD`。catalog summary、coverage、durable messages、Chat system prompt 能力段逐项一致，清单问题没有调用工具。
- 首轮真实观察冻结标题产品红：utility 只返回 reasoning、没有正文，旧 fallback 把长请求切成半个 `type i`。红图=`/private/tmp/anselm-rig-formal-20260801-4/sessions/20260814-172434/evidence/EP-228-auto-title-red.jpeg`；stop-and-fix 将本地 fallback 收紧为 60 rune、预留 `…`、英文词边界/无空格 rune 边界，并补 Go regression 与 chat contract 文档。最终 rail/header 均为 `What capabilities are currently available in this workspace…`，没有半词或永久 `New chat`。
- 五通道封口：backend 303 行无应用级 WARN/ERROR/panic/FATAL/stack trace；SSE 三流接通，messages durable `1..8`、notifications `16..17`，entities 只读无 durable mutation；frontend 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception 产品红线，仅已知 macOS IMK 噪声；llmtap managed proof/install/models、主 Chat 和 reasoning-only utility response 均上游 200，fallback 真实记录。`rig-check`/`rig-down` 通过，owned processes/listeners 清零。
- `judge.py` 按 first-unsettled 写入 EP-228 `G1/F1/A4/C2/H1` 五格，formal ledger `1830→1835 judgments`，COVERAGE=`848 rows / 360 carried / 0 tombstones`。写账触发 `gap-too-fast` 与 `discovery-collapse`；独立复审证据=`sessions/20260814-173754-ep228/evidence/EP-228-ledger-alarm-reaudit.md`，anchor `10/10`，两项逐一 ack 后 `alarms.py check`=`clean (1835 judgments)`，未改阈值、法典或锚点。
- 定向 Go tests、`gen_coverage.py --check`、`make -C docs verify`、`git diff --check` 通过。按用户每 50 格统一门禁规则，本批次仍为 `7/50`，不跑统一长门禁、不提交；下一原子前线为 EP-229。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-14 17:18 · EP-227 关系全图正式五级收口

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260814-171118`，真实 Flutter macOS App、真实受管 Anselm 网关、conductor 录屏、backend/frontend journal、三路 SSE witness 与 llmtap 均归属于同一台架；`screen.mov` 为 `2784x1808 / 60fps / 126.930000s`，正式证据=`sessions/20260814-171118/evidence/EP-227-relgraph-final-green-r1.md`，终帧=`EP-227-final-ui.png`。
- 真实产品路径从 Entities Overview 进入 Explore，打开 `Show provenance` 后由 `4/2` 变为 `6/3`；选中真实会话右岛完整展示长标题与 `CREATED → Function: ep227_provenance_probe`，选中 Function 反向展示 `CREATED BY → Conversation`；隐藏/恢复 Document 和 Fit to view 后拓扑、可读性与稳定性均正确。REST N1 envelope 复核真实 `create/link/equip` 三边、无关 query、缺失/未知 workspace `401 UNAUTH_NO_WORKSPACE`、错误方法 `405 METHOD_NOT_ALLOWED` + `Allow: GET, HEAD`。
- 本格三次真实 stop/fix 均保留红证据：模型第一次发 `kind` 而 parser 要求 `op`，修为显式 `op` schema、六值枚举和兼容边界；provenance 选中源会话曾为空 inspector，补齐 `createdBy/editedBy/created/edited` 双向投影、双语 UI 与测试；冷重建后长标题曾截断，改为右岛滚动面完整换行并重新逐帧通过。没有把旧红态从记录里抹掉。
- 五通道互证：backend 无应用级 `WARN/ERROR/panic/FATAL/stack trace`；三路 SSE 均连接并 clean close，最终只读图路径无 durable mutation；frontend 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception 产品红线，仅已知 `IMKCFRunLoopWakeUpReliable`；最终只读 llmtap 无需伪造 completion，构建 sessions `20260814-165150`、`20260814-165857` 保留 managed proof 与真实 Chat completion，响应全 `200`。`rig-check` 通过，`rig-down` 正常收台。
- anchor 重新校准 `10/10` 后，`judge.py` 按 first-unsettled 写入 EP-227 `G1/F1/A4/C4/G2`，五格 `✓✓✓✓✓`；formal ledger=`1830 judgments`，COVERAGE=`848 rows / 359 carried / 0 tombstones`。写账触发 `gap-too-fast` 与 `discovery-collapse`，按完整 stop/fix 链、录屏、REST 负路径、五通道和 anchors 独立复审后 ack；复审证据=`sessions/20260814-171118/evidence/EP-227-ledger-alarm-reaudit.md`，阈值/法典/锚点未改，`alarms.py check`=`clean (1830 judgments)`。
- 定向 Go tests、关系图/实体概览 focused Flutter tests、`flutter analyze`、Dart format、`gen_coverage.py --check` 均通过。批次三十五由 `5/50` 推进为 `6/50`；按用户规则未到 50 格不跑统一长门禁、不提交。下一原子前线为 EP-228；P12 的 400+ Journey 扩写按用户裁定推迟二期，EP-220 不删除、不重跑。
...
## 2026-08-14 16:36 · EP-226 relations neighborhood 正式五级收口

- EP-226 `GET /api/v1/relations/neighborhood` 在正式 session `/private/tmp/anselm-rig-formal-20260801-3/sessions/20260814-162648` 完成真实 Flutter macOS App、真实受管网关、Computer Use、backend journal、三路独立 SSE witness、frontend console 与 LLM tap 五通道验收。录屏已由 `rig-down` 封口，`ffprobe`=`2784x1808 / 60fps / 176.273333s`；最终画面为 `sessions/20260814-162648/evidence/EP-226-final-ui.png`。
- 产品路径先按名字找到 `EP226 Neighbour B`，再只调用一次 `get_relations`，最终用户可读地得到 `A → B → C`、B 的链接、引用 B 的 A 和一跳可达项。REST 正向覆盖默认/`depth=1`/`depth=2`，负向覆盖 `foo`、`1.5`、重复 query、越界 depth、缺 kind/id 和未知中心；非法 present query 均显式失败，不静默使用默认值。
- 本轮前置真实 session 暴露过中文/英文自然语言、bare exact-ID、name-parenthetical、JSON-like reasoning 和加粗 `文档 ID` 的跨 chunk 泄露；stop-and-fix 扩展 redactor 有界持有与自然语言替换，保留工具/审计面精确 ID，增加 loop 回归。最终 162648 assistant-facing reasoning/text close 扫描无内部 placeholder；旧红 session 全部保留为发现证据。
- 五通道交叉核对：backend 无 `WARN/ERROR/panic/FATAL/stack trace`；SSE 共 146 帧，三流均连接，messages durable `1..23`、notifications `1..2` 连续，entities 只读无 durable mutation；frontend 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception 产品红线，仅已知 IMK 平台噪声；LLM wire 16 条且 proof/bootstrap 与 4 次 chat completion 上游全 `200`。完整证据为 `sessions/20260814-162648/evidence/EP-226-relations-neighborhood-final-green-r1.md`。
- 依次执行 `gen_coverage.py --check`、锚点冻结答卷 `10/10`、`judge.py` 五级写账 `G1/F1/F4/C4/G2`；`COVERAGE EP-226=✓✓✓✓✓`，formal ledger `1820→1825 judgments`，清册 `848/357/0→848/358/0`（carried 是已裁决行数，不是格数）。写账后 `gap-too-fast` 与 `discovery-collapse` 打开，复审记录 `sessions/20260814-162648/evidence/EP-226-ledger-alarm-reaudit.md`，逐项 ack 后 `alarms.py check`=`clean (1825)`，未改阈值、法典或锚点。
- 定向 Go、关系图/实体概览 Flutter tests、Dart format、`git diff --check` 均通过。批次三十五由 `0→5/50`；未到 50 格不跑统一长门禁、不提交。下一原子前线为 EP-227 `GET /api/v1/relgraph`，P12 的 400+ Journey 扩写仍推迟二期。
## 2026-08-17 11:32 · EP-237 network 正式五级收口，批次三十五 40/50

- EP-237 `GET /api/v1/network` 完成真实 Flutter macOS App、真实受管 Anselm gateway、Computer Use、窗口录屏、backend/frontend journal、三路独立 SSE witness 和真实 llmtap；formal mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-112708-ep237`，录屏 `152.011667s / 2784x1808 / 60fps`。manifest 的 source→formal 身份错配先被 L2 gate 拒绝，修正 formal self-reference 后才继续写账。
- 真实产品路径从 Settings → System → Network 打开两次，中间返回 General；fresh AX/frame 看到 `This machine`、出站代理用途说明、HTTP/HTTPS/Bypass 三字段、空值直连占位、重启提示和无改动时惰性的 Save。真实 REST matrix：无 workspace=`401 UNAUTH_NO_WORKSPACE`；正确 workspace=`200 {"data":{}}`；未知 workspace=`401`；POST=`405 METHOD_NOT_ALLOWED` + `Allow: GET, HEAD, PATCH`；坏 Host=`403 FORBIDDEN_BAD_HOST`。本轮没有 PATCH 或其他 mutation，EP-238 写路径留待下一格。
- 首轮真实观察没有发现产品缺陷，不需要 stop-and-fix。稳定帧 `55/85/115s` 无 clipping、overlap、reflow、white flash 或按钮文字跳变，`measure diff` 无超阈值输出；空 response 没有被前端伪造成默认代理地址。
- 五通道封口：backend 无应用级 WARN/ERROR/panic/FATAL，ssetap 三流 clean close，frontend 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception 红线，llmtap readiness 指向 `https://api.anselm.website` 且没有伪造 completion；`rig-check`/`rig-down` 通过，owned processes/listeners 清零。正式证据为 `sessions/20260817-112708-ep237/evidence/EP-237-network-final-green.md`，警报复审为 `/private/tmp/anselm-rig-formal-20260801-3/evidence/EP-237-network-ledger-reaudit.md`。
- 正式按 `F1 / F2 / B2 / C2 / G1` 写入 `COVERAGE EP-237=✓✓✓✓✓`，formal ledger `1875→1880 judgments`，`gen_coverage.py --check=848/369/0`，anchors=`10/10`。写账触发 `gap-too-fast` 与 `discovery-collapse`；两条均依据同一封口 session、REST/AX/frame/五通道证据和未变锚点独立复审后 ack，未改阈值/算法/法典/锚点/gate，最终 `alarms.py check`=`clean (1880 judgments)`。
- Flutter `s5_network_test.dart` `1/1`、backend settings/handler Go tests、串行 Dart analysis、sealed recording/ffprobe、三帧 measure diff、rig 自测 `42/42`、coverage、anchors、alarms 与 `git diff --check` 通过。批次三十五由 `35→40/50`，未满 50 格不跑统一长门禁、不提交；下一原子前线为 EP-238。P12 的 400+ Journey 扩写按用户裁定推迟二期；EP-220 不删除、不重跑。
## 2026-08-17 12:18 · EP-238 network PATCH 正式五级收口，批次三十五 45/50

- EP-238 `PATCH /api/v1/network` 在 formal mirror=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260817-120647-ep238` 完成真实 Flutter macOS App、真实受管 Anselm gateway、Computer Use、窗口录屏、backend/frontend journal、三路独立 SSE witness 和 llmtap 五通道验收；`screen.mov` 为 `303.148333s / 2784x1808 / 60fps / 17086 frames`，manifest 已修正为 formal self-reference 后封口。
- 真实产品路径由用户在 action-time 确认网络敏感 Save 后完成：App 填入 HTTP/HTTPS `http://127.0.0.1:8790`、Bypass `localhost,127.0.0.1,::1`，真实点击 Save；backend journal 记录 UI `PATCH 200` 和紧随其后的 `GET 200`，录屏 `frames/network-save-40.0s.jpg` 有绿色 `Saved — fully effective after an engine re...` toast，返回页面后的最终稳定帧仍显示三项值，无 clipping、overlap、reflow、white flash 或 input jump。
- 独立 REST matrix 覆盖有效恢复、缺失 workspace `401 UNAUTH_NO_WORKSPACE`、POST `405 METHOD_NOT_ALLOWED` + `Allow: GET, HEAD, PATCH`、坏 Host `403 FORBIDDEN_BAD_HOST`、malformed JSON `400 INVALID_REQUEST`；`settings-final.json` 证明 limits/retention sibling block 未被网络整体替换抹掉。一次终端 `PATCH {}` 和随后恢复只用于隔离 fixture 控制，已在正式证据中披露，不冒充 UI 行为。
- 五通道封口：backend 无应用级 WARN/ERROR/panic/FATAL/stack trace；ssetap 三流连接并 clean close，settings-only 路径不虚构 durable 事件；frontend 仅正常 Dart VM/已知 macOS 输入法宿主噪声，无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception 红线；llmtap managed readiness 指向 `https://api.anselm.website` 且无伪造 completion；`rig-check`/`rig-down` 通过，owned processes/listeners 清零。`measure diff` 对七个 PNG 稳定帧在 `0.0005` 阈值无 changed-region 输出。
- `go test ./internal/app/settings ./internal/transport/httpapi/handlers`、Flutter `s5_network_test.dart` `1/1`、focused Dart analysis、rig 自测 `42/42`、SQLite integrity=`ok`、`gen_coverage.py --check`、anchors 和 `git diff --check` 全通过。`judge.py` 按 `F1/F2/B2/C2/G1` 写入 EP-238 五格，formal ledger `1880→1885 judgments`，COVERAGE=`848/370/0`；两条统计警报按独立复审 `sessions/20260817-120647-ep238/evidence/EP-238-ledger-alarm-reaudit.md` 串行 ack，最终 `alarms.py check`=`clean (1885 judgments)`，未改阈值/法典/锚点/gate。
- 批次三十五由 `40→45/50`；未满 50 格不跑统一长门禁、不提交。下一原子前线为 EP-239 `GET /api/v1/retention`。P12 的 400+ Journey 扩写按用户裁定推迟二期；EP-220 不删除、不重跑。

## 2026-08-18 21:13 · SURF-028 entities/tab-logs 正式五级收口，批次四十 5/50

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260818-205447`，隔离数据=`/private/tmp/anselm-data-surf028-20260818-r1`。真实 App 覆盖 Function、Handler、Agent 三种日志面：ok/failed 聚合、Load more、展开详情和当前会话新增执行均与 REST/SQLite 对账。最终计数为 Function `21 ok/5 failed`、Handler `6 ok/2 failed`、Agent `3 ok/0 failed`，SQLite `integrity_check=ok`。
- 首轮逐帧发现 Function 失败行把 stderr traceback 与业务日志重复展示；stop-and-fix 在 `backend/internal/app/function/sandbox_adapter.go` 接入 `splitFunctionStderr`，新增 `sandbox_adapter_test.go`，并同步 `docs/references/backend/domains/function.md`。修复后新失败行的 Error 只保留 traceback，Logs 只保留函数自身 print/debug；旧 durable 行不重写，迁移边界已在证据中明确。
- 五通道封口：录屏 `548.238333s / 2560x1584 / H.264 / 约57fps`，关键帧 `frame-210`、`frame-235`、`frame-500`；backend `634` 行无应用红线，frontend 无 Flutter/Dart/RenderFlex/Unhandled 应用红线，ssetap 三流 durable 序列单调并保留 `seq=0` delta，llmtap 本轮真实 completion 为 HTTP `200`。稳定 ROI `measure diff` 无异常输出，对比度 `17.40:1`，focused Function Go tests 与 entities Flutter `12/12` 通过。
- 完整证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260818-205447/evidence/SURF-028-entities-tab-logs-five-channel.md`；`judge.py` 按 `G1/F1/B2/C4/G1` 写入五格，formal ledger `2085→2090 judgments`，COVERAGE=`848/410/0→848/411/0`。写账后的 `gap-too-fast` 与 `discovery-collapse` 以 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-028-ledger-alarm-reaudit.md` 独立复审并 ack，anchors=`10/10`，最终 `alarms.py check`=`clean (2090 judgments)`；未改阈值、算法、法典、锚点或 gate。
- 批次四十由 `0→5/50`；未到 50 格不跑统一长门禁、不提交。下一原子前线为 `SURF-029 entities/tab-runs`。P12 的 400+ Journey 扩写按用户裁定推迟二期。

## 2026-08-19 01:23 · SURF-037 entities/rail-workflow 正式五级收口，批次四十 50/50

- 首轮真实 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-005343` 停在真实 UI 已显示 attention 的失败路径；根因为共享 fixture 将 approval 输入写成 `route.amount`，而 control 只提供可选路由标签。停线后修正 `testend/rig/seed_surf032.py` 为真实上游 `work.amount`/`work.message`，没有把 fixture 错误伪装成产品成功。
- 新数据目录和新 workspace 的正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-011009` 完成 Workflow rail inactive→active/listening、折叠→重开、Manual JSON trigger 和 settled terminal。右岛显示 `Done`、flowrun=`fr_0c9afd489615f725`、`last ok 164ms`，四个节点均 `completed`，无 alerts；录屏封口 `523.665000s / 2560x1584 / H.264 / 60fps`，最终帧=`sessions/20260819-011009/evidence/SURF-037-final.png`。
- 五通道交叉核对：backend `673` 行无应用 WARN/ERROR/panic/FATAL/stack trace；frontend `161` 行无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception 应用红线，AXTree 204/208/212 tooling churn 由 `frontend-ax-review.md` 明确复核且静置不增长；ssetap 三流连接，entities durable `7..10` 且 `run_started→run_terminal(completed)`，无 gap；llmtap managed proof/install/models 全 `200`；REST、SQLite `PRAGMA integrity_check=ok`、SSE terminal 与 UI 对账一致。正式证据=`sessions/20260819-011009/evidence/SURF-037-entities-rail-workflow-five-channel.md`。
- `judge.py` 依次写入 `G1/F1/B2/C4/G1`，formal ledger `2130→2135 judgments`，COVERAGE=`848 rows / 419→420 judged rows / 0 tombstones`；写账后的 `gap-too-fast` 与 `discovery-collapse` 以 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-037-ledger-alarm-reaudit.md` 独立复审并 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2135 judgments)`。
- 批次四十由 `45→50/50`，按用户规定解锁统一根门禁、完整 testend、工作树归属审计和提交；下一原子前线为 `SURF-038 entities/rail-control`。P12 的 400+ Journey 扩写继续按用户裁定推迟二期。

## 2026-08-19 01:49 · 批次四十统一长门禁完成

- 根 `make verify` 通过：backend、frontend、docs、demo 全绿；docs 仅保留仓内既有 6 条 review-due warning，不是本批次 drift。台架 Python `42/42`、coverage、anchors `10/10`、alarms clean、Python compile、Shell syntax、`git diff --check` 通过。
- backend 非缓存全量 `mise exec -- go test -count=1 -timeout 20m ./...` 通过；所有 app/domain/infra/transport/pkg 测试均绿。
- `make -C backend testend` 首次在约 `291.309s` 退出 `FAIL`，并行日志超过终端上限，具体失败行未能从该次输出 envelope 恢复；没有把首跑红抹掉。使用完全相同的 `-count=1 -parallel 16 -timeout 15m ./scenarios/...` 重新执行两次，JSON 日志分别为 `/private/tmp/anselm-testend-batch40-r2-1787074728.jsonl` 与 `/private/tmp/anselm-testend-batch40-r3-1787075070.jsonl`，两次 `testend_rc=0`、无 failed tests，场景汇总约 `265.6s`；第三次稳定复跑也通过。没有改源码、并发度、测试阈值或 gate 来“修绿”。
- 黑盒收台后 conductor-owned backend/App/tap/llama 进程与 formal 端口均为零，`/var/folders/.../anselm-testend` 仅剩 `0B` 空目录；本批次工作树归属审计待提交前最后核对。
- 批次四十满足用户规定的 50 格统一门禁条件；下一步是只暂存本批次归属文件、复核 staged diff 后提交，下一原子前线为 `SURF-038 entities/rail-control`。P12 的 400+ Journey 扩写继续按用户裁定推迟二期。

## 2026-08-19 02:16 · SURF-038 entities/rail-control 正式五级收口，批次四十一 5/50

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-020352`，隔离数据=`/private/tmp/anselm-data-surf038-20260819-r1`，workspace=`ws_52e3d3d1863100ee`。fixture `testend/rig/seed_surf038.py` 只经公共 API 幂等创建三个 Control：customer router（三输入分支）、review router（approve/inspect）和 default router（catch-all）。
- 真实 App + Computer Use 逐帧覆盖 Entities rail 的 Control 计数、搜索命中/清空、折叠/展开、三个实体选择、无匹配态、Overview 返回、Control Overview/Versions 往返和最终稳定帧。最终画面无 clipping、overlap、reflow、white flash、input jump、错误 CTA 或误导文案；没有发现产品缺陷，不需要 stop-and-fix。
- 五通道封口：录屏 `521.826667s / 2560x1584 / H.264 / 60fps`，最终帧=`sessions/20260819-020352/evidence/SURF-038-final.png`；backend `630` 行无应用 panic/fatal/error/warn/exception，HTTP `GET /api/v1/controls` 恰三项且 `hasMore=false`；frontend 仅正常启动、Dart VM 和已知 macOS 平台噪声，无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception；ssetap 三流连接，notifications durable `16..18` 逐条对应三个 `control.created`，无 gap；llmtap managed proof/install/models 全 `200`；SQLite `integrity_check=ok` 且 control/version rows 与 UI 逐字段对账。正式证据=`sessions/20260819-020352/evidence/SURF-038-entities-rail-control-five-channel.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger `2135→2140 judgments`，COVERAGE=`848 rows / 420→421 judged rows / 0 tombstones`，anchors=`10/10`。写账触发的 `gap-too-fast` 与 `discovery-collapse` 已以 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-038-ledger-alarm-reaudit.md` 独立复审并 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2140 judgments)`。
- `gen_coverage.py --check` 通过；批次四十一由 `0→5/50`，未到 50 格不跑统一长门禁、不提交；下一原子前线为 `SURF-039 entities/rail-approval`。P12 的 400+ Journey 扩写继续按用户裁定推迟二期，一期仍以 COVERAGE 为覆盖真相源。

## 2026-08-19 02:38 · SURF-039 entities/rail-approval 正式五级收口，批次四十一 10/50

- 首轮红 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-022010` 发现真实产品缺陷：Approval rail 搜索 `refund` 后只剩 `surf039_refund_gate`，中心详情仍停在被排除的 `surf039_note_gate`，用户的查询、列表和详情不一致。按 stop-and-fix 修复 `frontend/lib/features/entities/ui/entity_rail.dart` 的选择一致性，并新增 route-backed widget regression；没有绕过红帧直接判绿。
- 修复后二进制在新数据目录 `/private/tmp/anselm-data-surf039-20260819-r2`、新 workspace=`ws_cfb9b8395c117c21` 启动绿色 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-022641`。真实 App + Computer Use 逐帧覆盖三条 Approval 的不同 inputs/template/allowReason/timeout/timeoutBehavior、搜索排除与清空、空结果、详情、Versions、折叠/展开和最终稳定退款详情；绿色路径不再出现 stale detail。
- 五通道封口：录屏 `214.663333s / 2560x1584 / H.264 / 60fps`，最终帧=`sessions/20260819-022641/final-frame.png`；backend `369` 行无 WARN/ERROR/panic/fatal，frontend 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception 红线；ssetap 独立连接 notifications/messages/entities 三流，seed durable notifications `16..18` 严格递增且无 gap；llmtap managed proof/install/models 全 `200`；SQLite `PRAGMA integrity_check=ok`，三条 Approval/version rows 与 UI 对账一致。正式证据=`sessions/20260819-022641/evidence/SURF-039-entities-rail-approval-five-channel-green.md`。
- `judge.py` 写入五格 `G1/F1/B2/C4/G1`，formal ledger `2140→2145 judgments`，COVERAGE=`848 rows / 421→422 judged rows / 0 tombstones`，anchors=`10/10`。两轮 `gap-too-fast` 与 `discovery-collapse` 均以红/绿 session、五通道原始 journal、修复回归和未改锚点独立复审，记录=`/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-039-ledger-alarm-reaudit.md`，逐项 ack 后最终 `alarms.py check`=`clean (2145 judgments)`；阈值、算法、法典、锚点和 gate 未变。
- 定向 Flutter `entity_rail_test.dart` + `approval_overview_test.dart` 全部通过；`dart format`、`git diff --check`、`gen_coverage.py --check`、anchors `10/10` 与 rig/alarm checks 通过。批次四十一由 `5→10/50`，未满 50 格不跑统一长门禁、不提交；下一原子前线为 `SURF-040 entities/rail-trigger`。P12 的 400+ Journey 扩写继续按用户裁定推迟二期。

## 2026-08-19 02:59 · SURF-040 entities/rail-trigger 正式五级收口，批次四十一 15/50

- 正式 fixture=`testend/rig/seed_surf040.py`，只经公共 API 幂等创建四种 Trigger：`surf040_hot_cron` 连接 active workflow，`surf040_cold_webhook` 与 `surf040_cold_sensor` 保持 cold，`surf040_paused_fs` 保持 paused；数据目录=`/private/tmp/anselm-data-surf040-20260819-r1`，workspace=`ws_fbb258b6f3edf39a`，session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-024530`。
- 真实 App + Computer Use 逐帧覆盖四类 source badge、Listening/Idle/Paused、蓝点与 CTA 可用性、配置和 payload、Overview、`hot`/`cold`/`zzzz` 搜索、清空和最终稳定帧。点击 hot cron 的 Fire 后真实生成 activation=`tra_bc6fe650e5ee155e`、dispatch 和 flowrun=`fr_db91b4fc0799d77a`；UI 的 Dispatch 保持语义准确的 `run started`，REST/SQLite 显示同一 flowrun 最终 `completed`。没有发现产品/视觉 defect，不需要 stop-and-fix。
- 五通道封口：`screen.mov`=`286.338333s / 2560x1584 / H.264 / 60fps`，最终帧=`sessions/20260819-024530/final-frame.png`；backend journal 无应用 WARN/ERROR/panic/fatal/exception，frontend journal 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception 红线；ssetap notifications/entities/messages 三流均真实连接，durable notifications=`16..24`、entities=`7,8,9,10` 严格有序，messages 因本路径无 chat turn 无 durable frame；llmtap managed proof/install/models 全 `200` 且没有虚构 completion；SQLite `PRAGMA integrity_check=ok`，Trigger/Workflow/Activation/Firing/Flowrun 与 UI 对账，rig-check/rig-down/ffprobe 通过。正式证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-024530/evidence/SURF-040-entities-rail-trigger-five-channel-green.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger `2145→2150 judgments`，COVERAGE=`848 rows / 422→423 judged rows / 0 tombstones`，anchors=`10/10`。写账后 `gap-too-fast` 与 `discovery-collapse` 以 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-040-ledger-alarm-reaudit.md` 复审；复审确认批次写入速度触发统计信号，原始证据、阈值、算法、法典、锚点与 gate 未变。一次并行 `alarms.py ack` 文件重写造成 gap ack 丢失，先被 gate 检出，再串行复核、串行 ack，最终 `alarms.py check`=`clean (2150 judgments)`；这只改了操作方式，没有改任何产品判断。
- `python3 -m py_compile testend/rig/seed_surf040.py`、rig-check/rig-down、ffprobe、frontend/backend 红线扫描、anchors、`gen_coverage.py --check` 与 `git diff --check` 通过。批次四十一由 `10→15/50`，未满 50 格不跑统一长门禁、不提交；下一原子前线为 `SURF-041 entities/run-terminal`。P12 的 400+ Journey 扩写按用户裁定推迟二期，一期仍以 COVERAGE 为覆盖真相源。

## 2026-08-19 03:30 · SURF-041 entities/run-terminal 正式五级收口，批次四十一 20/50

- 正式 fixture=`testend/rig/seed_surf041.py`，全新数据目录=`/private/tmp/anselm-data-surf041-20260819-r1`，workspace=`ws_61fa03f0cc98ae73`，session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-031703`。真实 App + Computer Use 逐帧走 Handler source/Call、Agent Invoke、Workflow Webhook `body` 映射、Function action、Recent 和 Scheduler pinned run dossier；最终 flowrun=`fr_d521c9d0e7525126` 为 v4/completed。
- 首轮真实画面发现 Handler Code 卡把有 `label` 输入的 `inspect` 错画成 `def inspect(self):`。按 stop-and-fix 修改 `frontend/lib/features/entities/data/entity_format.dart`，让 source formatter 读取 `MethodSpec.inputs`，新增 `frontend/test/features/entities/data/entity_format_test.dart`，目标测试 `All tests passed!`。复跑后 Code 卡显示 `def inspect(self, label):`；真实鼠标拖选+键入 `ui-correct` 后 Handler API/SQLite/UI 三方一致。Workflow fixture 同步从扁平 `entry.*` 修正为真实 Webhook `entry.body.*`；旧失败 flowrun 历史保留，没有用重播后的绿行覆盖红行。
- 五通道封口：`screen.mov`=`665.585000s / 2560x1584 / H.264 / 60fps`；backend/frontend 无未解释应用红线；ssetap 三流真实连接，entities durable=`1..12`、notifications durable=`1`、messages 本路径无 durable chat frame，ephemeral `seq=0` 未推进游标；llmtap 记录 managed proof/bootstrap 与真实 Agent wire，未虚构 Function/Handler/Workflow completion；SQLite `PRAGMA integrity_check=ok`，API/SQLite/SSE/UI 对账一致。正式证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-031703/evidence/SURF-041-entities-run-terminal-five-channel.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger `2150→2155 judgments`，COVERAGE=`848 rows / 423→424 judged rows / 0 tombstones`，anchors=`10/10`。写账触发的 `gap-too-fast` 与 `discovery-collapse` 以 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-041-ledger-alarm-reaudit.md` 独立复审并串行 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2155 judgments)`。
- `rig-check`/`rig-down`、ffprobe、`python3 -m py_compile testend/rig/seed_surf041.py`、focused Flutter test、`gen_coverage.py --check`、anchors 和 `git diff --check` 通过。批次四十一由 `15→20/50`，未满 50 格不跑统一长门禁、不提交；下一原子前线为 `SURF-042 entities/workflow-editor-inspector`。P12 的 400+ Journey 扩写按用户裁定推迟二期，一期仍以 COVERAGE 为覆盖真相源。

## 2026-08-19 04:14 · SURF-042 entities/workflow-editor-inspector 正式五级收口，批次四十一 25/50

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-040751`，数据目录=`/private/tmp/anselm-data-surf042-20260819-r2`，workspace=`ws_2998f557f8cc1e2b`，workflow=`wf_9d05754c871a12c5`。重新构建真实 macOS App 后，Computer Use 逐帧覆盖 graph editor 空 inspector、8 节点/8 边画布、Action 的 Function/Handler/MCP 引用族、Handler target/method、input mapping、Retry、Max attempts、Save/Discard、右岛关闭/重开。
- 首轮停线发现 MCP 无 server 时目标下拉打开为空白浮层，判为产品可解释性缺陷。stop-and-fix：`frontend/lib/core/ui/an_dropdown.dart` 增加空选项契约——无选项且无 `emptyLabel` 时不可打开，声明空态时显示不可选说明行；`frontend/lib/features/entities/ui/detail/node_ref_picker.dart` 为 MCP server/tool、Handler method 与通用 target 接入双语空态；补 `an_dropdown_test.dart` 与 `node_ref_picker_test.dart`。focused Flutter suite=`31/31`，dart format 通过。
- 真实复跑确认 MCP 目标菜单显示 `No MCP servers configured yet`，标准菜单壳无空白；`set_value` 只改语义文本、未触发 working graph 脏态，被 stop-and-fix 复验拒绝。真实键盘输入 Max attempts=`5` 后出现 `Unsaved changes`，按用户已确认的 Save 点击后出现 `New version saved`，字段稳定显示 `5`。关键帧=`SURF-042-mcp-empty-state.jpeg`、`SURF-042-retry-saved.jpeg`。
- 五通道封口：`screen.mov`=`236.961667s / 2560x1584 / H.264 / 60fps`；backend/frontend 无应用红线；backend 真实记录 `POST /api/v1/workflows/wf_9d05754c871a12c5:edit status 200`；REST active version=`wfv_76f014eeabf70d23`/v3，`functionAction.retry.maxAttempts=5`，capability-check=`structurallyValid=true,resolved=true`；ssetap 三流真实连接，notifications durable `workflow.edited` version 3，messages 本路径无聊天 durable frame；llmtap 仅记录 ready，不虚构本路径 LLM completion。正式证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-040751/evidence/SURF-042-entities-workflow-editor-inspector-five-channel.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/E4`，formal ledger `2155→2160 judgments`，COVERAGE=`848 rows / 424→425 judged rows / 0 tombstones`，anchors=`10/10`。写账触发的 `gap-too-fast` 与 `discovery-collapse` 以 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-042-ledger-alarm-reaudit.md` 独立复审并串行 ack；未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2160 judgments)`。
- `rig-check`/`rig-down`、ffprobe、fixture `py_compile`、focused Flutter、`gen_coverage.py --check`、anchors 和 `git diff --check` 通过。批次四十一由 `20→25/50`，未满 50 格不跑统一长门禁、不提交；下一原子前线为 `SURF-043 entities/workflow-editor-edges`。P12 的 400+ Journey 扩写继续按用户裁定推迟二期，一期仍以 COVERAGE 为覆盖真相源。

## 2026-08-19 04:26 · SURF-043 entities/graph-entity-card 正式五级收口，批次四十一 30/50

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-042026`，隔离数据=`/private/tmp/anselm-data-surf043-20260819-r1`，workspace=`ws_ebf2131f32f3925d`。fixture=`testend/rig/seed_surf043.py` 只经公共 API 创建 Function、Agent、Trigger 与 deactivated Workflow；Function=`fn_2de15c2141055103`，真实被 Agent 和 Workflow 两条 `equip` 边引用。
- 真实 App + Computer Use 逐帧覆盖 Overview relationship graph preview、full graph explore、Function 右岛实体卡、关系 pill 飞行切换、Workflow `EQUIPS` 卡和 `Open in detail` 到 Function 详情。卡片显示 kind glyph、完整名称、`v1`、描述、`REFERENCED BY` 两个 hydrated pills；详情页显示相同 name/version/description/code/interface/env ready。未发现 stale inspector、空白卡、白闪、裁切、重排或 input jump，无 stop-and-fix defect。
- 五通道封口：录屏=`167.395000s / 2560x1584 / H.264 / 60fps`；backend 无应用 WARN/ERROR/panic/fatal，REST relgraph 的 8 nodes/5 edges、Function detail 和 capability-check=`structurallyValid=true,resolved=true` 与 UI 对账；ssetap 三流连接，notifications durable=`16..21`、entities durable=`7,8`、中间 delta=`seq=0`，messages 因本路径无 chat turn 无 durable frame；frontend 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception 红线；llmtap managed proof/install/models 全 `200`，deterministic read path 无 completion 不虚构。正式证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-042026/evidence/SURF-043-entities-graph-entity-card-five-channel.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger `2160→2165 judgments`，COVERAGE=`848 rows / 425→426 judged rows / 0 tombstones`，anchors=`10/10`。写账触发的 `gap-too-fast` 与 `discovery-collapse` 记录在 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-043-ledger-alarm-reaudit.md`；复审重校 anchors、重读完整封口 session 与五通道原始 journal 后串行 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2165 judgments)`。
- `python3 -m py_compile testend/rig/seed_surf043.py`、`rig-check`/`rig-down`、ffprobe、`gen_coverage.py --check` 和 `git diff --check` 通过。批次四十一由 `25→30/50`，未到 50 格不跑统一长门禁、不提交；下一原子前线为 `SURF-044 library/draft`。P12 的 400+ Journey 扩写按用户裁定推迟二期，一期仍以 COVERAGE 为覆盖真相源。

## 2026-08-19 04:41 · SURF-044 library/draft 正式五级收口，批次四十一 35/50

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-043243`，隔离数据=`/private/tmp/anselm-data-surf044-20260819-r1`，workspace=`ws_867b0cc89d411afc`。无选区进入 Library 的真实 App 首帧显示四个空态引导；点击正文只出现 caret，不创建文档。真实键入 `SURF-044 draft body` 后恰有一次 `POST /api/v1/documents`，文档=`doc_ba3237feabbeeb4b`，先 adopt 再导航；树刷新后 `Untitled` 行唯一选中，右岛显示 `17 chars / 19 B /Untitled`。再真实键入 `!`，正文变为 `SURF-044 draft body!`，右岛为 `18 chars / 20 B`，没有 remount、丢字或焦点跳变。
- 五通道封口：`screen.mov`=`378.571667s / 2560x1584 / H.264 / 60fps`；backend journal 无 WARN/ERROR/panic/FATAL/exception，frontend journal 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception；ssetap 三流连接，notifications durable=`16..17`，messages/entities 因本路径无 chat/entity mutation 无 durable frame，LLM tap managed proof/install/models 六个状态全 `200` 且无 completion 伪声明；REST/SQLite `PRAGMA integrity_check=ok` 与 UI 对账一致。原始 Computer Use 帧=`evidence/SURF-044-draft-empty.jpg`、`SURF-044-adopted.jpg`、`SURF-044-final.jpg`，录屏末帧=`evidence/SURF-044-video-final.jpg`，测量归一帧 adopted→final=`changedFrac 0.00099`、box `(495,67)-(1215,243)`。正式证据=`evidence/SURF-044-library-draft-five-channel.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger `2165→2170 judgments`，COVERAGE=`848 rows / 426→427 judged rows / 0 tombstones`。L2 在录像尚未收台时曾被 gate 正确拒绝；`rig-down.sh` 封口后 L2-L5 通过。写账触发的 `gap-too-fast`/`discovery-collapse` 由 `evidence/SURF-044-ledger-alarm-reaudit.md` 独立复审，anchors=`10/10`，两条警报串行 ack 后 `alarms.py check`=`clean (2170 judgments)`，未改阈值、算法、法典、锚点或 gate。
- `rig-check`/`rig-down`、ffprobe、SSE/LLM/backend/frontend/SQLite 审计、`gen_coverage.py --check` 与 `git diff --check` 通过。无产品 defect，不需要 stop-and-fix；批次四十一由 `30→35/50`，未到 50 格不跑统一长门禁、不提交；下一原子前线为 `SURF-045 library/document`。P12 的 400+ Journey 扩写按用户裁定推迟二期，一期仍以 COVERAGE 为覆盖真相源。

## 2026-08-19 05:03 · SURF-045 library/document 正式五级收口，批次四十一 40/50

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-045509`，隔离数据=`/private/tmp/anselm-data-surf045-20260819-r2`，workspace=`ws_be21c75959fb9fd0`；fixture=`testend/rig/seed_surf045.py` 通过公共 API 幂等构造 18 个标题、3083 字符长文档。真实 App + managed gateway + Computer Use 逐帧覆盖 Library 文档树、长文档头部/正文/Outline 同滚页、Outline 跳转、连续滚动到底和末尾边界。
- 首轮点击 `5. Review` 后标题落入固定 shell scrim，判为产品级遮挡缺陷并停线。stop-and-fix 修改 `frontend/lib/features/library/ui/an_document_editor.dart`：Outline 跳转目标扣除固定 head band 与呼吸间距，新增 `headingScrollTarget` regression；focused Flutter `library_test.dart`=`56/56`。重建真实 App 后标题完整落在 scrim 下方，active outline 在继续滚动后正确跟随，底部无白缝、裁切、重叠或未解释跳变。
- 五通道封口：`screen.mov`=`102.518333s / 2560x1584 / H.264 / 60fps`；backend/frontend 红线为空，REST/SQLite 字段对账且 `PRAGMA integrity_check=ok`；ssetap 三流真实连接，notifications durable=`16` 对应 `document.created`，messages/entities 本路径无对应 durable frame；llmtap managed proof/install/models 全 `200`，无 completion 伪声明；正式证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-045509/evidence/SURF-045-library-document-five-channel.md`，测量与原始 Computer Use 帧均保留。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger `2170→2175 judgments`，COVERAGE=`848 rows / 427→428 judged rows / 0 tombstones`，anchors=`10/10`。写账触发的 `gap-too-fast`/`discovery-collapse` 由 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-045-ledger-alarm-reaudit.md` 独立复审并串行 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2175 judgments)`。
- `rig-check`/`rig-down`、ffprobe、fixture `py_compile`、focused Flutter、`gen_coverage.py --check` 与 `git diff --check` 通过。批次四十一由 `35→40/50`，未到 50 格不跑统一长门禁、不提交；下一原子前线为 `SURF-046 library/skill-manifest`。P12 的 400+ Journey 扩写按用户裁定推迟二期，一期仍以 COVERAGE 为覆盖真相源。

## 2026-08-19 05:27 · SURF-046 library/skill-manifest 正式五级收口，批次四十一 41/50

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-051820`，全新数据目录=`/private/tmp/anselm-data-surf046-20260819-r2`，workspace=`ws_19d5f0d92fc7d646`。fixture=`testend/rig/seed_surf046.py` 通过公共 API 创建 manifest、references 和 scripts；真实 App + managed gateway + Computer Use 逐帧覆盖 raw source 编辑、用户确认后的 Save、source 预览和 rich view/Outline 回读。
- 首轮真实 Save 暴露产品缺陷：`SKILL.md` 的 `PUT` 已成功，但 mounted source preview 仍读旧 `skillFileTextProvider` 缓存。stop-and-fix 在 durable write 后 invalidate 当前文件文本 provider，并新增 `library_test.dart` 回归。修复后二进制显示 raw source=`33` 行、`Evidence note` 可见，rich view Outline=`5` 项；focused Flutter=`58/58`，dart format、fixture compile、`git diff --check` 通过。
- 五通道封口：录屏=`254.645000s`；backend PUT=`204`、后续 GET=`200`、journal 无应用红线；SQLite `integrity_check=ok`，REST/文件系统/UI 对账；ssetap 三流连接，最终 notifications durable=`seq=19 skill.updated/SKILL.md`，本路径无聊天/实体业务 frame 不虚构；frontend 无 Flutter/Dart/RenderFlex/Unhandled/Exception 红线；llmtap managed proof/install/models 全 `200`，无 completion 伪声明。正式证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-051820/evidence/SURF-046-library-skill-manifest-five-channel.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger `2175→2180 judgments`，COVERAGE=`848 rows / 428→429 judged rows / 0 tombstones`，anchors=`10/10`。警报复审=`/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-046-ledger-alarm-reaudit.md`，`gap-too-fast`/`discovery-collapse` 按原阈值独立复审并串行 ack，最终 `alarms.py check`=`clean (2180 judgments)`，未改阈值、算法、法典、锚点或 gate。批次四十一由 `40→41/50`，未满 50 格不跑统一长门禁、不提交；下一原子前线为 `SURF-047`。P12 的 400+ Journey 扩写按用户裁定推迟二期，一期仍以 COVERAGE 为覆盖真相源。

## 2026-08-19 05:48 · SURF-047 正式五级收口，Skill 文件预览族布局 stop-and-fix 后通过，批次四十一 42/50

- `SURF-047 library/skill-file-preview` 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-053819`，隔离数据=`/private/tmp/anselm-data-surf047-20260819-r2`，workspace=`ws_34aee3e308220323`；fixture=`testend/rig/seed_surf047.py` 通过公共 API 幂等构造 Markdown、Python、PNG、SVG、CSV、字体和未知/二进制文件共 8 个路径。真实 App + managed gateway + Computer Use 逐帧覆盖全部预览分派、Source/Preview 切换，以及 `Open with system` / `Reveal in Finder` 逃生口。
- 首轮真实点击 PNG 发现 `AnPage` 纵向滚动体内 `Flexible` 的无界高度 RenderFlex 缺陷，红证据保留于 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-053242`。stop-and-fix 将图片/SVG 预览改为有界媒体框，并新增 `skill_tree_preview_test.dart` 回归；绿色 session 重新覆盖所有分支，未再出现空白预览、路径丢失、source/preview stale、逃生口异常或 Flutter runtime 红线。
- 五通道封口：录屏=`304.250000s / 2560x1584 / H.264 / 60fps`；backend=`407` 行且状态仅 `200/201/204`，无 WARN/ERROR/panic/fatal，SQLite `integrity_check=ok`；ssetap 三流连接，notifications durable=`16..23`，messages/entities 本路径无 chat/entity durable frame 不虚构；frontend journal 无 Flutter/Dart/RenderFlex/overflow/Unhandled/Exception；llmtap managed proof/install/models 全 `200`，无本路径 completion 不虚构。正式证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-053819/evidence/SURF-047-library-skill-file-preview-five-channel.md`，关键帧在同目录 `evidence/`。
- `mise exec -- flutter test test/features/library/library_test.dart test/features/library/skill_tree_preview_test.dart`=`62/62`，fixture `py_compile`、rig-check/rig-down、ffprobe、SQLite 审计、`gen_coverage.py --check` 通过。`judge.py` 串行按 `G1/F1/B2/C4/G1` 写入，formal ledger=`2180→2185 judgments`，COVERAGE=`848 rows / 429→430 judged rows / 0 tombstones`，anchors=`10/10`。`gap-too-fast`/`discovery-collapse` 由 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-047-ledger-alarm-reaudit.md` 独立复审并串行 ack，最终 `alarms.py check`=`clean (2185 judgments)`，未改阈值、算法、法典、锚点或 gate。批次四十一由 `41→42/50`，未到 50 格不跑统一长门禁、不提交；下一原子前线为 `SURF-048`。P12 的 400+ Journey 按用户裁定推迟二期，一期仍以 COVERAGE 为覆盖真相源。

## 2026-08-19 06:25 · SURF-048 library/rail-documents 正式五级收口，批次四十一 43/50

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-061413`，隔离数据=`/private/tmp/anselm-data-surf048-20260819-r2`，workspace=`ws_c876681cc3c2d2dc`。fixture=`testend/rig/seed_surf048.py` 通过公共 API 幂等构造空/已写根页、递归子树、空叶和拖拽目标；真实 App + managed gateway + Computer Use 逐帧完成 Documents rail 的子页创建、用户确认 `Save`、重命名、单页复制、整树 deep duplicate、hover `[+]`/`[⋯]`、拖拽入树、同级重排、空/已写 icon 和带确认的子树删除。
- 首轮红场发现的是 Computer Use 的全选键盘序列误差：自动输入造成标题观察偏差，后端立即核出并没有被当成产品绿；用真实焦点、`ctrl+a` + `shift+End` 重做后，绿色 UI、detail path 和 REST 均收敛为精确 `SURF-048 Created Child`。没有新的产品 defect 需要 stop-and-fix。
- 真实最终 REST 清册恰有 8 行，复制出的 `SURF-048 Library Root 2` 及其全部后代已删除且无残留；父子关系、materialized path 与 position 对账一致，根下顺序为 `Child Note=0`、`Created Child 2=1`、`Empty Leaf=2`、`Created Child=3`、`Reorder Me=4`。深复制期间确认新 identity 与已写内容随树复制；删除确认后中心落到有效空白草稿，没有死详情页。
- 五通道封口：录屏=`452.713333s / 2560x1584 / H.264`，最终帧=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-061413/evidence/SURF-048-library-rail-documents-final.png`；backend=`584` 行无 WARN/ERROR/panic/fatal，SQLite `integrity_check=ok` 且 foreign-key check 为空；ssetap 三流真实连接，notifications durable=`1..13` 严格递增，messages/entities 本路径无对应业务 mutation 不虚构；frontend 无 Dart/Flutter/RenderFlex/overflow/Unhandled/Exception 红线，134 条精确 AXTree stale-node 由同 session `evidence/frontend-ax-review.md` 逐场审阅，未知 AX pattern 仍硬失败；llmtap managed proof/install/models 全 `200`，本确定性 Library 路径无 completion 不虚构。`rig-check`、`rig-down`、ffprobe、fixture compile、focused Flutter `62/62` 通过。正式证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-061413/evidence/SURF-048-library-rail-documents-five-channel.md`。
- `judge.py` 串行写入五格 `G1/F1/B2/C4/G1`，formal ledger=`2185→2190 judgments`，COVERAGE=`848 rows / 430→431 judged rows / 0 tombstones`，anchors=`10/10`。`gap-too-fast`/`discovery-collapse` 由 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-048-ledger-alarm-reaudit.md` 独立复审并串行 ack，确认是集中账本写入的统计信号；未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2190 judgments)`。批次四十一由 `42→43/50`，未到 50 格不跑统一长门禁、不提交；下一原子前线为 `SURF-049`。P12 的 400+ Journey 按用户裁定推迟二期，一期仍以 COVERAGE 为覆盖真相源。

## 2026-08-19 06:39 · SURF-049 library/rail-skills 正式五级收口，批次四十一 44/50

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-062938`，隔离数据=`/private/tmp/anselm-data-surf049-20260819-r1`，workspace=`ws_099053bde6cf3739`；fixture=`testend/rig/seed_surf049.py` 通过公共 API 幂等创建一个与 Skill 同名的 Document 和两个 user Skill。真实 App + managed gateway + Computer Use 逐帧覆盖 Documents/Skills 分区、同名文档/技能防撞、技能扁平无拖拽、技能详情与过滤/清空恢复。
- 同名 `surf049-alpha` 真实点击分别进入 document path `/surf049-alpha` 与 skill route `/library/skill/surf049-alpha`；`surf049-beta` 保持 flat/no-drag，输入 `beta` 只保留 Skills head 与 beta 行，使用真实键盘 `BackSpace` 清空后完整列表恢复，中心详情未漂移。没有产品 defect，不需要 stop-and-fix。
- 五通道封口：录屏=`211.728333s`，四个关键帧保留在 session evidence；backend=`302` 行无 WARN/ERROR/panic/fatal，REST/SQLite identity 与 UI 对账，integrity=`ok`；ssetap 三流真实连接，notifications durable=`1..3` 严格递增，messages/entities 本确定性 Library 路径无业务 durable frame 不虚构；frontend 无 Dart/Flutter/RenderFlex/RenderBox/Unhandled/Exception/AX 红线；llmtap managed proof/install/models 全 `200`，无 completion 伪声明。`rig-check`/`rig-down`、ffprobe、fixture `py_compile`、focused Library/skill-preview=`62/62` 通过。正式证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-062938/evidence/SURF-049-library-rail-skills-five-channel.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2190→2195 judgments`，COVERAGE=`848 rows / 431→432 judged rows / 0 tombstones`，anchors=`10/10`。写账后的 `gap-too-fast`/`discovery-collapse` 由 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-049-ledger-alarm-reaudit.md` 独立复审并串行 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2195 judgments)`。`gen_coverage.py --check` clean；批次四十一由 `43→44/50`，未满 50 格不跑统一长门禁、不提交；下一原子前线为 `SURF-050`。P12 的 400+ Journey 按用户裁定推迟二期，一期仍以 COVERAGE 为覆盖真相源。

## 2026-08-19 06:51 · SURF-050 library/inspector-doc 正式五级收口，批次四十一 45/50

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-064339`，隔离数据=`/private/tmp/anselm-data-surf050-20260819-r1`，workspace=`ws_79a21da22bcaa327`；fixture=`testend/rig/seed_surf050.py` 通过公共 API 幂等创建目标文档、三层标题与两个 backlink。真实 App + managed gateway + Computer Use 逐帧覆盖文档 inspector 身份头、glance、Outline/Properties/Backlinks 三折叠组、Expand all/Collapse all、组级折叠、backlink 跳转、关闭/重开和稳定终帧，无产品或视觉 defect。
- 五通道封口：录屏=`291.300000s`，关键帧=`SURF-050-inspector-initial.jpeg`、`SURF-050-inspector-final.jpeg`；backend=`391` 行无 WARN/ERROR/panic/fatal/exception，SQLite `integrity_check=ok` 且 foreign-key check 为空；ssetap 三流真实连接，notifications durable=`1..3` 严格递增，messages/entities 本路径无业务 durable frame 不虚构；frontend journal 仅正常 App/平台/VM 启动，无 Flutter/Dart/RenderFlex/RenderBox/Unhandled/Exception/AX 红线；llmtap managed proof/install/models 全 `200`，无 completion 伪声明。REST/SQLite 与 UI 逐字段对账，rig-check/rig-down、ffprobe、fixture compile、focused Library/skill-preview=`62/62` 通过。正式证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-064339/evidence/SURF-050-library-inspector-doc-five-channel.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2195→2200 judgments`，COVERAGE=`848 rows / 432→433 judged rows / 0 tombstones`，anchors=`10/10`。写账后的 `gap-too-fast` 与 `discovery-collapse` 由 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-050-ledger-alarm-reaudit.md` 独立复审并串行 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2200 judgments)`；`gen_coverage.py --check` clean；批次四十一由 `44→45/50`，未到 50 格不跑统一长门禁、不提交；下一原子前线为 `SURF-051`。P12 的 400+ Journey 按用户裁定推迟二期，一期仍以 COVERAGE 为覆盖真相源。

## 2026-08-19 07:22 · SURF-051 library/inspector-skill 正式五级收口，批次四十一 46/50

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-071515`，隔离数据=`/private/tmp/anselm-data-surf051-20260819-r1`，workspace=`ws_2c7c72b9a7052756`；fixture=`testend/rig/seed_surf051.py` 通过公共 API 构造带 manifest、references、script、template 的 installed skill，并绑定一个 function 和一个 handler。真实 Flutter macOS App + managed gateway + Computer Use 逐帧覆盖文件树/绑定、Properties 表单、allowed-tools picker、Arguments、Model can invoke/User-invocable、Provenance、Outline、More actions、manifest 编辑后的用户确认 `Save` 和稳定终帧；最终显示 `4 files · 2 bindings`、`Pre-approval active`。
- 首轮更新候选故意带保留 `.anselm-install.json`，真实暴露旧后端 destructive-update 缺陷：先擦除 installed skill，后在写保留文件时失败，导致 provenance 消失。该红路径保留、不计绿。stop-and-fix 在 `backend/internal/app/skill/install.go` 的 destructive land 前增加 reserved sidecar 校验，并新增 `TestUpdateInstalled_InvalidSourcePreservesInstallation`；前端 `library_inspector.dart` 透传后端错误原因，双语新增 `skillUpdateFailedWithReason`。修复后的真实顺序为 good update → invalid candidate（UI=`Update failed: invalid skill file path`，原文件/绑定/provenance 保持）→ good update（UI=`Updated to the upstream version`）→ 重新批准预授权工具。Go skill package 与 focused Flutter Library/preview=`62/62` 通过。
- 五通道封口：`screen.mov`=`308.643333s / 2560x1584`，最终帧=`evidence/SURF-051-inspector-final.jpeg`；backend journal=`317` 行无 WARN/ERROR/panic/fatal/exception，REST 返回 `source=installed`、`provenance.toolsApproved=true`、4 个文件和精确 frontmatter，SQLite `PRAGMA integrity_check=ok` 且 foreign-key check 为空；equip 关系恰有 handler/function 两条；ssetap notifications/messages/entities 三流真实连接，notifications durable `seq=1,2,3` 严格递增；frontend journal 仅正常 App/平台/VM 启动，无 Flutter/Dart/RenderFlex/RenderBox/Unhandled/Exception 红线；llmtap readiness 指向真实 `https://api.anselm.website`，managed wiring 由 `rig-check` 验证，无 completion 伪声明。正式证据=`evidence/SURF-051-library-inspector-five-channel.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2200→2205 judgments`，COVERAGE=`848 rows / 433→434 judged rows / 0 tombstones`，anchors=`10/10`。写账触发的 `gap-too-fast`/`discovery-collapse` 依据同一封存 session、红绿 stop-and-fix 证据、五通道 journal 和未变 anchors 独立复审并串行 ack，未改阈值、算法、法典、锚点或 gate；`alarms.py check`=`clean (2205 judgments)`，`gen_coverage.py --check`=`848 rows / 434 carried judgments / 0 tombstones`。批次四十一由 `45→46/50`，未到 50 格不跑统一长门禁、不提交；下一原子前线为 `SURF-052`。P12 的 400+ Journey 按用户裁定推迟二期，一期仍以 COVERAGE 为覆盖真相源。警报复审=`/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-051-ledger-alarm-reaudit.md`。

## 2026-08-19 08:01 · SURF-052 scheduler/overview 正式五级收口，批次四十一 47/50

- 正式 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-074544`，隔离数据=`/private/tmp/anselm-data-surf052-20260819-r2`，workspace=`ws_373809e601b29e62`；fixture=`testend/rig/seed_surf052.py` 通过公共 API 构造 healthy、failed、waiting/approval、running、never-ran 和 inactive 六类 workflow lane，并配套失败、完成、审批和慢执行 run。真实 Flutter macOS App + managed gateway + Computer Use 逐帧覆盖 Scheduler Overview 的 KPI/时间线、approval、失败 peek、Graph/Open/full run detail/return、空 workspace 首用教育卡和回切 populated workspace；临时空 workspace 已通过 App 创建、验证后删除。
- 首轮真实 failure peek 在 session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-073545` 暴露用户错误卡泄露 sandbox 绝对路径，判为产品/隐私缺陷并停线。stop-and-fix 修改 `scheduler_run_model.dart` 的 display projection，及 `run_peek_card.dart`、`scheduler_run.dart`、`scheduler_run_inspector.dart` 的用户错误面，新增路径脱敏 regression；focused Flutter=`43/43`。重建 App 后错误卡只显示 `File "main.py"`，原始 journal/API 保留完整 traceback 供诊断。
- 五通道封口：录屏=`516.450000s / 2560x1584 / H.264 / 60fps`；backend=`602` 行无应用 panic/fatal/WARN/ERROR/Flutter/RenderFlex/RenderBox/Unhandled/Exception 红线，settle 后无 running flowrun 且六个 fixture workflow 均 inactive；ssetap 三流真实连接两个 workspace，populated workspace entities durable=`1,2,3` 严格对应 run open/close/terminal，ephemeral=`seq=0` 不推进游标且无 gap；frontend journal 仅正常启动、Dart VM 与 macOS host notices；llmtap managed proof/install/models 全 `200`，本确定性 Scheduler 路径无 completion 不虚构；REST/SQLite/UI 对账一致。正式证据=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-074544/evidence/SURF-052-scheduler-overview-five-channel.md`。
- `judge.py` 串行写入五格 `G1/F1/B2/C4/G1`，formal ledger=`2205→2210 judgments`，COVERAGE=`848 rows / 434→435 judged rows / 0 tombstones`，anchors=`10/10`。写账触发的 `gap-too-fast`/`discovery-collapse` 由 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-052-ledger-alarm-reaudit.md` 按同一 sealed session、红绿 stop-and-fix、五通道原始 journal 独立复审并串行 ack，未改阈值、算法、法典、锚点或 gate；最终 `alarms.py check`=`clean (2210 judgments)`，`gen_coverage.py --check`=`848 rows / 435 carried judgments / 0 tombstones`。批次四十一由 `46→47/50`，未到 50 格不跑统一长门禁、不提交；下一原子前线为 `SURF-053`。P12 的 400+ Journey 按用户裁定推迟二期，一期仍以 COVERAGE 为覆盖真相源。
## 2026-08-19 21:19 · SURF-063 正式五级入账，资源目录 loading/空态/managed 状态通过，批次四十二 40/50

- `SURF-063 settings/rail-resources` 在全新工作区真实覆盖 Resources 五面板：Models & keys 的 managed free-tier、scenario defaults、音色与 search empty；MCP loading 后落到 `0-100 of 102` marketplace；Memory/Sandbox 诚实空态；Workspaces 当前行与 `Current` 标记。无旧 workspace 泄漏、无 secret 暴露、无产品 defect。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-211359`，workspace=`ws_627041276bc74cad`，录屏=`133.883333s / 2560x1584 / H.264 / 60fps`。五通道封口：backend=`196` 行无应用红线；ssetap 三流连接、本路径无 chat/entity 耐久帧不虚构；frontend 仅正常启动/VM/已知 IMK host warning；llmtap managed proof/install/models 全 `200`、无本路径 completion；rig-check/rig-down、资源 focused=`77/77`、analyze、coverage check、process audit 通过。证据=`sessions/20260819-211359/evidence/SURF-063-settings-rail-resources-five-level.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2260→2265 judgments`，COVERAGE=`848 / 445→446 / 0`，anchors=`10/10`。写账触发的 `gap-too-fast`/`discovery-collapse` 由 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-063-ledger-alarm-reaudit.md` 独立复审并串行 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2265 judgments)`。下一前线为 `SURF-064`，本批未满 50 格不统一门禁、不提交。P12 400+ Journey 按用户裁定推迟二期。
## 2026-08-19 21:25 · SURF-064 正式五级入账，系统目录五面板与诊断回执通过，批次四十二 45/50

- `SURF-064 settings/rail-system` 在全新工作区真实覆盖 Storage & logs、Advanced limits、Network、Shortcuts、About：sidecar 数据目录、disk/db/attachments、schema limits、proxy restart 提示、六个快捷键、About update error 与 diagnostics `Copied` 均可见。未执行不可逆 factory reset，无产品 defect。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-212039`，workspace=`ws_b34dfc17c20ccd09`，录屏=`116.973333s / 2560x1584 / H.264 / 60fps`。五通道封口：backend=`171` 行无应用红线；ssetap 三流连接、本路径无 chat/entity 耐久帧不虚构；frontend 仅正常启动/VM/已知 IMK host warning；llmtap managed proof/install/models 全 `200`、无本路径 completion；rig-check/rig-down、系统 focused=`26/26`、analyze、coverage check、process audit 通过。证据=`sessions/20260819-212039/evidence/SURF-064-settings-rail-system-five-level.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2265→2270 judgments`，COVERAGE=`848 / 446→447 / 0`，anchors=`10/10`。写账触发的 `gap-too-fast`/`discovery-collapse` 由 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-064-ledger-alarm-reaudit.md` 独立复审并串行 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2270 judgments)`。下一前线为 `SURF-065`，本批未满 50 格不统一门禁、不提交。P12 400+ Journey 按用户裁定推迟二期。
## 2026-08-19 21:31 · SURF-065 正式五级入账，设置搜索双视图与目标洗亮通过，批次四十二 50/50

- `SURF-065 settings/rail-search` 在全新 workspace `Acceptance SURF-065` 上真实验证空查询三段目录、`zzzz` 无匹配、真实退格清空恢复、`zoom` 跨面板结果、面板头跳转、具体项滚动定位和等高蓝色洗亮；没有幽灵结果、旧目录残留或产品 defect，全部输入均用真实键盘事件。
- session=`/private/tmp/anselm-rig-formal-20260801-3/sessions/20260819-212554`，data=`/private/tmp/anselm-data-surf065-20260819-r1`，workspace=`ws_53a147068c051721`，录屏=`127.786667s / 2560x1584 / H.264 / 60fps`。五通道封口：backend=`176` 行无应用红线；ssetap 三流连接、本路径无 chat/entity 耐久帧不虚构；frontend=`5` 行仅正常启动/VM/已知 IMK host warning；llmtap managed proof/install/models 全 `200`、无本路径 completion；rig-check/rig-down、focused settings=`42/42`、analyze、coverage check、process audit 通过。证据=`sessions/20260819-212554/evidence/SURF-065-settings-rail-search-five-level.md`。
- `judge.py` 串行写入 `G1/F1/B2/C4/G1`，formal ledger=`2270→2275 judgments`，COVERAGE=`848 / 447→448 / 0`，anchors=`10/10`。写账触发的 `gap-too-fast`/`discovery-collapse` 经 `/private/tmp/anselm-rig-formal-20260801-3/evidence/SURF-065-ledger-alarm-reaudit.md` 独立重审并串行 ack，未改阈值、算法、法典、锚点或 gate，最终 `alarms.py check`=`clean (2275 judgments)`。批次四十二达到 `50/50`；统一长门禁已通过：rig Python=`42/42`、`make verify` 四门全绿、`make -C backend testend` 全量黑盒=`283.748s`，gofmt/diff/process-listener audit 全绿。工作树审计后提交，下一原子前线为 `SURF-066 settings/panel-general`。P12 400+ Journey 按用户裁定推迟二期。
