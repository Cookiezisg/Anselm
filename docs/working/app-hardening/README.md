...
>
> **贯穿纪律:每个手工发现的缺陷,必须留下一个机械守卫再走。** 否则本轮是「打扫」不是「加固」,
> 下个月还要原样再扫一遍。
...
> 格式:`[编号] 现象 · 定位 · 修复提交 · 守卫`。**未修的留在表里,不许口头带过。**

| # | 现象 | 来源 | 状态 |
|---|---|---|---|
| B1–B5 | 见 §1 | 用户报告 | 待修 |
| **L1** | `GET /api/v1/conversations/cv_d45bb2114af8e189` 及其 `/messages` **404**,23:43 两次、00:59 又两次(另一 ID) | 后端日志考古(§2.2) | **待查**——app 在请求不存在的对话,疑似删除/分叉后残留旧选区 |
...
1. §3 矩阵每格有明确结论(✅ / ❌ / 🚫),**空格 = 未完成**。
2. §4 台账无「待修」;**每条修复配机械守卫**;两仓门禁全绿。
3. 一轮完整操作后:Flutter 终端零 overflow / 零 assertion / 零未处理异常;
   后端日志零 500、4xx 全部可解释。
4. 我测不了的三类如实转入 `ACCEPTANCE-GUIDE` C4,不冒充已验。
5. 本轮新增的守卫全部进 `make verify`——**下一轮不必重扫同一格**。
