...
| **L13** | **空态复读它上方的标题**——「模型与密钥」面板里一个图标下写着「搜索密钥」,而正上方的分区标题**也是**「搜索密钥」。用户当场点名:「这里又引入墓碑了」 | **不是文案手滑,是一种反复回来的形状**:空态**借用分区标题的 i18n key** 当自己的标题。它什么都不回答——空的是什么、该做什么、是不是出错了,一个都没答。用户此前已裁过一次(0718 大表尽头那句墓碑是**删掉**而非改写),settings 自己的 P1 一致性扫荡也把「标题重复」与「空态零人话」双双列为已修项——**它又回来了**,故这次上机械守卫而非只改文案 | `<L13 提交>` | `empty_state_tombstone_guard_test`:扫 `lib/**` 每个 `AnState(...AnStateKind.empty...)`,断言其 `title:` 不是 `*Section` 键。**两次自我纠错**:①初版用 6 行窗口,漏掉第一个犯例——**仅仅因为修复本身那段注释把 `title:` 挤出了窗口**,改为**括号配对 + 剥注释**;②初版还匹配 `*Title`/`*Header`,当场误判 `comingSoonTitle`/`notFoundTitle`/`firstUseTitle`/`relayFailedTitle`(那些是**专门写的**空态文案、只是恰好以 Title 结尾),**收窄到只认 `*Section`**——一条分不清「标题」与「分区标题」的守卫只会训练人无视它。先证红 ✓ | **无覆盖**:没有任何测试读过空态的**文案来源**;像素断言更抓不到——两种写法渲出来都是一个图标加一行字 | ✅ 已修 |
| **L15** | **`/代码块`(与 `/表格`)插入后,光标停在块**外**——继续打字,字落到块**下面**的新段落里,块是空的。用户判:「我感觉对表格也不合理。不应该是要写字母?」 | **不在那句「刻意收光标」的注释,而在 `_insertBlock` 对**所有**块型一刀切**:插块后一律追加空段 + 把光标 park 到那里。对无内部编辑的原子块(分隔线)合理,对**自带内部编辑器**的块(代码块、表格)全错:人插它就是为了往**里**写 | `fb3bde09`(代码块)+ `c10a560c`(表格) | `an_editor_test` 的 **E4 四格**,先证红:块下方无追加段 ×2 / 内部编辑器真持焦 ×2。**并反转一条既有守卫且留证**——`table on a NON-empty paragraph inserts BELOW` 的末句 `expect(nodes.last, isA<ParagraphNode>())` **本身就在要求那个尾随空段**,即把被判错的行为写死成了断言,按 B4 做法反转而非删除 | **有测,但测的是旧行为**:表格那条既有守卫**要求**尾随空段;代码块侧则从没有测问过「插完之后光标在哪」——插入成功与光标落点是两件事,而只有前者被测过 | ✅ 已修(两个坑记档:①同一次 execute() 里 replace/删触发段会撞 super_editor 仍活着的 action-tags reactor 抛 `Null as TextNode`,改为不动触发段;②builders 按主题色记忆化,只翻 autofocus id 的 setState 不重建 builder——该 id 纳入重建判据。真机:代码块与表格各验,字分别落进块内首行/左上角首格) |
| **L16** | **右岛属性面板在编辑中陈旧**:编辑并保存后「字数/大小/修改时间」仍显**打开时**的值(实测差了 4 分钟与数十字节),**切走再回来才刷新** | **最直白的修法恰好被另一条正确的设计禁止**:`openDocumentProvider` **刻意不在编辑中失效**(失效会重建编辑器、丢光标),故 loaded doc 一打字就陈旧。但属性面板要的只是字数/字节/时刻这些**派生值**,根本不需要那个被冻结的 provider——**大纲早就从编辑视图实时重喂**,只是度量没走同一条通道 | `fb3bde09` | `doc_live_metrics_test` 两格:feed 的派生与 inspector 自己的**剥空白 rune** 公式一致(不一致会让面板在活值/回退之间切换时数字**跳**)+ 换文档即清(B 页绝不闪现 A 页的大小)| **无覆盖**:右岛属性的数字**从哪来**没有测过;而它在整棵树重挂后又是对的,故手测极易漏——你得**在编辑中途**去看它 | ✅ 已修(加 `docLiveMetricsProvider`,编辑视图在喂大纲的**同一出口**一并喂;面板优先读它、回退 loaded doc。真机:边打字「30 字/37 B/20:00」实时跟动) |
...
| **L18** | **「保存并测试」成功了却给用户报错**:新建密钥 201、探测 `:test` **200**(真的通过了),面板上却是一行红字「this method is not allowed for this path」——**未翻译的后端英文**。§5 收尾核 4xx 账时,这条 405 是唯一说不出为什么的 | **客户端照着兄弟端点类推、发明了一个不存在的 REST 端点**。`testKey` 在 probe 之后用 `GET /api-keys/{id}` **重读那一行**取新状态,而 api-keys **没有单读端点**(后端与契约都只有 list / PATCH / DELETE / `:test`)。且这次重读**本就多余**——调用方 `ApiKeysController.test` 的 `finally` 里已经无条件重拉整列,那次单读取回的东西下一行就被覆盖,**唯一效果就是必然 405**。故修法是**删掉它**、`testKey` 返 `void`,而不是给后端加端点(端点面越小越好,原则 #6/#8)| `<本次提交>` | `test/guards/api_surface_guard_test.dart`:**从 Go 源码里读出真实路由表**(`HandleFunc("<METHOD> <path>")`),再拿 Dart 里每一条**字面量** `/api/v1/…` 调用去对,不在表内即红。先证红 ✓(精确点名 `settings_repository.dart:308 GET /api/v1/api-keys/{}`)。**两次自我纠错、都是误判方向**:①`memories/${pinned ? 'pin' : 'unpin'}` 被当成 id 段——那**两个取值都是真路由**;②`conversations:$action` 的**动作名**是变量。两者都改为「够不着就说出来并跳过」,并在文件头**声明覆盖面**(只认字面量路径)——一条乱喊的守卫会被人静音,一条假装覆盖了动态路径的守卫比说清边界的更糟。另设 `served.length > 80` 的**清醒断言**:路由表抓取一旦坏掉必须**大声红**、而不是空绿放行 | **无覆盖,且对其余每一道防线都隐形**:它编译得过;它自己的 fixture 测试全绿——因为 fixture **实现的是客户端想象的那个方法**(`Future<ApiKey> testKey`),而不是后端真有的东西;后端只在**运行时**用 405 回答。这与 L14 是**同一族**(客户端对后端形状的想象没人对过账),故这次上的是**机械对账**守卫、非单点修补 | ✅ 已修(真机复看:同一操作干净落地,行上绿色探测点、零红字;线缆 `:test` 200 → 列表 200,405 消失) |
...
> **2026-07-26 收口:1–8 全勾。** 每条写的是**依据**、不是一个勾。

1. ☑ **§3 矩阵每一格有明确结论**——56 格:**48 ✅** / 8 转 C4(🚫 物理测不了,见第 7 条)。零空格、零「应该没问题」。
2. ☑ **§4 台账清零**——**25 条,全部 ✅ 已修**,无「待修 / 待查 / 已知问题」。§0 铁律一那六种逃逸话术一条都没用上;
   两条**曾**被我记为「有明文设计、交用户裁」的(L15/L16),用户当场判为缺陷后即按缺陷修在根上,没有留成取舍。
3. ☑ **五栏全满**——25 条逐条有 现象 / **根因层** / commit / **守卫** / **门禁为何没抓到**。根因栏写的是**层**不是行:
   契约漂移(B1)、原语的无界约束(B2/L4/L5)、双事实源(B5)、保活记忆越过 workspace(L1)、信号与 resync 不同流(L7)、
   close 快照不自足(L6)、一个 deadline 盖两类开销(L11)、身份用了任意深度取法(L9)、信封假设与宪法相反(L14)、
   与删除事件对账而非与树对账(L17)、客户端类推出不存在的端点(L18)。
4. ☑ **每个守卫都验过修复前是红的**——且**六次自我纠错留在案上**,它们本身就是「守卫可能空绿」的证据:
   L1 三次(URL 断言空绿 / 夹具自造缺陷 / stub 网关空绿)· L11 两次(假件忽略 ctx / 一次性 gate 被主回合用掉)·
   L13 两次(6 行窗口漏第一例 / `*Title` 误判四条)· L18 两次(两类动态路径误判)。
5. ☑ **修完真机复看**——每条都在真机上重演过原场景;**其中一次复看作废重做**(L6:8742 上跑的是旧进程,我的重启因工作目录不对
   静默失败,核对进程启动时间才发现)。记此是因为「health 200 只证明有人在监听,不证明监听的是你刚改的那份」。
...
8. ☑ **新增守卫全部进 `make verify`,两仓门禁全绿**——根 `make verify` 四子门禁(backend / frontend / docs / demo)**全绿**;
   前端 **5057** 测。本轮新增的机械守卫按「覆盖整类」而非「补这一处」建:
   `row_family_meta_overflow_guard`(两原语 × 四宽度)· `lifecycle_resync_guard`(源码级:订阅信号者必订阅 resync)·
   `empty_state_tombstone_guard`(全库扫空态标题来源)· `workspace_write_guard`(源码级:禁绕行 `setActiveWorkspace`)·
...
- **本轮修的 25 条里,只有 4 条是用户报的**(B1–B4 的原始清单),其余 **21 条是扫查过程中逼出来的**——
  其中 **6 条**(L4/L5/L9/L10/L12/L18)是**盯日志与线缆**发现的、界面上完全看不出来。这佐证了 §2 那句
  「`make app` 的终端只有一半」:七只眼里,眼睛②(后端结构化日志)与眼睛④(SQLite 直查)贡献了最多的隐形缺陷。
- **一条留给用户的产品判断(非缺陷)**:S18 的「危险靠 LLM 自报 + 逐次确认」在真机上**几乎从不上膛**——
  171 次工具调用里模型**一次都没**主动报过 `dangerous`(删函数、`rm -rf` 目录都只报 `cautious`)。
  实际防线是**驻地越界写闸**(它无视自报等级、且两个豁免都不适用)。要不要给删除类工具加一条不依赖模型自觉的底线,是产品决策。
- **一条工具教训**:`lsof -ti :8742 | xargs kill -9` 会连**客户端**一起杀(app 握着三条 SSE 也占该端口)——
  杀后端要按**进程匹配**,不能按端口。
