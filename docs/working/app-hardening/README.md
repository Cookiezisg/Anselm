...
| E04 | 真执行 | ✅ **四个执行动词全验**(`:run`/`:trigger` 先前已验;本轮补 `:call`/`:invoke`,**并顺带把 L14 的真机复看在这两条路径上补齐**——此前只有 `:run` 真机看过,另两个仅测试绿)。**`:call`**:右岛调试台对 handler 多一个**方法选择器**、动词正确地是「调用」;跑 `pet` → 绿「完成」+ 真实结果(`happiness_gained: 5` 与猫的回话)+ 4ms + 台账实时长出「刚刚 pet 手动」。**先打线缆核形状**:`:call` 返 `{"data": …}`——N1 信封在,故 L14 的 `postData` 修法在此路径同样正确。**`:invoke`**:本机 0 agent,建探针 `e04_cat_keeper` 挂 `cyber_cat.status`。**建的过程本身验到两件事**:①**挂载 ref 必须精确到方法**,写 `hd_<id>` 被 422 拒且错误信息直接给出正确形态 `want hd_<id>.<method>`(可操作、非空洞);②`mount-health` 预检端点返 `allHealthy:true`,且该结果**上了脸**——详情页头部「挂载正常」绿 chip。跑 `{"question":"猫现在怎么样?"}` → **完成 · 3.7s · 2 步 · 输入 1338 / 输出 187**,**轨迹逐段摊开**(推理 → `cyber_cat__status` 真调用及原始结果 → 推理 → 定型输出),结果 `{"answer":"赛博猫心情不错😊,但饱腹度只有 44/100…"}` 与 handler 实际状态(hunger 44)**逐字对得上**——真跑了 ReAct loop。**JSON 校验也顺带验到**:我误删大括号后编辑器当场红字「载荷必须是合法 JSON。」且「唤起」钮变灰禁用,补回即恢复。**带外建 agent 时 rail 智能体段计数实时 0→1**(零刷新)。探针 agent 测毕已删 |
...
| L02 | 编辑器 | ✅ **markdown 即打即转全中**:`# ` → 一级标题、`- ` → 项目符号**且第二行自动续列表**、`**粗体**` 与反引号行内代码就地成形。**slash 命令**:`/` 唤面板(正文/标题 1-3/引用/代码块/表格/无序·有序列表/待办),**中文过滤**「代码」精确收敛到一项,回车插入成功。**@ 药丸未测**。**逼出 L15**(见下)——已修 |
...
| L05 | 右岛三组 | ✅ **三组俱全**:身份头(页名 + 字数 + 今天编辑)· **大纲**(随标题增删实时增减)· **属性**(路径/大小/修改时间)· **反向链接**(0 时给「还没有页面链接到这里。」的人话空态,不是墓碑)。**逼出 L16**(见下)——已修(见下「属性面板的陈旧」)|
| L06 | skill 侧 | ✅ **已扫,A/B 判决干净**。构造对照:一个 `user` 源 skill(信任门只扣**安装**来源)预授权 `Bash`,两组发**逐字相同**的指令、删两个一次性 scratch 目录。**对照组(无 skill)**:人闸**抬起**——红框「危险 · 等待你确认」+ 命令原文摊开 + 拒绝/总是允许/允许;点拒绝后目录**还在**,模型如实改道并说明「系统拦截了这次调用」。**测试组(`@l06-cleanup`)**:**零确认直接执行**,`exit 0`、目录已删。**两组的自报等级经库核对同为 `dangerous`**(唯一变量就是有没有挂 skill),故这免掉的确实是**同一个**本会抬起的闸——`allowed-tools` = 预授权、非限制白名单,得证。**顺带验到工具路径**:一次本想做无 skill 对照的运行里,模型**自己**搜索工具、发现 `l06-cleanup`、`activate_skill` 激活、再直接执行——@ 提及与 `activate_skill` 两条激活路径都授予预授权 |

#### 观察(非缺陷,交用户判断):危险自报在真机上几乎从不触发

L06 的构造过程逼出一条比该格本身更值得知道的事实:**这台机器的整个使用史里(171 次工具调用),模型从没有一次
主动自报 `dangerous`**——5 次 `cautious`、164 次 `safe`,其余全无。它把 `delete_function`(删掉用户的一个函数)
评为 `cautious`,把 `rm -rf` 掉一个目录也评为 `cautious`;**只有我在提示里明确要求「把 danger 标为 dangerous」时
它才标**。

后果:S18 那道「危险靠 LLM 自报 + 逐次内存阻塞确认」的闸,在真实使用里**几乎从不上膛**。本战役此前见到的每一次
人闸(C16/C18),全部来自**另一条**路——`writesOutsideWorkDir` 的驻地越界写闸,而它**刻意无视自报等级**、且
**两个豁免都不适用**(见 `dispatchWithGate` 的注释:approve_always 与 skill 预授权都是「关于工具」的承诺,而越界是
「关于路径」的事实)。

**这不是缺陷**——S18 明文就是「无中央权限门控、危险靠 LLM 自报」,代码与宪法逐字一致。记此是因为它改变了对**实际
防线**的判断:真正拦住破坏性动作的是驻地写闸,不是自报机制。是否要给删除类工具加一条不依赖模型自觉的底线,是**产品
决策**,留给用户。
...
| P06 | 字体三轴 · 快捷键改绑 · 出厂重置 · 更新检查 | ✅ **已扫**。**内容轴**——**即时生效**、零重启,且**辖区律同帧验通**:助手答案 prose 转**衬线**(思源宋,笔画末端三角衬线清晰)的同一帧里,工具卡「已列文档 · 3 个」**仍无衬线**、`daily-fortune` **仍走代码轴等宽**——三条辖区各守其地。**代码轴 / UI 轴**——两条都**先落盘、重启前不变、重启后变**,即面板那句「重启后生效」是**诚实标注**而非托词(先查 plist 确认选择已持久化,再重启对比)。**判据都取客观量、不靠「看着像」**:代码轴用**同一行代码、同一缩放区域**的字符步进宽度(尾端 x≈903→925,同串不同宽=换脸);UI 轴用**结构性**字形差(Inter 的数字 `1` 有横向底衬、San Francisco 没有)——因为 Inter 与 SF 同为人文无衬线、度量极近,**纯肉眼在本分辨率下不可判别**,这一点如实记下。**快捷键改绑**——点键位进捕获态「按下新组合键…」→ 按 ⌘J 当场落地(行首长出偏离标记)→ **真接管**(⌘J 折左岛、浮层头 reopen 钮出现)→ **旧键位已解绑**(⌘B 无反应,是**接管**不是并存)→ 「全部恢复默认」一键回 ⌘B、标记消失。**出厂重置**只核确认闸形态、**不点**(永久删整个数据目录);**更新检查**已在 P04 顺带证到。测毕三轴与快捷键**全部还原**(`font.{code,content,ui}` = `jetbrainsMono`/`system`/`bundled`,`shortcuts` 空=全默认),三源零新增异常 |
...
| **L17** | **打开着的页面被删掉之后,海洋继续把它整页呈现**——标题、可编辑正文、活属性一应俱全,而每一次键入的自动保存都 404,背后只有一句光秃秃的「操作失败」。真机:带外删掉打开着的页 → rail 的行消失了,中心纹丝不动;继续打字,字在屏上、`PATCH → 404`、**哪儿都没存**。AI 自己就能删文档(契约把 `document.deleted` 明标为「破坏性、AI 可删用户文档」),故这是**日常可达**路径 | **机制本来就有,只接了用户自己驱动的那一条路**。rail 的删除调 `_clearIfSelected`——它比对被删 id 与当前选区,对**本地**删除完全正确。但没有任何东西为**其余**路径对账:agent 的 `delete_document`、另一个视图、410 恢复。**而且它连 rail 自己也漏一格**:`Delete` **软删整棵子树**却只为根 `publish(ctx,"deleted", d)` **一条**帧——删父页时,打开着的**子页**被销毁,而任何地方都不会说出它的名字,按事件对账**在原理上**就够不着它。第二层是**错误被吞**:`catch (_)` 把 404 与网络抖动压成同一句,唯一必须让用户知道具体原因的场合(页面已不存在、这些字永远不会存住)信息在此丢失 | `<本次提交>` | **三格,先证红**(摘掉修复:两条判决格红、误判格绿——正是应有形态):①「见过又不见了 → 离开并明说」;②**祖先被删 → 子页同样离开**(rail 自己那条路也漏的一格,只有树知道);③**误判方向**——树里从没见过的 id **绝不**被踢(创建之后、树重取落地之前那个窗口,误判会把作者从**刚新建**的页面里踢出去,比原 bug 更严重)。故判据是**「见过、又不见了」**而非「树里没有」 | **无覆盖**:既有 library 测试覆盖了**用户自己删**(`_clearIfSelected` 有测),没有任何测试问过「**别人**删掉我正开着的这一页会怎样」——而 AI 删文档正是产品的常规动作。祖先那一格更是连**本地**路径都从没被测过 | ✅ 已修(与**树**对账、不与删除事件对账:行离开树就是没了,不论怎么没的;`_seenInTree` 记账放在 build 里[`ref.listen` 只在变化时触发,挂载时就已打开的页会永远记不上「见过」];404 存盘失败改说「无法保存——此页面已被删除」,非 ApiException 保留通用提示。真机:删除落地即自动离开 + 顶带「此页面已被删除」,三源零新增异常) |
...
### L15 / L16 —— 用户 0726 判「都有问题」,已修(而非留作设计取舍)

我原本把这两条记为「有明文设计、交你裁」;用户当场判定**都是缺陷**(「我感觉对表格也不合理。不应该是要写字母?
陈旧这个…我感觉都有问题」),据 §0 铁律一「用户拍板即为裁决」,两条均按缺陷修在根上。

**L15 · slash 插代码块后光标停在块外。** 根不在「刻意收光标」那句注释,而在 `_insertBlock` 对**所有**块型一刀切:
插块后一律追加空段 + 把光标 park 到那里。对无内部编辑的原子块(分隔线)合理,对**自带内部编辑器**的块(代码块——
以及用户点名的**表格**)全错:人插它就是为了往**里**写。**修法**:`_insertBlock` 加 `focusInside`——true 时既不追加
尾段、也不移文档光标,由块自己的嵌入编辑器 autofocus(`AnCodeEditor` 新增 opt-in `autofocus`,默认 false 保住「一篇
多代码块不各自夺焦抢滚动」)。**施工中踩的两个真坑,均记档**:①在插块同一次 execute() 里 replace/删触发段,会撞
...
重喂**;**修法**:加 `docLiveMetricsProvider`,编辑视图在喂大纲的**同一出口**一并喂字数(与 inspector 的剥空白 rune
公式**同款**,否则回退时数字跳)/utf8 字节/时刻,面板优先读它、回退 loaded doc,换文档即清。守卫两条(feed 派生一致 +
换文档清)。真机:边打字,「30 字 / 37 B / 20:00」实时跟动,不再等重挂。
