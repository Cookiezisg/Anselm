...
时长槽和播放状态文案。当前 transcript 解析尚未接真实播放器/时长探测，因此生产路径会诚实显示
“暂不能播放”，不会给用户一个假播放按钮；组件端口已预留 `durationMs`/`playbackProgress`/`playing`/
`onPlayTap`，widget test 已锁定“音频走专用卡、未知播放能力大声降级、可播放态点击只触发播放而不误触
generic open”。未完成：真实播放端口、附件内容 lease/loopback 播放源、感知状态、时间戳引用跳转、
录制为音频附件入口、原件删除/离线态的端到端 UI。
