...
upstream delta/error→client、nonce refresh 与控制帧白名单路径；gateway 已接受 `cancel` 并只关闭本次 ASR 会话、不向上游转发未知控制事件；sidecar 与 gateway 都对 client/upstream 两条 WebSocket leg 发送 ping，并用 message/pong 滚动 30s 读 deadline，同时保留 2min 会话绝对上限；网关 ASR 会话已经接入同一 quota/ledger：按 `qwen3-asr-flash-realtime` 新加坡时长费率预留 120s，实际按成功转发的 PCM16/16k/mono 字节换算秒数 settle，无音频或拨号前失败 rollback，总音频字节超过预留上限会拒绝继续转发并返回稳定 `SPEECH_AUDIO_TOO_LONG`；网关新增显式付费 `make qwen-asr-evals` live 入口，有 `EVALS_ASR_WAV` 时要求真实转写文本，否则只做 protocol/connect smoke。未落地/未验收：跨进程持久化 offline fallback、真 key eval 尚未在本机/服务器实际执行。
