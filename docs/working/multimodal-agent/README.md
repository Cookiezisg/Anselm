...
upstream delta→client、nonce refresh 与控制帧白名单路径；gateway 已接受 `cancel` 并只关闭本次 ASR 会话、不向上游转发未知控制事件；sidecar 与 gateway 都对 client/upstream 两条 WebSocket leg 发送 ping，并用 message/pong 滚动 30s 读 deadline，同时保留 2min 会话绝对上限。未落地/未验收：跨进程持久化 offline fallback、quota/ledger、真 key eval。
