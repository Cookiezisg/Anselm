...
瞬时读取/播放失败仍保留可重试状态。未完成：附件内容 lease/loopback 播放源的生产 E2E（当前 `audioplayers`
URL source 不支持 header，与 bearer loopback 鉴权直接冲突，不能硬接）、感知状态、时间戳引用跳转、
录制为音频附件入口、离线态端到端文案与恢复路径。
