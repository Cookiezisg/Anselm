...
发送位变成麦克风，实时听写结束后落成可编辑文本；语音输出在输入链稳定后接入。
...
- 纯文本当前走 `deepseek-v4-flash`；任意图片/视频当前走 `kimi-k2.6`；合法音频固定
  `AUDIO_UNAVAILABLE`。
- text route 已发布 1M；multimodal route 仍发布 262,144。
...
- provider、账本 CHECK、rate card、metrics、readiness、dashboard 和文档仍把 `kimi` 视为闭集成员。
...
- `deepseek`：纯文本主路线；
- `qwen`：图片/视频主路线、Omni 音频感知、ASR/TTS；
- `kimi`：迁移后从代码、数据库、账本、metrics、readiness、dashboard、env、文档和测试全部删除。
...
- speech realtime：ASR/TTS availability。
...
6. 开音频 perception；
7. 删除 Kimi 配置和代码；
8. 更新 dashboard、文档和回滚 bundle；
9. 旧二进制回滚时必须与对应 DB/config 一起回滚，禁止只换 symlink。
...
- sidecar 建一次性 WebSocket 到 BYOK Qwen 或 Anselm 网关；
...
- assistant 消息提供 speaker 按钮；
- 主 Agent 仍产文本；Qwen TTS Realtime 将文本流式合成；
- 支持暂停、继续、停止、倍速和 voice 设置；
- 新播放自动停止旧播放；
- 可按 `textHash + voice + speed` 本地短期缓存，正文变化即失效；
- 音频不进 conversation context。
...
```text
ASR → 1M tool-agent → streaming TTS
```

而非把整段产品切成 Omni Realtime 会话。这样保留工具、人闸、durable execution、压缩、1M history 和
统一审计。为降低延迟，TTS 可在主 Agent完成稳定句子边界后开始，不等待全文。
...
本战役先留契约接缝，不把 full-duplex 绑入第一批。
...
**出口**：本地 BYOK 可分别完成图、视频、音频和工具 continuation。
...
- rate card/ledger/reserve/settle；
- config/env/readiness/dashboard/metrics；
- capability v2；
- DB migration 移除 `kimi` 闭集、加入 `qwen`；
- e2e/fuzz/故障注入；
- canary + deploy + rollback 演练；
- 物理删除 Kimi 代码与文档。
...
- sidecar/gateway TTS；
- assistant speaker/playback controls；
- sentence-boundary streaming；
- voice/speed 设置；
- cancel、缓存、quota、设备切换；
- 三平台真机。

**出口**：播放不阻塞消息流和工具执行，新回复/用户操作可稳定打断。
