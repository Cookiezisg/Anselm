...
可见范围，切换会话或离开 transcript 会主动停止当前附件音频，避免后台残留播放和 active 状态串会话。未完成：附件内容
lease/loopback 播放源的生产 E2E（当前 `audioplayers` URL source 不支持 header，与 bearer loopback 鉴权直接冲突，
不能硬接）、感知状态、时间戳引用跳转、录制为音频附件入口、原件删除/离线态的端到端 UI。
