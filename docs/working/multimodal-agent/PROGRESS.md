...
### ⑫ ES 实体页空态墓碑退役 ✅ 门禁绿(`REAL_EXIT=0`)

用户原话:「函数如果没有依赖的话会有个墓碑。」派 Sonnet 5 建、主会话逐行复审 + 跑门禁。`insetEmpty` **已删**(零使用者,grep 实证)。

**用户点名那处**:依赖现在是一行 32px 的 `依赖 —`,与本卡其余五行同文法,且**无条件渲染**——顺带消掉一个「墓碑 ↔ 标签行」的条件分支(正是军规那一类)。

**本节分类有一处错,子代理查实并纠正**:`workflow_overview` 那处不是标签字段,是 `if (g == null)` 即图 blob **解析失败**——那是**错误**不是空值,压成一行 KV 会是范畴错误。改成 `AnState(kind: error)`。故真实构成是 6 A + 1 错误态 + 6 B = 13,总数对、分类不对。

**多出一处**:`trigger_overview` 也在用 `fieldList`(第 7 个消费者),同法一并改,否则删不掉 `insetEmpty`。
...
### 地基:`AnButton.iconOnly` 自带 tooltip ✅ 门禁绿(`REAL_EXIT=0`)

§5.13 的「顺带发现」,主会话亲自(牵动 87 处、不宜外派)。用户当初就是撞在这上面才来问「这个下载按钮是什么东西」——`semanticLabel` 只到语义树,对读屏是对的、**对眼睛不存在**。
...
### ⑧ 前半:CH-b 对话 fork 全套 ✅ 门禁绿(`REAL_EXIT=0`)+ testend 四场景主会话自跑全绿

派 Opus 5 建、主会话逐行复审 + **亲自核实两处裁决的依据** + 跑门禁 + 自跑 testend。

**两处契约草案与后端现实冲突,已裁决**(详见 chat-iteration.md §2.2 施工记录):
1. **relation 边的动词**:草案要「fork→源」的边,而 kind 是 **CHECK 强制的 4 动词封闭集**;铸第 5 个动词是 ~22 文件 + 整表重建 + 推翻一条立法注释,属独立架构决定。裁决:用已立法的 `create` 并**反转方向为 源→分叉**。反转还排掉一个真陷阱(主会话已核实):`SyncOutgoing` 是 replace-all,按源的出向侧写会**抹掉该对话建过的所有实体 create 边**;按分叉的入向侧写则恰好精确(一个分叉永远只有一个父)、重复 fork 幂等。
2. **零新错误码**:未知 message/对话各复用既有 404 码,其余情形无物理失败;为凑一条码造校验剧场违反原则 #6,故 `error-codes.md` 未改。

**summary 两分支的要害是水位重定基、不是照抄**——分叉把 block 从 1 重排,带走源的数字会藏错行。testend 那条正是用「水位 ≤ 分叉自身 block 数」证明它是重定基。

**顺带记一条 testend 抖动(非本批引入)**:自跑全量时 `TestMCP_ScriptedServerLifecycle` 红,**单独跑 6 秒即过**——`-parallel 16` 负载下的时序抖动,与 fork 零接触面。留档免得下次被当回归查。

**本步剩余:CH-c retry 未做**(`:retry` + `superseded_by` + 装配过滤 + 版本翻页 UI + testend)。

### ⑧ 后半:CH-c 重试 + 编辑重发 + 版本翻页 ✅ 门禁绿(`REAL_EXIT=0`)—— 施工序⑧ 收口

派 Opus 5 建、主会话逐行复审 + 亲自核实两条吃重发现 + 跑门禁 + 自跑九场景(retry 五 + fork 四)。

**`messages.superseded_by` 合 D1 的理由已在 `database.md` 讲清**——它只收窄**一类消费者**的视野;行仍在盘上、仍在三个 REST 读形态里、仍可 `?around=` 定位、UI 仍可翻看,把指针翻回 `''` 即完全恢复。两个物理删例外仍是唯一的两个。

**装配过滤是第三条同族条件**(主会话核实):`messages.go:402-403` 里 `WhereEq("subagent_id","")` 紧跟 `WhereEq("superseded_by","")`,与 summary 水位下推同族,顺路加、未另建机制。

**两条子代理自己挖出的隐藏陷阱,都已修**:
1. **压缩读的单向阀**(主会话核实在 `contextmgr.go:224-234`):若只过滤装配、不过滤压缩读,一条被重试掉的回答会被折进 `conversation.summary`,然后**从摘要那一侧流回**之后每一轮提示——过滤成了单向阀。
2. **fork × retry 交互**:`Fork` 现在预铸 message id 并 remap **两个**指针;否则分叉的版本链指回源线程,更糟的是每条复制版都重置成「现行」,模型对同一个问题拿到两个答案。**故 `fork.go` 被改动过——主会话自跑 CH-b 四场景确认仍全绿。**

**两处偏离 §3.2 措辞,已入档**:图标顺序保持 CH-a 已上线的次序(复制·分叉·编辑/重试·翻页),未把重试挪到分叉之前;**翻页器也给了 user 组**——编辑重发会产生 user 版本,不给就意味着读者的旧句子不可达,那与「旧版永可回看」矛盾。
...
### ⑮ WD1.5(CL 批)chat 左岛按驻地分组 ✅ 门禁绿(`REAL_EXIT=0`)+ 主会话自跑 5 testend + 事务四子例全绿

派 Opus 5 建、主会话逐行复审 + **亲自反证诚实律** + 跑门禁 + 自跑两套测试。

**诚实律做成了断言,我反证过它非空绿**:两语言 8 条组文案禁「目录 / director / folder」,**并且正面要求**删除框说出「磁盘/disk」且给出**计数**(可怕词的缺席不等于一句安抚)。我把一条文案改成好心的「删除该目录下的全部对话」→ 立刻转红并点名那句话;还原即绿。最终文案:「删除全部对话」+「『anselm』里的 12 个对话将被永久移除。**磁盘上什么都不会被删除——文件一个都不动。** 置顶的对话不受影响。」

**整组删除到底删了什么**(逐字):那些 `conversations` 行上的 `deleted_at` 戳 + 每条线程的 relation 边与触点台账。**没删**任何消息行、**没删**文件系统上任何东西。

**工单要求的「删整组后断言消息行仍在」在黑盒里不可观测**——每条消息读路径都被对话存在性检查所辖,故已立碑线程的 `GET /{id}/messages` 按设计答 404,线缆分不清「行被立碑」与「消息被抹」。改在两张表都看得见处证明:后端单测 `TestDeleteWorkDir_NeverTouchesAMessageRow`(同库立 messages schema → 写真回合 → 删整组 → 从 messages store **逐字节读回** message 与 block 行)。边界已写进场景注释与 `domains/conversation.md`。**这是「没跑的绝不写成跑过了」的正面用法:不可观测就换一个能观测的地方证明,而不是把断言删掉。**

**事务全或无的测试非空绿也被实证**:用 SQLite 触发器注入两种半写——`RAISE(ABORT)`(报错)与 **`RAISE(IGNORE)`(静默跳过、UPDATE 却成功)**。后者才是需要事务 + 行数交叉核对的那个;临时关掉核对 → 两个 IGNORE 子例转红。

**三处施工时裁决/查实**:①投影分列 `activeCount`/`archivedCount`(单个 `count` 要么让组头撒谎、要么让确认框撒谎——两个批量动作**刻意对「显示已归档」开关盲**,破坏性动作不该取决于视图偏好);②**工单外新增 `?pinned=`**,因为「置顶赢、不重复」下,住在**收起**的组里的置顶线程根本不会被取回来,置顶段会**漏行**(真 bug 非效率问题);③**查实一个工单未提的功能回归**:收起的文件夹什么都不取,故只收窄四段的搜索会对用户尚未滚进视野的对话答「没有匹配」——裁决:**搜索替换结构、不过滤结构**(有查询词时 rail 变成对整个 workspace 的一条平的无头结果列表,清词恢复四段)。

**顺带强化地基一处**:新增 `ormpkg.ParseDBTime`,flowrun store 原先手抄的三格式解码改为一行转发(原则 #8)。

### ⑯ WD2 git 操作 + WD3 worktree ✅ 门禁绿(`REAL_EXIT=0`)—— **施工序 ⓪→⑯ 全部收口**

派 Opus 5 建、主会话逐行复审 + **亲自核实三条安全细节** + 跑门禁 + 自跑测试。

**脏区切分支的护栏选了「直接拒绝」,理由值得记**:git 自己的语义是「能带过去就带、冲突才拒」——于是那个**令人意外**的结局(你的活现在待在一条你以为不是的分支上)落在**成功**分支上,而且是**静默**的。对一个 agent 正在其中干活的驻地,那比一个错误更糟:system prompt 里点出的分支变了、活也跟着搬走了。候选(b)「翻译 git 的失败」被否——它照样保留那个静默成功;候选(c)「提供 stash」被否——承担 stash 生命周期就是 §1 拍板 #10 明令不做的迷你 git 客户端。**拒绝是唯一一个连一行活都丢不了的选项。**

**一处刻意的不对称**:`:create-branch` 脏时照走——`checkout -b` 从当前 HEAD 起,工作树一个字节不变、冲突不可能存在。单测成对写死。脏态在**服务端此刻现读**,不信客户端上次看到的投影。

**三条安全细节主会话已核实**:①`gitinfo.go` **零 shell 调用**(无 `sh`/`bash`/`-c`),只有 `exec.CommandContext` × 5、参数数组;②分支名交给 git 自己的 `check-ref-format`,另加纯 Go 前置拒**前导 `-`**(以 `-` 开头的 ref 对 git 合法却会被下一条命令读成**选项**);③worktree 名必须是**单个路径段**(拒 `..`/`/`/`\`/绝对写法/前导 `-`),正是这份更严让「目标落在仓库兄弟位」**可证明**——端点因此**只收名字、绝不收路径**。

**与 `make worktree` 对齐**:根 Makefile 是 `git worktree add ../Anselm-$(NAME)` + 分支 `wt/$(NAME)`(分支已存在则复用)。转录为 `WorktreeBranchPrefix="wt/"` + `WorktreeTarget(top,name)=<top 父目录>/<top basename>-<name>`;量起点用 `MainToplevel`,否则约定会嵌套成 `Anselm-a-b`,而纪律要的是主仓库旁**一排平的**兄弟。

**顺手清一处既有文档漂移**(触发即停):`CLAUDE.md` 与 `contract.md` 写着「~261 错误码」,**实测 350 条**。已改正,记 `[doc-fix]`。

**两条诚实边界**:①WD3 的半状态——worktree 建成后驻地切换失败会留下完好 worktree + 原地线程,**不做补偿删除**(删一份刚建好的 checkout 才是真丢东西);②真机截图未做,demo fixture 已备「脏 + 两分支 + 一兄弟 worktree」形状,**别当已验**。

**没做**:commit/push/pull/merge/rebase/reset(#10 明令)· worktree 删除(破坏性,需自己的确认框与「未合并分支」立法)· 远端分支语义。
