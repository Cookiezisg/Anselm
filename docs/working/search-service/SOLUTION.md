...
1. **Tool call 检索**：现有 8 个垂搜 tool 保契约换引擎；新增 **`search_blocks`（搜积木）**——**只搜能直接塞进 workflow 的可调用单元**（function / handler 的方法 / mcp 的工具 / agent / control / approval），刻意不含对话、文档等内容类——LLM 搓工作流时心智负担最小化；全域搜索是人的入口（综搜框），不给 LLM。
...
| LLM 工具的检索范围 | **`search_blocks` 只搜 workflow 节点面板**（fn/hd 方法/mcp 工具/ag/control/approval），返回可直接接线的 ref（§7.4）；**不给 LLM 全域搜索**（对话/文档等内容类不入其结果） | 该 tool 的产品本质是搜积木：减轻 LLM 心智负担，搜到即可填进 workflow 节点；全域综搜是人的入口 |
...
| transport | handlers + tool | `GET /api/v1/search`、`POST /api/v1/search:reindex`、`search_blocks` tool、8 个垂搜 tool 换引擎。 |
...
| 期 | 内容 | 量级 |
|---|---|---|
| **M1 地基（BM25 全量）** | domain/app/infra 三层 + DDL/触发器 + 12 个 Source + Notifier 接线 + 队列/对账 + HTTP 端点 + `search_blocks` + 8 tool 换引擎 + §9 测试 + 文档四件套（api/database/error-codes/domains）| ~3 天，发版即 12 类全量可搜（综搜面向人）、积木可搜（面向 LLM） |
