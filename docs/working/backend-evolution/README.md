...
先读本页，再读 [`CURRENT.md`](CURRENT.md) 的当前边界与战役；选场景时读 [`FRONTIER.md`](FRONTIER.md)；确认发现后只向 [`LOG.md`](LOG.md) 追加证据。已收口工作和旧体系只在 [`HISTORY.md`](HISTORY.md) 与 [`legacy/`](legacy/) 中追溯，不能拿来替代当前结论。
...
`EVALS_MEDIA=1` 与 `EVALS_VOICE=1` 是明确的真钱接受层：未设置门控可以跳过；一旦显式要求运行却缺少必要凭证，必须失败，不能把“什么都没跑”记为通过。
