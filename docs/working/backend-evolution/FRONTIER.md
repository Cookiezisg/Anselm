...
| FRT-04 | workflow：生成者 → 下游观看者 | hybrid | 下游收到真实媒体而非“产物已生成”文本 | 录制请求包含原始媒体字节 | managed image→BYOK OpenAI、managed speech→BYOK Qwen audio、managed video→BYOK Qwen qwen3.7-plus viewer 的 workflow exact-byte wire through；Google native viewer producer 已稳定完成但连续真实调用受 provider-wide 429 阻断，保留为外部待恢复项；纯 managed 下游节点也完成且附件可回读；managed provider wire 仍待 gateway 侧 recorder |
