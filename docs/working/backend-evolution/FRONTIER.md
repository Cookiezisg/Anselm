...
|---|---|---|---|---|---|
| FRT-01 | 默认聊天 + 图片/视频附件 | managed-read/default | Anselm 的实际输入路由、lease 与能力降级正确 | 请求形状 + 回合/附件真相 | ready |
| FRT-02 | BYOK 视觉/音视频输入 | byok-read | 多模态输入是正式 BYOK 能力，不被生成边界误关 | 目录能力、wire part 或明确文本降级 | ready |
| FRT-03 | BYOK 模型调用受管出图 | hybrid | 模型调度与受管生成正确接合，生成者不被重复喂像素导致重画 | tool/receipt、调用次数、产物与后续请求 | ready |
...
| FRT-13 | 取消、重试与恢复中的媒体 | all applicable | 取消回合不留孤儿、重放不错误复用或重复消费 | durable 状态、附件溯源、调用计数 | ready |
