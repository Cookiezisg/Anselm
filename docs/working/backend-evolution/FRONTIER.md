...
| FRT-02 | BYOK 视觉/音视频输入 | byok-read | 多模态输入是正式 BYOK 能力，不被生成边界误关 | 目录能力、wire part 或明确文本降级 | OpenAI image / Google Gemini image / Qwen MP4 video / Qwen WAV audio 真实通过；OpenAI `gpt-audio` 目录滞后由 provider-owned capability bridge 覆盖，真实 WAV `input_audio` 产品路径及 audio+`run_function` continuation 已通过 |
...
| FRT-04 | workflow：生成者 → 下游观看者 | hybrid | 下游收到真实媒体而非“产物已生成”文本 | 录制请求包含原始媒体字节 | managed image→BYOK OpenAI / Google viewer 与 managed speech→BYOK Qwen audio viewer 的 workflow exact-byte wire through；纯 managed 下游节点也完成且附件可回读；managed provider wire 仍待 gateway 侧 recorder |
...
| FRT-13 | 取消、重试与恢复中的媒体 | all applicable | 取消回合不留孤儿、重放不错误复用或重复消费；审批停泊与外部 firing 在崩溃后仍可恢复，网络重试不重复执行 | durable 状态、附件溯源、调用计数、inbox/审计关联 | handler/workflow cancel/retry/crash no-orphan + image preparation ready/failed/cancel/retry + boot budget eviction/regeneration + crash requeue + parked approval restart/decide + webhook firing restart-before-drain + same-body minute dedup + fsnotify event payload/filter/hot-swap + sensor false/true transition/handler invoke failure/MCP target + fsnotify/sensor pause→SIGKILL→resume through；真实 webhook 双触发下 serial/skip/buffer_one/replace/allow_all 五种 overlap 策略的 firing disposition 与 flowrun 状态均通过；reprobe on worker/recovery changes |
