...
| FRT-05 | MCP/function/handler 产物 → 下游模型 | byok-read / hybrid | 各产地均能成为 MediaRef；不退化为占位字符串 | 产物字节、MediaRef、下游请求 | MCP/function/handler producers and chat/workflow vision wire through；subagent managed generation→receipt state through；subagent provider failure is annotated, durable, and parent-continuable；reprobe on media/ref encoder changes |
...
| FRT-15 | workflow 大图扇出与 AND-join | managed-write / hybrid | 多路 live 入边必须全部完成后才 join；节点按 `(node, iteration)` 只落一次，终值不丢 | HTTP flowrun、节点 durable rows、终点结果 | 12 节点/8 路扇出/两级 join 真实通过；25 节点深循环与大图 replay 仍待后续 |
