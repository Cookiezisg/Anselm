...
| FRT-13 | 取消、重试与恢复中的媒体 | all applicable | 取消回合不留孤儿、重放不错误复用或重复消费 | durable 状态、附件溯源、调用计数 | handler cancel/retry/crash no-orphan + image preparation ready/failed/cancel/retry through; boot/recovery reprobe pending |
