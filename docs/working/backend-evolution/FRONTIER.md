...
| FRT-04 | workflow：生成者 → 下游观看者 | hybrid | 下游收到真实像素而非“图片已生成”文本 | 录制请求包含原始像素 | managed 生成→receipt→BYOK OpenAI / Google viewer 的 workflow exact-byte wire through；纯 managed 下游节点也完成且附件可回读；managed provider wire 仍待 gateway 侧 recorder |
