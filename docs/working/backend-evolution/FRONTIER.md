...
| FRT-04 | workflow：生成者 → 下游观看者 | hybrid | 下游收到真实媒体而非“产物已生成”文本 | 录制请求包含原始媒体字节 | managed image→BYOK OpenAI、managed speech→BYOK Qwen audio、managed video→BYOK Qwen qwen3.7-plus viewer 的 workflow exact-byte wire through；Google native viewer producer 稳定完成，但约 1MB managed inlineData workflow 请求仍受 429 阻断，而独立 98-byte image probe 已恢复，暂保留为大媒体/request-shape 外部限流项；纯 managed 下游节点也完成且附件可回读；managed provider wire 仍待 gateway 侧 recorder |
| FRT-05 | MCP/function/handler 产物 → 下游模型 | byok-read / hybrid | 各产地均能成为 MediaRef；不退化为占位字符串 | 产物字节、MediaRef、下游请求 | 当前受管 + BYOK workflow 的真实 function、resident-handler、stdio MCP producer→OpenAI vision viewer exact-byte wire 均 through；chat/workflow vision through；subagent managed generation→receipt state through；subagent provider failure is annotated, durable, and parent-continuable；同一 execution group 的双 subagent 并发结果各自挂回独立 tool_call、父回合可继续；其余产地仍按 producer-specific reprobe；reprobe on media/ref encoder changes |
