...
> [`tool-card-blueprints.md`](tool-card-blueprints.md)(WRK-056)的证据底册,12 章:**01–09** 后端 113 工具逐个契约(args 全字段 / edit 补丁风格 / Execute 精确返回 / progress 流 / 错误模式)· **10** 框架机制(summary·danger 注入与剥离 / args delta 生命周期 / progress 块 / 拒绝散文常量 / 全量注册表 / E3 嵌套)· **palette** 前端乐器清单(逐件流式适性 + 缺口清单)· **graph** 图画布勘探(增量可行性 / settle-then-replay 路径)。
...
> 源:`backend/internal/app/tool/{document,skill,attachment}/`。共 **14 个工具**:document 7 + skill 5 + attachment 2。
...
## attachment 系(2 个,薄适配 attachmentapp.Service)
...
```json
{
  ...
  "availableModels": [{"apiKeyId","provider","modelId","displayName","contextWindow"}]
}
```
availableModels 尽力而为（catalog 读失败不 fail 工具、返空数组）。体积中等（key 数 × 模型目录）。
...
- **决议**：`POST /api/v1/conversations/{id}/interactions/{toolCallId}` body `{action, answer}`。合法 action 封闭集：`approve | approve_always | deny | accept | decline`（枚举外 422 `INTERACTION_INVALID_ACTION`；无待决 → `NO_PENDING_INTERACTION`）。
  ...
  - 决议后 chat 发**对称 resolved 信号**（同 scope、同 `node.type="interaction"`、content=`{"toolCallId":...,"conversationId":...,"resolved":true}`，ephemeral），前端据此清提示 + rail 的 awaitingInput 琥珀点，不用从 tool_result 反推。
...
- attachment（2）：`list_attachments` `read_attachment`
...
- **ToolCardSpec**(tool_card_catalog.dart)——每工具文法缝:`verb(t,{live})` 确定性动词对 / `target(state)`(须容忍 partial args)/ `receipt(t,state)` 终态回执(`danger:true` → 红 + 自动展开)/ `body` 族体 / `bodyless`(Read:回执即卡)/ `liveBody` 活窗。未编目 → generic,绝不无声。已编:F1 Read/Write/Edit、F2 Glob/Grep/LS、F3 Bash、F4 create/edit×9 实体。
...
| 乐器 | 渲染什么 | 关键参数 | 流式 | 约束 |
|---|---|---|---|---|
| **AnSunkenPanel** | 凹陷面板:surfaceSunken 底 + r-chip + s12/s8 内距;可选 header(s4 间距压顶) | `child, header, inset`(15 prose 传 `AnInset.bubble`) | **A**:纯装饰容器 | 非交互填充 |
| **ToolWindow**(feature 层) | =SizedBox(∞宽)+AnSunkenPanel;机器窗身份的具名化;header 槽=命令回显 | `child, header` | **A** | 同上;窗内长行 wrap(无横滚/无 ANSI) |
| **ToolLiveTail**(feature 层) | 活尾巴:progress 尾 N 行装小机器窗,随内容长缩(宿主 AnExpandReveal 提供 AnimatedSize 感) | `text, tailLines=3` | **A**:每帧 split 取尾,便宜;最强"真在干活"信号 | 只显尾巴,无回滚查看(完整窗在展开体) |
...
| 乐器 | 渲染什么 | 关键参数 | 流式 | 约束 |
|---|---|---|---|---|
| **AnBadge** | 状态/标签药丸:柔底+语义字+可选前置状态点 | `label, tone(AnTone: none/ok/warn/danger/accent), dot(AnStatus?)` | A | 截断封顶 `AnSize.block`(280) |
...
1. **有界回滚终端窗**:ToolLiveTail 只有"尾 N 行",没有可滚动、stick-to-bottom 的有界日志视口原语(长 progress 想在卡内回看必须展开体+封顶截断;无虚拟化日志列表)。
2. **ANSI 转义渲染**:Bash 输出若带 ANSI 色码会裸渲文字;机器窗无 ANSI→TextSpan 解析器。
3. **机器窗横滚/wrap 切换**:ToolWindow 内 Text 默认 wrap,长行(diff 外)无横滚选项、无 copy 按钮(copy 只在 AnCodeEditor 栏)。
...
12. **复制整卡结果**:结果/回执无一键复制 affordance(仅代码框有)。
