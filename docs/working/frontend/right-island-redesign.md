...
- **边缘 kind 纳入 ✅（2026-07-10,用户「边缘 kind 都需要」）**：探查工作流(tasks/wdyx3pao9)诚实判定 5 个边缘 kind 里 **3 可建、2 结构性不可达**——①**attachment**→展品座(提取 exhibit 的 `_AttachmentPedestal` 成公开 `AttachmentPedestal`,`_StageRow` settled 分支 attachment case 优先;不走 sceneFromTruth[无 stage body/无真身];**删死码** exhibit_stage.dart + exhibit_provider.dart[WRK-064 后无引用])②**skill**→sceneFromTruth(GET /skills/{name} 已有;chat repo 加 getSkillSnapshot + skillTruthProvider,args={name,context,allowedTools,disableModelInvocation,body},SkillStageBody body settled 显全文;id=name,create_skill)③**mcp**→sceneFromTruth(GET /mcp-servers/{name} 已有;getMcpSnapshot + mcpTruthProvider,args={name,tools:[names]}省 env,McpStageBody `_resultTools` 加 args 回退;id=name,create_mcp)。**2 不可达(诚实标注,非偷懒)**:**memory** 在后端 noTouch 名单(catalog.go:162)→ 永不产触点行 → 侧幕 ledger 永无 memory 行 → sceneFromTruth 触不到(端点在但不可达);**subagent** 无 executionId/表/端点,真身=LIVE-only 嵌套子消息(前端 hydration 现丢弃)+ 不产触点 → 需 B6 reload 重水合缝(跨切工作,待立项)。demo cv_sync 补种 skill(commit-helper)+mcp(github) 触点及快照。加 skill/mcp 单测。fe-verify 3333 全绿。至此侧幕**可点行的每一种实体都渲完整真身舞台**(8 Quadrinity/config + skill + mcp),attachment 展品座,memory/subagent 有据可查地留白。**对抗复审 2 修(2026-07-10)**:①mcp `_resultTools()` args-tools 回退门加 `toolName=='create_mcp'` 限定——live `mcp__srv__tool` 执行调用与 sceneFromTruth 真身共用 `McpStageBody`,落定 ~1.8s 窗内其自身顶层 `tools` 入参若被渲成「发现 N 个工具」即误标(旧注释只推理 install/reconnect、漏 mcp__ 执行);②skill 降级守卫去掉 `sk.context.isEmpty` 死条件(后端 write() 恒默认 context='inline'→此项恒 false→空 skill 不降摘要反渲稀薄铭牌),只按 body+allowedTools 判空。加 content-less skill 回归测 + mcp toolName 契约锁。

- **subagent 落定行 B6 重水合 ✅（2026-07-10,用户「Subagent 有办法的,parent id」）**：上一批把 subagent 判「不可达」的两个理由(不产触点 + hydration 丢弃嵌套子行)其实都是**前端现状、非硬约束**——5-agent 研究工作流(tasks/wm7dxo8r3)调和了那处措辞张力:subagent 每个回合是一条**真落盘的独立 Message 行**(`subagentId=runID` + `attrs.parentBlockId`=派它的 tool_call block id,后端 `subagent.go` 有 const + trace_test 佐证),其 reasoning/tool_call/text blocks 全落盘;「LIVE-only 不落盘」指的仅是父 tool_call 在 reducer 树里的 `children`(那棵嵌套树是 live 期临时折的)。**轨迹本体 100% 已持久化**(作 sibling sub-message 行 + 其 blocks),GET /messages 三读形态全返回、后端零过滤——缺的只是前端把这些 sib 行按 `parentBlockId` 折回父 tool_call 之下。**纯前端 + 现有端点、零后端改**。三步:①**折树缝**(`conversation_transcript`):4 处 hydration 不再丢 `subagentId≠''` 行,累积 `_subMessages` + `_foldSubagents()` 按 `attrs.parentBlockId` 折成父 tool_call 的 `message` 子(复现 live E3 形,`SubagentStageBody._trajectory` 自动摊平);孤儿(父未载)保持 pending 待后页,`_foldedSubs` 幂等;结算元数据(tokens/stopReason)抬到 tool_call 供结算行;新 `subagentBlocks` 访问器扫 settled+live 两层(live 回合永不搬迁,谢幕的在 `_live` reducer、reload 的在 settled)——**顺带点亮内联 transcript 嵌套轨迹重水合**(NestedRunPane)。②**`sceneFromSubagentNode`**(scene_from_truth,独立路、**不走 `_argsFromTruth`**):真身=已折进内存的节点,直接包成 `live:false` StageScene 交 `SubagentStageBody`(它只读 `scene.node.children`)。③**第三行源**(`_computeRows`):**仅 closed subagent**(live 的归 director 通道路径,同 `block:<id>` rowId→谢幕零跳),`_RowSpec.subagentNode`,置于合成 live 行区(谢幕后行不从顶跳到底);`_AccordionList` 挂 transcript 监听,**仅 subagent 签名(id:isOpen)变化才 setState**(避免逐帧全列重建,合 coalescer 纪律)。i18n 增 `delegated`。demo cv_sync 补种委派 run(Subagent tool_call + sibling sub-message)。加 6 折树单测(折/空/孤儿/幂等/元数据抬/live)+ 1 侧幕 widget 测。**真机验收**(cv_sync):右岛「调研现有通知渠道」·`Delegated` 行 → 点开渲折好的 ReAct 尾(thought/grep/read/reply)+ 绿落定记号 + `1840 in · 320 out` 结算行;内联 transcript 同步重水合。fe-verify 3341 全绿。至此侧幕**可点行的每一种实体/委派都渲完整真身舞台**(8 Quadrinity/config + skill + mcp + subagent),attachment 展品座,唯 **memory 仍留白**(后端 noTouch→永不产触点行,端点在但结构性不可达)。**已知降级(非崩)**:跨页孤儿 sib(父不在已载窗)保留 pending 不折;in-flight 父回合的已完成 subagent(sub 走 `_seedLive` 不处理)→ 空轨迹,与改前一致。
