...
> **完美态蓝图已拍板**(2026-07-06,114 工具逐个设计 + 50 新原语 + B1–B7 建造顺序):[`tool-card-blueprints.md`](tool-card-blueprints.md)(WRK-056,逐族实施的设计事实源)+ 线缆普查底册 [`tool-card-census.md`](tool-card-census.md)(WRK-057);拍板结果已入 §1 #6–9 与 §6。
...
| 6 | **完美态蓝图 + B1–B7 顺序**(2026-07-06) | 114 工具逐个完美态设计定稿于 [`tool-card-blueprints.md`](tool-card-blueprints.md)(WRK-056,含六种生长母语/五种 morph 笔法/50 新原语总表/R1–R12 裁决);建造顺序 **B1–B7 认可**(见 §6) |
...
| F6 entity-get | get×8+read_document/read_attachment/inspect_media (11) | 正在查看/阅读/检查 X→已查看/已阅读/已检查 X | args id(终态升 name) | 实体摘要+内容折叠区(截断+逃生口)+视觉证据窗 | 无 | — |
...
- **自动展开**:仅 错误(一次)+ 等待确认(人闸锁开);完成后重新收拢(历史读起来像目录不像日志)。**运行中绝不自动弹窗**(WRK-065 用户定调,`liveBody` 自动活窗通道已拆除):族体在用户 chevron 后拥有**两张脸**——在飞=流式舞台(活终端/生长秀/两幕手术/嵌套轨迹,`toolLive(state)` 分流)、落定=档案;在飞期结果向段落(输出/「无返回值」/档案回放注/结果条[今 AnStatBar,批3 并入])一律扣住不渲(渲了=撒谎)。
...
| **B3 目录感普查** ✅ | F5+F6+F7+F17 全落(50 工具,transcript 读成目录)。**B3.0** 面板能力注册表 `panelLocationFor`/`hasPanelFor`(#8 跨 feature 缝、镜像 router)+ `entityKindGlyph` 抽表(#9)· **B3.1** `searchReceipt`+`parseSearchHits` 双形状 nil 安全 + `ToolCardSpec.verbOf` 搜索↔列双声道(11 search 收起行脱 generic)· **B3.2** `ToolHitList`(#10)四族共享机器窗命中行列(glyph/主文15/次行13/尾 meta + 双截断态 footer + 级联淡入一次[`ToolCardReveal` 亲历落定信号]+ 行 intent + 当前记)· **B3.3** F07 落定体(searchHitBody + 逐族行投影 + `hasBodyOf` 空→回执即卡)· **B3.4** `EntityGetBody` 四段骨架(#31:ToolEntityHeader/KV/EntityCodeWindow/RawResultDisclosure)+ `AnKvRow` 行级 mono(#9)· **B3.5** F06 get×8 逐工具投影 + read×2 串模板解析器(#32,散文 vs 源码区分)· **B3.6** F05 26 极薄卡(回执群 revert/delete/lifecycle/restart/kill/meta + `ToolDependentsBlock` 删除审计#15 + `resultFailed` 绿壳红事实缝 + 动态改名动词 + `fmtStamp`)· **B3.7** F17 conversation 3 薄卡(manage action 分派动词+软失败中性 / list·search 迷你 rail)。新原语:panel_registry/entityKindGlyph/ToolHitList/ToolCardReveal/EntityGetBody/EntityCodeWindow/RawResultDisclosure/ToolDependentsBlock/AnKvRow 行级 mono。**结论待提取进 features/chat.md** | gallery 逐族 specimen + 真机截图审(命中门/四段陈列/删除审计/绿壳红事实/迷你 rail 全对)+ fe-verify **1965 测绿** |
| **B4 终端与文件手术** ✅ | F3+F1+F2 全落(shell/fs 最高频面,生长秀密度最高)。**F03 shell**:termFold+ansiSpans(#5 折叠原地重写+SGR→token)→ footer 解析器组(bashReceipt note 优先级/statusReceipt 四态/killShellReceipt 三态)→ AnTermTail(#46)→ `AnStickViewport`+`AnTermViewport`(#6 有界回滚钉底+懒加载)→ Bash 卡(footer→底条+copy+后台体)+ BashOutput(轮询诚实:exited 不展开、`suppressReceiptAutoExpand` 缝)+ KillShell 薄卡。**F01 fs-ops**:fsErrorKind 九类+四象限 readReceipt → `AnPathChip`(#16 basename+hover+copy)→ Write 活窗(内容流入,生长秀)→ Edit 两幕(−old/+new `ToolEditLivePane`)→ mount 三式(mcp__/handler__ 名路由皮)。**F02 fs-search**:`AnCountUp`(#9 计数揭示)→ LS/Glob(parseLsListing/parseGlobResult → 目录感 ToolHitList)→ Grep 三模式(parseGrepContent/Count/Files + `highlightMatches` + `GrepContentView` 编辑器式全局搜索面板)。新原语:termFold/ansiSpans/AnTermTail/AnStickViewport/AnTermViewport/WindowCopyButton/AnPathChip/AnCountUp/ToolEditLivePane/GrepContentView。**结论待提取进 features/chat.md** | gallery 逐族 specimen + 真机截图审(ANSI 折叠/终端回滚/生长秀/两幕手术/目录窗/Grep 点亮全对)+ fe-verify **2059 测绿** |
| **B5 执行与档案** ✅ | F8(6 exec 工具落定/薄卡)+ F9(13 run-log 档案全落)。**B5.1** `ToolIOSection`(#13 输入/输出显式硬渲染规则、绝非嗅探)+ `ExecResultBar`+`fmtElapsed` → run_function/call_handler · **B5.2** fire_trigger 薄卡 + `toolNavPill`/`toolNavTo` 导航缝(接 panel_registry) · **B5.3** ★`FlowrunNodeList`(#38 节点台账:失败/park 置顶+红摘要+80 封顶诚实账[计数取 nodeSummary、绝不数 nodes.length])+ `FlowrunNodeSummary` DTO(契约同步)→ replay_flowrun · **B5.4** trigger_workflow 薄卡(启动凭据+get_flowrun 指路;FlowrunSnapshotPane 推迟——版本图端点零证据) · **B5.5** invoke_agent 落定体(输出散文/逐键+`_InvokeStatBar` steps/tokens/executionId)+ ProseWindow 短稿内联修复 · **B5.6** ★`RunBeadStrip`+`RunLedger`(#缝:一眼看健康珠串+slim 记录列)→ 4 聚合 search(fn/hd/agent/mcp,**slim 投影铁律**:input/output/logs/transcript 一字不渲)· **B5.7** 3 计数 search(flowruns/firings/activations:五色处置+fire 标记+returnValue 惰性行内树,无聚合不造 ✓✗)· **B5.8** ★`RunDossier`+`ProvenanceLine`(状态头条+双端保留日志[MCP stderr 独立染红段]+出处可点/mono 分界)→ 4 薄 get(fn/hd/mcp/activation)+ 底盘灰回执 Flexible 加固 · **B5.9** get_flowrun(复用 FlowrunNodeList,不造第二套瀑布——record-once 无真时长)· **B5.10** ★共享 `transcript_hydration`(存储 block→BlockTreeReducer 重放成 live 同构树,B6/B8 共用)+ `TranscriptPeek`(有界步骤目录)→ get_agent_execution 重卡。新原语:ToolIOSection/ExecResultBar/toolNavPill/FlowrunNodeList/RunBeadStrip/RunLedger/RunDossier/ProvenanceLine/transcript_hydration/TranscriptPeek。**invoke_agent 活期 NestedRunPane 随 B6 落**(需 transcript 子块接线,与 subagent 共建)。**结论待提取进 features/chat.md** | gallery 逐族 specimen + 真机截图审(执行结果条/节点台账/珠串健康/卷宗双段日志/轨迹目录全对)+ fe-verify **2182 测绿** |
| **B6 嵌套对话** ✅ | F15 全落(Subagent + get_subagent_trace + 补 invoke_agent 活期)。**底盘缝** `ToolCardState.nested`(tool_call 的非 result/progress 子块=E3 轨迹子树,live 才在、reload 空)· **B6.1** `NestedRunPane`(#47 嵌套运行活窗:机器窗内 reasoning/text/tool_call 紧凑行、live 末行微光、只显尾)+ `transcriptBlockRow` 共享块行(TranscriptPeek/NestedRunPane 共用)· **B6.2** invoke_agent 补活期(B5.5 推迟的 live 今接上;落定 nested 在→显轨迹、reload→执行档案注)· **B6.3** Subagent 卡(活期 NestedRunPane / 落定=任务+轨迹+回答 ProseWindow)· **B6.4** get_subagent_trace 双形(list=RunLedger 运行台账 / detail=blocks 经**共享 transcript_hydration** 水合 TranscriptPeek,与 live/get_agent_execution 同一适配器)。新原语:NestedRunPane/transcriptBlockRow。**结论待提取进 features/chat.md** | gallery 四 specimen + 真机截图审(嵌套轨迹活窗/末行微光/回答散文/运行台账/水合轨迹全对)+ fe-verify **2195 测绿** |
| **B7 生态收尾** ✅ | 最后 9 未编目工具全落(脱 generic;**至此 catalog 全覆盖、无 generic 兜底剩余**)。**B7.1 F11 todo**:todo_write 投影 args 结构项 / todo_read 解析渲染 `- [x]/[→]/[ ]` 行(双源)→ `TodoChecklist`(✓ 划线灰 / ▶ activeForm accent / ☐ 待办)+ `AnIcons.circle` 新原子 · **B7.2 生态尾**(`tool_card_ecosystem.dart`):get_relations→可导航依赖边列(from 药丸→to 药丸)· capability_check_workflow→体检报告(ok/问题红自动展开/警示琥珀+结构·依赖章)· install/uninstall/reconnect_mcp→ServerStatus 状态卡(连接绿+工具章 / 未连接红+末错窗+连续失败数)· list_mcp_marketplace→服务器目录(名+runtime+必填 env 警示)· get_model_config→配置摘要(默认模型逐角色+密钥+可用模型)。新原语:TodoChecklist。**结论待提取进 features/chat.md** | gallery 10 specimen + 真机截图审(勾选清单/依赖边列/体检报告/mcp 状态/服务器目录全对)+ fe-verify **2230 测绿** |
| **WRK-065 默认收起重构** ✅ | 用户定调「平时收起,点开:在跑=精心流式舞台/结束=终态」。两阶段普查(35+4 agent 读码逐条对抗核实):①`showLiveBody` 自动活窗通道 22+ 工具(builds 族 helper 默认继承含 7 个恒空死注册)②展开体流式期逐族审计——**「活窗=展开体简化版」只对一半族成立**(Bash/builds fn·hd·agent/嵌套族 body 已活;Write/Edit/write_memory/WebFetch/mcp mount/document·skill 的活内容只在活窗,Edit 中间态还渲误导性全红 diff)。改法:**拆通道**(chassis 删 showLiveBody+首 AnExpandReveal;`ToolCardSpec.liveBody` 字段+全部注册+独立 live builder 删尽)+ **活态并入族体**(`toolLive(state)` 缝:Write 生长秀/Edit 两幕手术/builds 纯 mono 尾[落定才高亮]/create_workflow op ticker→画布两幕同体/write_memory 便笺尾[生 JSON 倾倒仅落定兜底]/WebFetch 蒸馏流[maxHeight 收身,替死 144px 定高]/mcp·handler-mount progress 直显/call_handler 活 yields 直显[落定才抽屉])+ **撒谎句门控**(run_function/call_handler/invoke_agent/handler-mount 在飞不渲输出段·「无返回值」·档案回放注·RunStatBar)+ **顺修**(Bash 头 argStringPartial 在途回显/ToolLiveTail trim 空白守卫/落定「先跳后滑」与 4px 幻影带随通道拆除消失/**handler 契约键错位**:buildContentOf+侧幕 handler_stage+sceneFromTruth 合成键改真线缆键 initBody·shutdownBody[原 code 键线缆上不存在→init/shutdown 流入期全盲])。保留:失败自动展开一次/人闸锁开/thinking 流窗(用户拍板)。4 旧活窗测改写+3 新回归测(call_handler 无假「无返回值」/mcp 活尾无空壳/ToolLiveTail 空白) | fe-verify **3344 测绿**;真机流式验收(待) |
