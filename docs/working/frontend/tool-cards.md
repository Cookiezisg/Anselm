...
- **自动展开**:仅 错误(一次)+ 等待确认(人闸锁开);完成后重新收拢(历史读起来像目录不像日志)。**运行中绝不自动弹窗**(WRK-065 用户定调,`liveBody` 自动活窗通道已拆除):族体在用户 chevron 后拥有**两张脸**——在飞=流式舞台(活终端/生长秀/两幕手术/嵌套轨迹,`toolLive(state)` 分流)、落定=档案;在飞期结果向段落(输出/「无返回值」/档案回放注/RunStatBar)一律扣住不渲(渲了=撒谎)。
