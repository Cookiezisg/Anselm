...
| **B1 人闸**(=V6) | F16 全族(pendingInteractionsProvider 三源合一 → ToolInteractionGate 人闸 → V6 危险确认门 approve/always/deny+决议后不可变+重连补拉 → ask_user → decide_approval → inbox)+ **底盘缝一揽子**(tone/count/classifyResult/verb-state)+ AnIcons 精确表全量收口 | gallery 五态 specimen + testend 联调 |
