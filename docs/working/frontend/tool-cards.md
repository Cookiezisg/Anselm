...
| **B1 人闸**(=V6)🔨 | F16 全族(pendingInteractionsProvider 三源合一 → ToolInteractionGate 人闸 → V6 危险确认门 approve/always/deny+决议后不可变+重连补拉 → ask_user → decide_approval → inbox)+ **底盘缝一揽子**(tone/count/classifyResult/verb-state)+ AnIcons 精确表全量收口。**进度**:B1.a AnIcons 精确表 ✅(`ccb43249`)· B1.b ToolReceipt tone 三声调 ✅(`75bc3098`)· B1.c pendingInteractionsProvider 三源合一 ✅(`800b8c36`)· B1.d ToolInteractionGate 人闸原语+gallery 七态 ✅(`11cad5ef`)· B1.e V6 门接底盘 ✅(ChatToolCard 纯 prop 收 interaction/onResolve、transcript select 单块下喂、待决锁定展开门、approve 出处章)· B1.f ask_user 卡 ✅(三段动词 正在提问→等待你回答→已回答/已跳过/空答案[terminalVerb 按结果散文]、落定 Q/A 复用 gate resolved 模式;顺带兑现 B1.b 延后的 verb-state 缝=ToolCardSpec.awaitingVerb/terminalVerb)· B1.g decide_approval 卡 ✅(判词章 批准/否决+理由 + 后果条 flowrun.status+节点计数[nodeSummary.byStatus 否则自数 nodes[]]、NOT_PARKED 友好呈现[ownsError 缝:族自管失败显示])· **B1.h inbox 待建** | gallery 七态 specimen(已过)+ testend 联调 |
| B2 builds 旗舰 | F4 深化(WRK-056 蓝图):partialJsonEvents 引擎 + RunStatBar 公共化 → fn/hd 完全体(活代码窗/EnvFixTimeline/AnDeltaChip)→ **AnMiniGraph + GraphReveal/Morph(create_workflow 图生长 / edit_workflow 图 morph,pivot 旗舰)** → control/approval/document/skill/trigger;ProseWindow 首落;取数缝首版 | gallery 逐帧 + 真机截图 |
