...
> **读法勘误(2026-07-12,WRK-066 批4)**:本文写于窗族收敛前——文中所有 `ToolWindow`(灰凹机器窗)读作 **`AnWindow`**(白框窗族当家件,[法典](../../archive/convergence/convergence-codex.md) 族一;ToolWindow 已物理删除),`AnSunkenPanel` 作容器的呈现读作 AnWindow(凹面唯一住户=用户聊天泡);`WindowCopyButton` 挂 `AnWindow.actions`;**批5 追记**:文中 `AnBadge` 读作 `AnChip`(filled 形)、`AnCopyChip` 读作 `AnChip(mono+copyValue)`(两件已物理删除并入芯片族当家件)。设计意图(机器窗身份/内容模式/copy 语义)全部不变,仅壳的物理实现已换。
