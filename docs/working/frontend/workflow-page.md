...
2. **图置顶当 hero**:概览第一屏就是编排图,meta/治理卡在下。demo 的「图在中下」顺序不照搬。
...
| W4 驾驶舱 | `flowrunTimeline` 纯派生(节点→甘特段,时间定位+零跨度顺序回退,7 单测)+ `AnNodeGantt`/`AnRunBoard` 原语(gallery 4 specimen)+ **运行 tab**(取代 workflow 日志 tab:run 历史列表 + 甘特 + run 态活图 + **内联**节点调试[status/iteration/result/error + parked 审批门]强链,`runCockpitProvider` 翻页拉全 + `:replay`/`:kill` 重取真相)+ 写面 `replayFlowrun`/`killWorkflow`/`fetchFlowrunFull`。**W3 同病灶解**:log_list flowrun 懒取单页现为 workflow 死路(cockpit 翻页拉全)。**节点调试放内联而非右岛**:右岛专属 run 操作台(触发/活 run 终端),观测历史入文档流(单滚壳一致) | ✅(fe-verify 1464 绿;完成/失败/循环×2 甘特 + run 图 + 节点调试截图过)。**对抗复审 6 缺陷已修(2 HIGH)**:①甘特 bar 宽 `clamp(s4,w)` 极窄轨下限>上限抛 ArgumentError(release 崩)→ minBar 夹到轨宽 ②`busy` 永卡:replay/decide 重读中途换选中 run,superseded 早退不清 busy → 三按钮永久锁死 → 任何出口都清 busy ③kill 用首页覆盖 runs 却留旧 cursor → loadMore 跳号丢中段 → 同步重置游标 ④selectRun 对当前 id 无条件早退 → fetch 失败无法重点重试 + 面板留陈旧 composite → 仅已加载才早退 + 换选区清 composite ⑤孤儿行丢弃后端已知 kind 渲成 unknown → 读行 kind ⑥节点 result JSON 无上限灌单 Text(F173 653KB 前例)→ `prettyJsonCapped` 4000 字硬截。AnRunBoard IntrinsicHeight 与甘特测宽 LayoutBuilder 冲突 → 甘特每行定高、LayoutBuilder 只测宽) |
| W5 编辑器 | `graph_edit_ops` 纯 diff(workflowEditOps/applyEditOps 往返)+ `AnGraphCanvas` 编辑面(节点拖移/连接柄拖连[裸 Listener 绕竞技场]/边点选;画布 pan 改 onPan 让子 pan 胜出)+ 全屏编辑器路由页(工具条/编辑态画布/检查器)+ `workflowEditorProvider`(本地 working 图 + 边校验 + diff→一个 `:edit`→一版 + INVALID_GRAPH/OPS toast 留 working)+ `editWorkflow` 写面 + 概览「进入编辑器」接线。**动态 control 端口解析 + capability-check 问题条 + input CEL wiring 精编**留后续(control 端口暂文本、approval yes/no 下拉) | ✅(fe-verify 1492 绿;编辑器页 + 检查器截图过) |

每批:gallery → 接线 → 截图过目 → `make -C frontend verify` 绿 → 文档同步。W1 先行(一切之基),W2–W4 吃只读+run 两态,W5 编辑态收官。**workflow 站 W1–W5 全落**——图地基/页面/活运行/驾驶舱/编辑器五批齐。

## 后续遗留(workflow 站定稿后并入后续批)

- **动态 control 端口解析**:编辑器边检查器 control 端口暂文本框(须匹配分支名),应拉 `ctl_` 实体 active 版本 `Branch.Port` 集合做下拉。
- **capability-check 问题条**:`POST :capability-check` → problems/warnings,编辑器顶部/保存前呈现。
- **input CEL wiring 精编**:输入映射暂 field→CEL 文本行,可升祖先可见性提示/自动补全。
- **触控板 pinch 缩放**:画布 pan 改 onPan 后暂无 pinch(滚轮+工具条覆盖缩放),PointerPanZoom 路后补。
