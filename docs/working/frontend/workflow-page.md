...
| W4 驾驶舱 | `AnRunBoard` + `AnNodeGantt` 原语 + 运行 tab 组装(run 选择 ↔ 甘特 ↔ 活图 ↔ 右岛节点调试强链)+ `:replay`/`:kill`。**并修 W3 复审同病灶**:log_list_provider 懒取 flowrun 节点也只取一页(展示面,危害轻)——运行 tab 重做时按同款翻页拉全 | ⏳ |
