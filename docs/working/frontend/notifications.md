...
| **N2a 后端补名字** | ✅ 用户拍板「后端补名字」。实体生命周期通知 payload 补 `name`(function/handler/agent/workflow/control/approval 全族 + handler.crashed + workflow run_failed/approval_pending/attention/lifecycle_changed),使通知中心可扫读「Agent『triager』已创建」。16 处新 repo.Get(删前捕获 / 接住原丢弃 Get,best-effort 空回退)。events.md 同步 + testend name 断言。make verify 全绿。 | ✅ |
