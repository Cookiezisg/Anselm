...
| A-057 | `lib/features/entities/ui/detail/workflow_editor_inspector.dart:297` | 行 | _RetryEditor 用「标签左·控件右」Row 排布(×2 行),与本文件自述并全 inspector 遵守的「标签上·块控件下」_Field 语汇相悖 | 开关/数值行归入 AnFormField 语汇或成文豁免开关行式 | S | open·批6c(settings 面板批同做) |
