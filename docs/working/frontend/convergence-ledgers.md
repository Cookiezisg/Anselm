...
| A-039 | `lib/features/chat/ui/run_ledger.dart:61` | 芯片 | 手搓状态点 Container+BoxShape.circle ×3(:61 珠/:164 空心/:167 实心),同型再现 handler_stage:111、trigger_stage:81、function_stage:133,全绕开 AnStatusDot | AnStatusDot 长任意色/空心 variant,六处统一 | M | open·**批5a 五处已换**(run_ledger×3+handler+trigger;剩 function_stage:133 opTicker 内点随 A-043 批5b) |
...
| A-045 | `lib/features/chat/ui/tool_card_skins.dart:63` | 芯片 | WindowCopyButton 手搓复制钮(✓一闪状态机),与 AnCopyChip、AnCodeEditor copy 条三套复制件并存 | 并入 chip 族 copy variant,一件三形态 | S | open·**批5a 缓议**(AnCopyChip 已并;窗头裸字形 vs 芯片壳=每窗可见的密度变化,待帧对比后裁——建造者不盲改) |
