...
| A-027 | `lib/features/settings/ui/panels/shortcuts_panel.dart:164` | 芯片 | 键帽/录制态 chip 用 GestureDetector+Container 手搓;Border 默认宽 1 非 AnSize.hairline | 芯片族长 keycap/recording variant 进 gallery | M | open·**批5c 专批**(settings 焦点序高危区[快捷键录制链],键帽原语须真机复验四链路,不随马拉松批尾 rush) |
| A-028 | `lib/features/settings/ui/panels/workspaces_panel.dart:104` | 芯片 | _ColorDot(size 10 裸数)与 _ColorPicker(:204,22×22+选中边宽 2 裸数)手搓圆点色板 | 收成 swatch/色点原语,尺寸走 AnSize 档 | M | open·**批5c 专批**(同上,与键帽同面板一起落) |
