...
| A-087 | `lib/features/chat/ui/chat_tool_card.dart:289` | 条 | ' · ' InlineSpan 手拼元数据链模式,同型散布(skins RunStatBar:864/workflow legend:227/flowrun _summaryBar:235) | 出一个 meta 链小件(spans+分隔符统一) | M | open·**范围收窄·批3**(RunStatBar/legend 两处已并入 AnStatBar;余=收起行回执尾[豁免记档:行内尾注非块级条]与 flowrun _summaryBar[窗首表头行,待窗族批]) |
