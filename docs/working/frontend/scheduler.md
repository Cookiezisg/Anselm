...
1. **健康头**:名+生命周期徽+成功率+均时(工单③,窗口=7d,见 §8 窗口立法)+[Run now]+⋯菜单(:kill / 去 entities 编辑)。**珠串已删**(0717 拍板问题2:矩阵列头就是同一排珠子,还多长了节点行)。健康统计到此为止,不开洞察页。
2. **矩阵区(页顶,0717 拍板)**:**页级时间范围胶囊**(`AnTimeRangePicker`,默认近 7 天,快速预设+日期时间双端绝对区间;**一颗胶囊治矩阵+大表**,大表原 24h/7d/30d 下拉已删)+ **时序 `AnRunMatrix`**(旧在左新在右=时间轴,视口 `reverse` 锚最新端;滑近最旧缘懒加载更旧页[`SchedulerMatrixWindowController`:翻 `GET /flowruns` 页 → 逐页 `flowrun-matrix?flowrunIds=` 批查 → 归并,页尺 50=批帽]、前插零位移;节点名车道冻结在滚动器外)。**格阵是发射台**(0717 拍板:「点击直接跳转子页面,不要跳转到下面」):点列头→直进该 run 旗舰页、点格→旗舰 + `?node=` 预选(旗舰自己的选区文法),行头惰性只作名;`?run=` 展开行的列被点亮(一个选区两处投影)。空窗渲诚实句「这段时间没有运行」。
3. **run 大表**:AnLedgerRow 列表+keyset 分页哨兵(不立表格新件;海量有官方 TableView 退路)。**行身份=来源短语**(GHA「cron run 全长一样」之鉴):主文本=来源徽+触发摘要(cron·08:00 / 对话名走 [[id]] 真名缝 / 手动 / webhook path),mono fr_ id 降为 meta;列:状态点+vN 版本+节点进度+**耗时(排队 · 执行 双数,判决⑤,工单⑫落地前只显执行段)**+相对时间右缘铁线+↻N 徽+失败行错误首句(sub,danger);hover 行尾 ⏹/↻。**过滤=状态计数条**(全部|在跑 n|失败 n|等人 n,真数可点即过滤;「等人」走 inbox 派生,**绝不 `?status=parked`**)+来源下拉(时间窗归页级胶囊)。新 run 不插行,表顶「N 条新运行」pill 归位。**行内速览卡(0717 拍板,底部联动格的后继)**:单击行=开合 `?run=`(URL 真相,一次恰一行展开、再点收起;`AnLedgerRow.expandBuilder` 惰性揭示——收起的行绝不建卡[C-006]),卡=[甘特 ⇄ 图]双脸+状态点+「打开 →」旗舰门,快速双击直进旗舰。**批量 replay(判决②)**:过滤到失败态后行首 checkbox 多选,AnBatchBar 给 [↻ 批量重放],确认弹窗合并真数字;批量 cancel 同构(danger 带清单)。
