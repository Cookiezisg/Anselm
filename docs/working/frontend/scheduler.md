...
1. **文档化页头**(0717-晚 需求②③,entities 同文法 `AnOceanHeader`):页内面包屑「调度」→ 大标题=名 → meta 行(生命周期徽 · **范围统计句**[成功率/均时**跟随页级范围**——`schedulerRangeStatsProvider` 逐 (workflow,范围) 1-id 批查,窗按 `statsWindowOf` 映射进 stats `since`/`until`;句子窗口词=胶囊之词] · **页级 `AnTimeRangePicker` 胶囊**)→ 右上动作([Run now]+⋯菜单[:kill / 去 entities 编辑])。**珠串已删**(0717 问题2)。健康统计到此为止,不开洞察页。
