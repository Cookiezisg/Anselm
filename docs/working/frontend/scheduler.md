...
# Scheduler 第四海洋 —— 产品形态与建造规范(草案)

> 终案 = **方案 B(渐进披露)为主胎** + **方案 C 活性宪法整体并入** + **方案 A 矩阵/失败聚合语义降级收编**(三判官共识:B 43/43、C 43/42.5/42,A 的矩阵为唯一真信息增量但须降为透镜)。判官指出的共同盲区(止血/批量/统计窗口/孤儿 run/T6 引擎报价/冷打开空窗/cancel 语义/misfire/空态/a11y)逐条吸收,见各节与 §15。
...
1. **KPI 四牌**(AnCard+AnCountUp,全部预过滤深链):在跑 N / 等你 N / 24h 失败 N(**delta 箭头** ▲2,A 收编)/ 下次调度 in 3m。
2. **等你处理**(最贵地皮):`/flowrun-inbox` 全集(工单④enrich 带 workflow 名+deadline),每行=workflow 名+节点+等待时长+**AnCountdown 超时倒计时**(新原语,单顶层 Timer 共驱)+`ApprovalGate` 就地批/拒(带 reason);决策后行滑出;first-wins 输家 422→诚实 toast+refetch。
3. **正在跑**:AnLedgerRow 活行=状态点+workflow 名+mono fr_ id+来源徽+节点进度 x/y+活耗时+hover ⏹ 取消。tick 只驱动本区外观;`run_started`(工单①)durable 加行。
4. **未来 24h 调度**:首发简版(triggers `nextFireAt` 行列表,纯前端);S5 升级 `AnScheduleTrack` 时间轴(工单⑧)——过去实心点着状态色/未来空心虚点(措辞「预计」)/now 线偏左/**missed 第三态 ✕ 灰叉**(工单⑨,机器睡过头的调度点必须有脸,桌面 app 第一现实)。仅 cron 有未来点,其余 kind 如实缺席。
...
2. **run 大表**:AnLedgerRow 列表+keyset 分页哨兵(不立表格新件;海量有官方 TableView 退路)。**行身份=来源短语**(GHA「cron run 全长一样」之鉴):主文本=来源徽+触发摘要(cron·08:00 / 对话名走 [[id]] 真名缝 / 手动 / webhook path),mono fr_ id 降为 meta;列:状态点+vN 版本+节点进度+耗时(tabular measure)+相对时间右缘铁线+↻N 徽+失败行错误首句(sub,danger);hover 行尾 ⏹/↻。**过滤=状态计数条**(C 收编:全部|在跑 n|失败 n|等人 n,真数可点即过滤;「等人」走 inbox 派生,**绝不 `?status=parked`**——封闭集无此值,422)+来源下拉+时间窗(工单⑥)。新 run 不插行,表顶「N 条新运行」pill(AnFollowPill 扩 label)归位。
3. **本次运行联动区**(地图+放大镜,永不跳页):选中 run → 下格 **[甘特 ⇄ 图] 双脸 toggle**,同一选区两投影,默认甘特(「甘特是单 run 的透镜非一等页面」——A 论证成文);格角「打开 →」进旗舰页。**P2 第三脸=矩阵**(A 降级收编:`AnRunMatrix` 节点×近 20 run 格阵+列顶时长微条+三粒度选区,720 内横滚——「哪个节点历史上常坏」唯它能答;依赖工单⑩,随 S5)。
4. **triggers 调度陈列**(只观测,编辑深链 entities):每 trigger 一卡=kind 脸+cron 人话+**下 3 次具体时刻**(B 收编,crontab.guru 双保险,防「翻译对但表达式错」)+lastFiredAt+近 30 次 firing 竖条串(Uptime Kuma,有界+hover)+「N 次 skipped ▸」两级钻取 activations→firings(skipped/superseded/shed 处置词,灰不染红)+**[暂停/恢复] 止血开关**(盲区补,工单⑦:连败晨间第一需求是「先别让它 09:00 再跑一次」;归属裁决见 §15-Q1)。
...
3. **完整甘特**(AnNodeGantt 扩 props 非新件):时间刻度眉+hover 起止/毫秒+now 线;**running 条几何诚实延伸到 now**(C);**条分段:执行段(状态色,工单⑤真时长)+parked 琥珀段(frn created→completed 真区间)——排队灰段首发诚实缺席**(判官2 读码裁定:`flowrun_nodes` 零时间列,排队段=引擎手术,见 §15-Q5;无工单⑤时整体回退等宽顺序槽,不撒谎)。control 瞬时点。
...
四处窗口各说各话本身即「乱」——全部窗口进单源常量档 `SchedulerWindows`(core),i18n 句显式含窗口词:**珠串=近 10**;**矩阵=近 20**(P2);**失败聚合=7d 滚动**(凌晨 26h 前的失败不能漏窗——24h 否决);**KPI 失败牌=24h**(带 delta);**成功率/均时=7d**(工单③ `since` 参数统一)。run 历史保留/归档策略列 §15-Q4 记档,不阻塞首发。
...
批量操作(批量批准/批量 replay)首发不做,列 §15-Q2。
...
- `AnRunMatrix`(P2,随 S5):`({rows: [nodeId], cols: [RunColumn(id, elapsedMs, status)], cellStatus, onCell/onCol/onRow, selection})` — 节点×run 格阵+列顶时长微条+三粒度选区,虚拟化列窗,720 内横滚;同套 Semantics 要求。
...
- **⑤ 按 run 聚合活动时长(P1,甘特+台账)**:`GET /flowruns/{id}/activity?cursor&limit`(N4)→`[{nodeId, iteration, kind, execId, status, startedAt, endedAt, elapsedMs}]`;UNION 四张执行日志表按 flowrun_id(偏索引已备)。**诚实报价**:此工单只给执行段;排队段=引擎手术(frn 零时间列),显式不做,见 Q5。
- **⑥ 列表过滤器(P1)**:`GET /flowruns` 增 `?startedAfter&startedBefore&triggerId&origin=`(依赖①);非法值 422 大声拒带 allowed;零 schema 变更。
- **⑦ 暂停/恢复调度(P1,先调研)**:核实 trigger 域既有生命周期端点可否直用(listening/停用轴);缺则补 N5 `:pause`/`:resume`;语义=不再产生新 firing、在途 run 不受影响、nextFireAt 置空;归属拍板见 Q1。
...
- **⑨ misfire 策略+missed 呈现(P2,先调研后设计)**:sidecar 睡眠错过的 cron fire——补跑/跳过/标 missed?先查 trigger 域现状;产品面要求:时间线与 firing 串必须有 missed 第三态,Overview 不装作什么都没错过。桌面 app 区别于服务器调度器的第一现实。
- **⑩ 矩阵批量端点(P2,随 S5 矩阵第三脸)**:节点×近 20 run 状态阵一次取(判官2 裁定:T1 聚合给不了,逐 run 拉详情=N+1,须立单再建)。

**记档不立单**:排队段时间戳(引擎手术,若 P2 后仍要三段条再议);run 保留/归档策略(Q4);list 行 `pinnedRefs` 冗余重量(见性能痛再议 `?slim=`)。
...
- **S2 Overview**(依赖③④):KPI 四牌+delta/等你处理(AnCountdown+ApprovalGate 就地+first-wins 422 toast)/正在跑/失败聚合(连败语义+[最新 run]+墓碑)/未来调度简版行/零数据首用卡。**验收**:四区空态与满态真机帧;倒计时单 Timer(性能预算);牌全深链。
- **S3 运营主页**(依赖①②⑥⑦):健康头+大表(来源行身份+计数条过滤+pill 归位)+联动格双脸+triggers 陈列(下 3 次时刻+firing 串+skipped 钻取+暂停开关)+:kill/Run now。**验收**:过滤计数=真数;新 run 不插行走 pill;暂停后 nextFireAt 消失。
- **S4 run 旗舰+右岛**(依赖⑤,②③已备):卷宗头+ProvenanceLine+错误同句三投影/钉版图(顺修 run_cockpit 错图 bug)/完整甘特(执行+parked 段,running 伸 now,无⑤回退等宽槽)/台账(失败置顶自动展开+×N 折叠)/右岛双脸+迭代切换器+就地人闸·replay/冷打开推测态/孤儿墓碑。**验收**:三海拔单选区联动真机走查;冷打开在跑 run 不空白;650KB 注入电池不卡(build 预算)。
- **S5 增益**(依赖⑧⑨⑩):AnScheduleTrack 真时间轴(+missed)+矩阵第三脸+批量操作(若 Q2 拍板做)。**验收**:CustomPainter 两件 a11y 读屏走查+键盘遍历;矩阵虚拟化列窗性能预算。
...
## 16. 遗留 open questions(需用户拍板)

- **Q1 暂停调度的归属**:暂停/恢复算运行时权力(本海洋就地开关,推荐)还是配置(仅 entities)?推荐:开关就地、cron 表达式编辑仍归 entities;判官1 指出这是被三案共同回避的止血刚需。
- **Q2 批量操作形态**:10 个等人逐行批、5 个失败逐个 replay 不可扩展——要不要多选批准/按 workflow 批清?连带 first-wins 422 的批量语义。推荐首发不做,观察单行清账是否够用,S5 再议。
- **Q3 矩阵第三脸与全宽**:AnRunMatrix 降级为联动格第三脸后,720 内横滚是否够用,还是给矩阵/甘特全宽破例?独立视觉拍板项(判官3:矩阵是明亮语系里最重的异物,降为透镜后异物感消解——但需真机确认)。
- **Q4 run 历史保留策略**:runs 无限累积,归档/清理线(如 90d)与 T1 扫表成本——记档,不阻塞首发。
- **Q5 排队段三段条**:排队灰段需引擎补时间戳(frn 零时间列,record-once 语义下 replay 旧戳呈现也未定义)——首发两段(执行+parked)是否够,还是排期引擎手术?推荐够用,P2 后按真痛再议。
- **Q6 misfire 策略**(工单⑨的产品面):睡醒补跑还是跳过标 missed?推荐跳过+missed 记账(本地 app 补跑风暴危险),需后端调研后连①的 origin 词表一起定。
