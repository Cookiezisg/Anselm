...
- **⑩ 矩阵批量端点 ✅ 已入库**:`GET /flowrun-matrix?workflowId=X&recentN=20` → `{cols,rows,cells}`,**两条**有界查询答完整个格阵(近 N run 走 `idx_fr_ws_workflow` + 这批 run 全部节点行一条 `flowrun_id IN (…)` 走 `idx_frn_run`;零 schema 变更、绝不逐 run 拉详情)。**形定案**:cols 新→旧带 run `elapsedMs?`(在跑无 completed_at→键缺席);**rows 序=首次出现序**(扫列新→旧、run 内按执行序 `COALESCE(started_at,ready_at,created_at)`)——**刻意不用图拓扑序**:每 run 钉死自己的 version_id,跨版本一批没有单一的图,硬解一个即对其余撒谎;而首次出现序在要紧处天然就是拓扑序(run 的执行顺序=该 run 冻结图的一个拓扑序),故行读作最新 run 的拓扑、更老 run 独有节点追加在后;行带 `kind`(取最新一次出现);**cells 稀疏**(没跑到即无格,前端渲「未及」);**多迭代=一格聚合**:status 取各迭代**最坏**处置(failed>parked>completed,后来的绿轮不能抹掉第 3 轮的失败——run 头也是 failed)、同档取最新轮,带 `iteration`(胜出行的轮)+`iterations`(总轮数,前端 >1 才渲「×N」);**刻意无逐格 elapsedMs**(flowrun_nodes 无 ended_at,凑出来的是谎;真相在工单⑤ activity)。**有据偏差**:`recentN` 越上限**钳制**(默认 20/上限 20;非数字或 <1 → 400 `INVALID_REQUEST`)、**非工单原话的 422**——逐字沿用 flowrun-stats 同名参数语义:同域两个 `recentN` 行为分裂本身即「乱」,且窗口是**视图**、钳它=渲 20 列而非 25=视觉降级非撒谎(对比 `workflowIds` >50 大声拒:静默截断请求 id **会**撒谎,客户端 1:1 对拉会把短答案读成完整);前端本就只发 SchedulerWindows 常量 20。零新错误码。有界批查 N4 豁免已登记 api.md 通则。
