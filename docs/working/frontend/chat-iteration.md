...
>
> **本册仍未归档**,因为归档闸是「真机验收过一遍」,而余下的是一批**只有人能判的视觉观感项**:WD1/WD3 的驻地按钮与护栏截图 · ES 批 `fieldList` 空表那行「Inputs —」是否读作重复 · RI 做法#3(窄窗下 transcript 跨重排是否仍有可感漂移)· TS 装饰性元数据(时间戳/计数)是否该退出选区 · 驻地按钮在极长目录名下的截断。**全部已收拢进** `docs/working/multimodal-agent/ACCEPTANCE-GUIDE.md` 的 C 类(C4/C5),那里是唯一待办清单。
>
...
**诚实边界(0725 当时)**:这是否就是用户那次报告的成因,尚未确证——确证需要真机。

→ **真机验收通过(0725 深夜,用户机器)**:`make -C backend seed && make -C frontend app` → composer 打 `@` → **面板正常弹出**,8 条候选跨 function/handler/agent/workflow/skill 五类、各带 kind 图标与描述;续打 `cyber` 收敛到 5 条(`cat_*` 亦命中,说明检索不是前缀匹配);点选 `cyber_cat_interactive` → **药丸正常插入** composer(蓝色 `@cyber_cat_interactive`)。**全链无缺**。

故 **嫌疑②③(`mention_names.dart` 的 NUL 转义 / OverlayPortal 宿主变化)就此排除**——它们若成立,面板不会在同一条路径上完好弹出。用户当时那次的成因**最可能就是嫌疑①**(search 扇出快速失败 → 静默关面板),该缺口已修且配了反证过的回归测试;即便当时另有他因,那个成因也已随本轮的 Flutter 3.44 升级、CR/RI 结构修复与本次故障隔离一并消失,现状是**功能完好**。
...
| # | 批 | 谁 |
|---|---|---|
| ⓪ | ~~CH-0 @ 提及回归~~ **已完成 0725**——嫌疑①(故障隔离缺口)查实并修;**真机全链复现通过**(`@` 弹面板 8 候选跨四类 → 输入收敛 → 选中插药丸),嫌疑②③ 就此排除,见 §5.16 | 主会话 |
