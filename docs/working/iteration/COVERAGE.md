...
| DF | 69 | 45 | 13 | 11 | 0 |
| E | 58 | 0 | 13 | 45 | 0 |
| **合计** | **645** | **252** | **121** | **230** | **42** |

**当前覆盖率**（locked+probed+exempt / 总）≈ **64.3%** · 目标 ~99%（unprobed → 全部翻绿或显式 exempt）。
...
| A-ws-3 | workspace 列表 cursor 往返·limit 边界 | unprobed | 建两页 ws 走 cursor 断不重不漏；域10端 |
...
| A-ws-8 | workspace POST/PATCH 带未知字段拒收/吞噬行为 | unprobed | POST 带多余字段观察 400 vs 静默吞；域10端 |
...
| A-key-3 | apikey 列表 cursor 往返·limit 边界 | unprobed | 多 key 翻页断序；域6端 |
| A-key-4 | apikey N1：空列表[]/204 删/掩码回显形 | unprobed | 零 key 空列表+DELETE 204 形状断言；域6端 |
| A-key-5 | apikey 跨 ws 404 | probed | ARCHIVE绿格·D2跨ws全404；域6端 |
| A-key-6 | apikey 软删：删后名字复用+列表过滤 | unprobed | 删→同名重建→列表只见新行；域6端 |
| A-key-7 | apikey :test 探活两态 | locked | TestPlatform_APIKeyProbeAndGuards；域6端 |
| A-key-8 | apikey 严格拒未知字段 | unprobed | POST 带杂字段；域6端 |
...
| A-model-4 | model N1：零 key 时 capabilities 空态形状 | unprobed | 聚合形已在 ModelConfig 锁,空态未打；域3端 |
| A-model-5 | model capabilities 跨 ws 不串（按各 ws key 聚合） | unprobed | 两 ws 各配 key 断聚合独立；域3端 |
...
| A-lim-7 | limits :reset 恢复 Default 并热换 | unprobed | PATCH 改值→:reset→读回默认+消费方生效；域4端 |
| A-lim-8 | limits PATCH 未知字段行为（部分合并是否吞） | unprobed | PATCH 带杂字段观察 400/吞；域4端 |
...
| A-sbx-3 | sandbox envs/runtimes 列表 cursor 往返 | unprobed | 多 env 翻页；域14端 |
| A-sbx-4 | sandbox N1：单 env 销毁 204 形 | locked | TestPlatform_SandboxGovernance；域14端 |
| A-sbx-5 | sandbox 隔离：envs 跨 ws 可见性（runtimes 机器级） | unprobed | 两 ws 各建 fn 断 envs 列表互不见；域14端 |
| A-sbx-6 | sandbox 软删 | exempt | env/runtime 物理销毁无软删；域14端 |
| A-sbx-7 | sandbox 动作::gc/:retry-bootstrap/对话 scratch {kind}:reset·:reset-all | unprobed | :gc 已锁(TestPlatformR4_SandboxRuntimesGCDisk)；:retry-bootstrap+scratch reset 全未打；域14端 |
| A-sbx-8 | sandbox POST runtimes 拒未知字段 | unprobed | 装 runtime 带杂字段；域14端 |
| A-free-1 | freetier GET /quota happy（网关代理 {limit,used,remaining,resetAt,available}） | probed | 免费档 0629 客户端端到端实测 LIVE(记忆格)；域1端 |
| A-free-2 | freetier 错误面：无受管行 404 FREETIER_NOT_PROVISIONED+网关错原样冒泡 LLM_* | unprobed | 删受管行后打 quota 断 404 码；域1端 |
| A-free-3 | freetier 分页 | exempt | 单资源；域1端 |
| A-free-4 | freetier N1 quota data 形状 | unprobed | 断五字段全在 envelope data 内；域1端 |
| A-free-5 | freetier 跨 ws（受管 key 按 ws） | unprobed | 无受管行 ws 打 quota 断 404；域1端 |
...
| A-fn-3 | function list/versions/executions cursor 往返·limit 边界 | unprobed | 多版本+多执行翻页断不重不漏；域13端 |
...
| A-fn-8 | function POST/PATCH 拒未知字段 | unprobed | 创建带杂字段观察吞/拒；域13端 |
...
| A-hd-3 | handler calls/versions cursor 往返 | unprobed | 多调用翻页；域17端 |
...
| A-hd-6 | handler 软删：名字复用+列表过滤+env 销毁 | unprobed | 删→同名重建→旧调用台账 D1 在；域17端 |
| A-hd-7 | handler 动作::call/:restart/:edit/:revert/:iterate+config PUT(Merge Patch/null 删 key)/DELETE 停机 | unprobed | 锁 :call·:restart(ResidentLifecycle)·:edit(EditPersistsMeta)·config 三端(ConfigFlow)；:revert/:iterate 未打(F20 revert meta 仅 fixed)；域17端 |
| A-hd-8 | handler 拒未知字段 | unprobed | 创建带杂字段；域17端 |
...
| A-ag-3 | agent executions/versions cursor 往返 | unprobed | 多执行翻页；域14端 |
...
| A-ag-6 | agent 软删：名字复用+列表过滤 | unprobed | 涟漪3面在 TestRippleR5_CreateRenameDeleteMatrix；名字复用/列表过滤未直打；域14端 |
...
| A-ag-8 | agent :edit 全量 Config 拒未知字段/坏挂载 ref 即拒（F96 eager 校验） | unprobed | F96/F98 已锁 dangling 校验,未知字段面未打；域14端 |
...
| A-wf-3 | workflow versions/list cursor 往返 | unprobed | 多版本翻页；域16端 |
...
| A-wf-6 | workflow 软删：名字复用+列表过滤 | unprobed | 涟漪面见 R5 矩阵；名复用未直打；域16端 |
| A-wf-7 | workflow 动作九连::trigger/:stage/:activate/:deactivate/:kill/:edit/:revert/:capability-check/:iterate | unprobed | 锁 :trigger·:edit·:revert·:capability-check(LinearRun+ReferenceRipples,F35族)+:activate/:deactivate 经 trigger 场景隐锁；:stage 仅探绿(stagewf 格,active→409 未打)；:kill 全未打；域16端 |
| A-wf-8 | workflow 拒未知字段（F42 静默吞 concurrency 已修族） | unprobed | F42 锁的是无效值,未知字段面未打；域16端 |
...
| A-run-2 | flowrun 错误面：decide first-wins 输家 422/坏 status 枚举 400/未知 run 404 | unprobed | 双 decide 竞速断 422；域6端 |
| A-run-3 | flowrun GET{id} 节点 keyset 分页 cursor 往返（长 loop 数千行） | probed | F168-M7 fixed(keyset分页)无零 token 锁；list cursor 未打；域6端 |
| A-run-4 | flowrun N1：POST /flowruns 202+entryNode 消歧 | unprobed | GET{id} 复合形经 wf 场景隐锁；POST /flowruns 直入口+entryNode 全未打；域6端 |
...
| A-run-8 | flowrun decide body 拒未知字段/decision 枚举外值 | unprobed | decide 带杂字段+decision=maybe；域6端 |
...
| A-trg-3 | trigger activations/firings cursor 往返+?status 过滤枚举 | unprobed | 多 firing 翻页+status 全枚举打一轮；域10端 |
| A-trg-4 | trigger N1：:fire 202 {data:{id}} activation 单 id 闭环 | unprobed | :fire→拿 id 直查 activation；域10端 |
| A-trg-5 | trigger 跨 ws 404 | probed | ARCHIVE绿格·D2跨ws全404；域10端 |
| A-trg-6 | trigger 软删：行软删名复用（activation/firing 为 log 表无软删） | unprobed | 删→同名重建→旧 activation 保留；域10端 |
| A-trg-7 | trigger 动作::fire/:iterate 逐打 | unprobed | :fire REST 面全未打(F94 修的是 tool 面)；:iterate 族仅 F36 锁 404 面；域10端 |
| A-trg-8 | trigger 拒未知字段（F14 宽容 env 不校命名空间已修族） | unprobed | 建 trigger 带杂字段；域10端 |
...
| A-ctl-3 | control versions cursor 往返 | unprobed | 多版本翻页；域10端 |
| A-ctl-4 | control N1：空列表[]/versions 形状 | unprobed | 零 control 空列表断 []；域10端 |
| A-ctl-5 | control 跨 ws 404 | probed | ARCHIVE绿格·D2跨ws全404；域10端 |
| A-ctl-6 | control 软删：名字复用+列表过滤 | unprobed | 删→重建同名；域10端 |
| A-ctl-7 | control 动作::edit/:revert/:iterate 逐打 | unprobed | 锁 :edit(v2 可读回)；:revert/:iterate 未打；域10端 |
| A-ctl-8 | control 拒未知字段 | unprobed | 建带杂字段；域10端 |
...
| A-apv-3 | approval versions cursor 往返 | unprobed | 多版本翻页；域10端 |
| A-apv-4 | approval N1：空列表[]形状 | unprobed | 零 approval 断 []；域10端 |
| A-apv-5 | approval 跨 ws 404 | probed | ARCHIVE绿格·D2跨ws全404；域10端 |
| A-apv-6 | approval 软删：名字复用+列表过滤 | unprobed | 删→重建同名；域10端 |
| A-apv-7 | approval 动作::edit/:revert/:iterate 逐打（运行时 decide 在 flowrun 域已锁） | unprobed | 三动词全未打；域10端 |
| A-apv-8 | approval 拒未知字段 | unprobed | 建带杂字段；域10端 |
...
| A-skl-2 | skill 错误面：POST 重名 409/未知 name 404 | unprobed | 双 POST 同名断冲突码；域6端 |
| A-skl-3 | skill 列表 cursor 往返 | unprobed | 多 skill 翻页；域6端 |
| A-skl-4 | skill N1：空列表[]/204 删形 | unprobed | 零 skill 断 []；域6端 |
| A-skl-5 | skill 跨 ws 404 | probed | ARCHIVE绿格·D2跨ws全404；域6端 |
| A-skl-6 | skill 删后同名重建语义（name 即 id 无软删名冲突） | unprobed | 删→PUT 同名→断新内容生效；域6端 |
...
| A-skl-8 | skill 拒未知字段 | unprobed | PUT 带杂字段；域6端 |
...
| A-mcp-3 | mcp calls cursor 往返（registry 列表即全量无分页） | unprobed | 多调用翻页；F91 已锁 registry query 过滤；域12端 |
...
| A-mcp-8 | mcp PUT 拒未知字段（F169 env required/optional 已修族） | unprobed | PUT 带杂字段；域12端 |
...
| A-doc-3 | document ?parentId 列表 cursor 往返（tree 设计无分页） | unprobed | 多子节点翻页；域9端 |
| A-doc-4 | document N1：:duplicate 201 返新根裸实体/tree 无 content 形 | unprobed | duplicate 后断裸实体+名自动去重；域9端 |
| A-doc-5 | document 跨 ws 404 | probed | ARCHIVE绿格·D2跨ws全404；域9端 |
| A-doc-6 | document 软删：名字复用+列表过滤+子树归属 | unprobed | 删父后子归属+同名重建；域9端 |
| A-doc-7 | document 动作::move(防环/nil=根)/:duplicate(深拷+parentId)/:iterate 逐打 | unprobed | :move F21 已锁(单测)+探绿；:duplicate 全未打；:iterate 未打；域9端 |
| A-doc-8 | document 拒未知字段 | unprobed | 建带杂字段；域9端 |
...
| A-conv-8 | conversation PATCH 拒未知字段 | unprobed | PATCH 带杂字段；域7端 |
...
| A-chat-8 | chat Send body 拒未知字段/action 枚举外值 422 | unprobed | Send 带杂字段+interactions action=whatever；域6端 |
...
| A-att-4 | attachment N1：GET content 直出/DELETE 204 形 | unprobed | content 字节等值+删 204；域4端 |
| A-att-5 | attachment 跨 ws 404 | probed | ARCHIVE绿格·D2跨ws全404；域4端 |
| A-att-6 | attachment DELETE 后引用行为（历史消息引用悬空语义） | unprobed | 删附件后旧对话再续断诚实报缺；域4端 |
...
| A-mem-2 | memory 错误面：未知 name 404/坏 name 校验 | unprobed | GET 不存在 name 断码；域6端 |
| A-mem-3 | memory 列表 cursor 往返 | unprobed | 12 写绿格未走 REST 分页；域6端 |
| A-mem-4 | memory N1：空列表[]/PUT upsert 返回形 | unprobed | 零 memory 断 []；域6端 |
| A-mem-5 | memory 跨 ws 隔离 | probed | ARCHIVE绿格·memory深用隔离；域6端 |
| A-mem-6 | memory 软删 | exempt | forget=物理删 by design,已锁(MemoryLLMFace)；域6端 |
| A-mem-7 | memory pin/unpin 逐打 | unprobed | pin 已锁(MemoryLLMFace)；unpin REST 未直打；域6端 |
| A-mem-8 | memory PUT 拒未知字段 | unprobed | PUT 带杂字段；域6端 |
...
| A-srch-8 | search PATCH settings 拒未知字段 | unprobed | PATCH 带杂字段；域4端 |
...
| A-ntf-2 | notification 错误面：未知 id mark-read 404 | unprobed | 打不存在 id 断码；域4端 |
| A-ntf-3 | notification 列表 cursor 往返 | unprobed | 大量通知翻页；域4端 |
...
| A-rel-2 | relation 错误面：neighborhood 未知中心 id 行为 | unprobed | 打不存在 id 断码/空图；域4端 |
| A-rel-3 | relation list cursor 往返 | unprobed | 14实体规模绿格未走 REST 分页；域4端 |
| A-rel-4 | relation N1：空图形状/relgraph 全景形 | unprobed | 零边断 []；域4端 |
...
| A-todo-2 | todo 错误面：未知 conversationId 404 | unprobed | 打不存在对话断码；域1端 |
| A-todo-3 | todo 分页 | exempt | api.md 未列分页参数,对话级小清单；域1端 |
| A-todo-4 | todo N1：空清单形状 | unprobed | 零 todo 断 []；域1端 |
...
| A-tp-4 | touchpoint N1：空台账[]形状 | unprobed | 新对话零触点断 []；域1端 |
| A-tp-5 | touchpoint 跨 ws 404 | unprobed | 新域晚于 D2 绿格探针,跨 ws 读断 404；域1端 |
...
| A-sys-2 | loopback 双门：坏 token 401 UNAUTH_BAD_TOKEN/坏 Host 403 FORBIDDEN_BAD_HOST/webhook·OPTIONS 豁免/health 不豁免 bearer | unprobed | 设 ANSELM_AUTH_TOKEN 起服打错 token+伪 Host 头逐断；testend 恒不设 token 即零覆盖；域2端 |
| A-sys-3 | system 分页 | exempt | 无列表端点；域2端 |
| A-sys-4 | system N1：health envelope 形/data-dir {dataDir} 形 | unprobed | health 200 隐锁但 envelope 未显断言；域2端 |
...
| A-sse-5 | SSE 跨 ws 事件不串流（流 workspace 级、ws 内不过滤 by design） | unprobed | 两 ws 并行造事件断各自流零串；域3端 |
...
| B-fn-3 | trim 越 cap 时回收被删版本的 per-version venv(reclaimTrimmedEnvs 经 DestroyEnv) | unprobed | 造51版后查 sandbox disk-usage/env 目录不残留 |
...
| B-fn-6 | 词法校验黑名单 import anselm_handler(无状态/有状态边界)创建即拒 | unprobed | create 带该 import 断言 INVALID_CODE |
| B-fn-7 | envfix 修复不丢包:缩短依赖数的"修复"被拒、env 留 failed+真实装错、声明 deps 不丢 | probed | F148 |
| B-fn-8 | env failed 不阻塞创建(实体建成+状态可见)、run 时才报 FUNCTION_ENV_NOT_READY | unprobed | 坏依赖 create 成功→读版本 env=failed→:run 断言码 |
| B-fn-9 | Edit 空 ops=重建 active env 重试安装+发 function.env_rebuilt | unprobed | 空 ops :edit 断言事件+版本号不变 |
| B-fn-10 | nil input 在 runner 前归一 {}(sensor/无接线节点无参调用不 TypeError) | unprobed | :run 无 input 的无参函数断言 ok |
| B-fn-11 | env 被 GC 回收(ErrEnvNotFound)时自动重建 env+重试一次 | unprobed | sandbox:gc 后直接 :run 断言成功 |
...
| B-hd-3 | 纯 meta edit/update_handler_meta 不铸版本不重启——内存态跨改名存活 | unprobed | 计数器 handler 改名后 self 计数不归零 |
...
| B-hd-6 | spawn 咽喉按 active schema 过滤孤儿 config key(revert 遗留 kwarg 不永久炸 __init__) | unprobed | 配 key→edit 删该 arg→revert→调用仍 spawn 成功 |
| B-hd-7 | spawn 单飞:并发首调共享一次 in-flight spawn、全批调用同一 instance_id | unprobed | 冷启并发 5 调断言 call 行 instance_id 唯一 |
...
| B-hd-11 | generator 终值:最后一个非 progress yield 或 return 值(StopIteration.value)两式都生效 | unprobed | yield 终值与 return 终值两 method 各断言结果 |
...
| B-ag-4 | mount-health 预检:逐挂载独立收集非 fail-fast、撞名对称标 unhealthy、knowledge doc 也覆盖 | unprobed | 删 doc/造撞名后 GET mount-health 断言行级 unhealthy |
...
| B-ag-10 | AgentInvokeSec 整次运行墙钟:超时压过 loop 自报→ExecutionStatusTimeout(durable 可 replay) | unprobed | 慢 mock 流+调小 AgentInvokeSec 断言 timeout 状态 |
...
| B-ag-14 | 嵌套人在环:agent 内 dangerous 工具经父 broker 阻塞至用户 resolve(嵌套不冒泡) | unprobed | invoke_agent 挂 dangerous 工具断言 interaction 挂起+decide 续跑 |
| B-wf-1 | ValidateGraph:无 trigger 节点/孤儿节点带 WORKFLOW_INVALID_GRAPH+reason 拒 | locked | TestWorkflow_GraphValidationRejections |
| B-wf-2 | 环纪律:回边必出自 control/approval,其它源回边创建即拒 | unprobed | fn→fn 回边图 create 断言 INVALID_GRAPH |
...
| B-wf-7 | 手动 :trigger/trigger_workflow 绕过并发策略立即建 run(两手动 run 可同途即便 replace) | unprobed | replace 策略下连发两次 :trigger 断言双 run 在途 |
...
| B-wf-10 | 活监听重绑:active workflow edit/revert 改入口 trigger ref 即 detach 旧 attach 新 | unprobed | 换 trigger 后 fire 旧/新各断言不触发/触发 |
...
| B-wf-12 | :deactivate 在途不杀→draining 由 run 结算收口翻 inactive;:kill 取消全部在途 | unprobed | 长跑 run 中 deactivate 断言跑完+状态收口 |
...
| B-trg-5 | 引用计数监听:N 个 active workflow 共享一个 listener(0→1 起、1→0 停) | unprobed | 两 workflow 挂同 cron 断言单 listener 双 firing、全 deactivate 后不再 fire |
...
| B-trg-8 | webhook 明文式两载体(X-Webhook-Secret 头 / ?token= 查询)+signatureHeader 可改头名 | unprobed | 三种携带方式各 POST 断言触发/401 |
...
| B-trg-12 | Edit 热更:监听中 trigger 新 config 重 Register、webhook 改路径后旧路径 404(catch-all registry 派发) | unprobed | 改 config.path 后旧/新路径各 POST 断言 404/202 |
...
| B-ctl-2 | 末条分支必须 When=="true" 兜底,缺 catchall 创建拒 CONTROL_NO_CATCHALL | unprobed | 无兜底分支 create 断言码 |
| B-ctl-3 | Port 非空且组内唯一,违者拒 CONTROL_INVALID_BRANCHES | unprobed | 重复 Port create 断言码 |
...
| B-ctl-8 | 运行时 Resolve 按钉死版本求值——在途 run 不吃编辑后的新分支 | unprobed | run park 期间 edit control 断言续跑仍走旧分支 |
...
| B-apf-4 | timeout=""=永不超时:run 长 park 不被任何定时器决策 | unprobed | 空 timeout park 后等待期断言仍 parked |
...
| B-apf-9 | ParseTimeout 扩展 d/w 粗粒度时长解析 | unprobed | timeout="2d" create 读回断言接受 |
| B-sk-1 | slug 正则既是身份也是路径穿越守卫:含 ../ 等非法 name 创建拒、合法名 1:1 映射目录 | unprobed | name 带路径分量 create 断言拒+盘上无逃逸目录 |
| B-sk-2 | body 自带 frontmatter 块(--- 开头)拒 SKILL_INVALID_FRONTMATTER、孤立 --- 分隔线放行 | unprobed | 双 frontmatter body 与孤立 --- 各 create 断言拒/过 |
...
| B-sk-5 | 刻意不支持 !`cmd` shell 注入——含该语法的 body 激活不产生任意执行 | unprobed | body 带 !`touch marker` 激活断言无执行痕迹 |
| B-sk-6 | fork 模式派隔离 subagent 跑正文同步拿回、sub-message 带 subagentId 落父对话 | locked | TestChatR3_SkillForkRoute |
| B-sk-7 | fork 缺 frontmatter.agent 拒 SKILL_FORK_REQUIRES_AGENT | unprobed | 无 agent 字段 skill :activate fork 断言码 |
| B-sk-8 | Guide(agent 挂载路径)只渲正文:不设 active-skill(预授权不泄父对话)、不 fork、不接 $ARGUMENTS | probed | F57 + ARCHIVE绿:skillagent指南真注入嵌套run |
| B-sk-9 | 创作面:create 同名 SKILL_NAME_CONFLICT、Replace 缺失 404 | unprobed | 同名两次 create+改不存在 skill 各断言码 |
| B-sk-10 | 坏 SKILL.md 文件扫描跳过不连坐——List 仍返其余健康 skill | unprobed | 盘上手写坏文件后 List 断言其余在场 |
...
| B-mcp-14 | 默认 call 超时 180s 长顶棚真切停滞工具调用 | unprobed | 挂 sleep 的脚本 server 调工具断言超时收尾非永挂 |
| B-doc-1 | Create 重名自动后缀 foo→foo 2(cap100 重试)、PATCH 显式改名严格 DOCUMENT_NAME_CONFLICT | unprobed | 同名两次 POST 断言后缀+PATCH 撞名断言码 |
| B-doc-2 | 并发同父 create:position 单事务 max+1 原子赋不撞车 | probed | F61 |
| B-doc-3 | 改名→子树 path 批量级联重写(后裔 path 全跟新) | unprobed | 三层树改中层名后按新 path 读孙节点 |
...
| B-doc-7 | attach 单篇不拖子树:挂载显式有界、子树不自动注入 | unprobed | 挂父 doc 后 promptdump 断言子 doc 正文不在场 |
...
| B-doc-10 | :duplicate 深拷子树:BFS 铸新 id 重映射 parent/path、复制 content、新根名去重 | unprobed | 复制三层树断言新 id 树形/原树不动 |
...
| B-doc-13 | 标题 ≤256 字符且不含 /(path 分隔符)守卫 | unprobed | 带 / 与超长标题 create 各断言拒 |
...
| B-chat-3 | 回合收尾活期间的 Send 落单槽缓冲、紧随其后被服务,槽满仍 409 | unprobed | 收尾窗口(真 utility 压缩检查拖秒级)并发 Send 断被服务或 409 |
| B-chat-4 | convQueue 5 分钟 idle 自毁+新 Send 重建;拆卸与投递 q.mu 原子互斥(task 不滞留死 channel) | unprobed | Go 单测注入短 idle 计时+并发投递竞态 |
| B-chat-5 | 回合总墙钟 ChatTurnSec:detached ctx 上过每步守卫的永跑回合被切、不卡 isGenerating/shutdown | probed | F100(round-5 实测验证) |
| B-chat-6 | Shutdown 即时:cancel 全部在跑回合+stop 信号短路每个队列(不等 idle timer) | unprobed | 流式中优雅停机断秒级退出+回合落 cancelled 终态 |
...
| B-chat-8 | LoadHistory 排除 subagent 子消息(`subagent_id≠''` 下推 SQL,内部 trace 不污染父历史) | unprobed | subagent run 后 promptdump 断父模型视角无子块 |
| B-chat-9 | AutoActivator:LLM 直接点名 lazy 工具即自动 discovered、免先跑 search_tools | locked | TestChat_SendStreamToolRoundtrip |
| B-chat-10 | WriteFinalize 恒 Detached+boot SweepOrphans:硬崩溃(kill -9)孤儿 pending/streaming 扫成 cancelled | unprobed | chat 流式中 kill -9 同目录重启断无 streaming 残留行 |
...
| B-chat-14 | approve_always 对话级白名单+删对话 ForgetConversation 整批清(授权不越过删除泄漏内存);:cancel 保留白名单 | unprobed | approve_always 后删对话再建同名对话断同工具重新要确认 |
| B-msg-1 | progress 一等持久块:落盘+实时流,但 LLM 历史投影类型白名单永不回喂 | unprobed | 带 progress 的工具跑完 promptdump 断模型历史无 progress 块 |
...
| B-msg-3 | 两表 append-only(D1):删对话后 messages/message_blocks 行物理留存 | unprobed | 删对话直查 DB 断行仍在 |
| B-msg-4 | SubagentID 双面:ListMessages 保留 sub-message 供 reload 重建子树 | locked | TestChatR3_SubagentNestedTree |
| B-msg-5 | ContextRole 是压缩器对块的投影变更,落库 Content 永不改写 | unprobed | 压缩后直读 DB 断 block 原文逐字未变 |
| B-msg-6 | stop_reason context_budget:回合 input 逼近 context window 时 loop 软停诚实终态 | probed | F58 |
| B-msg-7 | SweepNonTerminal boot 对账:pending/streaming → cancelled(messages 版 Recover) | unprobed | 同 B-chat-10 kill -9 场景合测 |
...
| B-conv-1 | PATCH ModelOverride 指针三态:缺=不变/null=清除/对象=设置 | unprobed | PATCH 矩阵三态读回断言 |
...
| B-conv-5 | activity 键 last_message_at 只随用户回合刷新(pin/改名/换模型不重排);未知/空 sort 静默落 activity 不 400 | unprobed | PATCH 元数据后 list 序不变+garbage sort 值矩阵 |
| B-conv-6 | ?search= title 大小写不敏感子串+%/_ 通配转义、不改排序键;换 search/切排序旧游标失效行为 | unprobed | 照 TestFunction_ListSearch 克隆到 conversations+异构旧 cursor 断不 500 |
| B-conv-7 | isGenerating 派生只读:在途生成时 List/Get 冷启动即填 true、不落库 | unprobed | 流式中 GET list 断 isGenerating=true、完后 false |
...
| B-sub-4 | Plan(+WebFetch/WebSearch,25轮)/general-purpose(父全集减 Subagent,25轮)白名单+轮上限 | unprobed | 三类型各 promptdump 工具集断言+轮耗尽终态 |
| B-sub-5 | 被取消的 subagent 仍落终态 sub-message 防孤儿(chatHost 系 Detached 落盘) | unprobed | 父 :cancel 中断在跑 subagent 后查子消息终态 |
| B-sub-6 | subagent 模型=workspace dialogue、刻意不承袭 per-conversation override | unprobed | 设对话 override 后 spawn 断请求落默认 mock 队列 |
...
| B-att-1 | CAS dedup:相同字节重复上传共享一 blob(sha256 非唯一多行共享)、GC 按活跃 sha 保留集回收 | unprobed | 双上传断盘上单 blob;删一行后 GC 不收、删两行后收 |
| B-att-2 | KindFromMIME 六桶分类+文件扩展名兜底 | unprobed | 上传 mime/扩展名矩阵断 kind 字段 |
| B-att-3 | 渲染按模型能力三路门控:vision→image_url/NativeDocs→file part/否则 sandbox 抽取内联 | locked | TestChatR3_AttachmentsThreeRoutes |
| B-att-4 | 不认的 mime → ATTACHMENT_EXTRACTION_UNSUPPORTED 降级占位、回合不失败 | unprobed | 上传怪 mime 后发消息断回合 completed+占位文案 |
| B-att-5 | 缺失/不可读 blob 告警跳过、绝不让回合失败 | unprobed | 手删盘上 blob 后发消息断回合完成 |
...
| B-att-8 | upload→download(:id/content) 字节 round-trip+软删后 get/download 404 | unprobed | REST round-trip 矩阵 |
| B-att-9 | catalog source:活跃附件报成 name+kind/mime/size 条目入 system prompt 目录 | unprobed | 上传后 promptdump 断附件条目在场 |
...
| B-todo-3 | reminder 0-open 抑制:全完成清单不再逐轮注入 | unprobed | 全完成后 promptdump 断无 reminder 段 |
| B-todo-4 | todo_read(常驻)读回当前作用域整张含已完成项、空清单软返 cleared 串 | probed | F39 |
| B-todo-5 | subagent run 独立作用域(scope_id=subagent id、父对话清单不动)+看板 ?subagentId= 读回 | unprobed | subagent 内 todo_write 断父清单不变+GET todos 双参 |
| B-todo-6 | ≤64 项上限:超出按 TODO_* 码拒(规划异味信号) | unprobed | 65 项 todo_write 断拒 |
| B-todo-7 | 写入即推 messages 流 todo 信号(前端实时面板) | unprobed | SSE 收 todo 信号帧断 payload |
| B-rel-1 | equip 边现身/relgraph 全景/Namers hydrate 改名读侧跟名 | locked | TestPlatform_RelationRipple / TestRippleR5_RelationGraphFaces |
| B-rel-2 | diff-sync 终态幂等:edit/revert 换挂载后旧边消失(声明集合对照增删,HTTP 与 agent 席均未锁) | unprobed | edit_agent 换挂载断旧边除名新边现身 |
...
| B-rel-6 | 守卫:自环禁止/ref 校验/邻域 depth 1-3 限制按 REL_* 码拒 | unprobed | REST 矩阵(自环边/depth=4/坏 ref) |
...
| B-rel-9 | hydrate/emit 失败只记录、绝不让删除失败(nil 容忍) | unprobed | Go 单测注入 failing namer/emitter 断删除照常 |
...
| B-srch-10 | 换 embedder 逐行 model 记账自动重嵌不混用+fts_schema_version 不匹配 boot 清空重建 | unprobed | PATCH 换 embedder 断旧行重嵌;篡改版本行重启断重建 |
...
| B-sup-4 | apikey:旋转自动重探(失败不挡 PATCH)/受管行 Update 422 API_KEY_IMMUTABLE/apiFormat 白名单 400 | unprobed | REST 矩阵三分支(注错 tester 仍 200) |
| B-sup-5 | freetier 全链:指纹铸 gwk_→CreateManaged 受管行(跳探针)→真补全→quota 扣减;每失败路径降级绝不挂 boot | probed | 0629 客户端侧 LIVE 端到端手测 |
| B-sup-6 | freetier quota 代理契约:无受管行 404 FREETIER_NOT_PROVISIONED/remaining 钳≥0/available 折全局预算/网关错按 LLM_* 分类冒泡 | unprobed | llmmock 伪网关断四分支 |
...
| B-sup-10 | webFetchMode local/jina(读不到收敛 local)+配真搜索 backend 后的 WebSearch 行为 | unprobed | frontier「websearch 真后端席」整列空白;伪 backend+PATCH 矩阵 |
...
| B-tp-6 | TouchEntity 自报路:挂载 fn/hd/mcp 以实体名运行自报 {kind,id,name} 记 executed、完全绕目录(用户实体撞目录键名不误提取) | unprobed | agent 挂载工具执行断 executed 行+同名撞键负控 |
| B-tp-7 | subagent 内触碰记到父对话名下+失败调用不记(失败的触碰不是触碰) | unprobed | subagent 建实体断父台账;注入必败工具断零行 |
| B-tp-8 | deleted 行兄弟借名快照(名字仍诚实可显)/没碰过就删的孤儿行诚实空名 | unprobed | 删已 executed 实体断 deleted 行带名;直删断空名 |
| B-tp-9 | 未知对话 GET touchpoints 返回空页非错(同 todos) | unprobed | GET 未知 id 断 200 空页 |
