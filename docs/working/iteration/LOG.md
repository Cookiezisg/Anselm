...
| F11 | open | 嵌入 provider 对 >512-token chunk 报 "input (590 tokens) too large, physical batch 512" → 语义搜索**静默退化 lexical**（仅 INFO 日志，搜索质量降级隐形） | 嵌入 server（llamasrv，`infra/sandbox/direct.go:707` 启动 flags）或 chunking | llamasrv 加 `--ubatch-size` ≥ 最大 chunk token（默认 ubatch 512），或 chunk 切 ≤512 token 再嵌 | — | — |
| F12 | open | chat 回合在大上下文 workspace（ws_dc44 累积大量实体）卡 `streaming`（0 block）>5min；deepseek `/models` 正常、其他后端请求正常 → 疑该 chat 请求 hang，idle-timeout(150s) 在 0-block 流上似未生效 | 待 CONFIRM | 查 LLM idle timeout 是否在 0-block 流上计时 | — | — |
