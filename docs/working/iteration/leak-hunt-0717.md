...
> **③ 存量库一次性迁移**（`EnsureIncrementalAutoVacuum`,`openDB` 在 Migrate 之后仅文件库调）：修复前安装文件头带 `NONE`,光 PRAGMA 翻不动 → 一次全量 `VACUUM` 用指针图重写文件,**同时回收攒下的死空间**（实测存量 NONE 库:123MB dead → `VACUUM` 后 mode=2、reclaim 123MB、**跨重开持久**、第二次调用幂等 no-op、零丢行）。**尽力而为**——`VACUUM` 需约等于库大小的临时空间,磁盘将满的用户（正是修复对象!）可能没有 → 失败只 `log.Warn` 不令 boot 失败、下次 boot 重试。
...
> **⑥ D1 裁定:不是物理删例外、无需新立法**——`VACUUM`/`incremental_vacuum` 都不删任何**逻辑行**,只把 `PurgeTerminalRunsBefore`(例外②,已立法)**已经腾空**的页还给 OS。纯空间回收 ≠ 物理删。文档:`database.md` flowrun 节 + `foundation/platform-pkgs.md`(infra/db 唯一事实源)+ `scheduler-flowrun.md` §4.2 + `bootstrap.md` Boot/Shutdown 同步。
> **守卫**:`vacuum_test.go` 真落盘库三测——主测〔全新库天生 mode=2 → 删 80% → `os.Stat` 断言**光删不缩**(棘轮/bug) → `ReclaimFreePages` 后文件**真缩**(61MB→12MB,49MB 归还)且**3000 行完好**〕+ 回收闸挡住日常 churn(删 5% → reclaim=0、文件不动)+ 存量 NONE 库迁移(→mode=2、reclaim>0、持久、幂等、零丢行)。
> **交你拍板的产品决策(未自作主张碰前端)**:见下「产品决策」栏——(a) 迁移 VACUUM 的 boot 时机(同步/异步)、(b) 存储面板手动「压缩数据库」按钮 + 诚实显示死空间。
...
- **守卫**:插 N 行→删→断言 `freelist_count` 低于阈值 或 文件 size 下降 → **已落 `infra/db/vacuum_test.go`**(非台账写的 `db_test.go`——回收逻辑独立成 `vacuum.go` 一文件,测跟着走)。
- **产品决策(交你拍板,我未碰前端)**:
  - **(a) 迁移 VACUUM 的 boot 时机**——我选**同步在 `openDB`**(best-effort log+continue、绝不失败 boot),存量库首次升级 boot 一次性 +VACUUM 时长(实测 405ms@257MB,GB 级几秒)。这是**一次性**代价、且正是「升级即把你的磁盘还回来」的诚实动作,故我判同步可接受。若你更看重零 boot 延迟,可改**异步在 retention goroutine**(primed kick 首跑时迁移)——更贴台账「retention 不内联跑」的调性,但把迁移耦进 retention 逻辑、复杂度更高。**默认我按同步落了**,一句话即可翻。
  - **(b) 存储面板增强(纯前端,未做)**——当前修法覆盖**主场景**:retention 删 run 后自动回收(唯一会产生越闸死空间的用户动作;version-trim/`:replay` 是稳态 churn、被回收闸有意忽略=复用更省)。可选加值:①存储面板加**手动「压缩数据库」按钮**(用户主动触发一次 `ReclaimFreePages` 或全量 `VACUUM`,覆盖 retention 之外的场景);②**诚实显示当前库大小 + 死空间**(`page_count`/`freelist_count` 现成)。二者皆需后端加一个 `POST /storage:compact` 端点 + GET 一个 storage-stat——**要不要做、交你拍板**,不阻塞本条核心修复。
...
| 插行→删→断言 freelist/size 下降 | T4 | `infra/db/vacuum_test.go` ✅ 已落(0717,真落盘库 `os.Stat` 前后 + 回收闸 + 存量迁移三测) |
