...
| 3 | ❌ **未跑**(MCP 要一个真返图的 server;fn 要真机 matplotlib) | ❌ | **待真机** |
...
| 5 | ⏳ `TestLiveMedia_VideoDirect` 跑中(直连侧提交/轮询/取回三动词,受管侧已在 H3 于线上网关证过) | ❌ 工具卡进度/不冻结 | **待落** |
...
4. **一条不走运的模型选择能把花钱的生成工具循环到步数预算耗尽**(记档,未修)。`qwen3-vl-plus` 拿到
   ...
   receipt 加人话前缀、去掉 `Input data` 块——全都照常重调),而**换模型立刻就好**。系统里没有任何**逐运行的
   支出上限**拦这件事——归 H10。
