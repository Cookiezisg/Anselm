...
> **状态(2026-07-27 08:00,自主循环收口):代码与机械证据侧已完成;真钱真机侧待一把 key。**
>
...
> [ADR 0014](../../decisions/0014-mediaref-one-currency.md) / `concepts/architecture.md` §3.5b /
> CLAUDE.md 与各 reference,但**本页仍是晨间那份清单的唯一去处**:
>
> - **待你解锁的四件**(全是同一件事:一把能花钱的 key + 一次部署):终点验收 §0.2 的 ①③⑤ 与 ⑦ 的部署半。
>   怎么跑写在 [ACCEPTANCE-GUIDE](../app-hardening/ACCEPTANCE-GUIDE.md) 的 **B5**。
> - **两笔价格债**(卡 ID 里带 `assumed-`,代拍 B3/C2):见 B6。这是代码里唯一明说「这个数可能不对」的地方。
> - **一个建议你过目的代拍**(D1):V1 视频产物渲文件卡、不内联播放——整个战役里唯一一处「能力建成但用户
>   看不到成品」,内联播放要引入桌面视频栈的原生依赖,是一次独立的依赖决策。
> - **两处按形状划出 V1 的**:E5(handler 媒体产物需把逐调用输出目录穿过 stdio RPC)、批F 的 slash 命令与
>   拖入粘贴(是**插入**入口,需要编辑器内上传流)。
>
> 方向与全部参数由用户拍板(P1–P19,2026-07-26/27 四轮对话);施工期新拍板记在 §1.1,可随时翻案。
>
...
#### 逐条核对(2026-07-27 07:xx,自主循环收口前)

**诚实律:未跑就写未跑。** 下面每条只写**已有的证据**,不把「代码写完了」当成「验收过了」。

| # | 代码 | 真钱真线缆证据 | 结论 |
|---|---|---|---|
| 1 | ✅ 全链(网关钱层/端点 + 五方言 + 工具卡) | ❌ **未跑** | **待晨间**:本地无 DASHSCOPE key、SSH 上生产被会话权限闸拒(代拍 B2) |
| 2 | ✅ | ✅ **黑盒真跑通**——`testend/scenarios/workflow_media_test.go`:viewer 节点的模型请求里是原生 `image_url` + 生成图的 **base64 真字节**,附件字节往返逐字节相同 | **过**(mock 上游,但**线缆**是真的;真钱那半随①一起补) |
| 3 | MCP ✅ / fn ✅ | ❌ **未跑**(MCP 要一个真返图的 server;fn 要真机 matplotlib) | **待真机** |
| 4 | ✅ 端点/工具/朗读/缓存 | 缓存零计费 ✅ **黑盒证**(`SpeechInputs()` 次数不变 + 整场零 chat 请求);真声 ❌ **未跑** | **半过**:钱的那半已被机械证明,声音那半待 key |
| 5 | ✅ 两方言 × 三动词 | ❌ **未跑** | **待晨间**:需要 DashScope 或 Gemini 的真 key |
| 6 | ✅ 全链(AI 侧描述 + 值形 + 渲染 + @注入 + 三保真守卫) | codec 三保真 ✅ 机械证;@注入模型真看见 ❌ **未跑** | **半过** |
| 7 | 主仓 `make verify` ✅ · 网关 `make verify` ✅ · 文档 ✅ | 网关**未推送**、故无 `gh run` | **待推送** |

**总结:七条里 1 条完全过、2 条半过、4 条待真钱/真机。** 差的全部是**同一件事**——一把能花钱的 key,
外加一次网关部署。代码与机械证据这一侧已经收口。
