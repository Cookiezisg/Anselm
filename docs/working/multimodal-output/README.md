...
>   命令已在 H6 施工。
> - **⏳ 未拍板 / 未开工**:A3(重复调用与逐运行支出上限,§0.2 findings 第 4 条,我的建议是不改)·
>   H9(X→X 多模态编辑)· H10(直连侧支出可见性)。A2(`danger` fail-open)**用户已裁定不改**。
>
...
3. **朗读缓存曾在真供应商面前必然不命中**(随 #2 一并修好)。缓存键含**路由身份**,而受管行是**异步**落地的、
   ...
   一个:数上游调用次数。
4. **一条不走运的模型选择能把花钱的生成工具循环到步数预算耗尽**(记档,未修)。`qwen3-vl-plus` 拿到
   `generate_image` 的 receipt 后**必定再调一次**,一路撞满 10 步——一个节点烧掉十张真图。用一次 chat 调用
   重放同样的历史零成本复现,再跑**七组对照**排除了我们这侧的每一个嫌疑(补句号、明写「已成功勿重调」、
   receipt 加人话前缀、去掉 `Input data` 块——全都照常重调),而**换模型立刻就好**。
   **复查订正(2026-07-27):「系统里没有任何闸」这句话是错的,盘上已有三道。**
   ①步数——chat `MaxSteps=25`、agent invoke `InvokeMaxTurns=10`(调用级 `MaxTurns` 可覆盖)、subagent 按类型
   30/25/25(`limits.go:131`、`invoke.go:310-312`、`subagent/registry.go:56/64/72`);②墙钟——`ChatTurnSec`
   限一整回合总时长;③错误风暴——`loop.go:339-355` 连续 3 轮**每条** tool_result 都带 error 即熔断。
   **缺的那一块很具体**:三道全部只对**失败**或**耗尽**起作用,`consecAllFail` 一遇成功就清零——所以
   **成功地**把同一工具连调 25 次,除了撞步数顶什么都碰不到。
   成本上三个方向里两个本来有界:受管档视频 10 条/天/install 撞不穿;chat 里每次 `generate_video` 都过
   danger 闸(25 次 = 25 次弹框,吵但不静默)。**真正没界的只有两处**:`humanloop.go:174` 的
   `Allow(convID, tool)` 是**永久、不计次、不过期**的许可(点一次「总是允许」后剩下的付费调用无声跑完);
   以及 subagent/workflow 里 `humanloopapp.From(ctx)` 恒为 nil(`tools.go:180`)——**根本没有闸**,纯信任。
   **未拍板。** 我的建议是**也不改**:按用户对第 6 条的同一裁定,这两处是同一条设计的延伸(自己点的
   「总是允许」、自己开的 subagent)。唯一站得住的补丁是给「同一工具+同一 args 连续成功 N 次」加一道与
   错误风暴并排的熔断,但它会**恰好误伤付费场景**——同一 prompt 调两次 `generate_image` 求另一版正是合法
   用法。若日后要做逐运行支出上限,归 H10。
