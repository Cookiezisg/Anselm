...
| 轮 | 范围 | 方法 | 状态 |
|---|---|---|---|
| R1 | **配置与基础设施面全检** | 旋钮清单逐个验：设置口在哪（HTTP/tool/文件）→ 读取链是否真生效 → 当前值可见吗 → 默认值合理吗 → 文档一致吗。覆盖：model 场景解析/override、api keys、workspace settings（web_fetch_mode 等全部键）、search settings/embedder、agent 模型配置、handler init-config、mcp config、trigger config、skill allowed-tools、pkg/limits 用户可调上限、加密落盘、locale | ← 进行中 |
