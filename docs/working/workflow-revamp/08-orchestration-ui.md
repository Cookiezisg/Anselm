...
- **`notifications` SSE 的 `flowrun` 进度 tick**【CANON-X4】 → 进度 tick = **best-effort、可丢、绝不背压执行引擎**;它走 notifications 的一个**可丢 best-effort 子类**,与会阻塞的实体变更事件**隔离对待**,且**不进 notifications 的 replay buffer**(标 `delivery: ephemeral`:不分配 durable seq、**不参与 N7 的 `Last-Event-ID` 重放** —— 重连不回补 tick,改从 `get_flowrun_trace` 拉全量;durable 实体变更事件走 `delivery: durable`、Last-Event-ID 正常重放,二者隔离实现,C2)。后端限流/合并:**每节点状态变化或每 N ms 一条**。前端收到 tick → react-query invalidate `flowruns` / `flowrun(id)` / `flowrunNodes(id)`(详 `cross-cutting.md` invalidate 映射)→ 重拉节点状态。节点颜色直接读 `FlowRunNode.status`(`pending / running / ok / failed / cancelled / timeout / skipped`)。
