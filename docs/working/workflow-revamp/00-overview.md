...
- **版本无漂移(确定性天然干净)**:flowrun **启动时 pin 住图拓扑(`version_id`)+ 它要调的 callable 版本**(快照进 `flowruns.pinned_callables`),整个生命周期(含重放 / parked resume)用同一份 → 控制流与 callable 解析都确定,**绝无"半路换版本"**(= Temporal versioning,见 §3 永远 prod)。新 flowrun 才用新 active。
...
- **fan-in = 汇合节点 = `await 被激活的入边完成`,然后只继续一次**。等什么由 split 类型定:**AND-split(并行扇出)→ 全部入边都激活 → 等全部**(WP3 同步);**`case` 的 XOR-split → 只激活走的那条 → 汇合只等被激活的入边**(WP5 simple-merge / **active-branch join**)。case 对**未走的分支下发 skip 信号**,join 据此**不等死那条**(A-1)。这是结构化 fork-join,不是"消息到了再判断点不点火"——**既无并发双点火、也无空票死等**。
...
| **空票**(没走的分支怎么不让下游死等) | case 对未走分支**下发 skip**,汇合 join **只等被激活的入边**(active-branch / simple-merge,WP5)→ 不等死没来的分支。"没走的分支就是没执行" + "join 按激活分支等" 两条一起才闭合(A-1 修;别只靠前半句)。 |
| **并发双点火**(join 被两个输入各触发一次) | join = `await 被激活的入边`,后续**只跑一次**;运行时调度它一次。 |
...
超长循环(几千轮)会让日志变大、重放变慢 → **continue-as-new**(Temporal 标准):journal 事件数 / 大小**超平台默认上限(走 `pkg/limits`,高默认)→ 自动把作用域变量快照成新 flowrun 的 input、开新 journal、旧段归档**;replay 只在当前段 journal 内,UI 标"已滚动续期"。"N 塞进工具"的哲学让循环天然短(有限重试,非数据迭代),本地单用户上限设很高、基本不触(C-2)。

### 终止(implicit termination,WP11)

flowrun = `completed` 当**所有活跃路径都到达"无后继出边"的节点、且无 parked(approval / timer)在等**;任一路径的 activity 用尽 retry 仍失败 = `failed`;被 `cancel_flowrun` = `cancelled`(取消语义见 [`07`](./07-error-handling.md))。
...
### 3. 永远 prod
> 所有"X 引用 Y"的关系,**Y 永远是 active version**。无 version pinning,没有 `@v3`。(这是**编排 / 引用**语义;**在途执行**另说 —— 见下"执行时 pin"。)
...
- 跟 K8s deployment "所有 pod 用同一 image" 心智一致 —— 简单 > 灵活。
- **执行时 pin(= Temporal versioning,标准做法;A-5 改判)**:"永远 prod" 只管**引用 / 编排**(引用永远指 active、改了下次自动跟、图里无 `@v3`)。但 **一次 flowrun 在启动时解析并 pin 住它要调的 callable(`fn_`/`hd_`/`ag_`)版本 + 图拓扑(`version_id`),整个生命周期(含崩溃重放、parked approval resume)都用这份快照;只有新 flowrun 才用新 active。** 这杜绝了长跑 / 挂起 run 的**版本漂移** —— 在途执行**永不半路换版本**,确定性天然干净。(原设计"callable 调用时解析 active"会漂移,现采纳标准 pin —— 用户拍板:标准更好。)
