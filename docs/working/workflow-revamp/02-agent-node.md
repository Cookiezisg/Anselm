...
type: agent
config:
  agentRef: ag_xxx      # 必填 — 引用 agent entity(引用语义永远指 active;但在途执行 pin 版本快照含传递闭包,A-5/ADR-020,见 17 §1)
```

节点 config 一个必填字段 `agentRef`(极简);另可挂可选的节点级 timer gate `at?` / `after?`(任意非 trigger 节点通用,见 [`17`](./17-execution-contract.md) §7)。
