...
ALTER TABLE flowruns ADD COLUMN trigger_node_id TEXT;   -- 触发来源由 trigger 节点 kind 可知,无需 is_from_listener 列
```
...
trigger_firings  ( id, workflow_id, trigger_node_id,
                  payload,
                  dedup_key,                          -- 幂等键:cron=scheduled_at / webhook=req-hash / polling=source-event-id
                  scheduled_at, enqueued_at,
                  status,                             -- pending / claimed / started / skipped / superseded
                  flowrun_id, outcome )               -- 派发后回填
-- UNIQUE(workflow_id, trigger_node_id, dedup_key):重复材化(崩在 append→bump 之间 / catchup 重算)幂等去重 —— "不丢"且"不重"(A3)
...
func onFire(workflowID, nodeID, payload) {
    appendFiring(workflowID, nodeID, payload, dedupKey)  // 落库后即 durable;dedupKey 上 UNIQUE → 重复材化幂等(A3)
    bumpLastFiredAt(workflowID, nodeID)                  // last_fired_at 落库(给 catchup);崩在两步之间 → catchup 重算同一 firing,被 dedupKey 去重
}
...
解释器照图走(从 trigger 节点 / 重放时从头)
   ...
   ├ 走到并行分叉 → 同时开几条分支 goroutine,汇合按 split 类型 await(AND-split 扇出 → 全部;case 分支 → active-branch 只等被激活,详 [`17`](./17-execution-contract.md) §3)
...
| 消息 version / 前沿 / 空票 配对 | 删除——循环用作用域变量、没走的分支不执行、join 按 split 类型(AND=全部 / case=active-branch,17 §3)只跑一次 |
...
| **fork-join 并行**:节点多条出边并发跑、汇合按 split 类型 await(AND=全部 / case=active-branch,17 §3)只继续一次(无双点火) | ✅ 并发模型通 |
