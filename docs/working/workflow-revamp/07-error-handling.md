...
- **零重复**:失败步的兄弟并行分支(同一 join 的其他入边)早已把结果写进事件日志 / 程序作用域,replay 重放时直接抄,**不重跑也不会拿到两份**。循环外算的值同理是作用域变量,重放白拿。**agent 节点的零重复是子步级**:它内部每个 tool-call 各记一笔账,崩在中途重放只补没记账的那个 tool-call、已发生的工具副作用不重放(详 [`02-agent-node.md`](./02-agent-node.md) 重放粒度段)。
