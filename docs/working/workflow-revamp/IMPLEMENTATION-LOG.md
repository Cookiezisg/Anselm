...
**Revamp status:** M0–M6 ✅, loop ✅, approvals **end-to-end ✅**, agent node functional ✅, **M8 trace/observability ✅**. Remaining, each with a concrete structural reason it's a focused fresh-budget piece — NOT exhausted-context work (§S20):

- **M8 live tick emission** — the per-node runtime tick (08【CANON-X4】: best-effort, ephemeral, *never backpressures the engine*). **Hard prerequisite discovered this session:** the current notifications `bridge.Publish` (infra/notifications/bridge.go:63) *assigns a durable seq, buffers for replay (1024-ring), AND blocks on a slow subscriber*. So per-node ticks CANNOT reuse it — high-frequency ticks would (a) evict real entity-change events from the replay ring (→ spurious `ErrSeqTooOld`) and (b) **backpressure the execution engine** on a slow UI (a stalled browser could stall a workflow). The tick therefore requires a NEW ephemeral delivery path in the bridge first (no seq / no buffer / non-blocking drop-on-full, the 08 C2 `delivery: ephemeral` class) — a notifications-contract change (E2) — then interpreter integration (thread the ephemeral emitter onto node_started/node_completed, ideally via a journal-seam decorator so the core walk stays untouched) + frontend `useFlowrunTicker`. The trace API shipped this session is the reconnect/full-pull half this tick refines; trace had to exist first.
- **M7 sub-step replay** (ADR-010 optimization) — threads the flowrun journal into `loop.Run` for per-step copy-hit; modifies the working, pipeline-tested agent ReAct loop (a regression there outweighs the missing optimization).
- **M5/M6 refinements** — cron-tick catchup determinism + polling cursor; both are for trigger re-materialization paths not yet live (building them now is speculative, §S20).

The "forge SSE 6-kind" milestone phrasing is retired — 08【CANON-X4】 is the truth: no new durable SSE; observability is the trace API (✅ shipped) + a future ephemeral tick path on a non-blocking delivery class.

### 2026-06-01 — M8 live tick emission DONE (the prerequisite ephemeral path + interpreter integration, built per user override)

The user overrode the "fresh-session" deferral — building all remaining pieces now. M8 live tick is complete end-to-end:

- **Ephemeral delivery class (the prerequisite).** `Bridge.PublishEphemeral` (+ on the domain `notificationsdomain.Bridge` + the `notificationspkg.Publisher` helper, best-effort log-only): fans out to live subscribers WITHOUT a seq, WITHOUT the replay buffer, and NEVER blocking (`select { case ch<-env: default: }` drops on full). Envelope `Seq=0` marks it; the notifications SSE handler now omits the `id:` line for `Seq==0` so an ephemeral tick never moves the client's Last-Event-ID (durable events keep `id: <seq>` + replay). This is the 08 C2 `delivery: ephemeral` class — it satisfies the hard constraint that killed reuse of `Publish` (no replay-ring eviction, no engine backpressure).
- **Interpreter integration (core walk untouched in spirit).** `Interpreter.WithTick(fn)` + a nil-safe `emitTick`; `activityRun` fires `running` after journaling node_started, `ok` after node_completed (and on a replay copy-hit), `failed` on dispatch error. The callback is the only addition to the walk — best-effort, non-blocking, no control-flow change.
- **Service wiring.** `executeRun` wires `.WithTick(...)` to `s.notif.PublishEphemeral(detachedCtx, "flowrun", run.ID, {action:"tick", workflowId, nodeId, status, iterationKey})` on a detached owner-scoped ctx, so ticks route even as the run ctx nears cancellation.
- **Frontend consumes it for free + one fix.** The existing `useNotifications` `flowrun` handler already invalidates flowruns/flowrun(id)/flowrunNodes(id) → the canvas refetches node states live on each tick. The one needed change: a `flowrun` notification with `data.action==="tick"` invalidates but does NOT bump the unread badge (ephemeral ticks are not user-facing). So no `useFlowrunTicker` is required for V1 — the invalidate path is the intended mechanism (a nodeId→status diff without full refetch is a later optimization, not a gap).

TDD: bridge ephemeral (live-only / Seq0 / no-buffer / no-seq-consumed) · interpreter tick sequence (running→ok per activity; running→failed on error) · HTTP E2E (subscribe→trigger→assert ephemeral `flowrun` tick, Seq0) · frontend (tick invalidates but no unread; lifecycle notification still bumps unread). New cross seam `scheduler_notifications:runtime_tick` registered + `make matrix` counts it. Docs: events.md §11.2a (ephemeral delivery class) + flowrun domain + cross-cutting.

**Revamp status:** M0–M6 ✅, loop ✅, approvals end-to-end ✅, agent node functional ✅, **M8 trace ✅ + live tick ✅**. Remaining: **M7 sub-step replay** (ADR-010) and **M5/M6 refinements** (cron-tick determinism + polling cursor) — building next, per the user override.

### 2026-06-01 — M7 sub-step replay DONE (ADR-010, via replay-via-history — not a loop copy-hit rewrite)

The agent node already ran as a journaled, crash-recoverable activity (M6 `:replay` re-runs it). M7 adds **sub-step** granularity so a replay of a crashed agent node doesn't re-run the LLM turns + tool calls it already completed. **Chosen mechanism: replay-via-history + a per-step record hook — deliberately NOT a rewrite of the shared `loop.Run` copy-hit logic** (that loop is shared with chat; restructuring its body for copy-hit was the high-risk path I'd flagged).

- **`loop.Run` change is one optional hook.** New `StepRecorder` interface (`RecordStep(ctx, step, assistant, toolResults)`), type-asserted — chat's host doesn't implement it, so the loop is byte-for-byte unchanged for chat. After a tool-step fully completes (tools ran + history extended), Run calls `RecordStep`. No control-flow change.
- **Record (live).** The interpreter hands every activity dispatch an `AgentSubStepJournal` (`DispatchInput.AgentSubSteps`) scoped to (flowrunID, nodeID, iteration, generation); only the agent dispatcher uses it. `agentHost.RecordStep` → journal `agent_step_completed` (`Turn`=absolute step, `Result`=RecordedStep{assistant, toolResults}).
- **Replay (reconstruct, don't re-dispatch).** On `:replay`, `agentHost.LoadHistory` prepends the prior run's completed steps — reconstructed into `[prompt, asst0, results0, …]` via the existing `loop.BlocksToAssistantLLM` — so the loop resumes past them; their tool side-effects are NOT re-run (they're history messages, never re-dispatched). `LoadSteps` takes the highest prior-generation per turn (ADR-019), so a replay-of-a-replay still sees every earlier copy-hit step. The turn budget (`maxTurns`) is reduced by replayed steps so total LLM turns across runs stay bounded. A step in-flight at crash (not journaled) re-runs wholesale — at-least-once; workflow tools must be idempotent (the standard durable contract).

TDD: loop StepRecorder hook fires once per tool-step (not the final text step) · agentSubSteps record→journal→load round-trip across generations (JSON round-trips chatdomain.Block) · merge-by-turn-highest-gen for replay-of-replay · agentHost.LoadHistory prepends reconstructed steps · HTTP E2E (fake-LLM agent node → trace shows `agent_step_completed`, proving interpreter→dispatcher→host→recorder→journal). New cross seam `workflow_scheduler:agent_substep_replay` + `make matrix`. Docs: 17 §4 (corrected the hypothetical `loop.Run(replayed []AgentStep)` to the actual mechanism) + 02-agent-node §implementation note.

**Revamp status:** M0–M6 ✅, loop ✅, approvals end-to-end ✅, agent node ✅, **M7 sub-step replay ✅**, **M8 trace ✅ + live tick ✅**. Remaining: **M5/M6 refinements** (cron-tick catchup determinism + polling cursor) — the last piece, building next.

### 2026-06-01 — M5/M6 refinement: cron-tick catchup determinism DONE (polling cursor + overlap are future-feature, not refinements)

The last item. **Cron-tick determinism — built.** The trigger firing's `dedup_key` was `wf|node|wall-clock-nanos` for ALL kinds, so a cron missed-tick catch-up that re-materializes an already-fired tick wouldn't dedup against it (different wall-clock → distinct key → the workflow runs twice for one scheduled tick). Fixed: each listener now supplies its natural idempotency key via `OnFireFunc(…, dedupKey string)` — **cron keys on the SCHEDULED TICK** (`cronDedupKey` = `wf|node|cron|<tick.Truncate(minute).Unix()>`; live fire uses `now`, catch-up uses the missed `next`, both truncate to the minute = ParseStandard's resolution → a live fire and a catch-up of the same tick collide). fsnotify/webhook pass `""` (each event/request is a genuinely distinct fire → the app falls back to the per-event wall-clock key). The firing inbox already enforces `UNIQUE(workflow_id, trigger_node_id, dedup_key)` + idempotent `AppendFiring` (returns the existing row on conflict), so the tick-keyed cron firing now dedups the double-run. TDD: `cronDedupKey` deterministic per minute-tick, distinct per tick/node/workflow. All trigger listener + app tests green; staticcheck clean.

**Polling cursor + overlap-beyond-serial are NOT refinements of existing code — they're future features (§S20).** There is **no polling listener**: the live trigger listeners are cron / fsnotify / webhook only (`triggerService` has no `s.polling`). The `polling_states` table + store exist as a persistence stopgap, but nothing fires polling triggers. A "polling cursor" presupposes a polling listener that doesn't exist — building it now is speculative. Likewise overlap policy beyond `serial` is an enhancement on a working serial baseline. Both are deferred as genuine new-feature work (they'd start with the polling listener itself), not as completion gaps of the durable-execution revamp.

**Revamp status — COMPLETE:** M0–M8 ✅, loop ✅, approvals end-to-end ✅, agent node + sub-step replay ✅, trace + live tick ✅, cron-tick determinism ✅. The durable execution engine — durable trigger inbox → single-tx claim → interpreter → (linear / case / fork-join / active-branch / loop / approval / agent with sub-step replay) → replay/failures/drain → trace + live tick → frontend approvals + canvas — is functionally complete, crash-recoverable end-to-end, observable, and review-verified (2 rounds) + TDD + full `-race` pipeline. The only deferred items are genuine NEW features (polling listener, overlap policies), not gaps in the revamp.

### 2026-06-01 — Production-grade delivery: durable timer, cron persistence, frontend palette + trace + status fixes

**Post-review complete delivery.** After the self-audit exposed real gaps (approval timeout never fired, cron lastFiredAt lost on restart, frontend using wrong status strings, palette still 14 kinds), delivered all missing pieces:

**Backend:**
- **Approval timeout (durable timer, M4 completion)**: `Approval.Deadline` + `TimeoutBehavior` fields; interpreter sets deadline on park when `config.timeoutSec` is set; `expiry.go` goroutine scans expired approvals every 30s, journals `signal_received(source=timeout)`, flips projection row to `timed_out`, then re-drives interpreter via ClaimStatus→loadFrozenGraph→spawnRun (same path as ResumeApproval). `ListExpired` store method. Wired via `schedulerService.StartExpiryChecker(ctx)` in main.go.
- **Cron lastFire persistence**: `ScheduleStore` port added to trigger.Service; trigger store gains `UpsertSchedule`/`GetSchedule`/`UpdateLastFiredAt`; `cron.Listener.RegisterWithLastFire` seeds in-memory `lastFire` from DB row on boot (cross-restart missed-tick catch-up now works); `onFire` callback persists `lastFiredAt` best-effort after each fire; domain `Spec` gains `LastFiredAt` field; wired `triggerService.SetScheduleStore(triggerStore)` in main.go.
- **timeout_dryrun_test.go rewrite**: Tests now go through the new interpreter (`New(journal, router).WithDryRun(true).Run(...)`) instead of the old `runReadyLoop`/`dispatchWithPolicies`. Covers: DeadlineExceeded classification, cancel→Canceled, dry-run skip side-effect/preserve-pure-logic/approval-auto-approve, `nodeTimeoutDuration`, and `checkExpiredApprovals` integration.

**Frontend:**
- **`qk.approvals()` in flowrun SSE invalidation**: `useNotifications` flowrun type now invalidates `qk.approvals()` → approval banner auto-refreshes when a run parks at an approval node (was silently never updating).
- **`awaiting_signal` status**: Backend's canonical durable status was `awaiting_signal`; frontend was filtering/coloring on the legacy `waiting_approval` alias only. Fixed in `FlowRunDetail.StatusBadge`/`STATUS_KIND`, `ExecuteOverview` waiting filter, `ProgressMini` color.
- **i18n hardcoded strings**: `"ok"`, `"fail"`, `"skip"` in `FlowRunDetail.tsx` replaced with `t("detail.nodeStatus.ok/fail/skip")`; keys added to zh/en `execute.json`; inspector tab/label/traceEmpty/traceIter keys added too.
- **Palette 14→5**: `WorkflowEditor.NODE_KINDS` collapsed to 5 entries (trigger/agent/tool/case/approval) per 08-orchestration-ui design. `backendKind` mapping (tool→function, case→condition) ensures existing workflows still render; palette button passes `backendKind` to the canvas.
- **Trace inspector**: `useFlowRunTrace` hook (GET `/flowruns/{id}/trace?nodeId=X`); `NodeInspectorBody` gains I/O|Journal tab pair — Journal tab shows per-node journal entries from the durable truth (seq, type, iterationKey, result). `TraceEntry` type exported from entities barrel.

**Gates**: `make unit` (200 pkgs, 0 fail), `make mock` (32 pkgs, 0 fail), `make web` (998 tests, 0 fail), `tsc --noEmit` (0 errors), steiger (0 FSD violations).

**Remaining genuine NEW features** (not revamp gaps): polling listener (no listener exists, `polling_states` is a stopgap), overlap policy beyond `serial`, `useFlowrunTicker` visual state machine + per-node canvas animation (the backend tick and trace API are both live; the frontend hook that maps notification+eventlog ticks to node visual states is not yet built — current behavior is coarse query-invalidation which refreshes the node list, not per-node animation).
